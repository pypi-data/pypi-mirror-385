...
def get_houses_and_angles(jd_now, lat, lng, offset=None):
    ...
    cusps, asc_mc = swe.houses_ex(jd_now, lat, lng, b"W", calc_flag)
    ...
    for cusp in range(12):
        house = f"H{cusp + 1}"
        cusp_keys.append(house)

    for cusp, obj_key in zip(cusps, cusp_keys):
        dms = swe.split_deg(cusp, 8)
        sign_name, sign_data = sign_from_index(dms[4])
        houses.append(
            {
                "obj_key": obj_key,
