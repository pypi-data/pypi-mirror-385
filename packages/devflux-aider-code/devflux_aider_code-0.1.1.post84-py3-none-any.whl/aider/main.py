...
from aider.i18n import I18n
from aider import mcpm

import locale

"""
DevFlux AIDER: DevFlux AIDER CLI Tool
Version: 0.1.1
History:
    0.1.1 (2025-08-30):
        - Core: Update 중지, install_from_main_branch, install_upgrade 
"""
...
def launch_gui(args):
    ...
    # Necessarily so streamlit does not prompt the user for an email address.
...
def load_oauth_key_from_file(model):
    ...
    try:
        with open(oauth_keys_file) as f:
            ...
            # print(f"Loaded {env_var} from {oauth_keys_file} as {os.environ[env_var]}")
...
def main(argv=None, input=None, output=None, force_git_root=None, return_coder=False):
    ...
    conf_fnames = [Path(".aider.conf.yml"), Path(".aider.conf.yaml")]
    ...
    if git_root:
        for conf_fname in conf_fnames:
            git_conf = Path(git_root) / ".aider" / conf_fname  # git root
    ...
    if not args.verify_ssl:
        ...
        litellm.lazy_module.client_session = httpx.Client(verify=False)
        litellm.lazy_module.aclient_session = httpx.AsyncClient(verify=False)
    ...
    def get_io(pretty, param_i18n=None):
        return InputOutput(
            pretty,
            args.yes_always,
    ...
    lang = args.chat_language or locale.getlocale()[0]
    i18n = I18n(lang)

    io = get_io(args.pretty, i18n)
    try:
        io.rule()
    ...
    for fn in args.read_rule_files or []:
        path = Path(os.path.join(".aider", "rules", fn)).expanduser().resolve()
        if path.is_dir():
            read_only_fnames.extend(str(f) for f in path.rglob("*") if f.is_file())
        else:
            read_only_fnames.append(str(path))
    ...
    if args.install_main_branch:
        success = 0  # install_from_main_branch(io)
        analytics.event("exit", reason="Installed main branch")
        return 0 if success else 1

    if args.upgrade:
        success = 0  # install_upgrade(io)
    ...
    load_oauth_key_from_file(main_model)
    ...
    # Configure MCPM manager
    mcpm.configure(args, i18n)
