...
"""
    ...
    def _to_cmd_mcp_server(self, servers):
        console = Console()

        if self.output_format == "markdown":
            # 마크다운 특수문자 이스케이프 (특히 파이프)
            ...
            # 헤더
            md_lines = [
                "| Name | Status | Command | Environment Variables |",
                "| --- | :---: | --- | --- |",
            ]
            ...
            for server in servers:
                ...
                if env:
                    # 테이블 셀 내 줄바꿈은 <br>로 표현
                    env_lines = []
                    for var, value in env.items():
                        masked = mask_value(value)
                        env_lines.append(f"**{md_escape(var)}**={md_escape(masked)}")
                    env_str = "<br>".join(env_lines)
                else:
                    env_str = "-"

                md_lines.append(
                    f"| {md_escape(getattr(server, 'name', ''))} | {md_escape(server_status)} | {md_escape(command_str)} | {env_str} |"
                )

            console.print(Markdown("\n".join(md_lines)))
            console.file.flush()  # 내부 버퍼 강제 비우기
        else:
            table = Table(
            ...
            for server in servers:
                # 상태 표시 (이모지와 색상으로 구분)
                if server.enabled:
                    server_status = "[green]✓ Enabled[/]"
                else:
                    server_status = "[red]✗ Disabled[/]"
                ...
                # 테이블에 행 추가
                table.add_row(server.name, server_status, command, env_vars)
    ...
    def cmd_mcp_server(self, args):
        ...
        if servers:
            self._to_cmd_mcp_server(servers)
        else:
            self.io.tool_output("No MCP(Model Context Protocol) servers configured.")

    def _to_cmd_mcp_server_tool(self, tool_server_name, tools_by_server):
        console = Console()
        if self.output_format == "markdown":
            for server_name, tools in tools_by_server.items():
                ...
                if not tools:
                    md_lines.append("| - | - | No tools available |")
                else:
                    for tool in tools:
                        description = (
                            tool.description
                        ...
                        name_md = str(getattr(tool, "name", "-")).replace("|", "\\|")
                        perm_val = (
                            getattr(tool, "permission", None)
                            if hasattr(tool, "permission")
                            else None
                        )
                        perm_md = f"**{perm_val}**" if perm_val else "-"
                        desc_md = str(description).replace("|", "\\|")
                        md_lines.append(f"| {name_md} | {perm_md} | {desc_md} |")
    ...
    def cmd_mcp_server_tool(self, args):
        ...
        if not tools_by_server:
            ...
            self._to_cmd_mcp_server_tool(tool_server_name, tools_by_server)
    ...
    def cmd_mcp_server_tool_execute(self, args):
        ...
        try:
            result = execute_tool(server_name, tool_name, arguments, self.io)
            ...
            try:
                with open(oauth_keys_file) as json_file:
                    ...
                    if provider and provider in json_config:
                        json_keys = json_config["providers"][provider].get(
