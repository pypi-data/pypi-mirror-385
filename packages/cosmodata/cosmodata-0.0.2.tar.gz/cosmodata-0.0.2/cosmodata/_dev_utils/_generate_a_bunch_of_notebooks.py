...
DFLT_DATASETS = [
    ...
    - **Label:** `nameWithOwner`""",
        ...
        "output_filename": "github_repositories.ipynb",
    },
    {
    ...
    - **Label:** `title`""",
        ...
        "output_filename": "hcp_publications.ipynb",
    },
    {
    ...
    - **Label:** `text`""",
        "related_code": None,
        "output_filename": "harris_trump_debate.ipynb",
    },
    {
    ...
    - **Label:** `track_name`""",
        "related_code": None,
        "output_filename": "spotify_playlists.ipynb",
    },
    {
    ...
    - **Label:** `content`""",
        ...
        "output_filename": "lmsys_chat_conversations.ipynb",
    },
    {
    ...
    - **Label:** `text`""",
        ...
        "output_filename": "prompt_injections.ipynb",
    },
    {
...
def _resolve_output_dir(output_dir=None):
    ...
    while not path.is_dir():
        candidate = input(f"Directory '{path}' does not exist. Please enter a valid directory: ").strip()
        if not candidate:
            continue
        path = Path(candidate).expanduser()
    return path
...
def generate_all_notebooks(datasets: dict = DFLT_DATASETS, output_dir=None):
    """Generate notebooks for all datasets."""
    resolved_output_dir = _resolve_output_dir(output_dir)
    for dataset_config in datasets:
        output_filename = dataset_config["output_filename"]
        output_path = resolved_output_dir / output_filename
        params_kwargs = {k: v for k, v in dataset_config.items() if k != "output_filename"}
