...
def set_icon(key_name="VSCode"):
    ...
    # icon 路徑
    vs_root = Path(os.getenv("vs_exe",r"C:\Users\moon-\AppData\Local\Programs\Python38\vscode_win\Code.exe")).parent
    icon_path = os.path.join(sys.exec_prefix, "vscode_win", "VS.ico")
    # 註冊表路徑
    # reg_icon = rf"SOFTWARE\Classes\Folder\shell\{key_name}\Icon"
    # 這裡只指到 key，不加 "Icon"
    reg_icon = rf"SOFTWARE\Classes\Folder\shell\{key_name}"
...
# 寫入註冊表
# -----------------------------
def set_vscode_registry(key_name="VSCode"):
    import os,sys,winreg
    # -----------------------------
    # 配置 VSCode 路徑
    # -----------------------------
    ...
    vs_root = Path(os.getenv("vs_exe",r"C:\Users\moon-\AppData\Local\Programs\Python38\vscode_win\Code.exe")).parent
    # 動態生成路徑 
    vscode_exe = os.path.join( vs_root, "Code.exe")
    user_data_dir = os.path.join( vs_root, "settings","user_data_dir")
    extensions_dir = os.path.join( vs_root, "settings","extensions_dir")
    # reg_key = rf'HKCR\Folder\shell\{key_name}'
    # reg_key = r'HKLM\SOFTWARE\Classes\Folder\shell\VSCode'   
    # reg_key = f'HKLM\\SOFTWARE\\Classes\\Folder\\shell\\{key_name}' 
    ...
    key_path = f"SOFTWARE\\Classes\\Folder\\shell\\{key_name}\\command"
    try:
        ## 建立
...
###########################
set_icon("VSCode")            ## ADD !!
set_vscode_registry("VSCode") ## ADD !!
