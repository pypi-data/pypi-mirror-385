...
Package ``ut_flt`` can be installed from PyPI.

To install with ``pip``:

.. code-block:: shell

	$ python -m pip install ut_flt
...
The Files of Package ``ka_uts_log`` could be classified into the follwing file types (c.f.: **Appendix**: `Python Glossary`):
