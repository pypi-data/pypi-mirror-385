...
def process_page(txt_path):
    ...
    out_json = os.path.join(pdf_dir,manifest_path)
...
def process_patent_dir(patent_dir: Path):
    ...
    # Pool with per-worker environment setup
    with mp.get_context("spawn").Pool(
        processes=N_PROCESSES, initializer=_init_worker
    ) as pool:
        for res in tqdm(
            pool.imap_unordered(process_page, txt_files),
            total=len(txt_files),
            desc=patent_dir.name,
        ):
            if res:
                results.append(res)
