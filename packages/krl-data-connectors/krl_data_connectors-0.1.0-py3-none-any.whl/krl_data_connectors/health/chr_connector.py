...
class CountyHealthRankingsConnector(BaseConnector):
    ...
    **Ranking Scale:**
    ...
    def get_health_outcomes(
        self,
        data: pd.DataFrame,
        rank_column: str = 'health_outcomes_rank'
    ) -> pd.DataFrame:
        """
    ...
    def get_health_factors(
        self,
        data: pd.DataFrame,
        rank_column: str = 'health_factors_rank'
    ) -> pd.DataFrame:
        """
    ...
    def get_available_measures(self, data: pd.DataFrame) -> Dict[str, List[str]]:
        ...
        self.logger.info(
            f"Found {sum(len(v) for v in measures.values())} measure columns"
        )
    ...
    def summarize_by_state(
        ...
        if measures is None:
            ...
            measures = []
            for measure in ['adult_smoking', 'adult_obesity', 'uninsured', 
                          'unemployment', 'high_school_graduation']:
