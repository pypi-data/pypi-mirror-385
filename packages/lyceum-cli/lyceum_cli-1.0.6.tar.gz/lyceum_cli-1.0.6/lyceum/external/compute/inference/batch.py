...
def upload_file(
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            response = httpx.post(
                f"{config.base_url}/api/v2/external/compute/inference/files",
...
def create_batch(
    ...
    try:
        ...
        response = httpx.post(
            f"{config.base_url}/api/v2/external/compute/inference/batches",
            headers={"Authorization": f"Bearer {config.api_key}"},
            json=request_data,
            timeout=30.0
        )
...
def get_batch(
    ...
    try:
        ...
        response = httpx.get(
            f"{config.base_url}/api/v2/external/compute/inference/batches/{batch_id}",
            headers={"Authorization": f"Bearer {config.api_key}"},
            timeout=30.0
        )
...
def cancel_batch(
    ...
    try:
        ...
        response = httpx.post(
            f"{config.base_url}/api/v2/external/compute/inference/batches/{batch_id}/cancel",
            headers={"Authorization": f"Bearer {config.api_key}"},
            timeout=30.0
        )
...
def download_file(
    ...
    try:
        ...
        response = httpx.get(
            f"{config.base_url}/api/v2/external/compute/inference/files/{file_id}/content",
            headers={"Authorization": f"Bearer {config.api_key}"},
            timeout=60.0
        )
