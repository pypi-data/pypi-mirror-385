...
def login_command(
    ...
    if manual:
        ...
        try:
            response = httpx.get(
                f"{config.base_url}/api/v2/external/user/status",
...
def status_command():
    ...
    if config.api_key:
        ...
        # Test connection and get recent executions
        try:
            ...
            if executions_response.status_code == 200:
                ...
                if executions:
                    console.print(f"\n[cyan]🔄 Active Executions ({len(executions)}):[/cyan]")
                    ...
                    for execution in executions[:10]:  # Show max 10
                        status = execution.get('status', 'unknown')
                        status_color = "yellow" if status == "pending" else "green" if status == "running" else "red"

                        table.add_row(
                            execution.get('execution_id', '')[:16] + "...",
                            f"[{status_color}]{status}[/{status_color}]",
                            execution.get('execution_type', 'N/A'),
                            execution.get('hardware_profile', 'N/A'),
                            execution.get('created_at', '')[:19]
                        )

                    console.print(table)
                else:
                    console.print("\n[dim]No active executions[/dim]")
