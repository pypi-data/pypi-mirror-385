...
class DefaultRequirements(SuiteRequirements):
    ...
    def non_updating_cascade(self):
        ...
        return fails_on_everything_except("sqlite", "oracle") + skip_if("mssql")
    ...
    def foreign_key_constraint_option_reflection_ondelete(self):
        return only_on(["postgresql", "mysql", "mariadb", "sqlite", "oracle", "mssql"])
    ...
    def tuple_in(self):
        ...
        return only_on(["mysql", "mariadb", "postgresql", _sqlite_tuple_in, "oracle"])
    ...
    def skip_autocommit_rollback(self):
        return exclusions.skip_if(
            ["mssql+pymssql"],
            "DBAPI has no means of testing the autocommit status of a " "connection",
        )
    ...
    def correlated_outer_joins(self):
        ...
        return skip_if(
            "oracle",
            'Raises "ORA-01799: a column may not be ' 'outer-joined to a subquery"',
        )
    ...
    def temporary_views(self):
        ...
        return only_on(["sqlite", "postgresql"]) + skip_if(self._sqlite_file_db)
    ...
    def mod_operator_as_percent_sign(self):
        ...
        return only_if(["mysql", "mariadb", "sqlite", "postgresql+psycopg2", "mssql"])
    ...
    def empty_strings_varchar(self):
        ...
        return fails_if(["oracle"], "oracle converts empty strings to a blank space")
    ...
    def symbol_names_w_double_quote(self):
        ...
        return skip_if([no_support("oracle", "ORA-03001: unimplemented feature")])
    ...
    def database_discards_null_for_autoincrement(self):
        ...
        """
        return succeeds_if(
            lambda config: (config.db.dialect.insert_null_pk_still_autoincrements)
        )
    ...
    def nullsordering(self):
        ...
        return fails_on_everything_except("postgresql", "oracle", "sqlite >= 3.30.0")

    @property
    def reflects_pk_names(self):
        """Target driver reflects the name of primary key constraints."""

        return fails_on_everything_except("postgresql", "oracle", "mssql", "sqlite")
    ...
    def tuple_valued_builtin_functions(self):
        return only_on(
            lambda config: self._sqlite_json(config) or against(config, "postgresql")
        )
        ...
        return only_on(
            [
                ...
                and (
                    ...
                    or (
                        config.db.dialect._mariadb_normalized_version_info >= (10, 2, 7)
                    )
                ),
                "mariadb>=10.2.7",
                "postgresql >= 9.3",
    ...
    def legacy_unconditional_json_extract(self):
        ...
        """
        return self.json_type + only_on(["postgresql", "mysql", "mariadb", "sqlite"])
    ...
    def _sqlite_memory_db(self, config):
        return against(config, "sqlite") and not config.db.dialect._is_url_file_db(
            config.db.url
        )
        ...
        if not against(config, "sqlite"):
            ...
            with config.db.connect() as conn:
                ...
                try:
                    ...
                    return (
                        cursor.description is not None and len(cursor.description) >= 3
                    )
                finally:
                    cursor.close()
    ...
    def cast_precision_numerics_many_significant_digits(self):
        ...
        """
        return self.precision_numerics_many_significant_digits + fails_if("mysql")
    ...
    def infinity_floats(self):
        return fails_on_everything_except(
            ...
            "postgresql+asyncpg",
            "postgresql+psycopg",
        ) + skip_if("postgresql+pg8000", "seems to work on pg14 only, not earlier?")
    ...
    def postgresql_working_nullable_domains(self):
        ...
        return skip_if(
            lambda config: (17, 0) < config.db.dialect.server_version_info < (17, 3),
    ...
    def skip_mysql_on_windows(self):
        ...
        return skip_if(self._has_mysql_on_windows, "Not supported on MySQL + Windows")
    ...
    def pyodbc_fast_executemany(self):
        ...
        return only_if(has_fastexecutemany, "only on pyodbc > 4.0.19 w/ msodbc driver")
    ...
    def mysql_zero_date(self):
        def check(config):
            ...
            with config.db.connect() as conn:
                row = conn.exec_driver_sql("show variables like 'sql_mode'").first()
    ...
    def _mysql_and_check_constraints_exist(self, config):
        ...
        if exclusions.against(config, ["mysql", "mariadb"]):
            if config.db.dialect._is_mariadb:
                norm_version_info = config.db.dialect._mariadb_normalized_version_info
    ...
    def _mysql_not_mariadb_103_not_mysql8031(self, config):
        return (against(config, ["mysql", "mariadb"])) and (
            (
                config.db.dialect._is_mariadb
                and config.db.dialect._mariadb_normalized_version_info < (10, 3)
            )
            or (
                not config.db.dialect._is_mariadb
    ...
    def _mysql_not_mariadb_104_not_mysql8031(self, config):
        return (against(config, ["mysql", "mariadb"])) and (
            (
                config.db.dialect._is_mariadb
                and config.db.dialect._mariadb_normalized_version_info < (10, 4)
            )
            or (
                not config.db.dialect._is_mariadb
                and config.db.dialect.server_version_info < (8, 0, 31)
    ...
    def _has_mysql_fully_case_sensitive(self, config):
        with config.db.connect() as conn:
            return (
                against(config, "mysql") and config.db.dialect._detect_casing(conn) == 0
            )
    ...
    def fetch_first(self):
        return only_on(["postgresql", "mssql >= 11", "oracle >= 12", "mariadb >= 10.6"])
...
class PyroMySQLRequirements(DefaultRequirements):
    """
