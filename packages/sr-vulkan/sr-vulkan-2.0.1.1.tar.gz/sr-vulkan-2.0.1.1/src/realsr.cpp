...
RealSR::RealSR(int gpuid, bool _tta_mode, int num_threads, const char* name)
{
...
int RealSR::process(const ncnn::Mat& inimage, ncnn::Mat& outimage, int tilesize2) const
{
    if (!vkdev)
    {
        // cpu only
...
int RealSR::process_cpu(const ncnn::Mat& inimage, ncnn::Mat& outimage, int tilesize2) const
{
    const unsigned char* pixeldata = (const unsigned char*)inimage.data;
    const int w = inimage.w;
    const int h = inimage.h;
