...
RealCUGAN::RealCUGAN(int gpuid, bool _tta_mode, int num_threads, const char* name)
{
...
int RealCUGAN::process(const ncnn::Mat& inimage, ncnn::Mat& outimage, int tileSize) const
{
    ...
    if (!vkdev)
    {
        ...
        if (syncgap_needed && syncgap)
        {
            ...
            return process_cpu(inimage, outimage, tileSize);
    }
...
int RealCUGAN::process_cpu(const ncnn::Mat& inimage, ncnn::Mat& outimage, int tileSize) const
{
    if (noise == -1 && scale == 1)
    {
        outimage = inimage;
...
int RealCUGAN::process_se_sync_gap(const ncnn::Mat& inimage, const std::vector<std::string>& names, const ncnn::Option& opt, FeatureCache& cache) const
{
    ...
    for (size_t i = 0; i < names.size(); i++)
    {
        for (int j = 0; j < tiles; j++)
        {
            if ((opt.use_fp16_storage || opt.use_fp16_packed) && ncnn::cpu_support_arm_asimdhp() && feats_cpu[i][j].elembits() == 16)
            {
                ncnn::Mat feat_fp32;
