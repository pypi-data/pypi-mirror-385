...
RealESRGAN::RealESRGAN(int gpuid, bool _tta_mode, int num_threads, const char* name)
{
...
#endif
{
    ...
    // initialize preprocess and postprocess pipeline
    {
        ...
        if (tta_mode)
        {
            if ((net.opt.use_fp16_storage || net.opt.use_fp16_packed) && net.opt.use_int8_storage)
            ...
            else if ((net.opt.use_fp16_storage || net.opt.use_fp16_packed))
...
int RealESRGAN::process(const ncnn::Mat& inimage, ncnn::Mat& outimage, int tilesize2) const
{
