...
async def nm_set_names(names_text: str) -> str:
    ...
    AllConfig["name"]["names"].extend(names)
...
async def nm_set_interval(hours: int) -> str:
    ...
    AllConfig["name"]["change_interval_h"] = hours
...
async def nm_toggle(enable: bool) -> str:
    ...
    """
    AllConfig["name"]["changenames"] = enable

    # اگر فعال شد
    if enable:
        if not AllConfig["name"]["names"]:
            return "❌ هیچ نامی برای تغییر وجود ندارد."

        if AllConfig["name"]["changenames_task"]:
            return "⚠️ فرآیند تغییر نام از قبل فعال است."

        AllConfig["name"]["changenames_task"] = asyncio.create_task(_change_name_loop())
        logger.info("🚀 Auto name changer started.")
        return "✅ تغییر خودکار نام‌ها فعال شد."

    # اگر غیرفعال شد
    task = AllConfig["name"].get("changenames_task")
    if task:
        try:
            task.cancel()
        except Exception:
            pass
        AllConfig["name"]["changenames_task"] = None
...
async def _change_name_loop():
    ...
    """
    app = AllConfig["name"].get("app")
    if not app:
        logger.warning("⚠️ App instance not found for name changer.")
        return

    while AllConfig["name"].get("changenames", False):
        try:
            names = AllConfig["name"].get("names", [])
            if not names:
                await asyncio.sleep(30)
                continue

            idx = AllConfig["name"].get("changenames_idx", 0)
            new_name = names[idx % len(names)]

            await app.update_profile(first_name=new_name)
            logger.info(f"🪄 Changed name to: {new_name}")

            AllConfig["name"]["changenames_idx"] = (idx + 1) % len(names)
            interval = AllConfig["name"].get("change_interval_h", 1)
