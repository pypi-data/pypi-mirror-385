...
- ثبت پیام‌ها + ذخیرهٔ مدیاها روی دیسک
- ساخت ساختار پوشهٔ جدا برای هر چت: <bk_dir>/<CHAT_ID>/
    - messages.txt
    - messages.json / messages.xlsx (در صورت نصب xlsxwriter)
    - media/
        - picture/
        - video/
        - voice/
        ...
        - gif/
        - (اختیاری: sticker/)
- تشخیص wipe و بکاپ خودکار (event + DB-aware)
- فرمان‌های بیرونی می‌توانند از این مسیرها استفاده کنند.

Config keys (AllConfig["backup"]):
    bk_enabled: bool
    bk_db: "downloads/backup.db"
    bk_dir: "downloads/bk_exports"
    bk_wipe_threshold: int
    bk_wipe_window_minutes: int (default=10 if missing)
    bk_cooldown_minutes: int (default=5 if missing)
"""
...
def _media_folder_name(telegram_media_attr: str) -> str:
    """
    نگاشت انواع تلگرام → نام پوشه‌ی مورد نظر شما
    photo → picture
    video → video
    animation(GIF) → gif
    voice → voice
    ...
    document → document
    sticker → sticker (اختیاری؛ اگر نخواستی، می‌توانی آن را هم به gif نگاشت کنی)
    """
    mapping = {
        "photo": "picture",
        "video": "video",
...
def _media_path_for(chat_id: int, msg_id: int, kind: str, suggested_name: str = "", mime: str = "") -> str:
    ...
    # نام فایل: <MSGID>_<KIND>_[SAFE_NAME].<ext?>
...
async def _persist_media_of_message(m: Message) -> None:
    ...
    try:
        ...
        if m.video:
            await _dl("video", m.video, getattr(m, "file_name", "") or "")
        if m.animation:  # GIF
...
# -----------------------------
# 📤 Export helpers (write files under <bk_dir>/<CHAT_ID>/)
# -----------------------------
...
async def bk_export_dialog_for_user(client, user_id: int, limit: Optional[int] = None) -> Optional[str]:
    ...
    async for msg in client.get_chat_history(user_id, limit=limit):
        ...
        rows.append({
            ...
            "media": db_fetch_media(user_id, msg.id)
        })
...
async def on_deleted(client, deleted_event) -> None:
    ...
    # brief report for single deletions
    del_ts = _now()
    for mid in ids:
...
def list_media_files(chat_id: int, kind_folder: str) -> List[str]:
    ...
    kind_folder باید یکی از: picture/video/voice/music/video_message/document/gif/sticker
    """
