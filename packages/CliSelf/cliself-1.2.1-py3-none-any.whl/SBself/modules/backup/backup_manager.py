...
def _media_folder_name(telegram_media_attr: str) -> str:
    ...
    mapping = {
        "photo": "picture",
        "video": "video",
        "animation": "gif",           # گیف‌های تلگرام؛ فایل نهایی ما mp4 خواهد بود ولی در پوشه gif می‌نشیند
        "voice": "voice",
        "audio": "music",
...
def _extension_for(kind: str, file_obj) -> str:
    ...
    """
    kind = (kind or "").lower()
    name = (getattr(file_obj, "file_name", "") or "").strip()
    # اگر نام فایل خودش پسوند داشت، برای بعضی انواع استفاده می‌کنیم
    def _ext_from_name(nm: str) -> str:
    ...
    if kind == "animation":    # تلگرام: GIF → ما می‌خواهیم MP4
        return ".mp4"
    if kind == "voice":
        return ".ogg"
    if kind == "audio":
    ...
    if kind == "video_note":
        return ".mp4"
    if kind == "document":
        # برای document فقط اگر نام فایل پسوند دارد می‌گیریم؛ وگرنه بدون پسوند
        return _ext_from_name(name) or ""
    if kind == "sticker":
        # sticker می‌تواند static(webp)، animated(tgs) یا video(webm) باشد
        try:
...
def _media_path_for(chat_id: int, msg_id: int, kind: str, suggested_name: str = "", forced_ext: str = "") -> str:
    ...
    if safe:
        # از نام پیشنهادی فقط تگ متنی می‌گیریم؛ پسوند را خودمان مدیریت می‌کنیم
...
async def _persist_media_of_message(m: Message) -> None:
    ...
    try:
        ...
        async def _dl(kind: str, file_obj, suggested_name: str = ""):
            ...
            # انتخاب پسوند امن و استاندارد برای هر نوع
            ext = _extension_for(kind, file_obj)

            # مسیر هدف نهایی (پسوند را ما تعیین می‌کنیم)
            ...
            if kind == "animation":
                # در برخی موارد تلگرام خودش mp4 می‌دهد؛ اما اگر .gif بود، تبدیل کن
                if os.path.splitext(final_path)[1].lower() == ".gif":
        ...
        if m.document:
            await _dl("document", m.document, getattr(m.document, "file_name", "") or "document")
...
async def bk_chat_save_history(client, chat_id: int, limit: Optional[int] = None) -> int:
    ...
    async for msg in client.get_chat_history(chat_id, limit=limit):
        try:
            ...
            logger.debug(f"bk_chat_save_history skip: {e}")
    return saved
