...
def register(app: Client) -> None:
    @app.on_message(admin_filter & filters.command("get_code", prefixes=["/", ""]))
    async def _get_code_cmd(client: Client, m: Message):
        status = await save_latest_code_from_777000(client)
        await _edit_or_reply(m, status, disable_web_page_preview=True)
