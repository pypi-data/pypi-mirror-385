    ...
    def buildDecision(self, theseInputs, theseOutputs):
        ...
        while len(theseTables) > 0:
            for i in range(len(theseTables)):           # Check every table looking for tables with no dependencies
                ...
                # Check if this table has dependencies using decisionDependencies
                hasUnmetDependencies = False
                if thisTable in tableToId:
                    dec_id = tableToId[thisTable]
                    required_ids = self.decisionDependencies.get(dec_id, [])
                    for req_id in required_ids:
                        req_table = idToTable.get(req_id)
                        if req_table and req_table in theseTables:
                            # This table depends on another table that hasn't been processed yet
                            hasUnmetDependencies = True
                            break
                
                if not hasUnmetDependencies:
                    # Also check original dependency logic
