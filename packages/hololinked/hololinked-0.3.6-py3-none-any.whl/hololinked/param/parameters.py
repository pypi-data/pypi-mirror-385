...
class Bytes(String):
    ...
    def _assert(
        obj,
        value: typing.Any,
        regex: typing.Optional[bytes] = None,
        allow_None: bool = False,
    ) -> None:
        """
...
class Number(Parameter):
    ...
    def __init__(
        self,
        default: typing.Optional[typing.Union[float, int]] = 0.0,
        *,
        bounds: typing.Optional[typing.Tuple] = None,
        crop_to_bounds: bool = False,
        inclusive_bounds: typing.Tuple = (True, True),
    ...
    def validate_and_adapt(self, value: typing.Any) -> typing.Union[int, float]:
        self._assert(
            value,
            self.dtype,
            None if self.crop_to_bounds else self.bounds,
            self.inclusive_bounds,
            self.allow_None,
        )
    ...
    def _assert(
        ...
        dtype: typing.Tuple,
        bounds: typing.Optional[typing.Tuple] = None,
        inclusive_bounds: typing.Tuple[bool, bool] = (True, True),
        allow_None: bool = False,
    ):
        ...
        if dtype is None:
            if not obj.isnumber(value):
                raise_TypeError(
                    "given value not of number type, but type {}.".format(type(value)),
                    obj,
                )
        elif not isinstance(value, dtype):
            raise_TypeError(
                "given value not of type {}, but type {}.".format(dtype, type(value)),
                obj,
            )
        if bounds:
            vmin, vmax = bounds
    ...
    def isnumber(cls, value: typing.Any) -> bool:
        ...
        elif hasattr(value, "__int__") and hasattr(value, "__add__"):
            return True
        # This is for older versions of gmpy
        elif hasattr(value, "qdiv"):
            return True
        else:
            return False
...
class Iterable(Parameter):
    ...
    def __init__(
        ...
        default: typing.Any,
        *,
        bounds: typing.Optional[typing.Tuple[int, int]] = None,
        length: typing.Optional[int] = None,
        item_type: typing.Optional[typing.Tuple] = None,
        deepcopy_default: bool = False,
        allow_None: bool = False,
        doc: typing.Optional[str] = None,
        constant: bool = False,
    ...
    def _assert(
        ...
        bounds: typing.Optional[typing.Tuple[int, int]] = None,
        length: typing.Optional[int] = None,
        dtype: typing.Union[type, typing.Tuple] = (list, tuple),
        ...
        if bounds is not None:
            if not (len(value) >= bounds[0] and len(value) <= bounds[1]):
                raise_ValueError(
                    "given iterable is not of the correct length ({} instead of between {} and {}).".format(
                        len(value), 0 if not bounds[0] else bounds[0], bounds[1]
                    ),
                    obj,
                )
        ...
        if item_type is not None:
            for val in value:
                if not isinstance(val, item_type):
                    raise_TypeError(
                        "not all elements of given iterable of item type {}, found object of type {}".format(
                            item_type, type(val)
                        ),
                        obj,
                    )
    ...
    def isinstance(
        obj,
        value: typing.Any,
        bounds: typing.Optional[typing.Tuple[int, int]],
        length: typing.Optional[int] = None,
        dtype: typing.Union[type, typing.Tuple] = (list, tuple),
        item_type: typing.Any = None,
        allow_None: bool = False,
...
class Composite(Parameter):
    ...
    def __init__(
        self,
        attribs: typing.List[typing.Union[str, Parameter]],
        *,
        doc: typing.Optional[str] = None,
        constant: bool = False,
        readonly: bool = False,
    ...
    def validate_and_adapt(self, value):
        if not len(value) == len(self.attribs):
            raise_ValueError(
                "Compound parameter got the wrong number of values (needed {}, but got {}).".format(
...
class Selector(SelectorBase):
    ...
    def validate_and_adapt(self, value: typing.Any) -> typing.Any:
        ...
        if not (value in self.objects or (self.allow_None and value is None)):
            raise_ValueError(
                "given value not in list of possible objects, valid options include {}".format(
    ...
    def validate_and_adapt(self, value):
        ...
        if self.isinstance:
            ...
            try:
                ...
                if str(ex).startswith("ssubclass() arg 1 must be a class"):
    ...
    def validate_and_adapt(self, value: typing.Any):
        ...
        if value not in self.objects:
            ...
            if not isinstance(value, tuple):
                raise_ValueError(
                    f"object {value} not specified as a valid member of list of objects.",
                    self,
                )
            else:
                for obj in value:
...
class resolve_path(ParameterizedFunction):
    ...
    def __call__(self, path: str, **params) -> str:
        ...
        if os.path.isabs(path):
            if (
                (p.path_to_file is None and os.path.exists(path))
                or (p.path_to_file is True and os.path.isfile(path))
                or (p.path_to_file is False and os.path.isdir(path))
            ):
                return path
            ...
            for prefix in p.search_paths:
                ...
                if (
                    (p.path_to_file is None and os.path.exists(try_path))
                    or (p.path_to_file is True and os.path.isfile(try_path))
                    or (p.path_to_file is False and os.path.isdir(try_path))
                ):
                    return try_path
            ...
            raise IOError(
                ...
                + " was not found in the following place(s): "
                + str(paths_tried)
                + "."
            )
...
class Path(Parameter):
    ...
    * any of the paths searched by resolve_path() (if search_paths
    ...
    def __init__(
        ...
        default: typing.Any = "",
        *,
        search_paths: typing.Optional[str] = None,
        doc: typing.Optional[str] = None,
        constant: bool = False,
        readonly: bool = False,
        allow_None: bool = False,
...
class Date(Number):
    ...
    def __init__(
        ...
        default,
        *,
        bounds: typing.Union[typing.Tuple, None] = None,
        crop_to_bounds: bool = False,
        inclusive_bounds: typing.Tuple = (True, True),
        step: typing.Any = None,
        doc: typing.Optional[str] = None,
    ...
    def serialize(cls, value):
        ...
        ):  # i.e np.datetime64, note numpy is imported only on requirement
...
class Range(Tuple):
    ...
    def __init__(
        self,
        default: typing.Optional[typing.Tuple] = None,
        *,
        bounds: typing.Optional[typing.Tuple[int, int]] = None,
        length: typing.Optional[int] = None,
        item_type: typing.Optional[typing.Tuple] = None,
...
class BaseConstrainedList(collections.abc.MutableSequence):
    ...
    def _validate_bounds_for_extension(self, value: typing.Any = [None]) -> None:
        if not (
            self._inner.__len__() + value.__len__() >= self.bounds[0]
...
class TypeConstrainedList(BaseConstrainedList):
    ...
    def __iadd__(self, value: typing.List[typing.Any]):
        ...
        return TypeConstrainedList(
            default=self._inner.__iadd__(value),
...
class TypedList(ClassSelector):
    ...
    def __init__(
        ...
        *,
        item_type: typing.Any = None,
        deepcopy_default: bool = True,
        accept_nonlist_object: bool = False,
...
class TypeConstrainedDict(collections.abc.MutableMapping):
    ...
    def __init__(
        ...
        default: typing.Dict,
        *,
        key_type: typing.Optional[typing.Union[type, typing.Tuple]] = None,
        item_type: typing.Optional[typing.Union[type, typing.Tuple]] = None,
        bounds: typing.Tuple = (0, None),
        constant: bool = False,
        skip_validate: bool = False,
    ):
        super().__init__()
    ...
    def _validate_for_insertion(self, value: typing.Dict) -> None:
        if self.constant:
            raise ValueError(
                f"Dict {get_iterable_printfriendly_repr(self._inner)} is a constant and cannot be modified."
            )
        self._validate_value(value)
        self._validate_bounds_for_extension(value)
        self._validate_items(value)
...
class TypedKeyMappingsConstrainedDict(TypeConstrainedDict):
    def __init__(
        ...
        *,
        ...
        allow_unspecified_keys: bool = False,
        bounds: tuple = (0, None),
        constant: bool = False,
        skip_validate: bool = False,
    ) -> None:
...
class TypedKeyMappingsDict(ClassSelector):
    ...
    def __init__(
        ...
        *,
        type_mapping: typing.Dict,
        allow_unspecified_keys: bool = True,
        bounds: tuple = (0, None),
        deepcopy_default: bool = True,
        allow_None: bool = True,
        doc: typing.Optional[str] = None,
        ...
        self.bounds = bounds
        super().__init__(
            class_=TypedKeyMappingsConstrainedDict,
            default=default,
            isinstance=True,
            deepcopy_default=deepcopy_default,
            doc=doc,
    ...
    def __set__(self, obj, value):
        if value is not None:
            container = TypedKeyMappingsConstrainedDict(
                default=value,
                type_mapping=self.type_mapping,
                allow_unspecified_keys=self.allow_unspecified_keys,
                bounds=self.bounds,
                constant=self.constant,
                skip_validate=False,
            )
