...
class AuthConnection:
    ...
    def _set_tokens(self, payload: object) -> str:
        ...
        if isinstance(payload, dict):
            ...
            if (
                ("refresh_token" in token and isinstance(token["refresh_token"], str))
                and ("access_token" in token and isinstance(token["access_token"], str))
                and ("expires_in" in token and isinstance(token["expires_in"], int))
                and ("refresh_expires_in" in token and isinstance(token["refresh_expires_in"], int))
            ):
