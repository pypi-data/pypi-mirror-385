...
from cfncli.ext_customizations.cloudformation.artifact_exporter import Template, Resource, make_abs_path
...
from cfncli.ext_customizations.cloudformation.artifact_exporter import RESOURCES_EXPORT_LIST as EXPORTS
...
def package_template(ppt, session, template_path, bucket_region, bucket_name=None, prefix=None, kms_key_id=None):
    ...
    try:
        s3_uploader = S3Uploader(s3_client, bucket_name, bucket_region, prefix, kms_key_id, force_upload=False)
    except TypeError:
        # HACK: since awscli 1.16.145+ the bucket region parameter is removed
        s3_uploader = S3Uploader(s3_client, bucket_name, prefix, kms_key_id, force_upload=False)

    template = Template(template_path, os.getcwd(), s3_uploader, resources_to_export=EXPORTS)

    exported_template = template.export()

    ppt.secho(
        "Successfully packaged artifacts and " "uploaded to s3://{bucket_name}.".format(bucket_name=bucket_name),
        fg="green",
    )
    ...
    if len(template_data) <= TEMPLATE_BODY_SIZE_LIMIT:
        ...
        with open(tempfile_path, "wb") as fp:
            ...
            fp.flush()
            # upload to s3
            template_location = s3_uploader.upload_with_dedup(fp.name, extension="template.json")
        ...
        template_url = s3_client.generate_presigned_url(
            ClientMethod="get_object", Params={"Bucket": bucket_name, "Key": template_key}, ExpiresIn=3600
        )
...
class ResourceWithInlineCode(Resource):
    ...
    def export(self, resource_id, resource_dict, parent_dir):
        ...
        if isinstance(property_value, dict):
            logging.debug("Property {0} of {1} resource is not a file path".format(self.PROPERTY_NAME, resource_id))
            return
