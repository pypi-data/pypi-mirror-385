    ...
    def collect_stack_outputs(self, *attributes):
        for attribute in attributes:
            try:
                ...
                self._ppt.secho("Invalid Config Reference %s" % attribute, color="yellow")
                raise e
            ...
            for context in self._contexts:
                ...
                try:
                    ...
                    self._ppt.secho(
                        "Collect Outputs: Unable to access outputs from %s" % qualified_name, color="yellow"
                    )
                    raise e

                if stack.outputs:
                    outputs = dict(
                        map(lambda o: (".".join((context.stack_key, o["OutputKey"])), o["OutputValue"]), stack.outputs)
                    )
                    if attribute not in outputs:
                        raise RuntimeError("Collect Outputs: Attribute %s not found" % attribute)

                    self._ppt.secho("Collected outputs from %s" % qualified_name, color="yellow")
