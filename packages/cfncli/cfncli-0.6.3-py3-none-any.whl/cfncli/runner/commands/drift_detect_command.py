...
class DriftDetectCommand(Command):
    ...
    def run(self, stack_context):
        ...
        self.ppt.pprint_stack_name(stack_context.stack_key, parameters["StackName"], "Detecting drift of ")
        ...
        if self.options.no_wait:
            ...
            waiter = botocore.waiter.create_waiter_with_client("DriftDetectionComplete", waiter_model, client)
            waiter.wait(StackDriftDetectionId=drift_detection_id)

            # self.ppt.secho('Drift detection complete.', fg='green')

            result = client.describe_stack_drift_detection_status(StackDriftDetectionId=drift_detection_id)
