...
class StackDeployOptions(
    namedtuple(
        "StackDeployOptions",
        ["no_wait", "on_failure", "disable_rollback", "timeout_in_minutes", "ignore_existing", "show_physical_ids"],
    )
):
    pass
...
class StackDeployCommand(Command):
    ...
    def run(self, stack_context):
        ...
        self.ppt.pprint_stack_name(stack_context.stack_key, parameters["StackName"], "Deploying stack ")
        ...
        try:
            ...
            if self.options.ignore_existing and is_stack_already_exists_exception(ex):
