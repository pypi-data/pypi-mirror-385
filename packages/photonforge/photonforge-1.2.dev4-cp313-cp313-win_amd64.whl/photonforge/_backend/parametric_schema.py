    ...
    if obj is not None:
        if isinstance(cls, type) and issubclass(cls, AbstractBase):
            ...
            if len(cls) == 1:
                ...
                return [make_serializable(x, c) for x, c in zip(obj, cls)]
...
def schema_from_type(
    ...
    if metadata.optional:
        ...
        field = Union[Annotated[field, metadata.non_optional_copy()], None]
        metadata = _Metadata()
    ...
    if isinstance(field, typing.ForwardRef):
        try:
            ...
            try:
                ...
                logger.warn(f"Unable to evaluate {field!r}.")
    ...
    if kwargs_for is not None:
        ...
        if kwargs_for is None:
            logger.warn(f"Kwargs target '{target}' not found in registries.")
...
def deserialize(value, cls):
    ...
    if isinstance(cls, list):
        ...
        if len(cls) == 1:
            ...
            # tuple == one class per item
            for i, (v, c) in enumerate(zip(value, cls)):
                result[i], valid = deserialize(v, c)
                if not valid:
                    break
        if valid:
