...
@cache
def get_browzy_version() -> str:
	"""Get the browzy package version using the same logic as BrowzyAgent._set_browzy_version_and_source"""
	try:
		...
		version = str(get_version('browzy'))
...
async def check_latest_browzy_version() -> str | None:
	"""Check the latest version of browzy from PyPI asynchronously.
	...
	try:
		async with httpx.AsyncClient(timeout=3.0) as client:
			response = await client.get('https://pypi.org/pypi/browzy/json')
