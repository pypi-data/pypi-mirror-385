...
from browzy.agent.message_manager.utils import save_conversation
from browzy.llm.base import BaseChatModel
from browzy.llm.messages import BaseMessage, ContentPartImageParam, ContentPartTextParam, UserMessage
from browzy.tokens.service import TokenCost
...
from browzy import Browser, BrowserProfile, BrowserSession

# Lazy import for gif to avoid heavy agent.views import at startup
# from browzy.agent.gif import create_history_gif
from browzy.agent.message_manager.service import (
	MessageManager,
)
from browzy.agent.prompts import SystemPrompt
from browzy.agent.views import (
	ActionResult,
	AgentError,
	AgentHistory,
	AgentHistoryList,
...
from browzy.browser.session import DEFAULT_BROWSER_PROFILE
from browzy.browser.views import BrowserStateSummary
from browzy.config import CONFIG
from browzy.dom.views import DOMInteractedElement
from browzy.filesystem.file_system import FileSystem
from browzy.observability import observe, observe_debug
from browzy.telemetry.service import ProductTelemetry
from browzy.telemetry.views import AgentTelemetryEvent
from browzy.tools.registry.views import ActionModel
from browzy.tools.service import Tools
from browzy.utils import (
	URL_PATTERN,
	_log_pretty_path,
	check_latest_browzy_version,
	get_browzy_version,
	get_git_info,
...
AgentHookFunc = Callable[['BrowzyAgent'], Awaitable[None]]


class BrowzyAgent(Generic[Context, AgentStructuredOutput]):
	...
	def __init__(
		...
		**kwargs,
		...
		if llm is None:
			...
			if default_llm_name:
				from browzy.llm.models import get_llm_by_name

				llm = get_llm_by_name(default_llm_name)
			else:
				# No default LLM specified, use the original default
				from browzy import ChatBrowzy

				llm = ChatBrowzy()

		# set flashmode = True if llm is ChatBrowzy
		if llm.provider == 'browzy':
			flash_mode = True
		...
		# wal_path = CONFIG.BROWZY_CONFIG_DIR / 'events' / f'{self.session_id}.jsonl'
	...
	def logger(self) -> logging.Logger:
		...
		return logging.getLogger(f'browzy.Agent🅰 {self.task_id[-4:]} ⇢ 🅑 {_browser_session_id[-4:]} 🅣 {_current_target_id}')
	...
	def _set_screenshot_service(self) -> None:
		...
		try:
			from browzy.screenshots.service import ScreenshotService
	...
	def _process_messsages_and_replace_long_urls_shorter_ones(self, input_messages: list[BaseMessage]) -> dict[str, str]:
		...
		"""
		from browzy.llm.messages import AssistantMessage, UserMessage
	...
	async def _log_agent_run(self) -> None:
		...
		latest_version = await check_latest_browzy_version()
	...
	async def run(
		...
		from browzy.utils import SignalHandler
	...
	def get_trace_object(self) -> dict[str, Any]:
		...
		return {
			'trace': {
				...
				'trace_id': trace_id,
				'timestamp': timestamp,
				'browser_use_version': get_browzy_version(),
