...
if '--mcp' in sys.argv:
	...
	os.environ['BROWZY_LOGGING_LEVEL'] = 'critical'
	os.environ['BROWZY_SETUP_LOGGING'] = 'false'
...
from browzy import BrowzyAgent, Controller
from browzy.agent.views import AgentSettings
from browzy.browser import BrowserProfile, BrowserSession
from browzy.logging_config import addLoggingLevel
from browzy.telemetry import CLITelemetryEvent, ProductTelemetry
from browzy.utils import get_browzy_version
...
try:
	...
	print('⚠️ CLI addon is not installed. Please install it with: `pip install "browzy[cli]"` and try again.')
	sys.exit(1)
...
os.environ['BROWZY_LOGGING_LEVEL'] = 'result'

from browzy.config import CONFIG

# Set USER_DATA_DIR now that CONFIG is imported
USER_DATA_DIR = CONFIG.BROWZY_PROFILES_DIR / 'cli'

# Ensure directories exist
CONFIG.BROWZY_CONFIG_FILE.parent.mkdir(parents=True, exist_ok=True)
...
def load_user_config() -> dict[str, Any]:
	...
	history_file = CONFIG.BROWZY_CONFIG_DIR / 'command_history.json'
...
class BrowserUseApp(App):
	...
	def setup_richlog_logging(self) -> None:
		...
		log_type = os.getenv('BROWZY_LOGGING_LEVEL', 'result').lower()

		class BrowserUseFormatter(logging.Formatter):
			def format(self, record):
				# if isinstance(record.name, str) and record.name.startswith('browzy.'):
		...
		browser_use_logger = logging.getLogger('browzy')
		...
		for logger_name in ['browzy.Agent', 'browzy.controller', 'browzy.agent', 'browzy.agent.service']:
		...
		for name, logger in logging.Logger.manager.loggerDict.items():
			if isinstance(name, str) and 'browzy.Agent' in name:
	...
	def on_mount(self) -> None:
		...
		logger = logging.getLogger('browzy.on_mount')
	...
	def run_task(self, task: str) -> None:
		...
		# Get the logger
		logger = logging.getLogger('browzy.app')
	...
	def compose(self) -> ComposeResult:
		...
		with Container(id='main-container'):
			...
			# Paths panel
			yield Static(
				f' ⚙️  Settings saved to:              {str(CONFIG.BROWZY_CONFIG_FILE.resolve()).replace(str(Path.home()), "~")}\n'
...
async def textual_interface(config: dict[str, Any]):
	...
	logger = logging.getLogger('browzy.startup')
	...
	try:
		...
		try:
			from browzy.logging_config import setup_log_pipes
...
async def run_auth_command():
	...
	if auth_client.is_authenticated:
		...
		frontend_url = CONFIG.BROWZY_CLOUD_UI_URL or auth_client.base_url.replace('//api.', '//cloud.')
	...
	try:
		...
		from browzy.agent.cloud_events import (
			CreateAgentSessionEvent,
			CreateAgentStepEvent,
			CreateAgentTaskEvent,
			UpdateAgentTaskEvent,
		)
		from browzy.sync.service import CloudSync
		...
		if task_id and sync_service:
			try:
				from browzy.agent.cloud_events import UpdateAgentTaskEvent
...
def run_main_interface(ctx: click.Context, debug: bool = False, **kwargs):
	...
	if kwargs.get('mcp'):
		...
		try:
			...
			pass
		# Run as MCP server
		from browzy.mcp.server import main as mcp_main
	...
	try:
		config = load_user_config()
		logger.debug(f'User configuration loaded from {CONFIG.BROWZY_CONFIG_FILE}')
	except Exception as e:
