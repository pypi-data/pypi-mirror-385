"""Configuration system for Browzy with automatic migration support."""
...
class OldConfig:
	...
	@property
	def BROWZY_LOGGING_LEVEL(self) -> str:
		return os.getenv('BROWZY_LOGGING_LEVEL', 'info').lower()
	...
	@property
	def BROWZY_CLOUD_SYNC(self) -> bool:
		return os.getenv('BROWZY_CLOUD_SYNC', str(self.ANONYMIZED_TELEMETRY)).lower()[:1] in 'ty1'

	@property
	def BROWZY_CLOUD_API_URL(self) -> str:
		url = os.getenv('BROWZY_CLOUD_API_URL', 'https://api.browser-use.com')
		assert '://' in url, 'BROWZY_CLOUD_API_URL must be a valid URL'
		return url

	@property
	def BROWZY_CLOUD_UI_URL(self) -> str:
		url = os.getenv('BROWZY_CLOUD_UI_URL', '')
		# Allow empty string as default, only validate if set
		if url and '://' not in url:
			raise AssertionError('BROWZY_CLOUD_UI_URL must be a valid URL if set')
		return url
	...
	@property
	def BROWZY_CONFIG_DIR(self) -> Path:
		path = Path(os.getenv('BROWZY_CONFIG_DIR', str(self.XDG_CONFIG_HOME / 'browseruse'))).expanduser().resolve()
		self._ensure_dirs()
		return path

	@property
	def BROWZY_CONFIG_FILE(self) -> Path:
		return self.BROWZY_CONFIG_DIR / 'config.json'

	@property
	def BROWZY_PROFILES_DIR(self) -> Path:
		path = self.BROWZY_CONFIG_DIR / 'profiles'
		self._ensure_dirs()
		return path

	@property
	def BROWZY_DEFAULT_USER_DATA_DIR(self) -> Path:
		return self.BROWZY_PROFILES_DIR / 'default'

	@property
	def BROWZY_EXTENSIONS_DIR(self) -> Path:
		path = self.BROWZY_CONFIG_DIR / 'extensions'
		self._ensure_dirs()
		return path
	...
	def _ensure_dirs(self) -> None:
		...
		if not self._dirs_created:
			config_dir = (
				Path(os.getenv('BROWZY_CONFIG_DIR', str(self.XDG_CONFIG_HOME / 'browseruse'))).expanduser().resolve()
			)
...
class FlatEnvConfig(BaseSettings):
	...
	BROWZY_LOGGING_LEVEL: str = Field(default='info')
	CDP_LOGGING_LEVEL: str = Field(default='WARNING')
	BROWZY_DEBUG_LOG_FILE: str | None = Field(default=None)
	BROWZY_INFO_LOG_FILE: str | None = Field(default=None)
	ANONYMIZED_TELEMETRY: bool = Field(default=True)
	BROWZY_CLOUD_SYNC: bool | None = Field(default=None)
	BROWZY_CLOUD_API_URL: str = Field(default='https://api.browser-use.com')
	BROWZY_CLOUD_UI_URL: str = Field(default='')

	# Path configuration
	XDG_CACHE_HOME: str = Field(default='~/.cache')
	XDG_CONFIG_HOME: str = Field(default='~/.config')
	BROWZY_CONFIG_DIR: str | None = Field(default=None)
	...
	BROWZY_CONFIG_PATH: str | None = Field(default=None)
	BROWZY_HEADLESS: bool | None = Field(default=None)
	BROWZY_ALLOWED_DOMAINS: str | None = Field(default=None)
	BROWZY_LLM_MODEL: str | None = Field(default=None)

	# Proxy env vars
	BROWZY_PROXY_URL: str | None = Field(default=None)
	BROWZY_NO_PROXY: str | None = Field(default=None)
	BROWZY_PROXY_USERNAME: str | None = Field(default=None)
	BROWZY_PROXY_PASSWORD: str | None = Field(default=None)
...
class Config:
	...
	def _get_config_path(self) -> Path:
		...
		env_config = FlatEnvConfig()
		if env_config.BROWZY_CONFIG_PATH:
			return Path(env_config.BROWZY_CONFIG_PATH).expanduser()
		elif env_config.BROWZY_CONFIG_DIR:
			return Path(env_config.BROWZY_CONFIG_DIR).expanduser() / 'config.json'
		else:
	...
	def _load_config(self) -> dict[str, Any]:
		...
		if env_config.BROWZY_HEADLESS is not None:
			config['browser_profile']['headless'] = env_config.BROWZY_HEADLESS

		if env_config.BROWZY_ALLOWED_DOMAINS:
			domains = [d.strip() for d in env_config.BROWZY_ALLOWED_DOMAINS.split(',') if d.strip()]
			config['browser_profile']['allowed_domains'] = domains

		# Proxy settings (Chromium) -> consolidated `proxy` dict
		proxy_dict: dict[str, Any] = {}
		if env_config.BROWZY_PROXY_URL:
			proxy_dict['server'] = env_config.BROWZY_PROXY_URL
		if env_config.BROWZY_NO_PROXY:
			# store bypass as comma-separated string to match Chrome flag
			proxy_dict['bypass'] = ','.join([d.strip() for d in env_config.BROWZY_NO_PROXY.split(',') if d.strip()])
		if env_config.BROWZY_PROXY_USERNAME:
			proxy_dict['username'] = env_config.BROWZY_PROXY_USERNAME
		if env_config.BROWZY_PROXY_PASSWORD:
			proxy_dict['password'] = env_config.BROWZY_PROXY_PASSWORD
		if proxy_dict:
		...
		if env_config.BROWZY_LLM_MODEL:
			config['llm']['model'] = env_config.BROWZY_LLM_MODEL
