...
from browzy.sync.auth import CloudAuthConfig
...
class CloudBrowserClient:
	...
	async def create_browser(self) -> CloudBrowserResponse:
		...
		api_token = os.getenv('BROWZY_API_KEY')
		...
		if not api_token:
			raise CloudBrowserAuthError(
				'No authentication token found. Please set BROWZY_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.browser-use.com'
			)
		...
		try:
			...
			if response.status_code == 401:
				raise CloudBrowserAuthError(
					'Authentication failed. Please make sure you have set BROWZY_API_KEY environment variable to authenticate with the cloud service. You can also create an API key at https://cloud.browser-use.com'
				)
	...
	async def stop_browser(self, session_id: str | None = None) -> CloudBrowserResponse:
		...
		try:
			...
			if response.status_code == 401:
				raise CloudBrowserAuthError(
					'Authentication failed. Please make sure you have set the BROWZY_API_KEY environment variable to authenticate with the cloud service.'
				)
