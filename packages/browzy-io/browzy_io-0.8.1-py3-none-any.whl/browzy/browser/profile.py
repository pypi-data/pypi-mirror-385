...
from browzy.utils import _log_pretty_path, logger
...
# Using constants from central location in browzy.config
...
class BrowserProfile(BrowserConnectArgs, BrowserLaunchPersistentContextArgs, BrowserLaunchArgs, BrowserNewContextArgs):
	...
	proxy: ProxySettings | None = Field(
		default=None,
		description='Proxy settings. Use browzy.browser.profile.ProxySettings(server, bypass, username, password)',
	)
	...
	def warn_user_data_dir_non_default_version(self) -> Self:
		...
		if self.user_data_dir == CONFIG.BROWZY_DEFAULT_USER_DATA_DIR and is_not_using_default_chromium:
			alternate_name = (
			...
			self.user_data_dir = CONFIG.BROWZY_DEFAULT_USER_DATA_DIR.parent / f'default-{alternate_name}'
		return self
	...
	def _ensure_default_extensions_downloaded(self) -> list[str]:
		...
		cache_dir = CONFIG.BROWZY_EXTENSIONS_DIR
