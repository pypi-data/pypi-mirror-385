"""
ChatBrowzy - Client for Browzy cloud API

This wraps the BaseChatModel protocol and sends requests to the Browzy cloud API
...
from browzy.llm.messages import BaseMessage
from browzy.llm.views import ChatInvokeCompletion
from browzy.observability import observe
...
class ChatBrowzy(BaseChatModel):
	"""
	Client for Browzy cloud API.

	This sends requests to the Browzy cloud API which uses optimized models
	...
	def __init__(
		...
		"""
		Initialize ChatBrowzy client.

		Args:
			fast: If True, uses fast model. If False, uses smart model.
			api_key: API key for Browzy cloud. Defaults to BROWZY_API_KEY env var.
			base_url: Base URL for the API. Defaults to BROWZY_LLM_URL env var or production URL.
			timeout: Request timeout in seconds.
		"""
		self.fast = fast
		self.api_key = api_key or os.getenv('BROWZY_API_KEY')
		self.base_url = base_url or os.getenv('BROWZY_LLM_URL', 'https://llm.api.browzy.io')
		self.timeout = timeout
		self.model = 'fast' if fast else 'smart'

		if not self.api_key:
			raise ValueError(
				'You need to set the BROWZY_API_KEY environment variable. '
				'Get your key at https://cloud.browzy.io/dashboard/api'
			)
	...
	async def ainvoke(
		...
		"""
		Send request to Browzy cloud API.
		...
		async with httpx.AsyncClient(timeout=self.timeout) as client:
			try:
				...
				error_msg = f'Failed to connect to Browzy API: {e}'
				raise ValueError(error_msg)
			...
			usage = None
			if 'usage' in result:
				from browzy.llm.views import ChatInvokeUsage
