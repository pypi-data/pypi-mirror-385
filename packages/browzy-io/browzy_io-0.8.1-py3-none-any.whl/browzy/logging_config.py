...
def setup_logging(stream=None, log_level=None, force_setup=False, debug_log_file=None, info_log_file=None):
	...
	Args:
		...
		log_level: Override log level (default: uses CONFIG.BROWZY_LOGGING_LEVEL)
	...
	log_type = log_level or CONFIG.BROWZY_LOGGING_LEVEL

	# Check if handlers are already set up
	if logging.getLogger().hasHandlers() and not force_setup:
		return logging.getLogger('browzy')
	...
	class BrowserUseFormatter(logging.Formatter):
		...
		def format(self, record):
			...
			if self.log_level > logging.DEBUG and isinstance(record.name, str) and record.name.startswith('browzy.'):
				...
				if 'Agent' in record.name:
					...
					record.name = 'dom'
				elif record.name.startswith('browzy.'):
...
def setup_log_pipes(session_id: str, base_dir: str | None = None):
	...
	for name in ['browzy.agent', 'browzy.tools']:
	...
	for name in ['bubus', 'browzy.browser.session']:
		logger = logging.getLogger(name)
