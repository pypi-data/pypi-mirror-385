...
def main(
    ...
    *,
    ...
    # Generate receipt ID from runlet_id
    receipt_id = f"rcpt-{runlet_id[:8]}"

    # Capture environment information
    environment_info = {}
    try:
        from .env import capture_full_environment
        environment_info = capture_full_environment()
    except ImportError:
        environment_info = {"python": "unknown", "packages": {}, "docker": None}

    # Extract git information
    git_info = {}
    if git_ref and git_ref != "unknown":
        git_info = {
            "repository": vigil_url,
            "commit": git_ref,
            "branch": "unknown",  # Could be extracted from git
            "dirty": False  # Could be checked
        }

    # Determine owner (could be from git config or environment)
    owner = "unknown"
    try:
    ...
    # Determine pipeline command
    pipeline_cmd = "unknown"
    try:
    ...
    # Evaluate policies against the receipt
    try:
        from .policy import evaluate_receipt_policy
        policy_result = evaluate_receipt_policy(out)
        if not policy_result.passed:
            print(f"⚠️  Policy evaluation failed: {policy_result.summary}")
        ...
        print("⚠️  Policy evaluation not available")
