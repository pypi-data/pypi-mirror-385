...
class UIElement:
    def __init__(self, name, size):
        ...
        self.mouse_held = False  # Already discussed
    ...
    def update_internal(self, mouse_down_pos, mouse_pos, mouse_pressed):
        ...
        # Start holding if mouse pressed down on element
        if mouse_down_pos is not None and self.is_inside(mouse_down_pos):
            self.mouse_held = True
            self.on_click(mouse_down_pos)

        # Continuous holding
        if self.mouse_held and mouse_pressed:
            if mouse_pos is not None and self.is_inside(mouse_pos):
                self.on_hold(mouse_pos)
            else:
                # Mouse moved outside element
                self.mouse_held = False

        # Stop holding when mouse released
        if not mouse_pressed:
            self.mouse_held = False

    def on_hold(self, mouse_pos):
        for script in self.scripts:
            # Check if the script instance has an on_hold method
