...
class VideoElement(VideoBase):
    ...
    def __init__(self, video_path: str, scale: float = 1.0) -> None:
        ...
        # Hardware decoder (optional, will fallback to OpenCV if unavailable)
        self.hw_decoder: Optional[HardwareVideoDecoder] = None
        self.use_hw_decode = False

        # Try to initialize hardware decoder if available
        if hw_caps.has_ffmpeg and hw_caps.hw_decoders:
            try:
                self.hw_decoder = HardwareVideoDecoder(video_path)
                if self.hw_decoder.width > 0 and self.hw_decoder.height > 0:
                    self.use_hw_decode = True
                    print(f"Hardware decoder initialized for: {os.path.basename(video_path)}")
            except Exception as e:
                print(f"Hardware decoder failed, using OpenCV: {e}")
    ...
    def _load_video_info(self) -> None:
        ...
        and frame count, and calculates the video duration. Uses hardware decoder info if available.
        """
        ...
        try:
            # Try to get info from hardware decoder first
            if self.use_hw_decode and self.hw_decoder:
                self.original_width = self.hw_decoder.width
                self.original_height = self.hw_decoder.height
                self.fps = self.hw_decoder.fps
                self.total_frames = self.hw_decoder.total_frames
            else:
                # Fallback to OpenCV
    ...
    def _get_frame_at_time(self, video_time: float) -> Optional[np.ndarray]:
        ...
        if self.use_hw_decode and self.hw_decoder:
            frame = self._get_frame_hw(video_time)
            if frame is not None:
                return frame
            # If hardware decode fails, fall back to OpenCV
            print(f"Hardware decode failed, falling back to OpenCV")
            self.use_hw_decode = False

        # OpenCV fallback
        return self._get_frame_opencv(video_time)

    def _get_frame_hw(self, video_time: float) -> Optional[np.ndarray]:
        """Get frame using hardware decoder.

        Args:
            video_time: Time in seconds

        Returns:
            Frame data as numpy array in RGBA format, or None if unavailable
        """
        if not self.hw_decoder:
            return None

        try:
            # Get frame from hardware decoder (returns RGB)
            frame = self.hw_decoder.get_frame_at_time(video_time)
            if frame is None:
                return None
            ...
            print(f"Hardware decode error: {e}")
            return None

    def _get_frame_opencv(self, video_time: float) -> Optional[np.ndarray]:
        """Get frame using OpenCV (fallback method).
