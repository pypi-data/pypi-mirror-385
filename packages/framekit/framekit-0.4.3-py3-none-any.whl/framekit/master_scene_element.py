...
from .gpu_utils import GPUDetector, init_opengl_context
...
class MasterScene:
    ...
    def _create_audio_mix(self, video_path: str):
        ...
        # Get GPU-aware encoder settings
        encoder_settings = GPUDetector.get_ffmpeg_encoder_settings(self.quality)
        ...
        if len(valid_audio_files) == 1:
            ...
            if getattr(audio_element, 'loop_until_scene_end', False) and audio_element.duration > audio_element.original_duration:
                ...
                cmd.extend(['-c:a', 'aac', '-t', str(self.total_duration), final_output])
            else:
                # 単一ファイル、ループなしの場合
                filter_chain = ""
    ...
    def render(self):
        """動画をレンダリング"""
        # Initialize GPU context if available (must be done before pygame/OpenGL init)
        gpu_initialized = init_opengl_context(use_gpu=True)
