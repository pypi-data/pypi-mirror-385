...
#. Mark strings in the code to be localized with an underscore like::
   ...
   (The underscore is just a recommended shorter substitute for e.g. ``settings.lang_sys.gettext`` as seen in the code)
...
#. Extract files into *.pot* files with Babel_::

    pybabel extract
      -o src/flowstrider/localization/<*domain*>/<*domain*>.pot
      <*paths to input files*>

    eg.:
    pybabel extract
      -o src/flowstrider/localization/attributes/attributes.pot
      src/flowstrider/rules/attributes_dict.py

   There are multiple localization domains like *messages*, *linddun*, *attributes*
   each having their own folder holding a *.pot* file. This is done to improve the
   maintainability of the localization files. Since each domain is responsible for a
   certain set of source code files, it is important to only include the correct files
   as input for each domain. For better overview, an  *input_files.txt* notice is
   included for each domain and should be maintained / created for new domains. The
   input files/directories must be separated by spaces.

#. Update the localization *.po* files from the *.pot* file with::

    pybabel update
      -l de
      -i src/flowstrider/localization/<*domain*>/<*domain*>.pot
      -d src/flowstrider/localization/<*domain*>/
      -D <*domain*>
