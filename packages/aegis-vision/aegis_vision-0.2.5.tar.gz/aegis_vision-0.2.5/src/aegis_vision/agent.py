...
class TrainingAgent:
    ...
    def initialize_firebase(self) -> None:
        """Initialize Firebase with custom token authentication"""
        try:
            # Get custom token
            custom_token = self.authenticator.authenticate()
            logger.info("Successfully obtained Firebase custom token")
            
            # Initialize Firebase Admin SDK with service account for agent
            # Note: For production, agents should use service accounts
            # For now, we use the custom token approach through the Python SDK
            
            # Initialize app if not already initialized
            if not firebase_admin._apps:
                # For agent use, we need application default credentials or service account
                # The custom token is for client SDKs, not Admin SDK
                # We'll use the Firestore client library directly instead
                pass
            
            # Initialize Firestore client with custom authentication
            from google.cloud import firestore
            from google.oauth2 import service_account
            
            # Create Firestore client
            # Note: In production, use service account JSON file
            self.db = firestore.Client(project=self.firestore_project)
            
            logger.info("Firebase initialized successfully")
            
        except Exception as e:
            logger.error(f"Failed to initialize Firebase: {e}")
            raise AgentAuthenticationError(f"Firebase initialization failed: {e}")
    ...
    def register_agent(self) -> None:
        ...
        try:
            ...
            agent_doc = {
                ...
                "hardwareInfo": hardware_info,  # Include platform detection results
                "currentTask": None,
                "heartbeat": firestore.SERVER_TIMESTAMP,
                "registeredAt": firestore.SERVER_TIMESTAMP,
                "scriptsValid": scripts_valid,  # Track if training scripts are valid
                "lastValidationAt": firestore.SERVER_TIMESTAMP
            }
            
            self.db.collection("agents").document(self.agent_id).set(
                agent_doc, merge=True
            )
    ...
    def listen_for_tasks(self, callback: Callable[[Dict[str, Any]], None]) -> None:
        """
        Listen for pending training tasks.
        ...
        def on_snapshot(doc_snapshot, changes, read_time):
            for change in changes:
                if change.type.name in ['ADDED', 'MODIFIED']:
                    ...
                    # Check if task is pending and matches capabilities
        ...
        logger.info("Started listening for training tasks")
    ...
    def claim_task(self, task_id: str) -> bool:
        ...
        try:
            ...
            claimed = claim_transaction(transaction)
    ...
    def listen_for_commands(self) -> None:
        """Listen for package management and configuration commands"""
        ...
        from google.cloud.firestore_v1 import FieldFilter
        query = self.db.collection("agent_commands").where(filter=FieldFilter("agentId", "==", self.agent_id)).where(filter=FieldFilter("status", "==", "pending"))
        
        # Start listening
        self.command_listener = query.on_snapshot(on_command_snapshot)
        logger.info("Started listening for agent commands")
    ...
    def start(self) -> None:
        ...
        try:
            ...
            # Start listening for tasks and commands
            self.running = True
            ...
            self.listen_for_commands()  # Also listen for package management commands
