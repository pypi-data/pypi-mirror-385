...
class Env(abc.ABC):
    ...
    def load(prefix: str = "APP", name: str | None = None, overrides: dict | None = None) -> "Env":
        _logger = logging.getLogger("apppy.env.Env.load")
        _logger.info(
            "Loading environment",
            extra={"env_prefix": prefix, "env_name": name, "overrides": overrides},
        )
        env_prefix = prefix
        ...
        if name:
            env_name = name
            _logger.debug("Received environment name directly", extra={"env_name": env_name})
        elif f"{env_prefix}_ENV" in os.environ:
            env_name = os.environ[f"{env_prefix}_ENV"]
            _logger.debug(
                "Found environment name in environment variable",
                extra={"variable": f"{env_prefix}_ENV", "env_name": env_name},
            )
        else:
            _logger.fatal("!!! No environment name provided !!!")
            os._exit(1)
        ...
        _logger.fatal(f"!!! {env_name} environment not found !!!")
        os._exit(1)

    @staticmethod
    def find_env_file(env: "Env") -> Path | None:
        _logger = logging.getLogger("apppy.env.Env.find_env_file")
    ...
    def find_secrets_dir(env: "Env") -> Path | None:
        ...
        """
        _logger = logging.getLogger("apppy.env.Env.find_secrets_dir")
        search_roots = Env._load_search_roots(env)
        if search_roots is None:
            return None
