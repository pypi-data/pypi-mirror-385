...
# parse_natural_language는 프롬프트를 반환하므로 호출측 LLM이 처리
prompt = await parse_natural_language("30세 남자이고 흡연자야. 건강 점수는 75야.")
# 호출측 LLM이 이 프롬프트를 사용하여 JSON으로 변환
...
#### 8. `parse_natural_language(text: str) -> str`
자연어 텍스트를 구조화된 컨텍스트로 변환하는 프롬프트를 반환합니다. 호출측 LLM이 이 프롬프트를 사용하여 자연어를 JSON으로 변환합니다.
