...
class HybridDMNEngine:
        ...
        for decision in decisions:
            ...
            for table in tables:
                ...
                for output in outputs:
                    output_info = {
                        "id": output.get("id"),
                        "name": output.get("name", ""),  # name 속성 추가
    ...
    def _execute_with_xml(self, rule_name: str, input_context: Dict[str, Any]) -> DecisionResult:
        ...
        trace = []
        results = {}
        # 작업 컨텍스트: 초기 입력 + 중간 결과를 포함
        working_context = input_context.copy()
        
        # DRD 의존성 해결: 입력이 준비된 decision부터 실행
        executed = set()
        max_iterations = len(decisions) * 2  # 무한 루프 방지
        iteration = 0
        
        while len(executed) < len(decisions) and iteration < max_iterations:
            iteration += 1
            progress = False
            ...
            for decision in decisions:
                ...
                # 모든 입력이 working_context에 있는지 확인
                all_inputs_ready = True
                for input_def in inputs:
                    input_expression = input_def.get("expression", "")
                    if input_expression not in working_context:
                ...
                # 입력이 모두 준비됨 - 이 decision 실행
                progress = True
                executed.add(decision_id)
                self._execute_single_decision(decision, working_context, results, trace)
            
            if not progress:
                # 더 이상 실행할 수 있는 decision이 없으면 종료
                break
        
        # 실행되지 않은 decision들도 시도 (입력이 없어서 실패할 것)
        for decision in decisions:
            decision_id = decision.get("id", "unknown")
            if decision_id not in executed:
                self._execute_single_decision(decision, working_context, results, trace)
    ...
    def _execute_single_decision(self, decision: Dict[str, Any], working_context: Dict[str, Any], 
                                 results: Dict[str, Any], trace: List[Dict[str, Any]]):
        """단일 decision을 실행합니다"""
        decision_id = decision.get("id", "unknown")
        ...
        for table in tables:
            ...
            # 입력값 매칭 및 규칙 평가 (working_context 사용)
            matched_rule = None
            for rule in rules:
                if self._evaluate_rule_conditions(rule, inputs, working_context):
                    matched_rule = rule
                    break
            ...
            if matched_rule:
                ...
                for i, output in enumerate(outputs):
                    if i < len(output_values):
                        ...
                        output_value = self._parse_output_value(output_values[i])
                        results[output_name] = output_value
                        
                        # 중간 결과를 working_context에 추가 (다음 decision에서 사용 가능)
                        # output의 name이 있으면 그것을 사용, 없으면 decision_id 사용
                        context_key = output.get("name") or decision_id
                        if context_key:
                            working_context[context_key] = output_value
                ...
                trace.append({
                    ...
                    "working_context": working_context.copy()
                })
            else:
                trace.append({
                    "step": len(trace) + 1,
    ...
    def _OLD_execute_with_xml(self, rule_name: str, input_context: Dict[str, Any]) -> DecisionResult:
        """[DEPRECATED] 순차 실행 버전"""
        metadata = self.model_metadata[rule_name]
        decisions = metadata.get("decisions", [])
        ...
        # 각 Decision Table을 순서대로 실행 (의존성 무시)
        for decision in decisions:
            decision_id = decision.get("id", "unknown")
            decision_name = decision.get("name", "unknown")
            tables = decision.get("tables", [])
    ...
    def _evaluate_condition(self, value: Any, condition: str) -> bool:
        ...
        try:
            # 범위 조건 [min..max]
            if condition.startswith("[") and ".." in condition and condition.endswith("]"):
                range_parts = condition[1:-1].split("..")
...
def main():
    ...
    # 버전 정보 import
    try:
        from cdmn_mcp_server import __version__
    except ImportError:
        __version__ = "unknown"
    
    print(f"Starting Hybrid DMN MCP Server v{__version__}")
    print(f"cDMN API available: {CDMN_AVAILABLE}")
    print("Supports both cDMN API and standard XML parsing")
    print("Features: DRD (Decision Requirements Diagram) with dependency resolution")
