...
@mcp.prompt()
async def parse_natural_language(text: str) -> str:
    """
    자연어 텍스트를 구조화된 컨텍스트로 변환하는 프롬프트입니다.
    호출측 LLM이 이 프롬프트를 사용하여 자연어를 key-value 딕셔너리로 변환합니다.
...
다음 필드들을 추출하여 JSON 형태로 반환해주세요:
- age: 나이 (정수)
- gender: 성별 ("male" 또는 "female")
- smoker: 흡연 여부 (true/false)
- health_score: 건강 점수 (0-100, 실수)
- income: 소득 (실수)
- credit_score: 신용 점수 (300-850, 정수)
- vehicle_age: 차량 연식 (정수)
- employment_years: 고용 연수 (정수)
