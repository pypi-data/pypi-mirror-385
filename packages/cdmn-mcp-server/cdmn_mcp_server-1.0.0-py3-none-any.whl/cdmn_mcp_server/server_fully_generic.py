...
from .llm_client_mock import MockLLMClient
...
async def infer_decision(rule_name: str, context_input: Union[str, Dict[str, Any]]) -> Dict[str, Any]:
    ...
    Args:
        ...
        context_input: 자연어 문자열 또는 key-value 딕셔너리
    ...
    try:
        ...
        if isinstance(context_input, str):
            # 자연어 처리 (Mock LLM 사용)
            parsed_data = await dmn_model.llm_client.parse_natural_language(context_input)
            input_context = parsed_data
        else:
            # 딕셔너리를 그대로 사용
...
async def parse_natural_language(text: str) -> Dict[str, Any]:
    ...
    Returns:
        파싱된 컨텍스트 딕셔너리 (key-value 형태)
    """
    try:
        return await dmn_model.llm_client.parse_natural_language(text)
    except Exception as e:
        return {"error": f"Error parsing natural language: {str(e)}"}
