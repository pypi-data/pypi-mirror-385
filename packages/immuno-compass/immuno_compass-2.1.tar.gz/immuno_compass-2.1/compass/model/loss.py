...
class CEWithNaNLabelsLoss(nn.Module):
    """
    Cross-entropy loss that safely handles NaN labels and supports one-hot or class-index formats.
    """
    ...
    def forward(self, predictions, labels):
        """
        Args:
            predictions: [B, C] tensor of logits or probabilities
            labels: [B] integer tensor or [B, C] one-hot tensor (with possible NaNs)
        """
        # 1. remove NaNs
        if labels.dim() == 2:
        ...
        if not mask.any():
            # all invalid → return zero loss
            return torch.tensor(0.0, device=labels.device, dtype=predictions.dtype)

        y_pred = predictions[mask]
        y_true = labels[mask]

        # 2. handle one-hot case
        if y_true.dim() == 2:
        ...
        if self.weights is not None:
            weight = torch.tensor(self.weights, dtype=predictions.dtype, device=predictions.device)
