...
function getThrottleMs(mods) {
  ...
  if (throttleValue === void 0) return DEFAULT_THROTTLE;
...
const scrollAttributePlugin = {
  ...
  onLoad(ctx) {
    ...
    if (shouldManageGlobal) {
      ...
      const updateGlobalScroll = () => {
        ...
        if (timeDelta > 0 && delta !== 0) {
          velocity = Math.abs(delta / timeDelta * 1e3);
    ...
    const throttledElementUpdate = throttleMs <= 16 ? createRAFThrottle(executeElementExpression) : createTimerThrottle(executeElementExpression, throttleMs);
    executeElementExpression();
