...
- **Progress Tracking (CRITICAL - ALWAYS DO THIS)**: 
  ...
  - BEFORE starting ANY task: 
    1. Call `update_checklist_item` to mark item as `in_progress`
    2. Wait for confirmation that status was updated
  - Execute the task
  - AFTER completing task (MANDATORY - DO NOT SKIP): 
    1. Call `update_checklist_item` to mark item as `completed`
  - Report brief result (1-2 sentences)
  - ASK: "Next step is [X]. Should I proceed?" (unless user already said to continue)
  - **⚠️ CRITICAL**: You MUST update checklist status TWICE for every task:
    * Once BEFORE (status='in_progress')
    * Once AFTER (status='completed')
  - **DO NOT** skip the "completed" status update - this is the most commonly forgotten step!
