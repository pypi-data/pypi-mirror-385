...
class NexusResourceManager:
    ...
    def _extract_workflow_inputs(self, workflow: Workflow) -> Dict[str, Any]:
        ...
        # If workflow has explicit metadata with parameters
        if hasattr(workflow, "metadata") and "parameters" in workflow.metadata:
            return workflow.metadata["parameters"]

        # Otherwise, try to infer from nodes
        if hasattr(workflow, "_nodes"):
            for node_id, node in workflow._nodes.items():
                if hasattr(node, "get_input_schema"):
                    schema = node.get_input_schema()
                    inputs.update(schema)
    ...
    def _extract_workflow_outputs(self, workflow: Workflow) -> Dict[str, Any]:
        ...
        if hasattr(workflow, "_nodes"):
            for node_id, node in workflow._nodes.items():
                if hasattr(node, "get_output_schema"):
                    schema = node.get_output_schema()
                    outputs[node_id] = schema
    ...
    def _get_configuration(self, config_key: str) -> Dict[str, Any]:
        ...
        configs = {
            "platform": {
                ...
                "features": {
                    "auth": self.nexus.enable_auth,
                    "monitoring": self.nexus.enable_monitoring,
                    "discovery": self.nexus.enable_discovery,
