...
# Constants
DEFAULT_FITTING_JOB_FACTORY: Final[Callable[[Info], Job]] = Fit(lr=0.5)
DEFAULT_FITTING_SUMMARY_JOB_FACTORIES: Final[list[Callable[[Info], Job]]] = [
    ComputeFIM(),
    ComputeCriteria(),
]
...
class MultiStateJointModel(PriorMixin, LongitudinalMixin, HazardMixin):
    ...
    def fit(
        ...
        job_factories: Sequence[Callable[[Info], Job]] = (),
        *,
        fitter: Callable[[Info], Job] = DEFAULT_FITTING_JOB_FACTORY,
        max_iterations: int | None = None,
        n_chains: IntStrictlyPositive | None = None,
        warmup: IntNonNegative | None = None,
        n_steps: IntStrictlyPositive | None = None,
        ...
        Interally calls `do(data, [fitter, *job_factories], ...)`. See `do` for more
        details.
        ...
        Args:
            ...
            job_factories (Sequence[Callable[[Info], Job]], optional): A sequence of job
                ...
                ().
            fitter (Callable[[Info], Job], optional): The fitter job factory. By default
                it is `Fit(lr=0.5)` with `torch.optim.Adam`. Defaults to
                `DEFAULT_FITTING_JOB_FACTORY`.
            max_iterations (int | None, optional): Maximum number of iterations.
                Defaults to None.
            n_chains (IntStrictlyPositive | None, optional): Batch size used. Defaults
                to None.
        ...
        Raises:
            ValueError: If data is not None and is not the same as before seen.

        Returns:
            Metrics: The metrics computed.
        """
        if self.data is not None and data is not self.data:
            raise ValueError(
                "Data must be None before fitting, or be the same as before seen"
            )

        metrics = self.do(
            data,
            [fitter, *job_factories],
    ...
    def compute_summary(
        ...
        *,
        ...
        metrics = self.do(
            self.data,
            [*DEFAULT_FITTING_SUMMARY_JOB_FACTORIES, *job_factories],
            max_iterations=max_iterations,
            n_chains=n_chains,
            warmup=warmup,
            n_steps=n_steps,
