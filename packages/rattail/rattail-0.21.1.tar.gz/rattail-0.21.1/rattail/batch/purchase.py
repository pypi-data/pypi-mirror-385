...
class PurchaseBatchHandler(BatchHandler):
    ...
    * ``rattail.enum.PURCHASE_BATCH_MODE_COSTING``
    ...
    def get_maxlen_row_description(self):
        if hasattr(self, 'maxlen_row_description'):
            return self.maxlen_row_description
        return self.app.maxlen(self.model.PurchaseBatchRow.description)

    def setup_populate(self, batch, progress=None):
        self.maxlen_row_description = self.get_maxlen_row_description()
    ...
    def make_row_from_invoice(self, batch, invoice_row):
        ...
        maxlen = self.get_maxlen_row_description()
        if row.description and len(row.description) > maxlen:
            log.warning("will truncate description from %s to %s chars: %s",
                        len(row.description), maxlen, row.description)
            row.description = row.description[:maxlen]
