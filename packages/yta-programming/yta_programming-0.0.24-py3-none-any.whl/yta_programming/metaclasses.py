...
class SingletonMeta(type):
    ...
    If you want to handle the behaviour when the instance
    is instantiated again (but it is not because it is
    a singleton instance), create the `__reinit__` method
    and set your code here. If you want a normal singleton
    class, don't create the `__reinit__` method.
