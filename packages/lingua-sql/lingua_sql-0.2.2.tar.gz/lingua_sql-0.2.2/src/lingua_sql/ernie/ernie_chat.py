...
class ERNIEChat(LLMBase):
    def __init__(self, config=None):
        if config is None:
            raise ValueError(
                "For ERNIE, config must be provided with an api_key and model"
            )
        ...
        if not api_key:
            raise ValueError("config must contain an ERNIE api_key")
        if not model:
            raise ValueError("config must contain an ERNIE model")
    ...
    def _get_access_token(self):
        ...
        params = {
            ...
            "client_id": self.api_key.split('.')[0],  # 假设api_key格式为client_id.client_secret
