...
class YiChat(LLMBase):
    def __init__(self, config=None):
        if config is None:
            raise ValueError(
                "For Yi, config must be provided with an api_key and model"
            )
        ...
        if not api_key:
            raise ValueError("config must contain a Yi api_key")
        if not model:
            raise ValueError("config must contain a Yi model")
    
        self.model = model
        # 零一万物的API端点
        self.client = OpenAI(api_key=api_key, base_url="https://api.lingyiwanwu.com/v1")
