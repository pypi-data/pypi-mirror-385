...
class MoonshotChat(LLMBase):
    def __init__(self, config=None):
        if config is None:
            raise ValueError(
                "For Moonshot, config must be provided with an api_key and model"
            )
        ...
        if not api_key:
            raise ValueError("config must contain a Moonshot api_key")
        if not model:
            raise ValueError("config must contain a Moonshot model")
