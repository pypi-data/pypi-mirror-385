...
# between mcp-n8n and chora-composer codebases.
#
# Usage: ./scripts/handoff.sh [TARGET_INSTANCE]
#   TARGET_INSTANCE: "mcp-n8n" or "chora-composer"
...
if [ -z "$TARGET" ]; then
    ...
    echo "  TARGET_INSTANCE: 'mcp-n8n' or 'chora-composer'"
    exit 1
fi

if [ "$TARGET" != "mcp-n8n" ] && [ "$TARGET" != "chora-composer" ]; then
    echo -e "${RED}Error: TARGET_INSTANCE must be 'mcp-n8n' or 'chora-composer'${NC}"
    exit 1
fi

# Determine current instance
CURRENT_DIR=$(basename "$PWD")
if [ "$CURRENT_DIR" = "mcp-n8n" ]; then
    CURRENT="mcp-n8n"
...
if [ "$TARGET" = "mcp-n8n" ]; then
    echo "     cd /Users/victorpiper/code/mcp-n8n"
else
    echo "     cd /Users/victorpiper/code/chora-compose"
fi
echo ""
