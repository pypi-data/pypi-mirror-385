...
def emit_event(
    ...
    **metadata: Any,
    ...
    Args:
        ...
        base_dir: Base directory for event storage (defaults to .chora/memory/events)
    ...
    # Write to event log
    _write_event(event, base_dir=base_dir)

    return event


def _write_event(event: dict[str, Any], base_dir: Path | str | None = None) -> None:
    ...
    if base_dir is None:
        base_dir = Path(".chora/memory/events")
    else:
        base_dir = Path(base_dir)

    # Determine log directory (monthly partitions)
    timestamp = datetime.fromisoformat(event["timestamp"].replace("Z", "+00:00"))
    month_dir = base_dir / timestamp.strftime("%Y-%m")
