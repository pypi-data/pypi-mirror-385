...
@given(parsers.parse("chora-configs/{relative_path} exists"))
def chora_config_path_exists(
    relative_path: str, chora_configs_dir: Path, workflow_context: dict[str, Any]
):
    """Create a chora-configs file at the specified relative path."""
    # Use chora_configs_dir fixture to get the test directory
    ...
    if relative_path.endswith(".md.j2"):
        # Jinja2 template
        template_content = """# Daily Report - {{ date }}
