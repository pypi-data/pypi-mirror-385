...
if ! grep -q "## \[Unreleased\]" project/CHANGELOG.md; then
    echo -e "  ${RED}✗${NC} No [Unreleased] section in project/CHANGELOG.md"
    exit 1
fi
...
{ print }
' project/CHANGELOG.md > "$TEMP_CHANGELOG"

# Replace original with updated version
mv "$TEMP_CHANGELOG" project/CHANGELOG.md

echo -e "  ${GREEN}✓${NC} project/CHANGELOG.md updated"
echo ""
...
if ! ./scripts/pre-merge.sh; then
    ...
    echo "  git checkout pyproject.toml project/CHANGELOG.md"
    exit 1
fi
...
git add pyproject.toml project/CHANGELOG.md
