...
if pytest tests/ --cov=src/mcp_n8n --cov-report=term-missing --cov-fail-under=85 > /dev/null 2>&1; then
    COVERAGE=$(pytest tests/ --cov=src/mcp_n8n --cov-report=term 2>/dev/null | grep "TOTAL" | awk '{print $4}' || echo "unknown")
    echo -e "  ${GREEN}✓${NC} Tests passed with $COVERAGE coverage"
else
    echo -e "  ${RED}✗${NC} Tests failed or coverage below 85%"
    echo "    Run: pytest tests/ --cov=src/mcp_n8n --cov-report=term-missing"
    ERRORS=$((ERRORS + 1))
fi
echo ""

# Check 4: CHANGELOG.md has [Unreleased] entries
echo -e "${YELLOW}[4/6] Checking project/CHANGELOG.md...${NC}"
if [ -f "project/CHANGELOG.md" ]; then
    if grep -q "## \[Unreleased\]" project/CHANGELOG.md; then
        # Check if there are any entries under [Unreleased]
        if awk '/## \[Unreleased\]/,/## \[/ {if (/^### (Added|Changed|Deprecated|Removed|Fixed|Security)/ && getline && NF > 0) exit 0} END {exit 1}' project/CHANGELOG.md; then
            echo -e "  ${GREEN}✓${NC} project/CHANGELOG.md has unreleased entries"
        else
            echo -e "  ${YELLOW}⚠${NC} project/CHANGELOG.md [Unreleased] section is empty"
            echo "    Add entries describing your changes"
            WARNINGS=$((WARNINGS + 1))
        fi
    else
        echo -e "  ${YELLOW}⚠${NC} project/CHANGELOG.md missing [Unreleased] section"
        echo "    Add entries describing your changes"
        WARNINGS=$((WARNINGS + 1))
    fi
else
    echo -e "  ${RED}✗${NC} project/CHANGELOG.md not found"
    ERRORS=$((ERRORS + 1))
fi
echo ""
...
if [ $ERRORS -eq 0 ] && [ $WARNINGS -eq 0 ]; then
    ...
    echo ""
    echo "Recommended:"
    echo "  1. Address project/CHANGELOG.md entries"
    ...
    echo "  3. Update project/CHANGELOG.md with your changes"
    echo "  4. Run this script again"
    echo ""
    echo "For help:"
    echo "  - just test (run full test suite)"
    echo "  - just smoke (quick validation)"
    echo "  - project/CHANGELOG.md (see format examples)"
    exit 1
fi
