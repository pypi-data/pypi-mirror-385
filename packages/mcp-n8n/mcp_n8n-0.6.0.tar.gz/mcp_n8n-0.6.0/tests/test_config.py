...
def clear_api_keys(monkeypatch: pytest.MonkeyPatch) -> None:
    """Clear all API keys from environment before each test.

    This ensures tests run in isolation regardless of developer environment.
    Pydantic BaseSettings reads from both prefixed (MCP_N8N_*) and unprefixed
    env vars due to validation_alias, so we must clear both.
    """
    # Anthropic/Chora
    monkeypatch.delenv("ANTHROPIC_API_KEY", raising=False)
    monkeypatch.delenv("MCP_N8N_ANTHROPIC_API_KEY", raising=False)
    # Coda
    monkeypatch.delenv("CODA_API_KEY", raising=False)
    monkeypatch.delenv("MCP_N8N_CODA_API_KEY", raising=False)
    monkeypatch.delenv("CODA_FOLDER_ID", raising=False)
    monkeypatch.delenv("MCP_N8N_CODA_FOLDER_ID", raising=False)
    # n8n
    monkeypatch.delenv("N8N_EVENT_WEBHOOK_URL", raising=False)
    monkeypatch.delenv("MCP_N8N_N8N_EVENT_WEBHOOK_URL", raising=False)
