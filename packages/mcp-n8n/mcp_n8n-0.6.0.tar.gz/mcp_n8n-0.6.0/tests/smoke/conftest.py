...
def mock_backend_registry(mock_chora_backend, mock_coda_backend):
    ...
    async def route_tool_impl(tool_name: str, args: dict[str, Any]) -> dict[str, Any]:
        """Route tool call to appropriate backend."""
        if tool_name.startswith("chora:"):
            return await mock_chora_backend.call_tool(tool_name, args)
        else:
            return await mock_coda_backend.call_tool(tool_name, args)

    registry.route_tool = route_tool_impl
