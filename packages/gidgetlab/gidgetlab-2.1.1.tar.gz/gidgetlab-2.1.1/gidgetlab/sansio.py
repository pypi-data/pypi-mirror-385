...
def decipher_response(
    status_code: int, headers: Mapping[str, str], body: bytes, method: str = ""
    ...
    The optional named parameter "method" is used to handle the response
    from the HTTP HEAD method.  In a HEAD, the response contains no body
    and just the headers.  So we return the headers in place of the body.
