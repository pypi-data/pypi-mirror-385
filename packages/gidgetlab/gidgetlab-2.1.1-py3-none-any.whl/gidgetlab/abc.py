...
class GitLabAPI(abc.ABC):
    ...
    async def _make_request(
    ...
    ) -> Tuple[Union[Mapping[str, str], bytes], Opt[str]]:
        ...
        if not (response[0] == 304 and cached):
            data, self.rate_limit, more = sansio.decipher_response(
    ...
    async def head(self, url: str, params: Dict[str, Union[str, bool]] = {}) -> Any:
        """Send a ``HEAD`` request to GitLab.

        This returns the X-Gitlab-* headers from the Gitlab endpoint
        instead of the actual data (which wasn't returned anyway).
        """
        headers, _ = await self._make_request("HEAD", url, params, b"")
        return headers
