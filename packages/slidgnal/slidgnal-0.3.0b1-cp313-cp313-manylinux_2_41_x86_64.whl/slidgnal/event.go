...
// WithSignalDataMessage partially populates fields for the [Message] from the given [signalpb.DataMessage].
func (m *Message) withSignalDataMessage(data *signalpb.DataMessage) {
	m.ID = makeMessageID(m.SenderID, data.GetTimestamp())
	m.Timestamp = data.GetTimestamp() / 1000
	m.Body = data.GetBody()
	m.ReplyTo.fromSignal(data.GetQuote())
}
...
// FromSignal attempts to populate the [Reply] from data derived from Signal.
func (r *Reply) fromSignal(data *signalpb.DataMessage_Quote) {
	// Don't attempt to set fields for invalid data from Signal.
	if data == nil {
		return
	}

	r.ID = makeMessageID(data.GetAuthorAci(), data.GetId())
	r.AuthorID = data.GetAuthorAci()
	r.Body = data.GetText()
}
...
func newMessageEvent(ctx context.Context, client *signalmeow.Client, event *events.ChatEvent) (EventKind, *EventPayload) {
	var msg = Message{
		...
		IsCarbon: event.Info.Sender == client.Store.ACI,
	}
	...
	case *signalpb.DataMessage:
		...
		if d := e.GetDelete(); d != nil {
			return newDeleteEvent(ctx, client, msg.SenderID, msg.ChatID, d)
		}

		msg.withSignalDataMessage(e)
	...
	case *signalpb.TypingMessage:
		return newTypingEvent(ctx, msg.SenderID, e)
	case *signalpb.EditMessage:
		...
		msg.withSignalDataMessage(e.GetDataMessage())
