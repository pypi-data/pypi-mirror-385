...
## ⚡ New in v0.1.24: Simple & Powerful API!

APDTFlow now features an **intuitive high-level interface** that makes time series forecasting as easy as:
...
# Visualize
model.plot_forecast(show_uncertainty=True)
```

**That's it!** No complex data loaders, no manual device handling, no configuration files. Just simple, powerful forecasting. 🎯

### Why APDTFlow?

| Feature | APDTFlow | Other Libraries |
|---------|----------|-----------------|
| **Neural ODEs** | ✅ Continuous-time modeling | ❌ Discrete-time only |
| **Multi-Scale Decomposition** | ✅ Captures trends + seasonality | ⚠️ Limited |
| **Simple `fit()/predict()` API** | ✅ 5 lines of code | ⚠️ Varies |
| **Uncertainty Quantification** | ✅ Probabilistic outputs | ⚠️ Limited |
