...
def test_precision_decay():
    ...
    for d in [False, True]:
        ...
        states = models.resting_states(inhom, Nframes=3)
        weights = pig.weights(1, 1, 1, inhom, diffusivity=0)

        # inhom == 1
        states[1] = pig.run(models, inhom * 1, weights, states[0], Nt=1000, dt=1)
        ...
        # inhom == 2
        states[2] = pig.run(models, inhom * 2, weights, states[1], Nt=1, dt=1)
