...
class Workers(SodecoBase):
    ...
    def create(self, payload: Dict[str, Any]) -> dict:
        ...
        """
        try:
            # Convert flat payload to nested structure
            nested_payload = self._flat_to_nested(payload)

            # Validate the payload using WorkerCreate
            validated_data = WorkerCreate(**nested_payload)
    ...
    def _flat_to_nested(self, flat_params: Dict[str, Any]) -> Dict[str, Any]:
        """
        Convert flat parameters with prefixes to nested structure.

        This function groups flat parameters by prefix (address_, family_, contract_) and creates
        nested list structures suitable for the Sodeco API. Similar to Functions.flat_dict_to_nested_dict
        but with prefix-based grouping logic.

        Args:
            flat_params: Flat dictionary with prefixed keys (e.g., address_street, family_civil_status)

        Returns:
            Nested dictionary suitable for WorkerCreate
        ...
        Example:
            ...
            Output:
            {
                ...
                "family_status": [{"civil_status": "Single"}]
            }
        """
        # Define prefix mappings to target keys
        prefix_mappings = {
            'address_': 'address',
            'family_': 'family_status',
            'contract_': 'contract',
        ...
        worker_fields = {}
        nested_groups = {target: {} for target in prefix_mappings.values()}
        ...
        for key, value in flat_params.items():
            ...
            matched = False
            for prefix, target_key in prefix_mappings.items():
                if key.startswith(prefix):
                    field_name = key[len(prefix):]  # Remove prefix (e.g., "start_date")
                    # Keep snake_case for nested fields - Pydantic accepts them with populate_by_name=True
                    nested_groups[target_key][field_name] = value
                    matched = True
                    break

            if not matched:
                # Regular worker field (no prefix) - keep snake_case (Pydantic accepts with populate_by_name=True)
                worker_fields[key] = value

        # Build nested structure
        payload = worker_fields.copy()

        # Add required nested lists (must have at least one item)
        required_groups = ['address', 'family_status', 'contract']
        for group in required_groups:
            if nested_groups[group]:
                payload[group] = [nested_groups[group]]
            else:
                raise ValueError(f"{group.replace('_', ' ').title()} fields are required")

        # Add optional nested lists (only if they have data)
        optional_groups = ['communication', 'tax', 'replacement']
    ...
    def update(self, worker_id: str, payload: Dict[str, Any]) -> dict:
        """
