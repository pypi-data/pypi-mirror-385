...
class OS:
    ...
    # Delete folder (admin privilege may be needed)
    ...
    # Delete file (admin privilege may be needed)
    ...
    def device_info(self):
        try:
            info = {
                ...
                "manufacturer": None  # Will try to fetch below if possible
            }

            # Manufacturer (best-effort, OS dependent)
            if "windows" in self.name:
                try:
                    output = subprocess.check_output("wmic computersystem get manufacturer", shell=True)
                    info["manufacturer"] = output.decode().split("\n")[1].strip()
                except:
                    pass
            elif "linux" in self.name:
                try:
