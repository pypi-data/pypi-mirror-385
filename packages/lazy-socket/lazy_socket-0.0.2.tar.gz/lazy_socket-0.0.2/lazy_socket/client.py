...
class LazyClient:
    ...
    async def _connect(self, reconnect=True):
        ...
        if not self.uri and not self.service:
            logger.warning("No service/URI to connect to.")
            return

        uri = self.uri if self.uri else f"ws://{self.service['address']}:{self.service['port']}"
        try:
