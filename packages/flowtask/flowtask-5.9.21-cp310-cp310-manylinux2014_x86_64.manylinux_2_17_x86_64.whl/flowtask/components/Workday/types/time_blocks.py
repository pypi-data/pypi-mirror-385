...
class TimeBlockType(WorkdayTypeBase):
    ...
    async def execute(self, **kwargs) -> pd.DataFrame:
        ...
        - include_deleted: Whether to include deleted time blocks (default: False)
        """
        # Extract parameters
        ...
        if any([start_date, end_date, worker_id, status, supervisory_org]):
            payload["Request_Criteria"] = {}
            
            # Date range filter (both dates are required for Request_Criteria)
            if start_date and end_date:
                payload["Request_Criteria"]["Start_Date"] = start_date
                payload["Request_Criteria"]["End_Date"] = end_date
            
            # Worker filter
            if worker_id:
                payload["Request_Criteria"]["Worker_Reference"] = [
                    {"ID": {"type": "Employee_ID", "_value_1": worker_id}}
                ]
            
            # Status filter
            if status:
                payload["Request_Criteria"]["Status_Reference"] = [
                    {"ID": {"type": "Time_Tracking_Set_Up_Option_ID", "_value_1": status}}
                ]
            
            # Supervisory organization filter
            if supervisory_org:
                payload["Request_Criteria"]["Supervisory_Organization_Reference"] = [
