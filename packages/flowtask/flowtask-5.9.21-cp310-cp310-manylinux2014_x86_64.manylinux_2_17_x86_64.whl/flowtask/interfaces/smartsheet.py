...
class SmartSheetClient(CredentialsInterface):
    ...
    async def _get_sheet_rows_data(
        ...
        try:
            async with session.get(sheet_url, headers=headers, ssl=self.ssl_ctx) as response:
                if response.status != 200:
                ...
                # Create column mapping: column_id -> column_name
                column_map = {col["id"]: col["title"] for col in columns}

                # Find column IDs for the requested filename columns
                target_column_ids = {}
                for col in columns:
                    if col["title"] in filename_columns:
                        target_column_ids[col["id"]] = col["title"]

                # Find column IDs for filter columns (if provided)
                filter_column_ids = {}
                if column_filter:
                    for col in columns:
                        if col["title"] in column_filter:
                            filter_column_ids[col["id"]] = col["title"]

                    # Debug logging
                    if not filter_column_ids:
                        # Column names in filter not found in sheet
                        available_columns = [col["title"] for col in columns]
                ...
                for row_index, row in enumerate(rows, start=1):
                    ...
                    for cell in row.get("cells", []):
                        ...
                        if column_id in target_column_ids:
                            ...
                            row_info[column_name] = str(value) if value else ""
                    ...
                    if column_filter:
                        for filter_col, filter_value in column_filter.items():
                            row_value = row_info.get(filter_col, "")
    ...
    async def download_attachments(
        ...
        if row_id:
            attachments_url = (
                f"https://api.smartsheet.com/2.0/sheets/{sheet_id}/rows/{row_id}/attachments"
            )
        else:
            attachments_url = (
                f"https://api.smartsheet.com/2.0/sheets/{sheet_id}/attachments"
            )
            ...
            async with aiohttp.ClientSession(timeout=timeout) as session:
                ...
                async with session.get(attachments_url, headers=headers, ssl=self.ssl_ctx) as response:
                    ...
                    for attachment in attachments:
                        ...
                        if filename_pattern:
                            if not filename_pattern.search(attachment_name):
