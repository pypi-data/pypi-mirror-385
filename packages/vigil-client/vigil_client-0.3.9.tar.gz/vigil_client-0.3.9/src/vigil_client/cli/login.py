...
def login_command(
    ...
    method: str = typer.Option("device", help="Authentication method: device, interactive, or token"),
) -> None:
    ...
    try:
        if token:
            ...
            auth_manager.login_device_flow(base_url)
        elif method == "interactive":
            auth_manager.login_interactive(base_url)
        else:
            console.print(f"[red]❌ Unknown authentication method: {method}[/red]")
            console.print("   Valid methods: device, interactive, token")
            raise typer.Exit(1)
