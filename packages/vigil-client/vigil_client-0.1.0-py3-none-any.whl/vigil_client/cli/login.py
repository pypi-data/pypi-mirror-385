...
def login_command(
    ...
    if auth_manager.is_authenticated():
        ...
        console.print(f"\n[green]✅ Already authenticated as {config.user}[/green]")
        console.print(f"   Platform: {config.remote}")
...
def whoami() -> None:
    ...
    try:
        ...
        console.print("=" * 40)
        console.print(f"[green]User:[/green] {config.user}")
        if config.organization:
            console.print(f"[green]Organization:[/green] {config.organization}")
        console.print(f"[green]Platform:[/green] {config.remote}")
        console.print(f"[green]Config File:[/green] {auth_manager.CONFIG_FILE}")

        # Show token status
        if config.token:
            console.print(f"[green]Token Status:[/green] ✅ Active")
            try:
                import time
                import jwt
