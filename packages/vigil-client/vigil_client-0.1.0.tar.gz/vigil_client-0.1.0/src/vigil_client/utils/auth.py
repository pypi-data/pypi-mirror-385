...
from .jwt_validator import create_clerk_validator
...
class ClientConfig(BaseModel):
    """Complete client configuration following AUTH.md spec."""
...
<p>You can close this window and return to the terminal.</p>
    ...
    def save_config(self, config: ClientConfig) -> None:
        ...
        if config.token and config.user:
            try:
                keyring.set_password(self.KEYRING_SERVICE, config.user, config.token)
    ...
    def logout(self) -> None:
        ...
        if config.user:
            try:
                keyring.delete_password(self.KEYRING_SERVICE, config.user)
        ...
        print("   Run 'vigil login' to authenticate again")

    def get_client_config(self) -> ClientConfig:
        """Get client configuration."""
        config = self.load_config()
        if not config:
            raise ValueError("Not authenticated. Run 'vigil login' first.")
