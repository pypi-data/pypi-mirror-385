...
I want the generated code to be executed directly, so do NOT use `if __name__ == "__main__":` in your code."""
    ...
    messages_copy = agentmake(messages_copy, system=TOOL_SYSTEM, tool="execute_python", print_on_terminal=False, word_wrap=False, **kwargs)
    trial = 0
    while "```buggy_python_code\n" in messages_copy[-1].get("content", "") and trial < MAXIMUM_AUTO_HEALING:
        messages_copy = agentmake(messages_copy, tool="correct_python", print_on_terminal=False, word_wrap=False, **kwargs)
        trial += 1
    return ""
