...
class CustomV1Config:
    ...
    # When no DOB, year/month/day each return 0.5 → (0.5×0.10)+(0.5×0.06)+(0.5×0.04)=0.10
    # When DOB mismatches, returns 0.0 → adds 0.00 + penalty
    DOB_YEAR_WEIGHT = 0.10
    DOB_MONTH_WEIGHT = 0.06
    DOB_DAY_WEIGHT = 0.04
...
class CustomV1(HeuristicAlgorithm):
    ...
    def compute_score(
        ...
        if middlename_missing:
            ...
            # Get base weights
            middlename_weight = CustomV1Config.MIDDLENAME_WEIGHT  # 0.08
            surname_weight = CustomV1Config.SURNAME_WEIGHT        # 0.20
            firstname_weight = CustomV1Config.FIRSTNAME_WEIGHT    # 0.12

            # Calculate total to redistribute to
            total_base = surname_weight + firstname_weight  # 0.32

            # Distribute proportionally
            surname_boost = middlename_weight * (surname_weight / total_base)  # 0.08 * (0.20/0.32) = 0.05
            firstname_boost = middlename_weight * (firstname_weight / total_base)  # 0.08 * (0.12/0.32) = 0.03

            adjusted_weights['custom_surname_match'] = surname_weight + surname_boost  # 0.20 + 0.05 = 0.25
            adjusted_weights['custom_firstname_match'] = firstname_weight + firstname_boost  # 0.12 + 0.03 = 0.15
