...
from nomenklatura.matching.compat import name_words
...
# Feature functions using 'name' property
def custom_name_match(query: E, result: E, config: ScoringConfig) -> FtResult:
    ...
    # Get names using type_pair for proper name handling
...
def custom_name_mismatch(query: E, result: E, config: ScoringConfig) -> FtResult:
    """Penalty for low name match"""
    print(f"\n[custom_name_mismatch] INPUT:")
    print(f"  Query schema: {query.schema.name}, ID: {query.id}")
    print(f"  Result schema: {result.schema.name}, ID: {result.id}")
    
    if not (query.schema.is_a("Person") and result.schema.is_a("Person")):
        print(f"[custom_name_mismatch] Not Person entities, returning FNUL")
        result_obj = FtResult(score=FNUL, detail="Not Person entities")
        print(f"[custom_name_mismatch] OUTPUT: score={result_obj.score}")
        return result_obj
    
    name_result = custom_name_match(query, result, config)
    print(f"  Name match score: {name_result.score:.4f}, threshold: {CustomV1Config.NAME_MIN_THRESHOLD}")
    
    if name_result.score < CustomV1Config.NAME_MIN_THRESHOLD:
        print(f"  Below threshold - applying penalty!")
        result_obj = FtResult(score=1.0, detail=f"Name below threshold: {name_result.score:.2f}")
        print(f"[custom_name_mismatch] OUTPUT: score={result_obj.score}")
        return result_obj
...
class CustomV1(HeuristicAlgorithm):
    ...
    features = [
        # Main name matching features (work with 'name' property)
        ...
        Feature(func=custom_name_mismatch, weight=CustomV1Config.NAME_MISMATCH_PENALTY, qualifier=True),
