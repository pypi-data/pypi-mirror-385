...
def custom_name_match(query: E, result: E, config: ScoringConfig) -> FtResult:
    """
    Match names using two approaches and take the maximum:
    1. Full name Levenshtein comparison (after stripping titles)
    2. Sum of parts: title (0.05) + firstname (0.30) + middlename (0.15) + surname (0.50)
    """
    ...
    for q_name in query_names:
        for r_name in result_names:
            ...
            print(f"    Query parsed: {q_parsed}")
            print(f"    Result parsed: {r_parsed}")

            # === APPROACH 1: Full name Levenshtein (without title) ===
            _, q_name_no_title = NameParser.extract_title(q_name)
            _, r_name_no_title = NameParser.extract_title(r_name)

            q_norm = NameParser.normalize_name(q_name_no_title)
            r_norm = NameParser.normalize_name(r_name_no_title)

            full_name_score = ratio(q_norm, r_norm)
            print(f"    Approach 1 - Full name Levenshtein: {full_name_score:.4f}")
            ...
            # Title (weight: 0.05)
            if q_parsed['title'] and r_parsed['title']:
                if q_parsed['title'] == r_parsed['title']:
                    title_score = 1.0
                    parts_detail.append(f"title:1.0")
                else:
                    # Use Levenshtein for titles with more than 3 characters
                    if len(q_parsed['title']) > 3 or len(r_parsed['title']) > 3:
                        lev_score = ratio(q_parsed['title'], r_parsed['title'])
                        if lev_score > 0.3:
                            title_score = 1.0  # Match
                            parts_detail.append(f"title:1.0(lev:{lev_score:.2f})")
                        elif lev_score > 0.2:
                            title_score = 0.5  # Partial credit
                            parts_detail.append(f"title:0.5(lev:{lev_score:.2f})")
                        else:
                            title_score = 0.0  # No match
                            parts_detail.append(f"title:0.0(lev:{lev_score:.2f})")
                    else:
                        # Short titles must match exactly
                        title_score = 0.0
                        parts_detail.append(f"title:0.0")
                parts_score += title_score * 0.05
            elif q_parsed['title'] or r_parsed['title']:
                # Only one has title - partial credit
                parts_score += 0.5 * 0.05
                parts_detail.append(f"title:0.5")

            # Firstname (weight: 0.30)
            if q_parsed['first'] and r_parsed['first']:
                first_score = ratio(
                    NameParser.normalize_name(q_parsed['first']),
                    NameParser.normalize_name(r_parsed['first'])
                )
                parts_score += first_score * 0.30
                parts_detail.append(f"first:{first_score:.2f}")

            # Middlename (weight: 0.15)
            if q_parsed['middle'] and r_parsed['middle']:
                middle_score = ratio(
                    NameParser.normalize_name(q_parsed['middle']),
                    NameParser.normalize_name(r_parsed['middle'])
                )
                parts_score += middle_score * 0.15
                parts_detail.append(f"middle:{middle_score:.2f}")
            else:
                # Either or both missing middle name - full credit
                parts_score += 1.0 * 0.15
                parts_detail.append(f"middle:1.0(missing)")

            # Surname (weight: 0.50)
            if q_parsed['last'] and r_parsed['last']:
                # Handle compound surnames
                q_surname_parts = NameParser.parse_compound_name(q_parsed['last'])
                r_surname_parts = NameParser.parse_compound_name(r_parsed['last'])

                if len(q_surname_parts) > 1 or len(r_surname_parts) > 1:
                    # Compound surname - match last part against all parts
                    if q_surname_parts and r_surname_parts:
                        last_q = q_surname_parts[-1]
                        surname_score = max(ratio(last_q, rp) for rp in r_surname_parts)
                    else:
                        surname_score = 0.0
                else:
                    # Simple surname
                    surname_score = ratio(
                        NameParser.normalize_name(q_parsed['last']),
                        NameParser.normalize_name(r_parsed['last'])
                    )
                parts_score += surname_score * 0.50
                parts_detail.append(f"surname:{surname_score:.2f}")

            print(f"    Approach 2 - Parts sum: {parts_score:.4f} ({', '.join(parts_detail)})")

            # === Take the MAXIMUM of both approaches ===
            final_score = max(full_name_score, parts_score)

            if full_name_score > parts_score:
                detail = f"Full name match: {q_name_no_title} ~ {r_name_no_title}"
            else:
                detail = f"Parts match: {' + '.join(parts_detail)}"

            print(f"    Final score (max): {final_score:.4f}")
