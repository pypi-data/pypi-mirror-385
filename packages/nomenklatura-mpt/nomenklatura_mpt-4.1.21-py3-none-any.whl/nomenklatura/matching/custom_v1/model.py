...
class CustomV1Config:
    ...
    # Main feature weights (base weights when all properties present)
    FULLNAME_WEIGHT = 0.50  # Changed from 0.40 to 0.50
    SURNAME_WEIGHT = 0.30   # Adjusted down from 0.50
    FIRSTNAME_WEIGHT = 0.20  # Adjusted down from 0.35
    
    # DOB component weights
    DOB_YEAR_WEIGHT = 0.12
    DOB_MONTH_WEIGHT = 0.05
    DOB_DAY_WEIGHT = 0.03
    ...
    # Total weight available for redistribution
    TOTAL_WEIGHT = 1.0
...
def custom_gender_mismatch(query: E, result: E, config: ScoringConfig) -> FtResult:
    ...
    if not query_genders or not result_genders:
        print(f"[custom_gender_mismatch] No genders to compare, returning FNUL (neutral)")
        result_obj = FtResult(score=FNUL, detail=None)
        print(f"[custom_gender_mismatch] OUTPUT: score={result_obj.score}")
        return result_obj
...
def custom_country_mismatch(query: E, result: E, config: ScoringConfig) -> FtResult:
    ...
    if not query_countries or not result_countries:
        print(f"[custom_country_mismatch] No countries to compare, returning FNUL (neutral)")
        result_obj = FtResult(score=FNUL, detail=None)
        print(f"[custom_country_mismatch] OUTPUT: score={result_obj.score}")
        return result_obj
...
def custom_dob_year_mismatch(query: E, result: E, config: ScoringConfig) -> FtResult:
    ...
    if not query_dobs or not result_dobs:
        print(f"[custom_dob_year_mismatch] No DOBs to compare, returning FNUL (neutral)")
        result_obj = FtResult(score=FNUL, detail=None)
        print(f"[custom_dob_year_mismatch] OUTPUT: score={result_obj.score}")
        return result_obj
...
class CustomV1(HeuristicAlgorithm):
    ...
    def compute_score(
        ...
        """
        Compute final score with dynamic weight adjustment
        If properties are missing, redistribute their weights to name matching
        """
        print(f"\n[compute_score] INPUT:")
        print(f"  Scores: {scores}")
        print(f"  Weights: {weights}")
        
        # Check which properties are present (scored as 1.0 means neutral/present)
        has_dob = (
            scores.get('custom_dob_year_match', FNUL) == 1.0 or
            scores.get('custom_dob_month_match', FNUL) == 1.0 or
            scores.get('custom_dob_day_match', FNUL) == 1.0
        )
        
        # Calculate total DOB weight
        dob_total_weight = (
            CustomV1Config.DOB_YEAR_WEIGHT +
            CustomV1Config.DOB_MONTH_WEIGHT +
            CustomV1Config.DOB_DAY_WEIGHT
        )
        
        # Adjust weights dynamically
        adjusted_weights = weights.copy()
        
        # If DOB is missing (all returned 1.0 as neutral), redistribute its weight
        if not has_dob or all(
            scores.get(f'custom_dob_{t}_match', FNUL) == 1.0 
            for t in ['year', 'month', 'day']
        ):
            print(f"  DOB missing or neutral - redistributing {dob_total_weight:.4f} weight to name features")
            
            # Redistribute DOB weight proportionally to name features
            name_features = ['custom_name_match', 'custom_surname_match', 'custom_firstname_match']
            total_name_weight = sum(weights.get(f, 0) for f in name_features)
            
            for feat in name_features:
                if feat in adjusted_weights and total_name_weight > 0:
                    proportion = weights.get(feat, 0) / total_name_weight
                    boost = dob_total_weight * proportion
                    adjusted_weights[feat] = weights.get(feat, 0) + boost
                    print(f"    Boosting {feat}: {weights.get(feat, 0):.4f} → {adjusted_weights[feat]:.4f}")
            
            # Zero out DOB weights since they're not contributing
            for dob_feat in ['custom_dob_year_match', 'custom_dob_month_match', 'custom_dob_day_match']:
                adjusted_weights[dob_feat] = 0
        
        # Calculate main features with adjusted weights
        mains: List[float] = []
        for feat in cls.features:
            if feat.qualifier:
                continue
            score = scores.get(feat.name, FNUL)
            weight = adjusted_weights.get(feat.name, FNUL)
            weighted = score * weight
            mains.append(weighted)
        ...
        for feat in cls.features:
            ...
            qual_weight = adjusted_weights.get(feat.name, FNUL)
