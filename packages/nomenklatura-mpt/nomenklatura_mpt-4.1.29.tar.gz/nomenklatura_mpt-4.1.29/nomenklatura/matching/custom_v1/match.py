...
def custom_dob_year_match(query: E, result: E, config: ScoringConfig) -> FtResult:
    ...
    - Only year matches: 0.75 (contributes 0.15)
    - Year + 01-01 (likely fake): 0.6 (contributes 0.12)
    - Year + month match, day different: 0.2 (contributes 0.04)
    ...
    for q_dob, r_dob in zip(query_dobs, result_dobs):
        ...
        if len(q_parts) >= 3 and len(r_parts) >= 3:
            ...
            # Check for 01-01 pattern (likely fake/unknown DOB)
            if (q_month == '01' and q_day == '01') or (r_month == '01' and r_day == '01'):
                print(f"    01-01 pattern detected (likely fake) - returning 0.6")
                result_obj = FtResult(score=0.6, detail=f"Year match with 01-01 pattern: {q_year}")
                print(f"[custom_dob_year_match] OUTPUT: score={result_obj.score}")
                return result_obj
        ...
        # Only year available - partial match
...
def custom_dob_month_match(query: E, result: E, config: ScoringConfig) -> FtResult:
    """DOB month matching is now handled by custom_dob_year_match - this returns FNUL"""
    return FtResult(score=FNUL, detail="Handled by year match")


def custom_dob_day_match(query: E, result: E, config: ScoringConfig) -> FtResult:
    """DOB day matching is now handled by custom_dob_year_match - this returns FNUL"""
    return FtResult(score=FNUL, detail="Handled by year match")
