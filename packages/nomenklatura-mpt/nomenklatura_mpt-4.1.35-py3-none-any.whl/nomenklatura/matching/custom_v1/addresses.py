"""Address matching functions for custom_v1 algorithm"""
...
def _address_match(query_addrs: List[str], result_addrs: List[str]) -> FtResult:
    ...
    for query_tokens, result_tokens in product(query_norms, result_norms):
        ...
        if len(overlap) == len(query_tokens) or len(overlap) == len(result_tokens):
            detail = f"Address matches subset: {' '.join(sorted(overlap)[:5])}"
        ...
        if score > max_result.score:
            detail = f"Matched addresses: {query_fuzzy[:30]} <-> {result_fuzzy[:30]}"
            max_result = FtResult(score=score, detail=detail)

    return max_result


def custom_address_match(query: E, result: E, config: ScoringConfig) -> FtResult:
    """
    Match addresses between entities.
