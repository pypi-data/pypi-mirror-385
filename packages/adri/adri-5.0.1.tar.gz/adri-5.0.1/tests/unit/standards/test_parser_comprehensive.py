...
class TestStandardsParserComprehensive:
    ...
    def test_standards_parser_initialization(self):
        ...
        # Test initialization (StandardsParser constructor takes no config parameter)
        configured_parser = StandardsParser()
    ...
    def test_yaml_file_parsing(self, temp_workspace):
        ...
        try:
            ...
            assert parsed_standard["standards"]["version"] == self.comprehensive_standard["standards"]["version"]
            ...
            assert parsed_minimal["standards"]["id"] == self.minimal_standard["standards"]["id"]
    ...
    def test_malformed_yaml_handling(self, temp_workspace):
        ...
        try:
            ...
            with pytest.raises((ValidationError, Exception)):
                test_parser.parse_standard("malformed")
            ...
            with pytest.raises((ValidationError, Exception)):
                test_parser.parse_standard("empty")
            ...
            with pytest.raises((ValidationError, Exception)):
                test_parser.parse_standard("wrong_structure")
    ...
    def test_file_system_error_handling(self, temp_workspace):
        ...
        try:
            ...
            with pytest.raises((FileNotFoundError, ValidationError, Exception)):
                test_parser.parse_standard("nonexistent_standard")
            ...
            with self.error_simulator.simulate_file_system_error("permission"):
                with pytest.raises((PermissionError, ValidationError, Exception)):
                    test_parser.parse_standard("fs_test_standard")
            ...
            with self.error_simulator.simulate_file_system_error("disk_full"):
                ...
                try:
                    result = test_parser.parse_standard("fs_test_standard")
    ...
    def test_schema_file_integration(self, temp_workspace):
        ...
        validation_result = self.parser.validate_standard_file(str(test_file))
    ...
    def test_validator_engine_integration(self, temp_workspace):
        ...
        try:
            ...
            parsed_standard = test_parser.parse_standard("integration_test")
            ...
            with patch('src.adri.validator.engine.ValidationEngine') as mock_engine:
                ...
                # Test integration
                engine = mock_engine()
                result = engine.assess(data=[], standard=parsed_standard)
    ...
    def test_parsing_performance(self, temp_workspace, performance_tester):
        ...
        try:
            ...
            small_result = test_parser.parse_standard("small_standard")
        ...
        try:
            ...
            start_time = time.time()
            large_result = test_parser.parse_standard("large_standard")
        ...
        try:
            ...
            for i in range(5):
                standard = {
                    **self.comprehensive_standard,
                    "standards": {
                        **self.comprehensive_standard["standards"],
            ...
            def parse_standard_with_id(standard_name):
                ...
                test_parser = StandardsParser()
                result = test_parser.parse_standard(standard_name)
                return {
    ...
    def test_standard_metadata_extraction(self, temp_workspace):
        ...
        try:
            ...
            parsed_standard = test_parser.parse_standard("metadata_test")
            metadata = parsed_standard["standards"]
            ...
            assert metadata["id"] == self.comprehensive_standard["standards"]["id"]
            assert metadata["name"] == self.comprehensive_standard["standards"]["name"]
            assert metadata["version"] == self.comprehensive_standard["standards"]["version"]
            assert metadata["authority"] == self.comprehensive_standard["standards"]["authority"]
    ...
    def test_schema_validation_edge_cases(self, temp_workspace):
        ...
        try:
            ...
            try:
                result = test_parser.parse_standard("extra_fields")
                # If parsing succeeds, verify core structure is intact
                assert result["standards"]["id"] is not None
        ...
        validation_result = self.parser.validate_standard_file(str(null_file))
        ...
        # Check validation result - may pass or fail depending on validator strictness
        validation_result = self.parser.validate_standard_file(str(invalid_types_file))
    ...
    def test_standard_versioning_support(self, temp_workspace):
        ...
        try:
            ...
            assert result["standards"]["version"] == "4.0.0"
            ...
            try:
                older_result = test_parser.parse_standard("older_version")
                assert older_result["standards"]["version"] == "3.0.0"
    ...
    def test_config_integration(self, temp_workspace):
        ...
        try:
            ...
            configured_parser = StandardsParser()
            result = configured_parser.parse_standard("config_integration")

            assert result is not None
            assert result["standards"]["id"] == self.comprehensive_standard["standards"]["id"]
...
@pytest.mark.system_infrastructure
class TestStandardsParserQualityValidation:
