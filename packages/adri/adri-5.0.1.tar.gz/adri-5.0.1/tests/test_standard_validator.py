...
This test suite validates the StandardValidator implementation including:
...
from src.adri.standards.validator import StandardValidator, get_validator
from src.adri.standards.exceptions import (
    ValidationResult,
    ValidationError,
    ValidationSeverity,
    SchemaValidationError
)
from src.adri.standards.schema import StandardSchema


# Fixture paths
FIXTURES_DIR = Path(__file__).parent / "fixtures" / "standards"
...
class TestStandardSchema:
    ...
    def test_validate_top_level_structure_missing_standards(self):
        """Missing 'standards' section should fail."""
        standard = {
            "requirements": {}
        }
        errors = StandardSchema.validate_top_level_structure(standard)
        assert len(errors) == 1
        assert "standards" in errors[0].lower()
...
class TestStandardValidatorBasics:
    """Test basic StandardValidator functionality."""
    ...
    def test_validate_invalid_missing_standards_section(self):
        """Missing 'standards' section should fail."""
        standard = {
            "requirements": {
                "dimension_requirements": {
        ...
        assert not result.is_valid
        assert result.has_errors
        assert any("standards" in err.message.lower() for err in result.errors)
...
class TestStandardValidatorFixtures:
    ...
    def test_validate_minimal_valid_fixture(self):
        ...
        validator = StandardValidator()
        result = validator.validate_standard_file(str(fixture_path), use_cache=False)
    ...
    def test_validate_missing_standards_section_fixture(self):
