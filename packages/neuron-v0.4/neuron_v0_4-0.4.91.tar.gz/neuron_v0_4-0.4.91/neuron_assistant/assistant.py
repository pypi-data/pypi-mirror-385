...
class NeuronAssistant:
    ...
    def _first_time_setup(self):
        """Handle first-time setup with improved user experience"""
        print("\n" + "=" * 60)
        print("🎉 WELCOME TO NEURON AI ASSISTANT!")
        print("=" * 60)
        print("This is your first time running Neuron. Let's get you set up!")
        
        # Get user name with better validation
        print(f"\n👤 STEP 1: Tell us your name")
        print(f"💡 Note: I'll use your name to make our conversations more personal!")
        while True:
            ...
            if self.user_name and len(self.user_name) <= 50:
                ...
                print(f"🤝 I'll remember your name and use it in our conversations!")
                break
            print("❌ Please enter a valid name (1-50 characters)")
        
        # Model selection with enhanced experience
        print(f"\n🤖 STEP 2: Choose your AI model")
        self._show_model_selection()
        
        # Create and save config
        print(f"\n💾 STEP 3: Saving your preferences...")
        self.identity = {
        ...
        print(f"✅ Next time you run Neuron, it will remember your choices!")
        print("=" * 60)
    ...
    def _load_existing_config(self):
        ...
        try:
            ...
            print(f"📊 System: {self.device.upper()} | RAM: {self.ram_gb}GB")
            if self.device == "cuda":
                print(f"🎮 GPU VRAM: {self.gpu_vram_gb}GB")
    ...
    def _show_model_selection(self):
        """Show model selection menu with PC requirement analysis"""
        print("\n" + "=" * 60)
        print("MODEL SELECTION")
        print("=" * 60)
        
        # Analyze system specs for recommendations
        self._analyze_system_specs()
        
        print("\nAvailable Models:")
        models = {
            "1": {
                "name": "openlm-research/open_llama_7b_v2",
                "display": "OpenLLaMA 7B v2",
                "type": "Open Source LLaMA (CPU/GPU)",
                "size": "13.5 GB",
                "ram_required": 8,
                "vram_required": 0,
                "mode": "mistral",
                "description": "High-quality open source model, works on CPU and GPU"
            },
            "2": {
                ...
                "vram_required": 12,
                "mode": "mistral",
                "description": "High-quality responses, needs powerful hardware"
            },
        }

        # Show models with detailed compatibility analysis
        for k, m in models.items():
            ...
            if m["ram_required"] > self.ram_gb:
                compatible = False
                recommendation = "❌ Not enough RAM"
            elif m["vram_required"] > 0 and self.gpu_vram_gb < m["vram_required"]:
                compatible = False
                recommendation = "❌ Not enough VRAM"
            else:
                # Determine recommendation based on system specs
                if k == "1":  # OpenLLaMA 7B v2
                    if self.ram_gb >= 8:
                        recommendation = "✅ Recommended for your PC"
                    else:
                        recommendation = "⚠️  May be slow on your PC"
                else:  # Mistral
                    if self.device == "cuda" and self.gpu_vram_gb >= 12 and self.ram_gb >= 16:
                        recommendation = "✅ Recommended for your PC"
                    elif self.device == "cuda" and self.gpu_vram_gb >= 8 and self.ram_gb >= 16:
                        recommendation = "⚠️  Will work but may be slow"
                    else:
                        recommendation = "❌ Not recommended for your PC"
        ...
        # Smart recommendation based on comprehensive analysis
        recommended_model = self._get_smart_recommendation(models)
        default_choice = recommended_model["choice"]
        reason = recommended_model["reason"]
        
        print(f"\n" + "=" * 60)
        print(f"🎯 RECOMMENDED FOR YOUR PC: Option {default_choice}")
        print(f"💡 Reason: {reason}")
        print("=" * 60)
        
        while True:
            user_choice = input(f"\nSelect model [default {default_choice}]: ").strip() or default_choice
    ...
    def _analyze_system_specs(self):
        """Analyze system specifications for model recommendations"""
        ...
        # Determine PC tier
        if self.device == "cuda" and self.gpu_vram_gb >= 12 and self.ram_gb >= 16:
            self.pc_tier = "High-End"
            print(f"   • PC Tier: 🚀 {self.pc_tier} (Gaming/Workstation)")
        elif self.device == "cuda" and self.gpu_vram_gb >= 6 and self.ram_gb >= 12:
            self.pc_tier = "Mid-Range"
            print(f"   • PC Tier: 💻 {self.pc_tier} (Gaming/Content Creation)")
        elif self.ram_gb >= 8:
            self.pc_tier = "Standard"
            print(f"   • PC Tier: 🖥️  {self.pc_tier} (Office/General Use)")
        else:
            self.pc_tier = "Basic"
            print(f"   • PC Tier: 📱 {self.pc_tier} (Light Usage)")

    def _get_smart_recommendation(self, models):
        """Get smart model recommendation based on system analysis"""
        # High-end systems: Recommend Mistral if GPU is powerful enough
        if self.pc_tier == "High-End" and self.device == "cuda" and self.gpu_vram_gb >= 12:
            return {
                "choice": "2",
                "reason": "Your high-end PC with powerful GPU can handle Mistral for best quality"
            }
        
        # Mid-range systems: Recommend OpenLLaMA for good balance
        elif self.pc_tier == "Mid-Range" or (self.ram_gb >= 8 and self.ram_gb < 16):
            return {
                "choice": "1", 
                "reason": "OpenLLaMA 7B v2 offers great performance for your system"
            }
        
        # Default to Mistral for all other cases (as requested)
        else:
            return {
                "choice": "2",
                "reason": "Mistral 7B is the default choice for optimal performance"
            }
    ...
    def _load_model(self):
        ...
        try:
            if self.model_mode == "mistral":
                self._load_mistral_model()
            else:
                # All models now use the same loading method
                self._load_mistral_model()
    ...
    def _load_mistral_model(self):
        """Load model with proper error handling (works for both OpenLLaMA and Mistral)"""
        try:
    ...
    def _setup_identity(self):
        ...
        self.base_prompt = (
            ...
            f"- Always address {self.user_name} by their name when appropriate\n"
            f"- Be friendly, helpful, and personal in your responses\n"
            f"- Remember that you're {self.user_name}'s personal AI assistant\n\n"
            f"Your purpose: {self.purpose}\n"
            f"Respond naturally, intelligently, and helpfully to {self.user_name}."
        )
