...
import re
import yaml
from engine import Node, BatchNode
...
class FetchRepo(Node):
    ...
    def post(self, shared, prep_res, exec_res):
        ...
        # Save state
        save_state(shared["project_name"], "fetch", {"files": exec_res})


class IdentifyAbstractions(Node):
    # Token limit for context (leave some buffer for prompt template)
    MAX_CONTEXT_TOKENS = 70000  # Conservative limit to fit within rate limits and total token limits
    ...
    def prep(self, shared):
        ...
        # Split files into batches based on token limits
        batches = self._split_files_into_batches(files_data)

        print(f"Split {len(files_data)} files into {len(batches)} batches for processing.")
    ...
    def _split_files_into_batches(self, files_data):
        ...
        for i, (path, content) in enumerate(files_data):
            ...
            if current_tokens + entry_tokens > self.MAX_CONTEXT_TOKENS and current_batch:
                batches.append({
                    "files": current_batch,
    ...
    def _process_single_batch(self, batch, batch_num, total_batches, project_name,
                             language, use_cache, max_abstraction_num):
        """Process a single batch of files to identify abstractions."""
        files = batch["files"]
        ...
        context = ""
        file_info = []
        for original_idx, path, content in files:
            entry = f"--- File Index {original_idx}: {path} ---\n{content}\n\n"
            context += entry
            file_info.append((original_idx, path))
        ...
        # Adjust abstraction count for batch
        batch_abstraction_target = max(3, max_abstraction_num // total_batches)

        batch_note = ""
        if total_batches > 1:
            batch_note = f"\nNote: This is batch {batch_num} of {total_batches}. Focus on the {batch_abstraction_target} most important abstractions in THIS batch of files.\n"
...
Identify the top {batch_abstraction_target} core most important abstractions in this set of files to help those new to the codebase.
...
# ... up to {batch_abstraction_target} abstractions
```"""
        ...
        available_indices = set(idx for idx, _, _ in files)
            ...
            for idx_entry in item["file_indices"]:
                try:
                    ...
                    if idx not in available_indices:
                        print(f"Warning: Index {idx} not in current batch, skipping.")
                        continue

                    validated_indices.append(idx)
                except (ValueError, TypeError):
                    print(f"Warning: Could not parse index from entry: {idx_entry}")
                    continue

            if validated_indices:  # Only add if we have valid indices
    ...
    def exec(self, prep_res):
        batches = prep_res["batches"]
        project_name = prep_res["project_name"]
        language = prep_res["language"]
        use_cache = prep_res["use_cache"]
        max_abstraction_num = prep_res["max_abstraction_num"]
        total_files = prep_res["total_files"]

        # Try to load cached results
        cached_state = load_state(project_name, "identify")
        if cached_state:
            print(f"✓ Resuming from cached identification results.")
            return cached_state["abstractions"]
        ...
        for i, batch in enumerate(batches, 1):
            ...
            batch_cache_key = f"identify_batch_{i}"
            cached_batch = load_state(project_name, batch_cache_key)

            if cached_batch:
                print(f"✓ Batch {i}/{len(batches)}: Loaded from cache")
                batch_abstractions = cached_batch["abstractions"]
            else:
                print(f"Processing batch {i}/{len(batches)}...")
                batch_abstractions = self._process_single_batch(
                    batch, i, len(batches), project_name,
                    language, use_cache, max_abstraction_num
                )
                # Save batch result
                save_state(project_name, batch_cache_key, {"abstractions": batch_abstractions})
                print(f"Batch {i}: Found {len(batch_abstractions)} abstractions.")

            all_abstractions.extend(batch_abstractions)

        # If we processed multiple batches, merge and deduplicate
        if len(batches) > 1:
            print(f"Merging abstractions from {len(batches)} batches...")
            merged_abstractions = self._merge_abstractions(
                all_abstractions, max_abstraction_num,
                project_name, language, use_cache, total_files
            )
            print(f"After merging: {len(merged_abstractions)} abstractions.")
            return merged_abstractions

        print(f"Identified {len(all_abstractions)} abstractions.")
        return all_abstractions

    def _merge_abstractions(self, all_abstractions, max_abstraction_num,
                           project_name, language, use_cache, total_files):
        """Merge and select the most important abstractions from all batches."""
        # Create a summary of all abstractions for LLM to select the best ones
        abstraction_summaries = []
        for i, abstr in enumerate(all_abstractions):
            file_indices_str = ", ".join(map(str, abstr["files"]))
            summary = f"""
{i}. {abstr['name']}
   Description: {abstr['description']}
   Relevant file indices: [{file_indices_str}]
"""
            abstraction_summaries.append(summary)

        abstractions_text = "\n".join(abstraction_summaries)

        language_instruction = ""
        if language.lower() != "english":
            language_instruction = f"IMPORTANT: Keep the `name` and `description` in **{language.capitalize()}** as provided.\n\n"

        prompt = f"""
For the project `{project_name}`:

We identified {len(all_abstractions)} abstractions from analyzing the codebase in batches.
Now select the TOP {max_abstraction_num} most important and representative abstractions.

Consider:
- Core functionality and entry points
- Avoiding redundancy (merge similar concepts)
- Coverage across different parts of the codebase

All identified abstractions:
{abstractions_text}

{language_instruction}Select the top {max_abstraction_num} abstractions by providing their indices (0-{len(all_abstractions)-1}).
You can optionally merge similar abstractions by combining their file_indices.

Format the output as YAML:

```yaml
selected:
  - index: 0  # Keep this abstraction as-is
  - index: 2  # Keep this one
  - merge: [3, 5]  # Merge these two abstractions
    name: |
      Merged Name
    description: |
      Combined description
# ... up to {max_abstraction_num} entries
```"""

        response = call_llm(prompt, use_cache=use_cache)

        yaml_str = response.strip().split("```yaml")[1].split("```")[0].strip()
        selection_data = yaml.safe_load(yaml_str)

        if not isinstance(selection_data, dict) or "selected" not in selection_data:
            raise ValueError("Invalid merge response format")

        merged_abstractions = []
        for entry in selection_data["selected"]:
            if "index" in entry:
                # Keep single abstraction
                idx = entry["index"]
                if 0 <= idx < len(all_abstractions):
                    merged_abstractions.append(all_abstractions[idx])
            elif "merge" in entry:
                ...
                for idx in indices:
                    if 0 <= idx < len(all_abstractions):
                        merged_files.update(all_abstractions[idx]["files"])

                merged_abstractions.append({
                    "name": entry.get("name", "Merged Abstraction"),
                    "description": entry.get("description", "Merged abstraction"),
                    "files": sorted(list(merged_files)),
                })

        return merged_abstractions[:max_abstraction_num]

    def post(self, shared, prep_res, exec_res):
        shared["abstractions"] = exec_res
        # Save final state
        save_state(shared["project_name"], "identify", {"abstractions": exec_res})
...
class CombineTutorial(Node):
    def prep(self, shared):
        ...
        single_file = shared.get("single_file_output", True)  # Default to single TUTORIAL.md file
        ...
        return {
            ...
            "chapters_content": chapters_content,  # For single file mode
        }
    ...
    def exec(self, prep_res):
        ...
        single_file = prep_res["single_file"]
        ...
        if single_file:
            # Generate single TUTORIAL.md file with all content
            tutorial_filepath = os.path.join(output_path, "TUTORIAL.md")

            # Combine index content and all chapters
            ...
            with open(tutorial_filepath, "w", encoding="utf-8") as f:
                f.write(full_content)
            print(f"  - Wrote {tutorial_filepath}")
        else:
            # Write index.md
