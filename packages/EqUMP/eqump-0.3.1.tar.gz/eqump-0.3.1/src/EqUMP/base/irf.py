...
# Type alias for collections of items (used in linking/equating functions)
# Maps item IDs (hashable keys) to their parameter dictionaries
ItemParamsCollection = Dict[Hashable, ItemParams]

# Type alias for collections of IRF objects (new API)
...
class IRF:
    ...
    def __init__(
        ...
        self.n_categories = self._get_n_categories()
        self.is_dichotomous = self.n_categories == 2
        
        # Set scores
        if scores is None:
            self.scores = np.arange(self.n_categories, dtype=float)
        else:
            self.scores = np.asarray(scores, dtype=float)
            if len(self.scores) != self.n_categories:
                raise ValueError(
                    f"Length of scores ({len(self.scores)}) must match "
                    f"number of categories ({self.n_categories})"
                )
    
    def _get_n_categories(self) -> int:
    ...
    def prob(
        ...
        if np.isscalar(theta):
            return irf(float(theta), self.params, self.model, self.D)
        ...
        rows = []
        for th in theta_arr:
            df_row = irf(float(th), self.params, self.model, self.D)
            rows.append(df_row)
        ...
        **mpl_kwargs
        ...
        if categories is None:
            if self.is_dichotomous:
                selected_cols = ["1"]  # P(correct)
            else:
                selected_cols = [str(i) for i in range(self.n_categories)]
        else:
        ...
        # Prepare labels
        if label is None:
            if self.is_dichotomous and selected_cols == ["1"]:
                labels = ["P(X=1)"]
            else:
