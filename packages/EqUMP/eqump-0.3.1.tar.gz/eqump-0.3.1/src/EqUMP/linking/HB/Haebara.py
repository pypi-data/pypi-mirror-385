...
def haebara_loss(
    ...
    for x, w in zip(nodes_old, weights_old):
        ...
        for c_new, c_old in zip(common_new, common_old):
            P_new_t = items_new_t[c_new].prob(x)
            P_old = items_old[c_old].prob(x)
            d = (P_old - P_new_t).to_numpy(dtype=float).ravel()
            diff1 += float(d @ d) * w
    
    M1 = sum(items_old[c].n_categories for c in common_old)
    ...
    for x, w in zip(nodes_new, weights_new):
        x = float(x); w = float(w)
        for c_new, c_old in zip(common_new, common_old):
            P_new = items_new[c_new].prob(x)
            P_old_t = items_old_t[c_old].prob(x)
            d = (P_new - P_old_t).to_numpy(dtype=float).ravel()
            diff2 += float(d @ d) * w

    M2 = sum(items_new[c].n_categories for c in common_new)
...
def haebara(
    ...
    The Haebara method uses the item response function of common items.
