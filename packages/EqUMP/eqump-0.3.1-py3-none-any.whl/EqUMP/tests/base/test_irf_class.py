...
class TestIRFClass:
    ...
    def test_prob_single_theta(self):
        ...
        assert "0" in prob.columns and "1" in prob.columns
        # At theta=b=0, with a=1, D=1.702, should be close to 0.5
        assert 0.4 < prob.loc[0.0, "1"] < 0.6
    ...
    def test_prob_polytomous(self):
        ...
        assert list(prob.columns) == ["0", "1", "2", "3"]
    ...
    def test_extreme_theta_values(self):
        ...
        assert prob_low.loc[-10.0, "1"] < 0.01  # Probability of correct is very low
        # Very high theta
        prob_high = item.prob(theta=10.0)
        assert prob_high.loc[10.0, "1"] > 0.99  # Probability of correct is very high
