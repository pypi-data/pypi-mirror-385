...
@dataclass
class CtecsQueryUpgradeOrderPriceV42Request:
    regionID: str  # 资源池ID
    resourceID: str  # 资源ID
    ...
    def to_dict(self) -> dict:
        if self.resourceType not in ResourceTypeEnum.get_all_values():
            raise ValueError(
                f"Invalid value for resourceType: {self.resourceType}. Valid values are: {ResourceTypeEnum.get_all_values()}")
...
@dataclass
class CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse:
    resourceType: Optional[str] = None  # 资源类型
    totalPrice: Optional[float] = None  # 总价格，单位CNY
    finalPrice: Optional[float] = None  # 最终价格，单位CNY

    @classmethod
    def from_json(cls, json_data: dict) -> Optional['CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse']:
        if not json_data:
            return None
        return cls(
...
@dataclass
class CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesResponse:
    serviceTag: Optional[str] = None  # 服务类型
    totalPrice: Optional[int] = None  # 总价格，单位CNY
    finalPrice: Optional[int] = None  # 最终价格，单位CNY
    orderItemPrices: Optional[List[Optional[CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse]]] = None  # 资源价格信息

    @classmethod
    def from_json(cls, json_data: dict) -> Optional['CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesResponse']:
        if not json_data:
            return None
        ...
        if 'orderItemPrices' in json_data and json_data.get('orderItemPrices') is not None:
            order_item_price = [
                CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse.from_json(item)
...
@dataclass
class CtecsQueryUpgradeOrderPriceV42ReturnObjResponse:
    """成功时返回的数据，参见returnObj对象结构"""
    totalPrice: Optional[float] = None  # 总价格，单位CNY
    discountPrice: Optional[float] = None  # 折后价格，单位CNY
    finalPrice: Optional[float] = None  # 最终价格，单位CNY
    subOrderPrices: Optional[List[Optional[CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesResponse]]] = None  # 子订单价格信息

    @classmethod
    def from_json(cls, json_data: dict) -> Optional['CtecsQueryUpgradeOrderPriceV42ReturnObjResponse']:
        if not json_data:
            return None
        ...
        if 'subOrderPrices' in json_data and json_data.get('subOrderPrices') is not None:
            sub_order_price = [
                CtecsQueryUpgradeOrderPriceV42ReturnObjSubOrderPricesResponse.from_json(sub_order)
...
@dataclass
class CtecsQueryUpgradeOrderPriceV42Response:
    """询价API响应
    包含状态码、错误信息和返回对象
    """
    ...
    returnObj: Optional[CtecsQueryUpgradeOrderPriceV42ReturnObjResponse] = None  # 返回参数
    error: Optional[str] = None  # 错误码，请求成功时不返回

    @classmethod
    def from_json(cls, json_data: dict) -> 'CtecsQueryUpgradeOrderPriceV42Response':
        """从JSON数据创建响应对象"""
        if not json_data:
        ...
        if 'returnObj' in json_data and json_data['returnObj'] is not None:
            return_obj = CtecsQueryUpgradeOrderPriceV42ReturnObjResponse.from_json(json_data['returnObj'])
...
class CtecsQueryUpgradeOrderPriceV42Api:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtecsQueryUpgradeOrderPriceV42Request) -> CtecsQueryUpgradeOrderPriceV42Response:
        """执行API请求
        Args:
            credential: 认证凭证
            client: CTYun客户端
        ...
        """
        url = f"{endpoint.rstrip('/')}/v4/upgrade-order/query-price"
        try:
            ...
            return CtecsQueryUpgradeOrderPriceV42Response.from_json(response.json())
