...
@dataclass
class CtecsQueryNewOrderPriceV42OrderDisksRequest:
...
@dataclass
class CtecsQueryNewOrderPriceV42DisksRequest:
    diskType: str  # 磁盘类型(SAS:高IO,SATA:普通IO,SSD:超高IO,FAST-SSD:极速型SSD)
    diskSize: int  # 磁盘大小
...
@dataclass
class CtecsQueryNewOrderPriceV42Request:
    regionID: str  # 资源池ID
    ...
    disks: Optional[List[Optional[CtecsQueryNewOrderPriceV42DisksRequest]]] = None  # 数据盘信息，当resourceType为VM选填，订购云主机时如果成套订购数据盘时需要该字段
    ...
    orderDisks: Optional[List[Optional[CtecsQueryNewOrderPriceV42OrderDisksRequest]]] = None  # 物理机云硬盘信息，当resourceType为BMS选填
...
@dataclass
class CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse:
    """子订单项价格信息响应"""
    ...
    @classmethod
    def from_json(cls, json_data: dict) -> Optional['CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse']:
        """从JSON数据创建对象"""
        if not json_data:
            return None
        return cls(
...
@dataclass
class CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesResponse:
    """子订单价格信息响应"""
    serviceTag: Optional[str] = None  # 服务类型
    totalPrice: Optional[float] = None  # 子订单总价格，单位CNY
    finalPrice: Optional[float] = None  # 子订单最终价格，单位CNY
    orderItemPrices: Optional[List[CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse]] = None  # item价格信息

    @classmethod
    def from_json(cls, json_data: dict) -> Optional['CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesResponse']:
        """从JSON数据创建对象"""
        if not json_data:
            return None
        ...
        if 'orderItemPrices' in json_data and json_data.get('orderItemPrices') is not None:
            order_item_price = [
                CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesOrderItemPricesResponse.from_json(item)
...
@dataclass
class CtecsQueryNewOrderPriceV42ReturnObjResponse:
    """询价返回对象
    ...
    finalPrice: Optional[float] = None  # 最终价格，单位CNY
    subOrderPrices: Optional[List[CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesResponse]] = None  # 子订单价格信息

    @classmethod
    def from_json(cls, json_data: dict) -> Optional['CtecsQueryNewOrderPriceV42ReturnObjResponse']:
        """从JSON数据创建对象"""
        if not json_data:
            return None
        ...
        if 'subOrderPrices' in json_data and json_data.get('subOrderPrices') is not None:
            sub_order_price = [
                CtecsQueryNewOrderPriceV42ReturnObjSubOrderPricesResponse.from_json(sub_order)
...
@dataclass
class CtecsQueryNewOrderPriceV42Response:
    """询价API响应
    包含状态码、错误信息和返回对象
    """
    ...
    returnObj: Optional[CtecsQueryNewOrderPriceV42ReturnObjResponse] = None  # 返回参数
    error: Optional[str] = None  # 错误码，请求成功时不返回

    @classmethod
    def from_json(cls, json_data: dict) -> 'CtecsQueryNewOrderPriceV42Response':
        """从JSON数据创建响应对象"""
        if not json_data:
        ...
        if 'returnObj' in json_data and json_data['returnObj'] is not None:
            return_obj = CtecsQueryNewOrderPriceV42ReturnObjResponse.from_json(json_data['returnObj'])
...
class CtecsQueryNewOrderPriceV42Api:
    """新订单询价API
    功能：购买云产品时询价，支持云主机、云硬盘、弹性公网IP、NAT网关、共享带宽、物理机、
         性能保障型负载均衡、云主机备份存储库和云硬盘备份存储库产品的包年/包月或按量订单的询价
    接口文档：/v4/order/new-query-price
    """
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtecsQueryNewOrderPriceV42Request) -> CtecsQueryNewOrderPriceV42Response:
        """执行API请求
        Args:
            credential: 认证凭证
            client: CTYun客户端
        ...
        """
        url = f"{endpoint.rstrip('/')}/v4/order/new-query-price"
        try:
            ...
            return CtecsQueryNewOrderPriceV42Response.from_json(response.json())
