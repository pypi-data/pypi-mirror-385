...
@dataclass
class CtecsInstanceDetachKeypairV41Request:
...
@dataclass
class CtecsInstanceDetachKeypairV41ReturnObjResponse:
    ...
    @staticmethod
    def from_json(json_data: dict) -> 'CtecsInstanceDetachKeypairV41ReturnObjResponse':
        if not json_data:
            return None
        obj = CtecsInstanceDetachKeypairV41ReturnObjResponse(
            instanceID=json_data.get('instanceID')
        )
        return obj

@dataclass
class CtecsInstanceDetachKeypairV41Response:
    ...
    returnObj: Optional[CtecsInstanceDetachKeypairV41ReturnObjResponse]  # 返回参数




    @staticmethod
    def from_json(json_data: dict) -> 'CtecsInstanceDetachKeypairV41Response':
        if not json_data:
            return None
        return_obj = None
        if 'returnObj' in json_data and json_data['returnObj']:
            return_obj = CtecsInstanceDetachKeypairV41ReturnObjResponse.from_json(json_data['returnObj'])

        obj = CtecsInstanceDetachKeypairV41Response(
...
class CtecsInstanceDetachKeypairV41Api:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtecsInstanceDetachKeypairV41Request) -> CtecsInstanceDetachKeypairV41Response:
        url = endpoint + "/v4/ecs/keypair/detach-instance"
        try:
            request_dict = request.to_dict()
            response = client.post(url=url,  data=request_dict, credential=credential)
            return CtecsInstanceDetachKeypairV41Response.from_json(response.json())
