...
def load_game_data():
    ...
    try:
        # load cards from package data
        cards_data = files('scrollbound.data').joinpath('cards.json').read_text(encoding='utf-8')
    ...
    try:
        # load scrolls from package data
        scrolls_data = files('scrollbound.data').joinpath('scrolls.json').read_text(encoding='utf-8')
        SCROLLS = json.loads(scrolls_data)
    except FileNotFoundError:
    ...
    try:
        # load enemies from package data
        enemies_data = files('scrollbound.data').joinpath('enemies.json').read_text(encoding='utf-8')
        ENEMIES = json.loads(enemies_data)
    except FileNotFoundError:
