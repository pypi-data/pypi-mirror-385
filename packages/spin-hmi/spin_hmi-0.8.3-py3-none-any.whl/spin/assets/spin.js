...
var Spin = {
    ...
    test_condition: function(cond, value) {
        let [op, val, invert] = cond;
        switch (op) {
        case ">":
            if (invert) return value <= val;
            else return value > val;
        case "<":
            if (invert) return value >= val;
            else return value < val;
        case "=":
            if (invert) return String(value) != val;
            else return value == val;
        default:
            return false;
        }
    ...
    preprocess_element: function(el, label) {
        ...
        for (let [_, mkey, mval] of opts) {
            ...
            case "hl":     descriptor.highlight = this.parse_cond(mval); break;
            case "nohl":   descriptor.highlight = this.parse_cond(mval, true); break;
            case "show":   descriptor.show_if = this.parse_cond(mval); break;
            case "hide":   descriptor.show_if = this.parse_cond(mval, true); break;
            case "input":
            case "toggle": descriptor.intent = [mkey, this.parse_toggle(mval)]; break;
            case "stop":
            case "reset":  descriptor.intent = [mkey, null]; break;
            case "set":    descriptor.intent = ["set", this.maybe_float(mval)]; break;
            case "showstatus":
