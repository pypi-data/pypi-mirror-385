...
from .config import get_config, config_watcher
from .utils import extract_addr, error_fallback
...
class SiteHandler:
    ...
    def handle_auth(self, request, required):
        if hdr := request.headers.get(hdrs.AUTHORIZATION):
            try:
                ...
                hashed_pass = sha256(auth.password.encode()).hexdigest()
                if auth.login == required[0] and hashed_pass == required[1]:
                    return
    ...
    async def page(self, request):
        ...
        # master template
        with open(self._asset_root / 'page.html', encoding='utf-8') as fp:
            template = fp.read()
        ...
        # main page content
        with open(self._conf_root / pageconf.file, encoding='utf-8') as fp:
            content = fp.read()
...
def init_app(handler):
    """Setup the aiohttp application and routes."""
    app = web.Application()
    ...
    for path in conf.plugins:
        ...
        try:
            if pluginpath.suffix == '':
                ...
                exec(pluginpath.read_bytes(), namespace)
...
def main():
    ...
    async def stop_and_restart():
        ...
        exitstatus[0] = 3
    loop.create_task(config_watcher(args, stop_and_restart))
