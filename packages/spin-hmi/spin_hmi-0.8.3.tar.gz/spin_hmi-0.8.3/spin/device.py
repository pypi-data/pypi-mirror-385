...
class BaseDevice:
    ...
    async def error_guard(self, conn, msg=None, reset_progress=True):
        """Context manager to catch errors of long running tasks spawned
        for this device.
    ...
    def format_exception(self, exc):
        """Callback to format exceptions for sending to the frontend.
    ...
    async def handle_request(self, request: Request):
        """Called to handle a Request from a frontend."""
...
class AsyncUpdateDevice(BaseDevice):
    ...
    def __init__(self, name, devconf, uri, backend, log):
        ...
        self.extra_params = devconf.params
