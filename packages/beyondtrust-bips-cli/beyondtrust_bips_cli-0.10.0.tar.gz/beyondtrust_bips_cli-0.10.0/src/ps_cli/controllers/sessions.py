...
class Session(CLIController):
    ...
    def create_session_admin(self, args):
        ...
        try:
            fields = self.get_fields(
                POST_SESSIONS_ADMIN, session_fields, Version.DEFAULT
            )
            self.display.v("Creating session for administrators")
            ...
            self.display.show(session, fields)
            success_msg = "Session for administrators created successfully"
            self.display.v(success_msg)
        except exceptions.CreationError as e:
            self.display.v(e)
            self.log.error(e)
            print_it("It was not possible to create the session for administrators")
