...
def update(
    ...
    if container:
        ...
        wrapped = f"{{ {cmd}; }} | tee {shlex.quote(str(host_log.resolve()))}; exit $PIPESTATUS"
    ...
    # --- Post-update: optional cache flush + signal for dev watcher ---
    if reload_after:
        # 1) Try to flush caches via RPC (best-effort)
        try:
            rt = {
        ...
        # 2) Touch/append signal file so `opm dev` broadcasts reload
        signal_file = cfg.get("runtime", "dev_signal_file") or ".opm/reload.flag"
        sp = Path(signal_file).expanduser()
        sp.parent.mkdir(parents=True, exist_ok=True)
        try:
            with open(sp, "a", encoding="utf-8") as f:
                f.write(f"{time.time()}\n")
            info(f"[opm] Dev reload signal written → {sp}")
        except Exception as e:
            info(f"[opm] Could not write dev signal: {e}")
