...
# ----------------------------------------------------------------------
# 💬 WebSocket Bus (safe & quiet)
#  - Ignores messages coming from clients (prevents echo/flood loops)
#  - Broadcasts only server-generated messages
# ----------------------------------------------------------------------
class _WSBus:
    def __init__(self):
        self.clients = set()
    ...
    async def handler(self, ws):
        """Handle a connected client; ignore incoming frames (optional: accept 'ping')."""
        self.clients.add(ws)
        try:
            async for _ in ws:
                # If you want to accept 'ping' from clients, parse here and ignore:
                # text = _.decode() if isinstance(_, (bytes, bytearray)) else str(_)
                # if text == "ping":
                #     pass
                continue
    ...
    async def broadcast(self, msg: str):
        """Send message to all connected clients; silently skip closed ones."""
        if not self.clients:
            return
        dead = []
...
# ----------------------------------------------------------------------
# ⏱️ Simple debounce helper (coalesces rapid consecutive events per file)
# ----------------------------------------------------------------------
_debounce_timers = {}

def _debounce(key: str, delay_ms: int, fn):
    import threading as _th
    old = _debounce_timers.get(key)
    if old and hasattr(old, "is_alive") and old.is_alive():
        try:
            old.cancel()
        except Exception:
            pass
    t = _th.Timer(delay_ms / 1000.0, fn)
...
def dev(
    ...
    # on_change handler with XML menu/data heuristic + debounce and always-broadcast
    def _handle_change(p: str):
        lower = p.lower()
        try:
            if p.endswith(".xml"):
                # Heuristic: menus/data usually need upgrade; views typically only need cache flush
    ...
    def on_change(path: Path):
        p = str(path)
        # Coalesce bursts from the same file (300ms)
        _debounce(p, 300, lambda: _handle_change(p))
