### Code here is from Jacob's Schreiber's implementation of BPNet, called BPNet-lite:
...
def signal_corr_mse(true_signals, pred_signals, batch_size=200):
    """Compute Pearson, Spearman and MSE **on raw per-position counts**.

    Args:
        true_signals: N x T x O x 2 array of true counts.
        pred_signals: N x T x O x 2 array of predicted counts.
        batch_size: mini-batch size for memory efficiency.

    Returns: three N x T arrays — (pearson, spearman, mse).
    """
    num_samples, num_tasks = true_signals.shape[:2]
    pears = np.zeros((num_samples, num_tasks))
    spear = np.zeros((num_samples, num_tasks))
    mse = np.zeros((num_samples, num_tasks))
    ...
    for start in range(0, num_samples, batch_size):
        ...
        true_batch = true_signals[start:end]  # B × T × O × 2
        pred_batch = pred_signals[start:end]

        # Pool strands and flatten profile dimension (O × 2 → flat)
        new_shape = (true_batch.shape[0], num_tasks, -1)
        true_flat = true_batch.reshape(new_shape)
        pred_flat = pred_batch.reshape(new_shape)
...
def compute_performance_metrics(
    ...
    # pred_counts_exp = np.exp(log_pred_counts)
    # pred_signals = np.exp(log_pred_profs) * pred_counts_exp[:, :, np.newaxis, :]

    # sig_pears, sig_spear, sig_mse = signal_corr_mse(true_profs, pred_signals)
...
# from https://github.com/lucidrains/enformer-pytorch
class MeanPearsonCorrCoefPerChannel(Metric):
    ...
    def __init__(
        self,
        n_channels: int,
        channel_dim: int = -1,  # default = last dim
        dist_sync_on_step: bool = False
    ):
        """Calculates the mean pearson correlation across channels aggregated over regions"""
        super().__init__(dist_sync_on_step=dist_sync_on_step)
        if channel_dim not in [-1, 1, 2]:
            raise ValueError("`channel_dim` must be one of 1 or 2 or -1")
        self.reduce_dims = (0, 1) if channel_dim in [-1, 2] else (0, 2)
...
def compute_per_channel_pcc(outputs: Tensor, y_true: Tensor, channel_dim=1) -> Tensor:
    """
    Returns Pearson r per channel as a 1-D tensor of length C.

    Parameters
    ----------
    outputs : Tensor
        Predicted outputs, shape (B, C, L) or (B, L, C)
    y_true : Tensor
        True labels, shape (B, C, L) or (B, L, C)
    channel_dim : int, optional
        The dimension index of channels in outputs and y_true. Default is 1 for (B, C, L).

    Returns
    -------
    Tensor
        A 1-D tensor of length C containing Pearson correlation coefficients for each channel.
    """
    pcc_metric = MeanPearsonCorrCoefPerChannel(
        n_channels=outputs.size(channel_dim),
        channel_dim=channel_dim,
    ).to(y_true.device)
    return pcc_metric(outputs, y_true) # (C,)
