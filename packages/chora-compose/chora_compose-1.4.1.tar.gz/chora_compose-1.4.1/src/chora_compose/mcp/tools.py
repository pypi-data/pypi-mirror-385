...
# Import server conditionally to allow testing without FastMCP
try:
...
async def _generate_content_impl(
    ...
    Args:
        ...
        context: Context variables (merged with config context). Can be dict or JSON string.
    ...
    # Pydantic should handle this, but guard against edge cases where validation is bypassed
        ...
        if context == "":
            ...
            try:
                ...
                if not isinstance(parsed_context, dict):
                    return _handle_tool_error(
                        MCPError(
                            ...
                            message=f"context must be JSON object; parsed to {type(parsed_context).__name__}",
                ...
                return _handle_tool_error(
                    MCPError(
                        code="invalid_context",
                        message="context must be JSON object; received string we could not parse",
                        details={
        ...
        if missing_content:
            ...
            return _handle_tool_error(
                MCPError(
                    ...
                    message=f"Cannot assemble artifact: {len(missing_content)} content piece(s) not generated yet. Artifact assembly uses cached content only.",
                    details={
                        ...
                        + "\n".join(suggestions),
                        "note": "Artifact assembly combines pre-existing cached content. It does not regenerate content pieces on demand. Generate all required content before assembling.",
                    },
                ),
                "assemble_artifact",
                start_time,
...
async def regenerate_content(
        ...
        try:
            ...
            if not isinstance(context, dict):
                return _handle_tool_error(
                    MCPError(
                        code="invalid_context",
                        message=f"context must be JSON object, got {type(context).__name__}",
    ...
    try:
        ...
        if compare and previous_content:
            ...
            # Calculate changes
            lines_added = len([l for l in new_lines if l not in prev_lines])
            lines_removed = len([l for l in prev_lines if l not in new_lines])
...
async def preview_generation(
    ...
    Args:
        ...
        context: Context variables for template substitution. Can be dict or JSON string (default: {})
...
async def batch_generate(
    ...
    Args:
        ...
        shared_context: Context applied to all generations. Can be dict or JSON string (default: {})
        individual_contexts: Per-ID context overrides. Values can be dict or JSON string (default: {})
            ...
            try:
                ...
                if not isinstance(parsed_shared_context, dict):
                    return _handle_tool_error(
                        MCPError(
                            code="invalid_context",
                            message=f"shared_context must be JSON object; parsed to {type(parsed_shared_context).__name__}",
                            details={
                ...
                return _handle_tool_error(
                    MCPError(
                        code="invalid_context",
                        message="shared_context must be JSON object; received string we could not parse",
                        details={
                    ...
                    try:
                        ...
                        if not isinstance(parsed_ctx, dict):
                            return _handle_tool_error(
                                MCPError(
                                    code="invalid_context",
                                    message=f"individual_contexts['{content_id}'] must be JSON object; parsed to {type(parsed_ctx).__name__}",
                                    details={
                                        "content_id": content_id,
                        ...
                        return _handle_tool_error(
                            MCPError(
                                code="invalid_context",
                                message=f"individual_contexts['{content_id}'] must be JSON object; received string we could not parse",
                                details={
                                    "content_id": content_id,
...
async def trace_dependencies(
    ...
    Args:
        ...
        check_status: Check if each content piece exists (slower but more informative)
        show_metadata: Include detailed metadata for each content (generator, versions, etc.)
    ...
    try:
        ...
        for content_id in content_ids:
            ...
            for v in versions_to_delete:
                # Support both real StoredVersion (with file_path) and mocks (with size_bytes)
            ...
            # Use version_id for comparison (Mock objects don't support identity comparison)
