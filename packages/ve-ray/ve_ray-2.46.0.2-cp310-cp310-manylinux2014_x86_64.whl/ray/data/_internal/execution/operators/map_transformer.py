...
class ApplyCkptSkipToReadBlocks(MapTransformFn):
    """Skip checkpoint parts on output blocks."""
    ...
    def __call__(self, blocks: Iterable[Block], ctx: TaskContext) -> Iterable[Block]:
        for block in blocks:
            yield self._checkpoint_manager.skip_processed_row(block)

    def __repr__(self):
        return f"ApplyCkptSkipToReadBlocks(ckpt_content={_truncated_repr(self._skip_metas)})"
