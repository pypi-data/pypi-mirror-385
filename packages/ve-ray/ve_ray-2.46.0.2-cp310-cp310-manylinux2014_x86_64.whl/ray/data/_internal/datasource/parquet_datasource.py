...
from ray.data.datasource.parquet_meta_provider import ParquetMetadataProvider, _ParquetFileFragmentMetaData
...
class SerializedFragment:
    def __init__(self, frag: "ParquetFileFragment"):
        row_group_ids = [row_group.id for row_group in frag.row_groups]
        self._data = cloudpickle.dumps(
            (frag.format, frag.path, frag.filesystem, frag.partition_expression, row_group_ids)
        )
    ...
    def deserialize(self) -> "ParquetFileFragment":
        ...
        (file_format, path, filesystem, partition_expression, row_group_ids) = cloudpickle.loads(
            self._data
        )
        return file_format.make_fragment(path, filesystem, partition_expression, row_group_ids)
...
class ParquetDatasource(Datasource):
    ...
    def __init__(
        ...
        *,
        ...
        try:
            ...
            meta_provider.prefetch_file_metadata(pq_ds.fragments, **prefetch_remote_args)
        except OSError as e:
            _handle_read_os_error(e, paths)

        pq_ds_fragments = split_fragment_to_fragments(file_split_chunk_size, pq_ds.fragments)
        self._metadata = _dedupe_metadata(pq_ds_fragments)
        ...
        self._pq_fragments = [SerializedFragment(p) for p in pq_ds_fragments]
        self._pq_paths = [p.path for p in pq_ds_fragments]
    ...
    def get_read_tasks(self, parallelism: int) -> List[ReadTask]:
        logger.debug(f"task parallelism: {parallelism}, num of fragments:{len(self._pq_fragments)}")
        ...
        if self._group_files_by_size:
            pq_fragments, pq_paths, pq_metadata = _group_files_by_size_greedy(
                pq_fragments, pq_paths, pq_metadata, parallelism
            )
        else:
            pq_fragments, pq_paths, pq_metadata = (
...
def split_fragment_to_fragments(file_split_chunk_size, fragments):
    start_time = time.time()
    if file_split_chunk_size > 0:
        new_fragments = []
        for fragment in fragments:
            file_metadata = fragment.metadata
            if file_metadata is None:
                new_fragments.append(fragment)
                continue
            current_group = []  # Store the rowgroup ID of the current group
            current_group_size = 0  # The cumulative size of the current group
            for row_group_info in fragment.row_groups:
                row_group_idx = row_group_info.id
                row_group_meta = file_metadata.row_group(row_group_idx)
                row_group_size = row_group_meta.total_byte_size

                # If the current group + current rowgroup exceeds the target size
                # and the group is not empty,submit the current group
                if current_group and (current_group_size + row_group_size) > file_split_chunk_size:
                    new_fragment = fragment.subset(row_group_ids=current_group)
                    new_fragments.append(new_fragment)
                    # Reset the current group
                    current_group = [row_group_idx]
                    current_group_size = row_group_size
                else:
                    current_group.append(row_group_idx)
                    current_group_size += row_group_size

            # Handle the last unsubmitted group
            if current_group:
                new_fragment = fragment.subset(row_group_ids=current_group)
                new_fragments.append(new_fragment)

        # Replace with the new list of split fragments
        fragments = new_fragments
        logger.debug(f"After chunking, new fragments len: {len(fragments)}, cost: {time.time() - start_time}")
    return fragments


def _group_files_by_size_greedy(
        fragments: List[SerializedFragment],
        paths: List[str],
        metadata: List[Optional[_ParquetFileFragmentMetaData]],
        parallelism: int,
) -> Tuple[List[List[SerializedFragment]], List[List[str]], List[List[_ParquetFileFragmentMetaData]]]:
    """
    Greedy grouping strategy (heap-optimized): After sorting by file size in descending order,
    use a priority queue to quickly find the group with the smallest current total size.
    Args:
        fragments: List of original fragments.
        paths: List of file paths.
        metadata: List of metadata (optional).
        parallelism: Target number of groups.
    Returns:
        Grouped fragments, paths, and metadata (type: List[List[...]])
    """
    # 1. Extract file sizes and generate indices (avoid moving large objects)
    sizes = [meta.total_byte_size if meta is not None else 0 for meta in metadata]
    # File indices sorted in descending order of size
    sorted_indices = sorted(range(len(sizes)), key=lambda i: -sizes[i])

    # 2. Initialize groups (each group stores fragments, paths, metadata in lists, and uses a heap to track total size)
    groups = [
        {"total_size": 0, "fragments": [], "paths": [], "metadata": []}
        for _ in range(parallelism)
    ]
    # Priority queue: elements are (current total size, group index) for quickly finding the smallest group
    import heapq
    heap = [(0, i) for i in range(parallelism)]
    heapq.heapify(heap)

    # 3. Assign files to groups (each time take the smallest group from the heap)
    for idx in sorted_indices:
        # Extract the group with the smallest current total size
        current_size, group_idx = heapq.heappop(heap)
        # Add the file to this group
        groups[group_idx]["fragments"].append(fragments[idx])
        groups[group_idx]["paths"].append(paths[idx])
        groups[group_idx]["metadata"].append(metadata[idx])
        # Update the group's total size and reinsert into the heap
        new_size = current_size + sizes[idx]
        groups[group_idx]["total_size"] = new_size
        heapq.heappush(heap, (new_size, group_idx))

    # 4. Extract the grouped results (only keep the required lists)
    return (
        [g["fragments"] for g in groups],
        [g["paths"] for g in groups],
        [g["metadata"] for g in groups],
    )
...
def read_fragments(
    ...
    fragment_split_chunk_size = to_batches_kwargs.pop("fragment_split_chunk_size", 1024 * 1024 * 1024)
    # split the fragment to avoid memory leaks caused by the fragment's to_batches method
    # when the fragment is too large, can referrence ray issue #49158
    fragments = split_fragment_to_fragments(fragment_split_chunk_size, fragments)
    for fragment in fragments:
        logger.debug(f"read row groups: {fragment.row_groups} from fragment: {fragment.metadata}")
        partitions = {}
...
def _dedupe_metadata(
    ...
    for fragment in fragments:
        ...
        group_ids = [row_group.id for row_group in fragment.row_groups]
        stripped_md = _ParquetFileFragmentMetaData(fragment_metadata, group_ids)
