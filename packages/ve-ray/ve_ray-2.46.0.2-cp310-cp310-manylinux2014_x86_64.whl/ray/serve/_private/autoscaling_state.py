...
class ReplicaMetricReport:
    """Report from a replica on ongoing requests and custom metrics.

    Args:
        running_requests: Average number of running requests at the
            replica.
        custom_metrics: Average number of custom metrics at the
            replica.
        timestamp: The time at which this report was received.
    """
...
class AutoscalingState:
    ...
    def record_request_metrics_for_replica(
        self, replica_id: ReplicaID, metrics_window_avg: Dict[str, Optional[float]], send_timestamp: float
    ) -> None:
        """Records average number of ongoing requests and custom metrics at a replica."""

        metrics = {mn: mv for mn, mv in metrics_window_avg.items() if mv is not None}
        if len(metrics) == 0:
            return
        ...
        if (
            ...
            assert 'num_ongoing_requests' in metrics, (
                ValueError(f'missing "num_ongoing_requests" in metrics'))
            running_requests = metrics.pop('num_ongoing_requests')
            self._replica_requests[replica_id] = ReplicaMetricReport(
                running_requests=running_requests,
                custom_metrics=[f'{key} {val if val is not None else 0}' for key, val in metrics.items()],
    ...
    def get_decision_num_replicas(
        ...
        decision_num_replicas = self._policy(
            ...
            custom_metrics=self.get_replica_custom_metrics(),
        )
    ...
    def get_replica_custom_metrics(self) -> Dict[ReplicaID, List[str]]:
        if (
            RAY_SERVE_COLLECT_AUTOSCALING_METRICS_ON_HANDLE
            or len(self._running_replicas) == 0
        ):
        ...
        return {
            id: self._replica_requests[id].custom_metrics
            for id in self._running_replicas if id in self._replica_requests
        }
    ...
    def record_request_metrics_for_replica(
        ...
        if deployment_id in self._autoscaling_states:
            self._autoscaling_states[deployment_id].record_request_metrics_for_replica(
                ...
                metrics_window_avg=metrics_window_avg,
