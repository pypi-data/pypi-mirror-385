...
def infer_line_separator(
    ...
    max_read_size: int = 196608
):
    """
    Infer the line terminator type of a CSV file using the PyArrow filesystem abstraction.
    Returns: '\n' (LF), '\r' (CR), '\r\n' (CRLF), or None (not detected)
    """
    counts = {b'\n': 0, b'\r': 0, b'\r\n': 0}

    # Sample by reading 3 chunks of data iteratively
    f.seek(0)
    for _ in range(max_read_size // read_buffer):
        chunk = f.read(read_buffer)
        if not chunk:
            break  # Exit loop early if end of file is reached

        # Process line terminators within the current chunk
        i = 0
        while i < len(chunk):
            # Prioritize detecting CRLF (occupies 2 bytes)
            if i + 1 < len(chunk) and chunk[i:i+2] == b'\r\n':
                counts[b'\r\n'] += 1
                i += 2
            # Detect standalone CR or LF
            else:
                if chunk[i] == 13:  # \r
                    counts[b'\r'] += 1
                    # Check if next character in chunk is \n
                    if i + 1 < len(chunk) and chunk[i+1] == 10:
                        counts[b'\r\n'] += 1
                        counts[b'\r'] -= 1  # Correct duplicate counting
                        i += 1  # Skip next character
                elif chunk[i] == 10:  # \n
                    counts[b'\n'] += 1
                i += 1

    # Find the most frequent terminator (prioritize composite types)
...
def find_head_n_line(
    ...
    *,
    ...
    """Find the buffers (byte lists) of the first n lines based on the specified line terminator"""
    f.seek(0)
    buffer = f.read(read_size)  # Read a fixed-size chunk of the file
    if not buffer:
        return []  # Return empty list for empty file

    lines = []
    start = 0
    # Length of line terminator (handles multi-byte terminators like \r\n)
    term_len = len(line_terminator)

    for _ in range(n):
        # Search for the line terminator starting from the current 'start' position
        end_pos = buffer.find(line_terminator, start)
        if end_pos == -1:
            break  # Insufficient content remaining for a full line, terminate early

        # Insufficient content remaining for a full line, terminate early
        line_buffer = buffer[start:end_pos]
        lines.append(line_buffer)

        # Update the start position for the next search (skip the current line terminator)
        start = end_pos + term_len
