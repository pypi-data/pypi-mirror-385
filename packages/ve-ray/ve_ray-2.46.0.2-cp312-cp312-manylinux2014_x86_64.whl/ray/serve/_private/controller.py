...
class ServeController:
    ...
    def record_autoscaling_metrics(
        self, replica_id: str, metrics_window_avg: Dict[str, Optional[float]], send_timestamp: float
    ):
        logger.debug(
            f"Received metrics from replica {replica_id}: {metrics_window_avg}"
        )
        self.autoscaling_state_manager.record_request_metrics_for_replica(
            replica_id, metrics_window_avg, send_timestamp
        )
