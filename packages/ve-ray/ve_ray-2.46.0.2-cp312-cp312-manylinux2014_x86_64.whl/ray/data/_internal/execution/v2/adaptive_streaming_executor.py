...
class AdaptiveStreamingExecutor(StreamingExecutor):
    ...
    def __init__(self, data_context: DataContext, dataset_id: str = "unknown_dataset"):
        super().__init__(data_context, dataset_id)

        thread_name = f"AdaptiveStreamingExecutor-{dataset_id}"
    ...
    def execute(
        ...
        class StreamIterator(OutputIterator):
            def __init__(self, executor: AdaptiveStreamingExecutor):
                self._executor = executor

            def get_next(self, output_split_idx: Optional[int] = None) -> RefBundle:
                try:
                    op, state = self._executor._output_node
                    item = state.get_output_blocking(
                        output_split_idx
                    )
                    if self._executor._global_info:
                        self._executor._global_info.update(1, op.num_outputs_total())
                    return item
