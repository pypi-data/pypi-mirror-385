    ...
    def __init__(
        ...
        try:
            ...
            if 'EMR_TOS_BUCKET_TAG_ENABLED' not in os.environ or \
                ...
                if hasattr(datasource, '_paths') and hasattr(datasource, '_filesystem'):
                    BucketTagMgr().add_tag_to_tos_buckets(datasource._paths(), datasource._filesystem)
        except Exception as e:
            try:
                logging.error("The path %s about filesystem %s tagged failed. The error msg is: %s.",
                              datasource._paths(), datasource._filesystem, e)
            except Exception as e:
