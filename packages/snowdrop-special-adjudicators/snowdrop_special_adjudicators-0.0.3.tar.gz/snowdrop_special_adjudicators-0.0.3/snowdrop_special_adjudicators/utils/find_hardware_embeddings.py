...
def get_zephyr_subgrid(qubit_connectivity_graph, rows, cols, grid_size=4):
    ...
    coords = [dnx.zephyr_coordinates(grid_size).linear_to_zephyr(v) for v in qubit_connectivity_graph.nodes]
    ...
    subgraph = qubit_connectivity_graph.subgraph([dnx.zephyr_coordinates(grid_size).zephyr_to_linear(c)
...
def raster_embedding_search(hardware_graph, subgraph, raster_breadth=2, delete_used=True,
                            verbose=False, grid_size=6, **kwargs):
    ...
    Args:
        ...
        grid_size (int, optional): Size of grid. Defaults to 16.
    ...
    embs = []
    for row_offset in range(grid_size - raster_breadth + 1):

        for col_offset in range(grid_size - raster_breadth + 1):
            zephyr_subgrid = get_zephyr_subgrid(hardware_graph_copy, range(row_offset, row_offset + raster_breadth),
                                                range(col_offset, col_offset + raster_breadth), grid_size)
    ...
    embedded_mat = np.asarray([[ie[v] for ie in independent_embs] for v in subgraph.nodes]).T
...
def get_embeddings(source_graph_number, qc_solver_to_use, data_dir):
    ...
    file_name = ('embeddings_graph_number_' + str(source_graph_number) + '_raster_breadth_' + str(raster_breadth) +
                 '_grid_size_' + str(grid_size) + '_qc_' + qc_solver_to_use + '.pkl')
    ...
    if os.path.isfile(file_path):
        ...
        # source graph
        graph_base = GraphProperties()
        graph = graph_base.graph_database[source_graph_number]
        source_graph = nx.Graph()
        source_graph.add_nodes_from([k for k in range(graph['num_nodes'])])
        source_graph.add_edges_from(graph['edge_list'])

        # find embeddings
        embedded_mat = raster_embedding_search(target_graph, source_graph,
                                               raster_breadth=raster_breadth,
                                               grid_size=grid_size)

        list_of_embeddings = [k.tolist() for k in embedded_mat]
