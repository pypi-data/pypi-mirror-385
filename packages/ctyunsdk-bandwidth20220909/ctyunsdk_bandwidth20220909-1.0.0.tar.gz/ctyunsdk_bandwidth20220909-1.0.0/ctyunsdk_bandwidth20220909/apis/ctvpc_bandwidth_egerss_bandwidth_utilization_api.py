...
@dataclass_json
@dataclass
class CtvpcBandwidthEgerssBandwidthUtilizationRequest:
    regionID: str  # 资源池 ID
...
@dataclass_json
@dataclass
class CtvpcBandwidthEgerssBandwidthUtilizationReturnObjResponse:
    """返回结果"""
...
@dataclass_json
@dataclass
class CtvpcBandwidthEgerssBandwidthUtilizationResponse:
    ...
    returnObj: Optional[CtvpcBandwidthEgerssBandwidthUtilizationReturnObjResponse] = None  # 返回结果


# 出方向带宽利用率。
class CtvpcBandwidthEgerssBandwidthUtilizationApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcBandwidthEgerssBandwidthUtilizationRequest) -> CtvpcBandwidthEgerssBandwidthUtilizationResponse:
        url = endpoint + "/v4/bandwidth/egress-bandwidth-utilization"
        params = {'regionID': request.regionID, 'bandwidthID': request.bandwidthID}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            return CtvpcBandwidthEgerssBandwidthUtilizationResponse.from_dict(response.json())
