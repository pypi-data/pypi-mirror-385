...
@dataclass_json
@dataclass
class CtvpcAssociateEipToBandwidthRequest:
    clientToken: str  # 客户端存根，用于保证订单幂等性, 长度 1 - 64
    regionID: str  # 共享带宽的区域id。
    bandwidthID: str  # 共享带宽id。
    eipIDs: List[str]  # EIP数组，限额1-50


@dataclass_json
@dataclass
class CtvpcAssociateEipToBandwidthResponse:
...
# 调用此接口可添加EIPs至共享带宽。
class CtvpcAssociateEipToBandwidthApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcAssociateEipToBandwidthRequest) -> CtvpcAssociateEipToBandwidthResponse:
        url = endpoint + "/v4/bandwidth/associate-eip"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcAssociateEipToBandwidthResponse.from_dict(response.json())
