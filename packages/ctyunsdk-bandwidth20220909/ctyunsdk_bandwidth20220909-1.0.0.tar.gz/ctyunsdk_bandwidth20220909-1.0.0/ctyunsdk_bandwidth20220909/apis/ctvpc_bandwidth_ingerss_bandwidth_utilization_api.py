...
@dataclass_json
@dataclass
class CtvpcBandwidthIngerssBandwidthUtilizationRequest:
    regionID: str  # 资源池 ID
...
@dataclass_json
@dataclass
class CtvpcBandwidthIngerssBandwidthUtilizationReturnObjResponse:
    """返回结果"""
...
@dataclass_json
@dataclass
class CtvpcBandwidthIngerssBandwidthUtilizationResponse:
    ...
    returnObj: Optional[CtvpcBandwidthIngerssBandwidthUtilizationReturnObjResponse] = None  # 返回结果


# 入方向带宽利用率。
class CtvpcBandwidthIngerssBandwidthUtilizationApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcBandwidthIngerssBandwidthUtilizationRequest) -> CtvpcBandwidthIngerssBandwidthUtilizationResponse:
        url = endpoint + "/v4/bandwidth/ingress-bandwidth-utilization"
        params = {'regionID': request.regionID, 'bandwidthID': request.bandwidthID}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            return CtvpcBandwidthIngerssBandwidthUtilizationResponse.from_dict(response.json())
