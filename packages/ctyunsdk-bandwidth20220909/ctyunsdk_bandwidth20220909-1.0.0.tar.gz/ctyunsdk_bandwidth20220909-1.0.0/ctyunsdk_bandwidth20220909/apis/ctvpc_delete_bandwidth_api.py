...
@dataclass_json
@dataclass
class CtvpcDeleteBandwidthRequest:
...
@dataclass_json
@dataclass
class CtvpcDeleteBandwidthReturnObjResponse:
    """返回删除的结果"""
...
@dataclass_json
@dataclass
class CtvpcDeleteBandwidthResponse:
    ...
    returnObj: Optional[CtvpcDeleteBandwidthReturnObjResponse] = None  # 返回删除的结果


# 调用此接口可删除共享带宽。
class CtvpcDeleteBandwidthApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcDeleteBandwidthRequest) -> CtvpcDeleteBandwidthResponse:
        url = endpoint + "/v4/bandwidth/delete"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcDeleteBandwidthResponse.from_dict(response.json())
