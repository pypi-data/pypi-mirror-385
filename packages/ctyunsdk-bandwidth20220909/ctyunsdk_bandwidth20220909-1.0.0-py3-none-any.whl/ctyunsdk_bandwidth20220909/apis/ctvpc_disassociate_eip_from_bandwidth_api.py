...
@dataclass_json
@dataclass
class CtvpcDisassociateEipFromBandwidthRequest:
    clientToken: str  # 客户端存根，用于保证订单幂等性, 长度 1 - 64
    regionID: str  # 共享带宽及EIP的区域id。
    bandwidthID: str  # 共享带宽id。
    eipIDs: List[str]  # EIP 数组，限额1-50


@dataclass_json
@dataclass
class CtvpcDisassociateEipFromBandwidthResponse:
...
# 调用此接口可从共享带宽移出EIPs。
class CtvpcDisassociateEipFromBandwidthApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcDisassociateEipFromBandwidthRequest) -> CtvpcDisassociateEipFromBandwidthResponse:
        url = endpoint + "/v4/bandwidth/disassociate-eip"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcDisassociateEipFromBandwidthResponse.from_dict(response.json())
