...
@dataclass_json
@dataclass
class CtvpcAssociateIPv6ToBandwidthRequest:
    clientToken: str  # 客户端存根，用于保证订单幂等性, 长度 1 - 64
    regionID: str  # 共享带宽的区域id。
    bandwidthID: str  # 共享带宽id。
    eipIDs: List[str]  # 4.0资源池传portid数组,共享带宽绑定 ipv6，只支持通过网卡进行绑定，限额1-50; 3.0资源池传vm_id即可


@dataclass_json
@dataclass
class CtvpcAssociateIPv6ToBandwidthResponse:
...
# 调用此接口可添加IPv6s至共享带宽。
class CtvpcAssociateIPv6ToBandwidthApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcAssociateIPv6ToBandwidthRequest) -> CtvpcAssociateIPv6ToBandwidthResponse:
        url = endpoint + "/v4/bandwidth/associate-ipv6"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcAssociateIPv6ToBandwidthResponse.from_dict(response.json())
