...
@dataclass_json
@dataclass
class CtvpcDisassociateIPv6FromBandwidthRequest:
    clientToken: str  # 客户端存根，用于保证订单幂等性, 长度 1 - 64
    regionID: str  # 共享带宽的区域id。
    bandwidthID: str  # 共享带宽id。
    eipIDs: List[str]  # portid数组。限额1-50


@dataclass_json
@dataclass
class CtvpcDisassociateIPv6FromBandwidthResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str] = None  # statusCode为900时的信息; statusCode为800时为success, 英文
...
# 调用此接口可从共享带宽中移出IPv6s。
class CtvpcDisassociateIPv6FromBandwidthApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcDisassociateIPv6FromBandwidthRequest) -> CtvpcDisassociateIPv6FromBandwidthResponse:
        url = endpoint + "/v4/bandwidth/disassociate-ipv6"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcDisassociateIPv6FromBandwidthResponse.from_dict(response.json())
