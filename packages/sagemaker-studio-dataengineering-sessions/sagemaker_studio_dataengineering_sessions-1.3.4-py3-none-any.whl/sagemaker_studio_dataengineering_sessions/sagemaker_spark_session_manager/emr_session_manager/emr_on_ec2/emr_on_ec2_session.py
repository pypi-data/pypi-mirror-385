    ...
    def __init__(self, connection_name: str):
        ...
        try:
            ...
            if self.release_label is None:
                self.get_logger().info("Release label not found in spark defaults, trying EMR API...")
                self.release_label = self.emr_gateway.get_ec2_release_label(self.connection_details.cluster_id)
            self.get_logger().info(f"Using release label: {self.release_label}")
        ...
        try:
            if (self.release_label and
                compare_emr_release_labels(self.release_label, EMR_EC2_VERSION_SUPPORT_FOR_MCMUFFIN) >= 0):
