...
"""
    ...
    def test_wrong_parameter_usage_in_code(self):
        """Test that wrong parameter usage in function body fails validation."""
        wrong_usage_codes = [
            """
def extract_data(html_content):
    soup = BeautifulSoup(htmlContent, 'html.parser')  # Wrong variable name
    return Model()
""",
            """
def extract_data(html_content):
    if html:  # Using 'html' instead of 'html_content'
        return Model()
""",
            """
def extract_data(html_content):
    data = process_content(content)  # Using 'content' instead of 'html_content'
    return Model(data=data)
"""
        ]

        for code in wrong_usage_codes:
            is_valid, error = validate_function_usage(code)
            assert not is_valid
            assert "instead of 'html_content'" in error
