...
class OnnxStableDiffusionInpaintPipelineLegacy(DiffusionPipeline):
    ...
    def __call__(self, prompt: Union[str, List[str]], image: Union[np.ndarray, PIL.Image.Image]=None, mask_image: Union[np.ndarray, PIL.Image.Image]=None, strength: float=0.8,
    num_inference_steps: Optional[int]=50, guidance_scale: Optional[float]=7.5, negative_prompt: Optional[Union[str, List[str]]]=None, num_images_per_prompt: Optional[int]=1,
    eta: Optional[float]=0.0, generator: Optional[np.random.RandomState]=None, prompt_embeds: Optional[np.ndarray]=None, negative_prompt_embeds: Optional[np.ndarray]=None,
    output_type: Optional[str]='pil', return_dict: bool=True, callback: Optional[Callable[[int, int, np.ndarray], None]]=None, callback_steps: int=1):
        ...
        if not isinstance(mask_image, np.ndarray): mask_image = preprocess_mask(mask_image, 8)
        mask_image = mask_image.astype(latents_dtype)
        mask = np.concatenate([mask_image] * num_images_per_prompt, axis=0)
        if not mask.shape == init_latents.shape: raise ValueError('The mask and image should be the same size!')
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            init_latents_proper = self.scheduler.add_noise(torch.from_numpy(init_latents_orig), torch.from_numpy(noise), torch.from_numpy(np.array([t])))
