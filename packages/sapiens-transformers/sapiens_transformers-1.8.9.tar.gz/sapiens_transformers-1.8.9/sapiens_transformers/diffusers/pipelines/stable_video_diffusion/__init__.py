...
try:
    ...
    _import_structure.update({'pipeline_stable_video_diffusion': ['StableVideoDiffusionPipeline', 'StableVideoDiffusionPipelineOutput']})
if TYPE_CHECKING or DIFFUSERS_SLOW_IMPORT:
    try:
        if not (is_transformers_available() and is_torch_available()): raise OptionalDependencyNotAvailable()
    except OptionalDependencyNotAvailable: from ...utils.dummy_torch_and_transformers_objects import *
    else: from .pipeline_stable_video_diffusion import StableVideoDiffusionPipeline, StableVideoDiffusionPipelineOutput
else:
    import sys
