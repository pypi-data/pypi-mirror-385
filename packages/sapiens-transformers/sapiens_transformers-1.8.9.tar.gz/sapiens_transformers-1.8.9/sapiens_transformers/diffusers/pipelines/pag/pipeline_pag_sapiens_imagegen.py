...
from ...models.attention_processor import PAGCFGSapiensImageGenLinearAttnProcessor2_0, PAGIdentitySapiensImageGenLinearAttnProcessor2_0
...
EXAMPLE_DOC_STRING = '\n    Examples:\n        ```py\n        >>> import torch\n        >>> from sapiens_transformers.diffusers import SapiensImageGenPAGPipeline\n\n        >>> pipe = SapiensImageGenPAGPipeline.from_pretrained(\n        ...     "Efficient-Large-Model/SapiensImageGen_1600M_1024px_BF16_diffusers",\n        ...     pag_applied_layers=["transformer_blocks.8"],\n        ...     torch_dtype=torch.float32,\n        ... )\n        >>> pipe.to("cuda")\n        >>> pipe.text_encoder.to(torch.bfloat16)\n        >>> pipe.transformer = pipe.transformer.to(torch.bfloat16)\n\n        >>> image = pipe(prompt=\'a cyberpunk cat with a neon sign that says "SapiensImageGen"\')[0]\n        >>> image[0].save("output.png")\n        ```\n'
...
class SapiensImageGenPAGPipeline(DiffusionPipeline, PAGMixin):
    bad_punct_regex = re.compile('[' + '#®•©™&@·º½¾¿¡§~' + '\\)' + '\\(' + '\\]' + '\\[' + '\\}' + '\\{' + '\\|' + '\\' + '\\/' + '\\*' + ']{1,}')
    model_cpu_offload_seq = 'text_encoder->transformer->vae'
    _callback_tensor_inputs = ['latents', 'prompt_embeds', 'negative_prompt_embeds']
    def __init__(self, tokenizer: AutoTokenizer, text_encoder: AutoModelForCausalLM, vae: AutoencoderDC, transformer: SapiensImageGenTransformer2DModel,
    scheduler: FlowMatchEulerDiscreteScheduler, pag_applied_layers: Union[str, List[str]]='transformer_blocks.0'):
        super().__init__()
        ...
        self.set_pag_applied_layers(pag_applied_layers, pag_attn_processors=(PAGCFGSapiensImageGenLinearAttnProcessor2_0(), PAGIdentitySapiensImageGenLinearAttnProcessor2_0()))
    ...
    negative_prompt_attention_mask: Optional[torch.Tensor]=None, clean_caption: bool=False, max_sequence_length: int=300, complex_human_instruction: Optional[List[str]]=None):
        """Args:"""
    ...
    def __call__(self, prompt: Union[str, List[str]]=None, negative_prompt: str='', num_inference_steps: int=20, timesteps: List[int]=None, sigmas: List[float]=None,
    guidance_scale: float=4.5, num_images_per_prompt: Optional[int]=1, height: int=1024, width: int=1024, eta: float=0.0, generator: Optional[Union[torch.Generator,
    ...
    negative_prompt_embeds: Optional[torch.Tensor]=None, negative_prompt_attention_mask: Optional[torch.Tensor]=None, output_type: Optional[str]='pil',
    return_dict: bool=True, clean_caption: bool=True, use_resolution_binning: bool=True, callback_on_step_end: Optional[Callable[[int, int, Dict], None]]=None,
    callback_on_step_end_tensor_inputs: List[str]=['latents'], max_sequence_length: int=300, complex_human_instruction: List[str]=["Given a user prompt, generate an 'Enhanced prompt' that provides detailed visual descriptions suitable for image generation. Evaluate the level of detail in the user prompt:", '- If the prompt is simple, focus on adding specifics about colors, shapes, sizes, textures, and spatial relationships to create vivid and concrete scenes.', '- If the prompt is already detailed, refine and enhance the existing details slightly without overcomplicating.', 'Here are examples of how to transform or refine prompts:', '- User Prompt: A cat sleeping -> Enhanced: A small, fluffy white cat curled up in a round shape, sleeping peacefully on a warm sunny windowsill, surrounded by pots of blooming red flowers.', '- User Prompt: A busy city street -> Enhanced: A bustling city street scene at dusk, featuring glowing street lamps, a diverse crowd of people in colorful clothing, and a double-decker bus passing by towering glass skyscrapers.', 'Please generate only the enhanced description for the prompt below and avoid including any additional commentary or evaluations:', 'User Prompt: '], pag_scale: float=3.0, pag_adaptive_scale: float=0.0) -> Union[ImagePipelineOutput, Tuple]:
        """Examples:"""
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.transformer(latent_model_input, encoder_hidden_states=prompt_embeds, encoder_attention_mask=prompt_attention_mask, timestep=timestep, return_dict=False)[0]
                noise_pred = noise_pred.float()
                if self.do_perturbed_attention_guidance: noise_pred = self._apply_perturbed_attention_guidance(noise_pred, self.do_classifier_free_guidance, guidance_scale, t)
                elif self.do_classifier_free_guidance:
