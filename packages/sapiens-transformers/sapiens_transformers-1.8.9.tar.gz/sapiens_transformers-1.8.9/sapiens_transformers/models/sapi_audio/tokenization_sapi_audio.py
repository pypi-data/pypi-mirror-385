"""
    ...
    #################################################################################################################################################################################################
"""
from .sapi_audio_import_variables import LANGUAGES, TO_LANGUAGE_CODE, VOCAB_FILES_NAMES
...
import numpy as np
import regex as re
MAX_MODEL_INPUT_SIZES, TASK_IDS = {'sapiens/sapi_audio': 448}, ["translate", "transcribe"]
class SAPIAudioTokenizer(PreTrainedTokenizer):
    ...
    def _normalize(self, text): return self.normalize(text)
    def _basic_normalize(self, text, remove_diacritics=False): return self.basic_normalize(text, remove_diacritics=remove_diacritics)
...
def _find_longest_common_sequence(sequences, token_timestamp_sequences=None):
    ...
    for seq_idx, right_sequence in enumerate(sequences[1:]):
        ...
        for i in range(1, left_length + right_length):
            ...
            if len(left) != len(right): raise RuntimeError("There is a bug within sapi_audio `decode_asr` function, please report it. Dropping to prevent bad inference.")
