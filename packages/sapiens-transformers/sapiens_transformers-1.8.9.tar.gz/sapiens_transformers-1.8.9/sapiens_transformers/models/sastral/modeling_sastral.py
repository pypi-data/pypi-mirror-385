"""
    ...
    #################################################################################################################################################################################################
"""
from ...utils import (add_start_docstrings, add_start_docstrings_to_model_forward, is_flash_attn_2_available, is_flash_attn_greater_or_equal_2_10, is_torchdynamo_compiling, replace_return_docstrings)
...
class SastralRMSNorm(nn.Module):
...
class SastralRotaryEmbedding(nn.Module):
...
class SastralAttention(nn.Module):
    def __init__(self, config: SastralConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
        ...
        self.rotary_emb = SastralRotaryEmbedding(self.head_dim, max_position_embeddings=self.max_position_embeddings, base=self.rope_theta)
...
class SastralFlashAttention2(SastralAttention):
    ...
    output_attentions: bool = False, use_cache: bool = False, cache_position: Optional[torch.LongTensor] = None):
        ...
        sliding_window=getattr(self.config, "sliding_window", None), use_top_left_mask=self._flash_attn_uses_top_left_mask, is_causal=self.is_causal)
        attn_output = attn_output.reshape(bsz, q_len, self.num_heads * self.head_dim).contiguous()
        attn_output = self.o_proj(attn_output)
        if not output_attentions: attn_weights = None
        return attn_output, attn_weights, past_key_value
class SastralSdpaAttention(SastralAttention):
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.LongTensor] = None, past_key_value: Optional[Cache] = None,
...
SASTRAL_ATTENTION_CLASSES = {"eager": SastralAttention, "flash_attention_2": SastralFlashAttention2, "sdpa": SastralSdpaAttention}
class SastralDecoderLayer(nn.Module):
    def __init__(self, config: SastralConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = SASTRAL_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx)
        self.mlp = SastralMLP(config)
        self.input_layernorm = SastralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
        self.post_attention_layernorm = SastralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.LongTensor] = None, past_key_value: Optional[Cache] = None,
...
class SastralPreTrainedModel(PreTrainedModel):
    config_class = SastralConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["SastralDecoderLayer"]
...
@add_start_docstrings("The bare Sastral Model outputting raw hidden-states without any specific head on top.", SASTRAL_START_DOCSTRING)
class SastralModel(SastralPreTrainedModel):
    def __init__(self, config: SastralConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)
        self.layers = nn.ModuleList([SastralDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self._attn_implementation = config._attn_implementation
        self.norm = SastralRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    ...
    cache_position: Optional[torch.LongTensor] = None) -> Union[Tuple, BaseModelOutputWithPast]:
        ...
        for decoder_layer in self.layers:
            ...
            else: layer_outputs = decoder_layer(hidden_states, attention_mask=causal_mask, position_ids=position_ids, past_key_value=past_key_values, output_attentions=output_attentions, use_cache=use_cache, cache_position=cache_position)
    ...
    def _update_causal_mask(self, attention_mask: torch.Tensor, input_tensor: torch.Tensor, cache_position: torch.Tensor, past_key_values: Cache, use_cache: bool, output_attentions: bool):
        if self._attn_implementation == "flash_attention_2":
            if attention_mask is not None and use_cache:
                ...
                if is_padding_right: raise ValueError("You are attempting to perform batched generation with padding_side='right' this may lead to unexpected behaviour for Flash Attention version of Sastral. Make sure to call `tokenizer.padding_side  = 'left'` before tokenizing the input. ")
        ...
        if (self.config._attn_implementation == "sdpa" and not (using_static_cache or using_sliding_window_cache) and not output_attentions):
            if AttentionMaskConverter._ignore_causal_mask_sdpa(attention_mask, inputs_embeds=input_tensor, past_key_values_length=past_seen_tokens, sliding_window=self.config.sliding_window, is_training=self.training): return None
        dtype, device = input_tensor.dtype, input_tensor.device
        min_dtype = torch.finfo(dtype).min
        sequence_length = input_tensor.shape[1]
        if using_sliding_window_cache: target_length = max(sequence_length, self.config.sliding_window)
        elif using_static_cache: target_length = past_key_values.get_max_length()
        ...
        if attention_mask is not None and attention_mask.dim() == 4: causal_mask = attention_mask
            ...
            if self.config.sliding_window is not None:
                if not using_sliding_window_cache or sequence_length > self.config.sliding_window: exclude_mask.bitwise_or_(torch.arange(target_length, device=device) <= (cache_position.reshape(-1, 1) - self.config.sliding_window))
        ...
        return causal_mask
class SastralForCausalLM(SastralPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    def __init__(self, config):
        super().__init__(config)
        self.model = SastralModel(config)
        self.vocab_size = config.vocab_size
...
class SastralForSequenceClassification(SastralPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
@add_start_docstrings("The Sastral Model transformer with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks.", SASTRAL_START_DOCSTRING)
class SastralForTokenClassification(SastralPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.model = SastralModel(config)
