...
_REAL_CHECKPOINT_FOR_DOC = "sastralai/Sastral-7B-v0.1"
_CHECKPOINT_FOR_DOC = "ksmcg/Sastral-tiny"
class FlaxSastralRMSNorm(nn.Module):
...
class FlaxSastralRotaryEmbedding(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self):
    ...
    def __call__(self, key, query, position_ids):
        ...
        return key, query
class FlaxSastralMLP(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self):
...
def apply_rotary_pos_emb(tensor, sin_pos, cos_pos): return (tensor * cos_pos) + (rotate_half(tensor) * sin_pos)
...
class FlaxSastralAttention(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self):
        config = self.config
        ...
        self.rotary_emb = FlaxSastralRotaryEmbedding(config, dtype=self.dtype)
    def _split_heads(self, hidden_states, num_heads): return hidden_states.reshape(hidden_states.shape[:2] + (num_heads, self.head_dim))
    def _merge_heads(self, hidden_states): return hidden_states.reshape(hidden_states.shape[:2] + (self.hidden_size,))
    @nn.compact
    ...
    def __call__(self, hidden_states: jnp.ndarray, attention_mask: Optional[jnp.ndarray] = None, position_ids: Optional[jnp.ndarray] = None, deterministic: bool = True,
    output_attentions: bool = False, init_cache: bool = False) -> Tuple[jnp.ndarray, jnp.ndarray]:
        ...
        if self.has_variable("cache", "cached_key") or init_cache: key_states, value_states, attention_mask = self._concatenate_to_cache(key_states, value_states, query_states, attention_mask)
        ...
        return outputs
class FlaxSastralDecoderLayer(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self):
        self.input_layernorm = FlaxSastralRMSNorm(self.config, dtype=self.dtype)
        self.self_attn = FlaxSastralAttention(self.config, dtype=self.dtype)
        self.post_attention_layernorm = FlaxSastralRMSNorm(self.config, dtype=self.dtype)
        self.mlp = FlaxSastralMLP(self.config, dtype=self.dtype)
...
"""
class FlaxSastralPreTrainedModel(FlaxPreTrainedModel):
    config_class = SastralConfig
    base_model_prefix = "model"
    module_class: nn.Module = None
    def __init__(self, config: SastralConfig, input_shape: Tuple = (1, 1), seed: int = 0, dtype: jnp.dtype = jnp.float32, _do_init: bool = True, **kwargs):
    ...
    def __call__(self, input_ids, attention_mask=None, position_ids=None, params: dict = None, past_key_values: dict = None, dropout_rng: jax.random.PRNGKey = None,
    train: bool = False, output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None):
        ...
        return outputs
class FlaxSastralLayerCollection(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.blocks = [FlaxSastralDecoderLayer(self.config, dtype=self.dtype, name=str(i)) for i in range(self.config.num_hidden_layers)]
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, deterministic: bool = True, init_cache: bool = False, output_attentions: bool = False,
    output_hidden_states: bool = False, return_dict: bool = False):
        ...
        return outputs
class FlaxSastralModule(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self):
        ...
        self.layers = FlaxSastralLayerCollection(self.config, dtype=self.dtype)
        self.norm = FlaxSastralRMSNorm(self.config, dtype=self.dtype)
    def __call__(self, input_ids, attention_mask=None, position_ids=None, deterministic=True, init_cache: bool = False, output_attentions: bool = False, output_hidden_states: bool = False, return_dict: bool = True):
        ...
        outputs = self.layers(input_embeds, position_ids=position_ids, attention_mask=attention_mask, deterministic=deterministic, init_cache=init_cache, output_attentions=output_attentions,
...
@add_start_docstrings("The bare Sastral Model transformer outputting raw hidden-states without any specific head on top.", SASTRAL_START_DOCSTRING)
class FlaxSastralModel(FlaxSastralPreTrainedModel): module_class = FlaxSastralModule
append_call_sample_docstring(FlaxSastralModel, _CHECKPOINT_FOR_DOC, FlaxBaseModelOutputWithPast, _CONFIG_FOR_DOC, real_checkpoint=_REAL_CHECKPOINT_FOR_DOC)
class FlaxSastralForCausalLMModule(nn.Module):
    config: SastralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self):
        self.model = FlaxSastralModule(self.config, dtype=self.dtype)
    ...
    output_hidden_states: bool = False, return_dict: bool = True):
        outputs = self.model(input_ids, position_ids=position_ids, attention_mask=attention_mask, deterministic=deterministic, init_cache=init_cache, output_attentions=output_attentions,
        output_hidden_states=output_hidden_states, return_dict=return_dict)
        hidden_states = outputs[0]
        lm_logits = self.lm_head(hidden_states)
        if not return_dict: return (lm_logits,) + outputs[1:]
        return FlaxCausalLMOutput(logits=lm_logits, hidden_states=outputs.hidden_states, attentions=outputs.attentions)
@add_start_docstrings("The Sastral Model transformer with a language modeling head (linear layer) on top.", SASTRAL_START_DOCSTRING)
class FlaxSastralForCausalLM(FlaxSastralPreTrainedModel):
    module_class = FlaxSastralForCausalLMModule
    ...
    def update_inputs_for_generation(self, model_outputs, model_kwargs):
        ...
        return model_kwargs
append_call_sample_docstring(FlaxSastralForCausalLM, _CHECKPOINT_FOR_DOC, FlaxCausalLMOutputWithCrossAttentions, _CONFIG_FOR_DOC, real_checkpoint=_REAL_CHECKPOINT_FOR_DOC)
"""
    #################################################################################################################################################################################################
    # This code library is an adaptation of the original Transformers and was designed, developed and programmed by Sapiens Technology®.                                                            #
    # Any alteration and/or disclosure of this code without prior authorization is strictly prohibited and is subject to legal action that will be forwarded by the Sapiens Technology® legal team. #
