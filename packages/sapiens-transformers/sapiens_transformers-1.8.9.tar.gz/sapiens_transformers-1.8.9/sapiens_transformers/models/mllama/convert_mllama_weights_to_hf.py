...
from sapiens_transformers import (GenerationConfig, MllamaConfig, MllamaForConditionalGeneration, MllamaImageProcessor, PreTrainedTokenizerFast)
from sapiens_transformers.convert_slow_tokenizer import TikTokenConverter
from sapiens_transformers.models.mllama.configuration_mllama import MllamaTextConfig, MllamaVisionConfig
from sapiens_transformers.models.mllama.image_processing_mllama import get_all_supported_aspect_ratios
ORIGINAL_TO_CONVERTED_KEY_MAPPING = {'text_model.norm.weight': 'language_model.model.norm.weight', 'text_model.output.weight': 'language_model.lm_head.weight',
...
def pre_compute_positional_embedding(embedding):
    ...
    precomputed_embeddings = torch.zeros(max_aspect_ratio_id + 1, max_num_tiles, num_patches, hidden_size, device=embedding.device, dtype=embedding.dtype)
...
def get_concat_dim(key):
    ...
    if any(re.search(pattern, key) for pattern in concat_dim_1): return 1
    return 0
def compute_intermediate_size(hidden_dim, multiple_of=1024, ffn_dim_multiplier=1.3):
    ...
    return hidden_dim
def interpolate_positional_embedding(embeddings: torch.Tensor, vision_tile_size: int, vision_patch_size: int) -> torch.Tensor:
    ...
    positional_embedding = F.interpolate(positional_embedding, size=(new_num_patches, new_num_patches), mode="bicubic", align_corners=False)
...
def write_model(model_path, input_base_path, num_shards, safe_serialization=True, instruct=False):
    ...
    with open(os.path.join(input_base_path, "params.json"), "r") as f: params = json.load(f)
    ...
    cross_attention_layers_shift = list(range(cross_attention_frequency - 1, text_num_total_layers, cross_attention_frequency + 1))
    ...
    pad_token_id = 128004
    text_config = MllamaTextConfig(num_attention_heads=text_num_heads, vocab_size=text_vocab_size, hidden_size=text_dim, rms_norm_eps=text_rms_norm_eps, rope_theta=text_rope_theta,
    ...
    vision_config = MllamaVisionConfig(hidden_size=vision_dim, patch_size=vision_patch_size, num_channels=vision_num_channels, intermediate_size=vision_intermediate_size,
    ...
    if num_shards == 1: loaded = [torch.load(os.path.join(input_base_path, "consolidated.pth"), map_location="cpu", mmap=True)]
    else: loaded = [torch.load(os.path.join(input_base_path, f"consolidated.{i:02d}.pth"), map_location="cpu", mmap=True) for i in range(num_shards)]
    ...
    for key in all_keys:
        ...
        if re.search("(k|v|q)_proj.weight", new_key) and "language_model" in new_key:
            ...
            shards = [param.view(vision_num_heads_per_shard, vision_dim_per_head, vision_dim) for param in current_parameter]
            ...
            state_dict[new_key] = current_parameter.reshape(-1, vision_num_channels, vision_patch_size, vision_patch_size)
            ...
            current_parameter = interpolate_positional_embedding(current_parameter, vision_tile_size, vision_patch_size)
            ...
            current_parameter = interpolate_positional_embedding(current_parameter, vision_tile_size, vision_patch_size)
            current_parameter = current_parameter.reshape(-1, vision_max_num_tiles, vision_max_num_tiles, vision_dim).permute(1, 2, 0, 3)
            ...
            if isinstance(current_parameter, list): current_parameter = torch.cat(current_parameter, dim=concat_dim)
            state_dict[new_key] = current_parameter
    state_dict["language_model.model.embed_tokens.weight"] = torch.cat([state_dict["language_model.model.embed_tokens.weight"], state_dict.pop("language_model.model.learnable_embedding.weight")], dim=0)
    del loaded
    gc.collect()
    print("Loading the checkpoint in a Mllama model.")
    with torch.device("meta"): model = MllamaForConditionalGeneration(config)
    ...
    if instruct:
        ...
        generation_config = GenerationConfig(do_sample=True, temperature=0.6, top_p=0.9, bos_token_id=bos_token_id, eos_token_id=eos_token_id, pad_token_id=pad_token_id)
...
def write_tokenizer(tokenizer_path: str, save_dir: str, instruct: bool = False):
    ...
    converter = MllamaConverter(vocab_file=tokenizer_path, pattern=pattern, special_tokens=special_tokens, model_max_length=model_max_length, chat_template=chat_template if instruct else None,
    ...
    if instruct:
        ...
        with open(chat_template_path, "w") as f: json.dump({"chat_template": chat_template}, f, indent=2)
def write_image_processor(config_path: str, save_dir: str):
    ...
    image_processor = MllamaImageProcessor(do_resize=True, size={"height": tile_size, "width": tile_size}, do_rescale=True, rescale_factor=1 / 255, do_normalize=True,
...
def main():
    ...
    parser.add_argument("--input_dir", default="Llama-3.2-11B-Vision/original", help="Location of LLaMA weights, which contains tokenizer.model and model folders")
    parser.add_argument("--output_dir", default="Llama-3.2-11B-Vision", help="Location to write HF model and tokenizer")
    ...
    write_tokenizer(tokenizer_path=os.path.join(args.input_dir, "tokenizer.model"), save_dir=args.output_dir, instruct=args.instruct)
    write_image_processor(config_path=os.path.join(args.input_dir, "params.json"), save_dir=args.output_dir)
