...
from sapiens_transformers import (AutoFeatureExtractor, AutoTokenizer, EncodecModel, MusicgenDecoderConfig, MusicgenForConditionalGeneration, MusicgenProcessor, T5EncoderModel)
from sapiens_transformers.models.musicgen.modeling_musicgen import MusicgenForCausalLM
...
def decoder_config_from_checkpoint(checkpoint: str) -> MusicgenDecoderConfig:
    ...
    else: raise ValueError(f"Checkpoint should be one of `['small', 'medium', 'large']` for the mono checkpoints, `['facebook/musicgen-stereo-small', 'facebook/musicgen-stereo-medium', 'facebook/musicgen-stereo-large']` for the stereo checkpoints, or a custom checkpoint with the checkpoint size as a suffix, got {checkpoint}.")
    if "stereo" in checkpoint:
        ...
        audio_channels = 1
        num_codebooks = 4
    config = MusicgenDecoderConfig(hidden_size=hidden_size, ffn_dim=hidden_size * 4, num_hidden_layers=num_hidden_layers, num_attention_heads=num_attention_heads,
