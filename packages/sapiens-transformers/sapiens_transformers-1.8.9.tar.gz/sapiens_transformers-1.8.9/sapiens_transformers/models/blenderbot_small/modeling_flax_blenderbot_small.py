...
class FlaxBlenderbotSmallDecoderLayer(nn.Module):
    ...
    def setup(self) -> None:
        ...
        self.activation_dropout_layer = nn.Dropout(rate=self.config.activation_dropout)
        self.self_attn_layer_norm = nn.LayerNorm(dtype=self.dtype, epsilon=1e-05)
        self.encoder_attn = FlaxBlenderbotSmallAttention(config=self.config, embed_dim=self.embed_dim, num_heads=self.config.decoder_attention_heads,
...
FLAX_BLENDERBOT_SMALL_CONDITIONAL_GENERATION_DOCSTRING = """
    ...
    ```py
    >>> from sapiens_transformers import AutoTokenizer, FlaxBlenderbotSmallForConditionalGeneration
