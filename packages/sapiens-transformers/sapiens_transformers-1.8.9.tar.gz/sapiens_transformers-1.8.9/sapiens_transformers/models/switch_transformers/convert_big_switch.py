...
from sapiens_transformers.models.switch_transformers.convert_switch_transformers_original_flax_checkpoint_to_pytorch import (rename_keys)
from sapiens_transformers.utils import WEIGHTS_INDEX_NAME, WEIGHTS_NAME
from sapiens_transformers.utils.hub import convert_file_size_to_int
...
def get_key_and_tensorstore_dict(layer, checkpoint_info, switch_checkpoint_path):
    ...
    if "kvstore/path" in layer: content = f"{switch_checkpoint_path}/{checkpoint_info[layer]}"
    elif "kvstore/driver" in layer: content = "file"
    else: content = checkpoint_info[layer]
    return curr_real_layer_name, split_layer, content
def rename_and_save_block(current_block, save_path):
    current_block = rename_keys(current_block)
    new_current_block = {}
    for k, v in current_block.items(): new_current_block[k.replace("/", ".")] = v
...
def shard_on_the_fly(switch_checkpoint_path, dump_path, max_shard_size, dtype, weights_name: str = WEIGHTS_NAME):
    ...
    for layer in checkpoint_info.keys():
        ...
        if curr_real_layer_name in all_layers: all_layers[curr_real_layer_name][split_layer[-1]] = content
        else: all_layers[curr_real_layer_name] = {split_layer[-1]: content}
...
def sanity_check():
    from sapiens_transformers import SwitchTransformersConfig, SwitchTransformersForConditionalGeneration, T5Tokenizer
