...
if is_sentencepiece_available(): from .tokenization_entity import EntityTokenizer
else: EntityTokenizer = None
...
class EntityTokenizerFast(PreTrainedTokenizerFast):
    vocab_files_names, slow_tokenizer_class, padding_side, model_input_names = VOCAB_FILES_NAMES, EntityTokenizer, "left", ["input_ids", "attention_mask"]
    ...
    def update_post_processor(self):
        ...
        eos, eos_token_id = self.eos_token, self.eos_token_id
        if eos is None and self.add_eos_token: raise ValueError("add_eos_token = True but eos_token = None")
        single = f"{(bos+':0 ') if self.add_bos_token else ''}$A:0{(' '+eos+':0') if self.add_eos_token else ''}"
        pair, special_tokens = f"{single}{(' '+bos+':1') if self.add_bos_token else ''} $B:1{(' '+eos+':1') if self.add_eos_token else ''}", []
