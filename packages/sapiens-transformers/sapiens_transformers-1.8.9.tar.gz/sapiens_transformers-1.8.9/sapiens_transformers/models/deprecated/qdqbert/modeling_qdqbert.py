...
MultipleChoiceModelOutput, NextSentencePredictorOutput, QuestionAnsweringModelOutput, SequenceClassifierOutput, TokenClassifierOutput)
...
if is_pytorch_quantization_available():
    ...
    except OSError: logger.error("QDQBERT model are not usable since `pytorch_quantization` can't be loaded. Please try to reinstall it following the instructions here: https://github.com/NVIDIA/TensorRT/tree/master/tools/pytorch-quantization.")
...
class QDQBertModel(QDQBertPreTrainedModel):
    ...
    output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, BaseModelOutputWithPoolingAndCrossAttentions]:
...
class QDQBertLMHeadModel(QDQBertPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        if not config.is_decoder: logger.warning("If you want to use `QDQBertLMHeadModel` as a standalone, add `is_decoder=True.`")
    ...
    @add_start_docstrings_to_model_forward(QDQBERT_INPUTS_DOCSTRING.format("batch_size, sequence_length"))
    def forward(self, input_ids: Optional[torch.LongTensor] = None, attention_mask: Optional[torch.Tensor] = None, token_type_ids: Optional[torch.LongTensor] = None,
    position_ids: Optional[torch.LongTensor] = None, head_mask: Optional[torch.Tensor] = None, inputs_embeds: Optional[torch.Tensor] = None, encoder_hidden_states: Optional[torch.FloatTensor] = None,
    encoder_attention_mask: Optional[torch.FloatTensor] = None, labels: Optional[torch.LongTensor] = None, past_key_values: Optional[Tuple[Tuple[torch.LongTensor]]] = None,
...
class QDQBertForMaskedLM(QDQBertPreTrainedModel):
    ...
    def __init__(self, config):
        super().__init__(config)
        if config.is_decoder: logger.warning("If you want to use `QDQBertForMaskedLM` make sure `config.is_decoder=False` for bi-directional self-attention.")
        self.bert = QDQBertModel(config, add_pooling_layer=False)
        self.cls = QDQBertOnlyMLMHead(config)
        self.post_init()
    def get_output_embeddings(self): return self.cls.predictions.decoder
...
@add_start_docstrings("Bert Model transformer with a sequence classification/regression head on top (a linear layer on top of the pooled output) e.g. for GLUE tasks.", QDQBERT_START_DOCSTRING)
...
@add_start_docstrings("Bert Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a softmax) e.g. for RocStories/SWAG tasks.", QDQBERT_START_DOCSTRING)
...
@add_start_docstrings("QDQBERT Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks.", QDQBERT_START_DOCSTRING)
...
@add_start_docstrings("QDQBERT Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layers on top of the hidden-states output to compute `span start logits` and `span end logits`).", QDQBERT_START_DOCSTRING)
