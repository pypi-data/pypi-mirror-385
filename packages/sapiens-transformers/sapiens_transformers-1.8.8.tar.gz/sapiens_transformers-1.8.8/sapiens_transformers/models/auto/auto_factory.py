...
if is_torch_available(): from ...generation import GenerationMixin
...
FROM_CONFIG_DOCSTRING = """
        ...
        Examples:
        ```python
        >>> from sapiens_transformers import AutoConfig, BaseAutoModelClass
        >>> # Download configuration and cache.
...
FROM_PRETRAINED_TORCH_DOCSTRING = """
        ...
        Args:
            ...
            resume_download:
                ...
                Will be removed in v1 of Sapiens Transformers.
            ...
            revision (`str`, *optional*, defaults to `"main"`):
                ...
                git-based system for storing models and other artifacts, so `revision` can be any
            ...
            code_revision (`str`, *optional*, defaults to `"main"`):
                ...
                system for storing models and other artifacts, so `revision` can be any identifier
                allowed by git.
        ...
        ```python
        >>> from sapiens_transformers import AutoConfig, BaseAutoModelClass
        >>> # Download model and configuration and cache.
...
def _get_model_class(config, model_mapping):
    ...
    if not isinstance(supported_models, (list, tuple)): return supported_models
    name_to_model = {model.__name__: model for model in supported_models}
    architectures = getattr(config, "architectures", [])
    for arch in architectures:
        if arch in name_to_model: return name_to_model[arch]
        elif f"TF{arch}" in name_to_model: return name_to_model[f"TF{arch}"]
        elif f"Flax{arch}" in name_to_model: return name_to_model[f"Flax{arch}"]
    return supported_models[0]
class _BaseAutoModelClass:
    _model_mapping = None
    def __init__(self, *args, **kwargs): raise EnvironmentError(f"{self.__class__.__name__} is designed to be instantiated using the `{self.__class__.__name__}.from_pretrained(pretrained_model_name_or_path)` or `{self.__class__.__name__}.from_config(config)` methods.")
    @classmethod
    def from_config(cls, config, **kwargs):
        ...
        if has_remote_code and trust_remote_code:
            ...
            if "--" in class_ref: repo_id, class_ref = class_ref.split("--")
            else: repo_id = config.name_or_path
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if token is not None: hub_kwargs["token"] = token
        ...
        if is_peft_available():
            if adapter_kwargs is None:
                adapter_kwargs = {}
                if token is not None: adapter_kwargs["token"] = token
        ...
        if not isinstance(config, PretrainedConfig):
            ...
            if kwargs.get("torch_dtype", None) == "auto": _ = kwargs.pop("torch_dtype")
            if kwargs.get("quantization_config", None) is not None: _ = kwargs.pop("quantization_config")
            ...
            if kwargs_orig.get("torch_dtype", None) == "auto": kwargs["torch_dtype"] = "auto"
            if kwargs_orig.get("quantization_config", None) is not None: kwargs["quantization_config"] = kwargs_orig["quantization_config"]
        has_remote_code = hasattr(config, "auto_map") and cls.__name__ in config.auto_map
        has_local_code = type(config) in cls._model_mapping.keys()
    ...
    def register(cls, config_class, model_class, exist_ok=False):
        if hasattr(model_class, "config_class") and str(model_class.config_class) != str(config_class): raise ValueError(f"The model class you are passing has a `config_class` attribute that is not consistent with the config class you passed (model has {model_class.config_class} and you passed {config_class}. Fix one of those so they match!")
...
class _BaseAutoBackboneClass(_BaseAutoModelClass):
    ...
    def _load_timm_backbone_from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if kwargs.get("out_features", None) is not None: raise ValueError("Cannot specify `out_features` for timm backbones")
        if kwargs.get("output_loading_info", False): raise ValueError("Cannot specify `output_loading_info=True` when loading from timm")
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        if use_timm_backbone: return cls._load_timm_backbone_from_pretrained(pretrained_model_name_or_path, *model_args, **kwargs)
        return super().from_pretrained(pretrained_model_name_or_path, *model_args, **kwargs)
def insert_head_doc(docstring, head_doc=""):
    if len(head_doc) > 0: return docstring.replace("one of the model classes of the library ", f"one of the model classes of the library (with a {head_doc} head) ")
...
def auto_class_update(cls, checkpoint_for_example="google-bert/bert-base-cased", head_doc=""):
    ...
    if name.startswith("TF"): from_pretrained_docstring = FROM_PRETRAINED_TF_DOCSTRING
    elif name.startswith("Flax"): from_pretrained_docstring = FROM_PRETRAINED_FLAX_DOCSTRING
    else: from_pretrained_docstring = FROM_PRETRAINED_TORCH_DOCSTRING
...
def get_values(model_mapping):
    ...
    for model in model_mapping.values():
        if isinstance(model, (list, tuple)): result += list(model)
        else: result.append(model)
    return result
def getattribute_from_module(module, attr):
    ...
    if module != transformers_module:
        try: return getattribute_from_module(transformers_module, attr)
        except ValueError: raise ValueError(f"Could not find {attr} neither in {module} nor in {transformers_module}!")
    else: raise ValueError(f"Could not find {attr} in {transformers_module}!")
def add_generation_mixin_to_remote_model(model_class):
    if "torch.nn.modules.module.Module" not in str(model_class.__mro__): return model_class
    if "GenerationMixin" in str(model_class.__bases__): return model_class
    ...
    if has_custom_generate or has_custom_prepare_inputs:
        model_class_with_generation_mixin = type(model_class.__name__, (model_class, GenerationMixin), {**model_class.__dict__})
...
class _LazyAutoMapping(OrderedDict):
    ...
    def register(self, key, value, exist_ok=False):
        if hasattr(key, "__name__") and key.__name__ in self._reverse_config_mapping:
            model_type = self._reverse_config_mapping[key.__name__]
            if model_type in self._model_mapping.keys() and not exist_ok: raise ValueError(f"'{key}' is already used by a Transformers model.")
