...
class FlaxMistralLayerCollection(nn.Module):
    config: MistralConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.blocks = [FlaxMistralDecoderLayer(self.config, dtype=self.dtype, name=str(i)) for i in range(self.config.num_hidden_layers)]
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, deterministic: bool = True, init_cache: bool = False, output_attentions: bool = False,
...
class FlaxMistralModel(FlaxMistralPreTrainedModel): module_class = FlaxMistralModule
...
@add_start_docstrings("The Mistral Model transformer with a language modeling head (linear layer) on top.", MISTRAL_START_DOCSTRING)
