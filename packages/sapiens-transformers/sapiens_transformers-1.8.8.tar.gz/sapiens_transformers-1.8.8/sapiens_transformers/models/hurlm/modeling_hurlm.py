...
from .configuration_hurlm import HurLMConfig, HurLMVisionConfig
...
import torch
_CONFIG_FOR_DOC = "HurLMConfig"
@dataclass
class HurLMBaseModelOutputWithPast(ModelOutput):
    """Args:"""
...
@dataclass
class HurLMCausalLMOutputWithPast(ModelOutput):
    """Args:"""
    ...
    attentions: Optional[Tuple[torch.FloatTensor]] = None
    image_hidden_states: Optional[Tuple[torch.FloatTensor]] = None
class HurLMVisionEmbeddings(nn.Module):
    def __init__(self, config: HurLMVisionConfig):
        super().__init__()
        self.embed_dim = config.hidden_size
        self.image_size = config.image_size
        self.patch_size = config.patch_size
        self.patch_embedding = nn.Conv2d(in_channels=config.num_channels, out_channels=self.embed_dim, kernel_size=self.patch_size,
    ...
    def forward(self, pixel_values: torch.FloatTensor, patch_attention_mask: torch.BoolTensor) -> torch.Tensor:
        ...
        return embeddings
class HurLMVisionAttention(nn.Module):
...
class HurLMVisionFlashAttention2(HurLMVisionAttention):
    ...
    **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        ...
        attn_output = _flash_attention_forward(query_states, key_states, value_states, attention_mask, q_len, dropout=dropout_rate,
        ...
        return attn_output, attn_weights
IDEFICS_VISION_ATTENTION_CLASSES = {"eager": HurLMVisionAttention, "flash_attention_2": HurLMVisionFlashAttention2}
class HurLMVisionMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
...
class HurLMSimpleMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
...
class HurLMEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, attention_mask: torch.Tensor, output_attentions: Optional[bool] = False) -> Tuple[torch.FloatTensor]:
        ...
        return outputs
class HurLMEncoder(nn.Module):
    def __init__(self, config: HurLMConfig):
        super().__init__()
        self.config = config
        self.layers = nn.ModuleList([HurLMEncoderLayer(config) for _ in range(config.num_hidden_layers)])
...
class HurLMConnector(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.scale_factor = config.scale_factor
        self.modality_projection = HurLMSimpleMLP(config)
...
HURLM_START_DOCSTRING = r"""
    ...
    Parameters:
        config ([`HurLMConfig`] or [`HurLMVisionConfig`]):
            Model configuration class with all the parameters of the model. Initializing with a config file does not
            load the weights associated with the model, only the configuration. Check out the
            [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""
@add_start_docstrings("The bare HurLM Model outputting raw hidden-states without any specific head on top.", HURLM_START_DOCSTRING)
class HurLMPreTrainedModel(PreTrainedModel):
    config_class = HurLMConfig
    base_model_prefix = "model"
    supports_gradient_checkpointing = True
    _no_split_modules = ["HurLMVisionAttention", "HurLMDecoderLayer"]
...
HURLM_VISION_START_DOCSTRING = r"""
    This model inherits from [`PreTrainedModel`]. Check the superclass documentation for the generic methods the
    library implements for all its model (such as downloading or saving, resizing the input embeddings, pruning heads
    etc.)
    ...
    Parameters:
        config ([`HurLMVisionConfig`]):
            Model configuration class with all the parameters of the model. Initializing with a config file does not
            load the weights associated with the model, only the configuration. Check out the
            [`~PreTrainedModel.from_pretrained`] method to load the model weights.
"""
@add_start_docstrings("The HurLM Vision Transformer Model outputting raw image embedding.", HURLM_VISION_START_DOCSTRING)
class HurLMVisionTransformer(HurLMPreTrainedModel):
    config_class = HurLMVisionConfig
    _supports_sdpa = False
    def __init__(self, config: HurLMVisionConfig):
        super().__init__(config)
        embed_dim = config.hidden_size
        self.embeddings = HurLMVisionEmbeddings(config)
        self.encoder = HurLMEncoder(config)
        self.patch_size = config.patch_size
    ...
    def forward(self, pixel_values, patch_attention_mask: Optional[torch.BoolTensor] = None, output_attentions: Optional[bool] = None,
    output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, BaseModelOutput]:
        ...
        encoder_outputs = self.encoder(inputs_embeds=hidden_states, attention_mask=patch_attention_mask, output_attentions=output_attentions,
...
"""
@add_start_docstrings("""HurLM model consisting of a SIGLIP vision encoder and Llama3 language decoder""", HURLM_START_DOCSTRING)
class HurLMModel(HurLMPreTrainedModel):
    def __init__(self, config: HurLMConfig):
        super().__init__(config)
        self.padding_idx = self.config.text_config.pad_token_id
        self.vocab_size = self.config.text_config.vocab_size
        self.vision_model = HurLMVisionTransformer._from_config(config.vision_config)
        self.connector = HurLMConnector(config)
    ...
    pixel_attention_mask: Optional[torch.BoolTensor] = None, image_hidden_states: Optional[torch.FloatTensor] = None, use_cache: Optional[bool] = None,
    output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, HurLMBaseModelOutputWithPast]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        use_cache = use_cache if use_cache is not None else self.config.use_cache
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        if self.training and self.text_model.gradient_checkpointing and use_cache: use_cache = False
        ...
        if inputs_embeds is None: inputs_embeds = self.text_model.get_input_embeddings()(input_ids).to(self.device)
        ...
        outputs = self.text_model(inputs_embeds=inputs_embeds, attention_mask=attention_mask, position_ids=position_ids, past_key_values=past_key_values,
        ...
        return HurLMBaseModelOutputWithPast(last_hidden_state=outputs.last_hidden_state, past_key_values=outputs.past_key_values,
        hidden_states=outputs.hidden_states, attentions=outputs.attentions, image_hidden_states=image_hidden_states)
@add_start_docstrings("""The HurLM Model with a language modeling head. It is made up a SigLIP vision encoder, with a language modeling head on top. """, HURLM_START_DOCSTRING)
class HurLMForConditionalGeneration(HurLMPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    def __init__(self, config):
        super().__init__(config)
        self.model = HurLMModel(config)
        self.image_token_id = self.config.image_token_id
    ...
    @add_start_docstrings_to_model_forward(HURLM_INPUTS_DOCSTRING)
    def forward(self, input_ids: torch.LongTensor = None, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.LongTensor] = None,
    past_key_values: Optional[List[torch.FloatTensor]] = None, inputs_embeds: Optional[torch.FloatTensor] = None, pixel_values: Optional[torch.FloatTensor] = None,
    ...
    return_dict: Optional[bool] = None) -> Union[Tuple, HurLMCausalLMOutputWithPast]:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        inputs_embeds=inputs_embeds, pixel_values=pixel_values, pixel_attention_mask=pixel_attention_mask, image_hidden_states=image_hidden_states,
        use_cache=use_cache, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict)
        ...
        return HurLMCausalLMOutputWithPast(loss=loss, logits=logits, past_key_values=outputs.past_key_values, hidden_states=outputs.hidden_states,
        attentions=outputs.attentions, image_hidden_states=outputs.image_hidden_states)
    def prepare_inputs_for_generation(self, input_ids, past_key_values=None, attention_mask=None, inputs_embeds=None, cache_position=None,
    pixel_values=None, pixel_attention_mask=None, image_hidden_states=None, num_logits_to_keep=None, **kwargs):
        ...
        if inputs_embeds is not None and cache_position[0] == 0: model_inputs = {"inputs_embeds": inputs_embeds, "input_ids": input_ids}
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs, is_encoder_decoder, **kwargs):
        ...
        return model_kwargs
__all__ = ["HurLMForConditionalGeneration", "HurLMPreTrainedModel", "HurLMModel", "HurLMVisionTransformer"]
"""
    #################################################################################################################################################################################################
    # This code library is an adaptation of the original Transformers and was designed, developed and programmed by Sapiens Technology®.                                                            #
    # Any alteration and/or disclosure of this code without prior authorization is strictly prohibited and is subject to legal action that will be forwarded by the Sapiens Technology® legal team. #
