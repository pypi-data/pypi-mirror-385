...
from sapiens_transformers import (MobileNetV1Config, MobileNetV1ForImageClassification, MobileNetV1ImageProcessor, load_tf_weights_in_mobilenet_v1)
...
def get_mobilenet_v1_config(model_name):
    ...
    if "_quant" in model_name: raise ValueError("Quantized models are not supported.")
...
def convert_movilevit_checkpoint(model_name, checkpoint_path, pytorch_dump_folder_path, push_to_hub=False):
    ...
    if model_name == "mobilenet_v1_1.0_224": expected_logits = torch.tensor([-4.1739, -1.1233, 3.1205])
    elif model_name == "mobilenet_v1_0.75_192": expected_logits = torch.tensor([-3.9440, -2.3141, -0.3333])
