...
from sapiens_transformers import Wav2Vec2Processor, logging
from sapiens_transformers.models.data2vec.configuration_data2vec_audio import Data2VecAudioConfig
from sapiens_transformers.models.data2vec.data2vec_audio import Data2VecAudioModel as Dummy
from sapiens_transformers.models.data2vec.modeling_data2vec_audio import Data2VecAudioForCTC, Data2VecAudioModel
logging.set_verbosity_info()
logger = logging.get_logger(__name__)
MAPPING = {'post_extract_proj': 'feature_projection.projection', 'models.0.layer_norm': 'feature_projection.layer_norm', 'self_attn.k_proj': 'encoder.layers.*.attention.k_proj',
...
'w2v_encoder.proj': 'lm_head', 'mask_emb': 'masked_spec_embed'}
    ...
    for name, value in fairseq_dict.items():
        ...
        if "conv_layers" in name:
            ...
            for key, mapped_key in MAPPING.items():
                if not is_headless: mapped_key = "data2vec_audio." + mapped_key if mapped_key not in TOP_LEVEL_KEYS else mapped_key
...
def set_weights(full_name, module, fsq_value, hf_weight_path):
    ...
    if fsq_value.shape != hf_value.shape: raise ValueError(f"{full_name} has size {fsq_value.shape}, but {hf_value.shape} was found.")
...
def load_conv_layer(full_name, value, feature_extractor, unused_weights):
    ...
    if type_id == 0: layer_type = "conv"
    elif type_id == 2: layer_type = "layer_norm"
    else:
...
def convert_wav2vec2_checkpoint(checkpoint_path, pytorch_dump_folder_path, config_path=None, dict_path=None, is_finetuned=True):
    if config_path is not None: config = Data2VecAudioConfig.from_pretrained(config_path)
    else: config = Data2VecAudioConfig()
    if not is_finetuned:
    ...
    if is_finetuned: processor.save_pretrained(pytorch_dump_folder_path)
    else: processor.feature_extractor.save_pretrained(pytorch_dump_folder_path)
