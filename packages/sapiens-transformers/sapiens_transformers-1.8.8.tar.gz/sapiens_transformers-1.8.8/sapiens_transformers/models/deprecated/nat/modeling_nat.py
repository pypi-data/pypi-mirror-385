...
from ....utils import (ModelOutput, OptionalDependencyNotAvailable, add_code_sample_docstrings, add_start_docstrings, add_start_docstrings_to_model_forward, is_natten_available,
logging, replace_return_docstrings, requires_backends)
from ....utils.backbone_utils import BackboneMixin
from .configuration_nat import NatConfig
if is_natten_available(): from natten.functional import natten2dav, natten2dqkrpb
else:
    def natten2dqkrpb(*args, **kwargs): raise OptionalDependencyNotAvailable()
    def natten2dav(*args, **kwargs): raise OptionalDependencyNotAvailable()
...
class NatPatchEmbeddings(nn.Module):
    def __init__(self, config):
        ...
        if patch_size == 4: pass
        else: raise ValueError("Dinat only supports patch size of 4 at the moment.")
...
class NatLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.Tensor, output_attentions: Optional[bool] = False) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if was_padded: attention_output = attention_output[:, :height, :width, :].contiguous()
        if self.layer_scale_parameters is not None: attention_output = self.layer_scale_parameters[0] * attention_output
        hidden_states = shortcut + self.drop_path(attention_output)
        layer_output = self.layernorm_after(hidden_states)
        layer_output = self.output(self.intermediate(layer_output))
        if self.layer_scale_parameters is not None: layer_output = self.layer_scale_parameters[1] * layer_output
...
class NatStage(nn.Module):
    def __init__(self, config, dim, depth, num_heads, drop_path_rate, downsample):
        ...
        if downsample is not None: self.downsample = downsample(dim=dim, norm_layer=nn.LayerNorm)
    ...
    def forward(self, hidden_states: torch.Tensor, output_attentions: Optional[bool] = False) -> Tuple[torch.Tensor]:
        ...
        if self.downsample is not None: hidden_states = self.downsample(hidden_states_before_downsampling)
...
class NatEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.levels = nn.ModuleList([NatStage(config=config, dim=int(config.embed_dim * 2**i_layer), depth=config.depths[i_layer], num_heads=config.num_heads[i_layer],
        drop_path_rate=dpr[sum(config.depths[:i_layer]) : sum(config.depths[: i_layer + 1])], downsample=NatDownsampler if (i_layer < self.num_levels - 1) else None) for i_layer in range(self.num_levels)])
    def forward(self, hidden_states: torch.Tensor, output_attentions: Optional[bool] = False, output_hidden_states: Optional[bool] = False, output_hidden_states_before_downsampling: Optional[bool] = False,
    return_dict: Optional[bool] = True) -> Union[Tuple, NatEncoderOutput]:
...
@add_start_docstrings("Nat Model transformer with an image classification head on top (a linear layer on top of the final hidden state of the [CLS] token) e.g. for ImageNet.", NAT_START_DOCSTRING)
class NatForImageClassification(NatPreTrainedModel):
    def __init__(self, config):
        ...
        self.post_init()
    @add_start_docstrings_to_model_forward(NAT_INPUTS_DOCSTRING)
    def forward(self, pixel_values: Optional[torch.FloatTensor] = None, labels: Optional[torch.LongTensor] = None, output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None,
    return_dict: Optional[bool] = None) -> Union[Tuple, NatImageClassifierOutput]:
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
...
class NatBackbone(NatPreTrainedModel, BackboneMixin):
    def __init__(self, config):
        ...
        hidden_states_norms = {}
        for stage, num_channels in zip(self.out_features, self.channels): hidden_states_norms[stage] = nn.LayerNorm(num_channels)
