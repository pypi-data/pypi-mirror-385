...
from ...utils import BACKENDS_MAPPING, PIL_INTERPOLATION, is_bs4_available, is_ftfy_available, replace_example_docstring
...
class IFInpaintingSuperResolutionPipeline(DiffusionPipeline, StableDiffusionLoraLoaderMixin):
    ...
    safety_checker: Optional[IFSafetyChecker], feature_extractor: Optional[CLIPImageProcessor], watermarker: Optional[IFWatermarker], requires_safety_checker: bool=True):
        ...
        self.register_modules(tokenizer=tokenizer, text_encoder=text_encoder, unet=unet, scheduler=scheduler, image_noising_scheduler=image_noising_scheduler,
        safety_checker=safety_checker, feature_extractor=feature_extractor, watermarker=watermarker)
    ...
    negative_prompt: Optional[Union[str, List[str]]]=None, prompt_embeds: Optional[torch.Tensor]=None, negative_prompt_embeds: Optional[torch.Tensor]=None, clean_caption: bool=False):
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            uncond_input = self.tokenizer(uncond_tokens, padding='max_length', max_length=max_length,
    ...
    def check_inputs(self, prompt, image, original_image, mask_image, batch_size, callback_steps, negative_prompt=None, prompt_embeds=None, negative_prompt_embeds=None):
        ...
        if isinstance(original_image, list): check_image_type = original_image[0]
        else: check_image_type = original_image
        if not isinstance(check_image_type, torch.Tensor) and (not isinstance(check_image_type, PIL.Image.Image)) and (not isinstance(check_image_type, np.ndarray)): raise ValueError(f'`original_image` has to be of type `torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, or List[...] but is {type(check_image_type)}')
        if isinstance(original_image, list): image_batch_size = len(original_image)
        elif isinstance(original_image, torch.Tensor): image_batch_size = original_image.shape[0]
        elif isinstance(original_image, PIL.Image.Image): image_batch_size = 1
        elif isinstance(original_image, np.ndarray): image_batch_size = original_image.shape[0]
        else: assert False
        if batch_size != image_batch_size: raise ValueError(f'original_image batch size: {image_batch_size} must be same as prompt batch size {batch_size}')
        if isinstance(mask_image, list): check_image_type = mask_image[0]
        else: check_image_type = mask_image
        if not isinstance(check_image_type, torch.Tensor) and (not isinstance(check_image_type, PIL.Image.Image)) and (not isinstance(check_image_type, np.ndarray)): raise ValueError(f'`mask_image` has to be of type `torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, or List[...] but is {type(check_image_type)}')
        if isinstance(mask_image, list): image_batch_size = len(mask_image)
        elif isinstance(mask_image, torch.Tensor): image_batch_size = mask_image.shape[0]
        elif isinstance(mask_image, PIL.Image.Image): image_batch_size = 1
        elif isinstance(mask_image, np.ndarray): image_batch_size = mask_image.shape[0]
        else: assert False
        if image_batch_size != 1 and batch_size != image_batch_size: raise ValueError(f'mask_image batch size: {image_batch_size} must be `1` or the same as prompt batch size {batch_size}')
    def preprocess_original_image(self, image: PIL.Image.Image) -> torch.Tensor:
        ...
        if isinstance(image[0], PIL.Image.Image):
            ...
            image = numpy_to_pt(image)
        elif isinstance(image[0], torch.Tensor): image = torch.cat(image, axis=0) if image[0].ndim == 4 else torch.stack(image, axis=0)
        return image
    ...
    def preprocess_mask_image(self, mask_image) -> torch.Tensor:
        if not isinstance(mask_image, list): mask_image = [mask_image]
        if isinstance(mask_image[0], torch.Tensor):
            ...
            if mask_image.ndim == 2: mask_image = mask_image.unsqueeze(0).unsqueeze(0)
            elif mask_image.ndim == 3 and mask_image.shape[0] == 1: mask_image = mask_image.unsqueeze(0)
            elif mask_image.ndim == 3 and mask_image.shape[0] != 1: mask_image = mask_image.unsqueeze(1)
    ...
    def prepare_intermediate_images(self, image, timestep, batch_size, num_images_per_prompt, dtype, device, mask_image, generator=None):
    ...
    def __call__(self, image: Union[PIL.Image.Image, np.ndarray, torch.Tensor], original_image: Union[PIL.Image.Image, torch.Tensor, np.ndarray,
    List[PIL.Image.Image], List[torch.Tensor], List[np.ndarray]]=None, mask_image: Union[PIL.Image.Image, torch.Tensor, np.ndarray,
    List[PIL.Image.Image], List[torch.Tensor], List[np.ndarray]]=None, strength: float=0.8, prompt: Union[str, List[str]]=None, num_inference_steps: int=100,
    ...
    cross_attention_kwargs: Optional[Dict[str, Any]]=None, noise_level: int=0, clean_caption: bool=True):
        """Examples:"""
        if prompt is not None and isinstance(prompt, str): batch_size = 1
        elif prompt is not None and isinstance(prompt, list): batch_size = len(prompt)
        else: batch_size = prompt_embeds.shape[0]
        self.check_inputs(prompt, image, original_image, mask_image, batch_size, callback_steps, negative_prompt, prompt_embeds, negative_prompt_embeds)
        do_classifier_free_guidance = guidance_scale > 1.0
        device = self._execution_device
        prompt_embeds, negative_prompt_embeds = self.encode_prompt(prompt, do_classifier_free_guidance, num_images_per_prompt=num_images_per_prompt, device=device,
        negative_prompt=negative_prompt, prompt_embeds=prompt_embeds, negative_prompt_embeds=negative_prompt_embeds, clean_caption=clean_caption)
        ...
        if mask_image.shape[0] == 1: mask_image = mask_image.repeat_interleave(batch_size * num_images_per_prompt, dim=0)
        else: mask_image = mask_image.repeat_interleave(num_images_per_prompt, dim=0)
        noise_timestep = timesteps[0:1]
        noise_timestep = noise_timestep.repeat(batch_size * num_images_per_prompt)
        intermediate_images = self.prepare_intermediate_images(original_image, noise_timestep, batch_size, num_images_per_prompt, dtype, device, mask_image, generator)
