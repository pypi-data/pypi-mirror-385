...
from ...models.sapiens_transformers import SD3Transformer2DModel
...
class StableDiffusion3ControlNetPipeline(DiffusionPipeline, SD3LoraLoaderMixin, FromSingleFileMixin):
    ...
    def __init__(self, transformer: SD3Transformer2DModel, scheduler: FlowMatchEulerDiscreteScheduler, vae: AutoencoderKL, text_encoder: CLIPTextModelWithProjection, tokenizer: CLIPTokenizer,
    text_encoder_2: CLIPTextModelWithProjection, tokenizer_2: CLIPTokenizer, text_encoder_3: T5EncoderModel, tokenizer_3: T5TokenizerFast, controlnet: Union[SD3ControlNetModel,
    List[SD3ControlNetModel], Tuple[SD3ControlNetModel], SD3MultiControlNetModel]):
        super().__init__()
        if isinstance(controlnet, (list, tuple)): controlnet = SD3MultiControlNetModel(controlnet)
    ...
    max_sequence_length: int=256, lora_scale: Optional[float]=None):
        ...
        if self.text_encoder is not None:
            if isinstance(self, SD3LoraLoaderMixin) and USE_PEFT_BACKEND: unscale_lora_layers(self.text_encoder, lora_scale)
        if self.text_encoder_2 is not None:
            if isinstance(self, SD3LoraLoaderMixin) and USE_PEFT_BACKEND: unscale_lora_layers(self.text_encoder_2, lora_scale)
    ...
    callback_on_step_end_tensor_inputs: List[str]=['latents'], max_sequence_length: int=256):
        ...
        controlnet_config = self.controlnet.config if isinstance(self.controlnet, SD3ControlNetModel) else self.controlnet.nets[0].config
