...
class MvpAttention(nn.Module):
    ...
    attention_mask: Optional[torch.Tensor] = None, layer_head_mask: Optional[torch.Tensor] = None, attn_prompt: Optional[torch.Tensor] = None,
...
class MvpEncoderLayer(nn.Module):
    ...
    def forward(self, hidden_states: torch.FloatTensor, attention_mask: torch.FloatTensor, layer_head_mask: torch.FloatTensor, self_attn_prompt: torch.FloatTensor,
    output_attentions: Optional[bool] = False) -> Tuple[torch.FloatTensor, Optional[torch.FloatTensor]]:
        residual = hidden_states
        hidden_states, attn_weights, _ = self.self_attn(hidden_states=hidden_states, attention_mask=attention_mask, layer_head_mask=layer_head_mask,
        attn_prompt=self_attn_prompt, output_attentions=output_attentions)
...
class MvpDecoderLayer(nn.Module):
    ...
    self_attn_prompt: Optional[torch.Tensor] = None, cross_attn_prompt: Optional[torch.Tensor] = None, past_key_value: Optional[Tuple[torch.Tensor]] = None,
    output_attentions: Optional[bool] = False, use_cache: Optional[bool] = True) -> Tuple[torch.FloatTensor, Optional[Tuple[torch.FloatTensor, torch.FloatTensor]]]:
        ...
        layer_head_mask=layer_head_mask, attn_prompt=self_attn_prompt, output_attentions=output_attentions)
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout, training=self.training)
        hidden_states = residual + hidden_states
        hidden_states = self.self_attn_layer_norm(hidden_states)
        ...
        if encoder_hidden_states is not None:
            ...
            attention_mask=encoder_attention_mask, layer_head_mask=cross_attn_layer_head_mask, attn_prompt=cross_attn_prompt, past_key_value=cross_attn_past_key_value,
...
"""
loss_model = 'model(**inputs, labels=labels)'
model_generate = 'model.generate(**inputs)'
MVP_CONDITIONAL_GENERATION_EXAMPLE = r"""
    Example of summarization:
    Fine-tuning a model
    ```python
    >>> import torch
    >>> from sapiens_transformers import AutoTokenizer, MvpForConditionalGeneration
    ...
    ```
"""
logits_model = 'model(**inputs)'
MVP_SEQUENCE_CLASSIFICATION_SAMPLE = r"""
    Example of single-label classification:
    Fine-tuning a model on `num_labels` classes
    ```python
    >>> import torch
    >>> from sapiens_transformers import AutoTokenizer, MvpForSequenceClassification
    ...
    ```python
    >>> with torch.no_grad():
    ...     logits = """+logits_model+r""".logits
    >>> predicted_class_id = logits.argmax()
    ```
"""
loss_model = 'model(**inputs, start_positions=target_start_index, end_positions=target_end_index)'
...
MVP_QUESTION_ANSWERING_SAMPLE = r"""
    ...
    ```python
    >>> import torch
    >>> from sapiens_transformers import AutoTokenizer, MvpForQuestionAnswering
    >>> tokenizer = AutoTokenizer.from_pretrained("RUCAIBox/mvp")
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            if to_drop: layer_outputs = (None, None)
                ...
                if self.gradient_checkpointing and self.training:
                    ...
                    (self_attn_prompt[idx] if self.use_prompt else None), output_attentions)
                else:
                    layer_outputs = encoder_layer(hidden_states, attention_mask, layer_head_mask=(head_mask[idx] if head_mask is not None else None),
                    self_attn_prompt=(self_attn_prompt[idx] if self.use_prompt else None), output_attentions=output_attentions)
    ...
    output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, BaseModelOutputWithPastAndCrossAttentions]:
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                head_mask[idx] if head_mask is not None else None, cross_attn_head_mask[idx] if cross_attn_head_mask is not None else None, self_attn_prompt[idx] if self.use_prompt else None,
                cross_attn_prompt[idx] if self.use_prompt else None, None, output_attentions, use_cache)
            else:
                ...
                self_attn_prompt=(self_attn_prompt[idx] if self.use_prompt else None), cross_attn_prompt=(cross_attn_prompt[idx] if self.use_prompt else None),
...
class MvpModel(MvpPreTrainedModel):
    ...
    output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, Seq2SeqModelOutput]:
...
class MvpForSequenceClassification(MvpPreTrainedModel):
    ...
    output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, Seq2SeqSequenceClassifierOutput]:
        ...
        decoder_attentions=outputs.decoder_attentions, cross_attentions=outputs.cross_attentions, encoder_last_hidden_state=outputs.encoder_last_hidden_state,
        encoder_hidden_states=outputs.encoder_hidden_states, encoder_attentions=outputs.encoder_attentions)
@add_start_docstrings("MVP Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`).", MVP_START_DOCSTRING)
class MvpForQuestionAnswering(MvpPreTrainedModel):
    ...
    def forward(self, input_ids: torch.Tensor = None, attention_mask: Optional[torch.Tensor] = None, decoder_input_ids: Optional[torch.LongTensor] = None,
    decoder_attention_mask: Optional[torch.LongTensor] = None, head_mask: Optional[torch.Tensor] = None, decoder_head_mask: Optional[torch.Tensor] = None,
    cross_attn_head_mask: Optional[torch.Tensor] = None, encoder_outputs: Optional[List[torch.FloatTensor]] = None, start_positions: Optional[torch.LongTensor] = None,
    ...
    return_dict: Optional[bool] = None) -> Union[Tuple, Seq2SeqQuestionAnsweringModelOutput]:
        return_dict = return_dict if return_dict is not None else self.config.use_return_dict
        ...
        inputs_embeds=inputs_embeds, decoder_inputs_embeds=decoder_inputs_embeds, use_cache=use_cache, output_attentions=output_attentions,
        ...
        return Seq2SeqQuestionAnsweringModelOutput(loss=total_loss, start_logits=start_logits, end_logits=end_logits, past_key_values=outputs.past_key_values,
