...
class TFConvNextV2Layer(keras.layers.Layer):
    ...
    def build(self, input_shape=None):
        ...
        if getattr(self, "grn", None) is not None:
            with tf.name_scope(self.grn.name): self.grn.build(None)
...
class TFConvNextV2Model(TFConvNextV2PreTrainedModel):
    ...
    def build(self, input_shape=None):
        ...
        if getattr(self, "convnextv2", None) is not None:
            with tf.name_scope(self.convnextv2.name): self.convnextv2.build(None)
@add_start_docstrings("ConvNextV2 Model with an image classification head on top (a linear layer on top of the pooled features), e.g. for ImageNet.", CONVNEXTV2_START_DOCSTRING)
