...
import torch
_CONFIG_FOR_DOC = "SapamaConfig"
class SapamaMLP(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config, self.hidden_size, self.intermediate_size = config, config.hidden_size, config.intermediate_size
        self.gate_proj, self.up_proj = nn.Linear(self.hidden_size, self.intermediate_size, bias=config.mlp_bias), nn.Linear(self.hidden_size, self.intermediate_size, bias=config.mlp_bias)
        self.down_proj, self.act_fn = nn.Linear(self.intermediate_size, self.hidden_size, bias=config.mlp_bias), ACT2FN[config.hidden_act]
    def forward(self, x):
        if self.config.pretraining_tp > 1:
            ...
            gate_proj, up_proj = torch.cat([Functional.linear(x, gate_proj_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1), torch.cat([Functional.linear(x, up_proj_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1)
        ...
        return down_proj
class SapamaRotaryEmbedding(nn.Module):
    def __init__(self, dim=None, max_position_embeddings=2048, base=10000, device=None, scaling_factor=1.0, rope_type="default", config: Optional[SapamaConfig] = None):
        super().__init__()
        self.rope_kwargs = {}
        if config is None:
...
class SapamaAttention(nn.Module):
    def __init__(self, config: SapamaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        ...
        self.q_proj, self.k_proj = nn.Linear(self.hidden_size, self.num_heads * self.head_dim, bias=config.attention_bias), nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias)
        self.v_proj, self.o_proj = nn.Linear(self.hidden_size, self.num_key_value_heads * self.head_dim, bias=config.attention_bias), nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=config.attention_bias)
        self.rotary_emb = SapamaRotaryEmbedding(config=self.config)
    ...
    output_attentions: bool = False, use_cache: bool = False, cache_position: Optional[torch.LongTensor] = None, position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
    **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        ...
        if self.config.pretraining_tp > 1:
            ...
            query_states = torch.cat([Functional.linear(hidden_states, query_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1)
            key_states = torch.cat([Functional.linear(hidden_states, key_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1)
            value_states = torch.cat([Functional.linear(hidden_states, value_slices[i]) for i in range(self.config.pretraining_tp)], dim=-1)
        ...
        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3)) / sqrt(self.head_dim)
        if attention_mask is not None: attn_weights = attn_weights + attention_mask[:, :, :, : key_states.shape[-2]]
        attn_output = torch.matmul(nn.functional.dropout(nn.functional.softmax(attn_weights, dim=-1, dtype=torch.float32).to(query_states.dtype), p=self.attention_dropout, training=self.training), value_states)
        ...
        if self.config.pretraining_tp > 1:
            attn_output, o_proj_slices = attn_output.split(self.hidden_size // self.config.pretraining_tp, dim=2), self.o_proj.weight.split(self.hidden_size // self.config.pretraining_tp, dim=1)
            attn_output = sum([Functional.linear(attn_output[i], o_proj_slices[i]) for i in range(self.config.pretraining_tp)])
        else: attn_output = self.o_proj(attn_output)
        if not output_attentions: attn_weights = None
        return attn_output, attn_weights, past_key_value
class SapamaFlashAttention2(SapamaAttention):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._flash_attn_uses_top_left_mask = not is_flash_attn_greater_or_equal_2_10()
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.LongTensor] = None, position_ids: Optional[torch.LongTensor] = None,
    past_key_value: Optional[Cache] = None, output_attentions: bool = False, use_cache: bool = False, cache_position: Optional[torch.LongTensor] = None,
...
class SapamaSdpaAttention(SapamaAttention):
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.LongTensor] = None, past_key_value: Optional[Cache] = None,
    output_attentions: bool = False, use_cache: bool = False, cache_position: Optional[torch.LongTensor] = None, position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
    **kwargs) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[Tuple[torch.Tensor]]]:
        if output_attentions: return super().forward(hidden_states=hidden_states, attention_mask=attention_mask, position_ids=position_ids, past_key_value=past_key_value, output_attentions=output_attentions,
        ...
        return self.o_proj(attn_output.transpose(1, 2).contiguous().view(bsz, q_len, -1)), None, past_key_value
class SapamaRMSNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        super().__init__()
        self.weight, self.variance_epsilon = nn.Parameter(torch.ones(hidden_size)), eps
    def forward(self, hidden_states):
        input_dtype = hidden_states.dtype
        hidden_states = hidden_states.to(torch.float32)
        return self.weight * (hidden_states * torch.rsqrt(hidden_states.pow(2).mean(-1, keepdim=True) + self.variance_epsilon)).to(input_dtype)
    def extra_repr(self): return f"{tuple(self.weight.shape)}, eps={self.variance_epsilon}"
ALL_LAYERNORM_LAYERS.append(SapamaRMSNorm)
class SapamaDecoderLayer(nn.Module):
    def __init__(self, config: SapamaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = {"eager": SapamaAttention, "flash_attention_2": SapamaFlashAttention2, "sdpa": SapamaSdpaAttention}[config._attn_implementation](config=config, layer_idx=layer_idx)
        self.mlp, self.input_layernorm, self.post_attention_layernorm = SapamaMLP(config), SapamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps), SapamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps)
    def forward(self, hidden_states: torch.Tensor, attention_mask: Optional[torch.Tensor] = None, position_ids: Optional[torch.LongTensor] = None, past_key_value: Optional[Cache] = None,
    output_attentions: Optional[bool] = False, use_cache: Optional[bool] = False, cache_position: Optional[torch.LongTensor] = None, position_embeddings: Optional[Tuple[torch.Tensor, torch.Tensor]] = None,
...
"""
@add_start_docstrings("The bare Sapama Model outputting raw hidden-states without any specific head on top.", SAPAMA_START_DOCSTRING)
class SapamaPreTrainedModel(PreTrainedModel):
    config_class, base_model_prefix, supports_gradient_checkpointing = SapamaConfig, "model", True
    _no_split_modules, _skip_keys_device_placement = ["SapamaDecoderLayer"], ["past_key_values"]
...
"""
@add_start_docstrings("The bare Sapama Model outputting raw hidden-states without any specific head on top.", SAPAMA_START_DOCSTRING)
class SapamaModel(SapamaPreTrainedModel):
    def __init__(self, config: SapamaConfig):
        super().__init__(config)
        self.padding_idx, self.vocab_size = config.pad_token_id, config.vocab_size
        self.embed_tokens, self.layers = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx), nn.ModuleList([SapamaDecoderLayer(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
        self.norm, self.rotary_emb, self.gradient_checkpointing = SapamaRMSNorm(config.hidden_size, eps=config.rms_norm_eps), SapamaRotaryEmbedding(config=config), False
        self.post_init()
    ...
    cache_position: Optional[torch.LongTensor] = None) -> Union[Tuple, BaseModelOutputWithPast]:
        ...
        position_embeddings, next_decoder_cache = self.rotary_emb(hidden_states, position_ids), None
        all_hidden_states, all_self_attns = () if output_hidden_states else None, () if output_attentions else None
    ...
    def _update_causal_mask(self, attention_mask: torch.Tensor, input_tensor: torch.Tensor, cache_position: torch.Tensor, past_key_values: Cache, output_attentions: bool):
        ...
        min_dtype, sequence_length = torch.finfo(dtype).min, input_tensor.shape[1]
        ...
        return causal_mask
class SapamaForCausalLM(SapamaPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["lm_head.weight"]
    def __init__(self, config):
        super().__init__(config)
        self.model, self.vocab_size = SapamaModel(config), config.vocab_size
    ...
    cache_position: Optional[torch.LongTensor] = None, num_logits_to_keep: int = 0) -> Union[Tuple, CausalLMOutputWithPast]:
        output_attentions, output_hidden_states = output_attentions if output_attentions is not None else self.config.output_attentions, (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        return_dict, loss = return_dict if return_dict is not None else self.config.use_return_dict, None
        ...
        if self.config.pretraining_tp > 1: logits = torch.cat([Functional.linear(hidden_states, self.lm_head.weight.split(self.vocab_size // self.config.pretraining_tp, dim=0)[i]) for i in range(self.config.pretraining_tp)], dim=-1)
    ...
    use_cache=True, num_logits_to_keep=None, **kwargs):
        ...
        if attention_mask is not None and position_ids is None:
            ...
            if past_key_values: position_ids = position_ids[:, -input_ids.shape[1] :].clone(memory_format=torch.contiguous_format)
        ...
        if isinstance(past_key_values, StaticCache) and attention_mask.ndim == 2:
            ...
            dtype=dtype, device=device, min_dtype=torch.finfo(dtype).min, cache_position=cache_position, batch_size=batch_size)
...
@add_start_docstrings("""
    The Sapama Model transformer with a sequence classification head on top (linear layer).
    [`SapamaForSequenceClassification`] uses the last token in order to do the classification, as other causal models
    (e.g. GPT-2) do.
    ...
    """, SAPAMA_START_DOCSTRING)
class SapamaForSequenceClassification(SapamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels, self.model, self.score = config.num_labels, SapamaModel(config), nn.Linear(config.hidden_size, self.num_labels, bias=False)
        self.post_init()
    def get_input_embeddings(self): return self.model.embed_tokens
    def set_input_embeddings(self, value): self.model.embed_tokens = value
    @add_start_docstrings_to_model_forward(SAPAMA_INPUTS_DOCSTRING)
    ...
    use_cache: Optional[bool] = None, output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Union[Tuple, SequenceClassifierOutputWithPast]:
        ...
        if self.config.pad_token_id is None: sequence_lengths = -1
            ...
            if input_ids is not None: sequence_lengths = ((torch.eq(input_ids, self.config.pad_token_id).int().argmax(-1) - 1) % input_ids.shape[-1]).to(logits.device)
            else: sequence_lengths = -1
        pooled_logits, loss = logits[torch.arange(batch_size, device=logits.device), sequence_lengths], None
        if labels is not None:
...
@add_start_docstrings("""
The Sapama Model transformer with a span classification head on top for extractive question-answering tasks like
SQuAD (a linear layer on top of the hidden-states output to compute `span start logits` and `span end logits`).
""", SAPAMA_START_DOCSTRING)
class SapamaForQuestionAnswering(SapamaPreTrainedModel):
    base_model_prefix = "transformer"
    def __init__(self, config):
        super().__init__(config)
        self.transformer, self.qa_outputs = SapamaModel(config), nn.Linear(config.hidden_size, 2)
        self.post_init()
    ...
    past_key_values: Optional[Union[Cache, List[torch.FloatTensor]]] = None, inputs_embeds: Optional[torch.FloatTensor] = None, start_positions: Optional[torch.LongTensor] = None,
...
@add_start_docstrings("The Sapama Model transformer with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks.", SAPAMA_START_DOCSTRING)
class SapamaForTokenClassification(SapamaPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels, self.model = config.num_labels, SapamaModel(config)
        ...
        self.dropout, self.score = nn.Dropout(classifier_dropout), nn.Linear(config.hidden_size, config.num_labels)
        self.post_init()
    def get_input_embeddings(self): return self.model.embed_tokens
    def set_input_embeddings(self, value): self.model.embed_tokens = value
    @add_start_docstrings_to_model_forward(SAPAMA_INPUTS_DOCSTRING)
...
class SapamaDynamicNTKScalingRotaryEmbedding(SapamaRotaryEmbedding):
    def __init__(self, *args, **kwargs):
        kwargs["rope_type"] = "dynamic"
        super().__init__(*args, **kwargs)
class SapamaLinearScalingRotaryEmbedding(SapamaRotaryEmbedding):
    def __init__(self, *args, **kwargs):
