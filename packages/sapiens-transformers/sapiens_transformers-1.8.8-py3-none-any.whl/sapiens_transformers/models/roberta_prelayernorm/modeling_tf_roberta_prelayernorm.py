...
class TFRobertaPreLayerNormEmbeddings(keras.layers.Layer):
    ...
    def call(self, input_ids=None, position_ids=None, token_type_ids=None, inputs_embeds=None, past_key_values_length=0, training=False):
        ...
        if position_ids is None:
            ...
            else: position_ids = tf.expand_dims(tf.range(start=self.padding_idx + 1, limit=input_shape[-1] + self.padding_idx + 1), axis=0)
...
class TFRobertaPreLayerNormAttention(keras.layers.Layer):
    ...
    past_key_value: Tuple[tf.Tensor], output_attentions: bool, training: bool = False) -> Tuple[tf.Tensor]:
        ...
        self_outputs = self.self_attention(hidden_states=hidden_states_pre_layer_norm, attention_mask=attention_mask, head_mask=head_mask, encoder_hidden_states=encoder_hidden_states,
...
class TFRobertaPreLayerNormModel(TFRobertaPreLayerNormPreTrainedModel):
    ...
    encoder_hidden_states: np.ndarray | tf.Tensor | None = None, encoder_attention_mask: np.ndarray | tf.Tensor | None = None, past_key_values: Optional[Tuple[Tuple[Union[np.ndarray, tf.Tensor]]]] = None,
    use_cache: Optional[bool] = None, output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None,
    training: Optional[bool] = False) -> Union[Tuple, TFBaseModelOutputWithPoolingAndCrossAttentions]:
        outputs = self.roberta_prelayernorm(input_ids=input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids,
        ...
        past_key_values=past_key_values, use_cache=use_cache, output_attentions=output_attentions, output_hidden_states=output_hidden_states,
    ...
    def build(self, input_shape=None):
        ...
        if getattr(self, "roberta_prelayernorm", None) is not None:
            with tf.name_scope(self.roberta_prelayernorm.name): self.roberta_prelayernorm.build(None)
...
class TFRobertaPreLayerNormForMaskedLM(TFRobertaPreLayerNormPreTrainedModel, TFMaskedLanguageModelingLoss):
    ...
    training: Optional[bool] = False) -> Union[TFMaskedLMOutput, Tuple[tf.Tensor]]:
        outputs = self.roberta_prelayernorm(input_ids, attention_mask=attention_mask, token_type_ids=token_type_ids, position_ids=position_ids, head_mask=head_mask,
...
class TFRobertaPreLayerNormForCausalLM(TFRobertaPreLayerNormPreTrainedModel, TFCausalLanguageModelingLoss):
    ...
    def __init__(self, config: RobertaPreLayerNormConfig, *inputs, **kwargs):
        super().__init__(config, *inputs, **kwargs)
        if not config.is_decoder: logger.warning("If you want to use `TFRobertaPreLayerNormLMHeadModel` as a standalone, add `is_decoder=True.`")
        self.roberta_prelayernorm = TFRobertaPreLayerNormMainLayer(config, add_pooling_layer=False, name="roberta_prelayernorm")
    ...
    labels: np.ndarray | tf.Tensor | None = None, training: Optional[bool] = False) -> Union[TFCausalLMOutputWithCrossAttentions, Tuple[tf.Tensor]]:
        ...
        if not return_dict:
            ...
            return ((loss,) + output) if loss is not None else output
        return TFCausalLMOutputWithCrossAttentions(loss=loss, logits=logits, past_key_values=outputs.past_key_values, hidden_states=outputs.hidden_states,
...
@add_start_docstrings("RoBERTa-PreLayerNorm Model transformer with a sequence classification/regression head on top (a linear layer on top of the pooled output) e.g. for GLUE tasks.", ROBERTA_PRELAYERNORM_START_DOCSTRING)
...
class TFRobertaPreLayerNormForMultipleChoice(TFRobertaPreLayerNormPreTrainedModel, TFMultipleChoiceLoss):
    ...
    training: Optional[bool] = False) -> Union[TFMultipleChoiceModelOutput, Tuple[tf.Tensor]]:
        ...
        outputs = self.roberta_prelayernorm(flat_input_ids, flat_attention_mask, flat_token_type_ids, flat_position_ids, head_mask, inputs_embeds,
        output_attentions, output_hidden_states, return_dict=return_dict, training=training)
...
@add_start_docstrings("RoBERTa-PreLayerNorm Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks.", ROBERTA_PRELAYERNORM_START_DOCSTRING)
class TFRobertaPreLayerNormForTokenClassification(TFRobertaPreLayerNormPreTrainedModel, TFTokenClassificationLoss):
    ...
    def build(self, input_shape=None):
        ...
        if getattr(self, "classifier", None) is not None:
            with tf.name_scope(self.classifier.name): self.classifier.build([None, None, self.config.hidden_size])
@add_start_docstrings("RoBERTa-PreLayerNorm Model with a span classification head on top for extractive question-answering tasks like SQuAD (a linear layers on top of the hidden-states output to compute `span start logits` and `span end logits`).", ROBERTA_PRELAYERNORM_START_DOCSTRING)
