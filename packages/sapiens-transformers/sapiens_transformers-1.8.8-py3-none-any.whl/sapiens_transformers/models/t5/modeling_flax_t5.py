...
class FlaxT5LayerFF(nn.Module):
    ...
    dtype: jnp.dtype = jnp.float32
    def setup(self):
        if self.config.is_gated_act: self.DenseReluDense = FlaxT5DenseGatedActDense(self.config, dtype=self.dtype)
        else: self.DenseReluDense = FlaxT5DenseActDense(self.config, dtype=self.dtype)
...
class FlaxT5LayerCollection(nn.Module):
    ...
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.layer = FlaxT5Block(self.config, has_relative_attention_bias=self.has_relative_attention_bias, dtype=self.dtype)
    def __call__(self, hidden_states, attention_mask=None, position_bias=None, encoder_hidden_states=None, encoder_attention_mask=None, encoder_decoder_position_bias=None,
...
class FlaxT5BlockCollection(nn.Module):
    ...
    def setup(self):
        ...
        else: self.blocks = [FlaxT5LayerCollection(self.config, has_relative_attention_bias=(i == 0), dtype=self.dtype, name=str(i)) for i in range(self.config.num_layers)]
...
class FlaxT5PreTrainedModel(FlaxPreTrainedModel):
    ...
    output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None, train: bool = False, params: dict = None, dropout_rng: PRNGKey = None):
...
class FlaxT5Model(FlaxT5PreTrainedModel): module_class = FlaxT5Module
...
FLAX_T5_MODEL_DOCSTRING = """
    ...
    Example:
    ```python
    >>> from sapiens_transformers import AutoTokenizer, FlaxT5Model
...
class FlaxT5EncoderModel(FlaxT5PreTrainedModel):
    ...
    def __call__(self, input_ids: jnp.ndarray, attention_mask: Optional[jnp.ndarray] = None, output_attentions: Optional[bool] = None, output_hidden_states: Optional[bool] = None,
    return_dict: Optional[bool] = None, train: bool = False, params: dict = None, dropout_rng: PRNGKey = None):
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        return_dict = return_dict if return_dict is not None else self.config.return_dict
...
FLAX_T5_CONDITIONAL_GENERATION_DOCSTRING = """
    ...
    Example:
    ```python
    >>> from sapiens_transformers import AutoTokenizer, FlaxT5ForConditionalGeneration
    >>> tokenizer = AutoTokenizer.from_pretrained("google-t5/t5-small")
