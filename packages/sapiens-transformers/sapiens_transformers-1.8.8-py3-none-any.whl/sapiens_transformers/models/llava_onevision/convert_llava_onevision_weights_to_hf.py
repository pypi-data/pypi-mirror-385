...
from sapiens_transformers import (AddedToken, AutoConfig, AutoTokenizer, LlavaOnevisionConfig, LlavaOnevisionForConditionalGeneration, LlavaOnevisionImageProcessor,
LlavaOnevisionProcessor, LlavaOnevisionVideoProcessor, SiglipVisionConfig)
...
def load_original_state_dict(model_id):
    ...
    if "lm_head.weight" not in original_state_dict: original_state_dict["lm_head.weight"] = original_state_dict["model.embed_tokens.weight"].clone()
...
def convert_llava_to_hf(model_id, pytorch_dump_folder_path, push_to_hub=False):
    ...
    with open(filepath) as f:
        data = json.load(f)
        print(data)
    if model_id in ["lmms-lab/llava-onevision-qwen2-0.5b-ov", "lmms-lab/llava-onevision-qwen2-0.5b-si"]: text_model_id = "Qwen/Qwen2-0.5B-Instruct"
    elif model_id in ["lmms-lab/llava-onevision-qwen2-7b-ov", "lmms-lab/llava-onevision-qwen2-7b-si", "lmms-lab/llava-onevision-qwen2-7b-ov-chat"]: text_model_id = "Qwen/Qwen2-7B-Instruct"
    elif model_id in ["lmms-lab/llava-onevision-qwen2-72b-ov", "lmms-lab/llava-onevision-qwen2-72b-si", "lmms-lab/llava-onevision-qwen2-72b-ov-chat"]: text_model_id = "Qwen/Qwen2-72B-Instruct"
    ...
    with init_empty_weights(): model = LlavaOnevisionForConditionalGeneration(config)
    ...
    with torch.inference_mode():
        ...
        if model_id == "lmms-lab/llava-onevision-qwen2-0.5b-si": expected_slice = torch.tensor([[-12.1953, -14.6797, -12.7891], [0.5840, -0.8467, 1.3799], [3.6055, 4.5430, 9.9062]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-0.5b-ov": expected_slice = torch.tensor([[-12.0234, -14.3828, -12.7500], [2.3594, 1.0000, 3.9336], [3.6582, 4.7148, 9.1172]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-7b-si": expected_slice = torch.tensor([[1.7656, 3.3418, 1.4033], [0.0757, 0.7427, 3.5098], [6.7109, 5.6797, 9.3828]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-7b-ov": expected_slice = torch.tensor([[1.8496, 3.4219, 1.3135], [3.0996, 3.0117, 3.1484], [4.2422, 4.7109, 9.9688]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-72b-si": expected_slice = torch.tensor([[4.1875, 4.4883, 2.7910], [1.2949, 5.1328, 3.1582], [0.9390, 6.4531, 8.4375]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-72b-ov": expected_slice = torch.tensor([[4.2930, 4.7305, 2.7363], [1.7529, 5.0742, 3.9590], [1.3936, 6.3438, 9.3984]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-7b-ov-chat": expected_slice = torch.tensor([[1.8662, 3.4316, 1.3174], [2.7109, 2.5488, 3.0117], [4.4648, 4.9648, 10.3359]], dtype=torch.float32, device=device)
        elif model_id == "lmms-lab/llava-onevision-qwen2-72b-ov-chat": expected_slice = torch.tensor([[4.3086, 4.7344, 2.6953], [1.7090, 5.1719, 4.0234], [1.3057, 6.3438, 9.5469]], dtype=torch.float32, device=device)
        else: raise ValueError(f"Model {model_id} not supported")
        assert torch.allclose(outputs.logits[0, :3, :3], expected_slice, atol=1e-4)
        print("Logits are ok!")
    output_ids = model.generate(**inputs, max_new_tokens=100, use_cache=True)
    generated_text = processor.batch_decode(output_ids, skip_special_tokens=True)[0].strip()
    print("Generated text:", repr(generated_text))
    if model_id == "lmms-lab/llava-onevision-qwen2-0.5b-si": expected_text = "system\nYou are a helpful assistant.\nuser\n\nWhat is shown in this image?\nassistant\nThe image is a radar chart that shows the performance of different algorithms or models in a specific domain, such as image classification or natural language processing. The chart is color-coded to represent different algorithms, with each color corresponding to a specific algorithm. The algorithms are labeled as BLIP-2, InstructBLIP, Owen-VL-Chat, and LLaVA-1.5. The chart also includes a legend at the bottom that explains the color coding and the algorithms represented."
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_id", help="Hub location of the model to convert", default="lmms-lab/llava-onevision-qwen2-0.5b-ov", choices=["lmms-lab/llava-onevision-qwen2-0.5b-ov",
    "lmms-lab/llava-onevision-qwen2-0.5b-si", "lmms-lab/llava-onevision-qwen2-7b-si", "lmms-lab/llava-onevision-qwen2-7b-ov", "lmms-lab/llava-onevision-qwen2-72b-si",
    "lmms-lab/llava-onevision-qwen2-72b-ov", "lmms-lab/llava-onevision-qwen2-7b-ov-chat", "lmms-lab/llava-onevision-qwen2-72b-ov-chat"], required=False)
