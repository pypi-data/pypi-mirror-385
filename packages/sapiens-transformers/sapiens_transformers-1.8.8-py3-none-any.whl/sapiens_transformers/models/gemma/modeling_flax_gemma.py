...
class FlaxGemmaAttention(nn.Module):
    ...
    def _merge_heads(self, hidden_states): return hidden_states.reshape(hidden_states.shape[:2] + (self.num_heads * self.head_dim,))
    @nn.compact
...
class FlaxGemmaMLP(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.hidden_activation is None:
            logger.warning_once(f"Gemma's activation function should be approximate GeLU and not exact GeLU. Changing the activation function to `gelu_pytorch_tanh`. if you want to use the legacy `{self.config.hidden_act}`, edit the `model.config` to set `hidden_activation={self.config.hidden_act}` instead of `hidden_act`.")
...
class FlaxGemmaLayerCollection(nn.Module):
    config: GemmaConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.blocks = [FlaxGemmaDecoderLayer(self.config, dtype=self.dtype, name=str(i)) for i in range(self.config.num_hidden_layers)]
    def __call__(self, hidden_states, attention_mask=None, position_ids=None, deterministic: bool = True, init_cache: bool = False, output_attentions: bool = False,
...
class FlaxGemmaModel(FlaxGemmaPreTrainedModel): module_class = FlaxGemmaModule
...
@add_start_docstrings("The Gemma Model transformer with a language modeling head (linear layer) on top.", GEMMA_START_DOCSTRING)
