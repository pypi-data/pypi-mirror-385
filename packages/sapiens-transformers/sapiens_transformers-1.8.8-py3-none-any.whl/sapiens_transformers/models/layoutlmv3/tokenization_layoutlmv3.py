...
class LayoutLMv3Tokenizer(PreTrainedTokenizer):
    ...
    def __init__(self, vocab_file, merges_file, errors="replace", bos_token="<s>", eos_token="</s>", sep_token="</s>", cls_token="<s>", unk_token="<unk>", pad_token="<pad>",
    mask_token="<mask>", add_prefix_space=True, cls_token_box=[0, 0, 0, 0], sep_token_box=[0, 0, 0, 0], pad_token_box=[0, 0, 0, 0], pad_token_label=-100, only_label_first_subword=True, **kwargs):
        ...
        mask_token=mask_token, add_prefix_space=add_prefix_space, cls_token_box=cls_token_box, sep_token_box=sep_token_box, pad_token_box=pad_token_box, pad_token_label=pad_token_label,
        only_label_first_subword=only_label_first_subword, **kwargs)
    @property
    ...
    def prepare_for_tokenization(self, text, is_split_into_words=False, **kwargs):
        ...
        if ((is_split_into_words or add_prefix_space) and (len(text) > 0 and not text[0].isspace()) and sum([text.startswith(no_split_token) for no_split_token in self.added_tokens_encoder]) == 0): text = " " + text
        return (text, kwargs)
    ...
    return_special_tokens_mask: bool = False, return_offsets_mapping: bool = False, return_length: bool = False,
    ...
    return_length: bool = False, verbose: bool = True) -> BatchEncoding:
        ...
        for idx, example in enumerate(zip(batch_text_or_text_pairs, boxes)):
            ...
            word_labels=word_labels[idx] if word_labels is not None else None, add_special_tokens=add_special_tokens, padding=PaddingStrategy.DO_NOT_PAD.value,
    ...
    return_length: bool = False, verbose: bool = True, prepend_batch_axis: bool = False, **kwargs) -> BatchEncoding:
        ...
        overflowing_labels = []
        if truncation_strategy != TruncationStrategy.DO_NOT_TRUNCATE and max_length and total_len > max_length: (ids, token_boxes, pair_ids, pair_token_boxes, labels, overflowing_tokens,
        overflowing_token_boxes, overflowing_labels) = self.truncate_sequences(ids, token_boxes, pair_ids=pair_ids, pair_token_boxes=pair_token_boxes, labels=labels, num_tokens_to_remove=total_len - max_length, truncation_strategy=truncation_strategy, stride=stride)
        ...
        if add_special_tokens:
            ...
            if pair_token_boxes: pair_token_boxes = [self.sep_token_box] + pair_token_boxes + [self.sep_token_box]
