"""
    ...
    #################################################################################################################################################################################################
"""
from sapiens_transformers import LlamaTokenizer, SastralConfig, SastralForCausalLM
...
except ImportError as e: tokenizer_class = LlamaTokenizer
NUM_SHARDS = {"7B": 1}
...
def write_model(model_path, input_base_path, model_size, tokenizer_path=None, safe_serialization=True, is_v3=False):
    ...
    def write_json(text, path):
        with open(path, "w") as file: json.dump(text, file)
    write_json(index_dict, os.path.join(tmp_model_path, "pytorch_model.bin.index.json"))
    config = SastralConfig(hidden_size=dim, intermediate_size=params["hidden_dim"], num_attention_heads=params["n_heads"], num_hidden_layers=params["n_layers"],
    ...
    del loaded
    gc.collect()
    print("Loading the checkpoint in a Sastral model.")
    model = SastralForCausalLM.from_pretrained(tmp_model_path, torch_dtype=torch.bfloat16, low_cpu_mem_usage=True)
...
def main():
    ...
    parser.add_argument("--input_dir", help="Location of Sastral weights, which contains tokenizer.model and model folders")
    parser.add_argument("--model_size", choices=["7B", "tokenizer_only"], help="")
