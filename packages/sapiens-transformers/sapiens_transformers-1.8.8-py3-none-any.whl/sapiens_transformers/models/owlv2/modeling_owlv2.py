...
class Owlv2Model(Owlv2PreTrainedModel):
    ...
    def __init__(self, config: Owlv2Config):
        super().__init__(config)
        if not isinstance(config.text_config, Owlv2TextConfig): raise TypeError(f"config.text_config is expected to be of type Owlv2TextConfig but is of type {type(config.text_config)}.")
        if not isinstance(config.vision_config, Owlv2VisionConfig): raise TypeError(f"config.vision_config is expected to be of type Owlv2VisionConfig but is of type {type(config.vision_config)}.")
    ...
    return_dict: Optional[bool] = None) -> Union[Tuple, Owlv2Output]:
        ...
        loss = None
        if return_loss: loss = owlv2_loss(logits_per_text)
        text_embeds = text_embeds_norm
        if not return_dict:
            output = (logits_per_image, logits_per_text, text_embeds, image_embeds, text_outputs, vision_outputs)
            return ((loss,) + output) if loss is not None else output
        return Owlv2Output(loss=loss, logits_per_image=logits_per_image, logits_per_text=logits_per_text, text_embeds=text_embeds, image_embeds=image_embeds,
...
class Owlv2ForObjectDetection(Owlv2PreTrainedModel):
    ...
    output_hidden_states: Optional[bool] = None) -> Tuple[torch.FloatTensor]:
        outputs = self.owlv2(pixel_values=pixel_values, input_ids=input_ids, attention_mask=attention_mask, output_attentions=output_attentions,
    ...
    output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Owlv2ImageGuidedObjectDetectionOutput:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        ...
        if not return_dict:
            ...
            return output
        return Owlv2ImageGuidedObjectDetectionOutput(image_embeds=feature_map, query_image_embeds=query_feature_map, target_pred_boxes=target_pred_boxes,
    ...
    output_hidden_states: Optional[bool] = None, return_dict: Optional[bool] = None) -> Owlv2ObjectDetectionOutput:
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states)
        return_dict = return_dict if return_dict is not None else self.config.return_dict
        ...
        if not return_dict:
            ...
            output = tuple(x for x in output if x is not None)
            return output
        return Owlv2ObjectDetectionOutput(image_embeds=feature_map, text_embeds=query_embeds, pred_boxes=pred_boxes, logits=pred_logits, objectness_logits=objectness_logits,
