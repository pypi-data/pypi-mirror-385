    ...
    callback_on_step_end: Optional[Callable[[int, int, Dict], None]]=None, callback_on_step_end_tensor_inputs: List[str]=['latents'], max_sequence_length: int=512):
        ...
        if isinstance(self.controlnet, FluxControlNetModel):
            ...
            if control_mode is not None:
                if not isinstance(control_mode, int): raise ValueError(' For `FluxControlNet`, `control_mode` should be an `int` or `None`')
