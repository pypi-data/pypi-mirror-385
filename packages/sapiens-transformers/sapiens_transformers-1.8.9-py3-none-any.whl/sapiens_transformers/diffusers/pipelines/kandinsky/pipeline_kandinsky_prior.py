...
class KandinskyPriorPipeline(DiffusionPipeline):
    ...
    negative_prompt: str='', guidance_scale: float=4.0, device=None):
        ...
        for cond, weight in zip(images_and_prompts, weights):
            ...
            negative_prompt=negative_prior_prompt, guidance_scale=guidance_scale).image_embeds
        ...
        out_zero = self(negative_prompt, num_inference_steps=num_inference_steps, num_images_per_prompt=num_images_per_prompt, generator=generator,
        latents=latents, negative_prompt=negative_prior_prompt, guidance_scale=guidance_scale)
    ...
    generator: Optional[Union[torch.Generator, List[torch.Generator]]]=None, latents: Optional[torch.Tensor]=None, guidance_scale: float=4.0, output_type: Optional[str]='pt', return_dict: bool=True):
        """Examples:"""
        ...
        for i, t in enumerate(self.progress_bar(prior_timesteps_tensor)):
            ...
            predicted_image_embedding = self.prior(latent_model_input, timestep=t, proj_embedding=prompt_embeds, encoder_hidden_states=text_encoder_hidden_states, attention_mask=text_mask).predicted_image_embedding
            if do_classifier_free_guidance:
        ...
        if negative_prompt is None:
            ...
            if hasattr(self, 'final_offload_hook') and self.final_offload_hook is not None: self.prior_hook.offload()
