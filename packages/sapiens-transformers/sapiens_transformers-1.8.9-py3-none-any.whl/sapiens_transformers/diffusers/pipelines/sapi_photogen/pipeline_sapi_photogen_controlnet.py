...
from ...models.controlnets.controlnet_sapi_photogen import SAPIPhotoGenControlNetModel, SAPIPhotoGenMultiControlNetModel
from ...loaders import SAPIPhotoGenLoraLoaderMixin, FromSingleFileMixin, TextualInversionLoaderMixin
...
class SAPIPhotoGenControlNetPipeline(DiffusionPipeline, SAPIPhotoGenLoraLoaderMixin, FromSingleFileMixin):
    """Args:"""
    ...
    tokenizer_2: T5TokenizerFast, transformer: SAPIPhotoGenTransformer2DModel, controlnet: Union[SAPIPhotoGenControlNetModel, List[SAPIPhotoGenControlNetModel], Tuple[SAPIPhotoGenControlNetModel], SAPIPhotoGenMultiControlNetModel]):
        super().__init__()
        if isinstance(controlnet, (list, tuple)): controlnet = SAPIPhotoGenMultiControlNetModel(controlnet)
        self.register_modules(vae=vae, text_encoder=text_encoder, text_encoder_2=text_encoder_2, tokenizer=tokenizer, tokenizer_2=tokenizer_2, transformer=transformer, scheduler=scheduler, controlnet=controlnet)
    ...
    def check_inputs(self, prompt, prompt_2, height, width, prompt_embeds=None, pooled_prompt_embeds=None, callback_on_step_end_tensor_inputs=None, max_sequence_length=None):
    ...
    num_inference_steps: int=28, sigmas: Optional[List[float]]=None, guidance_scale: float=7.0, control_guidance_start: Union[float, List[float]]=0.0,
    control_guidance_end: Union[float, List[float]]=1.0, control_image: PipelineImageInput=None, control_mode: Optional[Union[int, List[int]]]=None,
    controlnet_conditioning_scale: Union[float, List[float]]=1.0, num_images_per_prompt: Optional[int]=1, generator: Optional[Union[torch.Generator,
    List[torch.Generator]]]=None, latents: Optional[torch.FloatTensor]=None, prompt_embeds: Optional[torch.FloatTensor]=None,
    pooled_prompt_embeds: Optional[torch.FloatTensor]=None, output_type: Optional[str]='pil', return_dict: bool=True, joint_attention_kwargs: Optional[Dict[str, Any]]=None,
    callback_on_step_end: Optional[Callable[[int, int, Dict], None]]=None, callback_on_step_end_tensor_inputs: List[str]=['latents'], max_sequence_length: int=512):
        """Examples:"""
        ...
        if not isinstance(control_guidance_start, list) and isinstance(control_guidance_end, list): control_guidance_start = len(control_guidance_end) * [control_guidance_start]
            ...
            mult = len(self.controlnet.nets) if isinstance(self.controlnet, SAPIPhotoGenMultiControlNetModel) else 1
            control_guidance_start, control_guidance_end = (mult * [control_guidance_start], mult * [control_guidance_end])
        self.check_inputs(prompt, prompt_2, height, width, prompt_embeds=prompt_embeds, pooled_prompt_embeds=pooled_prompt_embeds,
        ...
        if isinstance(self.controlnet, SAPIPhotoGenControlNetModel):
            ...
            if control_mode is not None:
                if not isinstance(control_mode, int): raise ValueError(' For `SAPIPhotoGenControlNet`, `control_mode` should be an `int` or `None`')
                control_mode = torch.tensor(control_mode).to(device, dtype=torch.long)
                control_mode = control_mode.view(-1, 1).expand(control_image.shape[0], 1)
        elif isinstance(self.controlnet, SAPIPhotoGenMultiControlNetModel):
            control_images = []
            ...
            if isinstance(control_mode, list) and len(control_mode) != len(control_image): raise ValueError('For Multi-ControlNet, `control_mode` must be a list of the same ' + ' length as the number of controlnets (control images) specified')
            if not isinstance(control_mode, list): control_mode = [control_mode] * len(control_image)
            control_modes = []
        ...
        for i in range(len(timesteps)):
            ...
            controlnet_keep.append(keeps[0] if isinstance(self.controlnet, SAPIPhotoGenControlNetModel) else keeps)
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                if self.interrupt: continue
                timestep = t.expand(latents.shape[0]).to(latents.dtype)
                if isinstance(self.controlnet, SAPIPhotoGenMultiControlNetModel): use_guidance = self.controlnet.nets[0].config.guidance_embeds
                ...
                controlnet_block_samples=controlnet_block_samples, controlnet_single_block_samples=controlnet_single_block_samples, txt_ids=text_ids, img_ids=latent_image_ids,
                joint_attention_kwargs=self.joint_attention_kwargs, return_dict=False, controlnet_blocks_repeat=controlnet_blocks_repeat)[0]
