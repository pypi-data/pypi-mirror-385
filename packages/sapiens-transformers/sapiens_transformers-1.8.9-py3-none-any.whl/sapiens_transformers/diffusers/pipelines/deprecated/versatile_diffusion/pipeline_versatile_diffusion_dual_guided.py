...
class VersatileDiffusionDualGuidedPipeline(DiffusionPipeline):
    ...
    def __init__(self, tokenizer: CLIPTokenizer, image_feature_extractor: CLIPImageProcessor, text_encoder: CLIPTextModelWithProjection, image_encoder: CLIPVisionModelWithProjection,
    image_unet: UNet2DConditionModel, text_unet: UNetFlatConditionModel, vae: AutoencoderKL, scheduler: KarrasDiffusionSchedulers):
        super().__init__()
        ...
        if self.text_unet is not None and ('dual_cross_attention' not in self.image_unet.config or not self.image_unet.config.dual_cross_attention): self._convert_to_dual_attention()
    ...
    def _convert_to_dual_attention(self):
        for name, module in self.image_unet.named_modules():
            if isinstance(module, Transformer2DModel):
                ...
                dual_transformer = DualTransformer2DModel(num_attention_heads=config.num_attention_heads, attention_head_dim=config.attention_head_dim, in_channels=config.in_channels,
                num_layers=config.num_layers, dropout=config.dropout, norm_num_groups=config.norm_num_groups, cross_attention_dim=config.cross_attention_dim, attention_bias=config.attention_bias,
                sample_size=config.sample_size, num_vector_embeds=config.num_vector_embeds, activation_fn=config.activation_fn, num_embeds_ada_norm=config.num_embeds_ada_norm)
                dual_transformer.sapiens_transformers[0] = image_transformer
                dual_transformer.sapiens_transformers[1] = text_transformer
    ...
    def _revert_dual_attention(self):
        for name, module in self.image_unet.named_modules():
            if isinstance(module, DualTransformer2DModel):
                parent_name, index = name.rsplit('.', 1)
                index = int(index)
                self.image_unet.get_submodule(parent_name)[index] = module.sapiens_transformers[0]
    ...
    def check_inputs(self, prompt, image, height, width, callback_steps):
        if not isinstance(prompt, str) and (not isinstance(prompt, PIL.Image.Image)) and (not isinstance(prompt, list)): raise ValueError(f'`prompt` has to be of type `str` `PIL.Image` or `list` but is {type(prompt)}')
        if not isinstance(image, str) and (not isinstance(image, PIL.Image.Image)) and (not isinstance(image, list)): raise ValueError(f'`image` has to be of type `str` `PIL.Image` or `list` but is {type(image)}')
    ...
    @torch.no_grad()
    def __call__(self, prompt: Union[PIL.Image.Image, List[PIL.Image.Image]], image: Union[str, List[str]], text_to_image_strength: float=0.5, height: Optional[int]=None,
    ...
    callback: Optional[Callable[[int, int, torch.Tensor], None]]=None, callback_steps: int=1, **kwargs):
        ...
        latents = self.prepare_latents(batch_size * num_images_per_prompt, num_channels_latents, height, width, dual_prompt_embeddings.dtype, device, generator, latents)
