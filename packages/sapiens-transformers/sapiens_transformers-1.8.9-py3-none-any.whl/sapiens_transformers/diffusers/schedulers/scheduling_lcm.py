...
class LCMScheduler(SchedulerMixin, ConfigMixin):
    ...
    timesteps: Optional[List[int]]=None, strength: int=1.0):
        """Args:"""
    ...
    def get_scalings_for_boundary_condition_discrete(self, timestep):
        ...
        return (c_skip, c_out)
    def step(self, model_output: torch.Tensor, timestep: int, sample: torch.Tensor, generator: Optional[torch.Generator]=None, return_dict: bool=True) -> Union[LCMSchedulerOutput, Tuple]:
        """Returns:"""
        ...
        else: prev_timestep = timestep
        alpha_prod_t = self.alphas_cumprod[timestep]
        alpha_prod_t_prev = self.alphas_cumprod[prev_timestep] if prev_timestep >= 0 else self.final_alpha_cumprod
        beta_prod_t = 1 - alpha_prod_t
        beta_prod_t_prev = 1 - alpha_prod_t_prev
        c_skip, c_out = self.get_scalings_for_boundary_condition_discrete(timestep)
        if self.config.prediction_type == 'epsilon': predicted_original_sample = (sample - beta_prod_t.sqrt() * model_output) / alpha_prod_t.sqrt()
        elif self.config.prediction_type == 'sample': predicted_original_sample = model_output
        elif self.config.prediction_type == 'v_prediction': predicted_original_sample = alpha_prod_t.sqrt() * sample - beta_prod_t.sqrt() * model_output
        else: raise ValueError(f'prediction_type given as {self.config.prediction_type} must be one of `epsilon`, `sample` or `v_prediction` for `LCMScheduler`.')
        if self.config.thresholding: predicted_original_sample = self._threshold_sample(predicted_original_sample)
        elif self.config.clip_sample: predicted_original_sample = predicted_original_sample.clamp(-self.config.clip_sample_range, self.config.clip_sample_range)
        ...
        else: prev_sample = denoised
        self._step_index += 1
        if not return_dict: return (prev_sample, denoised)
