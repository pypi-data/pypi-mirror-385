...
from ..sapiens_transformers.transformer_sapi_photogen import SAPIPhotoGenSingleTransformerBlock, SAPIPhotoGenTransformerBlock
...
import torch
@dataclass
class SAPIPhotoGenControlNetOutput(BaseOutput):
    controlnet_block_samples: Tuple[torch.Tensor]
    controlnet_single_block_samples: Tuple[torch.Tensor]
class SAPIPhotoGenControlNetModel(ModelMixin, ConfigMixin, PeftAdapterMixin):
    ...
    pooled_projection_dim: int=768, guidance_embeds: bool=False, axes_dims_rope: List[int]=[16, 56, 56], num_mode: int=None, conditioning_embedding_channels: int=None):
        ...
        self.transformer_blocks = nn.ModuleList([SAPIPhotoGenTransformerBlock(dim=self.inner_dim, num_attention_heads=num_attention_heads, attention_head_dim=attention_head_dim) for i in range(num_layers)])
        self.single_transformer_blocks = nn.ModuleList([SAPIPhotoGenSingleTransformerBlock(dim=self.inner_dim, num_attention_heads=num_attention_heads, attention_head_dim=attention_head_dim) for i in range(num_single_layers)])
    ...
    return_dict: bool=True) -> Union[torch.FloatTensor, Transformer2DModelOutput]:
        ...
        return SAPIPhotoGenControlNetOutput(controlnet_block_samples=controlnet_block_samples, controlnet_single_block_samples=controlnet_single_block_samples)
class SAPIPhotoGenMultiControlNetModel(ModelMixin):
    """Args:"""
    ...
    joint_attention_kwargs: Optional[Dict[str, Any]]=None, return_dict: bool=True) -> Union[SAPIPhotoGenControlNetOutput, Tuple]:
