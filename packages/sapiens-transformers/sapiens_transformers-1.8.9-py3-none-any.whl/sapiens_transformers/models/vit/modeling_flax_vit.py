...
class FlaxViTPatchEmbeddings(nn.Module):
    ...
    def setup(self):
        ...
        self.projection = nn.Conv(self.config.hidden_size, kernel_size=(patch_size, patch_size), strides=(patch_size, patch_size), padding="VALID", dtype=self.dtype,
...
class FlaxViTSelfAttention(nn.Module):
    ...
    def __call__(self, hidden_states, deterministic: bool = True, output_attentions: bool = False):
        ...
        attn_weights = dot_product_attention_weights(query_states, key_states, dropout_rng=dropout_rng, dropout_rate=self.config.attention_probs_dropout_prob, broadcast_dropout=True,
...
class FlaxViTLayerCollection(nn.Module):
    config: ViTConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.layers = [FlaxViTLayer(self.config, name=str(i), dtype=self.dtype) for i in range(self.config.num_hidden_layers)]
...
class FlaxViTEncoder(nn.Module):
    config: ViTConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.layer = FlaxViTLayerCollection(self.config, dtype=self.dtype)
    def __call__(self, hidden_states, deterministic: bool = True, output_attentions: bool = False, output_hidden_states: bool = False, return_dict: bool = True): return self.layer(hidden_states,
    deterministic=deterministic, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict)
class FlaxViTPooler(nn.Module):
    config: ViTConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.dense = nn.Dense(self.config.hidden_size, kernel_init=jax.nn.initializers.variance_scaling(self.config.initializer_range**2, "fan_in", "truncated_normal"), dtype=self.dtype)
    def __call__(self, hidden_states):
...
class FlaxViTModel(FlaxViTPreTrainedModel): module_class = FlaxViTModule
FLAX_VISION_MODEL_DOCSTRING = """
    Returns:
    Examples:
    ```python
    >>> from sapiens_transformers import AutoImageProcessor, FlaxViTModel
...
@add_start_docstrings("ViT Model transformer with an image classification head on top (a linear layer on top of the final hidden state of the [CLS] token) e.g. for ImageNet.", VIT_START_DOCSTRING)
class FlaxViTForImageClassification(FlaxViTPreTrainedModel): module_class = FlaxViTForImageClassificationModule
FLAX_VISION_CLASSIF_DOCSTRING = """
    Returns:
    Example:
    ```python
    >>> from sapiens_transformers import AutoImageProcessor, FlaxViTForImageClassification
    >>> from PIL import Image
    >>> import jax
    >>> import requests
    >>> url = "http://images.cocodataset.org/val2017/000000039769.jpg"
