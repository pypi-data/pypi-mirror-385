...
class EncodecConv1d(nn.Module):
    def __init__(self, config, in_channels: int, out_channels: int, kernel_size: int, stride: int = 1, dilation: int = 1):
        ...
        if stride > 1 and dilation > 1: logger.warning(f"EncodecConv1d has been initialized with stride > 1 and dilation > 1 (kernel_size={kernel_size} stride={stride}, dilation={dilation}).")
        ...
        elif self.norm_type == "time_group_norm": self.norm = nn.GroupNorm(1, out_channels)
...
class EncodecConvTranspose1d(nn.Module):
    def __init__(self, config, in_channels: int, out_channels: int, kernel_size: int, stride: int = 1):
        ...
        elif config.norm_type == "time_group_norm": self.norm = nn.GroupNorm(1, out_channels)
    ...
    def forward(self, hidden_states):
        ...
        hidden_states = self.conv(hidden_states)
        if self.norm_type == "time_group_norm": hidden_states = self.norm(hidden_states)
        if self.causal: padding_right = math.ceil(padding_total * self.trim_right_ratio)
...
class EncodecResnetBlock(nn.Module):
    def __init__(self, config: EncodecConfig, dim: int, dilations: List[int]):
        ...
        if config.use_conv_shortcut: self.shortcut = EncodecConv1d(config, dim, dim, kernel_size=1)
...
class EncodecEncoder(nn.Module):
    def __init__(self, config: EncodecConfig):
        ...
        for ratio in reversed(config.upsampling_ratios):
            ...
            for j in range(config.num_residual_layers): model += [EncodecResnetBlock(config, current_scale, [config.dilation_growth_rate**j, 1])]
            model += [nn.ELU()]
...
class EncodecDecoder(nn.Module):
    def __init__(self, config: EncodecConfig):
        ...
        for ratio in config.upsampling_ratios:
            ...
            for j in range(config.num_residual_layers): model += [EncodecResnetBlock(config, current_scale // 2, (config.dilation_growth_rate**j, 1))]
            scaling //= 2
        model += [nn.ELU()]
...
class EncodecResidualVectorQuantizer(nn.Module):
    ...
    def get_num_quantizers_for_bandwidth(self, bandwidth: Optional[float] = None) -> int:
        ...
        if bandwidth is not None and bandwidth > 0.0: num_quantizers = int(max(1, math.floor(bandwidth * 1000 / bw_per_q)))
...
class EncodecModel(EncodecPreTrainedModel):
    ...
    def forward(self, input_values: torch.Tensor, padding_mask: Optional[torch.Tensor] = None, bandwidth: Optional[float] = None, audio_codes: Optional[torch.Tensor] = None,
    audio_scales: Optional[torch.Tensor] = None, return_dict: Optional[bool] = None) -> Union[Tuple[torch.Tensor, torch.Tensor], EncodecOutput]:
        return_dict = return_dict if return_dict is not None else self.config.return_dict
        if padding_mask is None: padding_mask = torch.ones_like(input_values).bool()
