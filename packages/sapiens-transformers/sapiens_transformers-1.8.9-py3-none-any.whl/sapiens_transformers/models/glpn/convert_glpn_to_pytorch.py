...
from sapiens_transformers import GLPNConfig, GLPNForDepthEstimation, GLPNImageProcessor
...
def rename_keys(state_dict):
    ...
    for key, value in state_dict.items():
        if key.startswith("module.encoder"): key = key.replace("module.encoder", "glpn.encoder")
        if key.startswith("module.decoder"): key = key.replace("module.decoder", "decoder.stages")
        ...
        if "bot_conv" in key: key = key.replace("bot_conv", "0.convolution")
        if "skip_conv1" in key: key = key.replace("skip_conv1", "1.convolution")
        if "skip_conv2" in key: key = key.replace("skip_conv2", "2.convolution")
        if "fusion1" in key: key = key.replace("fusion1", "1.fusion")
        if "fusion2" in key: key = key.replace("fusion2", "2.fusion")
        if "fusion3" in key: key = key.replace("fusion3", "3.fusion")
        if "fusion" in key and "conv" in key: key = key.replace("conv", "convolutional_layer")
        if key.startswith("module.last_layer_depth"): key = key.replace("module.last_layer_depth", "head.head")
...
def convert_glpn_checkpoint(checkpoint_path, pytorch_dump_folder_path, push_to_hub=False, model_name=None):
    ...
    if model_name is not None:
        if "nyu" in model_name: expected_slice = torch.tensor([[4.4147, 4.0873, 4.0673], [3.7890, 3.2881, 3.1525], [3.7674, 3.5423, 3.4913]])
        elif "kitti" in model_name: expected_slice = torch.tensor([[3.4291, 2.7865, 2.5151], [3.2841, 2.7021, 2.3502], [3.1147, 2.4625, 2.2481]])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--push_to_hub", action="store_true", help="Whether to upload the model to the hub.")
