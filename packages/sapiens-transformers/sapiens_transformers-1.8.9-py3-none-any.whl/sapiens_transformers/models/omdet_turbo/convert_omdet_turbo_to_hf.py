...
from sapiens_transformers import (CLIPTokenizer, DetrImageProcessor, OmDetTurboConfig, OmDetTurboForObjectDetection, OmDetTurboProcessor)
...
def get_omdet_turbo_config(model_name, use_timm_backbone):
    if "tiny" in model_name:
        ...
        num_heads = (3, 6, 12, 24)
        image_size = 640
    else: raise ValueError("Model not supported, only supports tiny variant.")
    config = OmDetTurboConfig(backbone_window_size=window_size, backbone_image_size=image_size, backbone_embed_dim=embed_dim, backbone_depths=depths, backbone_num_heads=num_heads,
    backbone_out_indices=(1, 2, 3), text_config={"model_type": "clip_text_model"}, use_timm_backbone=use_timm_backbone, backbone="swin_tiny_patch4_window7_224" if use_timm_backbone else None,
    apply_layernorm_after_vision_backbone=True if use_timm_backbone else False, use_pretrained_backbone=False)
    return config
def create_rename_keys_vision(state_dict, config):
    ...
    for layer_name in state_dict.keys():
        if layer_name.startswith("backbone") and not layer_name.startswith("backbone.norm"):
            ...
            else: layer_name_replace = layer_name.replace(f"backbone.norm{layer_num}", f"vision_backbone.vision_backbone.hidden_states_norms.stage{layer_num+1}")
        else: continue
    ...
    for layer_name_vision in state_dict_keys:
        if layer_name_vision.startswith("vision_backbone") and "qkv" in layer_name_vision:
            ...
            if "weight" in layer_name_vision:
                ...
                state_dict[layer_name_vision.replace("qkv.weight", "query.weight")] = in_proj_weight[hidden_size : hidden_size * 2, :]
                ...
                state_dict[layer_name_vision.replace("qkv.bias", "query.bias")] = in_proj_bias[hidden_size : hidden_size * 2]
    ...
    for layer_name_text in state_dict_keys:
        if layer_name_text.startswith("language_backbone") and "in_proj" in layer_name_text:
            if "weight" in layer_name_text:
                ...
                state_dict[layer_name_text.replace("in_proj_weight", "q_proj.weight")] = in_proj_weight[:hidden_size, :]
                state_dict[layer_name_text.replace("in_proj_weight", "k_proj.weight")] = in_proj_weight[hidden_size : hidden_size * 2, :]
                state_dict[layer_name_text.replace("in_proj_weight", "v_proj.weight")] = in_proj_weight[-hidden_size:, :]
                ...
                state_dict[layer_name_text.replace("in_proj_bias", "k_proj.bias")] = in_proj_bias[hidden_size : hidden_size * 2]
...
def convert_omdet_turbo_checkpoint(args):
    ...
    checkpoint_mapping = {'omdet-turbo-tiny': ['https://huggingface.co/omlab/OmDet-Turbo_tiny_SWIN_T/resolve/main/OmDet-Turbo_tiny_SWIN_T.pth', 'https://huggingface.co/omlab/OmDet-Turbo_tiny_SWIN_T/resolve/main/ViT-B-16.pt']}
    ...
    for src, dest in rename_keys_vision: rename_key(new_state_dict, src, dest)
    for src, dest in rename_keys_language: rename_key(new_state_dict, src, dest)
    if not use_timm_backbone: read_in_q_k_v_vision(new_state_dict, config)
    ...
    image_processor = DetrImageProcessor(size={"height": config.backbone_image_size, "width": config.backbone_image_size}, do_rescale=False, image_mean=IMAGE_MEAN, image_std=IMAGE_STD, do_pad=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", default="omdet-turbo-tiny", type=str, choices=["omdet-turbo-tiny"], help="Name of the OmDetTurbo model you'd like to convert.")
    ...
    parser.add_argument("--use_timm_backbone", action="store_true", help="Whether or not to use timm backbone for vision backbone.")
