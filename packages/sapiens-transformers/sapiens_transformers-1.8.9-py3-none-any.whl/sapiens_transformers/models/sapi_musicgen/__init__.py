...
_import_structure = {'configuration_sapi_musicgen': ['SAPIMusicGenConfig', 'SAPIMusicGenDecoderConfig'], 'processing_sapi_musicgen': ['SAPIMusicGenProcessor']}
try:
    if not sapiens_technology_torch(): raise SapiensTechnologyNotAvailable()
except SapiensTechnologyNotAvailable: pass
else: _import_structure["modeling_sapi_musicgen"] = ["SAPIMusicGenForConditionalGeneration", "SAPIMusicGenForCausalLM", "SAPIMusicGenModel", "SAPIMusicGenPreTrainedModel"]
if SAPIENS_TECHNOLOGY_CHECKING:
    from .configuration_sapi_musicgen import (SAPIMusicGenConfig, SAPIMusicGenDecoderConfig)
    from .processing_sapi_musicgen import SAPIMusicGenProcessor
    try:
        if not sapiens_technology_torch(): raise SapiensTechnologyNotAvailable()
    except SapiensTechnologyNotAvailable: pass
    else: from .modeling_sapi_musicgen import (SAPIMusicGenForCausalLM, SAPIMusicGenForConditionalGeneration, SAPIMusicGenModel, SAPIMusicGenPreTrainedModel)
else:
    import sys
