...
class FlaxBloomAttention(nn.Module):
    ...
    def _split_heads(self, hidden_states): return hidden_states.reshape(hidden_states.shape[:-1] + (self.num_heads, self.head_dim * 3))
...
class BloomGELU(nn.Module):
    def setup(self): self.dtype = jnp.float32
    def __call__(self, x): return x * 0.5 * (1.0 + tanh(0.79788456 * x * (1 + 0.044715 * x * x)))
...
class FlaxBloomBlock(nn.Module):
    ...
    def __call__(self, hidden_states, alibi, attention_mask=None, deterministic: bool = True, init_cache: bool = False, output_attentions: bool = False):
        ...
        if self.apply_residual_connection_post_layernorm: residual = post_layernorm
...
class FlaxBloomPreTrainedModel(FlaxPreTrainedModel):
    ...
    def __call__(self, input_ids, attention_mask=None, past_key_values: dict = None, params: dict = None, dropout_rng: jax.random.PRNGKey = None, train: bool = False,
...
class FlaxBloomBlockCollection(nn.Module):
    config: BloomConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.layers = [FlaxBloomBlock(self.config, name=str(layer_number), dtype=self.dtype) for layer_number in range(self.config.num_hidden_layers)]
...
class FlaxBloomModule(nn.Module):
    ...
    def __call__(self, input_ids=None, attention_mask=None, deterministic=True, init_cache: bool = False, output_attentions: bool = False, output_hidden_states: bool = False, return_dict: bool = True):
        ...
        if not return_dict: return tuple(v for v in [outputs[0], outputs[-1]] if v is not None)
...
class FlaxBloomModel(FlaxBloomPreTrainedModel): module_class = FlaxBloomModule
...
class FlaxBloomForCausalLM(FlaxBloomPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, max_length, attention_mask: Optional[jax.Array] = None):
        ...
        if attention_mask is not None: extended_attention_mask = lax.dynamic_update_slice(extended_attention_mask, attention_mask, (0, 0))
