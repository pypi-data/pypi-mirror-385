...
from sapiens_transformers import (CLIPTokenizer, Owlv2Config, Owlv2ForObjectDetection, Owlv2ImageProcessor, Owlv2Processor, Owlv2TextConfig, Owlv2VisionConfig)
...
def get_owlv2_config(model_name):
    if "large" in model_name:
        ...
        text_num_hidden_layers = 12
    vision_config = Owlv2VisionConfig(patch_size=patch_size, image_size=image_size, hidden_size=vision_hidden_size, num_hidden_layers=vision_num_hidden_layers,
    intermediate_size=vision_intermediate_size, num_attention_heads=vision_num_attention_heads)
    text_config = Owlv2TextConfig(hidden_size=text_hidden_size, intermediate_size=text_intermediate_size, num_attention_heads=text_num_attention_heads,
...
def flatten_nested_dict(params, parent_key="", sep="/"):
    ...
    for k, v in params.items():
        ...
        if isinstance(v, collections.MutableMapping): items.extend(flatten_nested_dict(v, new_key, sep=sep).items())
...
def convert_owlv2_checkpoint(model_name, checkpoint_path, pytorch_dump_folder_path, push_to_hub, verify_logits):
    ...
    for src, dest in rename_keys: rename_and_reshape_key(state_dict, src, dest, config)
    ...
    if "large" not in model_name: assert torch.allclose(inputs.pixel_values, original_pixel_values.float(), atol=1e-6)
    ...
    if verify_logits:
        if model_name == "owlv2-base-patch16":
            expected_logits = torch.tensor([[-10.0043, -9.0226, -8.0433], [-12.4569, -14.0380, -12.6153], [-21.0731, -22.2705, -21.8850]])
            ...
            expected_logits = torch.tensor([[-9.2391, -9.2313, -8.0295], [-14.5498, -16.8450, -14.7166], [-15.1278, -17.3060, -15.7169]])
            ...
            expected_logits = torch.tensor([[-12.6662, -11.8384, -12.1880], [-16.0599, -16.5835, -16.9364], [-21.4957, -26.7038, -25.1313]])
            expected_boxes = torch.tensor([[0.0136, 0.0161, 0.0256], [0.0126, 0.0135, 0.0202], [0.0498, 0.0948, 0.0915]])
            expected_objectness_logits = torch.tensor([[-6.7196, -9.4590, -13.9472]])
        elif model_name == "owlv2-large-patch14-finetuned":
            expected_logits = torch.tensor([[-9.5413, -9.7130, -7.9762], [-9.5731, -9.7277, -8.2252], [-15.4434, -19.3084, -16.5490]])
            expected_boxes = torch.tensor([[0.0089, 0.0080, 0.0175], [0.0112, 0.0098, 0.0179], [0.0375, 0.0821, 0.0528]])
            expected_objectness_logits = torch.tensor([[-6.2655, -6.5845, -11.3105]])
        elif model_name == "owlv2-large-patch14-ensemble":
            expected_logits = torch.tensor([[-12.2037, -12.2070, -11.5371], [-13.4875, -13.8235, -13.1586], [-18.2007, -22.9834, -20.6816]])
        ...
        print("Looks ok!")
    else: print("Model converted without verifying logits")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model_name", default="owlv2-base-patch16", choices=["owlv2-base-patch16", "owlv2-base-patch16-finetuned", "owlv2-base-patch16-ensemble",
    "owlv2-large-patch14", "owlv2-large-patch14-finetuned", "owlv2-large-patch14-ensemble"], type=str, help="Name of the Owlv2 model you'd like to convert from FLAX to PyTorch.")
