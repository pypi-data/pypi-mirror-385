...
from sapiens_transformers import XLMRobertaConfig, XLMRobertaXLForMaskedLM, XLMRobertaXLForSequenceClassification
...
from sapiens_transformers.models.roberta.modeling_roberta import RobertaAttention
from sapiens_transformers.utils import logging
if version.parse(fairseq.__version__) < version.parse("1.0.0a"): raise Exception("requires fairseq >= 1.0.0a")
...
def convert_xlm_roberta_xl_checkpoint_to_pytorch(roberta_checkpoint_path: str, pytorch_dump_folder_path: str, classification_head: bool):
    ...
    config = XLMRobertaConfig(vocab_size=roberta_sent_encoder.embed_tokens.num_embeddings, hidden_size=roberta.cfg.model.encoder_embed_dim, num_hidden_layers=roberta.cfg.model.encoder_layers,
    num_attention_heads=roberta.cfg.model.encoder_attention_heads, intermediate_size=roberta.cfg.model.encoder_ffn_embed_dim, max_position_embeddings=514, type_vocab_size=1, layer_norm_eps=1e-5)
    if classification_head: config.num_labels = roberta.model.classification_heads["mnli"].out_proj.weight.shape[0]
    ...
    if classification_head: their_output = roberta.model.classification_heads["mnli"](roberta.extract_features(input_ids))
    else: their_output = roberta.model(input_ids)[0]
