...
class Dinov2Embeddings(nn.Module):
    ...
    def forward(self, pixel_values: torch.Tensor, bool_masked_pos: Optional[torch.Tensor] = None) -> torch.Tensor:
        ...
        if bool_masked_pos is not None: embeddings = torch.where(bool_masked_pos.unsqueeze(-1), self.mask_token.to(embeddings.dtype).unsqueeze(0), embeddings)
...
class Dinov2LayerScale(nn.Module):
    ...
    def forward(self, hidden_state: torch.Tensor) -> torch.Tensor: return hidden_state * self.lambda1
...
DINOV2_ATTENTION_CLASSES = {"eager": Dinov2Attention, "sdpa": Dinov2SdpaAttention}
class Dinov2Layer(nn.Module):
    def __init__(self, config: Dinov2Config) -> None:
        ...
        if config.use_swiglu_ffn: self.mlp = Dinov2SwiGLUFFN(config)
        else: self.mlp = Dinov2MLP(config)
...
class Dinov2Model(Dinov2PreTrainedModel):
    def __init__(self, config: Dinov2Config):
        ...
        self.post_init()
    def get_input_embeddings(self) -> Dinov2PatchEmbeddings: return self.embeddings.patch_embeddings
    def _prune_heads(self, heads_to_prune: Dict[int, List[int]]) -> None:
        for layer, heads in heads_to_prune.items(): self.encoder.layer[layer].attention.prune_heads(heads)
    @add_start_docstrings_to_model_forward(DINOV2_BASE_INPUTS_DOCSTRING)
    def forward(self, pixel_values: Optional[torch.Tensor] = None, bool_masked_pos: Optional[torch.Tensor] = None, head_mask: Optional[torch.Tensor] = None, output_attentions: Optional[bool] = None,
...
@add_start_docstrings("Dinov2 backbone, to be used with frameworks like DETR and MaskFormer.", DINOV2_START_DOCSTRING)
