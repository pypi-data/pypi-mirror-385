...
class FlaxXLMRobertaLayer(nn.Module):
    ...
    def setup(self):
        ...
        if self.config.add_cross_attention: self.crossattention = FlaxXLMRobertaAttention(self.config, causal=False, dtype=self.dtype)
...
class FlaxXLMRobertaLayerCollection(nn.Module):
    ...
    def setup(self):
        ...
        else: self.layers = [FlaxXLMRobertaLayer(self.config, name=str(i), dtype=self.dtype) for i in range(self.config.num_hidden_layers)]
...
class FlaxXLMRobertaEncoder(nn.Module):
    ...
    dtype: jnp.dtype = jnp.float32
    gradient_checkpointing: bool = False
    def setup(self): self.layer = FlaxXLMRobertaLayerCollection(self.config, dtype=self.dtype, gradient_checkpointing=self.gradient_checkpointing)
    def __call__(self, hidden_states, attention_mask, head_mask, encoder_hidden_states: Optional[jnp.ndarray] = None, encoder_attention_mask: Optional[jnp.ndarray] = None,
    init_cache: bool = False, deterministic: bool = True, output_attentions: bool = False, output_hidden_states: bool = False, return_dict: bool = True):
...
class FlaxXLMRobertaModule(nn.Module):
    ...
    output_attentions: bool = False, output_hidden_states: bool = False, return_dict: bool = True):
        ...
        outputs = self.encoder(hidden_states, attention_mask, head_mask=head_mask, deterministic=deterministic, encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=encoder_attention_mask,
        init_cache=init_cache, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict)
...
class FlaxXLMRobertaModel(FlaxXLMRobertaPreTrainedModel): module_class = FlaxXLMRobertaModule
...
class FlaxXLMRobertaForMaskedLM(FlaxXLMRobertaPreTrainedModel): module_class = FlaxXLMRobertaForMaskedLMModule
...
@add_start_docstrings("XLM Roberta Model transformer with a sequence classification/regression head on top (a linear layer on top of the pooled output) e.g. for GLUE tasks.", XLM_ROBERTA_START_DOCSTRING)
class FlaxXLMRobertaForSequenceClassification(FlaxXLMRobertaPreTrainedModel): module_class = FlaxXLMRobertaForSequenceClassificationModule
...
@add_start_docstrings("XLM Roberta Model with a multiple choice classification head on top (a linear layer on top of the pooled output and a softmax) e.g. for RocStories/SWAG tasks.", XLM_ROBERTA_START_DOCSTRING)
class FlaxXLMRobertaForMultipleChoice(FlaxXLMRobertaPreTrainedModel): module_class = FlaxXLMRobertaForMultipleChoiceModule
...
@add_start_docstrings("XLM Roberta Model with a token classification head on top (a linear layer on top of the hidden-states output) e.g. for Named-Entity-Recognition (NER) tasks.", XLM_ROBERTA_START_DOCSTRING)
class FlaxXLMRobertaForTokenClassification(FlaxXLMRobertaPreTrainedModel): module_class = FlaxXLMRobertaForTokenClassificationModule
...
class FlaxXLMRobertaForQuestionAnswering(FlaxXLMRobertaPreTrainedModel): module_class = FlaxXLMRobertaForQuestionAnsweringModule
...
class FlaxXLMRobertaForCausalLMModule(nn.Module):
    ...
    encoder_attention_mask: Optional[jnp.ndarray] = None, init_cache: bool = False, deterministic: bool = True, output_attentions: bool = False, output_hidden_states: bool = False, return_dict: bool = True):
        outputs = self.roberta(input_ids, attention_mask, token_type_ids, position_ids, head_mask, encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=encoder_attention_mask,
        init_cache=init_cache, deterministic=deterministic, output_attentions=output_attentions, output_hidden_states=output_hidden_states, return_dict=return_dict)
        hidden_states = outputs[0]
        if self.config.tie_word_embeddings: shared_embedding = self.roberta.variables["params"]["embeddings"]["word_embeddings"]["embedding"]
        else: shared_embedding = None
...
@add_start_docstrings("XLM Roberta Model with a language modeling head on top (a linear layer on top of the hidden-states output) e.g for autoregressive tasks.", XLM_ROBERTA_START_DOCSTRING)
