"""
    ...
    #################################################################################################################################################################################################
"""
from sapiens_transformers import SapamaConfig, GenerationConfig, SapamaForCausalLM, PreTrainedTokenizerFast, SapamaTokenizer
from os import makedirs, path as _path, listdir
CONTEXT_LENGTH_FOR_VERSION = {"3.1": 131072, "3": 8192, "2": 4096, "1": 2048}
def write_model(model_path, input_base_path, model_size=None, safe_serialization=True, sapama_version="1", vocab_size=None, num_shards=None, instruct=False):
    makedirs(model_path, exist_ok=True)
    tmp_model_path = _path.join(model_path, "tmp")
    makedirs(tmp_model_path, exist_ok=True)
    from json import load, dump
    from torch import arange as sapiens_arange, load as sapiens_load, cat as sapiens_cat, save as sapiens_save, bfloat16 as sapiens_bfloat16, float16 as sapiens_float16
    def read_json(path):
        with open(path, "r") as file: return load(file)
    params = read_json(_path.join(input_base_path, "params.json"))
    num_shards = {'7B': 1, '8B': 1, '8Bf': 1, '7Bf': 1, '13B': 2, '13Bf': 2, '34B': 4, '30B': 4, '65B': 8, '70B': 8, '70Bf': 8, '405B': 8, '405B-MP16': 16}[model_size] if num_shards is None else num_shards
    ...
    inv_freq = 1.0 / (base ** (sapiens_arange(0, dims_per_head, 2).float() / dims_per_head))
    if base > 10000.0 and float(sapama_version) < 3: max_position_embeddings = 16384
    else: max_position_embeddings = CONTEXT_LENGTH_FOR_VERSION[sapama_version]
    ...
    if num_shards == 1: loaded = sapiens_load(_path.join(input_base_path, "consolidated.00.pth"), map_location="cpu")
    else: loaded = [sapiens_load(_path.join(input_base_path, file), map_location="cpu") for file in sorted([file for file in listdir(input_base_path) if file.endswith(".pth")])]
    ...
    for layer_i in range(n_layers):
        ...
        if num_shards == 1:
            ...
            state_dict[f"model.layers.{layer_i}.self_attn.q_proj.weight"] = permute(sapiens_cat([loaded[i][f"layers.{layer_i}.attention.wq.weight"].view(n_heads_per_shard, dims_per_head, dim)
            for i in range(len(loaded))], dim=0).reshape(dim, dim), n_heads=n_heads)
            state_dict[f"model.layers.{layer_i}.self_attn.k_proj.weight"] = permute(sapiens_cat([loaded[i][f"layers.{layer_i}.attention.wk.weight"].view(num_key_value_heads_per_shard, dims_per_head, dim)
            for i in range(len(loaded))], dim=0).reshape(key_value_dim, dim), num_key_value_heads, key_value_dim, dim)
            state_dict[f"model.layers.{layer_i}.self_attn.v_proj.weight"] = sapiens_cat([loaded[i][f"layers.{layer_i}.attention.wv.weight"].view(num_key_value_heads_per_shard, dims_per_head, dim)
            for i in range(len(loaded))], dim=0).reshape(key_value_dim, dim)
            state_dict[f"model.layers.{layer_i}.self_attn.o_proj.weight"] = sapiens_cat([loaded[i][f"layers.{layer_i}.attention.wo.weight"] for i in range(len(loaded))], dim=1)
            state_dict[f"model.layers.{layer_i}.mlp.gate_proj.weight"] = sapiens_cat([loaded[i][f"layers.{layer_i}.feed_forward.w1.weight"] for i in range(len(loaded))], dim=0)
            state_dict[f"model.layers.{layer_i}.mlp.down_proj.weight"] = sapiens_cat([loaded[i][f"layers.{layer_i}.feed_forward.w2.weight"] for i in range(len(loaded))], dim=1)
            state_dict[f"model.layers.{layer_i}.mlp.up_proj.weight"] = sapiens_cat([loaded[i][f"layers.{layer_i}.feed_forward.w3.weight"] for i in range(len(loaded))], dim=0)
        state_dict[f"model.layers.{layer_i}.self_attn.rotary_emb.inv_freq"] = inv_freq
        for k, v in state_dict.items(): index_dict["weight_map"][k], param_count = filename, param_count + v.numel()
        sapiens_save(state_dict, _path.join(tmp_model_path, filename))
    ...
    else: state_dict = {"model.norm.weight": loaded[0]["norm.weight"], "model.embed_tokens.weight": sapiens_cat([loaded[i]["tok_embeddings.weight"] for i in range(len(loaded))], dim=0 if sapama_version in ["3", "3.1"] else 1),
    "lm_head.weight": sapiens_cat([loaded[i]["output.weight"] for i in range(len(loaded))], dim=0)}
    for k, v in state_dict.items(): index_dict["weight_map"][k], param_count = filename, param_count + v.numel()
    sapiens_save(state_dict, _path.join(tmp_model_path, filename))
    ...
    config = SapamaConfig(hidden_size=dim, intermediate_size=compute_intermediate_size(dim, ffn_dim_multiplier, multiple_of), num_attention_heads=params["n_heads"],
    ...
    collect()
    print("Loading the checkpoint in a Sapama model.")
    model = SapamaForCausalLM.from_pretrained(tmp_model_path, torch_dtype=sapiens_bfloat16, low_cpu_mem_usage=True)
    del model.config._name_or_path
    model.config.torch_dtype = sapiens_float16
...
class SapamaConverter(TikTokenConverter):
...
try: from sapiens_transformers import SapamaTokenizerFast
except ImportError as error: SapamaTokenizerFast = None
def write_tokenizer(tokenizer_path, input_tokenizer_path, sapama_version="2", special_tokens=None, instruct=False):
    tokenizer_class = SapamaTokenizer if SapamaTokenizerFast is None else SapamaTokenizerFast
    if sapama_version in ["3", "3.1"]: tokenizer = SapamaConverter(input_tokenizer_path, special_tokens, instruct, model_max_length=CONTEXT_LENGTH_FOR_VERSION[sapama_version]).tokenizer
    else: tokenizer = tokenizer_class(input_tokenizer_path)
    print(f"Saving a {tokenizer_class.__name__} to {tokenizer_path}.")
    tokenizer.save_pretrained(tokenizer_path)
    return tokenizer
DEFAULT_SAPAMA_SPECIAL_TOKENS = {"3": ["<|begin_of_text|>", "<|end_of_text|>", "<|reserved_special_token_0|>", "<|reserved_special_token_1|>", "<|reserved_special_token_2|>", "<|reserved_special_token_3|>",
...
def main():
    ...
    parser.add_argument("--input_dir", help="Location of Sapama weights, which contains tokenizer.model and model folders")
    parser.add_argument("--model_size", default=None, help="")
    parser.add_argument("--output_dir", help="Location to write Sapiens model and tokenizer")
    parser.add_argument("--safe_serialization", default=True, type=bool, help="Whether or not to save using `safetensors`.")
    parser.add_argument("--sapama_version", choices=["1", "2", "3", "3.1"], default="1", type=str, help="Version of the Sapama model to convert.")
    ...
    parser.add_argument("--instruct", default=False, type=bool, help="Whether the model is an instruct model or not.")
    ...
    if args.special_tokens is None: args.special_tokens = DEFAULT_SAPAMA_SPECIAL_TOKENS.get(str(args.sapama_version), [])
    vocab_size = len(write_tokenizer(args.output_dir, _path.join(args.input_dir, "tokenizer.model"), sapama_version=args.sapama_version, special_tokens=args.special_tokens, instruct=args.instruct))
    if args.model_size != "tokenizer_only": write_model(model_path=args.output_dir, input_base_path=args.input_dir, model_size=args.model_size, safe_serialization=args.safe_serialization, sapama_version=args.sapama_version,
