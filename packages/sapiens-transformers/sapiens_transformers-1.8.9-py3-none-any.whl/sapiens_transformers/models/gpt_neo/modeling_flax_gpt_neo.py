...
class FlaxGPTNeoSelfAttention(nn.Module):
    ...
    def setup(self):
        ...
        if self.attention_type == "local": self.causal_mask = self.causal_mask ^ jnp.tril(self.causal_mask, -config.window_size)
...
class FlaxGPTNeoAttention(nn.Module):
    ...
    def __call__(self, hidden_states, attention_mask=None, deterministic: bool = True, init_cache: bool = False, output_attentions: bool = False): return self.attention(hidden_states, attention_mask=attention_mask, deterministic=deterministic, init_cache=init_cache, output_attentions=output_attentions)
...
class FlaxGPTNeoBlockCollection(nn.Module):
    config: GPTNeoConfig
    dtype: jnp.dtype = jnp.float32
    def setup(self): self.blocks = [FlaxGPTNeoBlock(self.config, layer_id=i, name=str(i), dtype=self.dtype) for i in range(self.config.num_hidden_layers)]
...
class FlaxGPTNeoModel(FlaxGPTNeoPreTrainedModel): module_class = FlaxGPTNeoModule
...
@add_start_docstrings("The GPTNeo Model transformer with a language modeling head on top (linear layer with weights tied to the input embeddings).", GPT_NEO_START_DOCSTRING)
