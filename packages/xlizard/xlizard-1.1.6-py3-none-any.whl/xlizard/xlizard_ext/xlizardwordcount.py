'''
This is an extension to xlizard. It counts the recurrence of every identifier
...
from xlizard_ext.keywords import IGNORED_WORDS


class xlizardExtension(object):
    ...
    def __call__(tokens, reader):
        ...
        for token in tokens:
            if token not in xlizardExtension.ignoreList\
                    and token[0] not in ('"', "'", '#'):
