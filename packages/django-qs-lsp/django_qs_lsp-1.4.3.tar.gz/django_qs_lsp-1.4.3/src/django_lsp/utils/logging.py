...
class UnifiedLogger:
    ...
    def __init__(self, name: str, use_lsp: bool = False):
        ...
        self._console_logger = logging.getLogger(f"django-lsp.{name}")
...
class DebugLogger:
    ...
    def __init__(self, name: str):
        self.logger = logging.getLogger(f"django-lsp.debug.{name}")
...
def setup_logging() -> None:
    ...
    root_logger = logging.getLogger("django-lsp")
    root_logger.setLevel(logging.DEBUG)  # Allow all logs to pass through

    # Create formatters
    user_formatter = logging.Formatter("[DJANGO-LSP] %(levelname)s: %(message)s")
    debug_formatter = logging.Formatter(
        "[DJANGO-LSP-DEBUG] %(name)s: %(levelname)s: %(message)s"
    )
