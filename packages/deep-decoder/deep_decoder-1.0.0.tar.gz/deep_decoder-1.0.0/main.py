...
if __name__ == "__main__":
    print("=" * 70)
    print("KIỂM THỬ DEEP DECODER HOÀN CHỈNH")
    print("=" * 70)

    test_cases = [
        {
            "name": "Ví dụ 1: URL Encoding đơn giản",
            "input": "q=%3Cscript%3Ealert%281%29%3C%2Fscript%3E",
            "description": "Giải mã URL-encoded script tag"
        },
        {
            "name": "Ví dụ 2: Base64 lồng nhau", 
            ...
            "description": "Base64 được encode nhiều lần"
        },
        {
            "name": "Ví dụ 3: HTML Entity XSS",
            "input": "&lt;script&gt;alert&#40;1&#41;&lt;&#x2F;script&gt;",
            "description": "HTML entities của XSS payload"
        },
        {
            "name": "Ví dụ 4: Hex encoding",
            "input": "48656C6C6F20576F726C6421",
            "description": "Hex encoding của 'Hello World!'"
        },
        {
            "name": "Ví dụ 5: Double URL-encoding",
            "input": "%252F%2545%2576%2569%256C%252F%253C%253E%253F",
            "description": "Double-encoded malicious URL"
        },
        {
            "name": "Ví dụ 6: Unicode Escape sequences",
            "input": "\\u0048\\u0065\\u006C\\u006C\\u006F\\u0020\\u0057\\u006F\\u0072\\u006C\\u0064",
            "description": "Unicode escape của 'Hello World'"
        },
        {
            "name": "Ví dụ 7: ROT13 văn bản thật",
            "input": "Uryyb Jbeyq",
            "description": "ROT13 của 'Hello World'"
        },
        {
            "name": "Ví dụ 8: Phức tạp - Multiple encoding layers",
            "input": "JTNDc2NyaXB0JTNFYWxlcnQoMSklM0MlMkZzY3JpcHQlM0U=",
            "description": "URL-encoded + Base64 (phức tạp)"
        }
    ]
    ...
    for i, test_case in enumerate(test_cases, 1):
        ...
        print(f"   Đầu vào: {test_case['input'][:100]}{'...' if len(test_case['input']) > 100 else ''}")
        print("-" * 70)

        result = deep_decode_data(test_case['input'], max_iterations=20)
        stats = get_decoding_statistics(result)

        print(f"✓ Kết quả cuối cùng: {result.final_data}")
        print(f"✓ Số lần lặp: {result.iterations}")
        print(f"✓ Thời gian xử lý: {result.total_time_ms:.2f}ms")
        print(f"✓ Phát hiện chu kỳ: {'Có' if result.cycle_detected else 'Không'}")
        
        if stats["successful_iterations"] > 0:
            print(f"✓ Thống kê: {stats['successful_iterations']} bước thành công, "
                  f"giảm {stats['size_reduction_percent']}% kích thước, "
                  f"độ tin cậy trung bình: {stats['average_confidence']}")

        if result.steps:
            print("\n📋 Chi tiết các bước giải mã:")
            for step in result.steps:
                status = "✅" if step.success else "❌"
                print(f"   [{step.iteration}] {status} {step.encoding_type:15} "
                      f"{step.input_length:3d} → {step.output_length:3d} "
                      f"(confidence: {step.confidence:.2%})")

    # =========================================================================
    # VÍ DỤ XUẤT JSON VÀ QUICK DECODE
    # =========================================================================
    print("\n" + "=" * 70)
    print("VÍ DỤ XUẤT KẾT QUẢ DƯỚI DẠNG JSON")
    print("=" * 70)
