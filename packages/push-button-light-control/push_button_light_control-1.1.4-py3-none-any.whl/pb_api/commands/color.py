...
class ColorCommand(BaseCommand):
    ...
    def __init__(self, device):
        ...
        self._current_state = [0] * 12  # Track current DC values    
    ...
    def set_led_color_preserve(self, device_id: int, led_position: int, color_name: str) -> bool:
        """
        Set color for specific LED while preserving other LEDs (stateful version)
        """
        if color_name not in COLOR_PRESETS_DC:
        ...
        # Map LED positions to array indices
        led_mapping = {
        ...
        self._current_state[base_index] = color_values['G']      # Green
        self._current_state[base_index + 1] = color_values['B']  # Blue
        self._current_state[base_index + 2] = color_values['R']  # Red
        
        print(f"[DEBUG] Setting LED {led_position} to {color_name}")
        print(f"[DEBUG] Current state: {self._current_state}")
        
        return self.set_color_values(device_id, self._current_state)
    
    def get_current_state(self) -> List[int]:
        """Get current DC values state"""
        return self._current_state.copy()
    
    def reset_state(self):
        """Reset current state to all zeros"""
        self._current_state = [0] * 12
    
    def set_multiple_leds_color(self, device_id: int, led_colors: dict) -> bool:
        """
        Set different colors for multiple LEDs using current state
        """
        # Update current state with new colors
        led_mapping = {
            0: 0, 1: 3, 2: 6, 3: 9
        }
        
        for led_position, color_name in led_colors.items():
            if color_name not in COLOR_PRESETS_DC:
                available_colors = ", ".join(COLOR_PRESETS_DC.keys())
                raise ValueError(f"Unknown color: {color_name} for LED {led_position}. Available: {available_colors}")
            ...
            self._current_state[base_index] = color_values['G']
            self._current_state[base_index + 1] = color_values['B']
            self._current_state[base_index + 2] = color_values['R']
        
        print(f"[DEBUG] Setting multiple LEDs: {led_colors}")
        print(f"[DEBUG] Current state: {self._current_state}")
    ...
    def set_led_color(self, device_id: int, led_position: int, color_name: str, 
                            current_state: List[int] = None) -> bool:
        """
        Set color for specific LED while preserving other LEDs
        
        Args:
            device_id: Target device ID
            led_position: LED position (0-3)
            color_name: Color name from COLOR_PRESETS_DC
            current_state: Current DC values (optional, if not provided, others will be set to 0)
        ...
        # Start with current state or zeros
        if current_state is None:
            dc_array = [0] * 12
        else:
            dc_array = current_state.copy()
        ...
        base_index = led_mapping[led_position]
        dc_array[base_index] = color_values['G']      # Green
        dc_array[base_index + 1] = color_values['B']  # Blue
        dc_array[base_index + 2] = color_values['R']  # Red
        
        print(f"[DEBUG] Setting LED {led_position} to {color_name} (preserving others)")
        print(f"[DEBUG] DC array: {dc_array}")
