...
import mlflow_oidc_auth_groups_plugin_adfs
...
def _fixture_loader() -> griffe.GriffeLoader:
    ...
    loader.load("mlflow_oidc_auth_groups_plugin_adfs")
...
def _fixture_internal_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mlflow_oidc_auth_groups_plugin_adfs._internal"]


@pytest.fixture(name="public_api", scope="module")
def _fixture_public_api(loader: griffe.GriffeLoader) -> griffe.Module:
    return loader.modules_collection["mlflow_oidc_auth_groups_plugin_adfs"]
...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `mlflow_oidc_auth_groups_plugin_adfs`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in mlflow_oidc_auth_groups_plugin_adfs.__all__ or not hasattr(mlflow_oidc_auth_groups_plugin_adfs, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in mlflow_oidc_auth_groups_plugin_adfs.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("mlflow_oidc_auth_groups_plugin_adfs")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "mlflow_oidc_auth_groups_plugin_adfs" or item.name.startswith("mlflow_oidc_auth_groups_plugin_adfs."))
        ):
