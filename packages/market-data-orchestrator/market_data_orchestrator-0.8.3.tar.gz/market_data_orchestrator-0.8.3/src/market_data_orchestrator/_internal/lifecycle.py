...
class LifecycleManager:
    ...
    async def start(self) -> None:
        ...
        try:
            # Connect to feedback bus
            if self._feedback_bus and self.settings.feedback_enabled:
                await self._feedback_bus.connect()
                log.info("Connected to feedback bus")
            ...
            if self._runtime and self._provider:
                await self._runtime.start_pipeline(source=self._provider)
    ...
    async def cleanup(self) -> None:
        ...
        # Disconnect feedback bus
        if self._feedback_bus:
            try:
                await self._feedback_bus.disconnect()
                log.info("Feedback bus disconnected")
            except Exception as e:
                log.error(f"Error disconnecting feedback bus: {e}")
