...
def start_project_activity(project_name, activity_name, blocking=False, data=None):
    """POST start activity."""
    if blocking:
        return start_blocking_project_activity(project_name, activity_name, data)
    return send_request(
        get_start_activity_uri().format(project_name, activity_name),
        method="POST",
        data=data,
    ).decode("utf-8")
...
def get_project_activity_status(project_name, activity_name):
    ...
    return json.loads(
        send_request(
            get_status_activity_uri().format(project_name, activity_name), method="GET"
        ).decode("utf-8")
    )
...
def cancel_project_activity(project_name, activity_name):
    ...
    return send_request(
        get_cancel_activity_uri().format(project_name, activity_name), method="POST"
    ).decode("utf-8")
...
def start_blocking_project_activity(project_name, activity_name, data):
    ...
    return send_request(
        get_start_blocking_activity_uri().format(project_name, activity_name),
        method="POST",
        data=data,
    ).decode("utf-8")
