...
from cmem.cmempy.workspace.activities import ACTIVITY_TYPE_EXECUTE_DEFAULTWORKFLOW
...
def start_task_activity(
    project_name,
    task_name,
    activity_name=ACTIVITY_TYPE_EXECUTE_DEFAULTWORKFLOW,
    blocking=False,
    data=None,
):
    ...
    if blocking:
        return start_blocking_task_activity(
            project_name, task_name, activity_name=activity_name, data=data
        )
    return send_request(
        get_start_activity_uri().format(project_name, task_name, activity_name),
        method="POST",
        data=data,
    ).decode("utf-8")
...
def get_activity_status(
    project_name, task_name, activity_name=ACTIVITY_TYPE_EXECUTE_DEFAULTWORKFLOW
):
    """GET retrieve activity status."""
    return json.loads(
        send_request(
            get_status_activity_uri().format(project_name, task_name, activity_name),
            method="GET",
        ).decode("utf-8")
    )
