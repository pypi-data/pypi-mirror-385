...
def get_task_activities(project_name, task_name):
    ...
    response = send_request(
        get_activities_uri().format(project_name, task_name), method="GET"
    )
...
def get_activity_status(project_name, task_name, activity_name):
    ...
    return taskactivity.get_activity_status(project_name, task_name, activity_name)
...
def get_activities_status(project_name=None, status=None, activity=None):
    ...
    if status:
        if status not in VALID_ACTIVITY_STATUS:
            raise ValueError(
                f"Status {status} is not a valid activity status, "
                f"one of {VALID_ACTIVITY_STATUS}"
            )
        params["statusFilter"] = status
    status_info = get_json(task_activities_status_api, params=params)
    if activity:
        status_info = [info for info in status_info if info["activity"] == activity]
    return status_info
