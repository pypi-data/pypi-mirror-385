...
def import_xml_zip(xml_zip_file_path):
    ...
    with open(xml_zip_file_path, "rb") as file_handle:
        xml_zip_file = {"file": (os.path.basename(xml_zip_file_path), file_handle)}
        import_uri = get_import_uri().format("xmlZip")
        response = send_request(import_uri, method="POST", files=xml_zip_file)
        return response
...
def import_rdf_turtle(rdf_turtle_zip_file_path):
    ...
    with open(rdf_turtle_zip_file_path, "rb") as file_handle:
        rdf_turtle_zip_file = {
            "file": (os.path.basename(rdf_turtle_zip_file_path), file_handle)
        }
        import_uri = get_import_uri().format("rdfTurtle")
        response = send_request(import_uri, method="POST", files=rdf_turtle_zip_file)
        return response
...
def import_workspace(file_path, plugin_name="xmlZip"):
    ...
    with open(file_path, "rb") as file_handle:
        file = {"file": (os.path.basename(file_path), file_handle)}
        import_uri = get_import_uri().format(plugin_name)
        response = send_request(import_uri, method="POST", files=file)
        return response
