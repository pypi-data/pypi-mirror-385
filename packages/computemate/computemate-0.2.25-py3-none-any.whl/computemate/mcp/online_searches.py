...
def search_weather(request:str) -> str:
    f"""Answer a query about weather; a query is required.

Args [required]:
    code: Generate python code that use my OpenWeatherMap API key '{OPENWEATHERMAP_API_KEY[0]}' to answer my query. Use Celsius as the unit for temperature.
"""
    global agentmake, getResponse
    messages = agentmake(request, **{'tool': 'qna/search_weather'}, **AGENTMAKE_CONFIG)
    return getResponse(messages)
