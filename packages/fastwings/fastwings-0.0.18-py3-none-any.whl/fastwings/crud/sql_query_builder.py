...
class QueryBuilder(Generic[ModelType]):
    ...
    def as_subquery(self, name: str | None = None) -> NamedFromClause:
        ...
        Returns:
            NamedFromClause: A subquery that can be joined or selected from.
        """
        stmt = self.as_select()
        if name:
    ...
    def clone(self) -> "Self":
        ...
        since the list contents (SQLAlchemy expressions) are typically immutable.
        ...
        # Shallow copy list attributes (contents are immutable SQLAlchemy expressions)
...
class SoftDeletableQueryBuilder(QueryBuilder[ModelType]):
    ...
    def __init__(self, model_class: type[ModelType]):
        ...
        Raises:
            ...
            ValueError: If 'is_deleted' is not a Boolean column.
        """
        ...
        # Validate that is_deleted is a Boolean column
        mapper = inspect(self.model_class)
        if mapper is not None:
            columns = mapper.columns
            if "is_deleted" in columns:
                is_deleted_col = columns["is_deleted"]
                if not isinstance(is_deleted_col.type, Boolean):
                    raise ValueError(
                        f"Model {self.model_class.__name__} has 'is_deleted' attribute "
                        f"but it's not a Boolean column (found: {type(is_deleted_col.type).__name__})"
                    )
    ...
    def include_deleted(self, include: bool = True) -> "Self":
        ...
        Args:
            include (bool, optional): If True, include soft-deleted records. Defaults to True.
    ...
    def only_deleted(self) -> "Self":
        ...
        Returns:
            Self: The QueryBuilder instance for chaining.
        """
        # Disable automatic filtering and manually add is_deleted=True filter
        self._filter_soft_deleted = False
        is_deleted_col = cast(ColumnElement[bool], self.model_class.is_deleted)
        return self.add_filters(is_deleted_col.is_(True))
    ...
    def _apply_common_clauses(self, stmt: Select[Any]) -> Select[Any]:
        ...
        """
        # Apply all filters from the parent class (join, where, filter_by)
    ...
    def clone(self) -> "Self":
        ...
        Returns:
            Self: A new SoftDeletableQueryBuilder with the same configuration.
        """
        new_builder = super().clone()
        # Copy the soft-delete filter state
