...
class Info(object):
    ...
    def get_short_name(self, process_name):
        if '.' in process_name:
...
class CpuInfo(Info):
    ...
    def get_plugins_with_process_name(self, process_name):
        short_name = self.get_short_name(process_name)
...
class ProcrankInfo(Info):
    def get_plugins_with_process_name(self, process_name):
        short_name = self.get_short_name(process_name)
        return [RegexPlugin('procrank.%s' % short_name, u'内存 (MB)', ['pss', 'uss'],
                            r'.*\d+\s+\d+K\s+\d+K\s+(?P<pss>\d+)K\s+(?P<uss>\d+)K\s+%s' % process_name, operation='%s / 1024.0'),
                            ]
