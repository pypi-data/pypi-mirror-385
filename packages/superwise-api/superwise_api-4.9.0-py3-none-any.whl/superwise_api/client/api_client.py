...
class ApiClient:
    ...
    def sanitize_for_serialization(self, obj):
        ...
        If obj is list or set, sanitize each element in the list.
    ...
    def call_api(
        ...
        :param post_params dict: Request post form parameters,
            for `agent/x-www-form-urlencoded`, `multipart/form-data`.
            ...
            for k, v in files.items():
                ...
                for n in file_names:
                    with open(n, "rb") as f:
                        ...
                        filedata = f.read()
                        mimetype = mimetypes.guess_type(filename)[0] or "agent/octet-stream"
                        params.append(tuple(["files", tuple([filename, filedata, mimetype])]))
    ...
    def __deserialize_model(self, data, klass):
        ...
        if isinstance(data, (dict)):
            ...
            return [klass.from_dict(item) for item in data]
    ...
    def update(
        ...
        _path_params = _params.get("path_params", {"id": entity_id})
        # Assumes that the resource path already has the parameterized params embedded
        # e.g., /resource/{id}/things/{other_param}, path_params = {"id": "123", "other_param": "456"}
        resource_path = resource_path + ("/{id}" if "path_params" not in _params else "")
    ...
    def get_by_id(
        ...
        if "path_params" in _params:
            # Assumes that the resource path already has the parameterized params embedded
            # e.g. /resource/{id}/things/{other_param}, path_params = {"id": "123", "other_param": "456"}
            _path_params = _params.get("path_params")
        else:
            _path_params = {"id": entity_id}
            resource_path = resource_path + "/" + "{id}"
    ...
    def get(
        ...
        **kwargs,
        ...
        _response_types_map = {
            "200": Page.set_model(model_class) if paginate else model_class,
            "404": None,
            "422": None,
            "500": None,
        }
