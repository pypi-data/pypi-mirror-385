...
class BeanieQueryEngine(Generic[TDocument]):
    ...
    def _build_filter_stage(
        ...
        if "filter" in query_components:
            ...
            filter_dict = cast(
                dict[str, object], self._convert_enums_to_values(filter_dict_raw)
            )
    ...
    def _process_string_sort(
        self, sort_spec: str, pipeline: list[dict[str, object]]
    ) -> str:
        """
    ...
    def _parse_list_sort_fields(self, sort_spec: list[object]) -> dict[str, int]:
        ...
        for item in sort_spec:
            if isinstance(item, (list, tuple)):
                item_cast: list[object] | tuple[object, ...] = cast(
                    list[object] | tuple[object, ...], item
                )
                if len(item_cast) == 2:
    ...
    def _transform_value(
        self, field_path: str, operator: ComparisonOperator | str, value: object
    ) -> object:
        """
    ...
    def _convert_enums_to_values(
        ...
        object
        | dict[object, object | dict[object, object] | list[object] | object]
        | list[object]
        | object
    ):
        """
        ...
        if isinstance(data, Enum):
            ...
            return {
                key: self._convert_enums_to_values(value)
                for key, value in cast(dict[object, object], data).items()
            }
        elif isinstance(data, list):
            return [
                self._convert_enums_to_values(item) for item in cast(list[object], data)
            ]
        else:
            return data
