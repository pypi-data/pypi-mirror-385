...
class MidjourneyResource:
    ...
    def edit(
        ...
        data = self._http.request(
            "POST", "/v1/edit", json=request.model_dump(by_alias=True, exclude_none=True)
        )
        return TaskResponse.model_validate(data)
    ...
    def upload_paint(
        ...
        data = self._http.request(
            "POST", "/v1/upload-paint", json=request.model_dump(by_alias=True)
        )
        return TaskResponse.model_validate(data)
    ...
    def video_diffusion(
        ...
        Example:
            ...
            ```
        """
        request = VideoDiffusionRequest(prompt=prompt, videoType=video_type, callback=callback)
        data = self._http.request(
            "POST", "/v1/video-diffusion", json=request.model_dump(exclude_none=True)
        )
        return TaskResponse.model_validate(data)
    ...
    def extend_video(
        ...
        data = self._http.request(
            "POST", "/v1/extend-video", json=request.model_dump(by_alias=True, exclude_none=True)
        )
        return TaskResponse.model_validate(data)
...
class AsyncMidjourneyResource:
    ...
    async def reroll(self, job_id: str, callback: Union[HttpUrl, str, None] = None) -> TaskResponse:
        ...
        request = RerollRequest(job_id=job_id, callback=callback)
        data = await self._http.request(
            "POST", "/v1/reroll", json=request.model_dump(by_alias=True)
        )
        return TaskResponse.model_validate(data)
