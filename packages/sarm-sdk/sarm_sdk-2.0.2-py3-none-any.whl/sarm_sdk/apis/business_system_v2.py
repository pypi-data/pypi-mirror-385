...
class BusinessSystemAPI:
    ...
    def publish(
        self,
        business_system_repeat_id: List[str]
    ) -> Dict[str, Any]:
        """
        发布业务系统（预审）
        ...
        Args:
            business_system_repeat_id: 业务系统重复性ID列表
        ...
        Returns:
            Dict[str, Any]: API响应，格式为：
            {
                "data": [
                    {
                        "business_system_id": 发布后的系统ID（整数类型）,
                        "status": "处理结果（success/fail）",
                        "error": "错误信息（失败时返回）"
                    }
                ],
                "code": 200
        ...
        Example:
            >>> # 发布单个业务系统
            >>> result = api.publish(
            ...     business_system_repeat_id=["1"]
            ... )
            >>> # 检查发布结果
            >>> for item in result['data']:
            ...     if item['status'] == 'success':
            ...         print(f"业务系统发布成功，新ID: {item['business_system_id']}")
            ...     else:
            ...         print(f"业务系统发布失败: {item['error']}")
            >>>
            >>> # 批量发布多个业务系统
            >>> result = api.publish(
            ...     business_system_repeat_id=["1", "2", "3"]
            ... )
            >>> # 处理批量发布结果
            ...
            >>> print(f"成功发布 {success_count} 个业务系统，失败 {fail_count} 个")
            >>>
            >>> # 分析发布结果
            >>> for item in result['data']:
            ...     if item['status'] == 'success':
            ...         print(f"系统ID {item['business_system_id']} 发布成功")
            ...     else:
            ...         print(f"发布失败: {item.get('error', '未知错误')}")

        Note:
            - 发布操作是不可逆的，请谨慎操作
            - 建议在发布前先通过 get_list() 方法查询预审业务系统状态
            - 发布成功后会返回正式数据的业务系统ID
            - 批量发布时，部分系统失败不会影响其他系统的发布
            - 建议先发布相关的组织架构，再发布业务系统
            - 业务系统发布后，才能发布该系统下的应用
        """
        request_data = {
            "business_system_repeat_id": business_system_repeat_id
        }

        response = self.client.post('/api/business_system/temporary_business_system/published', data=request_data)
        return response
