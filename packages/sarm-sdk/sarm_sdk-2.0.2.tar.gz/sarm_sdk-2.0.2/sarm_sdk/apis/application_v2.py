...
class ApplicationAPI:
    ...
    def publish(
        self,
        application_repeat_id: List[str]
    ) -> Dict[str, Any]:
        """
        发布应用（预审）
        ...
        Args:
            application_repeat_id: 应用重复性ID列表
        ...
        Example:
            >>> # 发布单个应用
            >>> result = api.publish(
            ...     application_repeat_id=["app_repeat_001"]
            ... )
            >>> # 检查发布结果
            >>> for item in result['data']:
            ...     if item['status'] == 'success':
            ...         print(f"应用发布成功，新ID: {item['application_id']}")
            ...     else:
            ...         print(f"应用发布失败: {item['error']}")
            >>>
            >>> # 批量发布多个应用
            >>> result = api.publish(
            ...     application_repeat_id=[
            ...         "app_repeat_001",
            ...         "app_repeat_002",
            ...         "app_repeat_003"
            ...     ]
            ...
            >>> print(f"成功发布 {success_count} 个应用，失败 {fail_count} 个")

        Note:
            - 发布操作是不可逆的，请谨慎操作
            - 建议在发布前先通过 get_list() 方法查询预审应用状态
            - 发布成功后会返回正式数据的应用ID
            - 批量发布时，部分应用失败不会影响其他应用的发布
            - 建议先发布相关的业务系统和组织架构，再发布应用
        """
        request_data = {
            "application_repeat_id": application_repeat_id
        }

        response = self.client.post('/api/application/temporary_application/publish', data=request_data)
        return response
