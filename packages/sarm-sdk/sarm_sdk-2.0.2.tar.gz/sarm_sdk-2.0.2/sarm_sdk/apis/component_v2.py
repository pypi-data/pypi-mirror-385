...
class ComponentAPI:
    ...
    def publish(
        self,
        component_repeat_id_list: List[str]
    ) -> Dict[str, Any]:
        """
        成分发布（预审）
        ...
        Args:
            component_repeat_id_list: 成分重复性标识列表
        ...
        Example:
            >>> # 发布单个成分
            >>> result = api.publish(
            ...     component_repeat_id_list=["comp_repeat_id_001"]
            ... )
            >>> # 检查发布结果
            >>> for item in result['data']:
            ...     if item['status'] == 'success':
            ...         print(f"成分 {item['component_repeat_id']} 发布成功")
            ...     else:
            ...         print(f"成分 {item['component_repeat_id']} 发布失败: {item['error']}")
            >>>
            >>> # 批量发布多个成分
            >>> result = api.publish(
            ...     component_repeat_id_list=[
            ...         "comp_repeat_id_001",
            ...         "comp_repeat_id_002",
            ...         "comp_repeat_id_003"
            ...     ]
        ...
        Note:
            - 发布操作是不可逆的，请谨慎操作
            - 建议在发布前先通过 get_list_temporary() 方法查询预审成分状态
            - 强烈建议只发布 data_status 为 "perfect"（待发布）的成分
            - 发布失败的成分会在返回结果中标明失败原因
            - 批量发布时，部分成分失败不会影响其他成分的发布
        """
        request_data = {
            "component_repeat_id_list": component_repeat_id_list
        }

        response = self.client.post('/api/v2/component/temporary/release', data=request_data)
        return response
