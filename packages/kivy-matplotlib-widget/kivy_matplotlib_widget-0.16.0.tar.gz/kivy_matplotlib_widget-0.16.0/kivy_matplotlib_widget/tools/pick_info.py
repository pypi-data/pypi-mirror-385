...
from matplotlib.patches import Patch, PathPatch, Polygon, Rectangle, Wedge
...
def _(artist, event):
    ...
    if (artist.get_linestyle() not in ["None", "none", " ", "", None]
            and len(artist.get_xydata()) > 1) and event.projection:
...
def _(artist, event):
    ...
    if sel and sel.dist < event.pickradius:
        return sel
