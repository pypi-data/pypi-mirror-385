...
class PathResolver:
    ...
    def resolve_and_validate(
        ...
        Returns:
            Result.success(resolved_path) or Result.failure(error_message)
        ...
        if absolute_path.is_symlink():
            return Result.failure(f"Symlinks not allowed for security: {absolute_path}")

        # Resolve to canonical path (follows symlinks in parents, normalizes ..)
        try:
            resolved_path = absolute_path.resolve()
        except (OSError, RuntimeError) as e:
            return Result.failure(f"Failed to resolve path '{path}': {e}")
        ...
        if ".." in str(resolved_path):
            return Result.failure(f"Path traversal detected after resolution: {resolved_path}")
...
class FileOperations:
    ...
    def read_text(
        ...
        Returns:
            Result.success(content) or Result.failure(error_message)
        ...
        if not path.is_file():
            return Result.failure(f"Path is not a file: {path}")
        ...
        if max_size_bytes is not None:
            try:
                ...
                return Result.failure(f"Failed to stat file '{path}': {e}")
        ...
        try:
            ...
            return Result.failure(f"Encoding error reading '{path}' with {encoding}: {e}")
        except OSError as e:
            return Result.failure(f"Failed to read file '{path}': {e}")
    ...
    def write_text(
        ...
        Returns:
            Result.success(bytes_written) or Result.failure(error_message)
        ...
        if create_parents:
            try:
                ...
                return Result.failure(f"Failed to create parent directories for '{path}': {e}")
        ...
        try:
            ...
            return Result.failure(f"Failed to write file '{path}': {e}")
        except UnicodeEncodeError as e:
            return Result.failure(f"Encoding error writing '{path}' with {encoding}: {e}")
        ...
        if mode is not None:
            try:
                ...
                return Result.failure(f"Failed to set permissions on '{path}': {e}")
...
class JSONOperations:
    ...
    @staticmethod
    def read_json(path: Path, required: bool = False) -> Result[dict[str, Any]]:
        ...
        Returns:
            Result.success(parsed_dict) or Result.failure(error_message)
        ...
        if not path.exists():
            if required:
                return Result.failure(f"Required JSON file not found: {path}")
            else:
        ...
        try:
            ...
            return Result.failure(f"Invalid JSON in '{path}': {e}")
        except OSError as e:
            return Result.failure(f"Failed to read JSON file '{path}': {e}")

    @staticmethod
    def write_json(path: Path, data: dict[str, Any]) -> Result[None]:
        ...
        Returns:
            Result.success(None) or Result.failure(error_message)
        ...
        try:
            ...
            return Result.failure(f"Failed to write JSON to '{path}': {e}")
