...
class DAGResolver:
    ...
    def topological_sort(self) -> Result[list[str]]:
        ...
        for block, deps in self.dependencies.items():
            if block not in self.blocks:
                return Result.failure(f"Block '{block}' in dependencies but not in blocks list")
        ...
        if len(result) != len(self.blocks):
            return Result.failure("Cyclic dependency detected in workflow")
    ...
    def get_execution_waves(self) -> Result[list[list[str]]]:
        ...
        for block, deps in self.dependencies.items():
            ...
            for dep in deps:
                if dep not in self.blocks:
                    return Result.failure(f"Dependency '{dep}' not found")
        ...
        while remaining:
            ...
            if not ready:
                return Result.failure("Cyclic dependency detected or isolated blocks")

            waves.append(ready)
            completed.update(ready)
            remaining.difference_update(ready)

        return Result.success(waves)
