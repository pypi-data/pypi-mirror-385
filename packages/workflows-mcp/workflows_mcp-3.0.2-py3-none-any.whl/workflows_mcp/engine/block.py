...
class Block:
    ...
    async def execute(self, context: dict[str, Any]) -> Result[BlockOutput]:
        ...
        """
        try:
            # Inject custom outputs schema into context if defined
            # This allows executors (like Shell) to read and populate custom outputs
            # Note: Convert OutputSchema Pydantic models to dicts for executor consumption
            if self._outputs_schema:
                outputs_dict = {
                    name: schema.model_dump() for name, schema in self._outputs_schema.items()
                }
                context["__block_custom_outputs__"] = outputs_dict

            result = await self.executor.execute(self._validated_inputs, context)

            # Clean up custom outputs from context
