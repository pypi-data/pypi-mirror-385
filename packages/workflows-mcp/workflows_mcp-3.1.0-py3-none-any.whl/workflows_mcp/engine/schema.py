...
class ValueType(str, Enum):
    ...
    def input_types(cls) -> tuple[str, ...]:
        ...
        return (cls.STR.value, cls.INT.value, cls.FLOAT.value, cls.BOOL.value,
                cls.LIST.value, cls.DICT.value)
...
class DependencySpec(BaseModel):
    ...
    Defines a dependency on another block with control over whether this
    dependency is required (block skips if dependency skips/fails) or optional
    (block runs even if dependency skips/fails).

    Attributes:
        block: Block ID this depends on
        required: Skip if dependency skipped/failed (default: True)

    Example:
        depends_on:
          - block: build
            required: true   # Skip if build fails/skips (default)
          - block: lint
            required: false  # Run even if lint fails/skips (optional)
    """

    block: str = Field(
        description="Block ID this depends on",
        pattern=r"^[a-z_][a-z0-9_]*$",
        min_length=1,
        max_length=100,
    )
    required: bool = Field(
        default=True, description="Skip if this dependency skipped or failed (default: true)"
    )
