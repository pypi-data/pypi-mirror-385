...
class PretrainProcessor(Processor):
    ...
    def transform(self, item, is_last_sample=False):
        ...
        if special_token_idxs and self.tokenizer.vec2txt(
            tokens[special_token_idxs[0]]
        ).startswith("</"):
            # flip the loss mask
            logging.warn(
                f"Flipping loss mask for sample from {item.get('filepath', 'unknown')}:{item.get('row_idx', 'unknown')}"
            )
        ...
        if self.remove_special_tokens and special_token_idxs:
            new_tokens = [
                tokens[i] for i in range(len(tokens)) if i not in special_token_idxs
            ]
            new_loss_mask = [
                loss_mask[i]
                for i in range(len(loss_mask))
