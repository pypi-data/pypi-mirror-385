...
class DonkitChunker:
    ...
    def chunk_text(
        ...
        if self._config.splitter == "json":
            ...
            return self._json_split(
                json_data, self._config.chunk_size, self._config.chunk_overlap
            )
    ...
    @staticmethod
    def _json_split(
        json_data: dict, max_chunk_size: int, overlap: int
    ) -> list[Document]:
        """
        ...
        for page in content:
            if isinstance(page, str):
                documents.append(
                    Document(
                        page_content=page,
                        metadata={"page": 0, "type": "text"},
                    )
                )
                continue
            # Extract metadata fields
            metadata = {"page": page.get("page", 0), "type": page.get("type", "text")}
    ...
    def _character_split(
        text: str, max_chunk_size: int, overlap: int
    ) -> list[Document]:
        """
        ...
        while start < text_length:
            ...
            if chunk:  # Only add non-empty chunks
                chunks.append(
                    Document(page_content=chunk, metadata={"chunk_index": len(chunks)})
                )
    ...
    @staticmethod
    def _sentence_split(text: str, max_chunk_size: int, overlap: int) -> list[Document]:
        """
    ...
    @staticmethod
    def _semantic_split(text: str, max_chunk_size: int, overlap: int) -> list[Document]:
        """
