...
class PostreSQLCache(BaseCache):
...
WHERE cache.hash = ? and categories.name = ? and cache.category = categories.cat_id and 
...
FROM cache, categories
WHERE cache.hash = ? AND categories.name = ? AND cache.category = categories.cat_id AND 
unixepoch('now') - cache.last_updated <= categories.duration order by sequence ASC;""",
    ...
    def upsert(self, category_tag: str, data: bytes, chunk_size: int=32 * 1024, *args, **kwargs) -> int:
        ...
        cache_id = self.connector.execute("""SELECT ROWID FROM """)

        try:
            res = self.connector.executemany("""INSERT INTO cache (hash, sequence, data, category, last_updated)
SELECT :hash, :sequenceuence, :data, cat_id, unixepoch('now') FROM categories WHERE name=:name
ON CONFLICT (hash, sequence) DO UPDATE SET data = excluded.data, category=excluded.category, last_updated = excluded.last_updated;""",
                            data_list)
        except TypeError:
            return False
        else:
    ...
    def delete(self, category_tag: str, *args, **kwargs) -> int:
        ...
        try:
            res = self.connector.execute("""DELETE FROM cache WHERE hash = :hash;""",
                                         delete_dict)
        except TypeError:
            return False
        else:
