...
from .utils.utils import auth_host, api_host
...
class BattlenetClient:
    ...
    def get(self, api_endpoint: Callable, category_name: str, *args, **kwargs) -> Optional[bytes]:
        ...
        if self.cache:
            if self.cache.check(category_name, *args, **kwargs):
                return self.cache.select(category_name, *args, **kwargs)
        ...
        for _ in range(5):
            try:
                ...
                if self.cache:
                    if self.cache.update(category_name, response.content, *args, **kwargs):
                        return self.cache.select(category_name, *args, **kwargs)
