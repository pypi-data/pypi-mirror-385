from sympy import Matrix, Symbol, Number, eye, latex, symbols, zeros
...
class PivotGaussSysteme:
    ...
    def __init__(
            self,
            tab: list[list[int|float|Fraction|Number]],
        ...
        """Prend en paramètres:
            ...
            affichage_lignes_zeros: bool
                permet d'afficher ou masquer les équations 0 = 0 dans le système
                résultat. Par défaut, les équations 0 = 0 sont affichées.
                Passer ce paramètre à False pour les masquer.
        """
        ...
        # Si le Jupyter Notebook est exécuté dans Google Colab
        if getenv("COLAB_RELEASE_TAG"):
            # on ne met pas de balise \begin{align}
            self._latex = self._latex_systeme()
        else: # sinon (dans un Jupyter Notebook classique)
            # on met la balise \begin{align}
    ...
    def _nbre_variables(self) -> int:
        ...
        """
        _, m = self._mat.shape
        return m - 1 # le -1 qui correspond à la dernière colonne des constantes
    ...
    def _preparer_coef(self,
            coef: int|float|Fraction|Number,
            en_tete: bool = False
        ) -> str:
        ...
        ... ])
        ...
        >>> piv1._preparer_coef(-3)
        ' - 3'
        >>> piv1._preparer_coef(-3, en_tete=True)
        '-3'
        >>> piv1._preparer_coef(1)
        ' + '
        >>> piv1._preparer_coef(1, en_tete=True)
        ''
        >>> piv1._preparer_coef(Fraction(-2, 3))
        ' - \frac{2}{3}'
        """
        if coef == 0:
            return ''
        ...
        for j in range(self._nb_var):
            ...
            if coef != 0:
                if res == '':
                    ...
                    en_tete = False
                coef = self._preparer_coef(coef, en_tete)
    ...
    def _normaliser_pivot(self, j) -> None:
        ...
        self._operations[-1][j] = self._noms_lignes[j] + r' \leftarrow ' \
                                + self._preparer_coef(inv_pivot, en_tete=True) \
                                + self._noms_lignes[j]
    ...
    def _creer_latex(self) -> str:
        ...
        # si on n'est pas dans Jypter Notebook exécuté dans Google Colab
        if not getenv("COLAB_RELEASE_TAG"):
            # on ajoute la balise \end{align}
            res += r"\end{align}"
        return res
