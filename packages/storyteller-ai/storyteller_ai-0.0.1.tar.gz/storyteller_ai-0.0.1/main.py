...
OPEN_DASHBOARD_APPS_IN_NEW_TAB = os.getenv(
    "OPEN_DASHBOARD_APPS_IN_NEW_TAB", False
)
...
templates_dir = os.path.join(
    os.path.dirname(__file__), "playground", "templates"
)
...
async def auth(request: Request):
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"OAuth error: {error.error}"
        )
    LOGGER.info(token)
