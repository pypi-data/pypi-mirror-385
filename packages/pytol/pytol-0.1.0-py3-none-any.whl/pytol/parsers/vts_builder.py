...
class Mission:
    ...
    def _generate_content_string(self):
        ...
        # Objectives
        objs_c = "".join([f"\t\tObjective\n\t\t{{\n\t\t\tobjectiveName = {o['name']}\n\t\t\tobjectiveInfo = {o['info']}\n\t\t\tobjectiveID = {o['id']}\n\t\t\trequired = {o['required']}\n\t\t\twaypoint = {o['waypoint']}\n\t\t\tautoSetWaypoint = {o['auto_set_waypoint']}\n\t\t\tstartMode = {'PreReqs' if o['prereqs'] else 'Immediate'}\n\t\t\tobjectiveType = {o['type']}\n\t\t\tpreReqObjectives = {_format_id_list(o['prereqs'])};\n{_format_block('fields', ''.join([f'\\t\\t\\t\\t{k} = {_format_value(v)}\\n' for k,v in o['fields'].items()]), 3)}\t\t}}\n" for o in self.objectives])
