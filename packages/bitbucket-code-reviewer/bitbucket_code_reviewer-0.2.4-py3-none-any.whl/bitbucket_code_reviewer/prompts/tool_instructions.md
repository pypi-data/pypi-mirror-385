...
### ✅ ALWAYS DO THIS:
1. **Identify 3-5 key files** from the diff
2. **For EACH file: use read_diff() FIRST** → See only what changed
3. **If needed: use read_file()** → Get broader context (rarely needed)
4. **⚠️ CRITICAL: FILTER OUT SEMANTIC DUPLICATES** → For each issue you plan to report, ask: "Would fixing the existing comment also fix this?" If YES, it's a duplicate - skip it! Check same code region (~5 lines), same root cause, not just exact line match.
