        ...
        for comment in comments:
            ...
            if path and file_changed:
                ...
                if anchor_snippet:
                    ...
                    if abs(anchor_to - s) > 20:
                        print(f"⏭️ Skipping comment (anchor line {anchor_to} is {abs(anchor_to - s)} lines away from requested line {s}): {path}")
                        continue
                else:
            ...
            try:
                ...
                if path and anchor_to is not None and not context_lines:
                    ...
                    print(f"⚠️ WARNING: Inline anchor failed (context_lines empty) for {path}:{anchor_to}")
                    print(f"   Comment was created but shows as general file comment, not inline")
                    print(f"   This usually means the line isn't in the visible diff or has conflicts")
