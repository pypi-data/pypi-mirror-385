...
from artmeta.utils import generate_yaml_template, get_stats, insert_in_tex
...
def test_get_stats_complete_metadata():
    """Test des statistiques avec métadonnées complètes"""
    meta = {
        "abstract": "This is a test abstract with multiple words in it.",
        "authors": [
            {"firstname": "John", "lastname": "Doe", "orcid": "0000-0002-1825-0097"},
            {"firstname": "Jane", "lastname": "Smith"},
        ],
        "affiliations": [
            {"id": 1, "name": "Lab A", "country": "France"},
            {"id": 2, "name": "Lab B", "country": "Germany"},
            {"id": 3, "name": "Lab C", "country": "France"},
        ],
        "keywords": ["test", "validation", "pytest"],
        "msc_codes": ["35K15", "65M12"],
    }

    stats = get_stats(meta)

    assert stats["abstract_words"] == 10
    assert stats["abstract_chars"] == 50
    assert stats["authors_count"] == 2
    assert stats["authors_with_orcid"] == 1
    assert stats["affiliations_count"] == 3
    assert stats["countries"] == ["France", "Germany"]
    assert stats["keywords_count"] == 3
