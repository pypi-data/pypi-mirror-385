...
from SharedData.Utils import datetype, normalize_date
from pymongo import ASCENDING, DESCENDING, UpdateOne, WriteConcern, ReadPreference
...
class CollectionMongoDB:
    ...
    def get_optimized_collection(self, operation_type='read', data_criticality='normal'):
        """
        Get a collection with optimized write concern and read preference based on operation type.
        
        Args:
            operation_type (str): 'read', 'write', or 'upsert'
            data_criticality (str): 'critical', 'normal', or 'high_frequency'
        
        Returns:
            pymongo.collection.Collection: Optimized collection instance
        """
        if operation_type == 'read':
            # Use secondary preferred for reads to distribute load
            return self._collection.with_options(
                read_preference=ReadPreference.SECONDARY_PREFERRED,
                read_concern=ReadConcern('local')
            )
        elif operation_type in ['write', 'upsert']:
            if data_criticality == 'critical':
                # Critical data - ensure durability
                write_concern = WriteConcern(w='majority', j=True)
            elif data_criticality == 'high_frequency':
            ...
            return self._collection.with_options(write_concern=write_concern)
        
        return self._collection    

    def upsert(self, data):
        """
        Perform optimized upsert operations on the collection, handling single or multiple documents.
        ...
        Key optimizations:
        - Deduplicates documents by primary key to reduce oplog entries
        - Only updates mtime if document is actually new or changed
        - Uses smaller batch sizes for better performance under load
        - Validates presence of primary key fields in each document
        - Removes MongoDB restricted fields like '_id'
        - Normalizes 'date' fields according to the collection's period setting
        ...
        # Deduplicate by primary key to reduce oplog entries
        grouped_data = {}
        missing_pkey_items = []
        ...
        for item in data:
            ...
            # Create primary key tuple for deduplication
            pkey_tuple = tuple(item.get(field, None) for field in self.pkey_columns)
            
            # Keep only the latest version of each document by primary key
            if all(val is not None for val in pkey_tuple):
                # Add modification time only if not present (avoid unnecessary updates)
                ...
                if 'date' in item and item['date'] is not None:
                    item = item.copy()
                    item['date'] = normalize_date(item['date'], self.period)

                # MongoDB does not allow '.' in field names, so replace them
                cleaned_item = {}
                for key, value in item.items():
                    clean_key = str(key).replace('.', '')
                    if clean_key == '':
                        clean_key = ' '
                    # Skip empty values to reduce document size
                    if not self.is_empty(value):
                        cleaned_item[clean_key] = value
                
                grouped_data[pkey_tuple] = cleaned_item
            else:
                missing_pkey_items.append(item)

        # Convert back to operations with deduplicated data
        operations = []
        for pkey_tuple, item in grouped_data.items():
            # Construct the filter condition using the primary key columns
            filter_condition = {field: val for field, val in zip(self.pkey_columns, pkey_tuple)}
        ...
        # Execute operations in smaller batches for better performance
        result = []
        batch_size = 500  # Smaller batches reduce lock time and memory usage
        
        if operations:
            for i in range(0, len(operations), batch_size):
                batch = operations[i:i + batch_size]
                try:
                    batch_result = self._collection.bulk_write(
                        batch, 
                        ordered=False,  # Allow parallel execution
                        bypass_document_validation=True  # Skip validation for performance
                    )
                    result.append(batch_result)
                except Exception as e:
                    Logger.log.error(f"Batch upsert failed for {self.relpath}: {str(e)}")
                    raise
    ...
    def extend(self, data):
        """
        Optimized append operation for collections without index, with performance enhancements.
        
        This method inserts new documents into a collection that does not have indexing enabled.
        The method processes input data by converting it to a standardized format, cleaning it,
        and inserting all documents in a single bulk operation.
        
        Key optimizations:
        - Uses optimized write concerns for high-frequency operations
        - Skips empty documents to reduce storage overhead
        - Uses unacknowledged writes for maximum throughput when appropriate
        - Better error handling and fallback mechanisms
        
        Parameters:
            data (dict, list of dict, or pandas.DataFrame): The documents to append.
        
        Processing:
            - Validates that the collection does not have an index.
            - Converts DataFrame to list of dictionaries if necessary.
            - Removes '_id' fields if present to avoid conflicts.
            - Adds modification time ('mtime') if not already present.
            - Normalizes 'date' fields based on the collection's period:
                - 'D1': floor to start of day.
                - 'M15': floor to nearest 15-minute interval.
                - 'M1': floor to nearest minute.
            - Cleans field names and removes empty values
            - Inserts all processed documents with optimized write concerns
        
        Returns:
            The result of the insert_many operation, or an empty list if no documents were inserted.
        """
        if self.hasindex:
            raise ValueError("Extend operation is not supported for collections with index. Use upsert instead.")
        ...
        for item in data:
            ...
            # Clean field names - MongoDB doesn't allow dots
            cleaned_item = {}
            for key, value in item.items():
                clean_key = str(key).replace('.', '')
                if clean_key == '':
            ...
            if cleaned_item:  # Only add non-empty documents
                documents_to_insert.append(cleaned_item)

        # Insert with optimized write concern for high throughput
        result = []
        if documents_to_insert:
            try:
                # Use high-frequency optimized collection for extend operations
                optimized_collection = self.get_optimized_collection('write', 'high_frequency')
                result = optimized_collection.insert_many(
                    documents_to_insert,
                    ordered=False,  # Allow parallel inserts
                    bypass_document_validation=True  # Skip validation for performance
                )
            except Exception as e:
                Logger.log.error(f"Bulk insert failed for {self.relpath}: {str(e)}")
                # Fallback to regular collection
                result = self._collection.insert_many(documents_to_insert, ordered=False)
        
        return result        
    
    def find(self, query, projection=None, sort=None, limit=None, skip=None, use_secondary=True):
        """
        Retrieve documents from the collection that match the given query criteria with optimized read routing.
        ...
        Args:
            ...
            use_secondary (bool): Whether to use secondary reads for better load distribution.
        
        Returns:
            list: A list of documents matching the query, optionally filtered, sorted, skipped, and limited.
        """
        # Use optimized collection for reads
        collection = self.get_optimized_collection('read') if use_secondary else self._collection
        ...
        # Add query hints for common patterns to optimize index usage
        if query and len(query) == 1:
            field_name = list(query.keys())[0]
            if field_name in self.pkey_columns and hasattr(self, 'pkey_columns'):
                # Hint to use primary key index for single field queries
                try:
                    pkey_index_name = '_'.join(f"{field}_1" for field in self.pkey_columns)
                    cursor = cursor.hint(pkey_index_name)
                except:
                    pass  # Ignore if index doesn't exist
            elif field_name == 'mtime':
                # Hint for time-based queries
                try:
                    cursor = cursor.hint([('mtime', -1)])
                except:
                    pass
    ...
    def is_empty(value):
        ...
        # Check type-specific empty values
        value_type = type(value)
        if value_type in CollectionMongoDB.EMPTY_VALUES:
            return value in CollectionMongoDB.EMPTY_VALUES[value_type]
