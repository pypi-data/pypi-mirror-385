...
def normalize_date(date, period):
    """
    Normalize a date to the start of its period boundary based on the period type.
    
    Parameters:
        date: A datetime object, pd.Timestamp, or np.datetime64 to normalize.
        period (str): Period identifier (e.g., 'D1', 'M15', 'M1', 'RT') used for flooring the date.
    
    Returns:
        datetime.datetime: The normalized date floored to the period boundary.
    
    Examples:
        - period='D1': Floors to start of day (00:00:00)
        - period='M15': Floors to nearest 15-minute interval
        - period='M1': Floors to nearest minute
        - period='RT': Floors to nearest second
    """
    if date is None:
        return None
    ...
    # Determine period length in nanoseconds
    if period == 'D1':
        # Floor to start of day
        return date.floor('D').to_pydatetime()
    elif period == 'M15':
        # Floor to 15-minute intervals
        return date.floor('15T').to_pydatetime()
    elif period == 'M1':
        # Floor to 1-minute intervals
        return date.floor('T').to_pydatetime()
    elif period == 'RT':
        # Floor to 1-second intervals
        return date.floor('S').to_pydatetime()
    else:
        # Default: no flooring
        return date.to_pydatetime() if isinstance(date, pd.Timestamp) else date
