...
def head_timeseries_source(database, period, source):
    ...
    This HEAD endpoint verifies the existence of a timeseries container for the specified
    database, period, and source without creating it. It returns metadata including
    available tags within the container.
    ...
    Returns:
    - 200 OK with JSON containing container metadata and list of available tags.
    - 401 Unauthorized if authentication fails.
    - 500 Internal Server Error with error details if the container doesn't exist or an exception occurs.
    """
    try:
        if not authenticate(request):
            return jsonify({'error': 'unauthorized'}), 401
        ...
        response_data = json.dumps({
            'status': 'success', 
            'message': 'Timeseries container exists',
            'tags': tags,
            'database': database,
            'period': period,
            'source': source,
