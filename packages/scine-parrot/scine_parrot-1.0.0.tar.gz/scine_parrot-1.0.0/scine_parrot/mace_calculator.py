...
class MaceCalculator(GFN2EnhancedCalculator):

    _mace_mp_large: Optional[mace_mp] = None
    _mace_mp_medium: Optional[mace_mp] = None
    _mace_mp_small: Optional[mace_mp] = None
    _mace_off_large: Optional[mace_off] = None
    _mace_off_medium: Optional[mace_off] = None
    _mace_off_small: Optional[mace_off] = None

    def __init__(self):
        super().__init__()
        torch.set_default_dtype(torch.float)
        self._settings['method'] = 'mace-mp_medium'
        self.mace: Union[mace_mp, mace_off]

    def __resolve_method(self) -> Union[mace_mp, mace_off]:
        if self._settings['method'] == 'mace-mp_large':
            if MaceCalculator._mace_mp_large is None:
                MaceCalculator._mace_mp_large = mace_mp(model='large', default_dtype='float32')
            return MaceCalculator._mace_mp_large
        if self._settings['method'] == 'mace-mp_medium':
            if MaceCalculator._mace_mp_medium is None:
                MaceCalculator._mace_mp_medium = mace_mp(model='medium', default_dtype='float32')
            return MaceCalculator._mace_mp_medium
        if self._settings['method'] == 'mace-mp_small':
            if MaceCalculator._mace_mp_small is None:
                MaceCalculator._mace_mp_small = mace_mp(model='small', default_dtype='float32')
            return MaceCalculator._mace_mp_small
        if self._settings['method'] == 'mace-off_large':
            if MaceCalculator._mace_off_large is None:
                MaceCalculator._mace_off_large = mace_off(model='large', default_dtype='float32')
            return MaceCalculator._mace_off_large
        if self._settings['method'] == 'mace-off_medium':
            if MaceCalculator._mace_off_medium is None:
                MaceCalculator._mace_off_medium = mace_off(model='medium', default_dtype='float32')
            return MaceCalculator._mace_off_medium
        if self._settings['method'] == 'mace-off_small':
            if MaceCalculator._mace_off_small is None:
                MaceCalculator._mace_off_small = mace_off(model='small', default_dtype='float32')
            return MaceCalculator._mace_off_small
        raise RuntimeError(
            f'Requested method {self._settings["method"]} is not available in Parrot::MaceCalculator'
        )
    ...
    def _calculate_impl(self, _: str = '') -> Results:
        ...
        self.mace = self.__resolve_method()
        ...
        if requires_energy and not (requires_gradients or requires_hessian or requires_thermo):
            self._results.energy = atoms.get_potential_energy() * utils.HARTREE_PER_EV
        ...
        self._results.description = 'MaceCalculation'
