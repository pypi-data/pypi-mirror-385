...
if DRAMATIQ_AVAILABLE:
    ...
    task_config = AdminConfig(
        ...
        display_fields=[
            FieldConfig(name="id", title="Task ID", ui_widget="badge", variant="info", icon=Icons.TAG),
            FieldConfig(name="actor_name", title="Actor", ui_widget="badge", variant="default", icon=Icons.FUNCTIONS, empty_value="Unknown"),
            FieldConfig(name="status", title="Status", ui_widget="badge", label_map={
                "pending": "warning", "running": "info", "done": "success",
                "failed": "danger", "cancelled": "secondary"
            }),
            FieldConfig(name="queue_name", title="Queue", ui_widget="badge", variant="info", icon=Icons.QUEUE, empty_value="default"),
            FieldConfig(name="created_at", title="Created", ui_widget="datetime_relative", ordering="created_at"),
        ],
        ...
        fieldsets=[
            FieldsetConfig(title="Task Information", fields=["id", "status", "actor_name", "queue_name"]),
        ...
        actions=[
            ActionConfig(name="retry_failed_tasks", description="Retry failed tasks", 
                        variant="warning", handler="django_cfg.apps.tasks.admin.actions.retry_failed_tasks"),
            ActionConfig(name="cancel_pending_tasks", description="Cancel pending tasks",
                        variant="danger", handler="django_cfg.apps.tasks.admin.actions.cancel_pending_tasks"),
        ],
        ordering=["-created_at"],
    )
