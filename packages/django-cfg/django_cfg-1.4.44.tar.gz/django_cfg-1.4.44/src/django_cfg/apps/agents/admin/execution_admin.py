...
from django_cfg.modules.django_admin.utils.displays import UserDisplay, DateTimeDisplay, MoneyDisplay
...
class AgentExecutionInlineForWorkflow(TabularInline):
    ...
    def status_badge_inline(self, obj):
        ...
        variant = status_config.custom_mappings.get(obj.status, 'secondary')

        config = StatusBadgeConfig(show_icons=True, icon=icon)
        return StatusBadge.create(
            text=obj.get_status_display() if hasattr(obj, 'get_status_display') else obj.status,
            variant=variant,
            config=config
        )
    ...
    def cost_display_inline(self, obj):
        ...
        if obj.cost:
            ...
            return MoneyDisplay.amount(obj.cost, config)
        return "—"
...
agent_execution_config = AdminConfig(
    ...
    list_filter=["status", "cached", "agent_name", "created_at"],
...
class WorkflowExecutionAdmin(PydanticAdmin):
    ...
    def pattern_display(self, obj: WorkflowExecution) -> str:
        ...
        config = StatusBadgeConfig(show_icons=True, icon=icon)
        return StatusBadge.create(
            text=obj.pattern.title() if obj.pattern else 'Unknown',
            variant=variant,
            config=config
        )
    ...
    def cost_display(self, obj: WorkflowExecution) -> str:
        ...
        if obj.total_cost:
            ...
            return MoneyDisplay.amount(obj.total_cost, config)
        return "—"
    ...
    cancel_running_workflows.short_description = "Cancel running workflows"
