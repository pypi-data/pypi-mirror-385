...
def _get_openapi_group_urls() -> List[URLPattern]:
    ...
    3. If the group has exactly 1 app with urls.py, include it
    4. Add path("{api_prefix}/{group_name}/", include("{app}.urls"))
    ...
    try:
        ...
        # Track already added groups to avoid duplicates
        ...
        for group_config in service.config.groups:
            ...
            # Only auto-include if the group has exactly 1 app
            if len(apps) != 1:
                continue
            ...
            try:
                ...
                # Add URL pattern for this group
                url_pattern = f"{api_prefix}/{group_name}/"
