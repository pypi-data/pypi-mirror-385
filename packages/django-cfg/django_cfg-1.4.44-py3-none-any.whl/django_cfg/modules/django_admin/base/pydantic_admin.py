...
class PydanticAdminMixin:
    ...
    def _build_from_config(cls):
        ...
        if config.list_display_links:
            cls.list_display_links = config.list_display_links
        ...
        # Form options
        if config.autocomplete_fields:
            cls.autocomplete_fields = config.autocomplete_fields
        if config.raw_id_fields:
            cls.raw_id_fields = config.raw_id_fields
        if config.prepopulated_fields:
            cls.prepopulated_fields = config.prepopulated_fields

        # Inlines
        if config.inlines:
            cls.inlines = config.inlines
    ...
    def _generate_display_method(cls, field_config):
        ...
        def display_method(self, obj):
            ...
            if value is None:
                return field_config.empty_value
    ...
    def _register_actions(cls, config: AdminConfig):
        """Register actions from ActionConfig."""
        action_functions = []
        ...
        for action_config in config.actions:
            ...
            # Set attributes
            handler.short_description = action_config.description

            # Store for later registration
            action_name = action_config.name
            setattr(cls, action_name, handler)
