...
class Scene3D:
    ...
    def __init__(self, width=800, height=600, title="3D Сцена", bg="black"): # <-- Теперь можно "black"
        ...
        # --- УПРОЩЕННАЯ НАСТРОЙКА OPENGL ---
        glViewport(0, 0, width, height)
        glMatrixMode(GL_PROJECTION)
        glLoadIdentity()
        gluPerspective(45, (width / height), 0.1, 5000.0) # Увеличим дальность
        glMatrixMode(GL_MODELVIEW)
        glLoadIdentity()

        # Включаем только самое необходимое
        glEnable(GL_DEPTH_TEST)  # Проверка глубины, чтобы объекты не просвечивали
        glEnable(GL_CULL_FACE)   # Отсечение задних граней для оптимизации

        # --- УДАЛЕНО ВСЕ, ЧТО СВЯЗАНО С GL_LIGHTING ---
