...
class FeatrixSphereClient:
    ...
    def __init__(self, base_url: str = "https://sphere-api.featrix.com", 
        ...
        # Compute cluster routing (for technical support use only)
    ...
    def set_compute_cluster(self, cluster_name: Optional[str] = None):
        """
        Set the target compute cluster for API requests.
        
        ⚠️  TECHNICAL SUPPORT USE ONLY ⚠️
        This function should only be used under guidance from Featrix technical support.
        Setting an incorrect compute cluster can cause unexpected behavior.
        
        Args:
            cluster_name: Name of the compute cluster (e.g., 'churro', 'burrito').
                         Pass None to clear the cluster setting and use default routing.
        
        Example:
            # Set compute cluster (only use with technical support guidance)
            client.set_compute_cluster('churro')
            
            # Clear compute cluster setting
            client.set_compute_cluster(None)
        """
        self._compute_cluster = cluster_name
        if cluster_name:
            print(f"⚠️  Compute cluster set to: {cluster_name}")
            print("   (This should only be used with technical support guidance)")
        else:
            print("✓ Compute cluster setting cleared (using default routing)")
    ...
    def _make_request(self, method: str, endpoint: str, max_retries: int = None, **kwargs) -> requests.Response:
        ...
        # Add X-Featrix-Node header if compute cluster is set
        if self._compute_cluster:
            if 'headers' not in kwargs:
                kwargs['headers'] = {}
            kwargs['headers']['X-Featrix-Node'] = self._compute_cluster
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if e.response is not None:
                    ...
                    # Retry on 503 Service Unavailable, 504 Gateway Timeout, or 500 with server restart patterns
                    if (status_code == 503 or status_code == 504 or (status_code == 500 and is_server_restarting)) and attempt < max_retries:
                        wait_time = self._calculate_backoff(attempt)
                        if status_code == 503:
                            print(f"503 Service Unavailable, retrying in {wait_time:.1f}s... (attempt {attempt + 1}/{max_retries + 1})")
                        elif status_code == 504:
                            print(f"504 Gateway Timeout, retrying in {wait_time:.1f}s... (attempt {attempt + 1}/{max_retries + 1})")
                        else:
    ...
    def _wait_with_notebook_display(self, session_id: str, max_wait_time: int, check_interval: int, show_live_training_movie: bool = False, training_interval_movie: int = 3) -> SessionInfo:
        ...
        try:
            ...
            while time.time() - start_time < max_wait_time:
                ...
                # Check completion - session must be in terminal state
                terminal_session_states = {'done', 'completed', 'ready', 'failed', 'cancelled'}
                if session_info.status in terminal_session_states:
                ...
                # Also check if all jobs are terminal AND session is in a ready state
                if session_info.jobs:
                    terminal_job_states = {'done', 'failed', 'cancelled'}
                    all_jobs_terminal = all(job.get('status') in terminal_job_states for job in session_info.jobs.values())
                    # Only return if jobs are terminal AND session is in a terminal/ready state
                    if all_jobs_terminal and session_info.status in terminal_session_states:
                        if show_live_training_movie and live_viz_state and live_viz_state['plot_initialized']:
                            print("🎬 Training completed - final visualization available via plot_training_movie()")
            ...
            with Progress(
                ...
                while time.time() - start_time < max_wait_time:
                    ...
                    if session_info.jobs:
                        terminal_job_states = {'done', 'failed', 'cancelled'}
                        all_jobs_terminal = all(job.get('status') in terminal_job_states for job in session_info.jobs.values())
                        # Only break if jobs are terminal AND session is in a terminal/ready state
                        if all_jobs_terminal and session_info.status in terminal_session_states:
                            progress.update(session_task, completed=100,
    ...
    def _resolve_predictor_id(self, session_id: str, predictor_id: str = None, target_column: str = None, debug: bool = False) -> Dict[str, Any]:
        ...
        if predictor_id:
            ...
            # Predictor ID not found - try to auto-recover
            all_predictor_ids = list(available_predictors.keys())
            
            # If there's only one predictor, auto-select it with a warning
            if len(available_predictors) == 1:
                new_predictor_id = all_predictor_ids[0]
                predictor_info = available_predictors[new_predictor_id]
                print(f"⚠️  Warning: Predictor ID '{predictor_id}' not found.")
                print(f"   Auto-selecting the only available predictor: '{new_predictor_id}'")
                print(f"   This likely happened because the predictor was retrained.")
                print(f"   Update your code to use the new predictor ID for future predictions.")
                return {
                    'target_column': predictor_info.get('target_column'),
                    'predictor_id': new_predictor_id,
                    'path': predictor_info.get('path'),
                    'type': predictor_info.get('type')
                }
            ...
            raise ValueError(
                ...
                f"This likely happened because the predictor was retrained, changing its ID.\n"
                f"Please update your code to use one of the available predictor IDs above."
            )
...
"""
    ...
    def similarity_search(self, session_id: str, query_record: Dict[str, Any], k: int = 5) -> Dict[str, Any]:
        ...
        Returns:
            ...
            - results: List of similar records with distances
            - stats: Query statistics (timing, distance metrics, search details)
        """
        data = {"query_record": query_record}
