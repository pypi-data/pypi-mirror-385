...
/*
 * texinfo: bap_set_tableof_polynom_mpzm
 * Assign @var{src} to @var{dst}.
 * Entries of @var{src} are allowed to be @code{BAP_NOT_A_POLYNOM_mpzm}.
 */

BAP_DLL void
bap_set_tableof_polynom_mpzm (
    struct bap_tableof_polynom_mpzm *dst,
    struct bap_tableof_polynom_mpzm *src)
{
  if (dst != src)
    {
      ba0_int_p i;
      ...
      for (i = 0; i < src->size; i++)
        {
          if (src->tab[i] == BAP_NOT_A_POLYNOM_mpzm)
            dst->tab[i] = BAP_NOT_A_POLYNOM_mpzm;
          else
            {
              if (dst->tab[i] == BAP_NOT_A_POLYNOM_mpzm)
                dst->tab[i] = bap_new_polynom_mpzm ();
              bap_set_polynom_mpzm (dst->tab[i], src->tab[i]);
            }
        }
      dst->size = src->size;
    }
...
/*
 * texinfo: bap_set_tableof_tableof_polynom_mpzm
 * Assign @var{src} to @var{dst}.
 * All tables are allowed to involve zero entries.
 */

BAP_DLL void
bap_set_tableof_tableof_polynom_mpzm (
    struct bap_tableof_tableof_polynom_mpzm *dst,
    struct bap_tableof_tableof_polynom_mpzm *src)
{
  if (dst != src)
    {
      ba0_int_p i;
      ...
      for (i = 0; i < src->size; i++)
        {
          if (src->tab[i] == (struct bap_tableof_polynom_mpzm *) 0)
            dst->tab[i] = (struct bap_tableof_polynom_mpzm *) 0;
          else
            {
              if (dst->tab[i] == (struct bap_tableof_polynom_mpzm *) 0)
                dst->tab[i] = (struct bap_tableof_polynom_mpzm *)
                    ba0_new_table ();
              bap_set_tableof_polynom_mpzm (dst->tab[i], src->tab[i]);
            }
        }
      dst->size = src->size;
    }
...
/*
 * texinfo: bap_mark_indets_tableof_polynom_mpzm
...
BAP_DLL void
bap_mark_indets_tableof_polynom_mpzm (
    struct bav_dictionary_variable *dict,
    struct bav_tableof_variable *vars,
    struct bap_tableof_polynom_mpzm *T)
{
  ...
  for (i = 0; i < T->size; i++)
    bap_mark_indets_polynom_mpzm (dict, vars, T->tab[i]);
}

/*
 * texinfo: bap_mark_indets_listof_polynom_mpzm
...
BAP_DLL void
bap_mark_indets_listof_polynom_mpzm (
    struct bav_dictionary_variable *dict,
    struct bav_tableof_variable *vars,
    struct bap_listof_polynom_mpzm *L)
{
  struct bap_listof_polynom_mpzm *M;

  for (M = L; M != (struct bap_listof_polynom_mpzm *) 0; M = M->next)
    bap_mark_indets_polynom_mpzm (dict, vars, M->value);
}
    ...
    struct bap_polynom_mpzm *A)
{
  ...
  bap_change_ordering_clot_mpzm (A->clot, bav_current_ordering ());
}
