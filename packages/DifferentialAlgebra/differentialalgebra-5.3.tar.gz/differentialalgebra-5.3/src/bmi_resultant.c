...
/*
 * EXPORTED
 * Resultant (list(ratfrac), regchain)
 */
    ...
    struct bmi_callback *callback)
{
  ...
  ba0_realloc2_table ((struct ba0_table *) &prod_den, ratfracs.size,
      (ba0_new_function *) & bap_new_product_mpz);
/*
 * The resultant of a fraction P/Q is defined as R/S where
 * - R is the resultant of P and
 * - S is the resultant of Q
 */
  for (i = 0; i < ratfracs.size; i++)
    {
      ...
      bad_resultant_mod_regchain (prod_num.tab[i], numer, &C);
      bad_resultant_mod_regchain (prod_den.tab[i], denom, &C);
