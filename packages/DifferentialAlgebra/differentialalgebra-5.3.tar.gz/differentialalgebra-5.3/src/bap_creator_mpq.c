...
{
  if (A == (struct bap_polynom_mpq *) 0 || A->readonly)
...
bap_write_all_creator_mpq (
    ...
    struct bap_polynom_mpq *A)
{
  bap_write_all_creator_clot_mpq (&crea->crea, A->clot, A->seq.first,
      A->seq.after);
}
...
bap_write_neg_all_creator_mpq (
    ...
    struct bap_polynom_mpq *A)
{
  bap_write_neg_all_creator_clot_mpq (&crea->crea, A->clot, A->seq.first,
      A->seq.after);
}
...
bap_write_mul_all_creator_mpq (
    ...
    ba0_mpq_t c)
{
  bap_write_mul_all_creator_clot_mpq (&crea->crea, A->clot, c, A->seq.first,
      A->seq.after);
}
...
bap_write_exquo_all_creator_mpz (
    ...
    ba0_mpz_t c)
{
  bap_write_exquo_all_creator_clot_mpq (&crea->crea, A->clot, c, A->seq.first,
      A->seq.after);
}
