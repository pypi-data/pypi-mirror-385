...
class AgentRuntimeServer:
            ...
            try:
                if inspect.isasyncgen(result):
                    ...
                    async for chunk in result:
                        yield self._serialize_chunk_to_sse(chunk)
    ...
    def _serialize_chunk_to_sse(chunk: Any) -> str:
        """
        Serialize chunk to SSE (Server-Sent Events) format
        
        Args:
            chunk: Data chunk of any type
                  - dict/list/str/int/float/bool/None: Direct JSON serialization
                  - Other objects: Convert to string first, then serialize
        
        Returns:
            str: SSE formatted data line, e.g. "data: {...}\n\n"
        
        Examples:
            >>> serialize_chunk_to_sse({"chunk": "Hello", "type": "content"})
            'data: {"chunk": "Hello", "type": "content"}\\n\\n'
            
            >>> serialize_chunk_to_sse("plain text")
            'data: "plain text"\\n\\n'
            
            >>> serialize_chunk_to_sse(42)
            'data: 42\\n\\n'
        """
        try:
