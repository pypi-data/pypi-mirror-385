...
class SandboxSession:
    ...
    async def invoke(
        ...
        try:
            ...
            # Fallback: Check if response body starts with "data: " (SSE format)
            # even if Content-Type is not set correctly
            if not is_streaming and response_text.startswith('data: '):
                logger.warning(f"Response appears to be SSE format (starts with 'data: ') but Content-Type is '{content_type}'")
                logger.warning("Treating as streaming response (fallback detection)")
                is_streaming = True
            ...
            if is_streaming:
                ...
                logger.debug('Detected streaming response, will handle as SSE')
    ...
    async def _handle_streaming_response_from_text(self, response_text: str):
        ...
        for line in response_text.split('\n'):
            ...
            if line.startswith("data: "):
                ...
                if data.strip():
                    chunks_found += 1
                    logger.debug(f"Yielding chunk #{chunks_found}: {data[:100]}...")
                    yield data
            elif line.strip() and not line.startswith(":"):  # Not empty line or comment
                # Plain text chunk (not SSE format)
                chunks_found += 1
                logger.debug(f"Yielding plain text chunk #{chunks_found}: {line[:100]}...")
                yield line
