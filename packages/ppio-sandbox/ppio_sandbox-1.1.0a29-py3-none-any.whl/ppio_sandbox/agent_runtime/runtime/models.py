...
class InvocationRequest(BaseModel):
    """Invocation request model (for reference only)
    
    Note: This model is NOT used for request validation in the runtime server.
    The server accepts any valid JSON and passes it directly to the entrypoint function.
    This model is kept for:
    - Documentation purposes
    - Optional client-side validation
    - Backward compatibility
    
    Users can send any arbitrary fields in their requests, and they will be preserved.
    """
    prompt: Optional[str] = None
    data: Optional[Dict[str, Any]] = None
    sandbox_id: Optional[str] = None  # Optional, extracted to context if present
    timeout: Optional[int] = None  # Not used by runtime server
