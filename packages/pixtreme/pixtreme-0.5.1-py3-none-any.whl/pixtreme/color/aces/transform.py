...
def rec709_to_aces2065_1(image: cp.ndarray | np.ndarray, tonemap: bool = True) -> cp.ndarray | np.ndarray:
    ...
    tonemap : bool, optional
        Deprecated parameter, kept for compatibility. Will be removed in future version.
        The function now always uses the correct Rec.709 inverse OETF.
    ...
    """
    if tonemap is not True:
        warnings.warn(
            "The 'tonemap' parameter is deprecated and will be removed in a future version. "
            "The function now always uses the correct Rec.709 Inverse OETF transform.",
            DeprecationWarning,
            stacklevel=2
        )
...
def aces2065_1_to_rec709(image: cp.ndarray | np.ndarray, tonemap: bool = True) -> cp.ndarray | np.ndarray:
    ...
    tonemap : bool, optional
        Deprecated parameter, kept for compatibility. Will be removed in future version.
        The function now always uses the correct Rec.709 OETF.
    ...
    if tonemap is not True:
        warnings.warn(
            "The 'tonemap' parameter is deprecated and will be removed in a future version. "
            "The function now always uses the correct Rec.709 OETF transform.",
            DeprecationWarning,
            stacklevel=2
        )
