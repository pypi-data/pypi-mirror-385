  ...
  public async continueMessage(cell_context?: string): Promise<void> {
    ...
    try {
      ...
      if (currentThread && currentThread.needsContinue) {
        ...
        console.log('[ChatInputManager] Set needsContinue to false after continueMessage finished');
      }
    }
  }
    ...
    this.sendButton.addEventListener('click', (event) => {
      ...
      if (this.isProcessingMessage) {
        ...
        if (this.cancelMessage) {
          ...
          console.warn('[ChatInputManager] cancelMessage callback is not defined! Using fallback...');
          const chatWidget = AppStateService.getChatContainerSafe()?.chatWidget;
          if (chatWidget) {
            console.log('[ChatInputManager] Calling cancelMessage via AppState fallback');
            chatWidget.cancelMessage();
          } else {
            console.error('[ChatInputManager] CRITICAL: Could not find chat widget to cancel message');
          }
        }
