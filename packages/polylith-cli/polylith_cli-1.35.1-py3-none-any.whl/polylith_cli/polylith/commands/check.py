...
def run_each(root: Path, ns: str, project_data: dict, options: dict) -> Tuple[bool, dict]:
    is_quiet = options['quiet']
    is_strict = options['strict']
    ...
    collected_libs = distributions.known_aliases_and_sub_dependencies(deps, alias, options, from_lock_file)
    details = check.report.create_report(project_data, collected_imports, collected_libs, is_strict or from_lock_file)
