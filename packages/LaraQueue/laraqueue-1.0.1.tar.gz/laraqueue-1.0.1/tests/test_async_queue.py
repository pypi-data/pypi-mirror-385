...
class TestAsyncQueue:
    ...
    async def test_async_metrics_collection(self, async_queue):
        ...
        start_time = async_queue.metrics.record_job_start('TestJob')
        await asyncio.sleep(0.01)  # Небольшая задержка
        async_queue.metrics.record_job_success('TestJob', start_time, 0.01)
        
        metrics = async_queue.metrics.get_metrics()
    ...
    async def test_async_metrics_reset(self, async_queue):
        ...
        start_time = async_queue.metrics.record_job_start('TestJob')
        async_queue.metrics.record_job_success('TestJob', start_time, 0.1)
...
class TestAsyncMetricsCollector:
    ...
    def test_async_metrics_job_recording(self):
        """Тест записи метрик задач."""
        metrics = AsyncMetricsCollector()
        
        # Записываем успешную задачу
        start_time = metrics.record_job_start('TestJob')
        metrics.record_job_success('TestJob', start_time, 0.5)
    ...
    def test_async_metrics_failure_recording(self):
        ...
        start_time = metrics.record_job_start('TestJob')
        error = ValueError("Test error")
        metrics.record_job_failure('TestJob', start_time, 0.3, error)
    ...
    def test_async_metrics_retry_recording(self):
        """Тест записи метрик retry."""
        metrics = AsyncMetricsCollector()
        
        # Записываем retry
        metrics.record_retry('TestJob')
        
        # Проверяем метрики
        assert metrics.total_retries == 1
    
    def test_async_metrics_get_metrics(self):
        """Тест получения полных метрик."""
        metrics = AsyncMetricsCollector()
        
        # Добавляем различные метрики
        start_time = metrics.record_job_start('TestJob1')
        metrics.record_job_success('TestJob1', start_time, 0.5)
        
        start_time = metrics.record_job_start('TestJob2')
        error = ValueError("Test error")
        metrics.record_job_failure('TestJob2', start_time, 0.3, error)
        
        metrics.record_retry('TestJob1')
        
        # Получаем полные метрики
        full_metrics = metrics.get_metrics()
    ...
    def test_async_metrics_reset(self):
        """Тест сброса метрик."""
        metrics = AsyncMetricsCollector()
    ...
    def test_async_metrics_recent_jobs(self):
        """Тест получения последних задач."""
        metrics = AsyncMetricsCollector()
        
        # Добавляем несколько задач
        for i in range(5):
            start_time = metrics.record_job_start(f'TestJob{i}')
            metrics.record_job_success(f'TestJob{i}', start_time, 0.1 * i)
        
        # Получаем последние задачи
        recent_jobs = metrics.get_recent_jobs(limit=3)
