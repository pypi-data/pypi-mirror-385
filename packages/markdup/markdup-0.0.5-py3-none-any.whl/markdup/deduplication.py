...
class BAMReader:
    ...
    def get_windows(self) -> Iterator[Dict[str, Any]]:
        """Get all windows for processing using sequential approach."""
                ...
                for chromosome in all_chromosomes:
                    ...
                    if contig_len <= self.window_size:
                        ...
                        for i in range(0, contig_len, self.window_size):
                            ...
                            has_reads = self._check_region_has_reads(
    ...
    def cleanup(self):
        ...
        if self.temp_dir and os.path.exists(self.temp_dir):
            ...
            try:
                ...
                print(f"⚠️  Warning: Could not remove temp directory {self.temp_dir}: {e}")
                ...
                try:
                    ...
                    logger.warning(f"Failed to cleanup individual temp files: {cleanup_error}")
...
def process_window(window_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"Error processing window {window_data.get('window_id', 'unknown')}: {e}")
        return {
            "window_id": window_data.get("window_id", "unknown"),
            "error": str(e),
            "success": False,
...
class UnifiedProcessor:
    ...
    def __init__(
        ...
        # Auto-detect UMI method only if --auto is specified
        if auto and method == "umi":
            self.method, self.umi_tag = self._detect_umi_method(umi_sep, umi_tag)
        elif method == "umi":
            # Use specified UMI method without auto-detection
            self.method = method
            self.umi_tag = umi_tag
        else:
            self.method = method
    ...
    def _detect_umi_method(self, umi_sep: str, umi_tag: str) -> tuple[str, str]:
        ...
        try:
            with pysam.AlignmentFile(self.bam_file, "rb") as f:
                ...
                # Decision logic
                # If umi_sep is specified but umi_tag is not, prioritize query name extraction
                if umi_sep and not umi_tag:
                    print(f"🔍 Using UMI extraction from query names (separator: '{umi_sep}')")
                    return "umi", None  # Use query name extraction
                # If umi_tag is specified but umi_sep is not, prioritize tag extraction
                if umi_tag and not umi_sep and umi_in_tag_count > 0:
                    print(f"🔍 Using UMI extraction from BAM tags (tag: '{umi_tag}')")
                    return "umi", umi_tag  # Use tag extraction
                # Auto-detection logic (when both or neither are specified)
            ...
            logger.warning(f"Error detecting UMI: {e}, using coordinate-based deduplication")
        ...
        for read in reads_at_position:
            if not read.is_paired and read.query_name not in processed_reads:
                ...
                fragment = Fragment(
                    ...
                    read1=read,
                    umi=extract_umi(read, umi_tag, umi_sep),
                )
        ...
        for read in reads_at_position:
            if not read.is_paired and read.query_name not in processed_reads:
                ...
                fragment = Fragment(
                    query_name=read.query_name,
                    read1=read,
                    umi=None,  # No UMI needed for coordinate-based deduplication
                )
                fragments.append(fragment)
                processed_reads.add(read.query_name)
                single_end += 1
...
def process_bam(
    ...
    Args:
        ...
        remove_unpaired: Remove unpaired reads (default: False)
        remove_chimeric: Remove chimeric reads (default: False)
        remove_unmapped: Remove unmapped reads (default: False)
        prefilter: Enable pre-filtering for empty windows (default: False)
    ...
    if input_bam.endswith('.sam'):
        ...
        try:
            ...
            with pysam.AlignmentFile(input_bam, 'r') as sam_file:
                with pysam.AlignmentFile(temp_bam_file, 'wb', header=sam_file.header) as bam_file:
                    for read in sam_file:
                        bam_file.write(read)
            
            # Create index for the temporary BAM file
            pysam.index(temp_bam_file)
    ...
    if method not in valid_methods:
        print(f"❌ Invalid method '{method}'. Valid methods are: {', '.join(valid_methods)}")
        return False
    
    # Validate quality criteria
    valid_quality_criteria = ["mapq", "avg_base_q"]
    if best_read_by not in valid_quality_criteria:
        print(f"❌ Invalid quality criteria '{best_read_by}'. Valid criteria are: {', '.join(valid_quality_criteria)}")
        return False
    ...
    if not 0 <= min_edit_dist_frac <= 1:
        print(f"❌ Min edit distance fraction must be between 0 and 1, got: {min_edit_dist_frac}")
        return False
    
    if not 0 <= min_frequency_ratio <= 1:
        print(f"❌ Min frequency ratio must be between 0 and 1, got: {min_frequency_ratio}")
        return False
