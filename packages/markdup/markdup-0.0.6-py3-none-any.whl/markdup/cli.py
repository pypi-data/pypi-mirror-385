...
def validate_bam_file(bam_file: str, threads: int = 1) -> bool:
    ...
    try:
        ...
        if bam_file.endswith('.sam'):
            console.print("📝 SAM file detected - will be converted to BAM for processing")
...
[green]markdup input.bam output.bam --method umi --umi-sep _ --min-edit-dist-frac 0.15[/green]

[dim]# UMI-based deduplication (extract from BAM tags)[/dim]
[green]markdup input.bam output.bam --method umi --umi-tag UB[/green]

[dim]# Coordinate-based deduplication with filtering[/dim]
[green]markdup input.bam output.bam --method coordinate --remove-unmapped --remove-chimeric[/green]
...
@click.option(
    ...
    default="umi",
    show_default=True,
    help="[bold]Deduplication method[/bold]\n\n[dim]• [bold]umi[/bold]: Use UMI for deduplication (extracts from read names by default, or BAM tags if --umi-tag specified)\n• [bold]coordinate[/bold]: Use alignment coordinates for deduplication[/dim]",
)
@click.option(
    "--umi-tag",
    default=None,
...
def main(
    ...
    if max_pair_dist <= 0:
        console.print(f"[red]❌ Max pair distance must be positive, got: {max_pair_dist}[/red]")
        return
    
    if not 0 <= min_edit_dist_frac <= 1:
        console.print(f"[red]❌ Min edit distance fraction must be between 0 and 1, got: {min_edit_dist_frac}[/red]")
        return
    
    if not 0 <= min_frequency_ratio <= 1:
        console.print(f"[red]❌ Min frequency ratio must be between 0 and 1, got: {min_frequency_ratio}[/red]")
        return
