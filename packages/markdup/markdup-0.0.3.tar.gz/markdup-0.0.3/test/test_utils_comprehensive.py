...
class TestFragmentComprehensive:
    ...
    def test_fragment_get_fragment_position(self):
        ...
        assert start == 1000
        assert end == 2100
        assert strand == 0  # Forward strand
    ...
    def test_fragment_get_cluster_name(self):
        ...
        fragment = Fragment(query_name="read1_UMI123", read1=read1)
        fragment.umi = "UMI123"
    ...
    def test_fragment_is_in_window(self):
        ...
        assert fragment.is_in_window(500, 1500) == True
        # Fragment is not in window
        assert fragment.is_in_window(2000, 3000) == False
        # Fragment partially overlaps
        assert fragment.is_in_window(1050, 1200) == True
    ...
    def test_fragment_is_in_overlap_region(self):
        ...
        assert fragment.is_in_overlap_region(1000, 2000) == False
        ...
        # Fragment is in overlap region (window_end=2000, max_pair_dist=1000)
        assert fragment.is_in_overlap_region(2000, 1000) == True
        # Fragment is not in overlap region
        assert fragment.is_in_overlap_region(500, 1000) == False
    ...
    def test_fragment_is_secondary(self):
        ...
        assert fragment.is_secondary() == False
        
        # Secondary read
        read1.is_secondary = True
        assert fragment.is_secondary() == True
...
class TestUtilityFunctionsComprehensive:
    ...
    def test_should_keep_read_comprehensive(self):
        """Test read filtering logic comprehensively."""
        read = Mock()
        read.is_unmapped = True
        read.is_supplementary = False
        read.is_secondary = False
        
        # Should keep unmapped reads by default
        assert should_keep_read(read, False, False, False) == True
        # Should remove unmapped reads when requested
        assert should_keep_read(read, False, False, True) == False
        
        # Test supplementary reads
        read.is_unmapped = False
        read.is_supplementary = True
        
        assert should_keep_read(read, False, False, False) == True
        assert should_keep_read(read, False, True, False) == False
        
        # Test secondary reads (not currently filtered by should_keep_read)
        read.is_supplementary = False
        read.is_secondary = True
    
        assert should_keep_read(read, False, False, False) == True
        # Note: should_keep_read doesn't currently filter secondary reads
        assert should_keep_read(read, True, False, False) == True
    
    def test_extract_umi_from_query_name_comprehensive(self):
        """Test UMI extraction comprehensively."""
        # Test with underscore separator
        assert extract_umi_from_query_name("read_UMI123", "_") == "UMI123"
        assert extract_umi_from_query_name("read_UMI123_extra", "_") == "extra"
        assert extract_umi_from_query_name("read_UMI123_extra_more", "_") == "more"
        
        # Test with colon separator
        assert extract_umi_from_query_name("read:UMI456", ":") == "UMI456"
        assert extract_umi_from_query_name("read:UMI456:extra", ":") == "extra"
        
        # Test with no separator
        assert extract_umi_from_query_name("read", "_") == "read"
        assert extract_umi_from_query_name("read", ":") == "read"
        
        # Test empty string
        assert extract_umi_from_query_name("", "_") == ""
        
        # Test separator at end
        assert extract_umi_from_query_name("read_", "_") == ""
        
        # Test separator at beginning
        assert extract_umi_from_query_name("_UMI1", "_") == "UMI1"
...
class TestEdgeCasesComprehensive:
    ...
    def test_calculate_average_base_quality_edge_cases(self):
        ...
        assert abs(calculate_average_base_quality([-10, 10, 20]) - 6.67) < 0.01  # Approximate
    ...
    def test_extract_umi_from_query_name_edge_cases(self):
        ...
        # Multiple separators
        assert extract_umi_from_query_name("read_UMI1_extra_more", "_") == "more"
        ...
        # Only separators
        assert extract_umi_from_query_name("___", "_") == ""
        
        # Mixed separators
        assert extract_umi_from_query_name("read_UMI1:extra", "_") == "UMI1:extra"
