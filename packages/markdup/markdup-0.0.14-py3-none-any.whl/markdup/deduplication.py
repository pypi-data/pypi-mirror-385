...
class ThreadSafeWriter:
    ...
    def _setup_temp_files(self):
        ...
        self.temp_dir = tempfile.mkdtemp(prefix="markdup_threads_")
...
def deduplicate_fragments_by_umi(
    ...
    for fragment in fragments:
        if start_only:
            # For all fragments: use biological start position based on strand
            ...
            # For all fragments: use biological end position
            if fragment.read1.is_reverse:
...
def deduplicate_fragments_by_coordinate(
    ...
    for fragment in fragments:
        ...
        if start_only:
            # For all fragments: use biological start position
            if fragment.read1.is_reverse:
                # Reverse strand: biological start is the 3' end
                bio_start = fragment.read1.reference_end
            else:
...
def process_bam(
    ...
    if not 0 <= max_dist_frac <= 1:
        print(
            f"❌ Max edit distance fraction must be between 0 and 1, got: {max_dist_frac}"
        )
        return False

    if not 0 <= max_frequency_ratio <= 1:
        print(
            f"❌ Max frequency ratio must be between 0 and 1, got: {max_frequency_ratio}"
        )
        return False
