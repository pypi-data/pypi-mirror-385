...
def test_group_by_common_query():
    ...
    ontology_api_name = 'ApiName.ontology_new.7a4569fa-cbfb-4cbf-ae8f-f343560a65c5'
...
def test_common_count_query():
    ...
    """
    api_name = 'agg'
    ontology_api_name = 'ApiName.ontology_new.dea3d732-f033-45c2-9439-995d1656cdef'
    client = TGClient(api_name, ontology_api_name)
    agg = client.ontology.objects._model_classes.get(api_name)
    results = client.ontology.objects.agg.where().limit(10).offset(2).count_all()
    #results = client.ontology_new.objects.eq_vessel.where(eq_vessel.id<10).group_by(eq_vessel.ship_type).count("ship_type").sum("ship_type").having("count >2 and sum > 17").compute()
    print(results)
...
def test_join_common_query():
    ...
    ontology_api_name = 'ApiName.ontology_new.a71d3e1a-30fc-42fb-830f-24d364641c44'
    client = TGClient(api_name, ontology_api_name)
    ...
    #results = client.ontology_new.objects.sit_ship_deployment_del.where().all()
    print(results)
...
def test_update():
    ...
    # 先创建测试数据
    # test_item = client.ontology_new.objects.agg.create(
    #             clicks = 1
    # )
    # print(test_item)
...
def test_create_dm():
        ...
        st_data_list = []
        st_data_list.append({"clicks" : 101})
        st_data_list.append({"clicks" : 102})
        st_data_list.append({"clicks" : 103})
        st_data_list.append({"clicks" : 104})
        ...
        # )
        # print(test_item)
        # new_item = client.ontology_new.objects.agg.create(
        #     ad_id=101,
        #     clicks = 101
        # )
        # print(f"创建成功: RID={new_item.ad_id}")
...
def test_create_generator_id():
        ...
        print("\n测试创建记录:")
        api_name = 'xinjiana'
        ontology_api_name = 'ApiName.ontology_new.81b53fa5-d7f9-4a21-bb49-bcc078b1ccf8'
        client = TGClient(api_name,ontology_api_name)
...
#
#
#     new_item = client.ontology_new.objects.OntologyModelWrite.create(
#         rid="test_009",
#         api_name="test_api9",
#         display_name="测试模型39",
#         create_uid="tester9"
...
#
#     # 执行删除
#     deleted_count = client.ontology_new.objects.OntologyModelWrite.where(
...
#
#     # 测试批量更新功能
#     update_count = client.ontology_new.objects.OntologyModelWrite.where(
#         OntologyModelWrite.display_name == "批量模型模型"
