        ...
        class _HandoffTool(Tool):
            ...
            def run(inner_self, **kwargs) -> str:
                ...
                hid = HandoffInputData(
                    user_message=(kwargs.get("message") or "Please take over from here."),
                ...
                # Get the actual user message from kwargs or memory
                user_msg = kwargs.get("message") or current_agent.memory.load_last_user_message() or "Please take over from here."

                # Build final message for the next agent
                if input_obj is not None:
                    takeover_message = f"{user_msg}\n\nAdditional data:\n{input_obj.model_dump_json(indent=2)}"
                else:
                    takeover_message = user_msg

                # Run target agent
                result = self.agent.run(takeover_message)
