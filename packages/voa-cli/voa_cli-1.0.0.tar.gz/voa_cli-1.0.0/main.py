...
W E L C O M E   {usnm}   T O   V O A
    ...
    click.echo(click.style(f"DateTime : {current_time}", fg="magenta"))
    click.echo(click.style("Project  : VOA Internal Secret Manager CLI", fg="blue"))
    click.echo(click.style("Version  : 1.0.0\n\n", fg="green"))
    click.echo(click.style("Type 'python main.py --help' to view all available commands.\n\n", fg="bright_white", bold=True))
...
def list():
    ...
    if response.status_code == 200:
        ...
        for secret in secrets:
            click.echo(click.style(f"Secret {secret['id']} - Name : {secret['name']} - Owner ID : {secret['owner_id']}",fg="green"))
    else: click.echo(response.text)
