...
from pipe.src.future_sales.feature_engineering.feature_importance_evaluator import FeatureImportanceEvaluator
...
class ModelInterpreter:
    ...
    def _plot_error_distribution(self, df):
        ...
        if self.run:
            self.run["error_analysis/histogram"].upload("error_histogram.png")
    ...
    def _plot_dynamic_vs_error(self, df):
        ...
        plt.tight_layout()
        plt.savefig("dynamic_vs_error.png")
        self.log("Saved dynamic vs error plot.")

        if self.run:
            self.run["error_analysis/dynamic_vs_error"].upload("dynamic_vs_error.png")
    ...
    def _explain_shap(self, X):
        try:
            ...
            if self.run:
                self.run["shap/beeswarm"].upload("shap_beeswarm.png")
    ...
    def _explain_lstm(self, X):
        try:
            ...
            plt.tight_layout()
            plt.savefig("shap_lstm_summary.png")
            self.log("Saved LSTM SHAP summary.")

            if self.run:
                self.run["shap/lstm_summary"].upload("shap_lstm_summary.png")
        except Exception as e:
