...
pub struct XacroProcessor {
    ...
    // Cache for macro expansion results: (macro_name, parameters) -> expanded nodes
    macro_cache: HashMap<(String, BTreeMap<String, String>), Vec<XMLNode>>,
}
...
impl XacroProcessor {
    ...
    fn eval_all(&mut self, element: &mut Element) -> Result<()> {
        ...
        if self.verbosity > 1 {
            eprintln!("[TIMING] Phase1 (includes+macros): {:?}", phase1_elapsed);
            eprintln!("[TIMING] Phase2 (expansion):       {:?}", phase2_elapsed);
        }
                    ...
                    if !is_builtin_xacro && self.macros.contains(element_local_name) {
                        ...
                        match element_local_name {
                            ...
                            "if" => {
                                ...
                                if keep {
                                    // Add if's children to new_children
                            ...
                            "unless" => {
                                ...
                                if keep {
                                    // Add unless's children to new_children
                            ...
                            _ => {
                                // Regular element - process recursively and add
    ...
    fn expand_macro(
        ...
        // Create cache key from macro name and parameters
        let cache_key = (
