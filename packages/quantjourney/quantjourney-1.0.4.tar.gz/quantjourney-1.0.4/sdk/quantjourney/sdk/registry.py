...
def _build_from_yaml() -> Dict[str, str]:
    ...
    for key, enabled in routers_map.items():
        if not enabled:
            continue
        # Non-connector routers live at top-level in the SDK for clarity
            mod = f"quantjourney.sdk.{key}"
        else:
...
_default = {
    ...
    "insider": "quantjourney.sdk.insider:INSIDEREndpoint",
    "bt": "quantjourney.sdk.bt:BTEndpoint",
    "analytics": "quantjourney.sdk.analytics:ANALYTICSEndpoint",
    "universe": "quantjourney.sdk.universe:UNIVERSEEndpoint",
}
