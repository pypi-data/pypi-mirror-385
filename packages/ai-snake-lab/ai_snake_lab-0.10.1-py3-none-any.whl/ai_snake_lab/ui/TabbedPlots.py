...
class TabbedPlots(Widget):
    ...
    game_score_epoch = deque(maxlen=Plot.MAX_DATA_POINTS)
    game_score_score = deque(maxlen=Plot.MAX_DATA_POINTS)
    highscores_game = []
    highscores_score = []
    loss = []
    loss_epoch = []
    ...
    def clear_data(self):
        self.game_score_epoch = deque(maxlen=Plot.MAX_DATA_POINTS)
        self.game_score_score = deque(maxlen=Plot.MAX_DATA_POINTS)
    ...
    def add_highscore_data(self, epoch, score):
        highscores = self.query_one(f"#{DLayout.HIGHSCORES_PLOT}", PlotWidget)
    ...
    def add_loss_data(self, epoch, loss):
