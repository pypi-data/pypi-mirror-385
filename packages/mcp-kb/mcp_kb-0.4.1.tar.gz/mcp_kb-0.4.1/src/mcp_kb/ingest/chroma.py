...
class ChromaIngestor(KnowledgeBaseListener, KnowledgeBaseReindexListener):
    ...
    def _find_orphaned_documents(self,kb: "KnowledgeBase",remove: bool = True) -> Set[str]:
        ...
        try:
            ...
            with tqdm(total=count, desc="Finding orphaned documents") as pbar:
                for i in range(0, count, self.configuration.max_batch_size):
                    batch = self._collection.get(include=["metadatas"], limit=self.configuration.max_batch_size, offset=i)
                    ids=batch.get("ids", [])
                    for ids,metadata in zip(ids, batch.get("metadatas", [])):
    ...
    def _reindex_document(
        ...
        for i in range(0, len(contents), self.configuration.max_batch_size):
            batch = contents[i:i+self.configuration.max_batch_size]
            batch_metadatas = metadatas[i:i+self.configuration.max_batch_size]
            batch_ids = ids[i:i+self.configuration.max_batch_size]
        ...
        for i in range(0, len(metadatas), self.configuration.max_batch_size):
            batch_metadatas = metadatas[i:i+self.configuration.max_batch_size]
            batch_ids = ids[i:i+self.configuration.max_batch_size]
