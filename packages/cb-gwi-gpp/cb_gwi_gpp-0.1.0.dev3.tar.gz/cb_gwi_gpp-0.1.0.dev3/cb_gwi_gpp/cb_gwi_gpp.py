...
class CBGwiGpp():
    ...
    def get_instrument_list(self, print_list: bool = True):
    ...
    def get_gwi_gpp_resource(self, inst_id: str):
        for resource in self.get_instrument_list(print_list=False):
            instr = pyvisa.ResourceManager().open_resource(
                resource,
                baud_rate=115200, timeout=1,
                read_termination='\n', write_termination='\n')

            current_id = instr.query('*IDN?', delay=0.1).strip()
            if inst_id in current_id:
                return resource
        return None
    ...
    def open(self, resource: str = 'ASRL/dev/ttyUSB0::INSTR'):
        ...
        if self.instr is None:
            raise ValueError(f'Could not open instrument at {resource}')

        inst_id = self.instr.query('*IDN?', delay=0.1).strip()
        if self.verbose:
            log.info(f"Let's see Instrument ID: {resource}, {inst_id}")
