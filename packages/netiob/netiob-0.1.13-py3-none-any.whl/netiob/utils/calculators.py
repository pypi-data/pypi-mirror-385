    ...
    def process_time(time):
        ...
        for _, row in last_xhr_records.iterrows():
            ...
            if row['FATEST'] == 'BASAL INSULIN' and row['INSSTYPE'] in basal_ins_subtypes:
                ...
                duration = float(duration.iloc[0]) if not duration.empty else 0.0
                duration = 0.0 if np.isnan(duration) else duration

                # <<< NEW: only add temp basal if commanded deviates from base >>>
                base = float(row.get('base_basal_rate', 0.0))
                commanded = float(row.get('commanded_basal_rate', base))
                if duration > 0 and abs(commanded - base) > 1e-6:
                    pumphistory_data.append(
                        {"timestamp": timestamp, "_type": "TempBasalDuration", "duration (min)": duration})
                    pumphistory_data.append({"timestamp": timestamp, "_type": "TempBasal",
                                             "temp": "absolute", "rate": commanded})

            elif row['FATEST'] == 'BOLUS INSULIN' and row['INSSTYPE'] in bolus_ins_subtypes:
                if row['INSSTYPE'] == 'normal':
                    amt = float(row['INSNMBOL']) if pd.notna(row['INSNMBOL']) else 0.0
                    if amt > 0:
                        pumphistory_data.append({"timestamp": timestamp, "_type": "Bolus",
                                                 "amount": round(amt, 3), "programmed": round(amt, 3),
                                                 "unabsorbed": 0, "duration": 0})
                else:  # extended
                    # If you truly need to model immediate + extended, add both **non-zero** parts explicitly.
                    ext = float(row['INSEXBOL']) if pd.notna(row['INSEXBOL']) else 0.0
                    if ext > 0:
                        pumphistory_data.append({"timestamp": timestamp, "_type": "Bolus",
                                                 "amount": round(ext, 3), "programmed": round(ext, 3),
                                                 "unabsorbed": 0, "duration": 0})
