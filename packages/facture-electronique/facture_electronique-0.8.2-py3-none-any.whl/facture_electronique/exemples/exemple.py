...
if __name__ == "__main__":
    ...
    if recherche_structure and recherche_structure["parametresRetour"]["total"] == 1:
        ...
        logger.debug(
            msg=f"ID Chorus Pro trouvé pour le SIRET {fournisseur_siret}: {identifiant_cpro}"
        )
    else:
        identifiant_cpro = 26300989  # Fallback pour l'exemple
        logger.debug(
            msg=f"Aucun ID Chorus Pro trouvé, utilisation de la valeur par défaut: {identifiant_cpro}"
        )
    ...
    # logger.debug(msg=exemple_facture_mode_api.to_api_payload())
    try:
        res = c.envoyer_facture(exemple_facture_mode_api.to_api_payload())
        logger.debug(msg=f"  -> Réponse de Chorus Pro: {res}")
    except ErreurConfiguration:
        res = None
    ...
    for profil in ProfilFacturX:
        ...
        try:
            ...
            logger.debug(
                msg=f"  -> Succès ! Fichier '{resultat['chemin_fichier']}' créé et validé."
            )
        except Exception as e:
            logger.debug(msg=f"  -> ERREUR lors de la génération du profil {profil.name}: {e}")
    ...
    try:
        ...
        if chemin_facture_en16931:
            ...
            logger.debug(
                msg=f"  -> La logique de signature serait exécutée ici pour générer '{chemin_facture_signee}'."
            )
        else:
            logger.debug(
                msg="  -> AVERTISSEMENT : Le fichier EN16931 n'a pas été généré, impossible de le signer."
            )

    except FileNotFoundError:
        # La nouvelle API de signature lèverait une FileNotFoundError
        logger.debug(
            msg="  -> AVERTISSEMENT : Fichiers de clé/certificat non trouvés. La signature est ignorée."
        )
    except Exception as e:
        logger.debug(msg=f"  -> ERREUR lors de la signature : {e}")
