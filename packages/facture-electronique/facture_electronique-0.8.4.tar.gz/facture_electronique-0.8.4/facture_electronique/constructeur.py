...
class ConstructeurFacturX:
    ...
    def integrer_dans_pdfa(self, chemin_ou_contenu_pdf_source: typing.Union[str, bytes, Path]):
        """
        Convertit un PDF source en PDF/A-3 et y attache le XML Factur-X.
        Le PDF source peut être un chemin (str, Path) ou un contenu binaire (bytes).
        """
        ...
        chemin_pdf_source_a_utiliser: typing.Union[str, Path]
        if isinstance(chemin_ou_contenu_pdf_source, bytes):
            # On a reçu un contenu binaire, on le sauvegarde dans un fichier temporaire
            chemin_pdf_source_temporaire = self._creer_fichier_temporaire(suffix=".source.pdf")
            chemin_pdf_source_temporaire.write_bytes(chemin_ou_contenu_pdf_source)
            chemin_pdf_source_a_utiliser = chemin_pdf_source_temporaire
        else:
            # On a reçu un chemin
            chemin_pdf_source_a_utiliser = chemin_ou_contenu_pdf_source
        ...
        convert_to_pdfa(str(chemin_pdf_source_a_utiliser), chemin_pdfa)
    ...
    def en_bytes(self) -> bytes:
        """Retourne le contenu du PDF final sous forme de bytes et nettoie les fichiers temporaires."""
        if not self.chemin_pdf_courant:
            raise RuntimeError("Aucun PDF n'a été généré.")

        try:
            pdf_bytes = self.chemin_pdf_courant.read_bytes()
        finally:
            self._nettoyer_temporaires()
