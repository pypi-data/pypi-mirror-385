...
def parse_birth_date(raw_date: str) -> str:
    """Parses YYMMDD into YYYY-MM-DD, correctly handling 1900s vs 2000s."""
    raw_date = raw_date.strip()

    # Parse two-digit year
    year_two_digit = int(raw_date[:2])
    month = int(raw_date[2:4])
    day = int(raw_date[4:6])

    # Get current year to decide century threshold dynamically
    current_year = datetime.now().year % 100  # e.g. 25 for 2025

    # Assume any YY greater than current year's last two digits → 1900s
    # Otherwise → 2000s
    if year_two_digit > current_year:
        year_full = 1900 + year_two_digit
    else:
        year_full = 2000 + year_two_digit
            ...
            if country == 'IRQ' and data.get('doc_type') == 'national_identity_card' and data.get('nfc_data', ''):
                ...
                if nfc_data.get('birthDate', ''):
                    try:
                        ...
                        main_properties['date_of_birth'] = parse_birth_date(nfc_data.get('birthDate','').strip())
                    except:
                        pass
