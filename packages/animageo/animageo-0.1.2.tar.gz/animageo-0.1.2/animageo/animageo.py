...
class AnimaGeoScene(MovingCameraScene):
    ...
    def addUpdater(self, tracker):
        tracker.add_updater(lambda v, self = self: self.updateGeoVar(tracker, tracker.name))
    def clearUpdater(self, tracker):
        tracker.clear_updaters()

    def loadCode(self, filepath, update = False, debug = False):
        short_parser.load(self.geo, filepath, update = update, debug = debug)
    ...
    def updateGeoVar(self, x, var_name):
        if self.geo.var(var_name).data != x.get_value():
            # print(f"{var_name} = {self.geo.var(var_name).data} -> {x.get_value()}")
...
def CreateMObject(elem, z_auto = False, style = None, debug = False):
    try:
        ...
        if type(elem.data) == geo.Angle: 
            ...
            # print('range_type', range_type, 'angle', angle)
            is_clockwise = False
            if range_type == 1: 
