...
class KeycloakProvider(AuthProvider):
    ...
    async def get_user(self, user_id: str) -> User:
        ...
        # Fetch user's roles
        try:
            user_roles = await self.admin.get_user_realm_roles(user_id)
            ...
            role_names = []
        return self._map_keycloak_user(keycloak_user, roles=role_names)
    ...
    async def list_users(self, filters: UserFilters) -> PaginatedResponse:
        ...
        # Map to User models and fetch roles for each user
        users = []
        for keycloak_user in keycloak_users:
            ...
            try:
                user_roles = await self.admin.get_user_realm_roles(user_id)
                role_names = [role.get("name", "") for role in user_roles if role.get("name")]
                logger.debug(f"[LIST_USERS] User {keycloak_user.get('username')} has roles: {role_names}")
            except Exception as e:
                logger.warning(f"[LIST_USERS] Failed to fetch roles for user {user_id}: {e}")
                role_names = []

            users.append(self._map_keycloak_user(keycloak_user, roles=role_names))
    ...
    async def create_team(self, team_data: TeamCreate) -> Team:
        ...
        # Ensure parent_id is provided - teams must belong to an organization
        if not parent_id:
            from authflow.core.exceptions import ValidationError
            raise ValidationError("Teams must belong to an organization or parent team")

        attributes = {
            "team_id": [team_data.name.lower().replace(" ", "-")],
            "display_name": [team_data.display_name or team_data.name],
            "organization_id": [team_data.organization_id],  # Store organization_id in attributes
        }
    ...
    async def list_teams(self, filters: TeamFilters) -> PaginatedResponse:
        ...
        if filters.organization_id:
            # Get children (subgroups) of the organization
            # Note: get_group() returns the group but subGroups might be nested IDs only
            # We need to use get_group_children() or fetch children separately
            org_group = await self.admin.get_group(filters.organization_id)

            # Check if subGroups contains full objects or just IDs
            sub_groups_data = org_group.get("subGroups", [])

            if sub_groups_data and isinstance(sub_groups_data[0] if sub_groups_data else {}, dict) and "id" in (sub_groups_data[0] if sub_groups_data else {}):
                # subGroups contains full group objects
                keycloak_groups = sub_groups_data
            else:
                # subGroups might be empty or incomplete, fetch children via API
                # Get all groups and filter by parent path
                all_groups = await self.admin.list_groups(search=filters.search, first=0, max_results=1000)
                org_path = org_group.get("path", "")
                keycloak_groups = [
                    g for g in all_groups
                    if g.get("path", "").startswith(org_path + "/") and g.get("path", "") != org_path
                ]
        else:
    ...
    async def update_role(self, role_id: str, role_data: RoleUpdate) -> Role:
        """Update role with description and/or permissions."""
        logger.info(f"[PROVIDER] Updating role {role_id}")

        # Build update payload
        update_payload = {}

        if role_data.description is not None:
            update_payload["description"] = role_data.description

        # Store permissions in role attributes
        if role_data.permissions is not None:
            update_payload["attributes"] = {
                "permissions": role_data.permissions
            }

        logger.debug(f"[PROVIDER] Update payload: {update_payload}")

        # Update the role
        updated_role = await self.admin.update_realm_role(role_id, update_payload)
        logger.info(f"[PROVIDER] Role {role_id} updated successfully")

        return self._map_keycloak_role(updated_role)
    ...
    async def assign_role_to_user(self, user_id: str, role_id: str, scope_id: Optional[str] = None) -> bool:
        """Assign role to user."""
        logger.info(f"[PROVIDER] Assigning role '{role_id}' to user '{user_id}'")
        await self.admin.assign_realm_roles_to_user(user_id, [role_id])
        logger.info(f"[PROVIDER] Role '{role_id}' assigned successfully to user '{user_id}'")
        return True
    ...
    def _map_keycloak_user(self, keycloak_user: Dict[str, Any], roles: Optional[List[str]] = None) -> User:
    ...
    def _map_keycloak_role(self, keycloak_role: Dict[str, Any], scope: Optional[str] = None, scope_id: Optional[str] = None) -> Role:
        ...
        permissions = []
        if "attributes" in keycloak_role and keycloak_role["attributes"]:
            # Permissions are stored as a list in attributes
            if "permissions" in keycloak_role["attributes"]:
                perm_attr = keycloak_role["attributes"]["permissions"]
                # Handle both list and single value
                if isinstance(perm_attr, list):
                    permissions = perm_attr
                else:
                    permissions = [perm_attr] if perm_attr else []
