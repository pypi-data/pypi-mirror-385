    ...
    async def get_realm_role(self, role_identifier: str) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                role = admin.get_realm_role(role_identifier, brief_representation=False)
                logger.info(f"[KEYCLOAK_ADMIN] Found role by name: {role.get('name')} (ID: {role.get('id')}), attributes: {role.get('attributes', {})}")
                return role
                ...
                if role:
                    logger.info(f"[KEYCLOAK_ADMIN] Found role by ID: {role.get('name')} (ID: {role.get('id')}), attributes: {role.get('attributes', {})}")
                    return role
                else:
