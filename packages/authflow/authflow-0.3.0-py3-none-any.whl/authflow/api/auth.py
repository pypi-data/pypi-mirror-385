    ...
    async def register(user_data: UserRegister) -> User:
        ...
        try:
            ...
            try:
                ...
                # Assign org_admin role to the user for their organization
                try:
                    await provider.assign_user_role(
                        user_id=user.id,
                        role_name="org_admin",
                    )
                    logger.info(f"Assigned org_admin role to user {user.username}")
                except Exception as role_error:
                    logger.warning(f"Could not assign org_admin role: {role_error}")
