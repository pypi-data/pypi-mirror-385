...
from authflow.core.initialization import initialize_system
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    # Initialize system (roles, etc.) - runs after authflow is set up
    # This is a deferred initialization that happens after setup_auth
    yield
...
# Startup event handler for system initialization
@app.on_event("startup")
async def on_startup():
    """Run system initialization after app is ready."""
    try:
        logger.info("Running system initialization...")
        result = await initialize_system(authflow.provider)
        if result["success"]:
        ...
        # Don't block startup on initialization failure
