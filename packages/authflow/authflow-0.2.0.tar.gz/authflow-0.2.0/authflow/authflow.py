...
from authflow.api.mfa import create_mfa_router
from authflow.api.audit import create_audit_router
from authflow.api.gdpr import create_gdpr_router
...
class AuthFlow:
    ...
    def __init__(
        ...
        redis_url = getattr(config.provider.keycloak, 'redis_url', 'redis://localhost:6379/1')
        self.audit_service = AuditService(redis_url=redis_url)
    ...
    def create_routers(self) -> Dict[str, APIRouter]:
        ...
        # MFA router (always enabled for security)
        routers["mfa"] = create_mfa_router(
            provider=self.provider,
            dependencies=self.dependencies,
        )

        # Audit router (always enabled for compliance)
        routers["audit"] = create_audit_router(
            audit_service=self.audit_service,
            dependencies=self.dependencies,
        )

        # GDPR compliance router (always enabled for data protection)
        routers["gdpr"] = create_gdpr_router(
            provider=self.provider,
            audit_service=self.audit_service,
            dependencies=self.dependencies,
        )
