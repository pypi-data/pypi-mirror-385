...
def subtree_mutation(tree, max_depth, functions, terminals):
    ...
    new_subtree = generate_tree(max_depth=max(2, max_depth // 2), functions=functions, terminals=terminals, method='grow')
...
def train_GP(df, equations, params=None):
    ...
    stagnation_limit = int(params.get('stagnation_limit', 10)) # NEW: Anti-stagnation control
    ...
    for i in range(population_size):
        # Ramped half-and-half initialization for better diversity
    ...
    for gen in range(n_generations):
        ...
        # **FIX**: Anti-stagnation population reset
        if stagnation_counter >= stagnation_limit:
            print(f"--- Stagnation detected. Resetting {population_size // 4} worst individuals. ---")
            population.sort(key=lambda x: x.fitness) # Sort worst to best
            num_to_replace = population_size // 4
            for i in range(num_to_replace):
                depth = (i % (max_depth - 1)) + 2
                method = 'full' if (i // (max_depth - 1)) % 2 == 0 else 'grow'
                new_trees = {fname: generate_tree(depth, library['functions'], library['terminals'], method=method) for fname in all_funcs_map}
        ...
        while len(new_population) < population_size:
            ...
            for fname in child_trees:
                if random.random() < subtree_mutation_prob:
                    subtree_mutation(child_trees[fname], max_depth, library['functions'], library['terminals'])
