...
def subtree_crossover(tree1, tree2):
    """ Swaps a random subtree between two parent trees. Returns a new tree. """
    t1_nodes = get_all_nodes(tree1)
    t2_nodes = get_all_nodes(tree2)
    
    # Simple crossover: replace root of t1 with root of t2
    # A more robust implementation would deepcopy and replace a random node.
    new_tree = Gene(t2.value)
    new_tree.children = t2.children
    return new_tree
...
def train_genetic_programming(df, equations, params=None):
    ...
    for gen in range(n_generations):
        ...
        for ind in population:
            ...
            args = [data_dict[s.name] for s in sorted(df_syms.values(), key=lambda s: s.name)] + \
                   ...
                   [ind.scalar_params[s.name] for s in sorted(a_syms.values(), key=lambda s: s.name)]
        ...
        while len(new_population) < population_size:
            ...
            child_trees = {fname: tree for fname, tree in p1.func_trees.items()}
