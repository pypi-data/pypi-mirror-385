...
def train_SparseR(df, equations, params=None):
    ...
    # --- FIX: Negate Y to solve for +(f1+f2...) instead of -(f1+f2...) ---
    # The regression target Y should be equal to the sum of the unknown functions.
    # original_equation: LHS = known_RHS + unknown_funcs_RHS
    # unknown_funcs_RHS = LHS - known_RHS
    # known_terms_expr is currently (LHS - RHS), which after removing funcs is (LHS - known_RHS).
    # We want to solve for the functions, which are on the other side of the equation.
    ...
    for f_name, var_name in all_funcs_map.items():
        ...
        # We want the library to represent the positive function, so we negate the coefficient
        # from the "full_expr = 0" equation.
