...
def evaluate_expr(expr, tensors, models, scalar_params, device):
    if expr.is_Number:
        return torch.tensor(float(expr), dtype=torch.float32).to(device)
    elif expr.is_Symbol:
        var_name = str(expr)
        ...
        arg = expr.args[0]
        arg_tensor = evaluate_expr(arg, tensors, models, scalar_params).to(device)
        ...
        args = [evaluate_expr(arg, tensors, models, scalar_params).to(device) for arg in expr.args]
        if expr.is_Add:
            return sum(args)
        elif expr.is_Mul:
            result = args[0]
...
def compute_constraint_loss(models, constraints, func_list, tensors, device):
    ...
    for c in constraints:
        ...
        if m_val_at:
            ...
            y_target = torch.tensor(y_target_val, dtype=torch.float32).to(device)
            if f_name in models:
                model = models[f_name]
...
#    optimizer = optim.Adam(param_groups)
    ...
    for epoch in range(epochs):
        ...
        for lhs_expr, rhs_expr, w in zip(lhs_exprs, rhs_exprs, weights):
            lhs_val = evaluate_expr(lhs_expr, tensors, models, scalar_params).to(device)
            rhs_val = evaluate_expr(rhs_expr, tensors, models, scalar_params).to(device)
        ...
        constraint_loss = compute_constraint_loss(models, constraints, func_order, tensors).to(device)
