...
def generate_tree(max_depth, functions, terminals, depth=0):
    """ Creates a random expression tree using the ramped half-and-half method. """
    method = 'grow' if depth < max_depth // 2 else 'full'
    
    if depth == max_depth or (method == 'grow' and random.random() < 0.5):
...
def subtree_crossover(tree1, tree2):
    ...
    # 1. Create a deep copy of the first parent to serve as the child's base
    child_tree = tree1.copy()
    
    # 2. Get all nodes from the new child and the second parent
    child_nodes = get_all_nodes(child_tree)
    parent2_nodes = get_all_nodes(tree2)
    
    # 3. Select a random node in the child that will be replaced
    crossover_point_child = random.choice(child_nodes)
    
    # 4. Select a random subtree from the second parent to insert
    subtree_to_insert = random.choice(parent2_nodes).copy()
    
    # 5. Replace the content (value and children) of the child's node
...
def train_GP(df, equations, params=None):
    ...
    for gen in range(n_generations):
        ...
        print(f"Gen {gen+1}/{n_generations} - Best Fitness: {best_overall_individual.fitness:.4f} | Params: {param_str}")
        ...
        while len(new_population) < population_size:
            ...
            # Create a deep copy of the parent to create the child
            ...
            if random.random() < crossover_rate:
                ...
                # Crossover operates on one function tree at a time
                fname_to_cross = random.choice(list(child_trees.keys()))
                child_trees[fname_to_cross] = subtree_crossover(p1.func_trees[fname_to_cross], p2.func_trees[fname_to_cross])
            ...
            if random.random() < mutation_rate:
                fname_to_mutate = random.choice(list(child_trees.keys()))
                child_trees[fname_to_mutate] = point_mutation(child_trees[fname_to_mutate], library['functions'], library['terminals'])
