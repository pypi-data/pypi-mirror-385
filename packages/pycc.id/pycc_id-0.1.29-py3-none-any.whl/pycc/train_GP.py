...
class Individual:
    ...
    def evaluate_tree(self, tree, data_dict):
        ...
        child_values = [self.evaluate_tree(child, data_dict) for child in self.children]
    ...
    def to_symbolic_tree(self, tree, var_name):
        ...
        child_syms = [self.to_symbolic_tree(child, var_name) for child in self.children]
...
def train_GP(df, equations, params=None):
    ...
    local_search_enabled = bool(params.get('local_search_enabled', True)) # NEW
    ...
    # Store the base residual expression with placeholders for f_i and a_i
    ...
    if local_search_enabled and best_overall_individual:
        ...
        # Build substitution map for f_i and a_i placeholders
        sub_map = {f_syms[fname]: expr for fname, expr in final_f_exprs.items()}
        sub_map.update({a_syms[pname]: val for pname, val in final_scalars.items()})
        
        # Create a single expression with all functions and scalars substituted
        full_solved_expr = base_residual_expr.subs(sub_map)
        
        constants_to_optimize = sorted(full_solved_expr.atoms(sp.Float), key=str)
        if constants_to_optimize:
            opt_syms = [sp.Symbol(f'c{i}') for i in range(len(constants_to_optimize))]
            expr_for_opt = full_solved_expr.subs(dict(zip(constants_to_optimize, opt_syms)))
            
            all_df_syms_ordered = sorted(df_syms.values(), key=lambda s: s.name)
            residual_for_opt_lambda = sp.lambdify(all_df_syms_ordered + opt_syms, expr_for_opt, 'numpy')

            def objective(consts):
                df_vals = [df[s.name].values for s in all_df_syms_ordered]
                residuals = residual_for_opt_lambda(*df_vals, *consts)
                return np.mean(residuals**2)

            initial_coeffs = [float(c) for c in constants_to_optimize]
            ...
            # Substitute optimized coefficients back into individual function expressions
            ...
            print("No floating-point constants found to optimize.")
