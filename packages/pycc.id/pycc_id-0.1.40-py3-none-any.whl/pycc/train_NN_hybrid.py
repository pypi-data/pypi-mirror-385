...
def evaluate_expr(expr, tensors, models, scalar_params, device):
    if expr.is_Number:
        ...
        args = [evaluate_expr(arg, tensors, models, scalar_params,device) for arg in expr.args]
        if expr.is_Add:
            return sum(args)
        elif expr.is_Mul:
            result = args[0]
...
#    optimizer = optim.Adam(param_groups)
    ...
    for epoch in range(epochs):
        ...
        for lhs_expr, rhs_expr, w in zip(lhs_exprs, rhs_exprs, weights):
            lhs_val = evaluate_expr(lhs_expr, tensors, models, scalar_params,device)
            rhs_val = evaluate_expr(rhs_expr, tensors, models, scalar_params,device)
