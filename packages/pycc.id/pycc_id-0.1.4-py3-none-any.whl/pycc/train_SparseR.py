...
def train_sparse_regression(df, equations, params=None):
    """
    ...
    # Create symbolic variables for parsing the RHS
    ...
    rhs_expr = sp.sympify(rhs_str, locals={**df_syms, **f_syms, **a_syms})
    ...
    for term in sp.Add.make_args(rhs_expr):
        ...
        for f_name, var_name in func_map.items():
            if f_syms[f_name] in term.free_symbols:
                sign_coeff = term.as_coeff_Mul(f_syms[f_name])[0]
