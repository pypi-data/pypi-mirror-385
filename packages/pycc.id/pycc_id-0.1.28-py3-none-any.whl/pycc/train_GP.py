...
def train_GP(df, equations, params=None):
    ...
    constant_mutation_prob = float(params.get('constant_mutation_prob', 0.1)) # NEW
    ...
    library.setdefault('terminals', ['x', 'ERC']) # ERC for Ephemeral Random Constant
    erc_range = params.get('erc_range', (-2.0, 2.0)) # NEW: Range for ERCs
    ...
    for f_name, var_name in all_funcs_map.items():
        ...
        try:
            # Round floats before simplifying to avoid tiny residual coefficients
