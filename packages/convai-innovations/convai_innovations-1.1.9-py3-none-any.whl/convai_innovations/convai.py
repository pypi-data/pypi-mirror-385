...
Use the button in the Code Practice Area to start/stop."""
    ...
    def _on_session_change(self, selected_session):
        ...
        if new_session_id and new_session_id != self.session_manager.progress.current_session_id:
            print(f"\n🔍 DEBUG - Session Switch:")
            print(f"   From: {self.current_session_id}")
            print(f"   To: {new_session_id}")

            # Save current code before switching
            current_code = self.code_editor.get_text().strip()
            print(f"   Current code length: {len(current_code)} characters")

            if current_code:
                self.session_code[self.current_session_id] = current_code
                self._save_session_code_to_file()
                print(f"💾 Saved code for session: {self.current_session_id}")
            else:
                print(f"   No code to save (empty editor)")

            # Update session ID
            old_session_id = self.current_session_id
            self.session_manager.progress.current_session_id = new_session_id
            self.current_session_id = new_session_id
            print(f"   Session ID updated: {old_session_id} → {new_session_id}")
    ...
    def _auto_save_session_code(self):
        ...
        try:
            ...
            print(f"   ✅ Saved to memory dictionary")
            ...
            if code and len(code) > 10:
                ...
                self.conversation_storage.save_conversation(entry)
                print(f"   ✅ Saved to conversation storage")

            print(f"💾 Code auto-saved for session: {self.current_session_id}\n")
        except Exception as e:
    ...
    def _save_session_code_to_file(self):
        ...
        try:
            ...
            print(f"      File path: {code_file}")
            print(f"      Sessions in memory: {list(self.session_code.keys())}")
            print(f"      File exists before save: {code_file.exists()}")

            with open(code_file, 'w', encoding='utf-8') as f:
                json.dump(self.session_code, f, indent=2, ensure_ascii=False)

            print(f"      ✅ File saved successfully")
            print(f"      File size: {code_file.stat().st_size} bytes")
    ...
    def _load_session_code(self):
        ...
        try:
            ...
            print(f"\n🔍 DEBUG - Loading session code:")
            print(f"   Storage directory: {storage_dir}")
            print(f"   Code file path: {code_file}")
            ...
            if code_file.exists():
                ...
                print(f"   Sessions found in file: {list(self.session_code.keys())}")
                print(f"   Total sessions: {len(self.session_code)}")
                ...
                if self.current_session_id in self.session_code:
                    ...
                    print(f"   Found code for current session: {len(saved_code)} characters")
                    print(f"   Code preview: {saved_code[:100]}..." if len(saved_code) > 100 else f"   Code: {saved_code}")

                    if saved_code:
                        self.code_editor.set_text(saved_code)
                        print(f"✅ Loaded saved code for session: {self.current_session_id}\n")
                        return
                else:
                    print(f"   ⚠️ No code found for session: {self.current_session_id}")
            else:
                print(f"   ⚠️ Code file does not exist yet")
        ...
        if not hasattr(self, 'session_code'):
            self.session_code = {}
            print(f"   Initialized empty session_code dictionary\n")
        ...
        if messagebox.askokcancel("Quit", "Do you want to exit ConvAI Innovations?"):
            ...
            try:
                ...
                if current_code:
                    self.session_code[self.current_session_id] = current_code
                    self._save_session_code_to_file()
                    print(f"💾 Saved code before closing: {self.current_session_id}\n")
                else:
                    print(f"   No code to save (empty editor)\n")
            except Exception as e:
