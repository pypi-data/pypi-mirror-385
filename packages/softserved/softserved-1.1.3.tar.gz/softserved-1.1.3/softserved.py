...
BANNERS = {
    "default": f"""{Fore.BLUE}{Style.BRIGHT}
  ╔═══════════════════════════════════════════════════════════════════╗
                        🍦 softserved v1.1 🍦               
        Simple, stylish, and surprisingly handy static file server
  ╚═══════════════════════════════════════════════════════════════════╝
{Style.RESET_ALL}""",
    "dark": f"""{Fore.CYAN}{Style.DIM}
  ┌───────────────────────────────────────────────────────────────────┐
...
def serve(directory=".", port=8000, open_browser=True, watch_reload=False, style="default"):
    ...
    if not os.path.isdir(directory):
        print_error(f"Directory not found: {directory}", style)
        sys.exit(1)
    ...
    try:
        with socketserver.TCPServer(("", port), handler) as httpd:
            ...
            print_info("Tip: Seeing an old served site? Try Cmd/Ctrl+Shift+R for a hard refresh", style)
            ...
            # Setup watchdog if reload enabled
            if watch_reload:
                try:
        ...
        if e.errno == 48:  # Address already in use (macOS/Linux)
            print_error(f"Port {port} is already in use.", style)
            print_warn(f"Try: softserved -p {port + 1}", style)
            print_info(f"Or kill the process: lsof -i :{port} | grep LISTEN | awk '{{print $2}}' | xargs kill -9", style)
        else:
            print_error(f"Server error: {e}", style)
        sys.exit(1)
    except KeyboardInterrupt:
        print("\n" + Fore.CYAN + "Shutting down server... Bye 👋" + Style.RESET_ALL)
    except Exception as e:
        print_error(f"Unexpected error: {e}", style)
        sys.exit(1)
    finally:
        if observer:
            observer.stop()
...
def main():
    ...
    parser.add_argument("--style", choices=["default", "dark", "mono"], default="default", help="Choose terminal style")
