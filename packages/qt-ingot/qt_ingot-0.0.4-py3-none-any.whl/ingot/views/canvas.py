...
class CanvasWidget(QWidget):
    ...
    def mouseMoveEvent(self, event):
        ...
        dist_sq = (self._mouse_pos.x() - self._last_log_pos.x())**2 + (self._mouse_pos.y() - self._last_log_pos.y())**2
        
        if dist_sq >= self._log_threshold**2:  # Only log if moved more than threshold
            # Create rich text with color visualization
            color_r, color_g, color_b, color_a = self._current_color.red(), self._current_color.green(), self._current_color.blue(), self._current_color.alpha()
            ...
            console.print(f"[bold]Position:[/] ({self._mouse_pos.x()}, {self._mouse_pos.y()}), "
    ...
    def mousePressEvent(self, event):
        ...
        color_r, color_g, color_b, color_a = self._current_color.red(), self._current_color.green(), self._current_color.blue(), self._current_color.alpha()
        color_block = f"[{color_r},{color_g},{color_b}]"
        console.print(f"[bold red]Mouse Press:[/] ({self._mouse_pos.x()}, {self._mouse_pos.y()}), "
                     f"[bold]Color:[/] ({color_r}, {color_g}, {color_b}, {color_a}) "
                     f"[on rgb({color_r},{color_g},{color_b})]{color_block}[/]")
    ...
    def set_zoom_level(self, zoom_level):
        ...
        console.print(f"[bold yellow]Zoom:[/] {self._zoom_level:.2f}x ({zoom_percent:.0f}%)")
        self.update()
...
class GradientCanvasWidget(CanvasWidget):
    ...
    def paint_content(self, painter):
        ...
        gradient = QRadialGradient(
            center_x, center_y, 0,        # Center with 0 radius
            center_x, center_y, actual_radius  # Focal point and radius
        )
