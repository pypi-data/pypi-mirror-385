...
class WorkspaceManager(QTabWidget):
    ...
    def __init__(self, view_config: dict):
        ...
        self._view_factory_callable = view_config.get("view_factory") # For backward compatibility
        self._layout_template = view_config.get("layout_template") # The new declarative layout
        self._widget_factories = view_config.get("widget_factories", {}) # Factories for the layout
    ...
    def new_tab(self):
        ...
        if self._layout_template and self._widget_factories:
            ...
            content_widget = self._build_layout_from_template(self._layout_template, self._widget_factories)
            ...
            raise ValueError("view_config must provide either 'view_factory' (callable) or 'layout_template' and 'widget_factories'.")
    ...
    def _build_layout_from_template(self, template: LayoutNode, factories: WidgetFactoryDict) -> QWidget:
        """
        ...
        if isinstance(template, Leaf):
            ...
            if not widget_class or not issubclass(widget_class, QWidget):
                raise ValueError(f"No valid QWidget factory found for placeholder '{template.name}' in factories: {list(factories.keys())}")
            return widget_class()

        elif isinstance(template, (HSplit, VSplit)):
            # Recursive step: Create a splitter
            orientation = Qt.Orientation.Horizontal if isinstance(template, HSplit) else Qt.Orientation.Vertical
            splitter = QSplitter(orientation)

            for child_template in template.children:
                child_widget = self._build_layout_from_template(child_template, factories)
