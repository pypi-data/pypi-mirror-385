...
class ThemeManager:
    ...
    def _build_theme_menu(self):
        ...
        try:
            ...
            for theme_name in discovered_themes:
                theme_menu_items.append({
                    ...
                    "function": lambda checked, name=theme_name: self.apply_theme(name)
                })
        ...
        menu_bar = self._target.menuBar() # Get the menu bar from the main window
        self._action_manager.build_menu_from_dict(view_menu_data, menu_bar)
    ...
    def apply_default_theme(self):
        ...
        try:
            ...
            with pkg_resources.path('ingot.resources.themes', 'theme.scss') as scss_path:
                include_paths = [str(scss_path.parent)]
                compiled_css = sass.compile(filename=str(scss_path), include_paths=include_paths)
                self._target.setStyleSheet(compiled_css)
        except Exception as e:
