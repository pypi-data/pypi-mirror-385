...
class TextFSMOptions(object):
    ...
    class TmpFilldown(OptionBase):
        ...
        def OnAssignVar(self):
            self._tmp_var = self.value.value

        def OnClearVar(self):
            self.value.value = self._tmp_var

        def ClearTmpVar(self):
            self._tmp_var = None
    ...
    class GroupBy(OptionBase):
        """Value constitutes part of the Key of the record."""

    class ExcludeGroup(OptionBase):
        """Value constitutes part of the Key of the record."""
...
class TextFSMValue(object):
    ...
    def ClearTmpFilldownAndOtherVar(self):
        """Clear this Value."""
        self.value = None
        # Call OnClearVar on options.
        _ = [
            (
                option.ClearTmpVar()
                if isinstance(option, TextFSMOptions.TmpFilldown)
                else option.OnClearVar()
            )
...
class TextFSM(object):
    ...
    def _AppendRecord(self):
        ...
        # Append Block Record
        if hasattr(self, "_block_record"):
            cur_record.append(self._block_record)
    ...
    def ParseText(self, text, eof=True):
        ...
        self._cur_line_num = 0
        self._raw_line_num = 0
        self._parser_line_num = 0
        while self._cur_line_num < len(lines):
            line = lines[self._cur_line_num]
            is_continue = self._CheckLine(line)
    ...
    def ParseTextToDicts(self, *args, **kwargs):
        ...
        if self.GetValuesByAttrib("Key"):
            result_dicts = self._ProccessKey(result_dicts)
        if self.GetValuesByAttrib("GroupBy"):
            result_dicts = self._ProccessGroupBy(result_dicts)

        return result_dicts

    def _ProccessKey(self, result_dicts):
        keys = self.GetValuesByAttrib("Key")
        ls = self.GetValuesByAttrib("List")
        map = {}
    ...
    def _ProccessGroupBy(self, result_dicts):
        map = result_dicts
        res = {}
        groups = self.GetValuesByAttrib("GroupBy")
        exGroups = self.GetValuesByAttrib("ExcludeGroup")
        for item in map:
            key = ""
            not_empty = False
            go = {}
            ...
            if key in res:
                for name in item:
                    if (name in exGroups) or (name == "BLOCK_RECORD"):
                        if item[name]:
                        ...
                        go[name] = item[name]
                if not_empty:
                    res[key]["groups"].append(go)
            else:
                obj = {"groups": []}
                for name in item:
                    if name in groups:
                        obj[name] = item[name]
                    elif (name in exGroups) or (name == "BLOCK_RECORD"):
                        obj[name] = item[name]
                    else:
                        if item[name]:
                            not_empty = True
        ...
        for rule in self._cur_state:
            ...
            if matched:
                ...
                if self._Operations(rule, line):
                    ...
                    if rule.new_state:
                        self._StateTransition(line, rule)
                    return False
                else:
    ...
    def _StateTransition(self, line, rule):
        if self._cur_state_name == "Start" and rule.new_state.startswith("BL_"):
            self._StartRecordBlock(line)
        elif self._cur_state_name.startswith("BL_") and not rule.new_state.startswith(
            "BL_"
        ):
            self._EndRecordBlock()
        elif self._cur_state_name.startswith(
            "BL_RAW_CONF_"
        ) and rule.new_state.startswith("BL_PARSER_"):
            self._raw_line_num = self._cur_line_num
            if self._parser_line_num > self._cur_line_num:
                self._cur_line_num = self._parser_line_num
            self._EndRecordBlock()
        if rule.new_state not in ("End", "EOF"):
            self._cur_state = self.states[rule.new_state]
        self._cur_state_name = rule.new_state

    def _StartRecordBlock(self, line):
        self._record_block = True
        self._block_record = collections.OrderedDict()
        self._RecordBlock(line)

    def _EndRecordBlock(self):
        self._record_block = False

    def _RecordGroupBy(self):
        self._AppendRecord()
        self._record_block = True

    def _RecordPrefixEq(self, line):
        first_key = next(iter(self._block_record))
        first_value = self._block_record[first_key]
        index = first_value.index(str(line.strip()[0]))
        self._block_record[first_key] = first_value[:index]
        self._AppendRecord()
        self._block_record = collections.OrderedDict()
        self._block_record[first_key] = first_value
        self._record_block = True

    def _RecordFirstAndLast(self):
        first_key = next(iter(self._block_record))
        first_value = self._block_record[first_key]
        self._AppendRecord()
        self._block_record = collections.OrderedDict()
    ...
    def _RecordAndRawConf(self):
        self._parser_line_num = self._cur_line_num
        self._cur_line_num = self._raw_line_num
        self._AppendRecord()

    def _RecordBlock(self, line):
        if self._record_block:
            self._block_record[self._cur_line_num] = line

    def _RecordLine(self, line):
        self._StartRecordBlock(line)
        self._AppendRecord()
        self._EndRecordBlock()
    ...
    def _Operations(self, rule, line):
        ...
        Operators come in two parts and are a '.' separated pair:
          ...
          Operators that affect the record being built for output (record_op).
            ...
            'RecordLine'    Adds the current record to the result and record the line to BLOCK_RECORD.
        ...
        if rule.record_op == "Record":
            ...
            # Clear record.
            self._ClearTmpFilldownAndOtherRecord()

        elif rule.record_op == "Clearall":
            # Clear all record entries.
            self._ClearAllRecord()

        elif rule.record_op == "RecordLine":
            self._RecordLine(line)

        elif rule.record_op == "RecordLastMatched":
            self._block_record.popitem()
            self._AppendRecord()

        elif rule.record_op == "RecordGroupBy":
            self._RecordGroupBy()

        elif rule.record_op == "RecordPrefixEq":
            self._RecordPrefixEq(line)

        elif rule.record_op == "RecordFirstAndLast":
            self._RecordFirstAndLast()

        elif rule.record_op == "RecordAndRawConf":
            self._RecordAndRawConf()
    ...
    def _ClearTmpFilldownAndOtherRecord(self):
        """Remove 'TmpFilldown' and other record entries."""
        self._EndRecordBlock()
        _ = [value.ClearTmpFilldownAndOtherVar() for value in self.values]
