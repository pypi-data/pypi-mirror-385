...
@dataclass_json
@dataclass
class CtvpcCreateAclRuleRulesRequest:
...
@dataclass_json
@dataclass
class CtvpcCreateAclRuleRequest:
    clientToken: str  # 客户端存根，用于保证订单幂等性, 长度 1 - 64
    regionID: str  # 资源池ID
    aclID: str  # aclID
    rules: List[CtvpcCreateAclRuleRulesRequest]  # rule 规则数组



@dataclass_json
@dataclass
class CtvpcCreateAclRuleReturnObjResponse:
    """接口业务数据"""
    aclID: Optional[str]  # 名称


@dataclass_json
@dataclass
class CtvpcCreateAclRuleResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为success, 英文
    description: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为成功, 中文
    errorCode: Optional[str]  # statusCode为900时为业务细分错误码，三段式：product.module.code; statusCode为800时为SUCCESS
    returnObj: Optional[CtvpcCreateAclRuleReturnObjResponse]  # 接口业务数据



# 创建 Acl 规则
class CtvpcCreateAclRuleApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtvpcCreateAclRuleRequest) -> CtvpcCreateAclRuleResponse:
        url = endpoint + "/v4/acl-rule/create"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcCreateAclRuleResponse.from_dict(response.json())
