...
@dataclass_json
@dataclass
class CtvpcNewVpcListReturnObjVpcsResponse:
...
@dataclass_json
@dataclass
class CtvpcNewVpcListReturnObjResponse:
    """接口业务数据"""
    vpcs: Optional[List[Optional[CtvpcNewVpcListReturnObjVpcsResponse]]]  # vpc 组
...
@dataclass_json
@dataclass
class CtvpcNewVpcListResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为success, 英文
    description: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为成功, 中文
    errorCode: Optional[str]  # statusCode为900时为业务细分错误码，三段式：product.module.code; statusCode为800时为SUCCESS
    returnObj: Optional[CtvpcNewVpcListReturnObjResponse]  # 接口业务数据



# 查询用户专有网络列表
class CtvpcNewVpcListApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtvpcNewVpcListRequest) -> CtvpcNewVpcListResponse:
        url = endpoint + "/v4/vpc/new-list"
        params = {'regionID':request.regionID, 'vpcID':request.vpcID, 'vpcName':request.vpcName, 'pageNumber':request.pageNumber, 'pageNo':request.pageNo, 'pageSize':request.pageSize, 'projectID':request.projectID, 'nextToken':request.nextToken, 'maxResults':request.maxResults}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            print("vpc new list response1:", response)
            print("vpc new list response:", response.json())
            return CtvpcNewVpcListResponse.from_dict(response.json())
