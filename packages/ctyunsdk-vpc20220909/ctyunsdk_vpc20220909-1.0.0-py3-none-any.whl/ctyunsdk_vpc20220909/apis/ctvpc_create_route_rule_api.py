...
@dataclass_json
@dataclass
class CtvpcCreateRouteRuleRequest:
    regionID: str  # 区域 id
...
@dataclass_json
@dataclass
class CtvpcCreateRouteRuleReturnObjResponse:
    """业务数据"""
...
@dataclass_json
@dataclass
class CtvpcCreateRouteRuleResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为success, 英文
    description: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为成功, 中文
    errorCode: Optional[str]  # statusCode为900时为业务细分错误码，三段式：product.module.code; statusCode为800时为SUCCESS
    returnObj: Optional[CtvpcCreateRouteRuleReturnObjResponse]  # 业务数据



# 创建单条路由规则
class CtvpcCreateRouteRuleApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtvpcCreateRouteRuleRequest) -> CtvpcCreateRouteRuleResponse:
        url = endpoint + "/v4/vpc/route-table/create-rule"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcCreateRouteRuleResponse.from_dict(response.json())
