...
class CtvpcListAclRequest:
    ...
    projectID: Optional[str] = None  # 企业项目 ID，默认为"0" = None
    name: Optional[str] = None  # acl Name = None


@dataclass_json
@dataclass
class CtvpcListAclReturnObjAclsResponse:
...
@dataclass_json
@dataclass
class CtvpcListAclReturnObjResponse:
    """接口业务数据"""
    acls: Optional[List[Optional[CtvpcListAclReturnObjAclsResponse]]]  # acls


@dataclass_json
@dataclass
class CtvpcListAclResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为success, 英文
    description: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为成功, 中文
    errorCode: Optional[str]  # statusCode为900时为业务细分错误码，三段式：product.module.code; statusCode为800时为SUCCESS
    requestID: Optional[str]  # 请求 id
    returnObj: Optional[CtvpcListAclReturnObjResponse]  # 接口业务数据
...
class CtvpcListAclApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcListAclRequest) -> CtvpcListAclResponse:
        url = endpoint + "/v4/acl/list"
        params = {'regionID': request.regionID, 'aclID': request.aclID, 'projectID': request.projectID,
                  'name': request.name, 'pageNumber': request.pageNumber, 'pageNo': request.pageNo,
                  'pageSize': request.pageSize}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            return CtvpcListAclResponse.from_dict(response.json())
