...
@dataclass_json
@dataclass
class CtvpcNewQuerySecurityGroupsRequest:
    regionID: str  # 区域id
...
@dataclass_json
@dataclass
class CtvpcNewQuerySecurityGroupsReturnObjSecurityGroupsSecurityGroupRuleListResponse:
...
@dataclass_json
@dataclass
class CtvpcNewQuerySecurityGroupsReturnObjSecurityGroupsResponse:
    ...
    securityGroupRuleList: Optional[List[Optional[CtvpcNewQuerySecurityGroupsReturnObjSecurityGroupsSecurityGroupRuleListResponse]]]  # 安全组规则信息


@dataclass_json
@dataclass
class CtvpcNewQuerySecurityGroupsReturnObjResponse:
    """返回结果"""
    securityGroups: Optional[List[Optional[CtvpcNewQuerySecurityGroupsReturnObjSecurityGroupsResponse]]]  # 安全组列表
...
@dataclass_json
@dataclass
class CtvpcNewQuerySecurityGroupsResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为success, 英文
    description: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为成功, 中文
    errorCode: Optional[str]  # statusCode为900时为业务细分错误码，三段式：product.module.code; statusCode为800时为SUCCESS
    returnObj: Optional[CtvpcNewQuerySecurityGroupsReturnObjResponse]  # 返回结果



# 查询用户安全组列表。
class CtvpcNewQuerySecurityGroupsApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtvpcNewQuerySecurityGroupsRequest) -> CtvpcNewQuerySecurityGroupsResponse:
        url = endpoint + "/v4/vpc/new-query-security-groups"
        params = {'regionID':request.regionID, 'vpcID':request.vpcID, 'queryContent':request.queryContent, 'instanceID':request.instanceID, 'pageNumber':request.pageNumber, 'pageNo':request.pageNo, 'pageSize':request.pageSize, 'nextToken':request.nextToken, 'maxResults':request.maxResults}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            return CtvpcNewQuerySecurityGroupsResponse.from_dict(response.json())
