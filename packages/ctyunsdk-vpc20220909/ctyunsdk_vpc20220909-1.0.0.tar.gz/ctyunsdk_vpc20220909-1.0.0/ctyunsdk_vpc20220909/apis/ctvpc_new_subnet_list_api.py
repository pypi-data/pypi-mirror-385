...
@dataclass_json
@dataclass
class CtvpcNewSubnetListRequest:
    regionID: str  # 资源池 ID
...
@dataclass_json
@dataclass
class CtvpcNewSubnetListReturnObjSubnetsResponse:
...
@dataclass_json
@dataclass
class CtvpcNewSubnetListReturnObjResponse:
    """接口业务数据"""
    subnets: Optional[List[Optional[CtvpcNewSubnetListReturnObjSubnetsResponse]]]  # subnets 组
...
@dataclass_json
@dataclass
class CtvpcNewSubnetListResponse:
    statusCode: Any  # 返回状态码（800为成功，900为失败）
    message: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为success, 英文
    description: Optional[str]  # statusCode为900时的错误信息; statusCode为800时为成功, 中文
    errorCode: Optional[str]  # statusCode为900时为业务细分错误码，三段式：product.module.code; statusCode为800时为SUCCESS
    returnObj: Optional[CtvpcNewSubnetListReturnObjResponse]  # 接口业务数据



# 查询用户专有网络下子网列表
class CtvpcNewSubnetListApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtvpcNewSubnetListRequest) -> CtvpcNewSubnetListResponse:
        url = endpoint + "/v4/vpc/new-list-subnet"
        ...
        try:
            ...
            return CtvpcNewSubnetListResponse.from_dict(response.json())
