...
@dataclass_json
@dataclass
class CtvpcListRouteTableRequest:
    regionID: str  # 区域id
...
@dataclass_json
@dataclass
class CtvpcListRouteTableReturnObjResponse:
...
@dataclass_json
@dataclass
class CtvpcListRouteTableResponse:
    ...
    totalPage: Any  # 总页数
    returnObj: Optional[List[Optional[CtvpcListRouteTableReturnObjResponse]]]  # 返回结果



# 查询路由表列表
class CtvpcListRouteTableApi:
    ...
    @staticmethod
    def do(credential: Credential, client: CtyunClient, endpoint: str, request: CtvpcListRouteTableRequest) -> CtvpcListRouteTableResponse:
        url = endpoint + "/v4/vpc/route-table/list"
        ...
        try:
            ...
            return CtvpcListRouteTableResponse.from_dict(response.json())
