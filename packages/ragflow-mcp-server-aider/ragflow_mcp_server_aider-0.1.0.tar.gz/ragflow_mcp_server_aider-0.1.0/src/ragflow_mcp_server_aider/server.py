...
logger = logging.getLogger('ragflow_mcp_server_aider')
logger.info("Starting RAGFlow MCP Aider")
...
async def serve() -> Server:
    server = Server("ragflow-mcp-server-aider")
    ...
    async def handle_call_tool(
        ...
        if name == "list_datasets":
            ...
            try:
                ...
                # 创建聊天助手
                assistant = ragflow.create_chat(name=chat_name, dataset_ids=dataset_ids)
    ...
    async def _run():
        ...
        async with mcp.server.stdio.stdio_server() as (read_stream, write_stream):
            ...
            await server.run(
                ...
                write_stream,
                InitializationOptions(
                    server_name="ragflow-mcp-server-aider",
