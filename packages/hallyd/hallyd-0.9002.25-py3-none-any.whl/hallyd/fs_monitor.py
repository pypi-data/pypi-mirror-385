...
class Watcher(abc.ABC):
    ...
    @abc.abstractmethod
    def wait_changed(self, *, timeout: t.Optional[float] = None) -> bool:
        pass
...
class _Watcher(Watcher):
    ...
    class WatchThread(threading.Thread, abc.ABC):
        ...
        @classmethod
        def try_create(cls, watcher: "_Watcher") -> t.Optional["_Watcher.WatchThread"]:
            try:
                return cls(watcher)
            except Exception:
                return
    ...
    def __compute_cookie__single_item(self, path: "_fs.Path", *, flat: bool = False):
        try:
            ...
            if not flat and stat.S_ISDIR(path_stat.st_mode):
                children = tuple([self.__compute_cookie__single_item(child_path, flat=True)
