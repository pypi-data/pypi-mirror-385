...
<kvirt:plan>{plan}</kvirt:plan>
        ...
        namespace = ''
        dnsmasqxml = ''
        dhcpoptions = {key: overrides[key] for key in overrides if key in DHCPKEYWORDS + ['routes'] or key.isdigit()}
        if dhcpoptions:
            namespace = "xmlns:dnsmasq='http://libvirt.org/schemas/network/dnsmasq/1.0'"
            dnsmasqxml = "<dnsmasq:options>"
            for key in dhcpoptions:
                if key == 'routes':
                    for route in dhcpoptions['routes']:
                        value = f"dhcp-option=option:classless-static-route,{route.replace(";", ",")}"
                        dnsmasqxml += f'<dnsmasq:option value="dhcp-option=option:classless-static-route,{value}"/>'
                    continue
                option = 'option'
                if family == 'ipv6':
                    option += '6'
