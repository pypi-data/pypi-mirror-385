...
def text_to_speech(text):
    magic = Magic()
    openai_spellbook = OpenAI()
    magic.add(openai_spellbook.get('tts_text_to_speech', voice='shimmer'))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        response = chat(
            text[-1],
            context=texts if texts else None,
            system='You are a translating assistant.'
        )

        # add using chatml
        texts.append({'role': 'user', 'content': text[-1]})
        texts.append({'role': 'assistant', 'content': response[-1]})

        # Convert text back to speech and save to a file
        tts_file_path = text_to_speech(response[-1])
