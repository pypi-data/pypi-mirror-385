...
If someone has the time and resources (maybe samples) improve that!
  ...
  >>> def printb(msgb):
  ...     if isinstance(msgb, bytes):
  ...         print(msgb.decode('utf-8'))
  ...     else:
  ...         print(msgb)
...
Checks if a boundary specified by the request is valid.
  ...
  >>> validBoundary("-----------------------------7d81772d2c0206")
  <...Match object...>

  >>> bool(validBoundary("-----------------------------7d81772d2c0206"))
  True
...
Content-type and a dictionary of options.
  ...
  >>> from p01.cgi.parser import parseHeader
  >>> header = "Content-Type: text/plain"
  >>> cdisp, pdict = parseHeader(header)
  >>> print(cdisp)
  ...
  >>> header = 'Content-Disposition: form-data; name="form.widgets.upload"; filename="test.txt"'
  >>> cdisp, pdict = parseHeader(header)
  >>> print(cdisp)
...
As you can see we will get an input stream as file content:
  ...
  >>> isinstance(mpf.file, io.IOBase)
  True

  >>> printb(mpf.file.read())
  -----------------------------7d81772d2c0206
...
Of course, we didn't get a value, we need to read the file data:
  ...
  >>> printb(multiPartField.list[0].file.read())
  <BLANKLINE>
  This is a test file
  <BLANKLINE>
...
The second field provides the submit button name and value:
  ...
  >>> printb(multiPartField.list[1].value)
  <BLANKLINE>
  Add
...
Set up a tempfile factory and arguments:

  >>> import tempfile
  >>> tmpFileFactory = tempfile.NamedTemporaryFile
  >>> tmpFileFactoryArguments = {'mode': 'w+b'}

Parse the *large* multipart with the factory (do not overwrite later):

  >>> fieldList = parseFormData(
  ...     'POST',
...
Get the MultiPartField with the upload data:

  >>> mpf = fieldList[1]
  >>> mpf
  <MultiPartField, ...'upload': ...'large.txt'>

As you can see we get our tempfile instance with the 'w+b' mode from
tmpFileFactoryArguments:

  >>> tname = type(mpf.file).__name__
  >>> tname in ('_TemporaryFileWrapper', 'BufferedRandom', 'TextIOWrapper', 'BufferedWriter', 'BufferedReader', 'FileIO', 'file', 'instance')
  True
