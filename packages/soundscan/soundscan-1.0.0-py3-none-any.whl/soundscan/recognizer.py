...
class ACRCloudRecognizer:
    ...
    def _prepare_request_data(self, file_path: str, file_size: int,
                              timestamp: str, signature: str) -> tuple:
    ...
    def _parse_response(self, result: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if result.get('status', {}).get('code') == 0 and 'metadata' in result:
            ...
            song_info = {
                ...
                'genres': [genre.get('name', '') for genre in music_data.get('genres', [])],
                'external_ids': music_data.get('external_ids', {})
            }
            logger.info(
                f"🎵 Found: {song_info['title']} by {song_info['artist']}")
            return song_info
        else:
