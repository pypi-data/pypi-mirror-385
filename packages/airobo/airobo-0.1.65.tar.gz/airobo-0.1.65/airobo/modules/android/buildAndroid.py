...
def build_android_bundle(app_path):
    ...
    # Ensure app icon is copied from config location if available
...
def _ensure_app_icon(android_dir: Path) -> None:
    ...
    if not source_icon:
        print("ℹ️ No appIcon.png found in config locations. Using default icon.")
        return
    ...
    try:
        ...
        with Image.open(source_icon) as img:
            ...
            print(f"   Generated {len(densities)} density variants")
        ...
        print(f"⚠️ Icon processing failed: {e}. Using default icon.")
