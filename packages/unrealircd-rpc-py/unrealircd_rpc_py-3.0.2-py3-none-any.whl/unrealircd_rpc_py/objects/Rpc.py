...
class Rpc:
    ...
    def info(self) -> list[Dfn.RpcInfo]:
        ...
        try:
            ...
            rpcinfos: dict[dict, dict] = response_model.result.get('methods', {})
    ...
    def set_issuer(self, name: str) -> Dfn.RPCResult:
        ...
        try:
            ...
            if response_model.error.code == 0:
                self.Logs.debug(f"Method: {response_model.method} - Result: Success")
            ...
            return Dfn.RPCResult(result=False, error=Dfn.RPCErrorModel(-1, ke.__str__()))
        except Exception as err:
            self.Logs.error(f'General error: {err}')
            return Dfn.RPCResult(result=False, error=Dfn.RPCErrorModel(-1, err.__str__()))

    def add_timer(self, timer_id: str, every_msec: int, request: dict) -> Dfn.RPCResult:
