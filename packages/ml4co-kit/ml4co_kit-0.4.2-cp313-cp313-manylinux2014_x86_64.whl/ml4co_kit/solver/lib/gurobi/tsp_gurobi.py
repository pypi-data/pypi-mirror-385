...
def tsp_gurobi(
    ...
    """ 
    if gurobi_tsp_use_mtz_or_lazy == "mtz":
        return _tsp_gurobi_mtz(
            task_data=task_data,
            gurobi_time_limit=gurobi_time_limit
        )
    elif gurobi_tsp_use_mtz_or_lazy == "lazy":
        return _tsp_gurobi_lazy(
            task_data=task_data,
            gurobi_time_limit=gurobi_time_limit)
    else:
        raise ValueError(
            f"Invalid value for gurobi_tsp_use_mtz_or_lazy: {gurobi_tsp_use_mtz_or_lazy}"
        )
...
def _tsp_gurobi_mtz(
    ...
    # Create gurobi model
    model = gp.Model(f"TSP-{task_data.name}")
    ...
    # Using MTZ to model the subtour elimination constraints
    u = {}
    for i in range(nodes_num):
        u[i] = model.addVar(
            lb=0.0, 
    ...
    # MTZ subtour elimination: for i != j, u[i] - u[j] + N * x[i,j] <= N-1
    ...
    # Object
    object = gp.quicksum(
        dists[i, j] * x[i, j] for i in range(nodes_num) \
            for j in range(nodes_num) if i != j
    )
    model.setObjective(object, gp.GRB.MINIMIZE)

    # Solve
    model.write(f"TSP-{task_data.name}.lp")
    model.optimize()
    os.remove(f"TSP-{task_data.name}.lp")
