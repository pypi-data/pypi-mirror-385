...
def mcl_rlsa(
    ...
    energy, grad = mcl_energy_func(adj_matrix, x, nodes_weight, rlsa_beta)
    ...
    comp_adj_matrix = torch.ones_like(adj_matrix) - adj_matrix 
    term2 = torch.sum((torch.matmul(sol_uq, comp_adj_matrix) * sol_uq).squeeze(1), 1)
