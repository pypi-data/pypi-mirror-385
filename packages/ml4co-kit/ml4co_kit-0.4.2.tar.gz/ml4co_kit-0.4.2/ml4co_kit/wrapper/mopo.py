...
class MOPOWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(MOPOWrapper, self).__init__(
            task_type=TASK_TYPE.MOPO, precision=precision
        )
        self.task_list: List[MOPOTask] = list()
    ...
    def from_txt(
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                # Parse data: returns, cov_matrix, var_factor
                data_parts = data_part.split(" var_factor ")
                if len(data_parts) != 2:
                    raise ValueError(f"Invalid data format in line {idx + 1}: {data_part}")
                
                matrix_part = data_parts[0]
                var_factor = float(data_parts[1])
                ...
                # Create task
                if overwrite:
                    mopo_task = MOPOTask(precision=self.precision)
                else:
                    mopo_task = self.task_list[idx]
                ...
                if overwrite:
                    self.task_list.append(mopo_task)
    ...
    def to_txt(
        ...
        with open(file_path, mode) as f:
            ...
            for task in tqdm_by_time(self.task_list, write_msg, show_time):
                ...
                cov = task.cov
                var_factor = task.var_factor
                sol = task.sol
                
                # Write data to ``.txt`` file
                # Format: returns cov var_factor output solution
                f.write(" ".join(str(x) for x in returns))
                f.write(" cov ")
                f.write(" ".join(str(x) for x in cov.flatten()))
                f.write(" var_factor ")
                f.write(str(var_factor))
                f.write(" output ")
