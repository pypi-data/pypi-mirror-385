...
class MVCWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(MVCWrapper, self).__init__(
            task_type=TASK_TYPE.MVC, precision=precision
        )
        self.task_list: List[MVCTask] = list()
    ...
    def from_txt(
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                # Add to task list
                if overwrite:
                    self.task_list.append(mvc_task)
    ...
    def from_gpickle_result_folder(
        ...
        for idx, (graph_file_path, result_file_path) in tqdm_by_time(
            ...
            if overwrite:
                mvc_task = MVCTask(node_weighted=None, precision=self.precision)
            else:
                mvc_task = self.task_list[idx]
            mvc_task.from_gpickle_result(
