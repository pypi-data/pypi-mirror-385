...
class MISWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(MISWrapper, self).__init__(
            task_type=TASK_TYPE.MIS, precision=precision
        )
        self.task_list: List[MISTask] = list()
    ...
    def from_gpickle_result_folder(
        ...
        for idx, (graph_file_path, result_file_path) in tqdm_by_time(
            ...
            if overwrite:
                mis_task = MISTask(node_weighted=None, precision=self.precision)
            else:
                mis_task = self.task_list[idx]
            mis_task.from_gpickle_result(
