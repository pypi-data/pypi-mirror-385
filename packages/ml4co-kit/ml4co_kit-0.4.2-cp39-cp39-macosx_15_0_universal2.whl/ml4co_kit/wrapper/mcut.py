...
class MCutWrapper(WrapperBase):
    def __init__(
        self, precision: Union[np.float32, np.float64] = np.float32
    ):
        super(MCutWrapper, self).__init__(
            task_type=TASK_TYPE.MCUT, precision=precision
        )
        self.task_list: List[MCutTask] = list()
        ...
        with open(file_path, "r") as file:
            ...
            for idx, line in tqdm_by_time(enumerate(file), load_msg, show_time):
                ...
                if "weights" in line:
                    ...
                    edges_weight = edges_weight.split(" ")
                    edges_weight = np.array(
                        [float(edge_weight) for edge_weight in edges_weight],
                    ...
                    if overwrite:
                        mcut_task = MCutTask(
                            edge_weighted=True, precision=self.precision
                        )
                    else:
                        mcut_task = self.task_list[idx]
                    mcut_task.from_data(
                        edge_index=edge_index, edges_weight=edges_weight,
                        sol=sol, ref=ref
                    )
                ...
                # Case2: Unweighted
                    ...
                    if overwrite:
                        mcut_task = MCutTask(
                            edge_weighted=False, precision=self.precision
                        )
                    else:
                        mcut_task = self.task_list[idx]
                    mcut_task.from_data(
                ...
                # Add to task list
                if overwrite:
                    self.task_list.append(mcut_task)
    ...
    def to_txt(
        ...
        with open(file_path, mode) as f:
            ...
            for task in tqdm_by_time(self.task_list, write_msg, show_time):
                ...
                if task.edge_weighted:
                    f.write(str(" ") + str("weights") + str(" "))
                    f.write(str(" ").join(str(edge_weight) for edge_weight in task.edges_weight))
    ...
    def from_gpickle_result_folder(
        ...
        for idx, (graph_file_path, result_file_path) in tqdm_by_time(
            ...
            if overwrite:
                mcut_task = MCutTask(edge_weighted=None, precision=self.precision)
            else:
                mcut_task = self.task_list[idx]
            mcut_task.from_gpickle_result(
