...
class MigManTransform:
    ...
    def get_active_adapter(self) -> str:
        """
        Determine the active adapter from the extraction configuration.

        Returns:
            str: The name of the active adapter (e.g., 'inforcom', 'sapecc').
        """
        adapter = None
    ...
    def nonempty(self) -> None:
        ...
        for project in self.cfg.setup.projects:
            ...
            if not columns_info:
                self.logger.warning(f"No columns found in table {table}")
                continue
