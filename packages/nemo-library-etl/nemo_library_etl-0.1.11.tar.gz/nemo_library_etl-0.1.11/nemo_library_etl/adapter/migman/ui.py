...
class MigManUI:
    ...
    def run_ui(self, *, open_browser: bool = True) -> None:
        ...
        migmanprojects = sorted([
            project_name for project_name in [
                (f"{m.project} {MIGMAN_PROJECT_POSTFIX_SEPARATOR} {m.postfix}") 
                if m.postfix and m.project else m.project
        ...
        def index():
            ...
            def render_extract():
                ...
                with content_container:
                    ...
                    with ui.card().classes("mt-4"):
                        ...
                        extract_active = extract_config.get("extract_active", True)
                        ...
                        nemo_project_prefix_input = ui.input(
                            label="NEMO project prefix",
                            value=extract_config.get(
                                "nemo_project_prefix", "migman_extract_"
                            ),
                            placeholder="migman_extract_",
                            on_change=on_nemo_project_prefix_change,
                        ).props("dense").classes("w-[36rem] mt-3")
                        if not extract_active:
                            nemo_project_prefix_input.props("disable")

                    # Adapter Configurations with Tabs
                    with ui.card().classes("mt-4") as adapters_card:
                        ui.label("Data Source Adapters").classes(
                            "text-lg font-semibold mb-2"
                        )
                        ui.label("Note: Only one extract method (INFORCOM or SAP ECC) can be active at a time.").classes(
                            "text-sm text-amber-600 mb-4"
                        )

                        if not extract_active:
                            # Show disabled message when extract is not active
                            ui.label("Extract must be active to configure data source adapters.").classes(
                                "text-sm text-gray-500 italic mb-4"
                            )
                            adapters_card.props("disable")
                        else:
                            with ui.tabs().classes("w-full") as tabs:
                                inforcom_tab = ui.tab("INFORCOM")
                                sapecc_tab = ui.tab("SAP ECC")

                            with ui.tab_panels(tabs, value=inforcom_tab).classes("w-full"):
                                # INFORCOM Tab Panel
                                with ui.tab_panel(inforcom_tab):
                                    render_inforcom_config(extract_config)

                                # SAP ECC Tab Panel
                                with ui.tab_panel(sapecc_tab):
            ...
            def render_inforcom_config(extract_config):
                ...
                with ui.column().classes("gap-4"):
                    # Active toggle
                    def on_inforcom_active_change(e):
                        inforcom_config["active"] = e.value
                        # If INFORCOM is activated, deactivate SAP ECC
                        if e.value:
                            sapecc_config = extract_config.setdefault("sapecc", {})
                            if sapecc_config.get("active", False):
                                sapecc_config["active"] = False
                                ui.notify("SAP ECC has been automatically deactivated", type="info")
                                # Re-render the extract section to update UI
                                render_extract()
                        mark_changed()

                    ui.switch(
                        "INFORCOM active",
                        value=inforcom_config.get("active", False),
                        on_change=on_inforcom_active_change,
                    )
                        ...
                        with ui.row().classes("gap-2 items-end mt-3"):
                            ...
                            def add_custom_table():
                                ...
                                if table_name and table_name not in all_available_tables:
                                    ...
                                    current_selection = inforcom_config.get("tables", [])
            ...
            def render_sapecc_config(extract_config):
                ...
                with ui.column().classes("gap-4"):
                    # Active toggle
                    def on_sapecc_active_change(e):
                        sapecc_config["active"] = e.value
                        # If SAP ECC is activated, deactivate INFORCOM
                        if e.value:
                            inforcom_config = extract_config.setdefault("inforcom", {})
                            if inforcom_config.get("active", False):
                                inforcom_config["active"] = False
                                ui.notify("INFORCOM has been automatically deactivated", type="info")
                                # Re-render the extract section to update UI
                                render_extract()
                        mark_changed()
                    ...
                    ui.switch(
                        "SAP ECC active",
                        value=sapecc_config.get("active", False),
                        on_change=on_sapecc_active_change,
                    )
                    ...
                    with ui.column().classes("w-full") as tables_container:
                        ...
                        # Combine default tables with any custom tables that might be in current config
                        all_available_tables = list(set(default_sapecc_tables + current_tables))
                        all_available_tables.sort()
                ...
                with content_container:
                    ...
                    with ui.card().classes("mt-4"):
                        ...
                        with ui.column().classes("gap-3"):
                            ...
                            transform_active = transform_config.get("transform_active", True)
                            ...
                            nemo_prefix_input = ui.input(
                                label="Nemo Project Prefix",
                                value=transform_config.get("nemo_project_prefix", "migman_transform_"),
                                on_change=on_nemo_project_prefix_change,
                    ...
                    with ui.card().classes("mt-4"):
                        ui.label("Transform Components").classes("text-lg font-semibold mb-3")
                        ...
                        if transform_active:
                            ...
                            with transform_components_container:
                                ui.label("Transform components are disabled because Transform is not active.").classes("text-sm text-gray-500 italic p-4")
            ...
            def render_transform_join_config(transform_config):
                ...
                ui.label("Configure table joins and relationships").classes("text-sm mb-4")
                ...
                with ui.column().classes("gap-4"):
                    ...
                    # Joins configuration
                    joins_config_label = ui.label("Join Configurations").classes("text-md font-medium mt-4")
                    if not join_active:
                    ...
                    for join_name, join_settings in joins_config.items():
                        with ui.card().classes("mt-2 p-3"):
                            with ui.row().classes("items-center justify-between w-full"):
                                join_label = ui.label(f"Join: {join_name}").classes("font-medium")
                                if not join_active:
                            ...
                            join_item_switch = ui.switch(
                                ...
                                on_change=lambda e, name=join_name: on_join_item_active_change(e, name),
                            )
                            if not join_active:
                                join_item_switch.props("disable")

                            join_file_input = ui.input(
                                label="File",
                                value=join_settings.get("file", ""),
                                placeholder="Path to join configuration file",
                                on_change=lambda e, name=join_name: on_join_item_file_change(e, name),
                            ).props("dense").classes("w-full")
                            if not join_active:
                    ...
                    if join_active:
                        ...
                        with ui.card().classes("mt-2 p-3 bg-gray-100"):
                            ui.label("Join configurations are disabled because Join is not active.").classes("text-sm text-gray-500 italic")
            ...
            def render_transform_nonempty_config(transform_config):
                ...
                ui.label("Non-Empty Configuration").classes("text-lg font-semibold mb-3")
                ui.label("Filter out empty records during transformation").classes("text-sm mb-4")
                ...
                with ui.column().classes("gap-4"):
                    ...
                    ui.label("This transformation will remove records with empty or null values in key fields.").classes("text-sm text-gray-600")
            ...
            def render_transform_duplicate_config(transform_config):
                ...
                ui.label("Duplicate Configuration").classes("text-lg font-semibold mb-3")
                ui.label("Configure duplicate detection and handling").classes("text-sm mb-4")
                ...
                with ui.column().classes("gap-4"):
                    ...
                    duplicates_config_label = ui.label("Duplicate Configurations").classes("text-md font-medium mt-4")
                    ...
                    for object_name, duplicate_settings in duplicates_config.items():
                        with ui.card().classes("mt-2 p-3"):
                            with ui.row().classes("items-center justify-between w-full"):
                                object_label = ui.label(f"Object: {object_name}").classes("font-medium")
                                if not duplicate_active:
                                    object_label.classes("text-gray-500")

                                delete_button = ui.button(
                                    icon="delete",
                                    on_click=lambda name=object_name: remove_duplicate_config(name)
                                ).props("flat round color=negative size=sm")
                                if not duplicate_active:
                                    delete_button.props("disable")
                            ...
                            def on_fields_change(e, name=object_name):
                                ...
                                fields = [field.strip() for field in e.value.split(",") if field.strip()]
                            ...
                            duplicate_obj_switch = ui.switch(
                                ...
                                on_change=lambda e, name=object_name: on_duplicate_obj_active_change(e, name),
                            )
                            if not duplicate_active:
                            ...
                            with ui.row().classes("gap-2 w-full"):
                                threshold_input = ui.number(
                                    ...
                                    min=0,
                                    max=100,
                                    on_change=lambda e, name=object_name: on_threshold_change(e, name),
                                ).props("dense").classes("w-48")
                                if not duplicate_active:
                                    threshold_input.props("disable")

                                primary_key_input = ui.input(
                                    label="Primary Key",
                                    value=duplicate_settings.get("primary_key", ""),
                                    placeholder="Enter primary key field",
                                    on_change=lambda e, name=object_name: on_primary_key_change(e, name),
                                ).props("dense").classes("flex-1")
                                if not duplicate_active:
                                    primary_key_input.props("disable")

                            fields_input = ui.input(
                                label="Fields (comma-separated)",
                                value=", ".join(duplicate_settings.get("fields", [])),
                                placeholder="field1, field2, field3",
                                on_change=lambda e, name=object_name: on_fields_change(e, name),
                            ).props("dense").classes("w-full")
                            if not duplicate_active:
                    ...
                    if duplicate_active:
                        with ui.card().classes("mt-2 p-3 border-dashed"):
                            ui.label("Add New Duplicate Configuration").classes("font-medium mb-2")
                            ...
                            def add_duplicate_config():
                                if new_object_name["value"] and new_primary_key["value"]:
                                    fields = [field.strip() for field in new_fields["value"].split(",") if field.strip()]
                        ...
                        with ui.card().classes("mt-2 p-3 bg-gray-100"):
                            ui.label("Duplicate configurations are disabled because Duplicate detection is not active.").classes("text-sm text-gray-500 italic")
            ...
            def render_load():
                ...
                with content_container:
                    ...
                    with ui.card().classes("mt-4"):
                        ...
                        load_active = load_config.get("load_active", True)
                        ...
                        nemo_project_prefix_input = ui.input(
                            label="NEMO project prefix",
                            value=load_config.get("nemo_project_prefix", "migman_load_"),
                            placeholder="migman_load_",
                            on_change=on_nemo_project_prefix_change,
                        ).props("dense").classes("w-[36rem] mt-3")
                        if not load_active:
                            nemo_project_prefix_input.props("disable")
