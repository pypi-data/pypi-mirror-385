...
try:
    from mcp_server_aliyun_observability.core.admin.sts_client import (
        get_arms_client,
        get_cms_client,
        get_sls_client,
    )

    ADMIN_STS_AVAILABLE = True
except ImportError:
    ADMIN_STS_AVAILABLE = False

try:
    from mcp_server_aliyun_observability.core.admin.thread_context import ThreadContext
...
class CMSClientWrapper:
    ...
    def with_region(self, region: str) -> CmsClient:
        if (
            THREAD_CONTEXT_AVAILABLE
            and ADMIN_STS_AVAILABLE
            and ThreadContext.get_user_id()
        ):
            logger.info(f"使用User-ID: {ThreadContext.get_user_id()} 获取CMS客户端")
            return get_cms_client(ThreadContext.get_user_id(), region)
...
class SLSClientWrapper:
    ...
    def with_region(
        ...
        if (
            ...
            logger.info(f"使用User-ID: {ThreadContext.get_user_id()} 获取客户端")
            return get_sls_client(ThreadContext.get_user_id(), region)
        config = _create_unified_config(self.credential)
...
class ArmsClientWrapper:
    ...
    def with_region(self, region: str, endpoint: Optional[str] = None) -> ArmsClient:
        if (
            ...
            logger.info(f"使用User-ID: {ThreadContext.get_user_id()} 获取ARMS客户端")
            return get_arms_client(ThreadContext.get_user_id(), region)
