...
public:
    ...
    static torch::Tensor toDenseTensor(
        std::vector<int64_t> rowIndices,
        std::vector<int64_t> colIndices,
        int64_t numRows,
        int64_t numCols,
        torch::Dtype dtype = torch::kFloat32)
    {
        torch::Tensor dense = torch::zeros({numRows, numCols}, torch::TensorOptions().dtype(dtype));

        for (size_t i = 0; i < rowIndices.size(); ++i) {
            dense.index_put_({rowIndices[i], colIndices[i]}, 1);
        }
