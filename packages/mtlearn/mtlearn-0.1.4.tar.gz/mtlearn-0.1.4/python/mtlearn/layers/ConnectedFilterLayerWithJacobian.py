...
class ConnectedFilterWithJacobianFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        jacobian = mtlearn.InfoTree.get_jacobian(tree).to(attrs2d.device).to(attrs2d.dtype)
        residues = mtlearn.InfoTree.get_residues(tree).to(attrs2d.device).to(attrs2d.dtype)

        y_pred = (jacobian.T @ (residues * sigmoid)).reshape(tree.numRows, tree.numCols)
