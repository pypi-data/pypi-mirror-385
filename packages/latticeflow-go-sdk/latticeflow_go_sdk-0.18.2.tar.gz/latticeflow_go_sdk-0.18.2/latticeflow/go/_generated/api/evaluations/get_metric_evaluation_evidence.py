...
from ...models.model import MetricEvaluationEvidence
from ...types import Response


def _get_kwargs(metric_evaluation_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": f"/metric-evaluations/{metric_evaluation_id}/evidence",
    }
...
) -> Union[Error, MetricEvaluationEvidence]:
    if response.status_code == 200:
        response_200 = MetricEvaluationEvidence.model_validate(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Error, MetricEvaluationEvidence]]:
    return Response(
...
def sync_detailed(
    metric_evaluation_id: str, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, MetricEvaluationEvidence]]:
    ...
    Returns:
        Response[Union[Error, MetricEvaluationEvidence]]
    """
    kwargs = _get_kwargs(metric_evaluation_id=metric_evaluation_id)
...
async def asyncio(
    ...
    """
    return (
        await asyncio_detailed(metric_evaluation_id=metric_evaluation_id, client=client)
    ).parsed
