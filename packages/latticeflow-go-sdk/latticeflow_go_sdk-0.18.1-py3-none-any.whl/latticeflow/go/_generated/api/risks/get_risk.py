...
def _get_kwargs(risk_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {"method": "get", "url": f"/risks/{risk_id}"}
...
def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = StoredRisk.model_validate(response.json())
...
def sync_detailed(
    risk_id: str, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, StoredRisk]]:
    """Get a risk
    ...
    Returns:
        Response[Union[Error, StoredRisk]]
    """
    kwargs = _get_kwargs(risk_id=risk_id)
...
def sync(
    ...
    Returns:
        Union[Error, StoredRisk]
    """
    return sync_detailed(risk_id=risk_id, client=client).parsed
...
async def asyncio(
    ...
    Returns:
        Union[Error, StoredRisk]
    """
    return (await asyncio_detailed(risk_id=risk_id, client=client)).parsed
