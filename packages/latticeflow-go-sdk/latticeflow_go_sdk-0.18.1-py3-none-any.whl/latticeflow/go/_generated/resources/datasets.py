...
class DatasetsResource(BaseModule):
    ...
    def get_datasets(self, *, key: Union[Unset, str] = UNSET) -> StoredDatasets:
        ...
        """
        with self._base.get_client() as client:
            response = get_datasets_sync(client=client, key=key)
            if isinstance(response, Error):
                raise ApiError(response)
            return response
...
class AsyncDatasetsResource(AsyncBaseModule):
    ...
    async def get_datasets(self, *, key: Union[Unset, str] = UNSET) -> StoredDatasets:
        """Get all registered datasets
        ...
        """
        with self._base.get_client() as client:
            response = await get_datasets_asyncio(client=client, key=key)
            if isinstance(response, Error):
                raise ApiError(response)
            return response
