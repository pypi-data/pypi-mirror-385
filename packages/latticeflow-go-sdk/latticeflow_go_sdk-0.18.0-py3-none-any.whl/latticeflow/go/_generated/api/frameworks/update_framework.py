...
from ...models.model import StoredFramework
from ...types import Response


def _get_kwargs(framework_id: str, body: Framework) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {"method": "put", "url": f"/frameworks/{framework_id}"}
...
) -> Union[Error, StoredFramework]:
    if response.status_code == 201:
        response_201 = StoredFramework.model_validate(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Error, StoredFramework]]:
    return Response(
...
def sync_detailed(
    framework_id: str, body: Framework, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, StoredFramework]]:
    """Update a framework
    ...
    Returns:
        Response[Union[Error, StoredFramework]]
    """
    kwargs = _get_kwargs(framework_id=framework_id, body=body)
...
def sync(
    ...
    Returns:
        Union[Error, StoredFramework]
    """
    return sync_detailed(framework_id=framework_id, client=client, body=body).parsed
...
async def asyncio(
    ...
    """
    return (
        await asyncio_detailed(framework_id=framework_id, client=client, body=body)
    ).parsed
