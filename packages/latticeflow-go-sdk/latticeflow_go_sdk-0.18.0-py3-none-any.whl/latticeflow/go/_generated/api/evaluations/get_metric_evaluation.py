...
from ...models.model import StoredMetricEvaluation
...
def _get_kwargs(metric_evaluation_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": f"/metric-evaluations/{metric_evaluation_id}",
    }
...
) -> Union[Error, StoredMetricEvaluation]:
    if response.status_code == 200:
        response_200 = StoredMetricEvaluation.model_validate(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Error, StoredMetricEvaluation]]:
    return Response(
