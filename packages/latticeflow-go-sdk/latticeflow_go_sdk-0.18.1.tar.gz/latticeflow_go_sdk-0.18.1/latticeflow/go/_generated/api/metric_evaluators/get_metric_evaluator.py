...
from ...models.model import StoredMetricEvaluator
from ...types import Response


def _get_kwargs(metric_evaluator_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": f"/metric-evaluators/{metric_evaluator_id}",
    }
...
) -> Union[Error, StoredMetricEvaluator]:
    if response.status_code == 200:
        response_200 = StoredMetricEvaluator.model_validate(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Error, StoredMetricEvaluator]]:
    return Response(
...
def sync_detailed(
    metric_evaluator_id: str, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, StoredMetricEvaluator]]:
    ...
    Returns:
        Response[Union[Error, StoredMetricEvaluator]]
    """
    kwargs = _get_kwargs(metric_evaluator_id=metric_evaluator_id)
...
def sync(
    ...
    Returns:
        Union[Error, StoredMetricEvaluator]
    """
    return sync_detailed(metric_evaluator_id=metric_evaluator_id, client=client).parsed
...
async def asyncio(
    ...
    """
    return (
        await asyncio_detailed(metric_evaluator_id=metric_evaluator_id, client=client)
    ).parsed
