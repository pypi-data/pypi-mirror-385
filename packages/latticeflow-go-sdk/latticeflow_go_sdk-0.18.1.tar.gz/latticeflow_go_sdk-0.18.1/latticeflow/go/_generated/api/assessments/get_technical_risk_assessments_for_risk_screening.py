...
from ...models.model import StoredEligibleTechnicalRiskAssessments
from ...types import Response


def _get_kwargs(risk_screening_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {
        "method": "get",
        "url": f"/risk-screenings/{risk_screening_id}/technical-risk-assessments",
    }
...
) -> Union[Error, StoredEligibleTechnicalRiskAssessments]:
    if response.status_code == 200:
        response_200 = StoredEligibleTechnicalRiskAssessments.model_validate(
            response.json()
        )
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Error, StoredEligibleTechnicalRiskAssessments]]:
    return Response(
...
def sync_detailed(
    risk_screening_id: str, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, StoredEligibleTechnicalRiskAssessments]]:
    ...
    Returns:
        Response[Union[Error, StoredEligibleTechnicalRiskAssessments]]
    """
    kwargs = _get_kwargs(risk_screening_id=risk_screening_id)
...
def sync(
    ...
    Returns:
        Union[Error, StoredEligibleTechnicalRiskAssessments]
    """
    return sync_detailed(risk_screening_id=risk_screening_id, client=client).parsed
...
async def asyncio(
    ...
    """
    return (
        await asyncio_detailed(risk_screening_id=risk_screening_id, client=client)
    ).parsed
