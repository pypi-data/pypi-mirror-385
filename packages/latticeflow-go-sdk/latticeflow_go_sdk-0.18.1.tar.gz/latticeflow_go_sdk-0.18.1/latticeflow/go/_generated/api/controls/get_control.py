...
def _get_kwargs(control_id: str) -> dict[str, Any]:
    _kwargs: dict[str, Any] = {"method": "get", "url": f"/controls/{control_id}"}
...
def _parse_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
    ...
    if response.status_code == 200:
        response_200 = StoredControl.model_validate(response.json())
...
def sync_detailed(
    control_id: str, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, StoredControl]]:
    """Get a control
    ...
    Returns:
        Response[Union[Error, StoredControl]]
    """
    kwargs = _get_kwargs(control_id=control_id)
...
def sync(
    ...
    Returns:
        Union[Error, StoredControl]
    """
    return sync_detailed(control_id=control_id, client=client).parsed
...
async def asyncio(
    ...
    Returns:
        Union[Error, StoredControl]
    """
    return (await asyncio_detailed(control_id=control_id, client=client)).parsed
