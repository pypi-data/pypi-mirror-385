...
from ...models.model import Risk
from ...models.model import StoredRisk
from ...types import Response


def _get_kwargs(risk_id: str, body: Risk) -> dict[str, Any]:
    headers: dict[str, Any] = {}

    _kwargs: dict[str, Any] = {"method": "put", "url": f"/risks/{risk_id}"}
...
) -> Union[Error, StoredRisk]:
    if response.status_code == 201:
        response_201 = StoredRisk.model_validate(response.json())
...
def _build_response(
    *, client: Union[AuthenticatedClient, Client], response: httpx.Response
) -> Response[Union[Error, StoredRisk]]:
    return Response(
...
def sync_detailed(
    risk_id: str, body: Risk, *, client: Union[AuthenticatedClient, Client]
) -> Response[Union[Error, StoredRisk]]:
    """Update a risk
    ...
    Returns:
        Response[Union[Error, StoredRisk]]
    """
    kwargs = _get_kwargs(risk_id=risk_id, body=body)
...
def sync(
    ...
    Returns:
        Union[Error, StoredRisk]
    """
    return sync_detailed(risk_id=risk_id, client=client, body=body).parsed
...
async def asyncio(
    ...
    Returns:
        Union[Error, StoredRisk]
    """
    return (await asyncio_detailed(risk_id=risk_id, client=client, body=body)).parsed
