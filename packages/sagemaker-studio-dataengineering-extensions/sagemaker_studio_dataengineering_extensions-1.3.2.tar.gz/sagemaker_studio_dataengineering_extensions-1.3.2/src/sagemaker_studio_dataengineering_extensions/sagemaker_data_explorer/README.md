# SageMaker Data Explorer JupyterLab Plugin
...
```bash
pip install sagemaker_data_explorer
```
...
```bash
pip uninstall sagemaker_data_explorer
```
...
# Change directory to the sagemaker_data_explorer directory
...
```bash
# Start the development mock API server
bb start:server
...
you can remove the symlink named `sagemaker-data-explorer` within that folder.
