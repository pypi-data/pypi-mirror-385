...
def verify_xmind_structure():
    ...
    for file_path in test_files:
        ...
        try:
            ...
            if not zipfile.is_zipfile(file_path):
                print("[ERROR] 不是有效的ZIP文件")
                all_valid = False
                continue
                
            print("[SUCCESS] 是有效的ZIP格式")
            ...
            with zipfile.ZipFile(file_path, 'r') as zf:
                ...
                if 'content.json' in files:
                    ...
                    if 'rootTopic' in content:
                        ...
                        print(f"[SUCCESS] 根主题标题: {root_title}")
                        ...
                        if 'children' in content['rootTopic'] and 'attached' in content['rootTopic']['children']:
                            ...
                            print(f"[SUCCESS] 子主题数量: {len(sub_topics)}")
                        else:
                            print("[WARNING] 没有子主题")
                    else:
                        print("[ERROR] 缺少rootTopic")
                        all_valid = False
    ...
    print(f"\n{'='*50}")
    if all_valid:
        print("[SUCCESS] 所有XMind文件结构验证通过!")
        print("[SUCCESS] 文件格式正确，内容完整")
        print("[SUCCESS] 可以正常在XMind软件中打开")
    else:
