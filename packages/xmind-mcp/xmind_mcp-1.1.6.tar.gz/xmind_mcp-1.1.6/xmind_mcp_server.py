...
if FASTMCP_AVAILABLE:
    ...
    def create_mind_map(ctx: Context, title: str, topics_json: str, output_path: str = None) -> str:
        ...
        Args:
            title: 思维导图标题
            topics_json: 主题JSON结构
            output_path: 可选的输出文件路径，如果不指定则使用默认路径
        """
        try:
            # 检查topics_json类型 - MCP框架可能自动解析JSON字符串
            if isinstance(topics_json, (dict, list)):
                # 如果已经是Python对象，转换为JSON字符串
                topics_json_str = json.dumps(topics_json, ensure_ascii=False)
                logger.info(f"topics_json已经是Python对象，转换为JSON字符串: {type(topics_json)}")
            else:
                # 确保是字符串类型
                topics_json_str = str(topics_json)
            ...
            if os.path.exists(final_output_path):
                ...
                if result_data.get("status") == "success":
                    ...
                    # 修改返回数据 - 只返回文件名，不包含路径
                    result_data["filename"] = os.path.basename(final_output_path)  # 只返回文件名
                    result_data["message"] = f"思维导图已创建: {abs_path}"  # message包含绝对路径
                    result_data["absolute_path"] = abs_path  # 额外添加绝对路径字段
                    result_data["output_path"] = final_output_path  # 添加输出路径字段
