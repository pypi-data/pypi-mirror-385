...
from config import (
    ...
    CLICKHOUSE_PASSWORD, CLICKHOUSE_DATABASE, CLICKHOUSE_TABLE, BATCH_SIZE, MAX_RETRIES,
    field_mapping
)
...
class ClickHouseClient:
    def __init__(self):
        try:
            ...
            CONNECTION_STRING = f"http://{CLICKHOUSE_USER}:{CLICKHOUSE_PASSWORD}@{CLICKHOUSE_HOST}:{CLICKHOUSE_PORT}/"
            ...
            # Try to connect
            self.client = clickhouse_connect.get_client(dsn=CONNECTION_STRING,database=CLICKHOUSE_DATABASE)
            ...
            error_msg = f"Failed to connect to ClickHouse at {CLICKHOUSE_HOST}:{CLICKHOUSE_PORT}: {str(e)}"
            logger.error(error_msg, exc_info=True)
            raise Exception(error_msg) from e

    def fetch_companies_batch(self, offset: int = 0, limit: int = BATCH_SIZE) -> List[Dict[str, Any]]:
        """
    ...
    def _build_select_query(self, offset: int, limit: int) -> str:
        ...
        if field_mapping:
            ...
            order_by_column = field_mapping.get_order_by()
            
            # Get fields to select from configuration
            source_fields = field_mapping.get_source_fields()
            fields_str = ",\n            ".join(source_fields)
        else:
