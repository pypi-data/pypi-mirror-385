...
class VerCtrl:
    ...
    def load_config(self):
        ...
        if not self.config_path.exists():
            ...
            print_info("Run 'versctrl --init' to create a default config file.")
            sys.exit(1)
    ...
    def init_config(self):
        ...
        try:
            ...
            print_info("Run 'versctrl --select' to choose files with GUI.")
        except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  versctrl --init                           Initialize config file
  versctrl --select                         PyQt6 GUI file selection with smart detection
  versctrl --smart-add smart                Smart add source files (respects .gitignore)
  versctrl --smart-add python               Add all Python files
  versctrl --smart-add recent --days 7      Add files modified in last 7 days
  versctrl --new                            Create new backup
  versctrl --list                           List all backups
  versctrl --restore file-v1.txt            Restore specific backup
  versctrl --stats                          Show statistics
  versctrl --config custom.json             Use custom config file
