...
def _generate_optimized_relationship_candidates(metadata, _record_pair, status_dict, _timed_out, min_similarity_threshold=0.6):
    ...
    for fk_table_name, fk_table_meta in metadata.items():
        ...
        for fk_norm, fk_meta in fk_table_meta["columns"].items():
            ...
            # Sort matches by score (highest first) for better debugging
            all_matches.sort(key=lambda x: x["score"], reverse=True)

            # Record ALL valid matches
            if all_matches:
    ...
    *,
    ...
    for (left_table, right_table), column_pairs in pairs.items():
        ...
        if reverse_key in deduplicated_pairs:
            ...
            current_left_col = column_pairs[0][0] if column_pairs else None
            current_right_col = column_pairs[0][1] if column_pairs else None

            reverse_column_pairs = deduplicated_pairs[reverse_key]
            reverse_left_col = reverse_column_pairs[0][0] if reverse_column_pairs else None
            reverse_right_col = reverse_column_pairs[0][1] if reverse_column_pairs else None
            ...
            current_right_is_pk = current_right_col and current_right_col.upper() in _pk_name_set(right_meta)
            reverse_right_is_pk = reverse_right_col and reverse_right_col.upper() in _pk_name_set(left_meta)

            # If current direction is FK->PK but reverse is not, replace with current
            if current_right_is_pk and not reverse_right_is_pk:
                print(f"  🔄 Replacing {reverse_key[0]} → {reverse_key[1]} with {left_table} → {right_table} (FK->PK direction)")
            ...
            # If reverse is FK->PK but current is not, keep reverse (skip current)
                ...
                print(f"  ⚠️  Skipping {left_table} → {right_table} (reverse {reverse_key[0]} → {reverse_key[1]} is correct FK->PK direction)")
                continue
            # If both or neither are PK, keep the first one encountered
            else:
