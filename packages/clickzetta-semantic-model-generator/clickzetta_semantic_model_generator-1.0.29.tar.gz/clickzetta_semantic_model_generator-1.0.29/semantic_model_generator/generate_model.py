    ...
    if len(tokens) >= 2:
        ...
        if last_token in {"ID", "KEY"}:
            ...
            if len(prefix) >= 1 and prefix not in _GENERIC_IDENTIFIER_TOKENS:
                return True
            # Handle single-letter table prefixes (common in many schemas)
    ...
    for fk_table_name, fk_table_meta in metadata.items():
        ...
        for fk_norm, fk_meta in fk_table_meta["columns"].items():
            ...
            for (pk_table_name, pk_norm), pk_info in pk_lookup.items():
                ...
                if total_score > best_score and total_score >= effective_threshold:
                    best_match = {
...
def _infer_relationships(
    ...
    *,
    ...
    # TEMPORARILY DISABLED: Shared column logic - causes conflicts with optimized candidate generation
    # Re-enable only if needed and after adding proper deduplication
    # Jump directly to "Build relationships with inferred cardinality" section
    goto_build_relationships = True
    if goto_build_relationships:
        pass  # Skip to line 3767
    else:  # Original shared column logic (disabled)
