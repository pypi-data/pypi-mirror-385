...
def _normalized_table_tokens(table_name: str) -> set[str]:
    """
    Returns a set of normalized tokens representing a table name.
    Tokens are upper-cased, singularized, and generic words are removed.
    """
...
def _normalized_column_tokens(column_name: str) -> set[str]:
    """
    Returns normalized tokens for a column name, excluding generic tokens.
    """
...
def _column_mentions_table(column_name: str, table_name: str) -> bool:
    """
    Determines whether a column name semantically references a target table.
    """
...
def _calculate_relationship_confidence(
    ...
    if column_pairs:
        ...
        if misaligned_fk_columns:
            ...
            alignment_bonus = min(0.2, 0.1 * aligned_fk_columns)
