...
def _is_valid_suffix_match(fk_column: str, pk_column: str, pk_table: str) -> bool:
    """
    Pure column-name-based validation that removes table name dependencies.

    Args:
        fk_column: Foreign key column name (normalized)
        pk_column: Primary key column name (normalized)
        pk_table: Primary key table name (not used in new logic)

    Returns:
        True if this appears to be a valid FK->PK relationship based on column patterns

    New Rules (P0 Fix - Remove table name dependencies):
    ...
    3. Focus on column naming patterns rather than table name matching
    """
    # Normalize inputs
...
def _looks_like_foreign_key(fk_table: str, pk_table: str, fk_column: str) -> bool:
    """
    Pure column-pattern-based heuristic to detect foreign key columns.
    Removed table name dependencies to work with ANY table names.

    Args:
        fk_table: Foreign key table name (not used in new logic)
        pk_table: Primary key table name (not used in new logic)
        fk_column: Foreign key column name
    ...
    P0 Fix: Pure column-name matching without hardcoded table lists
    """
    ...
    # Pattern 2: Compound FK columns with entity tokens
    ...
    # Pattern 3: Concatenated forms like customerid, orderkey
    ...
    # Pattern 4: Allow meaningful descriptive columns
    meaningful_patterns = {
