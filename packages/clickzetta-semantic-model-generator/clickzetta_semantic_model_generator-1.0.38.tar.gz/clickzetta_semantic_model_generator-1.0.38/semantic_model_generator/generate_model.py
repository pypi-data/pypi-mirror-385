        ...
        for table_a_name, table_a in metadata.items():
            ...
            for pk_norm, pk_cols in table_a["pk_candidates"].items():
                ...
                if candidates:
                    ...
                    best_score, best_similarity, best_is_single_pk, best_norm_b, best_meta_b = candidates[0]

                    # DEBUG: Log candidate selection for LINEITEM FK columns
                    if table_b_name.upper() == "LINEITEM" and best_meta_b["names"][0].upper() in ["L_PARTKEY", "L_SUPPKEY"]:
                        print(f"\n  🔍 DEBUG: LINEITEM FK candidate selection")
                        print(f"     FK column: {best_meta_b['names'][0].upper()}")
                        print(f"     Target table: {table_a_name}")
                        print(f"     Target PK: {pk_cols[0].upper()}")
                        print(f"     Score: {best_score:.3f}, Similarity: {best_similarity:.3f}, Single PK: {best_is_single_pk}")
                        if len(candidates) > 1:
                            print(f"     Other candidates ({len(candidates)-1}):")
                            for i, (score, sim, is_single, norm, meta) in enumerate(candidates[1:], 1):
                                print(f"       {i}. Score: {score:.3f}, Sim: {sim:.3f}, Single PK: {is_single}")
            ...
            for pk_norm, pk_cols in table_b["pk_candidates"].items():
                ...
                if candidates:
                    ...
                    # This prefers single-column PKs over composite PKs when scores are equal
                    candidates.sort(key=lambda x: (x[0], x[1], x[2]), reverse=True)
                    best_score, best_similarity, best_is_single_pk, best_norm_a, best_meta_a = candidates[0]

                    # DEBUG: Log candidate selection for LINEITEM FK columns
                    if table_a_name.upper() == "LINEITEM" and best_meta_a["names"][0].upper() in ["L_PARTKEY", "L_SUPPKEY"]:
                        print(f"\n  🔍 DEBUG: LINEITEM FK candidate selection (reverse)")
                        print(f"     FK column: {best_meta_a['names'][0].upper()}")
