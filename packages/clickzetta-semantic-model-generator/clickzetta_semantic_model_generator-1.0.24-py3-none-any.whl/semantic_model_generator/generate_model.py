...
def _detect_bridge_table_pattern(
    ...
    is_bridge = confidence >= 0.70  # P1 Fix: Balanced threshold to prevent over-generation while allowing legitimate relationships

    # Get unique connected tables with sufficient confidence
    connected_tables_set = {
        fk["references_table"] for fk in fk_like_columns if fk["confidence"] >= 0.75  # P1 Fix: Higher FK confidence threshold
    }
    ...
    if len(tokens) >= 2:
        ...
        if tail in {"ID", "KEY"}:
            ...
            for i in range(len(tokens) - 1):
                ...
                if len(token) >= 2 and token not in _GENERIC_IDENTIFIER_TOKENS:
                    return True

    # Pattern 4: Concatenated forms like customerid, orderkey
    if fk_upper.endswith("ID") and len(fk_upper) > 3:
        prefix = fk_upper[:-2]
        if len(prefix) >= 3 and prefix not in _GENERIC_IDENTIFIER_TOKENS:
            return True
