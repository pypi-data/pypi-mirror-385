...
from mcli.app.commands_cmd import _get_store_path, commands
...
class TestGetStorePath:
    ...
    def test_get_store_path_from_config(self, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.Path.home", return_value=tmp_path):
    ...
    def test_get_store_path_default(self, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.Path.home", return_value=tmp_path):
            with patch("mcli.app.commands_cmd.DEFAULT_STORE_PATH", tmp_path / "default"):
                result = _get_store_path()
...
class TestStoreInit:
    ...
    @patch("mcli.app.commands_cmd.subprocess.run")
    @patch("mcli.app.commands_cmd.Path.home")
    def test_init_creates_directory(self, mock_home, mock_run, runner, tmp_path):
        ...
        result = runner.invoke(commands, ["store", "init"])
    ...
    def test_init_with_remote(self, mock_home, mock_run, runner, tmp_path):
        ...
        result = runner.invoke(
            commands, ["store", "init", "--remote", "git@github.com:user/repo.git"]
        )
...
class TestStorePush:
    ...
    @patch("mcli.app.commands_cmd._get_store_path")
    @patch("mcli.app.commands_cmd.COMMANDS_PATH")
    @patch("mcli.app.commands_cmd.subprocess.run")
    def test_push_copies_commands(
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "push"])
    ...
    def test_push_with_all_includes_backups(self, mock_run, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "push", "--all"])
...
class TestStorePull:
    ...
    def test_pull_copies_from_store(self, mock_run, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "pull", "--force"])
    ...
    def test_pull_creates_backup(self, mock_run, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "pull"])
...
class TestStoreSync:
    ...
    def test_sync_pulls_then_pushes(self, mock_run, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "sync"])
...
class TestStoreList:
    ...
    def test_list_local_commands(self, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "list"])
    ...
    def test_list_store_commands(self, mock_get_store, runner, tmp_path):
        ...
        result = runner.invoke(commands, ["store", "list", "--store"])
...
class TestStoreStatus:
    ...
    def test_status_shows_git_status(self, mock_run, mock_get_store, runner, tmp_path):
        ...
        result = runner.invoke(commands, ["store", "status"])
...
class TestStoreShow:
    ...
    def test_show_local_command(self, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "show", "test.json"])
    ...
    def test_show_store_command(self, mock_get_store, runner, tmp_path):
        ...
        result = runner.invoke(commands, ["store", "show", "test.json", "--store"])
    ...
    def test_show_nonexistent_command(self, mock_get_store, runner, tmp_path):
        ...
        with patch("mcli.app.commands_cmd.COMMANDS_PATH", commands_path):
            result = runner.invoke(commands, ["store", "show", "nonexistent.json"])
...
class TestStoreConfig:
    ...
    def test_config_set_path(self, mock_home, mock_get_store, runner, tmp_path):
        ...
        result = runner.invoke(commands, ["store", "config", "--path", str(new_path)])
    ...
    def test_config_set_remote(self, mock_run, mock_get_store, runner, tmp_path):
        ...
        result = runner.invoke(
            commands, ["store", "config", "--remote", "git@github.com:user/repo.git"]
        )
