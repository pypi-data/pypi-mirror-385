    ...
    def generate_index_html(self) -> str:
        ...
        if file_reports:
            ...
            for report in file_reports:
                ...
                report_cards_html += f"""
                <div class="report-card file-report">
                    <div class="report-header">
                    ...
                    <div class="report-meta">
                        <span class="timestamp">🕒 {report['timestamp'][:19].replace('T', ' ')}</span>
                        <span class="file-size">💾 {file_size_mb:.1f} MB</span>
                        <span class="modified">📝 {report['last_modified'][:19].replace('T', ' ')}</span>
                    </div>
                    <div class="report-actions">
                        <a href="{report['filename']}" class="btn btn-primary" target="_blank">View Report</a>
                        <span class="file-path">{report['filename']}</span>
                    </div>
                </div>
                """
...
<body>
        ...
        <div class="dashboard-grid">
            ...
            <div class="dashboard-card">
                ...
                <div style="display: flex; flex-direction: column; gap: 0.5rem;">
                    <a href="../run_all_tests.py" class="btn btn-primary" onclick="return false;" title="Run this in terminal">🏃 Run All Tests</a>
                    <a href="../examples/" class="btn btn-primary" onclick="return false;" title="Browse examples directory">📂 View Examples</a>
                    <a href="#" class="btn btn-primary" onclick="window.location.reload(); return false;">🔄 Refresh Dashboard</a>
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {{
            const quickActions = document.querySelectorAll('a[onclick="return false;"]');
            quickActions.forEach(action => {{
                action.addEventListener('click', function(e) {{
                    e.preventDefault();
                    alert('Please run this command in your terminal:\\n\\n' + this.textContent.replace('🏃 ', 'python3 ').replace('📂 ', 'ls -la '));
                }});
            }});
        }});
    </script>
