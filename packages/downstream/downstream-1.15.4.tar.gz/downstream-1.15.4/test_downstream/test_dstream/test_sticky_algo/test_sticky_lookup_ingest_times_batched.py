...
from downstream.dstream.sticky_algo._sticky_lookup_ingest_times_batched import (
...
def test_sticky_time_lookup_batched_against_site_selection(s: int):
    S = 1 << s
...
@pytest.mark.parametrize("s", range(1, 12))
def test_sticky_time_lookup_batched_empty(s: int):
    S = 1 << s
...
def test_sticky_time_lookup_batched_fuzz(
    ...
    validate = validate_sticky_time_lookup(algo.lookup_ingest_times_batched)
    for S in testS:
        if S <= Tmax:
...
def test_sticky_time_lookup_batched_fuzz_parallel():
    dtype1 = np.int64
    dtype2 = np.int64
    ...
    validate = validate_sticky_time_lookup(
        _sticky_lookup_ingest_times_batched_jit,
    )
    for S in testS:
        if S <= Tmax:
