...
site_selection = validate_compressing_site_selection(algo.assign_storage_site)


def test_compressing_site_selection8():
    # fmt: off
    actual = (site_selection(8, T) for T in it.count())
    expected = [
        0, 1, 2, 3, 4, 5, 6, 7,  # T 0-7
        None, 2, None, 4, None, 6, None, None,  # T 8-15
        None, 3, None, None, None, 7, None, None,  # T 16-23
        None, 4, None, None, None, None, None, None,  # T 24-31
        None, 5, None, None, None, None, None, None, # T 32-39
        None, 6, None, None, None, None, None, None, # T 40-47
    ]
    assert all(x == y for x, y in zip(actual, expected))


def test_compressing_site_selection16():
    # fmt: off
    actual = (site_selection(16, T) for T in it.count())
    expected = [
        0, 1, 2, 3, 4, 5, 6, 7,  # T 0-7
        8, 9, 10, 11, 12, 13, 14, 15,  # T 8-15
        None, 2, None, 4, None, 6, None, 8,  # T 16-23
        None, 10, None, 12, None, 14, None, None,  # T 24-31
    ]
    assert all(x == y for x, y in zip(actual, expected))


def test_compressing_site_selection_fuzz():
...
def test_compressing_site_selection_exceeds_capacity():
