...
def validate_xtctail_time_lookup(fn: typing.Callable) -> typing.Callable:
...
time_lookup = validate_xtctail_time_lookup(algo.lookup_ingest_times)


@pytest.mark.parametrize("s", range(3, 12))
def test_xtctail_time_lookup_against_site_selection(s: int):
    S = 1 << s
    T_max = 1 << 17 - s
    expected = [None] * S
...
def test_xtctail_time_lookup_fuzz(S: int, T: int):
