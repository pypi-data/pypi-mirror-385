...
time_lookup = validate_stretchedxtc_time_lookup(algo.lookup_ingest_times)


@pytest.mark.parametrize("s", range(3, 12))
def test_stretchedxtc_time_lookup_against_site_selection(s: int):
    S = 1 << s
    T_max = 1 << 17 - s
    expected = [None] * S
...
@pytest.mark.parametrize("S", [2**s for s in range(3, 13)])
@pytest.mark.parametrize(
    "T", [*range(10**2), *np.random.randint(2**63, size=10**2)]
)
def test_stretchedxtc_time_lookup_fuzz(S: int, T: int):
