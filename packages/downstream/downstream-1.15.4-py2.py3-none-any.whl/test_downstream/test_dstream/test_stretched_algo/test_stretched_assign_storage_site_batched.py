...
site_selection = validate_stretched_site_selection(
    algo.assign_storage_site_batched,
)


def test_stretched_site_selection_batched8_scalar():
    # fmt: off
    actual = (site_selection(8, T) for T in it.count())
    expected = [
        0, 1, 5, 2, 4, 6, 7, 3,  # T 0-7
        8, 8, 8, 7, 8, 8, 8, 4,  # T 8-15
        8, 8, 8, 8, 8, 8, 8, 8,  # T 16-23
        8, 8, 8, 8, 8, 8, 8, 5,  # T 24-31
        8, 8, 8, 8, 8, 8, 8, 8 # T 32-39
    ]
    assert all(x.tolist() == [y] for x, y in zip(actual, expected))


def test_stretched_site_selection_batched16_scalar():
    # fmt: off
    ...
    expected = [
        ...
        16, 16, 16, 8, 16, 16, 16, 12,  # T 16-24 --- hv 0,1,0, ...
        16, 16, 16, 15, 16, 16, 16, 5,  # T 24-31
        16, 16, 16, 16, 16, 16, 16, 16 # T 32-39
    ]
    assert all(x.tolist() == [y] for x, y in zip(actual, expected))


def test_stretched_site_selection_batched8():
    # fmt: off
    T = np.arange(40)
...
def test_stretched_site_selection_batched16():
    # fmt: off
    T = np.arange(40)
...
def test_stretched_site_selection_batched_dualS():
    # fmt: off
...
def test_stretched_site_selection_batched_fuzz(
    dtype1: typing.Type,
    dtype2: typing.Type,
):
