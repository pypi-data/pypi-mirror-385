...
def test_steady_has_ingest_capacity_batched_fuzz1():
...
def test_steady_has_ingest_capacity_batched_fuzz2():
    Ts = np.random.randint(np.iinfo(np.uint64).max, dtype=np.uint64, size=500)
...
def test_steady_has_ingest_capacity_batched_fuzz3():
...
def test_steady_has_ingest_capacity_batched_empty():
