...
def validate_stretchedxtc_site_selection(
    fn: typing.Callable,
) -> typing.Callable:
...
site_selection = validate_stretchedxtc_site_selection(algo.assign_storage_site)


def test_stretchedxtc_site_selection8():
    # fmt: off
...
def test_stretchedxtc_site_selection16():
    # fmt: off
    ...
    assert site_selection(16, 2 ** 16) is None
    assert site_selection(16, 2 ** 17 - 1) == 2
    assert site_selection(16, 2 ** 17) is None


def test_stretchedxtc_site_selection_fuzz():
...
def test_stretchedxtc_site_selection_epoch0(S: int):
    actual = {site_selection(S, T) for T in range(S)}
    expected = set(range(S))
    assert actual == expected


def test_stretchedxtc_site_selection_exceeds_capacity():
