...
time_lookup = validate_sticky_time_lookup(algo.lookup_ingest_times)


@pytest.mark.parametrize("s", range(1, 12))
def test_sticky_time_lookup_against_site_selection(s: int):
    S = 1 << s
    T_max = 1 << 17 - s
    expected = [None] * S
...
def test_sticky_time_lookup_fuzz(S: int, T: int):
