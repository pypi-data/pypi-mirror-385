...
def circular_has_ingest_capacity_naive(S: int, T: int) -> bool:
...
def test_circular_has_ingest_capacity_vs_naive(S: int):
    for T in range(2**16):
        actual = algo.has_ingest_capacity(S, T)
        expected = circular_has_ingest_capacity_naive(S, T)
