...
def test_xtchead_time_lookup_batched_against_site_selection(s: int):
    S = 1 << s
    T_max = 1 << 20 - s
    expected = [None] * S
...
@pytest.mark.parametrize("s", range(1, 12))
def test_xtchead_time_lookup_batched_empty(s: int):
    S = 1 << s
...
def test_xtchead_time_lookup_batched_fuzz(
    ...
    validate = validate_xtchead_time_lookup(algo.lookup_ingest_times_batched)
    for S in testS:
        if S <= Tmax:
