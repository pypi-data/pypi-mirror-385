...
KEY_STRING = 'yuemenhuSpringiosjdgSJCFDGFG'  # 密串


def lock_string(string, action='ENCODE'):
    # 统一处理为字符串
    if not isinstance(string, (bytes, bytearray)):
        string = str(string)

    # 生成密钥（MD5 后截取 8-26 位）
    md5_key = hashlib.md5(KEY_STRING.encode('utf-8')).hexdigest()  # 使用 UTF-8 编码密钥
    key = md5_key[8:26]
    key_bytes = key.encode('utf-8')  # 改用 UTF-8 编码密钥
    key_len = len(key_bytes)

    # 输入数据预处理
    if action != 'DECODE':
        # 加密模式：字符串转字节（使用 UTF-8）
        plain_bytes = string.encode('utf-8')  # 替换 latin-1 为 UTF-8
    else:
        # 解密模式：Base64 解码
        padding = (4 - len(string) % 4) % 4
        padded_data = string + ('=' * padding)
        encrypted_data = base64.b64decode(padded_data)
        plain_bytes = encrypted_data  # 直接使用字节流，避免中间解码步骤

    # XOR 核心加密算法
    code_bytes = bytearray()
    for i, byte in enumerate(plain_bytes):
        k = i % key_len
        code_bytes.append(byte ^ key_bytes[k])

    # 输出处理
    if action != 'DECODE':
        # Base64 编码并去除填充
        encoded = base64.b64encode(code_bytes).decode('utf-8')  # 使用 UTF-8 解码 Base64 结果
        return encoded.rstrip('=')
    else:
        # 解密结果转字符串（使用 UTF-8）
        return code_bytes.decode('utf-8')  # 替换 latin-1 为 UTF-8


def unlock_string(string):
    return lock_string(string, action='DECODE')
