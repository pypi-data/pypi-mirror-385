...
def Fourier( geo:                           Geometry2D | Geometry3D,\
             ...
             tol:                           float         = 1e-9,\
             maxiter:                       int           = 5000,\
             compute_internal_flux:         bool          = False,\
        ...
        def Laplacian(X,t):
         ...
         #cf       = 2*jnp.einsum('...s,s->...s',kappa_i*kappa_j/cond,1/geo.dists.sum(axis=0))
         
        
         kappa_ij = kappa_i * kappa_j/(kappa_i*geo.dists[None,1] + kappa_j*geo.dists[None,0])*geo.dists.sum(axis=0)[None,:] #W/mK

         #Normal flux---Off-diagonal terms
         bulk_d = jnp.einsum('...s,s->...s',kappa_ij,geo.areas/geo.dists.sum(axis=0)) #W/mK
        ...
        def get_flux(X,Tb,kappa,conductance,t):
         ...
         #Add periodic contributions here
         p_sides,p_values = (lambda t: jax.vmap(lambda b: bcs.get_periodic(b, t),0,(None,0))(batch_indices))(t)
         ...
         kappa_ij         = (kappa_i * kappa_j/(kappa_i*geo.dists[None,1] + kappa_j*geo.dists[None,0]))*(geo.dists[None,0]+geo.dists[None,1])
         
      

        

         #This includes bulk and periodic contributions
         denominator    = kappa_i*geo.dists[1] + kappa_j*geo.dists[0]
         T_boundary     = (X[:,geo.smap[:,0]]*kappa_i*geo.dists[1]+(X[:,geo.smap[:,1]].at[:,p_sides].add(p_values))*kappa_j*geo.dists[0])/denominator

         T_boundary_virtual = (X[:,geo.smap[:,0]].at[:,p_sides].add(-p_values)*kappa_i*geo.dists[1]+X[:,geo.smap[:,1]]*kappa_j*geo.dists[0])/denominator
         contribution_i   = jnp.einsum('...s,sd,s->...sd',T_boundary*kappa_i,geo.normals,geo.areas)
         contribution_j   = jnp.einsum('...s,sd,s->...sd',T_boundary_virtual*kappa_j,geo.normals,geo.areas)
         ...
         if compute_internal_flux:
          T1 = X[:,geo.smap[:,0]]
          T0 = X[:,geo.smap[:,1]]
          T0 = T0.at[:,p_sides].add(p_values)
          P_internal = jnp.einsum('bs,bs,s->bs',kappa_ij,T0-T1,1/(geo.dists.sum(axis=0))) #W/m^{d-1}
         else:
          P_internal = jnp.zeros((batch_size,geo.smap.shape[0])) 
         ...
         T_0 = X[:,geo.smap[p_sides,0]]
         T_1 = X[:,geo.smap[p_sides,1]] + p_values
         P = (T_1 - T_0)/geo.dists[:,p_sides].sum(axis=0)[None,:]*kappa_ij[:,p_sides]*geo.areas[None,p_sides] #W
         kappa_effective = jnp.einsum('bs,bs->b',P,p_values) #W: This is the power going in the direction of the applied heat flux
        ...
        #TODO: batch should be BEFORE batches
