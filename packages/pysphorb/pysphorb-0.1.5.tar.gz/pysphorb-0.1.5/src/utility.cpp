...
static inline void _drawKeypoint( Mat& img, const KeyPoint& p, const Scalar& color, cv::DrawMatchesFlags flags )
{
	CV_Assert( !img.empty() );
	Point center( cvRound(p.pt.x * draw_multiplier), cvRound(p.pt.y * draw_multiplier) );

	if( (flags & DrawMatchesFlags::DRAW_RICH_KEYPOINTS) != 0 )
	{
...
static void _prepareImgAndDrawKeypoints( const Mat& img1, const vector<KeyPoint>& keypoints1,
	...
	const Scalar& singlePointColor, cv::DrawMatchesFlags flags )
{
	cv::Size size( MAX(img1.cols, img2.cols), img1.rows + img2.rows);
	if( (flags & DrawMatchesFlags::DRAW_OVER_OUTIMG) != 0 )
	{
	...
	// draw keypoints
	if( (flags & DrawMatchesFlags::NOT_DRAW_SINGLE_POINTS) == 0 )
	{
...
static inline void _drawMatch( Mat& outImg, Mat& outImg1, Mat& outImg2 ,
	const KeyPoint& kp1, const KeyPoint& kp2, const Scalar& matchColor, cv::DrawMatchesFlags flags )
{
	RNG& rng = theRNG();
