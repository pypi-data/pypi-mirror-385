...
namespace cv
{
	...
	void SPHORB::initSORB()
	{
		...
		for (int i=0;i<levels;i++)
		{
			...
			char fileName[256];
			sprintf(fileName, "src/Data/geoinfo%d.pfm", cells[i]);
			...
			// the mask image
			sprintf(fileName, "src/Data/mask%d.bmp", cells[i]);
...
static void computeOrbDescriptor(const KeyPoint& kpt, const Mat& img, const Point* pattern, sphorb_byte* desc, int dsize)
{
