...
from veri_agents_api.fastapi.thread.schema import ToolMessageChunk
...
async def throw_baseball(angle: int, tool_call_id: Annotated[str, InjectedToolCallId]):
    ...
    writer(ToolMessageChunk(id=tool_call_id, tool_call_id=tool_call_id, content=f"Preparing to throw baseball at angle {angle}...\n\n"))
    await asyncio.sleep(2)
    writer(ToolMessageChunk(id=tool_call_id, tool_call_id=tool_call_id, content=f"Starting countdown...\n\n"))
    await asyncio.sleep(1)
    for i in [3,2,1]:
        writer(ToolMessageChunk(id=tool_call_id, tool_call_id=tool_call_id, content=f"{i}..."))
        await asyncio.sleep(1)
    writer(ToolMessageChunk(id=tool_call_id, tool_call_id=tool_call_id, content="\n\n"))
        
    writer(ToolMessageChunk(id=tool_call_id, tool_call_id=tool_call_id, content=f"Baseball thrown!\n\n"))
