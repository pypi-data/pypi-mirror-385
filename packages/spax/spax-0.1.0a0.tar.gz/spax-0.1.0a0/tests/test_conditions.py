...
class TestLambda:
    ...
    def test_with_description(self):
        """Test that description appears in repr."""
        cond = Lambda(lambda x: x > 0, description="is positive")
        assert "is positive" in repr(cond)
...
class TestFieldCondition:
    ...
    def test_nested_field_condition(self):
        ...
        cond = FieldCondition(
            "inner",
            FieldCondition("num_layers", SmallerThan(10))
        )
        config = OuterConfig()
        assert cond(config) is True
