...
def get_train_loaders(config):
    ...
    # If files are already in config, use them
    path_needed = ['X_train', 'y_train', 'X_val', 'y_val']

    if all([path in loaders_config for path in path_needed]):

        X_train = loaders_config['X_train']
        y_train = loaders_config['y_train']
        X_val = loaders_config['X_val']
        y_val = loaders_config['y_val']

        phase = 'train'
        slice_builder_config = loaders_config[phase]['slice_builder']
        transform_config = loaders_config[phase]['transformer']

        train_datasets = [NumpyDataset(
            phase=phase,
            slice_builder_config = slice_builder_config,
            transformer_config = transform_config,
...
def get_test_numpy_loader(config, raw):
    """
