...
from cyst.platform.logic.access import AuthorizationImpl, AccessSchemeImpl
...
class Policy(EnvironmentPolicy):
    ...
    def create_authorization(self, identity: str, nodes: List[Union[str, Node]], services: List[Union[str, Service]],
        ...
        actual_node = nodes[0] if isinstance(nodes[0], Node) else self._config.get_object_by_id(nodes[0], Node) #type: ignore
    ...
    List[Authorization]:
        ...
        actual_node = node if isinstance(node, Node) else self._config.get_object_by_id(node, Node) #type: ignore
    ...
    Tuple[bool, str]:
        ...
        actual_node = node if isinstance(node, Node) else self._config.get_object_by_id(node, Node)
