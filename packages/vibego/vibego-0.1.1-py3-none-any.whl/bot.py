...
MODE = os.environ.get("MODE", "A").upper()                      # A 或 B
...
def _format_task_detail(
    ...
    description_text = _escape_markdown_text(task.description or "暂无")
...
def _build_task_actions(task: TaskRecord) -> InlineKeyboardMarkup:
    ...
    keyboard.append(
        [
            InlineKeyboardButton(
                text="🖊️ 编辑描述",
                callback_data=f"task:desc_edit:{task.id}",
            ),
            InlineKeyboardButton(
                text="📝 添加备注",
                callback_data=f"task:add_note:{task.id}",
            ),
        ]
    )
    keyboard.append(
        [
            ...
            InlineKeyboardButton(
                text="🧾 生成摘要",
                callback_data=f"task:summary:{task.id}",
            ),
        ]
    )
    keyboard.append(
        [
            ...
            InlineKeyboardButton(
                text="🔄 刷新详情",
                callback_data=f"task:refresh:{task.id}",
            ),
        ]
    )
    keyboard.append(
