        ...
        for test_info in tests:
            ...
            async with AppiumClient(
                ...
                try:
                    ...
                    # Shutdown signal - mark as stopped
                    ...
                    # Agent API unavailable - stop entire session
                    message = f"Cannot connect to agent API at {agent_api_url}. Please try again later or contact support."
                    logger.error(message)
                    self._finalize_test_state(
                        state=state, status=TestStatus.ERROR, message=message
                    )
                    results.append(state)
                    # Stop processing remaining tests
                    break
