...
from noqa_runner.domain.exceptions import (
    AgentAPIError,
        ...
        for test_info in tests:
            ...
            try:
                async with AppiumClient(
                    ...
                    try:
                        await task
                        # Task completed successfully - state was finalized in _run_single_test
                        ...
                        # Re-raise HTTP errors to be handled by outer handler
                        raise

                    except Exception as e:
                        # Test-level: any other error during test execution
                        # Mark test as ERROR and continue with next test
                        logger.error(
                ...
                # Session-level: Appium server connection failed - stop session
                logger.error(
                    "Cannot connect to Appium server, stopping session",
                    appium_url=appium_url,
                )
                raise AppiumConnectionError(
                    appium_url=appium_url, original_error=str(e)
                )

            except httpx.HTTPStatusError as e:
                # Session-level: Agent API authentication/authorization errors - stop session
                if e.response.status_code == 401:
                    logger.error("Invalid API token, stopping session")
                    raise InvalidTokenError()
                else:
                    logger.error(
                        "Agent API error, stopping session",
                        status_code=e.response.status_code,
                    )
                    raise AgentAPIError(
                        status_code=e.response.status_code,
                        error_message=e.response.text or "Unknown API error",
                    )
