...
class AppiumConnectionError(RunnerException):
    ...
    def __init__(self, appium_url: str, original_error: str) -> None:
        ...
        message = (
            f"❌ Could not connect to Appium at {appium_url}. "
            f"Make sure Appium server is running. Error: {original_error}"
        )
...
class InvalidTokenError(RunnerException):
    """Invalid noqa API token"""

    def __init__(self) -> None:
        message = (
            "❌ Invalid noqa API token. "
            "Please check your --noqa-api-token and try again."
        )
        super().__init__(message)
