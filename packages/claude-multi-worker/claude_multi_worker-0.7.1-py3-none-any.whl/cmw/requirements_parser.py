...
class RequirementsParser:
    ...
    def _infer_dependencies(self, tasks: List[Task]) -> List[Task]:
        ...
        戦略:
        1. セクション番号依存: 同じ章内ではセクション順序を優先（7.1 → 7.2 → 7.3）
        2. ファイルベース依存: 同じファイルを編集するタスクは順序付け
        3. レイヤー依存: models → schemas → routers の順序
        4. 機能依存: 認証 → 認証が必要な機能
        """
        # タスクをIDでマッピング
        ...
        for task in tasks:
            ...
            for other_task in tasks:
                ...
                other_section = self._extract_section_number(other_task.title)

                # 戦略1: 同じ章内ではセクション番号順序を優先
                if task_section and other_section:
                    task_chapter = int(task_section.split('.')[0])
                    other_chapter = int(other_section.split('.')[0])

                    # 同じ章内で、other が先行する場合
                    if task_chapter == other_chapter and other_section < task_section:
                        if other_task.id not in task.dependencies:
                            task.dependencies.append(other_task.id)
                        continue  # セクション順序を優先したので、レイヤー推論はスキップ

                other_layer = self._get_task_layer(other_task, layer_order)

                # 戦略2&3: レイヤーベース依存（セクション番号がない場合のみ）
    ...
    def _extract_section_number(self, title: str) -> Optional[str]:
        ...
        Args:
            title: タスクタイトル（例: "7.1 認証・認可"）

        Returns:
            セクション番号（例: "7.1"）、存在しない場合はNone
        """
        import re
