...
class RequirementsGenerator:
    ...
    def generate_interactive(self, output_path: Path) -> bool:
        ...
        """
        self.console.print(
            Panel.fit("📝 Requirements.md 対話型生成ウィザード", border_style="blue")
        )
    ...
    def _collect_basic_info(self) -> None:
        ...
        self.requirements_data["project_name"] = Prompt.ask(
            "プロジェクト名", default="新規プロジェクト"
        )

        self.requirements_data["description"] = Prompt.ask(
            "プロジェクトの説明（1行で）", default="RESTful APIを使用したシステム"
        )
    ...
    def _collect_tech_stack(self) -> None:
        ...
        if needs_auth:
            auth_method = Prompt.ask(
                "認証方式", choices=["jwt", "session", "oauth2", "other"], default="jwt"
            )
    ...
    def _collect_data_models(self) -> None:
        ...
        while True:
            model_name = Prompt.ask("モデル名 (完了したら空Enter)", default="")
            ...
            while True:
                field_name = Prompt.ask("  フィールド名 (完了したら空Enter)", default="")
    ...
    def _collect_api_features(self) -> None:
        ...
        for model in self.requirements_data.get("models", []):
            ...
            needs_crud = Confirm.ask(f"\n{model['name']}のCRUD操作が必要?", default=True)
    ...
    def _collect_non_functional(self) -> None:
        ...
        # パフォーマンス
        target_response_time = Prompt.ask("目標レスポンスタイム(ms)", default="200")

        # テストカバレッジ
        test_coverage = Prompt.ask("テストカバレッジ目標(%)", default="80")
    ...
    def _generate_markdown(self) -> str:
        ...
        if tech["auth"]:
            auth_map = {"jwt": "JWT (python-jose)", "session": "Session認証", "oauth2": "OAuth 2.0"}
        ...
        if self.requirements_data.get("features"):
            ...
            # 機能をグループ化
            auth_features = [
                f for f in self.requirements_data["features"] if "auth" in f["endpoint"]
            ]
            other_features = [
                f for f in self.requirements_data["features"] if "auth" not in f["endpoint"]
            ]
