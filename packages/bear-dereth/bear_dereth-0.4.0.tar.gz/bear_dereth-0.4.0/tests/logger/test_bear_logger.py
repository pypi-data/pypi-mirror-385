...
def test_logger_logging(tmp_logger: BearLogger[Any]) -> None:
    ...
    captured = capture.getvalue().strip().replace("\n", " ")
    assert "This is a debug message" in captured
    assert "This is an info message" in captured
    assert "This is a warning message" in captured
    assert "This is an error message" in captured
    assert "This is a success message" in captured
    assert "This is a failure message" in captured
