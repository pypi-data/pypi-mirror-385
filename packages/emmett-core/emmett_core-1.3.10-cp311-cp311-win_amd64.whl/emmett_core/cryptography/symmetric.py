...
def encrypt(data: Union[bytes, str], key: str) -> Tuple[bytes, bytes, bytes]:
    ...
    dkey = _emmett_core.pbkdf2_sha256(key.encode("utf8"), salt, 1000, klen * 2 + ivlen)
    k1 = dkey[:klen]
    k2 = dkey[klen : (klen * 2)]
    nonce = dkey[(klen * 2) : (klen * 2) + ivlen]
    cipher = _emmett_core.aes256_ctr128(data, k1, nonce)
...
def decrypt_b64(data: Union[bytes, str], key: str, jchar: str = ":") -> bytes:
    try:
        ...
        raise ValueError("Invalid data input")
    return decrypt(urlsafe_b64decode(cipher), urlsafe_b64decode(salt), urlsafe_b64decode(signature), key)
