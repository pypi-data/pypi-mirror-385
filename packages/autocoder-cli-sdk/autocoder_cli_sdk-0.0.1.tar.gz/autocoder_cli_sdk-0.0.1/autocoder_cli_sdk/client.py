...
from .process_manager import get_sync_process_manager
...
class AutoCoderClient:
    ...
    def _execute_via_subprocess_generator(
        ...
        try:
            ...
            # 使用进程管理器启动进程
            process_manager = get_sync_process_manager()
            process_manager.start_process(cmd, prompt, options.cwd)
            ...
            if options.output_format == "json":
                ...
                try:
                    for line in process_manager.stream_output_sync():
                        stdout_lines.append(line)

                    # 获取stderr
                    _, stderr = process_manager.read_output_sync()

                    # 等待进程完成
                    exit_code = process_manager.wait_for_completion(
                        timeout=300
                    )
                    ...
                    if exit_code is None:
                        ...
                        if full_output.strip():
                            try:
                                # 尝试解析混合输出格式
                                text_result, json_data = self._parse_mixed_json_output(full_output)
                                if json_data:
                                    response_model = parse_json_response(json.dumps(json_data))
                                    # 如果有文本结果，添加到响应中
                                    if text_result:
                                        response_model.metadata = {"text_result": text_result}
                                    yield response_model
                                else:
                                    # 纯JSON格式或解析失败，尝试直接解析
                                ...
                                yield self._create_error_response(f"JSON解析失败: {str(e)}")
                        else:
                            # 空输出
                    ...
                    # 进程异常
                    yield self._create_error_response(f"进程执行异常: {str(e)}")
                    
            elif options.output_format == "stream-json":
                # Stream-JSON格式：逐行解析JSON事件
                from .pydantic_models import QueryResponseModel, StreamEventModel
                import json
                ...
                try:
                    ...
                    # 等待进程完成
                    exit_code = process_manager.wait_for_completion(timeout=300)
                    
                    if exit_code is None:
                        events.append(StreamEventModel(
                            event_type="error",
                            data={"error": "进程执行超时"},
                        ))
                    elif exit_code != 0:
                        events.append(StreamEventModel(
                            event_type="error", 
                            data={"error": f"命令执行失败，退出码: {exit_code}"},
                        ))
                    ...
                    yield self._create_error_response(f"Stream-JSON处理异常: {str(e)}")
            else:
                # 文本格式：逐行流式输出
                try:
                    for line in process_manager.stream_output_sync():
    ...
    def _build_command_args(self, options: QueryOptions) -> List[str]:
        ...
        verbose_required = options.verbose or options.output_format == "stream-json"
        bool_flags = [
    ...
    def abort_force(self) -> bool:
        ...
        """
        process_manager = get_sync_process_manager()
        return process_manager.abort(force=True)
