...
from .process_manager import get_async_process_manager
...
class AsyncAutoCoderClient:
    ...
    async def _execute_via_subprocess_async_generator(
        ...
        try:
            ...
            # 使用异步进程管理器
            process_manager = get_async_process_manager()
            await process_manager.start_process(cmd, prompt, options.cwd)

            # 根据输出格式处理结果
            if options.output_format == "json":
                # JSON格式：收集所有输出然后解析
                output_lines = []
                async for line in process_manager.stream_output_async():
                    output_lines.append(line)

                # 等待进程完成
                exit_code = await process_manager.wait_for_completion(
                    timeout=300
                )
    ...
    async def abort(self) -> bool:
        ...
        """
        process_manager = get_async_process_manager()
        return await process_manager.abort()
    ...
    async def abort_force(self) -> bool:
        ...
        """
        process_manager = get_async_process_manager()
        return await process_manager.abort(force=True)
