            ...
            if result.get('fixed_code') and result['fixed_code'] != source_code:
                ...
                if apply:
                    try:
                        ...
                        print(f" Original file updated (apply flag): {source_path}")
                        ...
                        print(f"Error applying fixed file to original: {e}")
                else:
                    # Prompt user whether to apply the fixes to the original
                    apply_answer = input("\nAlso apply these fixes to the original file? (y/n): ").strip().lower()
                    if apply_answer == 'y':
                        try:
                            fixed_hash, orig_hash = _apply_fixed_content(fixed_file_path, source_path)
                            print(f" Original file updated: {source_path}")
                        ...
                        print(" Original file left unchanged. You can review the '-fixed' file and apply manually if desired.")
            else:
