...
class LinkType(ResourceTypeBaseModel, table=True):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            ...
            "api_name",
            name="uq_linktype_complete",
        ),
        # Garante que inverse_api_name é único globalmente
        UniqueConstraint("inverse_api_name", name="uq_linktype_inverse_api_name"),
    )
    ...
    from_object_type: Optional["ObjectType"] = Relationship(
        sa_relationship_kwargs={
            "primaryjoin": "LinkType.from_object_type_rid == ObjectType.rid",
            "lazy": "joined",
    ...
    to_object_type: Optional["ObjectType"] = Relationship(
        sa_relationship_kwargs={
            "primaryjoin": "LinkType.to_object_type_rid == ObjectType.rid",
            "lazy": "joined",
            "foreign_keys": "[LinkType.to_object_type_rid]",
        }
    )
    ...
    def validate_and_resolve_object_types(self, session: Session) -> None:
        ...
        self.from_object_type = from_obj_type
        ...
        self.to_object_type = to_obj_type
...
# Importações circulares no final
