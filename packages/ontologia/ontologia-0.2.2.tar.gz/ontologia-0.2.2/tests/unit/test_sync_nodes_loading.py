...
class _StubDF:
    def __init__(self, columns):
        ...
        self.rows = [
            ...
            {columns[0]: "e2", columns[1]: "Bob"},
        ]

    def rename(self, mapping):
        new_cols = [mapping.get(c, c) for c in self.columns]
        return _StubDF(new_cols)

    def select(self, cols):
        filtered_cols = [c for c in self.columns if c in cols]
        df = _StubDF(filtered_cols or self.columns)
        df.rows = [{col: row.get(col) for col in filtered_cols} for row in self.rows]
        return df
...
def test_sync_nodes_load_merges_into_unified_table(client: TestClient, session):
    ...
    def _fake_read_dataset(_self, dataset):
        # raw dataset columns already match ObjectType properties
        return _StubDF(["id", "name"])
    ...
    assert "employee" in merge_calls[-1]
