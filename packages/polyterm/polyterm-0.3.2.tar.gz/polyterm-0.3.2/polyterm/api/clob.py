            ...
            "subscriptions": [
                {
                    "topic": "activity",
                    "type": "trades",
                    "filters": f"{{\"market_slug\":\"{','.join(market_slugs)}\"}}"
                }
            ]
        }
            ...
            async for message in self.ws_connection:
                try:
                    ...
                    if data.get("topic") == "activity" and data.get("type") == "trades":
                        # Extract market slug from the trade data
                        market_slug = data.get("market_slug") or data.get("market")
                        if market_slug and market_slug in self.subscriptions:
