...
class LiveMarketMonitor:
    ...
    def run_live_monitor(self):
        ...
        try:
            ...
            # Extract market IDs and asset IDs for WebSocket subscription
            market_ids = []
            asset_ids = []
            market_titles = {}
            ...
            for market in markets_data:
                ...
                if market_id:
                    ...
                    # Extract asset IDs from nested markets
                    if market.get('markets') and len(market.get('markets', [])) > 0:
                        nested_market = market['markets'][0]
                        clob_token_ids = nested_market.get('clobTokenIds')
                        if clob_token_ids:
                            try:
                                import json
                                asset_id_list = json.loads(clob_token_ids)
                                asset_ids.extend(asset_id_list)
                                # Map asset IDs to market titles
                                for asset_id in asset_id_list:
                                    market_titles[asset_id] = market.get("title", "Unknown")
                            except:
                                pass

            self.console.print(f"[green]✅ Found {len(market_ids)} markets to monitor[/green]")
            self.console.print(f"[green]✅ Extracted {len(asset_ids)} asset IDs for WebSocket[/green]")
            self.console.print(f"[cyan]📊 Subscribing to live trade feeds...[/cyan]")
            self.console.print("=" * 80)

            # Run async WebSocket monitoring with asset IDs
            asyncio.run(self._run_websocket_monitor(asset_ids, market_titles))
    ...
    async def _run_websocket_monitor(self, asset_ids: List[str], market_titles: Dict[str, str]):
        """Run WebSocket monitoring for live trades"""
        try:
            # Connect to WebSocket
            await self.clob_client.connect_websocket()
            self.console.print("[green]✅ Connected to PolyMarket WebSocket[/green]")
            
            # Subscribe to trade feeds
            await self.clob_client.subscribe_to_trades(asset_ids, lambda trade: self._handle_trade(trade, market_titles))
            self.console.print(f"[green]✅ Subscribed to {len(asset_ids)} asset feeds[/green]")
            self.console.print()
            ...
            await self._run_polling_monitor(asset_ids, market_titles)
        finally:
    ...
    async def _handle_trade(self, trade_data: Dict[str, Any], market_titles: Dict[str, str]):
        ...
        try:
            ...
            side_symbol = "🟢 BUY" if side.lower() == "buy" else "🔴 SELL"
            ...
            # Print trade
            self.console.print(
                f"[{side_color}]{timestamp} | {title_short:<33} | {side_symbol} | "
                f"{size_str:>8} shares @ {price_str:>8} | {notional_str:>8}[/{side_color}]"
            )
            
        except Exception as e:
            self.console.print(f"[red]{datetime.now(timezone.utc).strftime('%H:%M:%S')} | ERROR processing trade: {e}[/red]")

    async def _run_polling_monitor(self, asset_ids: List[str], market_titles: Dict[str, str]):
