...
class CLOBClient:
    ...
    async def subscribe_to_trades(self, market_ids: List[str], callback: Callable):
        ...
        # Subscribe to RTDS trades topic
        subscribe_msg = {
            "action": "subscribe",
            "subscriptions": [
                {
                    "topic": "trades",
                    "type": "trades",
                    "filters": f"market_id in ({','.join(market_ids)})"
                }
            ]
        }
        ...
        try:
            async for message in self.ws_connection:
                try:
                    ...
                    if data.get("topic") == "trades" and data.get("type") == "trades":
                        # Extract market ID from the trade data
                        market_id = data.get("market_id") or data.get("market")
                        if market_id and market_id in self.subscriptions:
                            callback = self.subscriptions[market_id]
                            await callback(data)
                    
                    # Handle other RTDS message types
                    elif data.get("topic") == "trades":
                        market_id = data.get("market_id") or data.get("market")
                        if market_id and market_id in self.subscriptions:
                            callback = self.subscriptions[market_id]
                            await callback(data)
