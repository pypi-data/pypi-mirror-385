...
class LiveMarketMonitor:
    ...
    def run_live_monitor(self):
        """Run the live monitoring loop with proper cleanup"""
        ...
        try:
            ...
            # Use Live display with proper cleanup
            with Live(self.generate_live_table(), refresh_per_second=4, console=self.console) as live:
                self._live_display = live
                while self._running:
                    try:
                        live.update(self.generate_live_table())
                        time.sleep(0.25)  # Update every 250ms for smoother live updates
                    except Exception as e:
