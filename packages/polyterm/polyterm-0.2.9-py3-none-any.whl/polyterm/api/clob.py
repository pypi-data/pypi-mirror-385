...
class CLOBClient:
    ...
    def __init__(
        ...
        ws_endpoint: str = "wss://ws-subscriptions-clob.polymarket.com",
    ):
    ...
    async def connect_websocket(self):
        ...
        try:
            # Connect to the market channel endpoint
            ws_url = f"{self.ws_endpoint}/ws/market"
            self.ws_connection = await websockets.connect(ws_url)
            return True
        except Exception as e:
            raise Exception(f"Failed to connect to WebSocket: {e}")
    
    async def subscribe_to_trades(self, market_ids: List[str], callback: Callable):
        """Subscribe to live trade feeds for multiple markets
        ...
        subscribe_msg = {
            "assets_ids": market_ids,  # PolyMarket uses assets_ids for market channel
            "type": "market"
        }
    ...
    async def listen_for_trades(self):
        """Listen for incoming trade messages"""
        if not self.ws_connection:
            raise Exception("WebSocket not connected")
        ...
        try:
            async for message in self.ws_connection:
                try:
                    ...
                    # Handle trade messages - PolyMarket sends various message types
                    if data.get("type") == "trade" or "trade" in str(data):
                        # Extract market/asset ID from the message
                        asset_id = data.get("asset_id") or data.get("market") or data.get("condition_id")
                        if asset_id and asset_id in self.subscriptions:
                            callback = self.subscriptions[asset_id]
                            await callback(data)
                    
                    # Also handle orderbook updates that might contain trade info
                    elif data.get("type") == "orderbook" or "orderbook" in str(data):
