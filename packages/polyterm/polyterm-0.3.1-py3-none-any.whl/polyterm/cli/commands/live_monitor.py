...
class LiveMarketMonitor:
    ...
    def run_live_monitor(self):
        ...
        try:
            ...
            # Extract market IDs for RTDS WebSocket subscription
            market_ids = []
            market_titles = {}
            ...
            # Run async WebSocket monitoring with market IDs
            asyncio.run(self._run_websocket_monitor(market_ids, market_titles))
    ...
    async def _run_websocket_monitor(self, market_ids: List[str], market_titles: Dict[str, str]):
        ...
        try:
            ...
            await self.clob_client.subscribe_to_trades(market_ids, lambda trade: self._handle_trade(trade, market_titles))
            self.console.print(f"[green]✅ Subscribed to {len(market_ids)} market feeds[/green]")
            self.console.print()
            ...
            await self._run_polling_monitor(market_ids, market_titles)
        finally:
    ...
    async def _handle_trade(self, trade_data: Dict[str, Any], market_titles: Dict[str, str]):
        ...
        try:
            ...
            # Extract trade details from RTDS format
            market_id = trade_data.get("market_id", "unknown")
            market_title = market_titles.get(market_id, "Unknown Market")
            
            # Extract trade details - RTDS format
            size = float(trade_data.get("size", trade_data.get("amount", 0)))
            price = float(trade_data.get("price", 0))
            side = trade_data.get("side", trade_data.get("direction", "unknown"))
            
            # Calculate notional value
            notional = size * price
            
            # Format trade display
            side_symbol = "🟢 BUY" if side.lower() in ["buy", "long", "yes"] else "🔴 SELL"
            side_color = "green" if side.lower() in ["buy", "long", "yes"] else "red"
            ...
            # Print trade with size information
            self.console.print(
    ...
    async def _run_polling_monitor(self, market_ids: List[str], market_titles: Dict[str, str]):
