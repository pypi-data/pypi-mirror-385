...
class WaveResNetRegressor(PSANNRegressor):
    ...
    def __init__(
        ...
        *,
        ...
        if init_layers is not None and grow_interval <= 0:
            raise ValueError("progressive_depth_interval must be positive when progressive depth is enabled.")

        growth = int(progressive_depth_growth)
        if init_layers is not None and growth <= 0:
            raise ValueError("progressive_depth_growth must be positive when progressive depth is enabled.")
        ...
        self.first_layer_w0_initial = float(first_layer_w0_initial) if first_layer_w0_initial is not None else self.first_layer_w0
        self.hidden_w0_initial = float(hidden_w0_initial) if hidden_w0_initial is not None else self.hidden_w0
    ...
    def _progressive_enabled(self) -> bool:
        return (
            self.progressive_depth_initial is not None
            and self.progressive_depth_initial < int(self.hidden_layers)
        )
