...
from vigil.tools import attestation, receipt_index, signing
...
def load_metrics(path: Path) -> ReceiptMetrics:
    ...
    try:
        with path.open("r", encoding="utf-8") as f:
            data: ReceiptMetrics = json.load(f)
        return data
...
def main(
    ...
    for root, _, files in os.walk(indir):
        for fn in files:
            ...
            artifact: Artifact = {
                ...
                "checksum": sha256(file_path.as_posix()),
                "kind": "file",
            }
    ...
    artifacts.sort(key=lambda art: art["uri"])
    ...
    started_at = os.environ.get("RUN_STARTED_AT") or time.strftime(
    ...
    out = Path(outdir) / f"receipt_{timestamp}.json"
    ...
    if attest:
        ...
        attestation_statement = attestation.generate_attestation(
            receipt_path=out,
            vigil_url=vigil_url,
            git_ref=git_ref,
        ...
        attestation_out = Path(outdir) / f"attestation_{timestamp}.json"
        attestation.save_attestation(attestation_statement, attestation_out)
    ...
    evidence_path = Path(outdir) / "evidence_graph.json"
    ...
    index_path = (Path(outdir) / "index.json").resolve()
