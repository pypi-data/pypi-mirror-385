...
def check_vigil_version() -> CheckResult:
    """Check vigil version and compare local vs global installations."""

    try:
        # Get current running version
        from vigil import __version__ as current_version
    except ImportError:
        return CheckResult(
            code="vigil_version",
            status=Severity.ERROR,
            message="Unable to determine vigil version",
        )
    ...
    # Check for global vigil installation
    try:
        ...
        if result.returncode == 0:
            # Parse version from output (format: "vigil version X.Y.Z")
            global_version_match = re.search(r"version\s+([\d.]+)", result.stdout)
            if global_version_match:
                global_version = global_version_match.group(1)
                details["global_version"] = global_version
                ...
                if global_version != current_version:
                    return CheckResult(
                        ...
                        status=Severity.WARNING,
                        message=(
                            f"Version mismatch: current={current_version}, global={global_version}. "
                            f"Run 'uv tool upgrade cofactor-vigil' or 'pipx upgrade cofactor-vigil' to update."
                        ),
                        details=details,
                    )
    except (FileNotFoundError, subprocess.SubprocessError):
        # Global vigil not found or failed - not necessarily an error
        pass

    return CheckResult(
        code="vigil_version",
        status=Severity.OK,
        message=f"Using vigil version {current_version}",
        details=details,
    )
