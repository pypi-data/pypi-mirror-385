...
def build_model_with_cbam_weighted(
    ...
    # Fuse temporal context and spatial features safely (no broadcasting issues)
    fused_ctx = Concatenate(axis=-1, name='Fuse_Spatial_Context_Concat')([conv_lstm_output, temporal_context])
    modulated_output = Conv2D(
        filters=C3, kernel_size=(1, 1), padding='same',
        activation='relu', name='Fuse_Spatial_Context_1x1'
    )(fused_ctx)
