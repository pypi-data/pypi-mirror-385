...
def load_station_points(station_dir: str, start_idx: int, end_idx: int) -> List[Tuple[float, float]]:
    """
    Load station points from CSV files named like station_1.csv ... station_N.csv (inclusive).
    CSV expected columns (case-insensitive fallback):
        lon/longitude/x, lat/latitude/y
    Returns list of (lon, lat) tuples in the given order.
    """
    ...
    for i in range(start_idx, end_idx + 1):
        ...
        lon_col = next((cols[k] for k in ("lon", "longitude", "x", "long")), None)
        lat_col = next((cols[k] for k in ("lat", "latitude", "y")), None)
        if lon_col is None or lat_col is None:
            raise KeyError(f"Expected lon/lat columns in {path}; found {list(df.columns)}")
        # Take first row if there are multiple; or iterate all rows if multiple points per file
        row = df.iloc[0]
...
def plot_voronoi_on_floodmap(stations: Sequence[Tuple[float, float]],
    ...
    pc = PatchCollection(patches, alpha=0.3, edgecolor="blue", facecolor="lightblue", linewidths=0.8)
...
def run_workflow(
    ...
    """
    # load inputs
    stations = load_station_points(station_dir, station_range[0], station_range[1])
...
if __name__ == "__main__":
    ...
    _station_dir = "observation_points"
    ...
    run_workflow(
        ...
        x_ticks=_longitudes,
        y_ticks=_latitudes,
        out_shapefile="voronoi_cells.shp",
