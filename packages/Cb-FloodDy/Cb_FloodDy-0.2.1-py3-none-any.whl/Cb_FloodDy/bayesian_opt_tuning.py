...
def run_optimization(
    ...
    if dem_files is None or dem_timesteps is None:
        raise ValueError(
            "Please provide both dem_files (list of paths) and dem_timesteps (list of lengths per DEM)."
        )
    if len(dem_files) != len(dem_timesteps):
        raise ValueError("dem_files and dem_timesteps must have the same length.")

    # Load each DEM raster (single band assumed)
    dem_rasters = []
    for f in dem_files:
        img, _, _ = load_single_tiff_image(f)
        dem_rasters.append(img)

    # Validate lengths sum to total timesteps
    ...
    # Tile each DEM across its assigned time span and concatenate in time
    dem_stacks = [np.tile(d, (t, 1, 1)) for d, t in zip(dem_rasters, dem_timesteps)]
    dem = np.concatenate(dem_stacks, axis=0)  # shape: (T, H, W)
