...
train_dem_file1 = os.path.join(os.getcwd(), 'DEM/dem_1.tif')
train_dem_file2 = os.path.join(os.getcwd(), 'DEM/dem_2.tif')
...
def build_multi_dem_stack(dem_files, dem_timesteps):
    assert len(dem_files) == len(dem_timesteps), "dem_files and dem_timesteps must align"
    dem_tiles = []
    for fpath, nsteps in zip(dem_files, dem_timesteps):
        dem_img, _, _ = load_single_tiff_image(fpath)
        if dem_img is None:
            raise ValueError(f"DEM image could not be loaded: {fpath}")
        dem_tiles.append(np.tile(dem_img, (int(nsteps), 1, 1)))
    return np.concatenate(dem_tiles, axis=0)

# build DEM stack dynamically (expects args provided to run_optimization)
train_dem_images = build_multi_dem_stack(dem_files, dem_timesteps)
...
# created per-trial later, keep placeholder here
checkpoint_dir = None
...
def run_optimization(
    ...
    train_dem_images = build_multi_dem_stack(dem_files, dem_timesteps)
    assert train_dem_images.shape[0] == train_atm_pressure_images.shape[0], "DEM timesteps must sum to total T"
