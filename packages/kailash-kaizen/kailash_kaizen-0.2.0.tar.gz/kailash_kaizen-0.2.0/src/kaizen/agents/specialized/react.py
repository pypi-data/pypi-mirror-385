...
Zero-config usage:
    ...
    agent = ReActAgent()
    result = agent.solve_task("Book a flight to Paris")
...
class ReActAgent(BaseAgent):
    ...
    Usage:
        ...
        result = agent.solve_task("Calculate 15% tip on $42.50")
    ...
    def _check_convergence(self, result: Dict[str, Any]) -> bool:
        ...
        1. Action is "finish" → task is complete
        2. Confidence >= threshold → agent is confident
        3. Otherwise → continue with more cycles
        ...
        Example:
            ...
            >>> result = {"action": "tool_use", "confidence": 0.3}
            >>> should_stop = agent._check_convergence(result)
            >>> print(should_stop)
            False  # Low confidence continues
        """
        ...
        # Continue if action is "tool_use" or "clarify" with low confidence
        return False

    def solve_task(self, task: str, context: str = "") -> Dict[str, Any]:
        """
        Solve task using ReAct cycles (Reason → Act → Observe).

        Executes iteratively until:
        - Action is "finish"
