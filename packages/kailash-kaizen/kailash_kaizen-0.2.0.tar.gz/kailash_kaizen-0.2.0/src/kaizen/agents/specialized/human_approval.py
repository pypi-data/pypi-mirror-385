...
Zero-config usage:
    ...
    result = await agent.decide("Process payment $1000")
...
class HumanApprovalAgent(BaseAgent):
    ...
    Usage:
        ...
        result = asyncio.run(agent.decide("Process payment $100"))
        print(f"Approved: {result['_human_approved']}")
        ...
        result = asyncio.run(agent.decide("Delete customer data"))
    ...
    async def decide(self, prompt: str) -> Dict[str, Any]:
        ...
        Example:
            >>> import asyncio
            >>> agent = HumanApprovalAgent()
            >>> result = asyncio.run(agent.decide("Transfer funds"))
    ...
    def decide_sync(self, prompt: str) -> str:
        """
        ...
        Example:
            >>> agent = HumanApprovalAgent()
            >>> decision = agent.decide_sync("Publish content")
        """
        import asyncio

        result = asyncio.run(self.decide(prompt))
