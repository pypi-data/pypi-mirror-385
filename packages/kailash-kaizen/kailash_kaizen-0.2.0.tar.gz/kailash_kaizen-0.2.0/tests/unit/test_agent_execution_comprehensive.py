...
class TestAgentLegacyCompatibility:
    ...
    @pytest.mark.skip(
        reason="SignatureBase removed - no backward compatibility needed (Kaizen in early development)"
    )
    def test_agent_execute_with_legacy_signature_base(self, performance_tracker):
        """Test that agent works with legacy SignatureBase for backward compatibility."""
        # Skipped: SignatureBase removed in favor of new Signature system
        # No backward compatibility needed as Kaizen is in early development with no production users
        pass
