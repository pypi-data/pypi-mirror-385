...
class TestDocumentUnderstandingWorkflow:
    ...
    def test_multi_step_document_analysis(self, test_image, memory_pool):
        ...
        ocr_result = ocr_agent.analyze(image=test_image, store_in_memory=True)
...
class TestMultiModalIntegration:
    ...
    def test_cross_modal_workflow(self, test_image, test_audio, memory_pool):
        ...
        vision_config = VisionConfig(llm_provider="ollama", model="llava:13b")
        ...
        audio_config = TranscriptionConfig(model_size="base")
...
class TestCostTrackingIntegration:
    ...
    def test_track_ollama_usage(self, test_image):
        ...
        agent.analyze(image=test_image)
...
class TestMemoryIntegration:
    ...
    def test_store_multimodal_memories(self, test_image, test_audio, memory_pool):
        ...
        result = agent.analyze(image=test_image, audio=test_audio, store_in_memory=True)
...
class TestPerformance:
    ...
    def test_vision_processing_performance(self, test_image):
        ...
        config = VisionConfig(llm_provider="ollama", model="llava:13b")
        agent = VisionAgent(config=config)

        start = time.time()
        result = agent.analyze(image=test_image, prompt="Describe this")
