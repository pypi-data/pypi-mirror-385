...
class Color:
    ...
    def validate_input(args):
        if not (len(args) == 1 and isinstance(args[0], str)) and not (3 <= len(args) <= 4 and all(0 <= val <= 255 for val in args)):
            raise ValueError('Invalid input. Pass either a hex color string, CSS color name, or 3-4 integers for RGB(A).')
