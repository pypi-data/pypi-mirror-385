...
class ImageBenchmark:
    ...
    def benchmark_convert_operations(self):
        """Benchmark convert operations with various modes"""
        print("\n=== Convert Operations Benchmarks ===")
        
        test_file_rgb = self.test_images_dir / "test_1000x1000_rgb.png"
        test_file_gray = self.test_images_dir / "test_1000x1000_l.png"
        
        if not test_file_rgb.exists():
            return
        
        # Basic mode conversions
        basic_conversions = [
            ("RGB → L (Grayscale)", test_file_rgb, "L"),
            ("RGB → RGBA (Add Alpha)", test_file_rgb, "RGBA"),
            ("RGB → LA (Gray+Alpha)", test_file_rgb, "LA"),
        ]
        
        if test_file_gray.exists():
            basic_conversions.extend([
                ("L → RGB (Gray to Color)", test_file_gray, "RGB"),
                ("L → RGBA (Gray to Color+Alpha)", test_file_gray, "RGBA"),
            ])
        
        print("\nBasic Mode Conversions:")
        for conv_name, test_file, target_mode in basic_conversions:
            print(f"\n  {conv_name}:")
            ...
            if PILLOW_AVAILABLE:
                ...
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(pillow_convert)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        f"convert_{target_mode.lower()}", "Pillow", avg_time, avg_memory
                    ))
                    print(f"    Pillow: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
            ...
            if PUHU_AVAILABLE:
                ...
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(puhu_convert)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        f"convert_{target_mode.lower()}", "Puhu", avg_time, avg_memory
                    ))
                    print(f"    Puhu: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
        
        # Bilevel (black & white) conversions with dithering
        print("\n\nBilevel Conversions (1-bit):")
        bilevel_tests = [
            ("RGB → 1 (Floyd-Steinberg)", "1", None),
            ("RGB → 1 (No Dither)", "1", "NONE"),
        ]
        
        for conv_name, target_mode, dither in bilevel_tests:
            print(f"\n  {conv_name}:")
            
            # Pillow conversion
            if PILLOW_AVAILABLE:
                def pillow_convert_bilevel():
                    img = PILImage.open(test_file_rgb)
                    if dither is None:
                        # Default: Floyd-Steinberg dithering
                        return img.convert(target_mode)
                    else:
                        # NONE: no dithering (dither=0)
                        return img.convert(target_mode, dither=0)
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(pillow_convert_bilevel)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    avg_time = statistics.mean(times)
                    avg_memory = statistics.mean(memories)
                    dither_suffix = f"_{dither.lower()}" if dither else "_dither"
                    self.results.append(BenchmarkResult(
                        f"convert_bilevel{dither_suffix}", "Pillow", avg_time, avg_memory
                    ))
                    print(f"    Pillow: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
            
            # Puhu conversion
            if PUHU_AVAILABLE:
                def puhu_convert_bilevel():
                    img = puhu.Image.open(str(test_file_rgb))
                    if dither is None:
                        return img.convert(target_mode)
                    else:
                        return img.convert(target_mode, dither=dither)
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(puhu_convert_bilevel)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    dither_suffix = f"_{dither.lower()}" if dither else "_dither"
                    self.results.append(BenchmarkResult(
                        f"convert_bilevel{dither_suffix}", "Puhu", avg_time, avg_memory
                    ))
                    print(f"    Puhu: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
        ...
        print("\n\nPalette Conversions:")
        palette_tests = [
            ("RGB → P (WEB palette)", "P", "WEB", 256),
            ("RGB → P (ADAPTIVE 256)", "P", "ADAPTIVE", 256),
            ("RGB → P (ADAPTIVE 128)", "P", "ADAPTIVE", 128),
            ("RGB → P (ADAPTIVE 64)", "P", "ADAPTIVE", 64),
        ]
        
        for conv_name, target_mode, palette_type, colors in palette_tests:
            print(f"\n  {conv_name}:")
            
            # Pillow conversion
            if PILLOW_AVAILABLE:
                def pillow_convert_palette():
                    img = PILImage.open(test_file_rgb)
                    if palette_type == "WEB":
                        # WEB palette mode
                        return img.convert(target_mode, palette=0)  # 0 = WEB
                    else:  # ADAPTIVE
                        return img.convert(target_mode, palette=1, colors=colors)  # 1 = ADAPTIVE
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(pillow_convert_palette)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        f"convert_palette_{palette_type.lower()}_{colors}", "Pillow", avg_time, avg_memory
                    ))
                    print(f"    Pillow: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
            
            # Puhu conversion
            if PUHU_AVAILABLE:
                def puhu_convert_palette():
                    img = puhu.Image.open(str(test_file_rgb))
                    return img.convert(target_mode, palette=palette_type, colors=colors)
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(puhu_convert_palette)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        f"convert_palette_{palette_type.lower()}_{colors}", "Puhu", avg_time, avg_memory
                    ))
                    print(f"    Puhu: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
        ...
        if test_file_gray.exists():
            ...
            # 4-tuple matrix (L → RGB sepia tone)
            print("\n  L → RGB with 4-tuple matrix (Sepia):")
            sepia_matrix = (1.0, 0.95, 0.82, 0.0)
            
            # Pillow matrix conversion
            if PILLOW_AVAILABLE:
                def pillow_matrix_4():
                    img = PILImage.open(test_file_gray)
                    return img.convert("RGB", matrix=sepia_matrix)
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(pillow_matrix_4)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        "convert_matrix_4tuple", "Pillow", avg_time, avg_memory
                    ))
                    print(f"    Pillow: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
            
            # Puhu matrix conversion
            if PUHU_AVAILABLE:
                def puhu_matrix_4():
                    img = puhu.Image.open(str(test_file_gray))
                    return img.convert("RGB", matrix=sepia_matrix)
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(puhu_matrix_4)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        "convert_matrix_4tuple", "Puhu", avg_time, avg_memory
                    ))
                    print(f"    Puhu: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
            
            # 12-tuple matrix (RGB → RGB color transformation)
            print("\n  RGB → RGB with 12-tuple matrix (Color adjustment):")
            color_matrix = (
                1.2, 0.0, 0.0, 0.0,  # Red channel
                0.0, 0.9, 0.0, 0.0,  # Green channel
                0.0, 0.0, 1.1, 0.0   # Blue channel
            )
            
            # Pillow matrix conversion
            if PILLOW_AVAILABLE:
                def pillow_matrix_12():
                    img = PILImage.open(test_file_rgb)
                    return img.convert("RGB", matrix=color_matrix)
                
                times = []
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(pillow_matrix_12)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        "convert_matrix_12tuple", "Pillow", avg_time, avg_memory
                    ))
                    print(f"    Pillow: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
            ...
            if PUHU_AVAILABLE:
                ...
                memories = []
                for _ in range(self.iterations):
                    exec_time, memory, result, error = self.measure_performance(puhu_matrix_12)
                    if error is None:
                        times.append(exec_time)
                        memories.append(memory)
                ...
                if times:
                    ...
                    avg_memory = statistics.mean(memories)
                    self.results.append(BenchmarkResult(
                        "convert_matrix_12tuple", "Puhu", avg_time, avg_memory
                    ))
                    print(f"    Puhu: {avg_time:.2f}ms ± {statistics.stdev(times):.2f}ms")
...
"""
    ...
    def generate_recommendations(self, performance_data: List[Dict]) -> str:
        ...
        # Analyze convert performance
        convert_ops = [d for d in performance_data if 'convert' in d['operation']]
        if convert_ops:
            puhu_faster_converts = sum(1 for d in convert_ops if d['faster'] == 'Puhu')
            if puhu_faster_converts > len(convert_ops) / 2:
                recommendations.append(
                    "<div class='recommendation-item'>"
                    "<strong>✅ Use Puhu for color conversions:</strong> Puhu's convert() operations with parallel processing "
                    "show excellent performance, especially for matrix transformations and palette quantization. "
                    "Ideal for batch color space conversions and image preprocessing pipelines."
                    "</div>"
                )
            
            # Check matrix performance specifically
            matrix_ops = [d for d in convert_ops if 'matrix' in d['operation']]
            if matrix_ops and all(d['faster'] == 'Puhu' for d in matrix_ops):
                recommendations.append(
                    "<div class='recommendation-item'>"
                    "<strong>🚀 Matrix conversions excel in Puhu:</strong> Puhu's parallel processing provides 2-5x speedup "
                    "for matrix-based color transformations. Perfect for sepia tones, custom grayscale conversions, "
                    "and color grading workflows."
                    "</div>"
                )
            
            # Check palette performance
            palette_ops = [d for d in convert_ops if 'palette' in d['operation']]
            if palette_ops:
                adaptive_ops = [d for d in palette_ops if 'adaptive' in d['operation']]
                if adaptive_ops and all(d['faster'] == 'Puhu' for d in adaptive_ops):
                    recommendations.append(
                        "<div class='recommendation-item'>"
                        "<strong>🎨 Adaptive palette quantization strength:</strong> Puhu uses NeuQuant neural network "
                        "for high-quality color quantization with competitive performance. "
                        "Great for creating optimized images for web delivery."
                        "</div>"
                    )
...
"""
    ...
    def run_all_benchmarks(self):
        ...
        self.benchmark_convert_operations()
