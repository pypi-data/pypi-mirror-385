...
Kong Gateway와 통합된 JWT 토큰 관리 유틸리티입니다.
- 토큰 생성 (로그인 시)
- 토큰 검증 (개발/테스트용)
- 토큰 디코딩 (디버깅용)
- 서비스 간 통신용 토큰 생성
...
Usage Example:
    from mysingle_quant.auth.jwt_utils import JWTManager
    ...
    # 서비스 간 통신 토큰 생성
    service_token = jwt_manager.create_service_token(
...
class JWTManager:
    ...
    def create_service_token(
        ...
        Example:
            token = jwt_manager.create_service_token("strategy-service")
        """
    ...
    def _get_jwt_secret_for_consumer(self, consumer_key: str) -> str:
        ...
        if not secret:
            ...
            logger.warning(
                ...
                f"using default (NOT for production!)"
            )
            return f"dev-secret-for-{consumer_key}"
...
def create_access_token(
    ...
    Example:
        from mysingle_quant.auth.jwt_utils import create_access_token
