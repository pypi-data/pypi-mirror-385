...
def get_kong_user_id(request: Request) -> Optional[str]:
    ...
    Kong JWT Plugin이 JWT의 'sub' 클레임을 X-User-Id 헤더로 전달합니다.
...
def get_kong_consumer_id(request: Request) -> Optional[str]:
    """
    Kong Gateway Consumer ID 추출

    Kong이 인증한 Consumer의 custom_id를 X-Consumer-Custom-Id 헤더로 전달합니다.

    Args:
        request: FastAPI Request 객체

    Returns:
        Optional[str]: Kong Consumer의 custom_id 또는 None
    """
    return request.headers.get("x-consumer-custom-id") or request.headers.get(
...
def get_current_user(request: Request) -> User:
    """
    현재 인증된 사용자 반환 (Kong Gateway + AuthMiddleware + Cache 통합)

    인증 우선순위:
    1. Kong Gateway 헤더 (X-User-Id) 확인 - Kong을 통한 요청
       → User 캐시 확인 (Redis/In-Memory)
       → 캐시 미스 시 DB 조회 후 캐시 저장
    2. Request.state.user 확인 - AuthMiddleware가 이미 처리한 경우

    Kong Gateway를 통한 요청:
    - Kong JWT Plugin이 JWT를 검증하고 X-User-Id 헤더 설정
    - 캐시에서 User 조회 (5분 TTL)
    - 캐시 미스 시 DB에서 조회 후 캐시에 저장

    직접 요청 (Kong 미사용):
    - AuthMiddleware가 직접 인증하고 request.state.user 설정
    - 이미 User 객체가 있으므로 즉시 반환
    ...
    if kong_user_id:
        # Kong을 통한 요청 - 캐시 기반 User 조회
        logger.debug(f"Kong authenticated request for user_id: {kong_user_id}")
        ...
        try:
            ...
            # async 함수를 동기적으로 호출 (FastAPI의 경우 이미 async context)
            import asyncio

            if asyncio.iscoroutinefunction(cache.get_user):
                # 현재 실행 중인 이벤트 루프 확인
                try:
                    _ = asyncio.get_running_loop()
                    # 이미 async context 내부이므로 await 불가능
                    # request.state에 user를 미리 로드했다고 가정
                    cached_user = getattr(request.state, "cached_user", None)
                except RuntimeError:
                    # 동기 컨텍스트 - asyncio.run() 사용
                    cached_user = asyncio.run(cache.get_user(kong_user_id))

            if cached_user:
                logger.debug(f"User cache HIT for user_id: {kong_user_id}")
                return cached_user

            logger.debug(f"User cache MISS for user_id: {kong_user_id}")

        except Exception as e:
            logger.warning(f"Cache lookup failed: {e}, falling back to request.state")

        # 1.2 캐시 미스 - request.state.user 확인 (AuthMiddleware가 DB에서 로드)
        ...
        # 1.3 캐시에 저장 (다음 요청을 위해)
        try:
            cache = get_user_cache()
            import asyncio

            if asyncio.iscoroutinefunction(cache.set_user):
                # async context에서는 백그라운드 태스크로 저장
                # (응답 지연 방지)
                pass  # TODO: Background task로 처리
            else:
                asyncio.run(cache.set_user(user, ttl=300))
        except Exception as e:
