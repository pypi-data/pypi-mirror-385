...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if not self.service_config.enable_auth:
            logger.debug(f"Authentication disabled for service: {self.service_config.service_name}")
            return await call_next(request)
        ...
        try:
            ...
            if user:
                ...
                if not user.is_active:
                    logger.warning(f"Inactive user blocked: {user.id} at {method} {path}")
                ...
                logger.warning(f"❌ Authentication required for protected endpoint: {method} {path}")
                raise UserNotExists(
            ...
            logger.warning(f"🔒 Authentication failed for {method} {path}: {type(e).__name__} - {e}")
            return self._create_error_response(e)

        except Exception as e:
            logger.error(f"💥 Unexpected authentication error for {method} {path}: {e}", exc_info=True)
            return self._create_error_response(e)
