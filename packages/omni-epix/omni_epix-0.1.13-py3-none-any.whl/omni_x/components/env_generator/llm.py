...
class LLMEnvGenerator(EnvGenerator):
    ...
    def __call__(self, run: Run, task: Task) -> None:
        ...
        for attempt in range(max(self.config.max_fix_attempts + 1, 1)):
            code_file = run.dir / "smoke_test_logs" / f"{task.id}_attempt_{attempt + 1}.py"
            code_file.parent.mkdir(parents=True, exist_ok=True)
            code_file.write_text(generated_code)

            smoke_test_passed, error_traceback = self._run_smoke_test(generated_code, run=run, task=task, attempt=attempt)
            ...
            log(
                f"Smoke test failed attempt {attempt + 1}/{self.config.max_fix_attempts + 1}, see {error_log_file.relative_to(run.dir)}",
                run=run,
                task=task,
            )
    ...
    def _run_smoke_test(self, code: str, run: Run, task: Task, attempt: int) -> tuple[bool, str]:
        ...
        with tempfile.NamedTemporaryFile(mode="w", suffix=".py") as f:
            ...
            result = run_sandboxed(
                ...
                mounts=mounts,
                check=False,
                log_file=run.dir / "smoke_test_logs" / f"{task.id}_attempt_{attempt + 1}.log",
            )
