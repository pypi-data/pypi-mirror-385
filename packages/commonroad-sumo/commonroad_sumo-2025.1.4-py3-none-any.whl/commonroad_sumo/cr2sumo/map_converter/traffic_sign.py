...
class TrafficSignEncoder:
    ...
    def _bfs_until(
        self, start: Edge, element: TrafficSignElement
    ...
    def _set_priority(
        self, element: TrafficSignElement, edge: Edge, max_curvature: float = 1
    ):
        """
        ...
        new_type = self.edge_types.create_from_update_priority(
            old_type.id, old_type.priority + 1
        )
        ...
        def compute_max_path_curvature(edge: Edge, checked_parents=()):
            ...
            curvature = np.max(
                [curvature]
                + [
                    compute_max_path_curvature(
                        parent, checked_parents + tuple(parents[edge])
                    )
    ...
    def _set_ban_car_truck_bus_motorcycle(
        self, element: TrafficSignElement, edge: Edge
    ):
        """
