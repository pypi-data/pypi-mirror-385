...
class ESP32DeviceManager:
    ...
    def __init__(self, port: str = "/dev/ttyUSB0", baudrate: int = 115200):
        ...
        self.firmware_dir = Path(__file__).parent.parent / "esp32-rk3588-sensor-system" / "esp32_firmware"
        self.default_firmware = self.firmware_dir / ".pio" / "build" / "esp32dev" / "firmware.bin"
    ...
    def detect_device(self) -> bool:
        ...
        try:
            ...
            # 使用esptool检测ESP32
            cmd = ["esptool.py", "--port", self.port, "chip_id"]
    ...
    def check_firmware(self) -> Tuple[bool, str]:
        ...
        try:
            # 尝试导入ESP32通信模块进行测试
            sys.path.insert(0, str(Path(__file__).parent.parent / "esp32-rk3588-sensor-system" / "rk3588_controller"))
            from esp32_comm import ESP32Communicator

            # 尝试连接并发送PING命令
            comm = ESP32Communicator(self.port, self.baudrate)
            if comm.connect():
                response = comm.send_command("GET", "INFO")
                comm.disconnect()

                if response and response.startswith("OK:"):
                    # 解析固件信息
                    info = response[3:]  # 移除"OK:"前缀
                    logger.info(f"检测到固件: {info}")
                    return True, info
                else:
                    logger.warning("未收到有效响应，可能没有固件或固件不兼容")
                    return False, "无响应"
            else:
    ...
    def find_firmware_file(self) -> Optional[Path]:
        ...
        """
        # 首先查找默认编译输出路径
        if self.default_firmware.exists():
            logger.info(f"找到默认固件文件: {self.default_firmware}")
            return self.default_firmware

        # 查找固件目录下的所有.bin文件
        firmware_files = list(self.firmware_dir.rglob("*.bin"))
    ...
    def compile_firmware(self) -> bool:
        ...
        if not self.firmware_dir.exists():
            logger.error(f"固件源码目录不存在: {self.firmware_dir}")
            return False
    ...
    def flash_firmware(self, firmware_path: Optional[Path] = None) -> bool:
        ...
        try:
            ...
            erase_cmd = ["esptool.py", "--port", self.port, "erase_flash"]
