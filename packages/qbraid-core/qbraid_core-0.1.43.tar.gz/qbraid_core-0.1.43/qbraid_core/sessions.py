...
class QbraidSession(Session):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.jupyter_token = kwargs.pop("jupyter_token", None)
        self.verify = False
    ...
    def jupyter_token(self) -> Optional[str]:
        """Return the JupyterHub token."""
        return self._jupyter_token

    @jupyter_token.setter
    def jupyter_token(self, value: Optional[str]) -> None:
        """Set the JupyterHub token."""
        jupyter_token = value or self.get_config("jupyter-token")
        self._jupyter_token = jupyter_token or os.getenv("JUPYTERHUB_API_TOKEN")
