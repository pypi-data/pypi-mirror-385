...
def config_define(
    output_path: Optional[Path] = typer.Option(
        ...
        "--out",
        "-o",
        help="Optional path to write the template YAML. Prints to stdout when omitted.",
    ),
    overwrite: bool = typer.Option(False, "--overwrite", help="Allow overwriting an existing file."),
):
    """Generate a starter Bantam pretraining config with inline guidance."""

    model_template = textwrap.dedent(
        """\
        # Save as configs/models/template_model.yaml
        bantam_config:
          model_type: bantam
          vocab_size: 208334
          hidden_size: 768
          ...
          final_logit_softcapping: 32.0

          layer_configs:
            # ---------- Dense stack ----------
            - { window: 128, intermediate_size: 2048 }   # L00 (≈2.7×)
            - { window: 128, intermediate_size: 2048 }   # L01
            - { window: 128, intermediate_size: 2048 }   # L02
            - { window: null, intermediate_size: 2048 }  # L03 (global)
            - { window: 192, intermediate_size: 2048 }   # L04
            - { window: 192, intermediate_size: 2048 }   # L05
            - { window: 192, intermediate_size: 2048 }   # L06
            - { window: null, intermediate_size: 2304 }  # L07 (global, bump to 3×)
            - { window: 256, intermediate_size: 2304 }   # L08
            - { window: 256, intermediate_size: 2304 }   # L09
            - { window: 256, intermediate_size: 2304 }   # L10
            - { window: 256, intermediate_size: 2304 }   # L11
            - { window: null, intermediate_size: 2304 }  # L12 (global)
            - { window: 256, intermediate_size: 2304 }   # L13
            - { window: 256, intermediate_size: 2304 }   # L14
            - { window: 256, intermediate_size: 2304 }   # L15
            - { window: null, intermediate_size: 2304 }  # L16 (global)
            - { window: 256, intermediate_size: 2304 }   # L17
            - { window: 256, intermediate_size: 2304 }   # L18
            - { window: 256, intermediate_size: 2304 }   # L19
            - { window: null, intermediate_size: 2304 }  # L20 (global)
            - { window: 512, intermediate_size: 2304 }   # L21
            # ---------- MoE stack ----------
            - { window: 512, intermediate_size: 2304, expert_type: "topk", num_experts: 6, moe_top_k: 2, moe_intermediate_size: 3072, moe_capacity_factor: 1.25, moe_aux_loss_weight: 0.01, moe_router_jitter: 0.1, moe_drop_policy: "random" }  # L22
            - { window: 512, intermediate_size: 2304, expert_type: "topk", num_experts: 6, moe_top_k: 2, moe_intermediate_size: 3072, moe_capacity_factor: 1.25, moe_aux_loss_weight: 0.01, moe_router_jitter: 0.1, moe_drop_policy: "random" }  # L23
            - { window: null, intermediate_size: 2304, expert_type: "topk", num_experts: 8, moe_top_k: 2, moe_intermediate_size: 3072, moe_capacity_factor: 1.25, moe_aux_loss_weight: 0.01, moe_router_jitter: 0.1, moe_drop_policy: "random" }  # L24 (global+MoE)
            - { window: 512, intermediate_size: 2304, expert_type: "topk", num_experts: 6, moe_top_k: 2, moe_intermediate_size: 3072, moe_capacity_factor: 1.25, moe_aux_loss_weight: 0.01, moe_router_jitter: 0.1, moe_drop_policy: "random" }  # L25
            # ---------- Final dense ----------
            - { window: null, intermediate_size: 2304 }   # L26 (global)
            - { window: 512, intermediate_size: 2304 }    # L27
        """
    ).strip()

    training_template = textwrap.dedent(
        """\
        # Save as configs/training/template_pretrain.yaml
        seed: 1337

        training_args:
          # TODO: Replace with your dataset path (JSONL or Hugging Face dataset fields)
          dataset: ../../data/your_dataset.jsonl
          # TODO: Point to tokenizer directory (use `bantam-cli trainbpe` or bundled tokenizer)
          tokenizer: ../../tokenizers/your_tokenizer
          # TODO: Output directory for checkpoints and logs
          out_dir: ../../models/run1
          ...
          warmup_frac: 0.10
          log_every_n: 10
          steps_per_epoch: 100000   # TODO: set when streaming HF datasets; remove for finite JSONL
          ...
          # Optional SFT defaults (ignored during pure pretraining)
          sft_mode: lora
          lora_r: 64
          lora_alpha: 96
          lora_dropout: 0.05
    ...
    if output_path.exists() and not overwrite:
        print(f"❌ Refusing to overwrite existing file: {output_path}. Use --overwrite to proceed.")
        raise typer.Exit(code=1)
