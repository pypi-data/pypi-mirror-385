...
from win32crypt import CryptProtectData, CryptUnprotectData  # pyright: ignore[reportUnknownVariableType]
...
class EnvEncrypt:
    ...
    @staticmethod
    def encrypt_env(encrypted_env_path: StrPath, save: bool = True) -> EnvDict:
        """
        Encrypts a dotenv file and keeps the comments from the original file.
        Args:
            encrypted_env_path (StrPath): Path to the file to be encrypted.
            save (bool): Whether to save the encrypted variables to a file. Defaults to True.
        """
        ...
        try:
            ...
            f.close()
            if save:
                with open(encrypted_env_path, "w", encoding="utf-8") as f:
                    f.writelines(lines)
        except Exception as e:
            logger.error(f"Failed to encrypt env file {encrypted_env_path}: {e}")
        finally:
            f.close()
