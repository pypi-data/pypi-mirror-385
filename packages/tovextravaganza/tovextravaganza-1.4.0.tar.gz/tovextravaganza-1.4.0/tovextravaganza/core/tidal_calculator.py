...
class TidalCalculator:
    ...
    def _tov_tidal_equations(self, y, r):
        ...
        # Tidal: d(beta)/dr
        # f = de/dp = 1/cs2 (on-the-fly calculation, more accurate than precomputed)
        ...
        if abs(p_plus - p_minus) > 1e-30:
            f = (e_plus - e_minus) / (p_plus - p_minus)
        else:
            f = 1.0
        
        f = max(1.0, f)  # Only lower bound, NO upper clip (this was the bug!)
