...
class DataProcessingAndMessaging:
    def __init__(self, enable_console_log=None):
        ...
        self.WECHAT_DOC_LOG_FILE = "docs_operation_log.log"  # 文档功能独立日志文件
    ...
    def run_sql(self, path=None, sql_name=None, channel=False, sql_content=None):
        ...
        支持两种调用方式：
        1. 直接执行SQL语句：run_sql(sql_content='select * from table')
        2. 执行本地SQL文件：run_sql(path, 'filename.sql') 或 run_sql(path, 'filename.sql', channel=True)
        ...
        try:
            if channel:
                ...
                self.logger.debug(f"替换前SQL：\n{original_sql}\n替换后SQL：\n{sql}")

            self.logger.info(f"开始连接Hive服务器")
            ...
            print(f"插入表格：开始执行 sql：{sql_file_path}" if channel
                  else f"开始执行 sql：{sql_file_path}")
