...
| limit {max_results}'''
    ...
    def get_grafana_dashboard_url(self, service_name: str, start_time: datetime, end_time: datetime) -> Optional[str]:
        ...
        """
        # Grafana Dashboard URLs mapping
        GRAFANA_BASE_URL = "https://grafana.trendmicro.com"
        GRAFANA_DASHBOARD_URLS = {
            'vcs-inventory': f"{GRAFANA_BASE_URL}/d/vcs-inventory/vcs-inventory-dashboard",
            'ecs-inventory-clusters': f"{GRAFANA_BASE_URL}/d/ecs-inventory/ecs-inventory-clusters",
            'k8s-inventory-clusters': f"{GRAFANA_BASE_URL}/d/k8s-inventory/k8s-inventory-clusters",
            'vcs-public-api': f"{GRAFANA_BASE_URL}/d/vcs-public-api/vcs-public-api-dashboard",
            'list-k8s-sensor-events': f"{GRAFANA_BASE_URL}/d/vcs-public-api/k8s-sensor-events",
            'list-k8s-eval-events': f"{GRAFANA_BASE_URL}/d/vcs-public-api/k8s-evaluation-events",
            'get-ui-vulnerabilities': f"{GRAFANA_BASE_URL}/d/vcs-public-api/ui-vulnerabilities",
            'container-security': f"{GRAFANA_BASE_URL}/d/container-security/container-security-overview",
            'event-forwarder': f"{GRAFANA_BASE_URL}/d/event-forwarder/event-forwarder-dashboard",
            'health-info-forwarder': f"{GRAFANA_BASE_URL}/d/health-info/health-info-forwarder",
            'start-scan': f"{GRAFANA_BASE_URL}/d/scan/container-scanning",
            'ecs-resources-lister': f"{GRAFANA_BASE_URL}/d/ecs-resources/ecs-resources-lister",
            'evaluator': f"{GRAFANA_BASE_URL}/d/evaluator/security-evaluator",
            'data-migration-runtime-importer': f"{GRAFANA_BASE_URL}/d/data-migration/runtime-importer",
            'monitor': f"{GRAFANA_BASE_URL}/d/synthetics/synthetics-monitoring",
        }
    ...
    def get_cloudwatch_logs_url(
        ...
        # CloudWatch Logs Console URL 模板
        CLOUDWATCH_LOGS_URL_TEMPLATE = (
            "https://{region}.console.aws.amazon.com/cloudwatch/home?"
            "region={region}#logsV2:log-groups/log-group/{encoded_log_group}/"
            "log-events$3FfilterPattern$3D{encoded_filter}$26start$3D{start_time_ms}$26end$3D{end_time_ms}"
        )
