...
def to_chart_segment(data_list: list,
                     name: str = None,
                     date_is_time: bool = False,
                     name_raw: bool = False,
                     return_file: bool = True) -> str:
    ...
    #         [2, 2, {'name': 'a', 'differ': 1, 'from': '1', 'to': '2'}],
    #     ],
    #     symbolSize: 15,
    #     'itemStyle': {
    #         'normal': {
    ...
    x_max = 1
    color_list = get_default_color_list()
    for y_index in range(len(data_list)):
        d_o_data = []
        d_one = data_list[y_index]
        start_index = get_timestamp_ms(d_one[0]) if date_is_time else int(d_one[0])
        end_index = get_timestamp_ms(d_one[1]) if date_is_time else int(d_one[1])
        x_min = min(x_min, start_index)
        x_max = max(x_max, end_index)
        d_o_data.append([start_index, start_index, {'name': d_one[2], 'differ': end_index - start_index, 'from': d_one[0], 'to': d_one[1]}])
        d_o_data.append([end_index, end_index, {'name': d_one[2], 'differ': end_index - start_index, 'from': d_one[0], 'to': d_one[1]}])
        series.append({
            'data': d_o_data,
            'symbolSize': 15,
            'itemStyle': {
                'normal': {
                    'color': color_list[y_index % len(color_list)]
                }
            },
            'type': 'line'
        })
