...
def to_chart_line_x(x_list: list[list[list[str | int]]] = None,
    ...
    if x_list is None:
        x_list = []
    x_p_list: list[dict] = []
    color_list = ['#7fffd4', '#dc143c', '#ffd700', '#6b8e23', '#4682b4', '#ff7f50', '#90ee90', '#ff1493', '#ffa500',
    ...
    for x_d_one_i in range(len(x_list)):
        ...
        for x_d_i in range(len(x_one_list)):
            ...
            start_index = int(x_d[1])
            end_index = int(x_d[2])
            x_p_list.append(
                {'value': [x_d_one_i, start_index, end_index, end_index - start_index, x_d[0]],
