...
def to_chart_segment(data_list: list,
    ...
    for y_index in range(len(data_list)):
        ...
        end_index = get_timestamp_ms(d_one[2]) if date_is_time else int(d_one[2])
        x_min = start_index if x_min is None else min(x_min, start_index)
        x_max = end_index if x_max is None else max(x_max, end_index)
        d_o_data.append([start_index, start_index, {'name': d_one[0], 'differ': end_index - start_index, 'from': d_one[1], 'to': d_one[2]}])
        d_o_data.append([end_index, end_index, {'name': d_one[0], 'differ': end_index - start_index, 'from': d_one[1], 'to': d_one[2]}])
        series.append({
            'data': d_o_data,
