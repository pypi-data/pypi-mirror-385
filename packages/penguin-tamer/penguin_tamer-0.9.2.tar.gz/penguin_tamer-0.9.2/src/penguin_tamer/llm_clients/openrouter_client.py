...
class OpenRouterClient(AbstractLLMClient):
    ...
    This class contains ONLY OpenRouter API-specific logic:
    ...
    def _create_stream(self, api_params: dict):
        """Create API stream object (OpenRouter-specific).
    ...
    def _extract_chunk_content(self, chunk) -> Optional[str]:
        """Extract text content from stream chunk (OpenRouter-specific).
    ...
    def _extract_usage_stats(self, chunk) -> Optional[dict]:
        """Extract usage statistics from chunk (OpenRouter-specific).
