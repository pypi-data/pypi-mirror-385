...
class TestUnwrap:
    ...
    def test_unwrap_422_with_detailed_errors_includes_details_in_message(self):
        """Test that 422 validation errors include detailed error messages."""
        ...
        assert "ValidationError: Invalid request data" in error_msg
        # Check that detailed errors are included
        assert "Validation errors:" in error_msg
        assert "Expected number, received string" in error_msg
        assert "path: body.default_supplier_id" in error_msg
        assert "String must contain at least 1 character(s)" in error_msg
        assert "path: body.name" in error_msg
        # Check that validation_errors attribute is populated
        assert len(exc_info.value.validation_errors) == 2
        assert exc_info.value.status_code == 422

    def test_unwrap_includes_additional_properties_in_error_message(self):
        """Test that additional properties from error response are included in message."""
        error_response = ErrorResponse(
            name="BadRequest",
        ...
        error_msg = str(exc_info.value)
        assert "BadRequest: Invalid parameters" in error_msg
        assert "Additional context:" in error_msg
        assert "request_id: abc123" in error_msg
        assert "timestamp: 2024-01-01T12:00:00Z" in error_msg
