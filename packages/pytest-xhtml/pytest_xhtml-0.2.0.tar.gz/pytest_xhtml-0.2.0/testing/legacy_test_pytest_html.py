...
class TestHTML:
    ...
    def test_rerun(self, testdir):
        testdir.makeconftest(
            ...
            @pytest.hookimpl(hookwrapper=True)
            def pytest_runtest_makereport(item, call):
                pytest_xhtml = item.config.pluginmanager.getplugin("html")
                outcome = yield
                report = outcome.get_result()

                extra = getattr(report, "extra", [])
                if report.when == "call":
                    extra.append(pytest_xhtml.extras.url("http://www.example.com/"))
                report.extra = extra
        """
        )
    ...
    def test_resources_inline_css(self, testdir):
        ...
        content = pkg_resources.resource_string(
            "pytest_xhtml", os.path.join("resources", "style.css")
        )
    ...
    def test_resources(self, testdir):
        ...
        content = pkg_resources.resource_string(
            "pytest_xhtml", os.path.join("resources", "old_main.js")
        )
        content = content.decode("utf-8")
        assert content
        assert content in html
    ...
    def test_custom_content_in_summary(self, testdir):
        ...
        testdir.makeconftest(
            ...
            def pytest_xhtml_results_summary(prefix, summary, postfix):
    ...
    def test_extra_image_windows(self, mocker, testdir):
        mock_isfile = mocker.patch("pytest_xhtml.result.isfile")
    ...
    def test_ansi_color(self, testdir, mocker, with_ansi):
        if not with_ansi:
            mock_ansi_support = mocker.patch("pytest_xhtml.html_report.ansi_support")
            mock_ansi_support = mocker.patch("pytest_xhtml.result.ansi_support")
