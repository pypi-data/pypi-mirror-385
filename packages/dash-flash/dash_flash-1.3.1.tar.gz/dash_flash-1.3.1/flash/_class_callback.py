...
def callback(
    ...
    **_kwargs,
    ...
    print("ARGS", _args, flush=True)
    print("Keargs", _kwargs, flush=True)
...
def _setup_background_callback(
    ...
    assert (
        callback_manager is not None
    ), "Background callback manager is required for background callbacks"
...
def register_callback(
    ...
    def wrap_func(func):
        ...
        # If the decorated target is a classmethod/staticmethod, use its underlying function
        # for metadata (wraps) to avoid AttributeError on functools.wraps(classmethod).
        _wraps_target = (
        ...
        async def add_context(*args, **kwargs):
            ...
            if background is not None:
                ...
                try:
                    # Support classmethod callbacks: bind the class and call the bound method
                    call_target = func
                    if isinstance(func, classmethod):
                        # Resolve owning class from the underlying function's qualname
                        underlying = func.__func__
                        module = sys.modules.get(underlying.__module__)
                        owner = module
                        # Split qualname: e.g., "MyComponent.my_callback"
                        qn_parts = underlying.__qualname__.split(".")
                        # Walk down to the owner (exclude the method name)
                        for part in qn_parts[:-1]:
                            owner = getattr(owner, part, None) if owner is not None else None
                        if owner is not None:
                            call_target = getattr(owner, underlying.__name__, underlying)
                        else:
                            # Fallback to underlying function if resolution fails
                            call_target = underlying
                    elif isinstance(func, staticmethod):
                        # Unwrap staticmethod to its underlying function
                        call_target = func.__func__

                    kw = func_kwargs if isinstance(func_kwargs, dict) else {}
                    output_value = await _invoke_callback(
                        call_target, *func_args, **kw
                    )
