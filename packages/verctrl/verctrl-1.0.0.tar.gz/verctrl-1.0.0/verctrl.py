...
try:
    ...
    # Minimal fallback icons (only critical ones)
    FALLBACK_ICONS = {
...
class SmartFileDetector:
    ...
    # Common patterns to exclude (similar to .gitignore defaults) [[0]](#__0)
    DEFAULT_EXCLUDES = {
        # Version control
        '.git', '.svn', '.hg', '.bzr',
        # Dependencies
    ...
    # File extensions that should typically be version controlled [[1]](#__1)
    SOURCE_EXTENSIONS = {
    ...
    # Binary/media files to typically exclude [[2]](#__2)
    BINARY_EXTENSIONS = {
...
class ModernFileTreeWidget(QTreeWidget):
    ...
    selectionChanged = pyqtSignal(int)  # Emits count of selected files
    ...
    def __init__(self, root_path: Path, existing_files: Set[str] = None):
        ...
        self.file_items = {}  # Map rel_path to QTreeWidgetItem
        
        self.setup_ui()
    
    def setup_ui(self):
        """Setup the tree widget UI with modern styling"""
        # Configure columns [[3]](#__3)
...
class ModernFileSelector(QMainWindow):
    ...
    def apply_smart_detection(self):
        ...
        strategy = strategy_text.split()[0]  # Extract strategy name
...
class VerCtrl:
    ...
    def gui_select(self):
        ...
        try:
            ...
            # Save selection if not cancelled
            if selector.selected_files is not None:
                ...
                for f in selector.selected_files[:10]:  # Show first 10
    ...
    def get_backup_name(self, filepath, backup_dir):
        ...
        scheme = self.config.get('naming_scheme', 'version')  # Default to 'version'
    ...
    def create_backup(self):
        ...
        for filepath in self.config['files']:
            ...
            try:
                ...
                # Create new empty file if requested
    ...
    def smart_add(self, strategy='smart', max_age_days=30):
        ...
        for f in detected_rel[:20]:  # Show first 20
            print(f"  {Colors.CYAN}•{Colors.RESET} {f}")
