...
"""
.. note::
    This plugin is deprecated in favour of the `bazel_file <./bazel_file.html>`_
    plugin which is a redesigned and improved version of this plugin. It is
    recommend to use `bazel_file` for new code.
...
This option is available for all versions of Bazel `up to 9.0`__.

__ https://github.com/bazelbuild/bazel/commit/7898334e92bd78133a9a1fa4a22b1f3c9a9034de.
...
This plugin works by parsing the repository resolved file and downloading any
sources into a "distdir" at ``_bst_distdir`` that can be used in the build
by passing the ``--distdir=_bst_distdir`` option to ``bazel build``. This
is done automatically by the `bazel_build <../elements/bazel_build.html>`_
plugin (by default).
