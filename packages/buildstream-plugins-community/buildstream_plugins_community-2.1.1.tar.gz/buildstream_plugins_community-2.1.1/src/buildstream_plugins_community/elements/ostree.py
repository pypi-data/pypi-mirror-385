        ...
        with self.timed_activity("Staging sysroot", silent_nested=True):
            for dep in self.sysroot:
                self.stage_dependency_artifacts(
                    sandbox, self.sysroot, path=self.get_variable("sysroot")
                )

    def assemble(self, sandbox):
        sysroot = os.path.join(self.get_variable("build-root"), "sysroot")
        barerepopath = os.path.join(
            self.get_variable("build-root"), "barerepo"
        )
        ...
        with sandbox.batch():
            ...
            sandbox.run(
                ["ostree", "init", "--repo", repopath, "--mode", "archive"]
            )
            sandbox.run(
                ["ostree", "pull-local", "--repo", repopath, barerepopath]
            )
