...
with Maqet() as maqet:
    try:
        # Create and start VM
        vm_id = maqet.add(config="config.yaml", name="auto-cleanup-vm")
        maqet.start(vm_id)
...
# Usage
with Maqet() as maqet:
    vm_id = maqet.add(config="vm.yaml", name="test-vm")
    maqet.start(vm_id)
...
for i in $(seq 1 $NODE_COUNT); do
    ...
    maqet add --config "$BASE_CONFIG" --name "$vm_name" \
...
def configure_load_balancer(maqet, backend_vms):
    ...
    lb_vm = maqet.add(config="lb-config.yaml", name="load-balancer")
    maqet.start(lb_vm)
...
with Maqet() as maqet:
    ...
    backends = []
    for i in range(3):
        vm_id = maqet.add(config="backend.yaml", name=f"web-{i+1}")
        maqet.start(vm_id)
...
# Usage
with Maqet() as maqet:
    vm_id = maqet.add(config="vm.yaml", name="retry-test")
...
def safe_vm_operation(vm_config, operation_func):
    ...
    try:
        # Create and start VM
        vm_id = maqet.add(config=vm_config, name="temp-vm")
        maqet.start(vm_id)
...
def test_vm_lifecycle():
    ...
    with Maqet() as maqet:
        # Create VM
        vm_id = maqet.add(config="tests/ci-vm.yaml", name="ci-test")
...
def run_vm_tests():
    ...
    with Maqet() as maqet:
        vm_id = maqet.add(config="tests/test-vm.yaml")
        maqet.start(vm_id)
