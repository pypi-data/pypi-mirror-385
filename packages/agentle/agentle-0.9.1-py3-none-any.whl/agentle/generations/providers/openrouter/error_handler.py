...
def parse_and_raise_openrouter_error(
    ...
    if status_code == 400:
        ...
        if "context length" in error_message_lower or "maximum context" in error_message_lower:
            # Try to extract token numbers from message
            max_tokens = None
            requested_tokens = None
            
            # Pattern: "maximum context length is X tokens. However, you requested Y tokens"
            match = re.search(r"(\d+)\s+tokens.*?requested\s+(\d+)\s+tokens", error_message, re.IGNORECASE)
            if match:
                max_tokens = int(match.group(1))
                requested_tokens = int(match.group(2))
            
            raise ContextLengthExceededError(max_tokens, requested_tokens, response_body)
        
        # Provider returned error (invalid request)
        if "provider returned error" in error_message_lower and "invalid_request" in error_message_lower:
            raise ProviderInvalidRequestError(error_message, response_body)
        
        # Invalid prompt / malformed request
        if (error_code and "invalid_prompt" in error_code) or "invalid" in error_message_lower or "malformed" in error_message_lower:
    ...
    # ==================== 404 Not Found ====================
        ...
        if "data policy" in error_message_lower or "privacy settings" in error_message_lower:
            raise DataPolicyMismatchError(response_body)
        
        # No allowed providers
        if "no allowed providers" in error_message_lower or "allowed providers" in error_message_lower:
    ...
    # ==================== 429 Rate Limit ====================
        ...
        if "free-models-per-day" in error_message_lower or "daily" in error_message_lower:
        ...
        # Upstream rate limit
        if "upstream" in error_message_lower or "temporarily rate-limited" in error_message_lower:
            # Try to extract model name
            model_id = None
