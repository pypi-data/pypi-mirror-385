...
class CrosstabTask(BaseModel):
    ...
    mtask: Annotated[cwt.MediaVortexTask, Field(default_factory=lambda: cwt.MediaVortexTask(check_version=False))]
    mnet: Annotated[
    ...
    async def execute(self) -> pl.DataFrame:
        ...
        for task, task_info in tasks_plus:
            ...
            if task_info.basedemo_filter.name is not None and task_info.statistic not in [K7Statistic.QUANTITY_SUM]:
                df = df.rename(
                    {
