...
class IncLevel(Enum):
    ...
    NOT_ENOUGH_FOR_FOOD = 1  # Денег не хватает даже на еду.
    ENOUGH_FOR_FOOD_BUT_NOT_CLOTHES = 2  # Хватает на еду, но покупать одежду нет возможности.
    ENOUGH_FOR_FOOD_AND_CLOTHES_BUT_NOT_EXPENSIVE_THINGS = (
        3  # Хватает на еду и одежду, но нет возможности покупать дорогие вещи.
    )
    CAN_BUY_EXPENSIVE_THINGS_BUT_NOT_EVERYTHING = 4  # Есть возможность покупать дорогие вещи, но не всё, что захочется.
    COMPLETE_PROSPERITY = 5  # Полный достаток, нет ограничений в средствах.
    DIFFICULT_TO_ANSWER = 6  # Затрудняется ответить.
