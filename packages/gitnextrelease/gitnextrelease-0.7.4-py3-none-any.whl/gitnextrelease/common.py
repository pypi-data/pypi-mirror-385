...
class GitActions(CommonFunctions):
    ...
    def gather_git_info(self):
        ...
        if self.check_for_origin_remote(self.args.no_remote):
            self.git(["pull"])
    ...
    def check_for_origin_remote(self, override):
        if override:
            return False
        result = self.git(["remote"], everything=True)
        # print(result)
        if "origin" in str(result["out"]):
            return True
        return False
