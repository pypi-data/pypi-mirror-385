...
class DocxProcessor:
    ...
    def read_document(
        self, file_path: str, pages: Optional[str] = None, include_image_ocr: bool = False
    ) -> DocumentContent:
        """
        ...
        """
        try:
            logger.info(f"Reading document: {file_path}, OCR: {include_image_ocr}")
            content = self.reader.read_with_markitdown(
                file_path, include_image_ocr=include_image_ocr
            )
    ...
    def read_document_from_bytes(
        ...
        try:
            ...
            doc_content = self.reader.read_from_bytes(
                content, file_name, include_image_ocr=include_image_ocr
            )
    ...
    def process_multiple_files(
        self, files: List[FileObject], operation: str, include_image_ocr: bool = False, **kwargs
        ...
        logger.info(
            f"Processing {len(files)} files with operation: {operation}, OCR: {include_image_ocr}"
        )

        # If only one file, process it directly
        if len(files) == 1:
            return self._process_single_file(
                files[0], operation, include_image_ocr=include_image_ocr, **kwargs
            )
        ...
        for idx, file_obj in enumerate(files):
            ...
            result = self._process_single_file(
                file_obj, operation, include_image_ocr=include_image_ocr, **kwargs
            )
    ...
    def _process_single_file(
        self, file_obj: FileObject, operation: str, include_image_ocr: bool = False, **kwargs
        ...
        content = self.read_document_from_bytes(
            file_obj.content, file_obj.name, pages, include_image_ocr=include_image_ocr
        )
