...
class DocxReader:
    ...
    def __init__(self, ocr_enabled: bool = True, chat_model: Optional[BaseChatModel] = None):
        ...
        logger.debug(f"Initialized DocxReader with OCR enabled: {self.ocr_enabled}")
    
    def read_with_markitdown(self, file_path: str, include_image_ocr: bool = False) -> DocumentContent:
        """
        Read a DOCX document using MarkItDown for comprehensive reading.

        Args:
            file_path: Path to the DOCX file
            include_image_ocr: Whether to perform OCR on images (default: False)
        ...
        """
        logger.info(f"Reading document with MarkItDown: {file_path}, OCR: {include_image_ocr}")
        ...
        try:
            ...
            text = DocxReader._extract_text(document)

            # Extract document structure
            structure = DocxReader._extract_structure(document)

            # Extract formatting information
            formatting = DocxReader._extract_formatting(document)
            ...
            # Extract images (with or without OCR based on parameter)
            images = self._extract_images(file_path, include_ocr=include_image_ocr)
    ...
    def read_from_bytes(self, content: bytes, file_name: str, include_image_ocr: bool = False) -> DocumentContent:
        """
        ...
        Raises:
            ...
            UnsupportedFormatError: If the format is not supported
        """
        logger.info(f"Reading document from bytes: {file_name}, OCR: {include_image_ocr}")
        ...
        try:
            ...
            try:
                ...
                return self.read_with_markitdown(temp_path, include_image_ocr=include_image_ocr)
            finally:
    ...
    def _add_style_if_new(paragraph, styles, default_style_name: str = "Normal"):
        ...
        if style_name not in [s.name for s in styles]:
            style = StyleInfo(
                name=style_name,
                font=paragraph.style.font.name if hasattr(paragraph.style, 'font') and paragraph.style.font else "Default",
