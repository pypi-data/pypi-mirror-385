...
class DocxTool(CodeMieTool):
    ...
    def _handle_direct_processor_queries(
        ...
        # Only enable OCR for image extraction
        include_image_ocr = query == QueryType.IMAGE_EXTRACTION
    ...
    def _handle_metadata_queries(
        ...
        for file_obj in self.files:
            # Only enable OCR for TEXT_WITH_IMAGES query
            include_image_ocr = query == QueryType.TEXT_WITH_IMAGES
    ...
    def _handle_analyze_query(
        ...
        analysis_results = []
        for file_obj in self.files:
            # Analyze queries don't need OCR - analyzing document text only
            content = self.docx_processor.read_document_from_bytes(
                file_obj.content, file_obj.name, pages, include_image_ocr=False
            )
