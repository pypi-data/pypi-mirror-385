    ...
    async def _run_in_sandbox(self, code: str, user_id: str):
        ...
        if result.type == "image/png":
            if self.file_repository:
                ...
                return f"This '[[sandbox]]:/{stored_file.to_encoded_url()}' is image URL. You MUST not transform it. Return as it is"
            else:
                return f"This '[[sandbox]]:/{result.content}' is image URL. You MUST not transform it. Return as it is"
