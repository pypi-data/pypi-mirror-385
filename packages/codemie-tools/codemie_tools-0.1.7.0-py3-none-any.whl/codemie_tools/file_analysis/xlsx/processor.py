...
def _filter_dataframe(
    ...
    """
    Filter DataFrame rows by checking if any cell in the row matches the filter value.
    This works with pivot tables and complex Excel layouts where column structure is not predictable.

    Args:
        df: DataFrame to filter
        filter_value: Value to search for in any cell of the row
        filter_mode: Matching mode ('exact' - exact match, 'contains' - substring match)

    Returns:
        Filtered DataFrame containing only rows where at least one cell matches the filter criteria
    """
    ...
    try:
        ...
        # Helper function to check if a cell matches the filter criteria
        def cell_matches(cell_value):
        ...
        # Helper function to check if any cell in a row matches
        def row_matches(row):
            return any(cell_matches(cell_value) for cell_value in row)

        # Apply filter to each row
        mask = df.apply(row_matches, axis=1)
        filtered_df = df[mask]

        logger.debug(f"Filtered DataFrame: {len(filtered_df)} rows out of {len(df)} matched filter criteria (filter_value='{filter_value}', mode='{filter_mode}')")
        return filtered_df
    ...
    def load(
        ...
        try:
            ...
            for sheet_name, df in sheets.items():
                ...
                df = _filter_dataframe(df, self.filter_value, self.filter_mode)
