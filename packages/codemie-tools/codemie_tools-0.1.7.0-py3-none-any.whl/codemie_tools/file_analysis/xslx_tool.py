...
class XslxToolInput(BaseModel):
    ...
    filter_value: Optional[str] = Field(default=None, description="""Value to search for across all cells in each row. Only rows where at least one cell matches this value will be included in the output. Works with pivot tables and complex Excel layouts.""")
    filter_mode: str = Field(default="exact", description="""Filter matching mode: 'exact' (default) - exact match, 'contains' - substring match. Case-insensitive.""")
...
class XlsxTool(CodeMieTool):
    ...
    def _load_excel_file(
        ...
        Args:
            ...
            filter_value: Value to search for across all cells in each row
    ...
    def execute(self, query: str = "", **kwargs):
        ...
        return self._process_files(sheet_names, sheet_index, get_sheet_names, 
                               get_stats, visible_only, filter_value, filter_mode)
    
    def _process_files(self, sheet_names: List[str], sheet_index: int,
                       get_sheet_names: bool, get_stats: bool, visible_only: bool,
                       filter_value: Optional[str], filter_mode: str) -> str:
        ...
        for file_object in self.files:
            content = self._process_file_content(file_object, sheet_names, sheet_index, get_sheet_names,
                                              get_stats, visible_only, filter_value, filter_mode)
    ...
    def _process_file_content(self, file_object: FileObject, sheet_names: List[str], sheet_index: int,
                             get_sheet_names: bool, get_stats: bool, visible_only: bool,
                             filter_value: Optional[str], filter_mode: str) -> str:
        ...
        Args:
            ...
            filter_value: Value to filter rows
        ...
        if sheet_index is not None:
            return self._get_sheet_by_index_content(file_object, sheet_index, visible_only, filter_value, filter_mode)
    
        # Check if we need filtering
        if filter_value:
            return self._get_filtered_content(file_object, sheet_names, visible_only, filter_value, filter_mode)
    ...
    def _get_filtered_content(self, file_object: FileObject, sheet_names: List[str],
                            visible_only: bool, filter_value: str, filter_mode: str) -> str:
        """Get filtered content as markdown"""
        sheets = self._load_excel_file(file_object, sheet_names=sheet_names,
                                    visible_only=visible_only, filter_value=filter_value,
                                    filter_mode=filter_mode)
    ...
    def _get_sheet_by_index_content(self, file_object: FileObject, sheet_index: int, 
                                   visible_only: bool, filter_value: Optional[str], 
                                   filter_mode: str) -> str:
        ...
        """
        sheets = self._load_excel_file(file_object, visible_only=visible_only,
                                     filter_value=filter_value, filter_mode=filter_mode)
