...
EXCEL_TOOL = ToolMetadata(
    ...
    and filter rows based on cell values. Works with pivot tables and complex Excel layouts.
    ...
    - **Filter rows by value**: Use `filter_value` to include only rows where any cell matches the specified value
    ...
    Examples:
    - Exact match (default): `filter_value="NATI-5DME"` to get only rows with cells exactly matching "NATI-5DME"
    - Substring match: `filter_value="NATI"` with `filter_mode="contains"` to get rows containing "NATI" anywhere in a cell
    - Status filtering: `filter_value="Complete"` for exact status matching
    """,
