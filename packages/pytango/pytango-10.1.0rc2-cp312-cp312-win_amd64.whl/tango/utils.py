...
def obj_2_property(value):
    ...
    raise TypeError(
        "Value must be a str, tango.DbDatum, tango.DbData, "
        "a sequence<DbDatum>, a sequence<str> or a dictionary"
    )


def property_2_obj(self, propname, value):
    new_value = value
    ...
    if is_pure_str(propname) or isinstance(propname, StdStringVector):
    ...
    elif isinstance(propname, DbData):
        new_value = propname
        self._get_property(new_value)
        ...
        if is_pure_str(propname[0]):
            ...
            raise TypeError(
                "propname must be a str, tango.DbDatum, tango.DbData, "
                "a sequence<DbDatum> or a sequence<str"
            )
    else:
        raise TypeError(
            "propname must be a str, tango.DbDatum, tango.DbData, "
