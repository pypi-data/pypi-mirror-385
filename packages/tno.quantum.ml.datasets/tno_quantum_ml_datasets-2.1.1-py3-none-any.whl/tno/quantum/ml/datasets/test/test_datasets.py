...
from tno.quantum.ml.datasets._utils import _pre_process_data
...
def test_size_get_bars_and_stripes_dataset(n_rows: int, n_cols: int) -> None:
    """Size of the full bars and stripes dataset is known."""
    X_train, y_train, _, _ = get_bars_and_stripes_dataset(
        n_samples=None, shape=(n_rows, n_cols), test_size=0
    )
    expected_size = 2**n_rows + 2**n_cols - 4
    assert len(X_train) == expected_size
    assert sum(y_train) == 2**n_cols - 2

    # Test noiseless images only contain values 0 or 255
    for img in X_train:
...
def test_seed(method_to_test: Any, args: list[Any]) -> None:
    ...
    for el_a, el_b in zip(data_a, data_b, strict=False):
        assert np.array_equal(el_a, el_b)

    for el_a, el_c in zip(data_a, data_c, strict=False):
