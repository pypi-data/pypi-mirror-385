...
class FastApiRedisCache(metaclass=MetaSingleton):
    ...
    async def expire_entire_router_cache_async(
        ...
        exlude_routes: Optional[List[str]] = None,
        ...
        if keys:
            keys = self._filter_keys_excluding_routes(exlude_routes, keys)
    ...
    def _filter_keys_excluding_routes(self, skip_routes: Optional[List[str]], keys: List[bytes]) -> List[bytes]:
        if not skip_routes:
            return keys
        
        try:
            # filter the keys to exclude any that contain the skip_routes
            filtered_keys = []
            for key in keys:
                key_str = key.decode('utf-8')
                # Keep the key only if it doesn't match any of the skip_routes
                if not any(f".{route}." in key_str for route in skip_routes):
                    filtered_keys.append(key)
            return filtered_keys
        except Exception as e:
            self.log(RedisEvent.FAILED_TO_CACHE_KEY, msg=f"Error filtering skip_routes: {e}")
