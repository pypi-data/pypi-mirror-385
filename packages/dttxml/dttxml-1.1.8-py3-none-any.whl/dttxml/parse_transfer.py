...
def create_linear_fhz(len, f0_Hz, df_Hz):
    """
    Create an array of frequencies of length len
    linear from f0 with a step size of df
    :param len: the length of the resulting array
    :param f0_Hz: starting frequency in Hz
    :param df_Hz: step from one frequency to the next in Hz
    :returns: a numpy array of f0 + i*df for i from 0 to len-1
    """
    fhz = np.array(range(len))
    fhz = fhz * df_Hz + f0_Hz
    return fhz
...
def parse_transfer(LW_node):
    ...
    if specbunch.subtype_raw == 0:
        ...
        specbunch.FHz = create_linear_fhz(N, specbunch.f0, specbunch.df)
        ...
        specbunch.subtype = 'transfer function B/A in format (f,Y)'
        specbunch.type_name = 'TF'
        freqs = np.frombuffer(streambuff, 'f8')
        specbunch.FHz = freqs[:N]
        data = np.frombuffer(streambuff, 'c8').real
        specbunch.xfer = data[N:].reshape(M, -1)
    elif specbunch.subtype_raw == 7:
        specbunch.subtype = 'coherence B/A in format (f,Y)'
        specbunch.type_name = 'COH'
        data = np.frombuffer(streambuff, dtype='f8')
        specbunch.FHz = data[:N]
        specbunch.coherence = data[N:].reshape(M, -1)
    else:
