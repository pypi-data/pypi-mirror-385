...
from sklearn.datasets import load_iris, load_wine, load_breast_cancer
from sklearn.model_selection import train_test_split, cross_val_score
from sklearn.preprocessing import StandardScaler  # ★ Added for manual scaling
from sklearn.decomposition import PCA             # ★ Added for manual PCA
from sklearn.neighbors import KNeighborsClassifier
from sklearn.naive_bayes import GaussianNB
from sklearn.svm import SVC
from sklearn.tree import DecisionTreeClassifier, plot_tree
...
# This block shows how to apply preprocessing manually.
# IMPORTANT: You must .fit() on x_train and then .transform() both x_train and x_test.

# Option A: StandardScaler
# 1. Instantiate the scaler
# scaler = StandardScaler()
# 2. Fit on TRAINING data and transform it
# x_train_scaled = scaler.fit_transform(x_train)
# 3. Transform TEST data using the *same* scaler (fitted on train)
...
# 3. Transform TEST data using the *same* PCA (fitted on train)
...
# --- Fit ---
# Option 1: Original data (No preprocessing)
knn1.fit(x_train, y_train)
...
# ★ Option 4: With Manual StandardScaler
# (Make sure to uncomment 'Option A' in the preprocessing block above)
# knn1.fit(x_train_scaled, y_train)

# ★ Option 5: With Manual PCA
# (Make sure to uncomment 'Option B' in the preprocessing block above)
# knn1.fit(x_train_pca, y_train)


# --- Predict ---
# Option 1: (If model was fit with Option 1)
y_pred_knn1 = knn1.predict(x_test)

# ★ Option 4: (If model was fit with Option 4)
# y_pred_knn1 = knn1.predict(x_test_scaled)

# ★ Option 5: (If model was fit with Option 5)
# y_pred_knn1 = knn1.predict(x_test_pca)


# --- Evaluation: Accuracy ---
# Option 1: (If model was fit/predicted with Option 1)
print(f"KNN (Original): {knn1.score(x_test, y_test)}")

# ★ Option 4: (If model was fit/predicted with Option 4)
# print(f"KNN (Scaled): {knn1.score(x_test_scaled, y_test)}")

# ★ Option 5: (If model was fit/predicted with Option 5)
# print(f"KNN (PCA): {knn1.score(x_test_pca, y_test)}")


# --- Evaluation: K-Fold Cross Validation ---
# cv=4/5/10, scoring='accuracy'/'precision'/'recall'/'f1'/'neg_mean_squared_error'

# Option 1: CV on original training data
cv_scores = cross_val_score(knn1, x_train, y_train, cv=4)
print(f"Cross-Validation Scores (Original): {cv_scores}")
print(f"Mean CV Score (Original): {np.mean(cv_scores)}")

# ★ CAUTION: Running CV on pre-scaled data (e.g., x_train_scaled)
# is generally INCORRECT and leads to data leakage.
# The 'scaler' was fit on the *entire* x_train, so information
# from the validation folds "leaked" into the training folds.
# The correct way to do CV with preprocessing is to use a Pipeline,
# which ensures scaling happens *inside* each fold.
#
# INCORRECT (Data Leakage):
# cv_scores_scaled = cross_val_score(knn1, x_train_scaled, y_train, cv=4)
# print(f"Mean CV Score (Scaled, Leaky): {np.mean(cv_scores_scaled)}")


# --- Evaluation: Confusion Matrix ---
# (Uses y_pred_knn1 from the 'Predict' section)
print("--- Confusion Matrix ---")
print(confusion_matrix(y_test, y_pred_knn1))
