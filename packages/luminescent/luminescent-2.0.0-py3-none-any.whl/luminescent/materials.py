...
def Material(epsilon=1, mu=1, sigma=0, chi2=0, chi3=0, mesh_density=None):
    """
    Define material properties in normalized units

    Args:

    - epsilon: relative permittivity
    - mu: relative permeability
    - sigma: conductivity (multiply by `377 * (characteristic wavelength in m)` to convert from metric (`S/m, 1 /(Ω m)`) to normalized units)
    - chi2: second order nonlinear susceptibility (request for implementation)
    - chi3: third order nonlinear susceptibility (request for implementation)
...
ks = copy.deepcopy(list(MATERIALS_LIBRARY.keys()))
for k in ks:
    MATERIALS_LIBRARY[k.lower()] = MATERIALS_LIBRARY[k]
