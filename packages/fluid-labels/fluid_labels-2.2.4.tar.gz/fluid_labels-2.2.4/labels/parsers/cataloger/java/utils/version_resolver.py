...
from labels.model.ecosystem_data.java import JavaArchive, JavaPomParent, JavaPomProperties
...
def resolve_version(
    dependency: Tag, pom_context: PomContext, java_archive: JavaArchive, full_name: str
) -> str | None:
...
def _resolve_parent_version(java_archive: JavaArchive, parent_info: dict[str, str]) -> str | None:
    if java_archive.pom_properties:
        return _find_version_from_parent_pom_chain(
            pom_properties=java_archive.pom_properties, parent_info=parent_info
        )
...
def _find_version_from_parent_pom_chain(
