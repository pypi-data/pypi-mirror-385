...
from .callback_handler import TraceloopCallbackHandler
...
class LangchainInstrumentor(BaseInstrumentor):
    ...
    def _instrument(self, **kwargs):
        ...
        def wrapper_for_invoke(wrapped, instance, args, kwargs):
            # Create a new empty context
            empty_context = context_api.Context()

            # Attach the empty context to start a new trace
            _ = context_api.attach(empty_context)

            result = wrapped(*args, **kwargs)

            return result

        # A flag to indicate wether to open a new trace on each langgraph
        if self.new_trace_on_workflow:
            wrap_function_wrapper(
                module="langgraph.graph.graph",
                name="CompiledGraph.invoke",
                wrapper=wrapper_for_invoke,
            )
