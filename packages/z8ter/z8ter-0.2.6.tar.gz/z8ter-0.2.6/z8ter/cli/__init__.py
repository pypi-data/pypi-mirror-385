...
directory and derives project paths (views, templates, api, static, ts).
It also exposes a cached `Jinja2Templates` bound to the current templates dir.

Key ideas:
- App root precedence: explicit `set_app_dir(...)` > env `Z8TER_APP_DIR` > cwd.
- Paths are resolved lazily and cached; cache is cleared when app dir changes.
