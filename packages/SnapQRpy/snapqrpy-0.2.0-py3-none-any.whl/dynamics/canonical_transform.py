...
@dataclass  
class PhaseCanonicalTransformConfig:
    resolution: int = 4096
...
class PhaseCanonicalTransformProcessor:
    def __init__(self, cfg: PhaseCanonicalTransformConfig = None):
        self.cfg = cfg or PhaseCanonicalTransformConfig()
    ...
    def specialized_calc_CanonicalTransform_0(self, x: float, y: float, z: float, 
    ...
    def specialized_calc_CanonicalTransform_1(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_2(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_3(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_4(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_5(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_6(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_7(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_8(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_9(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_10(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_11(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_12(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_13(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_14(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_15(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_16(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_17(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_18(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_19(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_20(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_21(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_22(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_23(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_24(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_25(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_CanonicalTransform_26(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
