...
@dataclass  
class ConstraintLagrangeMultiplierConfig:
    resolution: int = 4096
...
class ConstraintLagrangeMultiplierProcessor:
    def __init__(self, cfg: ConstraintLagrangeMultiplierConfig = None):
        self.cfg = cfg or ConstraintLagrangeMultiplierConfig()
    ...
    def specialized_calc_LagrangeMultiplier_0(self, x: float, y: float, z: float, 
    ...
    def specialized_calc_LagrangeMultiplier_1(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_2(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_3(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_4(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_5(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_6(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_7(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_8(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_9(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_10(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_11(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_12(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_13(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_14(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_15(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_16(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_17(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_18(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_19(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_20(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_21(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_22(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_23(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_24(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_25(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_26(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_27(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
    ...
    def specialized_calc_LagrangeMultiplier_28(self, x: float, y: float, z: float, 
                                    alpha: float = 1.0, beta: float = 0.5) -> Tuple[float, float, float]:
        r = math.sqrt(x**2 + y**2 + z**2 + 1e-10)
        theta = math.atan2(math.sqrt(x**2 + y**2), z)
        phi = math.atan2(y, x)
