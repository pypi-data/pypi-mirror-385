...
class ContentValidationMixin:
    ...
    def detect_image_format(self: "ContentValidator", binary_data: Union[bytes, str]) -> Dict[str, Any]:
        """
        检测图片格式并提供详细的格式信息

        Args:
            binary_data: 二进制图片数据（bytes或base64字符串）
        ...
        if isinstance(binary_data, str):
            try:
                ...
                return {
                    ...
                    'details': f'Base64 decode failed: {str(e)}'
                }
        ...
        # 检测格式
        detected_format = self._detect_format(binary_data)

        # 确定置信度
        if detected_format != ContentFormat.UNKNOWN:
            confidence = 'high'
            details = f'Format detected: {detected_format.value}'
        else:
            confidence = 'low'
        ...
        mime_mapping = {
            ...
            ContentFormat.UNKNOWN: 'application/octet-stream'
        }

        mime_type = mime_mapping.get(detected_format, 'application/octet-stream')
