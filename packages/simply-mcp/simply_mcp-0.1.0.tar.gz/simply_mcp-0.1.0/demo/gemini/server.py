...
try:
    ...
    types = _types_module
    GENAI_AVAILABLE = True
    logger.debug("google-genai SDK imported at module load")
except ImportError:
    # Will be imported lazily when needed
    logger.debug("google-genai SDK will be imported on first use (lazy import)")
    pass
...
async def main() -> None:
    ...
    # Check SDK availability (lazy import if needed)
    if not _ensure_genai_available():
    ...
    # Print info
    print("=" * 70)
    print("Gemini MCP Server - Feature Layer")
    print("=" * 70)
    print()
    print("Server: gemini-server v0.1.0")
    print("Transport: stdio")
    print()
    print("Available Tools:")
    print("  1. upload_file      - Upload files to Gemini Files API")
    print("  2. generate_content - Generate content with optional file context")
    print("  3. start_chat       - Start chat sessions with initial messages")
    print("  4. send_message     - Send messages to continue chat sessions")
    print("  5. list_files       - List all uploaded files with metadata")
    print("  6. delete_file      - Delete files from Gemini Files API")
    print()
    print("Features:")
    print(f"  - Gemini SDK: {'Available' if GENAI_AVAILABLE else 'Not available (install google-genai)'}")
    print("  - API Key: Configured")
    print("  - File Registry: Enabled")
    print("  - Chat Sessions: Enabled")
    print("  - File Management: Enabled")
    print()
    print("=" * 70)
    print()
    print("Server is ready and listening on stdio...")
    print()
    ...
    try:
        ...
        await run_stdio_server(mcp.server)
