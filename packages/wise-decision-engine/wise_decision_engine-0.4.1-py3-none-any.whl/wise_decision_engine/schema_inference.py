...
class SchemaInferenceEngine:
    ...
    def create_expanded_dataframe(self, df, result_column: str = "wd_result", 
        ...
        Args:
            ...
            keep_original: Se deve manter a coluna original
        ...
        if schema is None:
            ...
            if schema is None:
                print("⚠️ Schema inference falhou, mantendo resultado como JSON")
                return df
        
        # Detecta estrutura do JSON dinamicamente
        samples = df.select(result_column).limit(1).collect()
        sample_values = [row[result_column] for row in samples if row[result_column]]
        
        if not sample_values:
            print("⚠️ Nenhuma amostra encontrada para análise de estrutura")
            return df
        
        try:
            sample_json = json.loads(sample_values[0])
        except json.JSONDecodeError:
            print("⚠️ JSON inválido na amostra, mantendo como string")
            return df
        
        # Detecta se tem wrapper ou é flat
        if isinstance(sample_json, dict) and 'result' in sample_json:
            # Tem wrapper 'result' - estrutura completa
            full_schema = StructType([
                StructField("performance", StringType(), True),
                StructField("result", schema, True)
            ])
            field_path = "_parsed_result.result"
        else:
            # É flat - usa schema direto
            full_schema = schema
            field_path = "_parsed_result"
        ...
        for field in schema.fields:
            expanded_df = expanded_df.withColumn(
                field.name,
                col(f"{field_path}.{field.name}")
            )
        
        # Remove coluna temporária
        final_df = expanded_df.drop("_parsed_result")
        
        # Remove coluna original se solicitado
        if not keep_original:
            final_df = final_df.drop(result_column)
...
class AutoSchemaHelper:
    ...
    def enhance_dataframe_with_schema(df, 
        ...
        expanded_df = engine.create_expanded_dataframe(df, result_column, keep_original=keep_original)
...
def auto_expand_decision_results(df, 
    ...
    return AutoSchemaHelper.enhance_dataframe_with_schema(
        df, result_column, selected_strategy, prefix, keep_original
    )
