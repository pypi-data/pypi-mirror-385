...
class TestHelperFunctions:
    ...
    def test_apply_decision_to_dataframe(self, mock_evaluator_class):
        ...
        # Mock adapter to return valid JSON
        ...
        mock_evaluator.apply_to_dataframe.assert_called_once_with(
            mock_df, "result", ["col1", "col2"], False, True, "conservative"
        )
...
class TestUtilityFunctions:
    ...
    @patch('wise_decision_engine.spark_utils.StringType')
    def test_create_legacy_udf(self, mock_string_type, mock_pandas_udf):
        """Testa criação de UDF legada."""
        # Setup mock - pandas_udf(StringType()) retorna um decorador que retorna a função
        mock_decorator = Mock()
        mock_final_udf = Mock()
        mock_pandas_udf.return_value = mock_decorator
        mock_decorator.return_value = mock_final_udf
        ...
        # Verificações
        mock_pandas_udf.assert_called_once_with(mock_string_type())
        mock_decorator.assert_called_once()  # O decorador foi aplicado à função
        assert result == mock_final_udf
