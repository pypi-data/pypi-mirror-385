...
class SparkDecisionEvaluator:
    ...
    def apply_to_dataframe(self, 
                          ...
                          return_only_new_fields: bool = False) -> DataFrame:
        """
        ...
        Args:
            ...
            return_only_new_fields: Se True, retorna apenas campos gerados pela decisão
        ...
        # Filtra apenas campos novos se solicitado
        if return_only_new_fields:
            final_columns = set(result_df.columns)
            new_columns = final_columns - original_columns
            
            if new_columns:
                # Retorna apenas colunas novas, ordenadas
                result_df = result_df.select(*sorted(new_columns))
            else:
                # Edge case: nenhum campo novo gerado
                # Retorna DataFrame vazio com mesmo número de linhas
                from pyspark.sql.functions import lit
                result_df = result_df.select(lit(None).alias("_no_new_fields")).drop("_no_new_fields")
...
def apply_decision_to_dataframe(df: DataFrame,
    ...
    return evaluator.apply_to_dataframe(
        df, result_column, input_columns, expand_result, auto_schema, schema_strategy, return_only_new_fields
    )
...
class DatabricksHelper:
    ...
    def quick_decision_apply(catalog: str,
        ...
        Example:
            ...
            # Apenas campos novos gerados pela decisão:
            apenas_novos = DatabricksHelper.quick_decision_apply(
                "wisedecisions_catalog", "public", "decision",
                "credito-pj", clientes_df,
                auto_schema=True,
                return_only_new_fields=True
            )
            ```
        """
