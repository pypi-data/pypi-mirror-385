...
def eval_map(
    ...
    assert eval_mode in ["area", "11points"], f'Unrecognized {eval_mode} mode, only "area" and "11points" are supported'
    if not use_legacy_coordinate:
        extra_length = 0.0
    else:
        extra_length = 1.0
    ...
    for i in range(num_classes):
        ...
        if use_group_of:
            tp, fp, cls_dets = tuple(zip(*tpfp, strict=False))
        else:
            tp, fp = tuple(zip(*tpfp, strict=False))
        # calculate gt number of each scale
        # ignored gts or gts beyond the specific scale are not counted
        num_gts = np.zeros(num_scales, dtype=int)
        for _j, bbox in enumerate(cls_gts):
            if area_ranges is None:
