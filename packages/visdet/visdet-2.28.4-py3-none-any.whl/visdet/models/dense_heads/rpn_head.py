...
from visdet.structures.bbox import cat_boxes, empty_box_as, get_box_tensor, get_box_wh, scale_boxes
from visdet.utils import InstanceList, OptInstanceList

# MultiConfig type
MultiConfig = Union[ConfigDict, List[ConfigDict]]

# Implementation of batched_nms
def batched_nms(boxes, scores, idxs, nms_cfg):
    """Performs non-maximum suppression in a batched fashion.

    Modified from mmcv's implementation. In order to perform NMS independently per class,
    ...
    # Get NMS type, default to 'nms' if not specified
    nms_type = nms_cfg.get('type', 'nms')
    iou_thr = nms_cfg.get('iou_threshold', 0.3)

    if nms_type == 'nms':
        from torchvision.ops import nms
        keep = nms(boxes_for_nms, scores, iou_thr)
        dets = torch.cat([boxes[keep], scores[keep].reshape(-1, 1)], -1)
        return dets, keep
    else:
...
class RPNHead(AnchorHead):
    ...
    def _predict_by_feat_single(
        ...
        for level_idx, (cls_score, bbox_pred, priors) in enumerate(
            zip(cls_score_list, bbox_pred_list, mlvl_priors, strict=False)
        ):
    ...
    def _bbox_post_process(
        ...
        if results.bboxes.numel() > 0:
            ...
            nms_cfg = cfg.get('nms', {'type': 'nms', 'iou_threshold': 0.7})
            det_bboxes, keep_idxs = batched_nms(bboxes, results.scores, results.level_ids, nms_cfg)
            results = results[keep_idxs]
            # Some NMS operations might reweight the score, such as SoftNMS
