    ...
    for gt_ann, pred_ann in annotation_set:
        ...
        if len(pred_labels_set) != 0:
            raise KeyError(
                ...
                "are presented in JSON and not presented in PNG.".format(gt_ann["image_id"], list(pred_labels_set))
            )
...
def pq_compute_multi_core(
    ...
    for proc_id, annotation_set in enumerate(annotations_split):
        p = workers.apply_async(
            ...
            (proc_id, annotation_set, gt_folder, pred_folder, categories, file_client),
        )
        processes.append(p)
