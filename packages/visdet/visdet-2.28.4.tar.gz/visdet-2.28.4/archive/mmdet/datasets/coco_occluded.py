...
class OccludedSeparatedCocoDataset(CocoDataset):
    ...
    def evaluate(self, results, metric=[], score_thr=0.3, iou_thr=0.75, **kwargs):
        ...
        eval_res = self.evaluate_occluded_separated(results, score_thr, iou_thr)
        coco_metric_res.update(eval_res)
        return coco_metric_res

    def evaluate_occluded_separated(self, results, score_thr=0.3, iou_thr=0.75):
        ...
        for i in range(len(results)):
            ...
            for cat_id in range(len(results[i][1])):
                ...
                for instance_id in range(len(results[i][1][cat_id])):
                    cur_binary_mask = coco_mask.decode(results[i][1][cat_id][instance_id])
    ...
    def compute_recall(self, result_dict, gt_ann, score_thr=0.3, iou_thr=0.75, is_occ=True):
