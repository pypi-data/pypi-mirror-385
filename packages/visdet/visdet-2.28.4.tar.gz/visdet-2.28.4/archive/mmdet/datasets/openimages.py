...
class OpenImagesDataset(CustomDataset):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(OpenImagesDataset, self).__init__(ann_file=ann_file, file_client_args=file_client_args, **kwargs)
    ...
    def get_meta_from_file(self, meta_file=""):
        ...
        metas = mmcv.load(meta_file, file_format="pkl", file_client_args=self.file_client_args)
    ...
    def _filter_imgs(self, min_size=32):
        ...
        if self.filter_empty_gt:
            warnings.warn("OpenImageDatasets does not support filtering empty gt images.")
    ...
    def _convert_hierarchy_tree(self, hierarchy_map, class_label_tree, parents=[], get_all_parents=True):
        ...
        if "Subcategory" in hierarchy_map:
            for node in hierarchy_map["Subcategory"]:
                ...
                class_label_tree = self._convert_hierarchy_tree(node, class_label_tree, parents=children)
    ...
    def process_results(self, det_results, annotations, image_level_annotations):
        ...
        for i in range(len(det_results)):
            ...
            valid_classes = np.where(np.array([[bbox.shape[0]] for bbox in det_results[i]]) != 0)[0]
            if image_level_annotations is not None:
                labels = annotations[i]["labels"]
                image_level_labels = image_level_annotations[i]["image_level_labels"]
                allowed_labeles = np.unique(np.append(labels, image_level_labels))
            else:
            ...
            for valid_class in valid_classes:
                ...
                for index in det_cls:
                    if index in allowed_labeles and index != valid_class and self.get_supercategory:
                        det_results[i][index] = np.concatenate((det_results[i][index], results[valid_class]))
                    elif index not in allowed_labeles and self.filter_labels:
                        # Remove useless parts
                        det_results[i][index] = np.empty((0, 5)).astype(np.float32)
        return det_results
    ...
    def load_image_label_from_csv(self, image_level_ann_file):
        ...
        with open(image_level_ann_file) as f:
            reader = csv.reader(f)
            for i, line in enumerate(reader):
                if i == 0:
                    continue
    ...
    def evaluate(
        ...
        if self.get_metas and self.load_from_file:
            assert self.meta_file.endswith("pkl"), "File name must be pkl suffix"
        ...
        results = self.process_results(results, annotations, image_level_annotations)
        if use_group_of:
        ...
        if len(self.CLASSES) == 500:
            ...
            warnings.warn("Cannot infer dataset type from the length of the classes. Set `oid_v6` as dataset type.")
    ...
    def load_annotations(self, ann_file):
        ...
        while i < len(lines):
            ...
            for j in range(img_gt_size):
                sp = lines[i + j].split()
                bboxes.append([float(sp[1]), float(sp[2]), float(sp[3]), float(sp[4])])
        ...
        with open(image_level_ann_file) as f:
            ...
            for line in reader:
                ...
                if i == 0:
                    ...
                    item_lists[img_id].append(dict(image_level_label=image_level_label, confidence=confidence))
        return item_lists
