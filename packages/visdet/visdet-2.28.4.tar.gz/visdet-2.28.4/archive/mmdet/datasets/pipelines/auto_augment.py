...
class Shear:
    ...
    def __init__(
        ...
        if isinstance(img_fill_val, (float, int)):
            ...
            assert len(img_fill_val) == 3, f"img_fill_val as tuple must have 3 elements. got {len(img_fill_val)}."
        ...
        assert 0.0 <= max_shear_magnitude <= 1.0, (
            f"Defaultly max_shear_magnitude should be in range [0,1]. got {max_shear_magnitude}."
        )
        self.level = level
...
class Rotate:
    ...
    def __init__(
        ...
        if isinstance(center, (int, float)):
            ...
            assert len(center) == 2, f"center with type tuple must have 2 elements. got {len(center)} elements."
        else:
