...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def __init__(
        ...
        train_cfg=None,
        test_cfg=None,
        init_cfg=dict(type="Normal", std=0.01, override=dict(name="convs_pred")),
    ):
    ...
    def loss(self, pred_maps, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore=None):
        ...
        for img_id in range(len(img_metas)):
            responsible_flag_list.append(
                self.prior_generator.responsible_flags(featmap_sizes, gt_bboxes[img_id], device)
            )
    ...
    def _get_targets_single(self, anchors, responsible_flags, gt_bboxes, gt_labels):
        ...
        if self.one_hot_smoother != 0:  # label smooth
            gt_labels_one_hot = (
                gt_labels_one_hot * (1 - self.one_hot_smoother) + self.one_hot_smoother / self.num_classes
            )
