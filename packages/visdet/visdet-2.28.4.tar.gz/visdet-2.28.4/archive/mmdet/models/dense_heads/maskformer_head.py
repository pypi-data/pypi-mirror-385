...
class MaskFormerHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        loss_mask=dict(type="FocalLoss", use_sigmoid=True, gamma=2.0, alpha=0.25, loss_weight=20.0),
        loss_dice=dict(
            type="DiceLoss",
            use_sigmoid=True,
            activate=True,
    ...
    def loss_single(self, cls_scores, mask_preds, gt_labels_list, gt_masks_list, img_metas):
        ...
        # shape (num_total_gts, h, w)
        mask_preds = F.interpolate(mask_preds.unsqueeze(1), target_shape, mode="bilinear", align_corners=False).squeeze(
            1
        )
    ...
    def forward(self, feats, img_metas):
        ...
        padding_mask = (
            F.interpolate(padding_mask.unsqueeze(1), size=feats[-1].shape[-2:], mode="nearest")
            .to(torch.bool)
            .squeeze(1)
        )
