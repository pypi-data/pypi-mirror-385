...
class SCNetRoIHead(CascadeRoIHead):
    ...
    def simple_test(self, x, proposal_list, img_metas, rescale=False):
        ...
        det_bbox_results = [
            bbox2result(det_bboxes[i], det_labels[i], self.bbox_head[-1].num_classes) for i in range(num_imgs)
        ]
        ...
        if self.with_mask:
            if all(det_bbox.shape[0] == 0 for det_bbox in det_bboxes):
                ...
                _bboxes = [
                    det_bboxes[i][:, :4] * scale_factors[i] if rescale else det_bboxes[i] for i in range(num_imgs)
                ]
