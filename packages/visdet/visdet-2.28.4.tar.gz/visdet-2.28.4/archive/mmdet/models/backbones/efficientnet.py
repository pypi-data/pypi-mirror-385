...
class EfficientNet(BaseModule):
    ...
    def __init__(
        ...
        assert arch in self.arch_settings, (
            f'"{arch}" is not one of the arch_settings ({", ".join(self.arch_settings.keys())})'
        )
        ...
        for index in out_indices:
            if index not in range(0, len(self.layer_setting)):
                raise ValueError(
                    f"the item in out_indices must in range(0, {len(self.layer_setting)}). But received {index}"
                )

        if frozen_stages not in range(len(self.layer_setting) + 1):
            raise ValueError(
                f"frozen_stages must be in range(0, {len(self.layer_setting) + 1}). But received {frozen_stages}"
            )
