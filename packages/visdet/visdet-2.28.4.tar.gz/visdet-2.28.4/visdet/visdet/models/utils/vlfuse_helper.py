...
class BiMultiHeadAttention(nn.Module):
    ...
    def __init__(
        self, v_dim: int, l_dim: int, embed_dim: int, num_heads: int, dropout: float = 0.1
    ):
        super().__init__()
...
class BiAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        for _i, feat_per_level in enumerate(visual_features):
    ...
    def single_attention_call(
        ...
        delta_v, delta_l = self.attn(
            visual, lang, attention_mask_v=attention_mask_v, attention_mask_l=attention_mask_l
        )
    ...
    def forward(self, x: dict) -> dict:
        ...
        if self.use_checkpoint:
            ...
            vf0, vf1, vf2, vf3, vf4, language_features = self.b_attn(
                *visual_features, language_dict_features["hidden"], language_dict_features["masks"]
            )
