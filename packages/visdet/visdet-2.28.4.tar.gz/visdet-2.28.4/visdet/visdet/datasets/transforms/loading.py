...
class LoadAnnotations(MMCV_LoadAnnotations):
    ...
    def _poly2mask(self, mask_ann: list | dict, img_h: int, img_w: int) -> np.ndarray:
...
class LoadProposals(BaseTransform):
    ...
    def __init__(self, num_max_proposals: int | None = None) -> None:
...
class FilterAnnotations(BaseTransform):
    ...
    def transform(self, results: dict) -> dict | None:
        ...
        if self.by_box:
            tests.append(
                ((gt_bboxes.widths > self.min_gt_bbox_wh[0]) & (gt_bboxes.heights > self.min_gt_bbox_wh[1])).numpy()
            )
        if self.by_mask:
    ...
    def __repr__(self):
        return self.__class__.__name__ + f"(min_gt_bbox_wh={self.min_gt_bbox_wh}, keep_empty={self.keep_empty})"
...
class LoadEmptyAnnotations(BaseTransform):
    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.with_seg:
            h, w = results["img_shape"]
            results["gt_seg_map"] = self.seg_ignore_label * np.ones((h, w), dtype=np.uint8)
        return results
...
class InferencerLoader(BaseTransform):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.from_ndarray = TRANSFORMS.build(dict(type="visdet.LoadImageFromNDArray", **kwargs))

    def transform(self, results: str | np.ndarray | dict) -> dict:
        """Transform function to add image meta information.
