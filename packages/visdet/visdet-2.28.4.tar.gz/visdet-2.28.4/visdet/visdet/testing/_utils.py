...
def _get_config_directory():
    ...
    try:
        # Assume we are running in the source visdet repo
        repo_dpath = dirname(dirname(dirname(__file__)))
    except NameError:
        # For IPython development when this __file__ is not defined
        import visdet

        repo_dpath = dirname(dirname(visdet.__file__))
...
def _rand_masks(rng, num_boxes, bboxes, img_w, img_h):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        mask = (rng.rand(1, bbox[3] - bbox[1], bbox[2] - bbox[0]) > 0.3).astype(np.int64)
...
def demo_mm_inputs(
    ...
    for idx in range(batch_size):
        ...
        if with_semantic:
            ...
            gt_sem_seg_data = {"sem_seg": gt_semantic_seg}
...
def demo_mm_sampling_results(
    ...
    for i in range(len(batch_gt_instances)):
        ...
        assign_result = bbox_assigner.assign(proposals, batch_gt_instances[i], batch_gt_instances_ignore[i])
        sampling_result = bbox_sampler.sample(assign_result, proposals, batch_gt_instances[i], feats=feats)
        sampling_results.append(sampling_result)

    return sampling_results


def random_boxes(num_boxes, scale, rng=None):
    """Simple function to generate random boxes.

    Args:
        num_boxes (int): number of boxes
        scale (float or int): a scale factor that is multiplied to the
            generated boxes
        rng (None or int or numpy.random.RandomState): seed or random number
            generator

    Returns:
        boxes: random boxes with shape of (num_boxes, 4). Each row is in
            (x1, y1, x2, y2) format.
    """
    rng = np.random.RandomState(0 if rng is None else rng)

    box_coord = rng.rand(num_boxes, 4) * scale
