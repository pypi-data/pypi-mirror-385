...
def find_latest_checkpoint(path, suffix="pth"):
    ...
    if not osp.exists(path):
        warnings.warn("The path of checkpoints does not exist.", stacklevel=2)
        return None
    ...
    if len(checkpoints) == 0:
        warnings.warn("There are no checkpoints in the path.", stacklevel=2)
        return None
    latest = -1
    latest_path = None
...
def get_test_pipeline_cfg(cfg: str | ConfigDict) -> ConfigDict:
