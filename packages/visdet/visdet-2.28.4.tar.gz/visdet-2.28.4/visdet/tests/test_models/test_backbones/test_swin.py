...
def test_swin_block():
    ...
    # Flash backend should construct even when flash attention is unavailable on CPU
    block_flash = SwinBlock(embed_dims=64, num_heads=4, feedforward_channels=256, attn_backend="flash")
    x_out_flash = block_flash(x, (56, 56))
    assert x_out_flash.shape == torch.Size([1, 56 * 56, 64])
    expected_backend = "flash" if flash_swin_attn_func is not None else "torch"
    assert block_flash.attn.w_msa.backend == expected_backend
...
def test_swin_transformer_flash_backend_cpu():
    model = SwinTransformer(attn_backend="flash")
    temp = torch.randn((1, 3, 32, 32))
    outs = model(temp)
    assert outs[0].shape == (1, 96, 8, 8)
    expected_backend = "flash" if flash_swin_attn_func is not None else "torch"
    for stage in model.stages:
        for block in stage.blocks:
            assert block.attn.w_msa.backend == expected_backend
