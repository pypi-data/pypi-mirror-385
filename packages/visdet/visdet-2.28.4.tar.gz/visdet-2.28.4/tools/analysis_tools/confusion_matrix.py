...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="nms IoU threshold, only applied when users want to change thenms IoU threshold.",
    )
    parser.add_argument(
        "--cfg-options",
        nargs="+",
    ...
    for i in range(num_classes):
        for j in range(num_classes):
            ax.text(
                j,
                i,
                f"{int(confusion_matrix[i, j]) if not np.isnan(confusion_matrix[i, j]) else -1}%",
                ha="center",
                va="center",
                color="w",
                size=7,
