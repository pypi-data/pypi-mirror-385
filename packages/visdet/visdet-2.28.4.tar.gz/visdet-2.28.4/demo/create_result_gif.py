...
def parse_args():
    ...
    parser.add_argument("--out", type=str, default="result.gif", help="gif path where will be saved")
...
def create_gif(frames, gif_name, duration=2):
    ...
    """
    if imageio is None:
        raise RuntimeError("imageio is not installed,Please use “pip install imageio” to install")
...
def create_frame_by_matplotlib(image_dir, nrows=1, fig_size=(300, 300), font_size=15):
    ...
    for paths in _generate_batch_data(zip(*images_list), nrows):
        ...
        fig.suptitle("Good/bad case selected according to the COCO mAP of the single image")
        ...
        fig.tight_layout()
        # set subplot margin
        plt.subplots_adjust(hspace=0.05, wspace=0.05, left=0.02, right=0.98, bottom=0.02, top=0.98)

        for i, (path_tuple, ax_tuple) in enumerate(zip(paths, axes)):
            image_path_left = osp.join(osp.join(image_dir, result_dir_names[0], path_tuple[0]))
            image_path_right = osp.join(osp.join(image_dir, result_dir_names[1], path_tuple[1]))
            ...
            ax_tuple[0].imshow(image_left, extent=(0, *fig_size, 0), interpolation="bilinear")
            ax_tuple[0].axis("off")
            ax_tuple[1].imshow(image_right, extent=(0, *fig_size, 0), interpolation="bilinear")
            ax_tuple[1].axis("off")
