...
def test_ascend_anchor_head_loss():
    ...
    feat = [
        torch.rand(1, 1, s // (2 ** (i + 2)), s // (2 ** (i + 2))) for i in range(len(self.prior_generator.strides))
    ]
...
def test_ascend_retina_head_loss():
    ...
    img_metas = [{"img_shape": img_shape, "scale_factor": 1, "pad_shape": pad_shape}]
    ...
    self = AscendRetinaHead(num_classes=num_classes, in_channels=in_channels, train_cfg=cfg)

    # Anchor head expects a multiple levels of features per image
    feat = [
        torch.rand(1, in_channels, pad_shape[0] // strides[0], pad_shape[1] // strides[1])
...
def test_ascend_ssd_head_loss():
    ...
    img_metas = [
        {"img_shape": img_shape, "scale_factor": 1, "pad_shape": pad_shape},
        {"img_shape": img_shape, "scale_factor": 1, "pad_shape": pad_shape},
    ]
