...
def test_pisa_retinanet_head_loss():
    ...
    feat = [
        torch.rand(1, 1, s // (2 ** (i + 2)), s // (2 ** (i + 2))) for i in range(len(self.anchor_generator.strides))
    ]
...
def test_pisa_roi_head_loss():
    ...
    proposal_list = [torch.Tensor([[22.6667, 22.8757, 238.6326, 151.8874], [0, 3, 5, 7]])]
    ...
    empty_gt_losses = self.forward_train(feat, img_metas, proposal_list, gt_bboxes, gt_labels, gt_bboxes_ignore)
    ...
    one_gt_losses = self.forward_train(feat, img_metas, proposal_list, gt_bboxes, gt_labels, gt_bboxes_ignore)
    onegt_cls_loss = one_gt_losses["loss_cls"].sum()
    onegt_box_loss = one_gt_losses["loss_bbox"].sum()
    assert onegt_cls_loss.item() > 0, "cls loss should be non-zero"
    assert onegt_box_loss.item() > 0, "box loss should be non-zero"
