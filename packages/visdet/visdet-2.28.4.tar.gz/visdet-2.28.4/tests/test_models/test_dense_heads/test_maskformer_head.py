...
def test_maskformer_head_loss():
    ...
    empty_gt_losses = self.loss(all_cls_scores, all_mask_preds, gt_labels_list, gt_masks_list, img_metas)
    ...
    # random inputs
    gt_labels_list = [torch.tensor([10, 100]).long(), torch.tensor([100, 10]).long()]
    ...
    two_gt_losses = self.loss(all_cls_scores, all_mask_preds, gt_labels_list, gt_masks_list, img_metas)
    ...
    self.forward_train(feats, img_metas, gt_bboxes, gt_labels, gt_masks, gt_semantic_seg)
