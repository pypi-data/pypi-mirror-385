...
def test_sabl_retina_head_loss():
    ...
    if torch.cuda.is_available():
        ...
        empty_gt_losses = head.loss(cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore)
        ...
        assert empty_box_cls_loss.item() == 0, "there should be no box cls loss when there are no true boxes"
        assert empty_box_reg_loss.item() == 0, "there should be no box reg loss when there are no true boxes"
        ...
        one_gt_losses = head.loss(cls_scores, bbox_preds, gt_bboxes, gt_labels, img_metas, gt_bboxes_ignore)
