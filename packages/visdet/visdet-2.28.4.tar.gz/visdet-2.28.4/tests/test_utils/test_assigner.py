...
def test_center_region_assigner_with_ignore():
    ...
    assign_result = self.assign(bboxes, gt_bboxes, gt_bboxes_ignore=gt_bboxes_ignore, gt_labels=gt_labels)
...
def test_uniform_assigner():
    ...
    gt_labels = torch.LongTensor([2, 3])
    assign_result = self.assign(pred_bbox, anchor, gt_bboxes, gt_labels=gt_labels)
    assert len(assign_result.gt_inds) == 4
    assert len(assign_result.labels) == 4
...
def test_sim_ota_assigner():
    self = SimOTAAssigner(center_radius=2.5, candidate_topk=1, iou_weight=3.0, cls_weight=1.0)
    ...
    assign_result = self.assign(pred_scores, priors, decoded_bboxes, gt_bboxes, gt_labels)
...
def test_task_aligned_assigner():
    ...
    assign_result = self.assign(pred_score, pred_bbox, anchor, gt_bboxes=gt_bboxes, gt_labels=gt_labels)
    assert len(assign_result.gt_inds) == 4
    assert len(assign_result.labels) == 4
    ...
    assign_result = self.assign(pred_score, pred_bbox, anchor, gt_bboxes=gt_bboxes)
    expected_gt_inds = torch.LongTensor([0, 0, 0, 0])
    assert torch.all(assign_result.gt_inds == expected_gt_inds)
...
def test_mask_hungarian_match_assigner():
    ...
    img_meta = None
    assign_result = self.assign(cls_pred, mask_pred, gt_labels, gt_masks, img_meta)
    assert torch.all(assign_result.gt_inds == 0)
    assert torch.all(assign_result.labels == -1)
    ...
    assigner_cfg = dict(
        cls_cost=dict(type="ClassificationCost", weight=0.0),
        mask_cost=dict(type="FocalLossCost", weight=0.0, binary_input=True),
        dice_cost=dict(type="DiceCost", weight=1.0, pred_act=True, eps=1.0, naive_dice=False),
    )
    self = MaskHungarianAssigner(**assigner_cfg)
    assign_result = self.assign(cls_pred, mask_pred, gt_labels, gt_masks, img_meta)
    assert torch.all(assign_result.gt_inds > -1)
    ...
    assigner_cfg = dict(
        cls_cost=dict(type="ClassificationCost", weight=0.0),
        mask_cost=dict(type="CrossEntropyLossCost", weight=1.0, use_sigmoid=True),
        dice_cost=dict(type="DiceCost", weight=0.0, pred_act=True, eps=1.0),
    )
    self = MaskHungarianAssigner(**assigner_cfg)
    assign_result = self.assign(cls_pred, mask_pred, gt_labels, gt_masks, img_meta)
    ...
    assigner_cfg = dict(
        cls_cost=dict(type="ClassificationCost", weight=0.0),
        mask_cost=dict(type="CrossEntropyLossCost", weight=1.0, use_sigmoid=False),
        dice_cost=dict(type="DiceCost", weight=0.0, pred_act=True, eps=1.0),
    )
    with pytest.raises(AssertionError):
        self = MaskHungarianAssigner(**assigner_cfg)
...
def test_ascend_max_iou_assigner():
    ...
    assert torch.all(
        assign_result.batch_anchor_gt_indes * assign_result.batch_pos_mask == expected_batch_anchor_gt_indes
    )
    assert torch.all(
        assign_result.batch_anchor_gt_labels * assign_result.batch_pos_mask == expected_batch_anchor_gt_labels
    )
