...
def test_dataset_wrapper():
    ...
    for _ in range(len_a):
        ...
        gt_bbox = np.concatenate(
            [np.random.randint(1, 5, (2, 2)), np.random.randint(1, 5, (2, 2)) + 5],
            axis=1,
        )
    ...
    for idx in np.random.randint(0, len(repeat_factor_dataset), 3):
        ...
        assert (
            repeat_factor_dataset.get_ann_info(idx) == ann_info_list_a[bisect.bisect_right(repeat_factors_cumsum, idx)]
        )
    ...
    multi_image_mix_dataset = MultiImageMixDataset(
        dataset_a, pipeline, skip_type_keys=("MixUp", "RandomFlip", "Resize", "Pad")
    )
    for idx in range(len_a):
        results_ = multi_image_mix_dataset[idx]
        assert results_["img"].shape == (img_scale[0], img_scale[1], 3)
