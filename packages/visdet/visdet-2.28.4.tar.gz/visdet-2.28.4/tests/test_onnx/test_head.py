...
def test_cascade_onnx_export():
    ...
    with torch.no_grad():
        ...
        dynamic_axes = {
            "input_img": {0: "batch", 2: "width", 3: "height"},
            "dets": {
                0: "batch",
                1: "num_dets",
            },
...
def test_retina_head_forward():
    ...
    feats = [
        torch.rand(1, retina_model.in_channels, s // (2 ** (i + 2)), s // (2 ** (i + 2)))  # [32, 16, 8, 4, 2]
...
def test_fcos_head_onnx_export():
    ...
    cls_scores = [
        torch.rand(1, fcos_model.num_classes, s // feat_size, s // feat_size) for feat_size in [4, 8, 16, 32, 64]
    ]
...
def test_fsaf_head_forward():
    ...
    s = 128
    feats = [
        torch.rand(1, fsaf_model.in_channels, s // (2 ** (i + 2)), s // (2 ** (i + 2)))
...
def test_ssd_head_forward():
    ...
    feats = [
        torch.rand(1, ssd_model.in_channels[i], featmap_size[i], featmap_size[i])
