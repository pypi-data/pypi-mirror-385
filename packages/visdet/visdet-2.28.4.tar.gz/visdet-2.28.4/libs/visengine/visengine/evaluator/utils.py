...
def get_metric_value(indicator: str, metrics: dict) -> Any:
    ...
    if "/" in indicator:
        ...
        if indicator in metrics:
            ...
            raise ValueError(f'The indicator "{indicator}" can not match any metric in {list(metrics.keys())}')
    else:
        ...
        if not matched:
            ...
            raise ValueError(f'The indicator "{indicator}" matches multiple metrics {matched}')
        else:
