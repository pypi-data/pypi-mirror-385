...
from visengine.registry import EVALUATOR, METRICS
...
class Evaluator:
    ...
    def __init__(self, metrics: dict | BaseMetric | Sequence):
    ...
    def process(self, data_samples: Sequence[BaseDataElement], data_batch: Any | None = None):
    ...
    def offline_evaluate(self, data_samples: Sequence, data: Sequence | None = None, chunk_size: int = 1):
        ...
        if data is not None:
            assert len(data_samples) == len(data), (
                f"data_samples and data should have the same length, but got data_samples length: {len(data_samples)} data length: {len(data)}"
            )
