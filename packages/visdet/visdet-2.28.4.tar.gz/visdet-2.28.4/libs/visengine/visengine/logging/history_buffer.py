...
class HistoryBuffer:
    ...
    def __init__(
        ...
        if len(log_history) > max_length:
            warnings.warn(
                f"The length of history buffer({len(log_history)}) exceeds the max_length({max_length}), the first few elements will be ignored.",
                stacklevel=2,
            )
    ...
    def update(self, log_val: int | float, count: int = 1) -> None:
    ...
    def mean(self, window_size: int | None = None) -> np.ndarray:
    ...
    def max(self, window_size: int | None = None) -> np.ndarray:
    ...
    def min(self, window_size: int | None = None) -> np.ndarray:
