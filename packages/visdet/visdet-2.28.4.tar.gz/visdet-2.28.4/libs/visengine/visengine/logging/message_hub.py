...
from visengine.utils import ManagerMixin
...
class MessageHub(ManagerMixin):
    ...
    def __init__(
        self,
        name: str,
        log_scalars: dict | None = None,
        runtime_info: dict | None = None,
        resumed_keys: dict | None = None,
    ):
        super().__init__(name)
        ...
        for key in self._resumed_keys.keys():
            assert key in self._log_scalars or key in self._runtime_info, (
                f"Key in `resumed_keys` must contained in `log_scalars` or `runtime_info`, but got {key}"
            )
    ...
    def update_scalars(self, log_dict: dict, resumed: bool = True) -> None:
        ...
        Examples:
            ...
            >>> # The count of `c` is 2.
        """
        assert isinstance(log_dict, dict), f"`log_dict` must be a dict!, but got {type(log_dict)}"
    ...
    def pop_info(self, key: str, default: Any | None = None) -> Any:
    ...
    def update_info_dict(self, info_dict: dict, resumed: bool = True) -> None:
        ...
        Args:
            ...
            resumed (bool): Whether the corresponding ``HistoryBuffer``
                could be resumed.
        """
        assert isinstance(info_dict, dict), f"`log_dict` must be a dict!, but got {type(info_dict)}"
    ...
    def _set_resumed_keys(self, key: str, resumed: bool) -> None:
        ...
        if key not in self._resumed_keys:
            ...
            assert self._resumed_keys[key] == resumed, (
                f"{key} used to be {self._resumed_keys[key]}, but got {{resumed}} now. resumed keys cannot be modified repeatedly."
            )
        ...
        for key, value in self._runtime_info.items():
            if self._resumed_keys.get(key, False):
                try:
                    ...
                    print_log(
                        f"{key} in message_hub cannot be copied, just return its reference. ",
                        logger="current",
    ...
    def load_state_dict(self, state_dict: Union["MessageHub", dict]) -> None:
        ...
        if isinstance(state_dict, dict):
            for key in ("log_scalars", "runtime_info", "resumed_keys"):
                assert key in state_dict, f"The loaded `state_dict` of `MessageHub` must contain key: `{key}`"
            ...
            for key, value in state_dict["log_scalars"].items():
                if not isinstance(value, HistoryBuffer):
                    print_log(
                        f"{key} in message_hub is not HistoryBuffer, just skip resuming it.",
                        logger="current",
                        level=logging.WARNING,
                    )
                    continue
            ...
            for key, value in state_dict["resumed_keys"].items():
                if key not in set(self.log_scalars.keys()) | set(self._runtime_info.keys()):
                    print_log(
                        f"resumed key: {key} is not defined in message_hub, just skip resuming this key.",
                        logger="current",
                        level=logging.WARNING,
                    )
                    continue
    ...
    def _parse_input(self, name: str, value: Any) -> OrderedDict:
        ...
        if value is None:
            ...
            raise TypeError(f"{name} should be a dict or `None`, but got {type(name)}")
