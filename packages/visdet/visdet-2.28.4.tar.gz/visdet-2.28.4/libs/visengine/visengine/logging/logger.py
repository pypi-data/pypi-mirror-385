...
from visengine.utils.manager import _accquire_lock, _release_lock
...
class MMLogger(Logger, ManagerMixin):
    ...
    def __init__(
        ...
        file_mode: str = "w",
        distributed=False,
        file_handler_cfg: dict | None = None,
    ):
...
def print_log(msg, logger: Logger | str | None = None, level=logging.INFO) -> None:
    """Print a log message.
...
def _get_world_size():
    ...
    try:
        # requires torch
        from visengine.dist import get_world_size
    except ImportError:
        return 1
    else:
...
def _get_rank():
    ...
    try:
        # requires torch
        from visengine.dist import get_rank
    except ImportError:
        return 0
    else:
        return get_rank()
...
def _get_host_info() -> str:
    ...
    try:
        ...
        warnings.warn(f"Host or user not found: {e!s}", stacklevel=2)
    return host
