...
import visengine
from visengine.version import __version__
from visengine.utils.version_utils import get_git_hash
from visengine.config import Config, ConfigDict
from visengine.dataset import worker_init_fn as default_worker_init_fn
from visengine.device import get_device
from visengine.dist import (
    broadcast,
    get_dist_info,
    get_rank,
    get_world_size,
...
from visengine.evaluator import Evaluator
from visengine.fileio import FileClient, join_path
from visengine.hooks import Hook
from visengine.logging import MessageHub, MMLogger, print_log
from visengine.model import (
...
from visengine.utils import apply_to, get_git_hash, is_seq_of
from visengine.utils.dl_utils import collect_env, set_multi_processing
...
class Runner:
    ...
    cfg: Config
    _train_loop: BaseLoop | dict | None
    _val_loop: BaseLoop | dict | None
    _test_loop: BaseLoop | dict | None
    readable_config: dict | None
    ...
    def __init__(
        ...
        test_dataloader: DataLoader | dict | None = None,
        train_cfg: dict | None = None,
        val_cfg: dict | None = None,
        test_cfg: dict | None = None,
        auto_scale_lr: dict | None = None,
        optim_wrapper: OptimWrapper | dict | None = None,
        ...
        data_preprocessor: nn.Module | dict | None = None,
        ...
        self.optim_wrapper: OptimWrapper | dict | None
        ...
        if param_scheduler is not None and self.optim_wrapper is None:
            raise ValueError(f"param_scheduler should be None when optim_wrapper is None, but got {param_scheduler}")
    ...
    def from_cfg(cls, cfg: ConfigType) -> "Runner":
        ...
        runner = cls(
            ...
            env_cfg=cfg.get("env_cfg", {"dist_cfg": {"backend": "nccl"}}),
    ...
    def setup_env(self, env_cfg: dict) -> None:
        ...
        if platform.system() != "Windows":
            ...
            hard_limit = rlimit[1]
            soft_limit = min(max(env_cfg.get("resource_limit", 4096), base_soft_limit), hard_limit)
    ...
    def set_randomness(self, seed, diff_rank_seed: bool = False, deterministic: bool = False) -> None:
        ...
        self._seed = set_random_seed(seed=seed, deterministic=deterministic, diff_rank_seed=diff_rank_seed)

    def build_logger(self, log_level: int | str = "INFO", log_file: str | None = None, **kwargs) -> MMLogger:
    ...
    def build_message_hub(self, message_hub: dict | None = None) -> MessageHub:
        ...
        if message_hub is None:
            message_hub = {"name": self._experiment_name}
        elif isinstance(message_hub, dict):
            # ensure message_hub containing name key
            message_hub.setdefault("name", self._experiment_name)
        else:
            raise TypeError(f"message_hub should be dict or None, but got {message_hub}")

        return MessageHub.get_instance(**message_hub)

    def build_visualizer(self, visualizer: Visualizer | dict | None = None) -> Visualizer:
        ...
        if isinstance(visualizer, dict):
            ...
            raise TypeError(f"visualizer should be Visualizer object, a dict or None, but got {visualizer}")

    def build_model(self, model: nn.Module | dict) -> nn.Module:
        """Build model.
        ...
        if isinstance(model, nn.Module):
            ...
            raise TypeError(f"model should be a nn.Module object or dict, but got {model}")

    def wrap_model(self, model_wrapper_cfg: dict | None, model: nn.Module) -> DistributedDataParallel | nn.Module:
        ...
        if not self.distributed:
            ...
            if sync_bn is not None:
                try:
                    ...
                    self.logger.error(f'cfg.sync_bn should be "torch" or "mmcv", but got {sync_bn}')
                    raise e
        if model_wrapper_cfg is None:
            ...
            model_wrapper_type = MODEL_WRAPPERS.get(model_wrapper_cfg.get("type"))  # type: ignore
            default_args: dict = {}
    ...
    def _init_model_weights(self) -> None:
        ...
        if hasattr(model, "init_weights"):
            ...
            for _name, params in model.state_dict().items():
                broadcast(params)

    def scale_lr(self, optim_wrapper: OptimWrapper, auto_scale_lr: dict | None = None) -> None:
        ...
        assert "base_batch_size" in auto_scale_lr, "Lack of `base_batch_size` in `auto_scale_lr`."
        dataloader: DataLoader | dict = self._train_dataloader
        bs = dataloader.batch_size if isinstance(dataloader, DataLoader) else dataloader["batch_size"]
        ...
        def _is_built(schedulers):
            if isinstance(schedulers, dict):
                return False if "type" in schedulers else any(_is_built(s) for s in schedulers.values())
        ...
        assert isinstance(optim_wrapper, OptimWrapper), "`scale_lr should be called after building OptimWrapper"
        wrappers = list(optim_wrapper.values()) if isinstance(optim_wrapper, OptimWrapperDict) else [optim_wrapper]
        for wrapper in wrappers:
            for group in wrapper.optimizer.param_groups:
                group["lr"] = group["lr"] * ratio

    def build_optim_wrapper(self, optim_wrapper: Optimizer | OptimWrapper | dict) -> OptimWrapper | OptimWrapperDict:
            ...
            if optimizer is not None or "constructor" in optim_wrapper:
                ...
                for name, optim in optim_wrapper.items():
                    if not isinstance(optim, OptimWrapper):
                        raise ValueError(
                            f'each item mush be an optimizer object when "type" and "constructor" are not in optimizer, but got {name}={optim}'
                        )
    ...
    def build_param_scheduler(self, scheduler: _ParamScheduler | dict | list) -> ParamSchedulerType:
        ...
        Examples:
            ...
            >>> schedulers
            [<visengine.optim.scheduler.lr_scheduler.MultiStepLR at 0x7f70f6966290>]  # noqa: E501
            ...
            >>> schedulers = runner.build_param_scheduler(scheduler_cfg)
            >>> schedulers
            [<visengine.optim.scheduler.lr_scheduler.MultiStepLR at 0x7f70f60dd3d0>,  # noqa: E501
            <visengine.optim.scheduler.lr_scheduler.StepLR at 0x7f70f6eb6150>]
        ...
        .. _optimizer-docs:
           https://visengine.readthedocs.io/en/latest/tutorials/optim_wrapper.html
        """
        ...
        if not isinstance(self.optim_wrapper, OptimWrapperDict):
            ...
            assert isinstance(self.optim_wrapper, OptimWrapper), (
                "`build_optimizer` should be called before`build_param_scheduler` because the latter depends on the former"
            )
            param_schedulers = self._build_param_scheduler(scheduler, self.optim_wrapper)  # type: ignore
            return param_schedulers
        else:
            param_schedulers = {}
            for name, optimizer in self.optim_wrapper.items():
                if isinstance(scheduler, dict) and "type" not in scheduler:
                    ...
                    param_schedulers[name] = self._build_param_scheduler(scheduler[name], optimizer)
                else:
                    param_schedulers[name] = self._build_param_scheduler(scheduler, optimizer)

            return param_schedulers

    def build_evaluator(self, evaluator: dict | list | Evaluator) -> Evaluator:
        """Build evaluator.
        ...
        if isinstance(evaluator, Evaluator):
            ...
            raise TypeError(f"evaluator should be one of dict, list of dict, and Evaluator, but got {evaluator}")
    ...
    def build_dataloader(
        ...
        if isinstance(sampler_cfg, dict):
            ...
            sampler = DATA_SAMPLERS.build(sampler_cfg, default_args={"dataset": dataset, "seed": sampler_seed})
        else:
            # fallback to raise error in dataloader
        ...
        if "worker_init_fn" in dataloader_cfg:
            ...
            if seed is not None:
                ...
                assert isinstance(disable_subprocess_warning, bool), (
                    f"disable_subprocess_warning should be a bool, but got {type(disable_subprocess_warning)}"
                )
                init_fn = partial(
        ...
        # However, in visengine, if `collate_fn` is not defined in
        ...
        if isinstance(collate_fn_cfg, dict):
            ...
            raise TypeError(f"collate_fn should be a dict or callable object, but got {collate_fn_cfg}")
    ...
    def build_train_loop(self, loop: BaseLoop | dict) -> BaseLoop:
        """Build training loop.
        ...
        if isinstance(loop, BaseLoop):
            ...
            raise TypeError(f"train_loop should be a Loop object or dict, but got {loop}")
    ...
    def build_val_loop(self, loop: BaseLoop | dict) -> BaseLoop:
    ...
    def build_test_loop(self, loop: BaseLoop | dict) -> BaseLoop:
        """Build test loop.
        ...
        if isinstance(loop, BaseLoop):
            ...
            raise TypeError(f"test_loop should be a Loop object or dict, but got {loop}")
    ...
    def build_log_processor(self, log_processor: LogProcessor | dict) -> LogProcessor:
        ...
        if isinstance(log_processor, LogProcessor):
            ...
            raise TypeError(f"log processor should be a LogProcessor object or dict, butgot {log_processor}")
    ...
    def train(self) -> nn.Module:
        ...
        if modules is not None:
            self.logger.info(f'Enabling the "efficient_conv_bn_eval" feature for sub-modules: {modules}')
    ...
    def register_hook(self, hook: Hook | dict, priority: str | int | Priority | None = None) -> None:
        ...
        if not isinstance(hook, Hook | dict):
            raise TypeError(f"hook should be an instance of Hook or dict, but got {hook}")
    ...
    def register_default_hooks(self, hooks: dict[str, Hook | dict] | None = None) -> None:
        ...
        """
        default_hooks: dict = {
            "runtime_info": {"type": "RuntimeInfoHook"},
    ...
    def resume(
        ...
        if resumed_seed is not None and resumed_seed != current_seed:
            if current_seed is not None:
                self.logger.warning(
                    f'The value of random seed in the checkpoint "{resumed_seed}" is different from the value in `randomness` config "{current_seed}"'
                )
    ...
    def save_checkpoint(
        ...
        Args:
            ...
            file_client_args (dict, optional): Arguments to instantiate a
                FileClient. See :class:`visengine.fileio.FileClient` for
        ...
        if file_client_args is not None:
            ...
            filepath = join_path(out_dir, filename, backend_args=backend_args)  # type: ignore
    ...
    def _check_scheduler_cfg(self, param_scheduler: dict | list | _ParamScheduler | None) -> None:
        ...
        Examples:
            ...
            >>> from visengine.optim import MultiStepLR
        ...
        if is_seq_of(param_scheduler, dict):
            ...
            if "type" not in param_scheduler:
                for _key, _param_scheduler in param_scheduler.items():
                    assert isinstance(_param_scheduler, dict | tuple | list | _ParamScheduler), (
                        f"Each value of `param_scheduler` should be a dict or a list, but got {_param_scheduler} with type {type(_ParamScheduler)}"
                    )
    ...
    def _maybe_compile(self, target: str) -> None:
        ...
        self.logger.info('Model has been "compiled". The first few iterations will be slow, please be patient.')
