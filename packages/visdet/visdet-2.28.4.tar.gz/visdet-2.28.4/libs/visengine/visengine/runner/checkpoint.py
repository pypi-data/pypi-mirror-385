...
import visengine
from visengine.dist import get_dist_info
from visengine.fileio import FileClient, get_file_backend
from visengine.fileio import load as load_file
from visengine.logging import print_log
from visengine.model import is_model_wrapper
from visengine.utils import apply_to, deprecated_function, digit_version, mkdir_or_exist
from visengine.utils.dl_utils import load_url
...
class CheckpointLoader:
    ...
    def load_checkpoint(cls, filename, map_location=None, logger="current"):
        ...
        print_log(
            f"Loads checkpoint by {class_name[10:]} backend from path: {filename}",
            logger=logger,
        )
...
def load_from_local(filename, map_location):
    ...
    from visengine.logging.history_buffer import HistoryBuffer

    filename = osp.expanduser(filename)
    if not osp.isfile(filename):
        raise FileNotFoundError(f"{filename} can not be found.")

    # Try loading with safe_globals first
    try:
        with torch.serialization.safe_globals([HistoryBuffer]):
            # GEORGE EDIT HERE
            # weights_only=True to prevent warning on newer torch versions
        ...
        try:
            ...
            # If both attempts fail, try with weights_only=True as last resort
            try:
                checkpoint = torch.load(filename, map_location=map_location, weights_only=True)
                return checkpoint
            except Exception as e3:
                raise RuntimeError(
                    f"Failed to load checkpoint with all methods. Original file: {filename}\n"
                    f"1. safe_globals error: {e1!s}\n"
                    f"2. weights_only=False error: {e2!s}\n"
                    f"3. weights_only=True error: {e3!s}"
                )
...
def load_from_openmmlab(filename, map_location=None):
    ...
    if model_name in deprecated_urls:
        print_log(
            f"{prefix_str}{model_name} is deprecated in favor of {prefix_str}{deprecated_urls[model_name]}",
            logger="current",
            level=logging.WARNING,
        )
...
def _load_checkpoint_to_model(model, checkpoint, strict=False, logger=None, revise_keys=None):
...
@deprecated_function(
    since="0.3.0",
    removed_in="0.5.0",
    instructions="`_save_to_state_dict` will be deprecated in the future, please use `nn.Module._save_to_state_dict` directly.",
)
...
def get_state_dict(module, destination=None, prefix="", keep_vars=False):
    ...
    destination._metadata[prefix[:-1]] = local_metadata = {"version": module._version}
...
def save_checkpoint(checkpoint, filename, file_client_args=None, backend_args=None):
    ...
    if filename.startswith("pavi://"):
        if file_client_args is not None or backend_args is not None:
            raise ValueError('"file_client_args" or "backend_args" should be "None" if filename starts with "pavi://"')
        try:
...
def find_latest_checkpoint(path: str) -> str | None:
