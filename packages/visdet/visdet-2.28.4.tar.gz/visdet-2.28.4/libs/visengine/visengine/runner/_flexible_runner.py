...
from visengine.logging import MessageHub, print_log
from visengine.optim import OptimWrapper, OptimWrapperDict, _ParamScheduler
...
class FlexibleRunner:
    ...
    def __init__(
        ...
        *,
        ...
        strategy: BaseStrategy | dict | None = None,
        ...
        default_scope: str | None = "mmengine",
        ...
        if not isinstance(compile, bool) and not isinstance(compile, dict):
            raise TypeError(f"compile should be a bool or dict, but got {type(compile)}")
    ...
    def build_strategy(
        ...
        if isinstance(strategy, BaseStrategy):
            ...
            if launcher == "none":
                if strategy is None:
                    strategy = {"type": "SingleDeviceStrategy"}
            else:
                if strategy is None:
            ...
            if strategy_name == "DeepSpeedStrategy":
                if self._train_dataloader is None:
                    ...
                    strategy["train_micro_batch_size_per_gpu"] = _get_batch_size(self._train_dataloader)
    ...
    def build_message_hub(
        ...
        if message_hub is None:
            message_hub = {"name": self.experiment_name}
    ...
    def build_train_loop(self, loop: BaseLoop | dict) -> BaseLoop:
        ...
        if isinstance(loop, BaseLoop):
            ...
            raise TypeError(f"loop should be a Loop object or dict, but got {loop}")
    ...
    def build_log_processor(
        self,
        log_processor: LogProcessor | dict,
    ) -> LogProcessor:
    ...
    def train(self) -> nn.Module:
        ...
        dispatch_kwargs = {
            ...
            "train_micro_batch_size_per_gpu": _get_batch_size(self.train_dataloader),
        }  # type: ignore
    ...
    def val(self) -> dict:
        ...
        dispatch_kwargs = {"init_weights_for_test_or_val": self.cfg.get("init_weights_for_test_or_val", True)}
    ...
    def register_hook(
        self,
        hook: Hook | dict,
        priority: str | int | Priority | None = None,
    ) -> None:
    ...
    def register_default_hooks(
        self,
        hooks: dict[str, Hook | dict] | None = None,
    ) -> None:
    ...
    def save_checkpoint(
        ...
        meta.update(cfg=self.cfg.pretty_text, experiment_name=self.experiment_name)

        if hasattr(self.train_dataloader.dataset, "metainfo"):
            meta.update(dataset_meta=self.train_dataloader.dataset.metainfo)

        checkpoint = {"meta": meta, "message_hub": self.message_hub.state_dict()}
    ...
    def _log_env(self) -> None:
        ...
        env_info = "\n    " + "\n    ".join(f"{k}: {v}" for k, v in system_env.items())
