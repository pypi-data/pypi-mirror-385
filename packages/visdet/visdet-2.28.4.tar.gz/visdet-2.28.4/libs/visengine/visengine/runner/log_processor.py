...
from visengine.device import (
    get_max_cuda_memory,
    is_cuda_available,
)
from visengine.registry import LOG_PROCESSORS
...
class LogProcessor:
    ...
    def get_log_after_iter(self, runner, batch_idx: int, mode: str) -> tuple[dict, str]:
        ...
        if self.by_epoch:
            ...
            if mode in ["train", "val"]:
                ...
                if not (isinstance(runner._train_loop, dict) or runner._train_loop is None):
                    ...
                    # in " [{max_epochs}]"
                    cur_epoch_str = f"[{cur_epoch}]".rjust(len(str(max_epochs)) + 3, " ")
                else:
                    cur_epoch_str = f"[{cur_epoch}]"
                tag["epoch"] = cur_epoch
                log_str = f"Epoch({mode}){cur_epoch_str}[{cur_iter_str}/{dataloader_len}]  "
            else:
                log_str = f"Epoch({mode}) [{cur_iter_str}/{dataloader_len}]  "
        else:
            if mode == "train":
                cur_iter = self._get_iter(runner, batch_idx)
                cur_iter_str = str(cur_iter).rjust(len(str(runner.max_iters)))
                log_str = f"Iter({mode}) [{cur_iter_str}/{runner.max_iters}]  "
            else:
                dataloader_len = self._get_dataloader_size(runner, mode)
        ...
        # recorded.
        if all(item in log_tag for item in ["time", "data_time"]) and "eta" in runner.message_hub.runtime_info:
    ...
    def get_log_after_epoch(self, runner, batch_idx: int, mode: str, with_non_scalar: bool = False) -> tuple[dict, str]:
        ...
        Return:
            ...
            recorded by :obj:`runner.message_hub` and :obj:`runner.visualizer`.
        """
        assert mode in ["test", "val"], f"`_get_metric_log_str` only accept val or test mode, but got {mode}"
        dataloader_len = self._get_dataloader_size(runner, mode)
        ...
        if self.by_epoch:
            if mode == "val":
                cur_epoch = self._get_epoch(runner, mode)
                log_str = f"Epoch({mode}) [{cur_epoch}][{dataloader_len}/{dataloader_len}]  "
            else:
        ...
        # remove prefix
        custom_keys = [self._remove_prefix(cfg["data_src"], f"{mode}/") for cfg in custom_cfg_copy]
        # Count the averaged time and data_time by epoch
        if "time" not in custom_keys:
            custom_cfg_copy.append({"data_src": "time", "window_size": "epoch", "method_name": "mean"})
        if "data_time" not in custom_keys:
            custom_cfg_copy.append({"data_src": "data_time", "window_size": "epoch", "method_name": "mean"})
    ...
    def _collect_scalars(self, custom_cfg: list[dict], runner, mode: str, reserve_prefix: bool = False) -> dict:
    ...
    def _parse_windows_size(self, runner, batch_idx: int, custom_cfg: list | None = None) -> list:
        ...
        for log_cfg in custom_cfg:
            ...
            if window_size is None or isinstance(window_size, int):
                ...
                raise TypeError(f"window_size should be int, epoch or global, but got invalid {window_size}")
        return custom_cfg
