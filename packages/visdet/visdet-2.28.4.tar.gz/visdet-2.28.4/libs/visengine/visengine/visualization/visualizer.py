...
from visengine.registry import VISBACKENDS, VISUALIZERS
from visengine.structures import BaseDataElement
from visengine.utils import ManagerMixin, is_seq_of
from visengine.visualization.utils import (
    check_type,
...
from visengine.visualization.vis_backend import BaseVisBackend

VisBackendsType = Union[list[list | BaseDataElement], BaseDataElement, dict, None]
...
class Visualizer(ManagerMixin):
    ...
    def __init__(
        ...
        if not is_seq_of(vis_backends, (dict, BaseVisBackend)):
            raise TypeError("vis_backends must be a list of dicts or a list of BaseBackend instances")
    ...
    @master_only
    def dataset_meta(self) -> dict | None:
    ...
    def show(
        self,
        drawn_img: np.ndarray | None = None,
        win_name: str = "image",
        wait_time: float = 0.0,
        continue_key: str = " ",
        ...
        if backend == "matplotlib":
            ...
            cv2.imshow(str(id(self)), self.get_image() if drawn_img is None else drawn_img)
            cv2.waitKey(int(np.ceil(wait_time * 1000)))
        else:
            raise ValueError(f'backend should be "matplotlib" or "cv2", but got {backend} instead')
    ...
    def set_image(self, image: np.ndarray) -> None:
        ...
        # Handle both CHW (channels, height, width) and HWC (height, width, channels) formats
        if len(image.shape) == 3:
            if image.shape[0] <= 4:  # Assume CHW format if first dim is small (likely channels)
    ...
    def draw_points(
        ...
        sizes: np.ndarray | torch.Tensor | None = None,
    ):
        ...
        assert positions.shape[-1] == 2, f"The shape of `positions` should be (N, 2), but got {positions.shape}"
    ...
    def draw_texts(
        ...
        texts: str | list[str],
        positions: np.ndarray | torch.Tensor,
        font_sizes: int | list[int] | None = None,
        colors: str | tuple | list[str] | list[tuple] = "g",
        vertical_alignments: str | list[str] = "top",
        horizontal_alignments: str | list[str] = "left",
        font_families: str | list[str] = "sans-serif",
        bboxes: dict | list[dict] | None = None,
        font_properties: Union["FontProperties", list["FontProperties"]] | None = None,
    ) -> "Visualizer":
    ...
    def draw_lines(
        self,
        x_datas: np.ndarray | torch.Tensor,
        y_datas: np.ndarray | torch.Tensor,
        colors: str | tuple | list[str] | list[tuple] = "g",
        line_styles: str | list[str] = "-",
        line_widths: int | float | list[int | float] = 2,
    ) -> "Visualizer":
    ...
    def draw_circles(
        ...
        line_styles: str | list[str] = "-",
        line_widths: int | float | list[int | float] = 2,
        face_colors: str | tuple | list[str] | list[tuple] = "none",
        alpha: float | int = 0.8,
    ) -> "Visualizer":
    ...
    def draw_polygons(
        self,
        polygons: np.ndarray | torch.Tensor | list[np.ndarray | torch.Tensor],
        edge_colors: str | tuple | list[str] | list[tuple] = "g",
        line_styles: str | list[str] = "-",
        line_widths: int | float | list[int | float] = 2,
        face_colors: str | tuple | list[str] | list[tuple] = "none",
    ...
    def draw_binary_masks(
        ...
        logger.debug(f"draw_binary_masks - img.shape: {img.shape}")
        logger.debug(f"draw_binary_masks - binary_masks.shape: {binary_masks.shape}")
    ...
    def draw_featmap(
        ...
        overlaid_image: np.ndarray | None = None,
        channel_reduction: str | None = "squeeze_mean",
        topk: int = 20,
    ...
    @master_only
    def add_scalar(self, name: str, value: int | float, step: int = 0, **kwargs) -> None:
    ...
    @master_only
    def add_scalars(self, scalar_dict: dict, step: int = 0, file_path: str | None = None, **kwargs) -> None:
