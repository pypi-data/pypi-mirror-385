...
from visengine.fileio import dump
from visengine.hooks.logger_hook import SUFFIX_TYPE
from visengine.logging import MMLogger, print_log
from visengine.registry import VISBACKENDS
from visengine.utils import digit_version, scandir
...
class LocalVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import LocalVisBackend
        >>> import numpy as np
...
class WandbVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import WandbVisBackend
        >>> import numpy as np
    ...
    def __init__(
        ...
        define_metric_cfg: dict | list | None = None,
        commit: bool | None = True,
        log_code_name: str | None = None,
    ...
    def add_config(self, config: Config, **kwargs) -> None:
        ...
        self._wandb.config.update(config.to_dict(), allow_val_change=allow_val_change)
...
class TensorboardVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import TensorboardVisBackend
        >>> import numpy as np
    ...
    def add_scalar(
        ...
        **kwargs,
        ...
        if isinstance(value, int | float | torch.Tensor | np.ndarray | np.number):
            ...
            warnings.warn(
                f"Got {type(value)}, but numpy array, torch tensor, int or float are expected. skip it!",
                stacklevel=2,
            )
...
class MLflowVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import MLflowVisBackend
    ...
    def __init__(
        ...
        tracked_config_keys: dict | None = None,
    ...
    def _init_env(self):
        ...
        if self._mlflow.get_experiment_by_name(self._exp_name) is None:
            self._mlflow.create_experiment(self._exp_name, artifact_location=self._artifact_location)
...
class ClearMLVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import ClearMLVisBackend
        >>> from mmengine import Config
        >>> import numpy as np
...
class NeptuneVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import NeptuneVisBackend
        >>> from mmengine import Config
        >>> import numpy as np
    ...
    def __init__(self, save_dir: str | None = None, init_kwargs: dict | None = None):
        super().__init__(save_dir)  # type:ignore
        self._init_kwargs = init_kwargs
    ...
    def _init_env(self):
        ...
        try:
            ...
            raise ImportError('Please run "pip install -U neptune" to install neptune')
        if self._init_kwargs is None:
    ...
    def add_image(self, name: str, image: np.ndarray, step: int = 0, **kwargs) -> None:
        ...
        self._neptune["images"].append(File.as_image(img), name=name, step=step)
...
class DVCLiveVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import DVCLiveVisBackend
        >>> import numpy as np
    ...
    def _init_env(self):
        ...
        if digit_version(platform.python_version()) < digit_version("3.8"):
            raise RuntimeError("Please use Python 3.8 or higher version to use DVCLiveVisBackend.")
    ...
    def _to_dvc_paramlike(
        self,
        value: int | float | dict | list | tuple | Config | ConfigDict | torch.Tensor | np.ndarray,
    ):
...
class AimVisBackend(BaseVisBackend):
    ...
    Examples:
        >>> from visengine.visualization import AimVisBackend
        >>> import numpy as np
