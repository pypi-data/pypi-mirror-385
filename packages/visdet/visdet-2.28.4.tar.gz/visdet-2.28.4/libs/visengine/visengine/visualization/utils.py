...
def tensor2ndarray(value: np.ndarray | torch.Tensor) -> np.ndarray:
...
def value2list(value: Any, valid_type: type | tuple[type, ...], expand_dim: int) -> list[Any]:
...
def check_type(name: str, value: Any, valid_type: type | tuple[type, ...]) -> None:
    ...
    if not isinstance(value, valid_type):
        raise TypeError(f"`{name}` should be {valid_type}  but got {type(value)}")
...
def color_val_matplotlib(
    colors: str | tuple | list[str | tuple],
