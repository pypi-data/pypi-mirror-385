...
from visengine.utils import check_file_exist
...
class ImportTransformer(ast.NodeTransformer):
    ...
    Args:
        ...
        base_dict (dict): All variables defined in base files.
            ...
            Examples:
                >>> from visengine.config import read_base
                >>>
                >>>
                >>> with read_base():
    ...
    def visit_Import(self, node) -> Union[ast.Assign, ast.Import]:
        ...
        if alias.asname is not None:
            ...
            return ast.parse(  # type: ignore
                f'{alias.asname} = LazyObject("{alias.name}",location="{self.filename}, line {node.lineno}")'
            ).body[0]
        return node
