...
from visengine.fileio import dump, load
from visengine.logging import print_log
from visengine.utils import (
    check_file_exist,
    digit_version,
    get_installed_path,
...
def _lazy2string(cfg_dict, dict_type=None):
    if isinstance(cfg_dict, dict):
        ...
        return f"{cfg_dict.module}.{cfg_dict!s}"
    else:
        return cfg_dict
...
class ConfigDict(Dict):
    ...
    def __reduce_ex__(self, proto):
        ...
        if digit_version(platform.python_version()) < digit_version("3.8"):
            return (self.__class__, (dict(super().items()),), None, None, None)
        else:
            return (self.__class__, (dict(super().items()),), None, None, None, None)
...
class Config:
    ...
    def _substitute_env_variables(filename: str, temp_config_name: str):
        ...
        for var_name, value in keys:
            ...
            if var_name in os.environ:
                ...
                print_log(
                    f"Using env variable `{var_name}` with value of {value} to replace item in config.",
                    logger="current",
                )
            if not value:
                raise KeyError(
    ...
    def _file2dict(
        ...
        """
        # Auto-detect lazy_import if not specified
        if lazy_import is None:
            lazy_import = Config._is_lazy_import(filename)

        # If this is a lazy import file, use lazy import parsing
        if lazy_import:
            cfg_dict, imported_names = Config._parse_lazy_import(filename)
            cfg_dict = Config._dict_to_config_dict_lazy(cfg_dict)
            return cfg_dict, ""
    ...
    def _parse_lazy_import(filename: str) -> tuple[ConfigDict, set]:
        ...
        with open(filename, encoding="utf-8") as f:
            ...
            for base_module in base_modules:
                ...
                _base_cfg_dict, _base_imported_names = Config._parse_lazy_import(module_path)
    ...
    @staticmethod
    def _dict_to_config_dict(cfg: dict, scope: str | None = None, has_scope=True):
    ...
    def _get_cfg_path(cfg_path: str, filename: str) -> tuple[str, str | None]:
        ...
        if "::" in cfg_path:
            ...
            if not is_installed(package):
                raise ModuleNotFoundError(f"{package} is not installed, please install {package} manually")
        ...
        for k, v in a.items():
            if allow_list_keys and k.isdigit() and isinstance(b, list):
                ...
                if k in b and not v.pop(DELETE_KEY, False):
                    allowed_types: tuple | type = (dict, list) if allow_list_keys else dict
    ...
    def pretty_text(self) -> str:
        ...
        if self._format_python_code:
            ...
            try:
                ...
                raise SyntaxError(f"Failed to format the config file, please check the syntax of: \n{text}")
        return text
    ...
    def __getstate__(self) -> tuple[dict, str | None, str | None, dict, bool, set]:
        state = (
            self._cfg_dict,
            self._filename,
            self._text,
    ...
    def __setstate__(self, state: tuple[dict, str | None, str | None, dict, bool, set]):
    ...
    def dump(self, file: str | Path | None = None):
