    ...
    def build(self) -> Any:
        ...
        if isinstance(self._module, str):
            try:
                ...
                raise type(e)(f"Failed to import {self._module} in {self.location} for {e}")

            if self._imported is not None:
                if hasattr(module, self._imported):
                    module = getattr(module, self._imported)
                else:
                    raise ImportError(f"Failed to import {self._imported} from {self._module} in {self.location}")
            ...
            try:
                ...
                raise type(e)(f"Failed to import {self.module} in {self.location} for {e}")
...
class LazyAttr:
    ...
    Examples:
        ...
        >>> print(type(model['type']))  # <class 'visengine.config.lazy.LazyAttr'>
    ...
    def build(self) -> Any:
        ...
        try:
            ...
            raise ImportError(f"Failed to import {self.module}.{self.name} in {self.location}")
