...
def traverse_registry_tree(registry: Registry, verbose: bool = True) -> list:
    ...
    def _dfs_registry(_registry):
        if isinstance(_registry, Registry):
            ...
            registry_info = {"num_modules": num_modules, "scope": scope}
            ...
            if verbose:
                print_log(
                    f"Find {num_modules} modules in {scope}'s '{_registry.name}' registry ",
                    logger="current",
                )
...
def count_registered_modules(save_path: str | None = None, verbose: bool = True) -> dict:
    ...
    for item in dir(root):
        if not item.startswith("__"):
            ...
            if isinstance(registry, Registry):
                registries_info[item] = traverse_registry_tree(registry, verbose)
    scan_data = {
        "scan_date": datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "registries": registries_info,
    }
    if verbose:
        print_log(f"Finish registry analysis, got: {scan_data}", logger="current")
