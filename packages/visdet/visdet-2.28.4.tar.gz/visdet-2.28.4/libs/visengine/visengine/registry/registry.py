...
from visengine.utils import get_object_from_string, is_seq_of
...
class Registry:
    ...
    def switch_scope_and_registry(self, scope: str | None) -> Generator:
        ...
        Examples:
            >>> from visengine.registry import Registry, DefaultScope, MODELS
            >>> import time
            >>> # External Registry
            ...
            if default_scope is not None:
                ...
                try:
                    ...
                    if scope in MODULE2PACKAGE:
                        ...
                        print_log(
                            f"Failed to import `{scope}.registry` make sure the registry.py exists in `{scope}` package.",
                            logger="current",
                            level=logging.WARNING,
                        )
    ...
    def import_from_location(self) -> None:
        ...
        if not self._imported:
            ...
            if len(self._locations) == 0 and self.scope in MODULE2PACKAGE:
                ...
                try:
                    ...
                    if self.scope in MODULE2PACKAGE:
                        ...
                        print_log(
                            f"Failed to import {self.scope} and register its modules, please make sure you have registered the module manually.",
                            logger="current",
                            level=logging.WARNING,
                        )
                else:
            ...
            for loc in self._locations:
                import_module(loc)
                print_log(
                    f"Modules of {self.scope}'s {self.name} registry have been automatically imported from {loc}",
                    logger="current",
                    level=logging.DEBUG,
                )
            self._imported = True
    ...
    def get(self, key: str) -> type | None:
        ...
        Examples:
            ...
            >>> @CLASSIFIER.register_module(force=True)
            >>> class MobileNet:
            >>>     pass
        ...
        if obj_cls is None:
            ...
            # `dict(type='visengine.model.BaseModel')`, which can
    ...
    def register_module(
        ...
        # use it as a decorator: @x.register_module(force=True)
