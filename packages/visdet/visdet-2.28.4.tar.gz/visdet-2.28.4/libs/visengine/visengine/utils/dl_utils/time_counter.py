...
from visengine.dist.utils import master_only
...
class TimeCounter:
    ...
    Examples:
        >>> import time
        >>> from visengine.utils.dl_utils import TimeCounter
        >>> @TimeCounter()
        ... def fun1():
        ...     time.sleep(0.1)
        ... fun1()
    ...
    def __call__(self, fn):
        ...
        def wrapper(*args, **kwargs):
            ...
            if self.with_sync and is_cuda_available():
    ...
    def print_time(self, elapsed: int | float) -> None:
        """Print times per count."""
        if self.__count >= self.warmup_interval:
            self.__pure_inf_time += elapsed

            if self.__count % self.log_interval == 0:
                times_per_count = 1000 * self.__pure_inf_time / (self.__count - self.warmup_interval + 1)
                print_log(
                    f"[{self.tag}]-time per run averaged in the past {self.__count} runs: {times_per_count:.1f} ms",
                    self.logger,
                )
