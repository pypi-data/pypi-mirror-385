...
class ProgressBar:
    ...
    def update(self, num_tasks: int = 1):
        ...
        if self.task_num > 0:
            ...
            msg = f"\r[{{}}] {self.completed}/{self.task_num}, {fps:.1f} task/s, elapsed: {int(elapsed + 0.5)}s, ETA: {eta:5}s"
...
def track_progress(func: Callable, tasks: Sequence, bar_width: int = 50, file=sys.stdout, **kwargs):
    ...
    if isinstance(tasks, tuple):
        ...
        raise TypeError(f'"tasks" must be a tuple object or a sequence object, but got {type(tasks)}')
