...
def apply_to(data: Any, expr: Callable, apply_func: Callable):
    ...
    Examples:
        >>> from visengine.utils import apply_to
        >>> import numpy as np
        >>> import torch
    ...
    if isinstance(data, dict):
        ...
        # namedtuple
        return type(data)(*(apply_to(sample, expr, apply_func) for sample in data))  # type: ignore  # yapf:disable
...
def deprecated_api_warning(name_dict: dict, cls_name: str | None = None) -> Callable:
...
def is_method_overridden(method: str, base_class: type, derived_class: type | Any) -> bool:
