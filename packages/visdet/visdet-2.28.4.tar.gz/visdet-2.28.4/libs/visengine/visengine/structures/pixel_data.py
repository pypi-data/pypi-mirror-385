...
class PixelData(BaseDataElement):
    ...
    def __setattr__(self, name: str, value: torch.Tensor | np.ndarray):
        ...
        if name in ("_metainfo_fields", "_data_fields"):
            ...
            if self.shape:
                assert tuple(value.shape[-2:]) == self.shape, (
                    f"The height and width of values {tuple(value.shape[-2:])} is not consistent with the shape of this :obj:`PixelData` {self.shape}"
                )
            assert value.ndim in [2, 3], f"The dim of value must be 2 or 3, but got {value.ndim}"
            if value.ndim == 2:
                value = value[None]
                warnings.warn(
                    f"The shape of value will convert from {value.shape[-2:]} to {value.shape}",
                    stacklevel=2,
                )
            super().__setattr__(name, value)

    # TODO torch.Long/bool
    def __getitem__(self, item: Sequence[int | slice]) -> "PixelData":
        """
        Args:
