...
from visengine.registry import FUNCTIONS
...
def pseudo_collate(data_batch: Sequence) -> Any:
    ...
    if isinstance(data_item, str | bytes):
        ...
        # named tuple
        return data_item_type(*(pseudo_collate(samples) for samples in zip(*data_batch, strict=False)))
        ...
        transposed = list(zip(*data_batch, strict=False))
...
def default_collate(data_batch: Sequence) -> Any:
    ...
    if isinstance(data_item, BaseDataElement | str | bytes):
        ...
        # named_tuple
        return data_item_type(*(default_collate(samples) for samples in zip(*data_batch, strict=False)))
    elif isinstance(data_item, Sequence):
        # check to make sure that the data_itements in batch have
        # consistent size
        it = iter(data_batch)
