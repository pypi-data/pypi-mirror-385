...
from visengine.fileio import join_path, list_from_file, load
from visengine.logging import print_log
from visengine.registry import TRANSFORMS
...
class Compose:
    ...
    def __init__(self, transforms: Sequence[dict | Callable] | None):
...
class BaseDataset(Dataset):
    ...
    def __init__(
        self,
        ann_file: str | None = "",
        metainfo: Mapping | Config | None = None,
        data_root: str | None = "",
    ...
    def parse_data_info(self, raw_data_info: dict) -> dict | list[dict]:
        ...
        for prefix_key, prefix in self.data_prefix.items():
            assert prefix_key in raw_data_info, (
                f"raw_data_info: {raw_data_info} dose not contain prefix key{prefix_key}, please check your data_prefix."
            )
    ...
    @classmethod
    def _load_metainfo(cls, metainfo: Mapping | Config | None = None) -> dict:
    ...
    @force_full_init
    def get_subset_(self, indices: Sequence[int] | int) -> None:
    ...
    @force_full_init
    def get_subset(self, indices: Sequence[int] | int) -> "BaseDataset":
    ...
    def _get_serialized_subset(self, indices: Sequence[int] | int) -> tuple[np.ndarray, np.ndarray]:
        ...
        Returns:
            ...
            information.
        """
        sub_data_bytes: list | np.ndarray
        sub_data_address: list | np.ndarray
    ...
    def _get_unserialized_subset(self, indices: Sequence[int] | int) -> list:
