...
from visengine.registry import DATASETS
...
class ConcatDataset(_ConcatDataset):
    ...
    def __init__(
        self,
        datasets: Sequence[BaseDataset | dict],
        ...
        if ignore_keys is None:
            ...
            raise TypeError(f"ignore_keys should be a list or str, but got {type(ignore_keys)}")
        ...
        for i, dataset in enumerate(self.datasets, 1):
            for key in meta_keys:
                ...
                if key not in dataset.metainfo:
                    raise ValueError(f"{key} does not in the meta information of the {i}-th dataset")
    ...
    def _get_ori_dataset_idx(self, idx: int) -> tuple[int, int]:
        ...
        if idx < 0:
            if -idx > len(self):
                raise ValueError(f"absolute value of index({idx}) should not exceed datasetlength({len(self)}).")
            idx = len(self) + idx
    ...
    def get_subset_(self, indices: list[int] | int) -> None:
...
class RepeatDataset:
    ...
    def __init__(self, dataset: BaseDataset | dict, times: int, lazy_init: bool = False):
