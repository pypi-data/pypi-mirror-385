...
class DefaultOptimWrapperConstructor:
    ...
    def __init__(self, optim_wrapper_cfg: dict, paramwise_cfg: dict | None = None):
        ...
        assert "optimizer" in optim_wrapper_cfg, '`optim_wrapper_cfg` must contain "optimizer" config'
    ...
    def add_params(
        ...
        module: nn.Module,
        prefix: str = "",
        is_dcn_module: int | float | None = None,
    ) -> None:
        ...
        is_norm = isinstance(module, GroupNorm | LayerNorm | _BatchNorm | _InstanceNorm)
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            if bypass_duplicate and self._is_in(param_group, params):
                print_log(
                    f"{prefix} is duplicate. It is skipped since bypass_duplicate={bypass_duplicate}",
                    logger="current",
            ...
            if not param.requires_grad:
                print_log(
                    (f"{prefix}.{name} is skipped since its requires_grad={param.requires_grad}"),
                    logger="current",
                    level=logging.WARNING,
                )
                continue
            ...
            if not is_custom:
                ...
                if self.base_wd is not None:
                    ...
                    # bias lr and decay
                    elif name == "bias" and not is_dcn_module and bias_decay_mult is not None:
                    ...
                    elif param.ndim == 1 and not is_dcn_module and flat_decay_mult is not None:
    ...
    def __call__(self, model: nn.Module):  # -> OptimWrapper:
        ...
        if not self.paramwise_cfg:
            ...
            optimizer = OPTIMIZERS.build(optimizer_cfg)
        optim_wrapper = OPTIM_WRAPPERS.build(optim_wrapper_cfg, default_args={"optimizer": optimizer})
