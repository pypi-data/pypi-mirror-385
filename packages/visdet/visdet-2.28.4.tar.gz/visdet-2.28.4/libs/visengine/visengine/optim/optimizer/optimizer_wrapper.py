...
from visengine.logging import MessageHub, print_log, MMLogger
from visengine.registry import OPTIM_WRAPPERS
from visengine.utils.dl_utils import has_batch_norm
...
class OptimWrapper(BaseOptimWrapper):
    ...
    Examples:
        ...
        >>> from visengine.optim import OptimWrapper
        >>>
        >>> model = nn.Linear(1, 1)
    ...
    def __init__(
        ...
        if clip_grad is not None:
            ...
            assert isinstance(clip_grad, dict) and clip_grad, (
                "If `clip_grad` is not None, it should be a `dict` which is the arguments of `torch.nn.utils.clip_grad_norm_` or clip_grad_value_`."
            )
            ...
            if clip_type == "norm":
                ...
                raise ValueError(f'type of clip_grad should be "norm" or "value" but got {clip_type}')
            assert clip_grad, (
        ...
        if len(optimizer.param_groups) > 1:
            self.base_param_settings = {"params": torch.tensor([0.0], dtype=torch.float)}
    ...
    def update_params(  # type: ignore
        ...
        # Debug log: Check loss before scaling
        logger.debug(f"[OptimWrapper] update_params called with loss: {loss.item()}")
        if torch.isnan(loss):
            logger.error(f"[OptimWrapper] NaN loss detected before scaling! Loss: {loss}")
        if torch.isinf(loss):
            logger.error(f"[OptimWrapper] Inf loss detected before scaling! Loss: {loss}")
        ...
        # Debug log: Check loss after scaling
        logger.debug(f"[OptimWrapper] Loss after scaling: {loss.item()}")
        if torch.isnan(loss):
            logger.error(f"[OptimWrapper] NaN loss detected after scaling! Scaled loss: {loss}")
        if torch.isinf(loss):
            logger.error(f"[OptimWrapper] Inf loss detected after scaling! Scaled loss: {loss}")
        ...
        if self.should_update():
            logger.debug(f"[OptimWrapper] Updating parameters at inner_count: {self._inner_count}")
    ...
    def backward(self, loss: torch.Tensor, **kwargs) -> None:
        ...
        """
        logger = MMLogger.get_current_instance()
        logger.debug(f"[OptimWrapper] Starting backward pass with loss: {loss.item()}")

        # Check for NaN/Inf before backward
        if torch.isnan(loss):
            logger.error(f"[OptimWrapper] NaN loss detected before backward! Cannot compute gradients.")
        if torch.isinf(loss):
            logger.error(f"[OptimWrapper] Inf loss detected before backward! Cannot compute gradients.")

        loss.backward(**kwargs)

        # Check gradients after backward
        logger.debug(f"[OptimWrapper] Backward pass completed, checking gradients...")
    ...
    def step(self, **kwargs) -> None:
        ...
        """
        logger = MMLogger.get_current_instance()
        logger.debug(f"[OptimWrapper] Starting optimizer step...")
        ...
        if self.clip_grad_kwargs:
            ...
            self._check_gradients("after clipping")
        ...
        self._check_parameters("after step")

        logger.debug(f"[OptimWrapper] Optimizer step completed")
    ...
    def initialize_count_status(self, model: nn.Module, init_counts: int, max_counts: int) -> None:
        ...
        if has_batch_norm(model) and self._accumulative_counts > 1:
            print_log(
                "Gradient accumulative may slightly decrease performance because the model has BatchNorm layers.",
                logger="current",
                level=logging.WARNING,
            )
    ...
    def _check_gradients(self, stage: str = "") -> None:
        """Check gradients for NaN/Inf values and log statistics."""
        logger = MMLogger.get_current_instance()
            ...
            for j, param in enumerate(param_group["params"]):
                if param.grad is not None:
                    ...
                    if torch.isnan(param.grad).any():
                        nan_grad_params.append(f"group_{i}_param_{j}")
                        logger.error(f"[OptimWrapper] NaN gradient detected {stage} in param group {i}, param {j}")
                    elif torch.isinf(param.grad).any():
                        inf_grad_params.append(f"group_{i}_param_{j}")
                        logger.error(f"[OptimWrapper] Inf gradient detected {stage} in param group {i}, param {j}")
                    elif grad_norm == 0:
                        zero_grad_params.append(f"group_{i}_param_{j}")
                    else:
        ...
        if nan_grad_params or inf_grad_params:
            logger.error(
                f"[OptimWrapper] Gradient check {stage}: "
                f"NaN params: {len(nan_grad_params)}, "
                f"Inf params: {len(inf_grad_params)}, "
                f"Zero grad params: {len(zero_grad_params)}, "
                f"Normal params: {normal_grad_params}"
            )
        else:
            logger.debug(
                f"[OptimWrapper] Gradient check {stage}: "
                f"Total grad norm: {total_grad_norm:.6f}, "
                f"Zero grad params: {len(zero_grad_params)}, "
                f"Normal params: {normal_grad_params}"
            )

    def _check_parameters(self, stage: str = "") -> None:
        """Check parameters for NaN/Inf values and log statistics."""
        logger = MMLogger.get_current_instance()
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            for j, param in enumerate(param_group["params"]):
                if torch.isnan(param).any():
                    nan_params.append(f"group_{i}_param_{j}")
                    logger.error(f"[OptimWrapper] NaN parameter detected {stage} in param group {i}, param {j}")
                elif torch.isinf(param).any():
                    inf_params.append(f"group_{i}_param_{j}")
                    logger.error(f"[OptimWrapper] Inf parameter detected {stage} in param group {i}, param {j}")
                else:
                    normal_params += 1

        if nan_params or inf_params:
            logger.error(
                f"[OptimWrapper] Parameter check {stage}: "
                f"NaN params: {len(nan_params)}, "
                f"Inf params: {len(inf_params)}, "
                f"Normal params: {normal_params}"
            )
        else:
            logger.debug(f"[OptimWrapper] Parameter check {stage}: All {normal_params} parameters are normal")
