...
class OptimWrapperDict(OptimWrapper):
    ...
    Examples:
        ...
        >>> from visengine.optim import OptimWrapperDict, OptimWrapper
    ...
    def update_params(  # type: ignore
        ...
        """
        raise NotImplementedError("`update_params` should be called by each optimizer separately`")
    ...
    def backward(self, loss: torch.Tensor, **kwargs) -> None:
        ...
        its `backward`.
        """
        raise NotImplementedError("`backward` should be called by each optimizer separately`")

    def step(self, **kwargs) -> None:
        """Since the backward method is not implemented, the step should not be
        implemented either."""
        raise NotImplementedError("`step` should be called by each optimizer separately`")
    ...
    def optim_context(self, model: nn.Module):
        ...
        raise NotImplementedError("`optim_context` should be called by each optimizer separately")

    def initialize_count_status(self, model: nn.Module, cur_iter, max_iters) -> None:
    ...
    def get_momentum(self) -> dict[str, list[float]]:
        ...
        momentum_dict = {}
        for name, optim_wrapper in self.optim_wrappers.items():
            momentum_dict[f"{name}.momentum"] = optim_wrapper.get_momentum()["momentum"]
    ...
    def load_state_dict(self, state_dict: dict) -> None:
        ...
        for name, _state_dict in state_dict.items():
            assert name in self.optim_wrappers, f"Mismatched `state_dict`! cannot found {name} in OptimWrapperDict"
