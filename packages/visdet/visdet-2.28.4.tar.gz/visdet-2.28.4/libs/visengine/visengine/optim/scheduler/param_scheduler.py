...
from visengine.registry import PARAM_SCHEDULERS
...
class _ParamScheduler:
    ...
    def __init__(
        ...
        if not isinstance(optimizer, Optimizer | BaseOptimWrapper):
            raise TypeError(f"``optimizer`` should be an Optimizer,but got {type(optimizer).__name__}")
        self.optimizer = optimizer
        self.param_name = param_name

        if end <= begin:
            raise ValueError(f"end should be larger than begin, but got begin={begin}, end={end}")
        self.begin = begin
        self.end = end
...
class StepParamScheduler(_ParamScheduler):
    ...
    def _get_value(self):
        ...
        return [group[self.param_name] * self.gamma for group in self.optimizer.param_groups]
...
class CosineAnnealingParamScheduler(_ParamScheduler):
    ...
    def __init__(
        self,
        optimizer: Optimizer | BaseOptimWrapper,
        param_name: str,
        T_max: int | None = None,
        ...
        by_epoch: bool = True,
        verbose: bool = False,
        eta_min_ratio: float | None = None,
    ):
        # To preserve backwards compatibility
        if eta_min is None and eta_min_ratio is None:
            eta_min = 0.0
        assert (eta_min is None) ^ (eta_min_ratio is None), "Either `eta_min` or `eta_min_ratio should be specified"
...
class OneCycleParamScheduler(_ParamScheduler):
    ...
    def __init__(
        ...
        assert param_name == "lr", f"OneCycle only works for learning rate updating, but got patam_name as {param_name}"
        ...
        if last_step == -1:
            for idx, group in enumerate(optimizer.param_groups):
                ...
                group[f"min_{param_name}"] = group[f"initial_{param_name}"] / final_div_factor
...
class CosineRestartParamScheduler(_ParamScheduler):
    ...
    def build_iter_from_epoch(
        ...
        **kwargs,
        ...
        return cls(*args, periods=periods, begin=begin, end=end, by_epoch=by_epoch, **kwargs)
    ...
    @staticmethod
    def get_position_from_periods(iteration: int, cumulative_periods: list[int]) -> int | None:
...
class ReduceOnPlateauParamScheduler(_ParamScheduler):
    ...
    def __init__(
        ...
        if isinstance(min_value, list | tuple):
            if len(min_value) != len(raw_optimizer.param_groups):
                raise ValueError(f"expected {len(raw_optimizer.param_groups)} min_lrs, got {len(min_value)}")
            ...
            if isinstance(optimizer, BaseOptimWrapper) and optimizer.base_param_settings is not None:  # type: ignore
                self.min_values.append(self.min_values[-1])

        else:
            self.min_values = [min_value] * len(optimizer.param_groups)  # type: ignore
        ...
        self._init_is_better(rule=rule, threshold=threshold, threshold_rule=threshold_rule)
        self._reset()
    ...
    def step(self, metrics=None):
        ...
        if not isinstance(metrics, dict):
            raise TypeError(f"metrics type should be dict, but got type {type(metrics)}")
        ...
        if self.begin <= self._global_step < self.end:
            ...
            if metric is not None:
                ...
                raise KeyError(f"Excepted key in {list(metrics.keys())}, but got key {self.monitor} is not in dict")
    ...
    def print_value(self, is_verbose: bool, group: int, value: float) -> None:
        ...
        if is_verbose:
            ...
            print_log(
                f"Adjusting parameter value of group {group} to {value:.4e} in {step_name} {self.last_step}.",
                logger="current",
            )

    def _get_value(self):
        """Compute value using chainable form of the scheduler."""
        values = [float(group[self.param_name]) * self.factor for group in self.optimizer.param_groups]
        return [max(v, min_v) for v, min_v in zip(values, self.min_values, strict=False)]
