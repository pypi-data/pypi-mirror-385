...
class ProfilerHook(Hook):
    ...
    def __init__(
        ...
        *,
        ...
        on_trace_ready: Callable | dict | None = None,
        ...
        if not check_kineto():
            raise ImportError(
                "Due to Kineto support issues, please upgrade pytorch above 1.8.1(windows users above 1.9.1)"
            )
        ...
        if profile_times < 1:
            raise ValueError(f"profile_iters should be greater than 0, but got {profile_times}")
        ...
        assert isinstance(activity_with_cpu, bool), "``activity_with_cpu`` should be a boolean."
        assert isinstance(activity_with_cuda, bool), "``activity_with_cuda`` should be a boolean."
        self.activities = []
    ...
    def before_run(self, runner):
        ...
        if max_times < self.profile_times:
            raise ValueError(f"``profile_times`` should not be greater than {max_times}")
        ...
        if self.on_trace_ready is None:
            ...
            if trace_type == "log_trace":
                ...
                if "dir_name" not in trace_cfg:
                    ...
                    trace_cfg["dir_name"] = osp.join(runner.log_dir, trace_cfg["dir_name"])
                runner.logger.info(f"trace_files of ProfilerHook will be saved to {trace_cfg['dir_name']}.")
            ...
            raise ValueError(f"``on_trace_ready`` should be a handler, or dict, or None, but got {self.on_trace_ready}")
...
class NPUProfilerHook(Hook):
    ...
    def __init__(
        ...
        *,
        ...
        if begin >= end:
            raise ValueError("The iteration to start profiling should not be greaterthan or equal to profile end")
        ...
        self.profiler = torch_npu.npu.profile(self.result_path, use_e2e_profiler=use_e2e_profiler)

    @master_only
    def before_run(self, runner):
        if self.end > runner.max_iters:
            raise ValueError("The profiling end iteration should not be greaterthan the max iteration")
    ...
    def after_train_iter(self, runner, batch_idx, data_batch=None, outputs=None):
        if runner.iter == self.end - 1:
            runner.logger.info("profiler may take a few minutes to save the profiling result.")
            self.profiler.__exit__(None, None, None)
