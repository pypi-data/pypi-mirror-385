...
from visengine.registry import HOOKS, MODELS
...
class EMAHook(Hook):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert begin_iter >= 0, f"`begin_iter` must larger than or equal to 0, but got begin_iter: {begin_iter}"
        assert begin_epoch >= 0, f"`begin_epoch` must larger than or equal to 0, but got begin_epoch: {begin_epoch}"
    ...
    def before_run(self, runner) -> None:
        ...
        self.ema_model = MODELS.build(self.ema_cfg, default_args={"model": self.src_model})
    ...
    def before_train(self, runner) -> None:
        ...
        if self.enabled_by_epoch:
            assert self.begin_epoch <= runner.max_epochs, (
                f"self.begin_epoch should be smaller than or equal to runner.max_epochs: {runner.max_epochs}, but got begin_epoch: {self.begin_epoch}"
            )
        else:
    ...
    def after_test_epoch(self, runner, metrics: dict[str, float] | None = None) -> None:
    ...
    def after_load_checkpoint(self, runner, checkpoint: dict) -> None:
        ...
        Args:
            runner (Runner): The runner of the testing process.
        """
        from visengine.runner.checkpoint import load_state_dict
        ...
        # Support load checkpoint without ema state dict.
            ...
            if runner._resume:
                print_log(
                    "There is no `ema_state_dict` in checkpoint. `EMAHook` will make a copy of `state_dict` as the initial `ema_state_dict`",
                    "current",
                    logging.WARNING,
                )
            load_state_dict(
    ...
    def _swap_ema_parameters(self) -> None:
        ...
        avg_param = (
            itertools.chain(self.ema_model.module.parameters(), self.ema_model.module.buffers())
