...
from visengine.dist import is_main_process, master_only
from visengine.fileio import FileClient, get_file_backend
from visengine.logging import print_log
from visengine.registry import HOOKS
from visengine.utils import is_list_of, is_seq_of
...
class CheckpointHook(Hook):
    ...
    def __init__(
        ...
        greater_keys: Sequence[str] | None = None,
        less_keys: Sequence[str] | None = None,
        file_client_args: dict | None = None,
        filename_tmpl: str | None = None,
        backend_args: dict | None = None,
        published_keys: str | list[str] | None = None,
        save_begin: int = 0,
        **kwargs,
    ) -> None:
        ...
        # save best logic
        assert isinstance(save_best, str) or is_list_of(save_best, str) or (save_best is None), (
            f'"save_best" should be a str or list of str or None, but got {type(save_best)}'
        )

        if isinstance(save_best, list):
            if "auto" in save_best:
                assert len(save_best) == 1, 'Only support one "auto" in "save_best" list.'
            assert len(save_best) == len(set(save_best)), 'Find duplicate element in "save_best".'
        else:
            # convert str to list[str]
            if save_best is not None:
                save_best = [save_best]  # type: ignore
        self.save_best = save_best

        # rule logic
        assert isinstance(rule, str) or is_list_of(rule, str) or (rule is None), (
            f'"rule" should be a str or list of str or None, but got {type(rule)}'
        )
        if isinstance(rule, list):
            # check the length of rule list
            assert len(rule) in [1, len(self.save_best)], (  # type: ignore
                f'Number of "rule" must be 1 or the same as number of "save_best", but got {len(rule)}.'
            )
        else:
        ...
        if isinstance(published_keys, str):
            ...
            assert len(published_keys) == len(set(published_keys)), 'Find duplicate elements in "published_keys".'
        self.published_keys = published_keys

        self.last_ckpt = None
        if save_begin < 0:
            raise ValueError("save_begin should not be less than 0, but got {save_begin}")
    ...
    def before_train(self, runner) -> None:
        ...
        if self.out_dir != runner.work_dir:
            ...
            self.out_dir = self.file_backend.join_path(self.out_dir, basename)  # type: ignore
        ...
        if self.max_keep_ckpts > 0:
            ...
            if "keep_ckpt_ids" in runner.message_hub.runtime_info:
                ...
                while len(keep_ckpt_ids) > self.max_keep_ckpts:
                    ...
                    if is_main_process():
                        path = self.file_backend.join_path(self.out_dir, self.filename_tmpl.format(step))
    ...
    def _publish_model(self, runner, ckpt_path: str) -> None:
        ...
        """
        from visengine.runner import save_checkpoint
        from visengine.runner.checkpoint import _load_checkpoint
        ...
        if self.max_keep_ckpts > 0:
            ...
            if len(self.keep_ckpt_ids) > 0 and self.keep_ckpt_ids[-1] == step:
                ...
                if len(self.keep_ckpt_ids) == self.max_keep_ckpts:
                    ...
                    if is_main_process():
                        ckpt_path = self.file_backend.join_path(self.out_dir, self.filename_tmpl.format(_step))
    ...
    def _save_best_checkpoint(self, runner, metrics) -> None:
        ...
        if "auto" in self.key_indicators:
            self._init_rule(self.rules, [next(iter(metrics.keys()))])

        best_ckpt_updated = False
        # save best logic
        # get score from messagehub
        for key_indicator, rule in zip(self.key_indicators, self.rules, strict=False):
            ...
            if key_score is None or not self.is_better_than[key_indicator](key_score, best_score):
                continue
            ...
            if best_ckpt_path and is_main_process():
                ...
                if is_removed:
                    runner.logger.info(f"The previous best checkpoint {best_ckpt_path} is removed")
            ...
            if len(self.key_indicators) == 1:
                self.best_ckpt_path = self.file_backend.join_path(  # type: ignore
                    self.out_dir, best_ckpt_name
                )
                runner.message_hub.update_info(runtime_best_ckpt_key, self.best_ckpt_path)
            else:
                self.best_ckpt_path_dict[key_indicator] = self.file_backend.join_path(  # type: ignore
                    self.out_dir, best_ckpt_name
                )
                runner.message_hub.update_info(runtime_best_ckpt_key, self.best_ckpt_path_dict[key_indicator])
            runner.save_checkpoint(
                self.out_dir,
    ...
    def _init_rule(self, rules, key_indicators) -> None:
        ...
        self.rules = []
        for rule, key_indicator in zip(rules, key_indicators, strict=False):
