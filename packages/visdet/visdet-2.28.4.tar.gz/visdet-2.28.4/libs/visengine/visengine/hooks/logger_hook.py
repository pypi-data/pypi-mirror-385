...
from visengine.fileio import FileClient, dump
from visengine.fileio.io import get_file_backend
from visengine.hooks import Hook
from visengine.logging import print_log
from visengine.registry import HOOKS
from visengine.utils import is_seq_of, scandir
...
class LoggerHook(Hook):
    ...
    def __init__(
        ...
        if not (isinstance(out_suffix, str) or is_seq_of(out_suffix, str)):
            raise TypeError(f"out_suffix should be a string or a sequence of string, but got {type(out_suffix)}")
        ...
        self.json_log_path: str | None = None
    ...
    def before_run(self, runner) -> None:
        ...
        if self.out_dir is not None:
            ...
            runner.logger.info(f"Text logs will be saved to {self.out_dir} after the training process.")
    ...
    def after_run(self, runner) -> None:
        ...
        for filename in scandir(runner._log_dir, self.out_suffix, True):
            ...
            if not self.keep_local:
                runner.logger.info(
                    f"{local_filepath} was removed due to the `self.keep_local=False`. You can check the running logs in {out_filepath}"
                )
