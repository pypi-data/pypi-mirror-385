...
from visengine.utils.dl_utils import tensor2imgs
...
class NaiveVisualizationHook(Hook):
    ...
    def after_test_iter(
        ...
        if self.every_n_inner_iters(batch_idx, self._interval):
            for data, output in zip(data_batch, outputs, strict=False):  # type: ignore
                input = data["inputs"]
                data_sample = data["data_sample"]
                input = tensor2imgs(input, **data_sample.get("img_norm_cfg", {}))[0]
