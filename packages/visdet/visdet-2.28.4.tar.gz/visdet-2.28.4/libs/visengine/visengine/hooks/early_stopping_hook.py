...
class EarlyStoppingHook(Hook):
    ...
    def _check_stop_condition(self, current_score: float) -> tuple[bool, str]:
        ...
        if self.check_finite and not isfinite(current_score):
            stop_training = True
            reason_message = f"Monitored metric {self.monitor} = {current_score} is infinite. Previous best value was {self.best_score:.3f}."

        elif self.stopping_threshold is not None and compare(current_score, self.stopping_threshold):
            stop_training = True
            self.best_score = current_score
            reason_message = f"Stopping threshold reached: `{self.monitor}` = {current_score} is {self.rule} than {self.stopping_threshold}."
        elif compare(self.best_score + self.min_delta, current_score):
            self.wait_count += 1

            if self.wait_count >= self.patience:
                reason_message = f"the monitored metric did not improve in the last {self.wait_count} records. best score: {self.best_score:.3f}. "
                stop_training = True
        else:
            self.best_score = current_score
            self.wait_count = 0
    ...
    def before_run(self, runner) -> None:
        ...
        assert hasattr(runner.train_loop, "stop_training"), "`train_loop` should contain `stop_training` variable."
