...
# Use torch's SyncBatchNorm directly
...
def stack_batch(
    ...
    assert len({tensor.ndim for tensor in tensor_list}) == 1, (
        f"Expected the dimensions of all tensors must be the same, but got {[tensor.ndim for tensor in tensor_list]}"
    )
...
def detect_anomalous_params(loss: torch.Tensor, model) -> None:
    ...
    for n, p in model.named_parameters():
        if p not in parameters_in_graph and p.requires_grad:
            print_log(
                f"{n} with shape {p.size()} is not in the computational graph \n",
                logger="current",
                level=logging.ERROR,
            )
...
try:
    ...
    warnings.warn(
        "Cannot import torch.fx, `merge_dict` is a simple function to merge multiple dicts",
        stacklevel=2,
    )
