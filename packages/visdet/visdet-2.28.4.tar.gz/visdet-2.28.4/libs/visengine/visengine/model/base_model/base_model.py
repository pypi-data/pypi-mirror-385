...
class BaseModel(BaseModule):
    ...
    def __init__(
        ...
        if data_preprocessor is None:
            data_preprocessor = {"type": "BaseDataPreprocessor"}
    ...
    def train_step(self, data: dict | tuple | list, optim_wrapper) -> dict[str, torch.Tensor]:
        ...
        logger.debug("[BaseModel] Starting train_step")
        ...
        with optim_wrapper.optim_context(self):
            ...
            # Debug log: Check all loss components
            logger.debug(f"[BaseModel] Raw losses from forward: {list(losses.keys())}")
            for loss_name, loss_value in losses.items():
                if isinstance(loss_value, torch.Tensor):
                    logger.debug(f"[BaseModel] {loss_name}: {loss_value.item()}")
                    if torch.isnan(loss_value):
                        logger.error(f"[BaseModel] NaN detected in {loss_name}!")
                    if torch.isinf(loss_value):
                        logger.error(f"[BaseModel] Inf detected in {loss_name}!")

        parsed_losses, log_vars = self.parse_losses(losses)  # type: ignore
        logger.debug(f"[BaseModel] Parsed total loss: {parsed_losses.item()}")
    ...
    def parse_losses(self, losses: dict[str, torch.Tensor]) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        ...
        """
        logger = MMLogger.get_current_instance()
        logger.debug(f"[BaseModel] Parsing losses: {list(losses.keys())}")

        log_vars = []
        for loss_name, loss_value in losses.items():
            if isinstance(loss_value, torch.Tensor):
                loss_mean = loss_value.mean()
                log_vars.append([loss_name, loss_mean])
                logger.debug(f"[BaseModel] {loss_name} mean: {loss_mean.item()}")

                if torch.isnan(loss_mean):
                    logger.error(f"[BaseModel] NaN detected in mean of {loss_name}!")
                if torch.isinf(loss_mean):
                    logger.error(f"[BaseModel] Inf detected in mean of {loss_name}!")

            elif is_list_of(loss_value, torch.Tensor):
                loss_sum = sum(_loss.mean() for _loss in loss_value)
                log_vars.append([loss_name, loss_sum])
                logger.debug(f"[BaseModel] {loss_name} sum of means: {loss_sum.item()}")

                if torch.isnan(loss_sum):
                    logger.error(f"[BaseModel] NaN detected in sum of {loss_name}!")
                if torch.isinf(loss_sum):
                    logger.error(f"[BaseModel] Inf detected in sum of {loss_name}!")
            else:
                raise TypeError(f"{loss_name} is not a tensor or list of tensors")

        loss = sum(value for key, value in log_vars if "loss" in key)
        logger.debug(f"[BaseModel] Total loss (sum of all loss components): {loss.item()}")

        if torch.isnan(loss):
            logger.error(f"[BaseModel] NaN detected in total loss!")
        if torch.isinf(loss):
            logger.error(f"[BaseModel] Inf detected in total loss!")
    ...
    def to(self, *args, **kwargs) -> nn.Module:
        ...
        if args and isinstance(args[0], str) and "npu" in args[0]:
            ...
            args = (
                next(iter(args)).replace(
                    "npu",
                    (torch_npu.npu.native_device if hasattr(torch_npu.npu, "native_device") else "privateuseone"),
                ),
            )
