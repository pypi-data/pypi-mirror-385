...
# yapf: enable
from visengine.registry import FUNCTIONS, MODEL_WRAPPERS
from visengine.structures import BaseDataElement
from visengine.utils import digit_version, is_seq_of
...
class MMFullyShardedDataParallel(FullyShardedDataParallel):
    ...
    def __init__(
        self,
        module: nn.Module,
        process_group: dict | ProcessGroup | None = None,
        sharding_strategy: str | ShardingStrategy = None,
        cpu_offload: bool | CPUOffload | None = None,
        auto_wrap_policy: str | Callable | None = None,
        backward_prefetch: str | BackwardPrefetch | None = None,
        mixed_precision: dict | MixedPrecision | None = None,
        param_init_fn: str | Callable[[nn.Module], None] | None = None,  # type: ignore
        ...
        **kwargs,
        ...
        if not (isinstance(cpu_offload, CPUOffload) or cpu_offload is None):
            raise TypeError(f"`cpu_offload` should be `None`, `bool`or `CPUOffload`, but has type {type(cpu_offload)}")

        with FUNCTIONS.switch_scope_and_registry(None):
            if isinstance(auto_wrap_policy, str):
                auto_wrap_policy = FUNCTIONS.get(auto_wrap_policy)  # type: ignore
            ...
            if not (isinstance(backward_prefetch, BackwardPrefetch) or backward_prefetch is None):
                raise TypeError(
            ...
            if isinstance(param_init_fn, str):
                param_init_fn = FUNCTIONS.get(param_init_fn)  # type: ignore
        ...
        def parse_dtype(dtype):
            if dtype is None:
                ...
                raise TypeError(f"`dtype` should be `None`, `str` or `torch.dtype`, but has type {type(dtype)}")

        if isinstance(mixed_precision, dict):
            mixed_precision["param_dtype"] = parse_dtype(mixed_precision.get("param_dtype", None))
            mixed_precision["reduce_dtype"] = parse_dtype(mixed_precision.get("reduce_dtype", None))
            mixed_precision["buffer_dtype"] = parse_dtype(mixed_precision.get("buffer_dtype", None))
        ...
        if "ignored_parameters" in kwargs:
            kwargs["ignored_parameters"] = self._get_ignored_params(module, kwargs["ignored_parameters"])

        if "ignored_modules" in kwargs:
            kwargs["ignored_modules"] = self._get_ignored_modules(module, kwargs["ignored_modules"])
    ...
    def train_step(self, data: dict, optim_wrapper: "OptimWrapper") -> dict[str, torch.Tensor]:
    ...
    def _run_forward(self, data: dict | tuple | list, mode: str) -> dict[str, torch.Tensor] | list:
    ...
    def _get_ignored_params(self, module: nn.Module, ignored_parameters: Iterable[str] | Iterable[nn.Module]):
        """Get params from string."""
        params_dict = dict(module.named_parameters())
        if is_seq_of(ignored_parameters, str):
            ignored_parameters = [params_dict[name] for name in ignored_parameters]
        if not is_seq_of(ignored_parameters, nn.Parameter) and ignored_parameters is not None:
            raise TypeError(
                f"`ignored_modules` should be `None`, `Iterable[str]` or `Iterable[nn.Parameters]`, but has type {type(ignored_parameters)}"
            )
        return ignored_parameters

    def _get_ignored_modules(self, module: nn.Module, ignored_modules: Iterable[str] | Iterable[nn.Module]):
        ...
        def set_state_dict_type(
            ...
            for submodule in traversal_utils._get_fsdp_states(module):
                ...
                if prev_state_dict_config is None:
                    ...
                    assert isinstance(submodule._state_dict_config, type(prev_state_dict_config)), (
