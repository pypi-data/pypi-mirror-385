...
class BaseDataPreprocessor(nn.Module):
    ...
    def cast_data(self, data: CastData) -> CastData:
        ...
        if isinstance(data, Mapping):
            ...
            # namedtuple
            return type(data)(*(self.cast_data(sample) for sample in data))  # type: ignore  # yapf:disable
    ...
    def to(self, *args, **kwargs) -> nn.Module:
        ...
        if args and isinstance(args[0], str) and "npu" in args[0]:
            args = (next(iter(args)).replace("npu", torch.npu.native_device),)
    ...
    def __init__(
        ...
        if mean is not None:
            ...
            assert len(std) == 3 or len(std) == 1, (  # type: ignore
                "`std` should have 1 or 3 values, to be compatible with RGB "  # type: ignore
