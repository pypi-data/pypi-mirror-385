...
def efficient_conv_bn_eval_forward(bn: nn.modules.batchnorm._BatchNorm, conv: nn.modules.conv._ConvNd, x: torch.Tensor):
...
def efficient_conv_bn_eval_control(bn: nn.modules.batchnorm._BatchNorm, conv: nn.modules.conv._ConvNd, x: torch.Tensor):
...
def efficient_conv_bn_eval_graph_transform(fx_model):
    ...
    for conv_node, bn_node in pairs:
        ...
        conv_get_node = fx_model.graph.create_node(op="get_attr", target=conv_node.target, name="get_conv")
        bn_get_node = fx_model.graph.create_node(op="get_attr", target=bn_node.target, name="get_bn")
...
def turn_on_efficient_conv_bn_eval(model: torch.nn.Module, modules: list[str] | str):
