...
from visengine.dist import broadcast, get_dist_info, infer_launcher, is_distributed
from visengine.logging import MMLogger
from visengine.model.wrappers import is_model_wrapper
from visengine.registry import MODELS, OPTIM_WRAPPERS, PARAM_SCHEDULERS
...
if TYPE_CHECKING:
    ...
    ParamSchedulerType = Union[list, dict[str, list]]
...
class BaseStrategy(metaclass=ABCMeta):
    ...
    def prepare(
        ...
        *,
        optim_wrapper: "BaseOptimWrapper" | dict | None = None,
    ...
    def _set_randomness(
        ...
        Args:
            ...
            deterministic (bool): Whether to set the deterministic option for
                ...
                more details.
        """
        from visengine.runner import set_random_seed
    ...
    def _init_model_weights(self, model: nn.Module) -> nn.Module:
        ...
        :meth:`init_weights`"""
        if hasattr(model, "init_weights") and self.dispatch_kwargs.get("init_weights_for_test_or_val", True):
            model.init_weights()
    ...
    def build_optim_wrapper(
        self,
        optim_wrapper: Optimizer | BaseOptimWrapper | dict,
    ...
    def build_param_scheduler(
        ...
        if default_args is None:
            ...
            if "epoch_length" in self.dispatch_kwargs:
                default_args["epoch_length"] = self.dispatch_kwargs["epoch_length"]
        ...
        if not isinstance(optim_wrapper, OptimWrapperDict):
            ...
            param_schedulers = self._build_param_scheduler(scheduler, optim_wrapper, default_args)  # type: ignore
            return param_schedulers
        else:
            param_schedulers = {}
            for name, optimizer in optim_wrapper.items():
                if isinstance(scheduler, dict) and "type" not in scheduler:
                    ...
                    param_schedulers[name] = self._build_param_scheduler(scheduler[name], optimizer, default_args)
                else:
                    param_schedulers[name] = self._build_param_scheduler(scheduler, optimizer, default_args)
    ...
    def _scale_lr(self) -> None:
        ...
        assert "base_batch_size" in self._auto_scale_lr, "Lack of `base_batch_size` in `auto_scale_lr`."
    ...
    def model_state_dict(self) -> dict[str, Any]:
        ...
        from visengine.runner import weights_to_cpu

        return weights_to_cpu(self.model.state_dict())

    def optim_state_dict(self) -> dict[str, Any]:
        """Get optimizer state dict."""
        if isinstance(self.optim_wrapper, BaseOptimWrapper):
            return self.optim_wrapper.state_dict()
        else:
            raise TypeError(f"self.optim_wrapper should be a `BaseOptimWrapper` instance, but got {self.optim_wrapper}")

    def scheduler_state_dict(self) -> dict | list:
    ...
    def load_scheduler_state_dict(self, state_dict: dict | list) -> None:
        """Load scheduler state from dict."""
        if isinstance(self.param_schedulers, dict):
            assert isinstance(state_dict, dict)
            for name, schedulers in self.param_schedulers.items():
                for scheduler, ckpt_scheduler in zip(schedulers, state_dict[name], strict=False):
                    scheduler.load_state_dict(ckpt_scheduler)
        else:
            for scheduler, ckpt_scheduler in zip(self.param_schedulers, state_dict, strict=False):  # type: ignore
                scheduler.load_state_dict(ckpt_scheduler)
    ...
    def load_or_resume(
        ...
        *,
        ...
        """
        from visengine.runner import find_latest_checkpoint
