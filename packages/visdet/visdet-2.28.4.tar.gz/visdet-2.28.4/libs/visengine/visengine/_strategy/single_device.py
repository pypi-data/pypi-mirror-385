...
from visengine.model import revert_sync_batchnorm
from visengine.optim import BaseOptimWrapper, _ParamScheduler
from visengine.registry import STRATEGIES
from visengine.utils import get_git_hash
...
class SingleDeviceStrategy(BaseStrategy):
    ...
    def prepare(
        ...
        *,
        ...
        if optim_wrapper is not None:
            ...
            accumulative_counts = getattr(self.optim_wrapper, "_accumulative_counts", 1)
            if accumulative_counts > 1:
                if "max_iters" not in self.dispatch_kwargs:
                    raise ValueError(
                        f'"max_iters" must be specified because "accumulative_counts" was set as {accumulative_counts} which is greater than 1.'
                    )
    ...
    def resume(
        ...
        *,
        ...
        checkpoint = self.load_checkpoint(filename, map_location=map_location, callback=callback)
