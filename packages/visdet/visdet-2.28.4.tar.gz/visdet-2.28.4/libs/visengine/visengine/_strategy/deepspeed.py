...
import visengine
from visengine.dist import init_dist, is_main_process
from visengine.optim import BaseOptimWrapper, _ParamScheduler
from visengine.registry import MODEL_WRAPPERS, OPTIM_WRAPPERS, OPTIMIZERS, STRATEGIES
from visengine.runner.checkpoint import save_checkpoint, weights_to_cpu
from visengine.utils import apply_to, digit_version, get_git_hash
...
class DeepSpeedOptimWrapper(BaseOptimWrapper):
    ...
    def zero_grad(self, **kwargs) -> None:
        raise NotImplementedError("DeepSpeedOptimWrapper does not support zero_grad method currently.")
...
class MMDeepSpeedEngineWrapper:
    def __init__(
        self,
        *,
        model: Any,  # Use Any instead of 'deepspeed.DeepSpeedEngine'
        inputs_to_half: list[int | str] | None = None,
    ):
        self.model = model
    ...
    def _run_forward(self, data: dict | tuple | list, mode: str) -> dict | list:
        ...
        if isinstance(data, dict):
            ...
            raise TypeError(f"Output of `data_preprocessor` should be list, tuple or dict, but got {type(data)}")
        return results

    def _cast_inputs_half(self, inputs: list | tuple | dict | None) -> list | tuple | dict:
        ...
        Returns:
            list or tuple or dict: Casted inputs.
        """
        if self._inputs_to_half is None or inputs is None:
            # Return an empty dict instead of None to satisfy the type checker
        ...
        if isinstance(inputs, list | tuple):
            ...
            for i, v in enumerate(inputs):
                if i in self._inputs_to_half:
                    new_inputs.append(apply_to(v, lambda x: hasattr(x, "to"), lambda x: x.to(dtype)))
                else:
                    new_inputs.append(v)
            ...
            for k, v in inputs.items():
                if k in self._inputs_to_half:
                    inputs[k] = apply_to(v, lambda x: hasattr(x, "to"), lambda x: x.to(dtype))
            return inputs
        else:
            raise TypeError(f"inputs should be list, tuple or dict, but got {type(inputs)}")
...
class DeepSpeedStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        *,
        ...
        zero_optimization: dict | None = None,
        ...
        **kwargs,
        ...
        if train_micro_batch_size_per_gpu is not None:
            self.config["train_micro_batch_size_per_gpu"] = train_micro_batch_size_per_gpu
        ...
        self._inputs_to_half = inputs_to_half
        assert exclude_frozen_parameters is None or digit_version(deepspeed.__version__) >= digit_version("0.13.2"), (
            "DeepSpeed >= 0.13.2 is required to enable exclude_frozen_parameters"
        )
    ...
    def prepare(
        ...
        *,
        ...
        if param_scheduler is not None:
            self.param_schedulers = self.build_param_scheduler(param_scheduler, self.optim_wrapper)
        self._prepared = True
        # Store the components but don't return anything to match BaseStrategy
        self._prepared_components()
        # Return None to match the base class return type
        return None
    ...
    def load_checkpoint(
        ...
        *,
        ...
        if digit_version(deepspeed.__version__) >= digit_version("0.13.2"):
            ...
            _, extra_ckpt = self.model.load_checkpoint(dirname, tag=basename, load_optimizer_states=False)
    ...
    def resume(
        ...
        *,
        ...
        if digit_version(deepspeed.__version__) >= digit_version("0.13.2"):
            ...
            _, extra_ckpt = self.model.load_checkpoint(dirname, tag=basename, load_optimizer_states=resume_optimizer)
    ...
    def save_checkpoint(
        ...
        *,
        ...
        state_dict_kwargs = {}
        if digit_version(deepspeed.__version__) >= digit_version("0.13.2"):
            state_dict_kwargs["exclude_frozen_parameters"] = self.exclude_frozen_parameters
        ...
        if save_optimizer:
            ...
            if self.model.zero_optimization_partition_weights():
                state_dict = self.model._zero3_consolidated_16bit_state_dict(**state_dict_kwargs)
            else:
