...
from visengine.dist import get_rank, is_main_process
from visengine.model import is_model_wrapper

if TYPE_CHECKING:
    from visengine.model import BaseDataPreprocessor
...
from visengine.utils import get_git_hash, mkdir_or_exist
...
class FSDPStrategy(DDPStrategy):
    ...
    def __init__(
        ...
        *,
        ...
        state_dict_cfg: str | dict = "local",
        activation_checkpointing: dict | None = None,
        **kwargs,
    ):
        ...
        if not isinstance(skip_init_weights, bool):
            raise TypeError(f"skip_init_weights must be a boolean, but got {type(skip_init_weights)}")
    ...
    def _wrap_model(self, model: nn.Module) -> None:
        ...
        if self.model_wrapper is None:
            self.model_wrapper = {"type": "MMFullyShardedDataParallel"}
        ...
        *,
        ...
        Keyword Args:
            ...
            callback (callable, callable): Callback function to modify the
                ...
                Defaults to None.
        """
        from visengine.runner.checkpoint import save_checkpoint
    ...
    def load_optim_state_dict(self, state_dict: dict) -> None:
        ...
        """
        optim_state_dict = FSDP.optim_state_dict_to_load(state_dict, self.model, self.optim_wrapper.optimizer)
        self.optim_wrapper.load_state_dict(optim_state_dict)

    def _init_state_dict_cfg(self, state_dict_cfg: str | dict) -> None:
        ...
        if isinstance(state_dict_cfg, str):
            if state_dict_cfg == "full":
                ...
                self.state_dict_config = FullStateDictConfig(rank0_only=True, offload_to_cpu=True)
                self.optim_state_dict_config = FullOptimStateDictConfig(rank0_only=True, offload_to_cpu=True)
                ...
                raise ValueError(f"FSDP only supports `full` and `local` state_dict_type, but got {state_dict_cfg}")
            ...
            if "state_dict_type" not in state_dict_cfg:
                ...
                if isinstance(state_dict_type, str):
                    self.state_dict_type = StateDictType[state_dict_cfg["state_dict_type"]]
                else:
            ...
            if state_dict_config is None:
                ...
                self.state_dict_config = FSDP_CONFIGS.build(state_dict_cfg["state_dict_config"])
            else:
                self.state_dict_config = state_dict_config

            optim_state_dict_config = state_dict_cfg.get("optim_state_dict_config")
            if optim_state_dict_config is None:
                self.optim_state_dict_config = LocalOptimStateDictConfig()
            elif isinstance(optim_state_dict_config, dict):
                self.optim_state_dict_config = FSDP_CONFIGS.build(state_dict_cfg["optim_state_dict_config"])
            else:
                self.optim_state_dict_config = optim_state_dict_config
        else:
            raise TypeError(f"state_dict_cfg should be a `str` or a `dict`, but got {type(state_dict_cfg)}")

        if not isinstance(self.state_dict_type, StateDictType):
            raise TypeError(f"state_dict_type must be StateDictType, but got {type(self.state_dict_type)}")
        if not isinstance(self.state_dict_config, StateDictConfig):
            raise TypeError(f"state_dict_config must be StateDictConfig, but got {type(self.state_dict_config)}")
        if not isinstance(self.optim_state_dict_config, OptimStateDictConfig):
            raise TypeError(
                f"optim_state_dict_config must be OptimStateDictConfig, but got {type(self.optim_state_dict_config)}"
            )
    ...
    def build_optim_wrapper(
        ...
        if isinstance(optim_wrapper, BaseOptimWrapper):
            ...
            assert not optim_state_dict["state"], (
                "Optimizer state_dict should be empty when giving an built optim_wrapper to FSDPStrategy"
            )
            # Align the state_dict with state_dict generated by
            # FSDP.full_optim_state_dict
            new_param_groups = []
            for group in param_groups:
                new_group = {key: value for key, value in group.items() if key != "param"}
                new_group["params"] = [self.id_to_name[id(param)] for param in group["params"]]
                new_param_groups.append(new_group)
            optim_state_dict["param_groups"] = new_param_groups
            defaults = {k: v for k, v in optimizer.defaults.items() if k != "differentiable"}
            ...
            for param_group in new_param_groups:
                ...
                for param_name in param_group["params"]:
                    if param_name not in params_dict:
                        raise RuntimeError(
                            "Failed to reconstruct the sharded optimizer. You can try to set `use_orig_params=True` in `model_wrapper`"
                        )
                    _params.append(params_dict[param_name])
                param_group = {k: v for k, v in param_group.items() if k != "param"}
            ...
            with FSDP.state_dict_type(model, StateDictType.FULL_STATE_DICT):
                optim_state_dict = FSDP.optim_state_dict_to_load(optim_state_dict, model, new_optimizer)
        ...
        if isinstance(optim_wrapper, dict | ConfigDict | Config):
            ...
            if optim_wrapper.get("type", "AmpOptimWrapper") in (
                "AmpOptimWrapper",
                AmpOptimWrapper,
            ):
    ...
    def _build_param_scheduler(
        ...
        for scheduler in schedulers:
            ...
            if isinstance(scheduler, _ParamScheduler | LRScheduler):
                parameter_keys = inspect.signature(scheduler.__class__).parameters.keys()
                kwargs = {k: v for k, v in scheduler.state_dict().items() if k in parameter_keys}
                ...
                if _scheduler.get("by_epoch", True):
                    if max_epochs is None:
                        raise ValueError("max_epochs must be specified in default_args")
