...
from visengine.config.utils import MODULE2PACKAGE
from visengine.dataset import pseudo_collate
from visengine.device import get_device
from visengine.fileio import get_file_backend, isdir, join_path, list_dir_or_file, load
from visengine.logging import print_log
from visengine.registry import FUNCTIONS, MODELS, VISUALIZERS, DefaultScope
from visengine.runner.checkpoint import _load_checkpoint, _load_checkpoint_to_model
...
class InferencerMeta(ABCMeta):
    ...
    def __init__(cls, *args, **kwargs):
        ...
        assert isinstance(cls.preprocess_kwargs, set)
        assert isinstance(cls.forward_kwargs, set)
        assert isinstance(cls.visualize_kwargs, set)
        assert isinstance(cls.postprocess_kwargs, set)
...
class BaseInferencer(metaclass=InferencerMeta):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        visualization = self.visualize(ori_inputs, preds, **visualize_kwargs)  # type: ignore
    ...
    @torch.no_grad()
    def forward(self, inputs: dict | tuple, **kwargs) -> Any:
        ...
        if not cfg:
            ...
            try:
                ...
                if "cfg" in meta:
                    ...
                    raise ValueError("Cannot find the config in the checkpoint.")
    ...
    def _load_weights_to_model(self, model: nn.Module, checkpoint: dict | None, cfg: ConfigType | None) -> None:
    ...
    def _dispatch_kwargs(self, **kwargs) -> tuple[dict, dict, dict, dict]:
        ...
        method_kwargs = self.preprocess_kwargs | self.forward_kwargs | self.visualize_kwargs | self.postprocess_kwargs
    ...
    @staticmethod
    def list_models(scope: str | None = None, patterns: str = r".*"):
