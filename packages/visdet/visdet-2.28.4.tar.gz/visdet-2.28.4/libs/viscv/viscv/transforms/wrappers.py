...
class KeyMapper(BaseTransform):
    ...
    def __init__(
        ...
        if self.auto_remap:
            if remapping is not None:
                raise ValueError("KeyMapper: ``remapping`` must be None if`auto_remap` is set True.")
    ...
    def _map_input(self, data: dict, mapping: dict | None) -> dict[str, Any]:
    ...
    def _map_output(self, data: dict, remapping: dict | None) -> dict[str, Any]:
    ...
    def _apply_transforms(self, inputs: dict) -> dict:
        ...
        if outputs is None:
            raise ValueError(f"Transforms wrapped by {self.__class__.__name__} should not return None.")
...
class TransformBroadcaster(KeyMapper):
    ...
    def __init__(
        ...
        super().__init__(transforms, mapping, remapping, auto_remap, allow_nonexist_keys)
    ...
    def transform(self, results: dict):
        ...
        with ctx(self.transforms):
            output_scatters = [self._apply_transforms(_input) for _input in input_scatters]

        # Collate output scatters (list of dict to dict of list)
        outputs = {key: [_output[key] for _output in output_scatters] for key in output_scatters[0]}
...
class RandomApply(BaseTransform):
    ...
    def __init__(self, transforms: Transform | list[Transform], prob: float = 0.5):
        super().__init__()
        self.prob = prob
        self.transforms = Compose(transforms)
