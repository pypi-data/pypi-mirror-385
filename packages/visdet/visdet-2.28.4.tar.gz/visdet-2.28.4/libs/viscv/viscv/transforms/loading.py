...
class LoadImageFromFile(BaseTransform):
    ...
    Args:
        ...
        file_client_args (dict, optional): Arguments to instantiate a
            FileClient. See :class:`visengine.fileio.FileClient` for details.
        ...
        enable_cache (bool): Whether to enable on-disk caching of resized images.
            When enabled, images are cached after resizing to avoid repeated
            decoding and resizing in subsequent epochs. Defaults to False.
        cache_dir (str, optional): Directory to store cached images. If None,
            uses default cache directory (~/.cache/viscv/image_cache).
            Only used if enable_cache is True.
        cache_max_size_gb (float): Maximum cache size in gigabytes. When exceeded,
            least recently used images are evicted. Defaults to 10.0.
            Only used if enable_cache is True.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.cache = ImageCache(
            cache_dir=cache_dir if cache_dir is not None else ImageCache.__init__.__defaults__[0],  # type: ignore
    ...
    def transform(self, results: dict) -> dict | None:
        ...
        Args:
            results (dict): Result dict from
                :class:`visengine.dataset.BaseDataset`.
        ...
        if target_size is not None and self.cache.enabled:
            cached_img = self.cache.get(filename, target_size)
            if cached_img is not None:
                ...
                results["ori_shape"] = cached_img.shape[:2]  # Store original cached size
                results["_cache_hit"] = True
                results["_image_cache"] = self.cache
                return results
        ...
        # Pass cache instance to downstream transforms
...
class LoadAnnotations(BaseTransform):
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        """
        if self.file_client_args is not None:
            file_client = engine_fileio.FileClient.infer_client(self.file_client_args, results["seg_map_path"])
        ...
        results["gt_seg_map"] = viscv.imfrombytes(img_bytes, flag="unchanged", backend=self.imdecode_backend).squeeze()
