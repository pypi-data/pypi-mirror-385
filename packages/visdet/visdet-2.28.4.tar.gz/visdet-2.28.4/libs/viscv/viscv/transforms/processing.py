...
from viscv.image import hsv2bgr, imcrop, imflip, imnormalize, impad
...
class Normalize(BaseTransform):
    ...
    def __init__(self, mean: Sequence[Number], std: Sequence[Number], to_rgb: bool = True) -> None:
...
class Resize(BaseTransform):
    ...
    Args:
        scale (tuple[int]): Images scales for resizing in (w, h) order.
        keep_ratio (bool): Whether to keep the aspect ratio when resizing the
            image. Defaults to True.
        enable_cache (bool): Whether to enable caching of resized images.
            When enabled, resized images will be saved to disk cache if the
            LoadImageFromFile transform has caching enabled. Defaults to True.
    """

    def __init__(self, scale, keep_ratio=True, enable_cache=True):
    ...
    def transform(self, results):
        ...
        """
        from viscv.image import imresize
            ...
            if hasattr(gt_masks, "resize"):
                ...
                for mask in gt_masks:
                    ...
                    for poly in mask:
                        ...
                        poly[:, 0] *= scale_factor[0]
                        poly[:, 1] *= scale_factor[1]
                        resized_mask.append(poly.reshape(-1).tolist())
                    resized_masks.append(resized_mask)
                results["gt_masks"] = resized_masks

        # Save to cache if enabled and not already cached
        if self.enable_cache and not results.get("_cache_hit", False):
            img_path = results.get("img_path")
            cache = results.get("_image_cache")

            if img_path is not None and cache is not None:
                # Store target size so LoadImageFromFile can check cache on next load
                target_size = (new_w, new_h)
                results["_cache_target_size"] = target_size

                # Save resized image to cache
                cache.put(img_path, target_size, resized_img)
    ...
    def _crop_bboxes(self, results: dict, bboxes: np.ndarray) -> None:
        ...
        if "gt_bboxes" in results:
            ...
            if self.clip_object_border:
                ...
                # Clip x coordinates
                gt_bboxes[:, 0] = np.clip(gt_bboxes[:, 0], 0, img_w)  # x_min
                gt_bboxes[:, 2] = np.clip(gt_bboxes[:, 2], 0, img_w)  # x_max

                # Clip y coordinates
                gt_bboxes[:, 1] = np.clip(gt_bboxes[:, 1], 0, img_h)  # y_min
                gt_bboxes[:, 3] = np.clip(gt_bboxes[:, 3], 0, img_h)  # y_max
        ...
        if self._random_prob() < self.prob:
            if num_output_channels > 1:
                ...
                if self.keep_channels:
                    img = img[:, :, None]
                    results["img"] = np.dstack([img for _ in range(num_output_channels)])
                else:
                    results["img"] = img
                return results
        img = img.astype("uint8")
    ...
    def transform(self, results: dict) -> dict:
        ...
        for scale in self.scales:
            for flip, direction in flip_args:
                ...
                data_samples.append(packed_results["data_sample"])  # type: ignore
...
class TestTimeAug(BaseTransform):
    ...
    def __init__(self, transforms: list):
        ...
        self.subroutines = [Compose(subroutine) for subroutine in product(*transforms)]
    ...
    def transform(self, results: dict) -> dict:
        ...
        for subroutine in self.subroutines:
            ...
            assert isinstance(result, dict), f"Data processed by {subroutine} must return a dict, but got {result}"
...
class RandomFlip(BaseTransform):
    ...
    def __init__(
        ...
        swap_seg_labels: Sequence | None = None,
    ) -> None:
        ...
        if self.swap_seg_labels is not None:
            ...
            for pair in self.swap_seg_labels:
                assert isinstance(pair, (tuple, list)) and len(pair) == 2, (
                    f"swap_seg_labels must be a sequence with pair, but got {self.swap_seg_labels}."
                )
...
class RandomResize(BaseTransform):
    ...
    def _random_scale(self) -> tuple:
        ...
        if is_tuple_of(self.scale, int):
            ...
            raise NotImplementedError(f'Do not support sampling function for "{self.scale}"')
