...
async def detect_blocked_tasks(
    task_service: TaskService, tasks: list[dict[str, Any]]
) -> list[dict[str, Any]]:
    ...
    Args:
        task_service: TaskService for fetching dependencies
        ...
        try:
            ...
            if dependencies:
                ...
                incomplete_deps = [
                    ...
                    if dep.status and dep.status.value != "done"
                ]
...
async def build_workspace_dependency_graph(
    ...
    tasks = [task.model_dump() for task in result.items]
...
def show_board(
    ...
    async def fetch_and_display() -> None:
        try:
            ...
            blocked_tasks = await detect_blocked_tasks(task_service, tasks)
            blocked_identifiers = {
...
def show_summary(
    ...
    async def fetch_and_display() -> None:
        try:
            ...
            tasks = [task.model_dump() for task in result.items]
            total = result.total or len(tasks)
