...
class TestDependencyListCommand:
    ...
    def test_dep_list_shows_dependencies_and_dependents(
        ...
        # Create test tasks using Pydantic models
        current_task = create_test_task(
            id=2,
            identifier="DEV-2",
            title="Current Task",
            status=Status.IN_PROGRESS,
        )
        dependency_task = create_test_task(
            id=1,
            identifier="DEV-1",
            title="Dependency",
            status=Status.DONE,
        ...
        mock_client = AsyncMock()
        mock_client.get_task = AsyncMock(return_value=current_task)
        mock_client.get_task_dependencies = AsyncMock(return_value=[dependency_task])
        mock_client.get_task_dependents = AsyncMock(return_value=[dependent_task])
