...
class TestAIDecomposeCommand:
    ...
    def test_decompose_basic(
        self, cli_runner: CliRunner, mock_config, mock_workspace_config, monkeypatch
    ):
        """Test basic goal decomposition."""
        monkeypatch.setattr("cli.config.GlobalConfig.load", lambda: mock_config)
        monkeypatch.setattr(
            "cli.config.WorkspaceConfig.load", lambda: mock_workspace_config
        )

        with patch(
            "cli.client.ai.AIAPIClient.decompose_goal", new_callable=AsyncMock
        ) as mock_decompose:
            ...
            result = cli_runner.invoke(
                app, ["ai", "decompose", "Add user authentication"]
            )

            assert result.exit_code == 0
            assert (
                "Decomposition complete" in result.output
                or "Tasks created" in result.output
            )
            mock_decompose.assert_called_once()

    def test_decompose_with_options(
        self, cli_runner: CliRunner, mock_config, mock_workspace_config, monkeypatch
    ):
    ...
    def test_decompose_json_output(
        self, cli_runner: CliRunner, mock_config, mock_workspace_config, monkeypatch
    ):
...
class TestAIOrganizeCommand:
    ...
    def test_organize_basic(
        ...
        with patch(
            "cli.client.ai.AIAPIClient.organize_workspace", new_callable=AsyncMock
        ) as mock_organize:
...
class TestAIFillCommand:
    ...
    def test_fill_task(
        ...
        with patch(
            "cli.client.ai.AIAPIClient.fill_task_details", new_callable=AsyncMock
        ) as mock_fill:
...
class TestAISuggestCommand:
    ...
    def test_suggest_tasks(
        ...
        with patch(
            "cli.client.ai.AIAPIClient.get_ai_suggestions", new_callable=AsyncMock
        ) as mock_suggest:
...
class TestAIReviewCommand:
    ...
    @pytest.mark.skip(
        reason="Review task test needs fixing - complex mocking requirements"
    )
    def test_review_task(
        ...
        with patch(
            "cli.client.tasks.TasksAPIClient.get_task", new_callable=AsyncMock
        ) as mock_get:
            with patch(
                "cli.client.ai.AIAPIClient.review_task", new_callable=AsyncMock
            ) as mock_review:
...
class TestAISummaryCommand:
    ...
    def test_summary_basic(
        ...
        with patch(
            "cli.client.ai.AIAPIClient.generate_summary", new_callable=AsyncMock
        ) as mock_summary:
...
class TestAIUsageCommand:
    ...
    def test_usage_basic(
        ...
        with patch(
            "cli.client.ai.AIAPIClient.get_ai_usage", new_callable=AsyncMock
        ) as mock_usage:
