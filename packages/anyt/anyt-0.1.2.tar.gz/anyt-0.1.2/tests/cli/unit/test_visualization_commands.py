...
class TestBoardCommand:
    ...
    def test_board_basic_display(
        ...
        with patch(
            "cli.client.tasks.TasksAPIClient.list_tasks", new_callable=AsyncMock
        ) as mock_list:
            mock_list.return_value = PaginatedResponse[Task](
                items=sample_tasks,
    ...
    def test_board_with_mine_filter(
        ...
        # First arg should be TaskFilters object
    ...
    def test_board_with_status_filter(
        ...
        with patch(
            ...
            mock_list.return_value = PaginatedResponse[Task](
                items=[t for t in sample_tasks if t.status == Status.IN_PROGRESS],
                total=1,
                limit=200,
                offset=0,
            )

            result = cli_runner.invoke(app, ["board", "--status", "in_progress"])
...
class TestTimelineCommand:
    ...
    def test_timeline_basic_display(self, cli_runner: CliRunner, monkeypatch):
        ...
        with patch(
            ...
            mock_get.return_value = create_test_task(
                id=1,
                identifier="DEV-42",
                title="Test Task",
                status=Status.TODO,
...
class TestGraphCommand:
    ...
    def test_graph_for_specific_task(self, cli_runner: CliRunner, monkeypatch):
        ...
        with patch(
            ...
            with patch(
                "cli.client.tasks.TasksAPIClient.get_task_dependencies",
                new_callable=AsyncMock,
            ) as mock_deps:
                with patch(
                    "cli.client.tasks.TasksAPIClient.get_task_dependents",
                    new_callable=AsyncMock,
                ) as mock_depts:
                    mock_get.return_value = create_test_task(
                        id=2,
