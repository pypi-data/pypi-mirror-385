...
def login(
    ...
    try:
        ...
        if agent_key_value:
            ...
            client = BaseAPIClient(base_url=env_config.api_url, agent_key=key)
            ...
            client = BaseAPIClient(base_url=env_config.api_url, auth_token=pat)
            ...
            if env_agent_key:
                ...
                client = BaseAPIClient(
                    base_url=env_config.api_url, agent_key=env_agent_key
                )
    ...
    try:
        ...
        if credential:
            if is_agent_key:
                ...
                ws_client = WorkspacesAPIClient(base_url=api_url, agent_key=credential)
            else:
                console.print("Authentication: [green]User Token[/green]")
                ws_client = WorkspacesAPIClient(base_url=api_url, auth_token=credential)

            async def get_info() -> list[Any] | None:
                try:
                    await ws_client.health_check()
            ...
            if workspaces:
                ...
                if workspaces:
                    ...
                    for ws in workspaces:
                        console.print(f"  - {ws.name} ({ws.identifier})")
            else:
