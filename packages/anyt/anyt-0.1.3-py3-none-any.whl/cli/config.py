...
class GlobalConfig(BaseModel):
    ...
    def get_effective_config(self) -> dict[str, Any]:
        ...
        1. Environment variables (ANYT_AUTH_TOKEN, ANYT_AGENT_KEY)
        2. Workspace config (.anyt/anyt.json) - agent_key only
        3. Global config (~/.config/anyt/config.json)
        """
        config = {
        ...
        # Check workspace config for agent_key (overrides global config)
        workspace_config = WorkspaceConfig.load()
        if workspace_config and workspace_config.agent_key:
            config["agent_key"] = workspace_config.agent_key
            # If workspace has agent key, clear auth_token (agent key takes precedence)
            config["auth_token"] = None
        ...
        if env_name := os.getenv("ANYT_ENV"):
            ...
            if env_name in self.environments:
                ...
                # Only load agent_key/auth_token from overridden environment if not set by workspace
                # Workspace config takes precedence over environment config
                if not workspace_config or not workspace_config.agent_key:
                    if env_config.agent_key:
                        config["agent_key"] = env_config.agent_key
                    if env_config.auth_token:
                        # Detect if auth_token is actually an agent key
