...
class LegalEntityMatcher:
    def __init__(self, vocab, label):
        ...
        # 2. COURT NAME (Example: Supreme Court, Appellate Division)
        self.matcher.add("COURT_NAME_PATTERN",
            [
                [{"LOWER": "supreme"}, {"LOWER": "court"}],
                [{"LOWER": "national"}, {"LOWER": "green"}, {"LOWER": "tribunal"}], # NGT
                [{"LOWER": "nclt"}] # NCLT
            ],
        ...
        # EXPANDED DOMAIN-SPECIFIC ENTITIES
        
        # 5. CONTRACT TYPE (Commercial, Employment, Energy)
        self.matcher.add("CONTRACT_TYPE_PATTERN",
            [
                [{"TEXT": {"IN": ["NDA", "MOU", "PPA"]}}], # PPA: Power Purchase Agreement
                [{"LOWER": "service"}, {"LOWER": "agreement"}],
                [{"LOWER": "bot"}, {"LOWER": "contract"}], # Build-Operate-Transfer
            ],
        ...
        # 7. CRIME ACT / COMPLIANCE ACT (Criminal, Regulatory, Data Privacy)
        self.matcher.add("CRIME_ACT_PATTERN",
            [
                [{"LOWER": "section"}, {"IS_DIGIT": True}, {"TEXT": "IPC"}],
                [{"TEXT": "DPDP"}, {"LOWER": "act"}], # Data Protection Act (India)
                [{"TEXT": "GDPR"}], # GDPR (EU)
                [{"LOWER": "it"}, {"LOWER": "act"}], # Information Technology Act
            ],
            on_match=self.set_crime_act_label)
    ...
    def set_crime_act_label(self, matcher, doc, i, matches): self._set_span_label(doc, matches, i, "ACT_STATUTE")
...
def load_legal_nlp(model_name: str = DEFAULT_MODEL):
    ...
    This is the function the user calls: lexy_ner.load()
    """
    try:
