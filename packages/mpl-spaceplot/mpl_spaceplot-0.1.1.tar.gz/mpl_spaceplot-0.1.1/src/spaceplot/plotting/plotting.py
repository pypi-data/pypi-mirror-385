...
def plt_image(
    ...
    **kwargs,
    ...
    if roi_crop:
        crop_arr = roi._crop_array(image, thumbnail=False)
    else:
        crop_arr = image

    plot_image = tools.downsample_mask(crop_arr, mask_length=None, ax=ax, downsample=downsample)

    if roi is None and 'extent' not in kwargs:
        shp = image.shape
        extent = (0, shp[1], 0, shp[0])
    elif roi is None and 'extent' in kwargs:
    ...
    if scale_bar:
        ...
        theme = 'light' if scale_corner_value > 0.25 else 'dark'

        if scale_bar_size is None:
            width = extent[1] - extent[0]
            um = width * 0.3125
            rounded_scale = decs.tools._round_scale(um / 5)
        else:
