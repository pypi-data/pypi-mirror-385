"""
    Functions that define how to render categorical variables information statistics
"""
...
def render_dirty_categories(
    config: Settings, summary: dict, varid: str
) -> Renderable:
    table = Table(
        summary['dirty_categories_values'].to_dict(),
        name="Dirty Categories",
        achor_id=f"{varid}_dirty_categories",
        style=config.html.style
    )
    return table
...
def _render_dirty_categories_tab(config: Settings, summary: dict) -> Container:

    summary_list = [
        {
            "name": "Cluster Representative",
            "value": "Detecter Variants",
            "alert": False,
        }
    ]
    summary_list += [
        {
            "name": row['Cluster Representative'],
            "value": row['Detected Variants'],
            "alert": False
        }
        for _, row in summary["dirty_categories_values"].iterrows()
    ]

    dirty_categories_table = Table(
        summary_list,
        name = 'Dirty Categories overview',
        anchor_id=f"{summary['varid']}dirtycategories",
        style=config.html.style,
    )
    return Container(
        items=[dirty_categories_table],
        sequence_type="grid",
        name="Dirty categories",
        anchor_id=f"{summary['varid']}_dirty_categories"
    )

def _get_categorical_summary_stats(config: Settings, summary: dict) -> Table:
    ...
    if config.vars.cat.dirty_categories:
        table_info += [
            {
                "name": "Dirty Categories",
                "value": fmt(summary["n_fuzzy_vals"]),
                "alert": "n_fuzzy_vals" in summary["alert_fields"],
            },
            {
                "name": "Dirty Categories (%)",
                "value": fmt_percent(summary["p_fuzzy_vals"]),
                "alert": "p_fuzzy_vals" in summary["alert_fields"],
            },
        ]
...
def render_categorical(config: Settings, summary: dict) -> dict:
    ...
    table = _get_categorical_summary_stats(config, summary)
    ...
    if config.vars.cat.dirty_categories and config.vars.cat.redact is False:
        bottom_items.append(_render_dirty_categories_tab(config, summary))
