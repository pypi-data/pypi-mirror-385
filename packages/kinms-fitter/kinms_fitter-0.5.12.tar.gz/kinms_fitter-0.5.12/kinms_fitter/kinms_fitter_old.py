...
class kinms_fitter:
    ...
    def __init__(self,filename,spatial_trim=None,spectral_trim=None,linefree_chans=[1,5]):
        ...
        smallest_dim=np.min([np.diff(xcent_range-self._xc_img)[0],np.diff(ycent_range-self._yc_img)[0]])
        self.xcent_range=np.array([-smallest_dim/2,smallest_dim/2])+self._xc_img
        self.ycent_range=np.array([-smallest_dim/2,smallest_dim/2])+self._yc_img
    ...
    def get_header_coord_arrays(self,hdr):
        ...
        # breakpoint()
        # cd1=self.spectralcube.wcs.pixel_scale_matrix[0,0]
        # cd2=self.spectralcube.wcs.pixel_scale_matrix[1,1]
