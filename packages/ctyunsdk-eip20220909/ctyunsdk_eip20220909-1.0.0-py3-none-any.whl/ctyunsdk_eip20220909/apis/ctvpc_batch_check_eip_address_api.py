...
@dataclass_json
@dataclass
class CtvpcBatchCheckEipAddressRequest:
    regionID: str  # 资源池 ID
    eipAddresses: List[str]  # 弹性公网IP地址列表
    clientToken: Optional[str] = None  # 客户端存根，用于保证订单幂等性, 长度 1 - 64


@dataclass_json
@dataclass
class CtvpcBatchCheckEipAddressReturnObjResponse:
...
@dataclass_json
@dataclass
class CtvpcBatchCheckEipAddressResponse:
    ...
    returnObj: Optional[List[Optional[CtvpcBatchCheckEipAddressReturnObjResponse]]] = None  # 接口业务数据


# 调用此接口可批量检查弹性公网IP地址是否已经被使用。
class CtvpcBatchCheckEipAddressApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcBatchCheckEipAddressRequest) -> CtvpcBatchCheckEipAddressResponse:
        url = endpoint + "/v4/eip/check-addresses"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcBatchCheckEipAddressResponse.from_dict(response.json())
