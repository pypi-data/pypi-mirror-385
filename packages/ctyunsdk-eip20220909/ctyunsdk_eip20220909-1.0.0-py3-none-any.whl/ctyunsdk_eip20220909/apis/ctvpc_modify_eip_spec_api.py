...
@dataclass_json
@dataclass
class CtvpcModifyEipSpecRequest:
    clientToken: str  # 客户端存根，用于保证订单幂等性, 长度 1 - 64
    regionID: str  # 资源池 ID
    eipID: str  # eip id
    bandwidth: Any  # 弹性 IP 带宽
    payVoucherPrice: Optional[str] = None  # 代金券金额，支持到小数点后两位，仅包周期支持代金券


@dataclass_json
@dataclass
class CtvpcModifyEipSpecReturnObjResponse:
    """object"""
...
@dataclass_json
@dataclass
class CtvpcModifyEipSpecResponse:
    ...
    returnObj: Optional[CtvpcModifyEipSpecReturnObjResponse] = None  # object


# 调用此接口修改 EIP 带宽峰值。
class CtvpcModifyEipSpecApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcModifyEipSpecRequest) -> CtvpcModifyEipSpecResponse:
        url = endpoint + "/v4/eip/modify-spec"
        params = {}
        try:
            header_params = {}
            ...
            return CtvpcModifyEipSpecResponse.from_dict(response.json())
