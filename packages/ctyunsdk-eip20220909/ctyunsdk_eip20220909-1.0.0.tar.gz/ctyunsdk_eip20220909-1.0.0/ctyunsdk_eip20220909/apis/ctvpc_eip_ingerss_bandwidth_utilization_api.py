...
@dataclass_json
@dataclass
class CtvpcEipIngerssBandwidthUtilizationRequest:
    regionID: str  # 资源池 ID
    eipID: str  # 弹性公网IP的ID
...
@dataclass_json
@dataclass
class CtvpcEipIngerssBandwidthUtilizationReturnObjResponse:
    """返回结果"""
...
@dataclass_json
@dataclass
class CtvpcEipIngerssBandwidthUtilizationResponse:
    ...
    returnObj: Optional[CtvpcEipIngerssBandwidthUtilizationReturnObjResponse] = None  # 返回结果


# 入方向带宽利用率。
class CtvpcEipIngerssBandwidthUtilizationApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcEipIngerssBandwidthUtilizationRequest) -> CtvpcEipIngerssBandwidthUtilizationResponse:
        url = endpoint + "/v4/eip/ingress-bandwidth-utilization"
        params = {'regionID': request.regionID, 'eipID': request.eipID}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            return CtvpcEipIngerssBandwidthUtilizationResponse.from_dict(response.json())
