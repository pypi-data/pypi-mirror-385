...
@dataclass_json
@dataclass
class CtvpcEipEgerssBandwidthUtilizationRequest:
    regionID: str  # 资源池 ID
    eipID: str  # 弹性公网IP的ID
...
@dataclass_json
@dataclass
class CtvpcEipEgerssBandwidthUtilizationReturnObjResponse:
    """返回结果"""
...
@dataclass_json
@dataclass
class CtvpcEipEgerssBandwidthUtilizationResponse:
    ...
    returnObj: Optional[CtvpcEipEgerssBandwidthUtilizationReturnObjResponse] = None  # 返回结果


# 出方向带宽利用率。
class CtvpcEipEgerssBandwidthUtilizationApi:
    ...
    def do(credential: Credential, client: CtyunClient, endpoint: str,
           request: CtvpcEipEgerssBandwidthUtilizationRequest) -> CtvpcEipEgerssBandwidthUtilizationResponse:
        url = endpoint + "/v4/eip/egress-bandwidth-utilization"
        params = {'regionID': request.regionID, 'eipID': request.eipID}
        try:
            request_dict = request.to_dict() if hasattr(request, 'to_dict') else request
            request_dict = {key: value for key, value in request_dict.items() if value is not None}
            response = client.get(url=url, params=params, header_params=request_dict, credential=credential)
            return CtvpcEipEgerssBandwidthUtilizationResponse.from_dict(response.json())
