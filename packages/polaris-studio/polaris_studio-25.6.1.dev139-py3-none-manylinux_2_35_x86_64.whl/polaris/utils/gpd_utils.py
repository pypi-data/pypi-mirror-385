...
def write_spatialite_layer(
    ...
    with conn or commit_and_close(db=db_path, spatial=True) as conn:
        ...
        gdf = gdf.assign(geo_wkt=gdf.geometry.to_wkb(), srid=srid)
        data = gdf[cols + ["geo_wkt", "srid"]].to_records(index=False)
        for s in queries:
            conn.execute(s)
...
def read_spatialite_layer(
    ...
    with conn or read_and_close(db_path, spatial=True) as conn:
        ...
        keys = ",".join(fields)
        sql = f"select {keys} from '{table_name}' WHERE {geo_field} IS NOT null;"
