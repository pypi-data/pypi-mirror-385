...
class OpenData:
    ...
    **FOR LARGE MODELLING AREAS IT IS RECOMMENDED TO DEPLOY YOUR OWN OVERPASS SERVER**
    ...
    def get_traffic_signal(self, point) -> TrafficLight:
        ...
        if self.__traffic_lights.get("highway", {}).get("traffic_signals", gpd.GeoDataFrame([])).empty:
    ...
    def __load_traffic_light_data(self):
        ...
        logging.info(f"Downloading OSM data. {len(bboxes)} bounding boxes for traffic signals")
