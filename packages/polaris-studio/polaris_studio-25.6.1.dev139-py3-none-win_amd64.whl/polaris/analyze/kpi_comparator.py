...
class KpiComparator:
    ...
    def plot_garage_parking_demand(self, **kwargs):
        import matplotlib.pyplot as plt

        df = self._get_results("garage_parking_demand", **kwargs)
        ...
        fig, ax = plt.subplots(nrows=1, ncols=1, figsize=(10, 8), sharex="all")
        sns.lineplot(df, hue="run_id", x="start_hr", y="parking_demand", ax=ax)
        ax.set(title="Parking Demand", xlabel="Time of Day (hr)", ylabel="# of parkers")
    ...
    def plot_parking_access(self, **kwargs):
        ...
        sns.barplot(df, hue="run_id", x="Escooter_Borrowed", y="access_min", ax=ax[0])
        ax[0].set(title="Parking Access Time", xlabel="Used Escooter?", ylabel="Minutes")

        sns.barplot(df, hue="run_id", x="Escooter_Borrowed", y="demand", ax=ax[1])
