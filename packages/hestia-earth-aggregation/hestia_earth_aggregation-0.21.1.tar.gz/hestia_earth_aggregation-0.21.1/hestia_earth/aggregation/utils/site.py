...
def format_site_results(data: dict, should_cleanup_measurements: bool = True):
    measurements = list(map(_format_aggregate(new_measurement), data.get('measurements', [])))
    management = list(map(_format_aggregate(new_management), data.get('management', [])))
    values = (
        ({'measurements': measurements} if measurements else {}) |
        ({'management': cleanup_blank_nodes(management)} if management else {})
    )
    return values | ({
        'measurements': cleanup_blank_nodes(measurements)
    } if measurements and should_cleanup_measurements else {})
