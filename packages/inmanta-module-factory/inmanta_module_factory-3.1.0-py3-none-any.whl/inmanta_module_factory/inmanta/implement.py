...
class Implement(ModuleElement):
    def __init__(
        ...
        *,
        ...
        if implementation is not None and implementations:
            raise ValueError(
                "Parameter implementation and implementations can not be used together"
            )
        elif implementation is not None:
            self._logger.warning(
                "The implementation argument is deprecated, use implementations instead"
            )
    ...
    def implementation(self) -> Implementation:
        self._logger.warning(
            "Usage of implementation is deprecated, use implementations instead"
        )
        return self.implementations[0]

    def _ordering_key(self) -> str:
        implementations_key = ".".join(
            implementation.full_path_string for implementation in self.implementations
        )
        return (
            f"{chr(255)}.implement.{self.entity.full_path_string}.{implementations_key}"
        )
    ...
    def validate(self) -> bool:
        if not self.implementations and not self.using_parents:
            ...
            self._logger.warning(
                "Every implement statement should reference at least one implementation or "
                "have using_parents set to True"
            )
            return False
