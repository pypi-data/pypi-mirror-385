    ...
    def validate(self) -> bool:
        if len(self.fields) == 0:
            self._logger.warning(
                f"An index should always have more than one field: {repr(self.entity)}"
            )
            return False
        ...
        if not len(set(self.fields) - all_fields) == 0:
            self._logger.warning(
                "Some fields part of the index, are not part of the entity: "
                f"{set(self.fields) - all_fields}"
            )
            return False
