...
def test_multiple_implementations(project: Project) -> None:
    ...
    module_builder.generate_module(module_path)
...
def test_no_implementations(project: Project) -> None:
    ...
    with pytest.raises(ValueError):
        module_builder.generate_module(Path(project._test_project_dir) / "modules")

    shutil.rmtree(
        str(Path(project._test_project_dir) / "modules" / module_builder._module.name)
    )
