...
from .choicelists import Shortcuts, UploadAreas, ImageFormats, ImageSizes
...
class Upload(UploadBase, UserAuthored, Controllable, Publishable):
    ...
    def as_html(self, ar, mf, image_format=None, image_size=None,
                title='', class_names='', clickable=True):
        if ar is None:
            return format_html("<em>{}</em>", self)
        ctx = dict(src=mf.get_download_url())
        ...
        if image_format is None:
            image_format = ImageFormats.default
