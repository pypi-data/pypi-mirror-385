...
def test_ipc_tilde_in_path():
    expected_path = str(pathlib.Path.home()) + (
        "\\foo" if sys.platform.startswith("win") else "/foo"
    )
...
def test_get_endpoint_uri_or_ipc_path_returns_ipc_path():
    ...
    expected = (
        "D:\\path\\to\\file" if sys.platform.startswith("win") else "/path/to/file"
    )
    assert provider.get_endpoint_uri_or_ipc_path() == expected == provider.ipc_path
